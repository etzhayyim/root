    ...
    match tline_type {
        TLineType::Microstrip { width, height, er } => {
            ...
            let z0 = (87.0 / (er + 1.41).sqrt())
                * (5.98 * height / (0.8 * width + t)).ln();
            let eff_er = (er + 1.0) / 2.0 + (er - 1.0) / (2.0 * (1.0 + 12.0 * height / width).sqrt());
            let delay = eff_er.sqrt() / C_MM_PER_PS;
            let loss = 0.001 * (1.0 + 1.0 / z0); // simplified conductor + dielectric loss
            TLineParams { z0_ohm: z0, delay_ps_per_mm: delay, loss_db_per_mm: loss, length_mm }
        }
        TLineType::Stripline { width, height1, height2, er } => {
            let b = height1 + height2;
            let t = TRACE_THICKNESS_MM;
            let z0 = (60.0 / er.sqrt()) * (4.0 * b / (0.67 * std::f64::consts::PI * (0.8 * width + t))).ln();
            let delay = er.sqrt() / C_MM_PER_PS;
            let loss = 0.0008 * (1.0 + 1.0 / z0);
            TLineParams { z0_ohm: z0, delay_ps_per_mm: delay, loss_db_per_mm: loss, length_mm }
        }
        TLineType::Coplanar { width, gap, height, er } => {
...
mod tests {
    ...
    fn microstrip_z0_near_50_ohm() {
        ...
        let tline = TLineType::Microstrip { width: 0.2, height: 0.2, er: 4.3 };
        let params = calculate_z0(&tline, 25.0);
        assert!(params.z0_ohm > 30.0 && params.z0_ohm < 80.0,
            "Z0 should be in reasonable range, got {}", params.z0_ohm);
        assert_eq!(params.length_mm, 25.0);
    }

    #[test]
    fn stripline_z0_positive() {
        let tline = TLineType::Stripline { width: 0.15, height1: 0.2, height2: 0.2, er: 4.3 };
