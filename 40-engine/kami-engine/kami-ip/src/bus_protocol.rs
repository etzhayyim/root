...
pub fn generate_axi4_master(config: &AxiConfig) -> String {
    ...
    // Read data
    v.push_str(&format!("  input  wire [{w}:0] RDATA,\n", w = config.data_width - 1));
    v.push_str("  input  wire [1:0]  RRESP,\n");
    v.push_str("  input  wire        RLAST,\n");
    v.push_str(&format!("  input  wire [{w}:0] RID,\n", w = config.id_width - 1));
...
pub fn generate_axi4_slave(config: &AxiConfig) -> String {
    let strb_width = config.data_width / 8;
    let mut v = String::with_capacity(2048);
    v.push_str(&format!("// AXI4 Slave — addr={}, data={}, id={}\n", config.addr_width, config.data_width, config.id_width));
    v.push_str("module axi4_slave (\n");
    v.push_str("  input  wire        ACLK,\n");
    v.push_str("  input  wire        ARESETn,\n");
    // Write address (inputs for slave)
    v.push_str(&format!("  input  wire [{w}:0] AWADDR,\n", w = config.addr_width - 1));
    v.push_str(&format!("  input  wire [{w}:0] AWID,\n", w = config.id_width - 1));
    ...
    // Write data
    v.push_str(&format!("  input  wire [{w}:0] WDATA,\n", w = config.data_width - 1));
    v.push_str(&format!("  input  wire [{w}:0] WSTRB,\n", w = strb_width - 1));
    v.push_str("  input  wire        WLAST,\n");
    v.push_str("  input  wire        WVALID,\n");
    v.push_str("  output wire        WREADY,\n");
    // Write response
    v.push_str("  output wire [1:0]  BRESP,\n");
    v.push_str(&format!("  output wire [{w}:0] BID,\n", w = config.id_width - 1));
    v.push_str("  output wire        BVALID,\n");
    v.push_str("  input  wire        BREADY,\n");
    // Read address
    v.push_str(&format!("  input  wire [{w}:0] ARADDR,\n", w = config.addr_width - 1));
    v.push_str(&format!("  input  wire [{w}:0] ARID,\n", w = config.id_width - 1));
    ...
    // Read data
    v.push_str(&format!("  output wire [{w}:0] RDATA,\n", w = config.data_width - 1));
    v.push_str("  output wire [1:0]  RRESP,\n");
    v.push_str("  output wire        RLAST,\n");
    v.push_str(&format!("  output wire [{w}:0] RID,\n", w = config.id_width - 1));
...
mod tests {
    ...
    fn axi4_signal_count() {
        ...
        assert!(AXI4_SIGNALS.len() >= 35, "AXI4 should have 35+ signals, got {}", AXI4_SIGNALS.len());
    }

    #[test]
    fn axi4_master_contains_signals() {
        let config = AxiConfig { addr_width: 32, data_width: 64, id_width: 4, user_width: 0 };
