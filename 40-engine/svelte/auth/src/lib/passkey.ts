...
export async function completeAuth(did: string, handle: string, oauth?: OAuthParams | null): Promise<void> {
	...
	let session: any;
	try {
		session = await authRpc('/xrpc/ai.gftd.auth.issueSession', { did, handle });
	} catch {
...
export async function sendOtp(phone: string): Promise<void> {
	await authRpc('/xrpc/ai.gftd.auth.smsOtpSend', { phone });
}

export async function verifyOtp(phone: string, code: string): Promise<{ did: string }> {
	return authRpc('/xrpc/ai.gftd.auth.smsOtpVerify', { phone, code });
}
...
export async function passkeyAuth(): Promise<PasskeyResult> {
	const beginResp = await authRpc('/xrpc/ai.gftd.auth.passkeyBeginAuth');
	...
	try {
		verifyResult = await authRpc('/xrpc/ai.gftd.auth.passkeyVerifyAuth', {
		...
		const isCredentialNotFound =
			rpcError?.path === '/xrpc/ai.gftd.auth.passkeyVerifyAuth' &&
			rpcError?.status === 404 &&
...
export async function passkeyRegister(): Promise<PasskeyResult> {
	...
	const credentialId = base64urlEncode(credential.rawId);
	const verifyResult = await authRpc('/xrpc/ai.gftd.auth.passkeyVerifyRegister', {
		challenge: beginResp.challenge,
		clientDataJson: base64urlEncode(response.clientDataJSON),
