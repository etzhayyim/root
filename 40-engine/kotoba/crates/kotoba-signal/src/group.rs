...
mod tests {
    ...
    fn max_skipped_keys_constant_is_positive_and_bounded() {
        const _: () = assert!(MAX_SKIPPED_KEYS >= 100,  "limit must allow normal out-of-order delivery");
        const _: () = assert!(MAX_SKIPPED_KEYS <= 5_000, "limit must cap memory to a sane ceiling");
    }
