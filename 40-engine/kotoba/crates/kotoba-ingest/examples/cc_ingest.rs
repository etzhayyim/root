...
fn parse_args() -> (PathBuf, String, Option<usize>) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--max-files" => { i += 1; max_files = args[i].parse().ok(); }
            _ => {}
        }
        i += 1;
    }
...
async fn main() -> Result<()> {
    ...
    let embed_client: Arc<dyn EmbedClient> = match HttpEmbedClient::from_env() {
        Ok(c) => {
            tracing::info!(model = c.model_id(), dim = c.dim(), "Using HTTP embed client");
            Arc::new(c)
        }
        Err(_) => {
    ...
    if mode == "pages" || mode == "both" {
        ...
        tracing::info!(files, quads, elapsed_s = start.elapsed().as_secs_f32(),
    ...
    if mode == "chunks" || mode == "both" {
        ...
        tracing::info!(chunks, embeddings, elapsed_s = start.elapsed().as_secs_f32(),
