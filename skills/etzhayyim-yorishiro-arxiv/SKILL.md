...
description: Drive the arxiv yorishiro (kami: arxiv.org) via MCP tools, XRPC, or in-process kotodama actor calls.
...
1. **Lexicon** at `00-contracts/lexicons/ai/etzhayyim/yorishiro/arxiv/*.json` (XRPC + kotodama-host-sdk consumers)
2. **Pregel cell** at `40-engine/kotoba/crates/kotoba-kotodama/cells/yorishiro_arxiv/cell.py` (in-cluster Murakumo runtime)
3. **MCP server** at `40-engine/kotoba/crates/kotoba-kotodama/mcp/yorishiro-arxiv-mcp/` (stdio + Streamable HTTP)
