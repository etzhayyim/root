...
description: Drive the example-portal yorishiro (kami: browser:example-portal) via MCP tools, XRPC, or in-process magatama actor calls.
...
1. **Lexicon** at `00-contracts/lexicons/ai/etzhayyim/yorishiro/example-portal/*.json` (XRPC + magatama-host-sdk consumers)
2. **Pregel cell** at `20-actors/magatama/cells/yorishiro_example-portal/cell.py` (in-cluster Murakumo runtime)
3. **MCP server** at `20-actors/magatama/mcp/yorishiro-example-portal-mcp/` (stdio + Streamable HTTP)
