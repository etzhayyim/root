...
description: Drive the argparse-demo yorishiro (kami: bin:argparse-demo) via MCP tools, XRPC, or in-process magatama actor calls.
...
1. **Lexicon** at `00-contracts/lexicons/ai/etzhayyim/yorishiro/argparse-demo/*.json` (XRPC + magatama-host-sdk consumers)
2. **Pregel cell** at `20-actors/magatama/cells/yorishiro_argparse-demo/cell.py` (in-cluster Murakumo runtime)
3. **MCP server** at `20-actors/magatama/mcp/yorishiro-argparse-demo-mcp/` (stdio + Streamable HTTP)
