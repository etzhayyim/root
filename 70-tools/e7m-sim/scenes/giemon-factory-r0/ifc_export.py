#!/usr/bin/env python3
"""giemon-factory-r0 — IFC4 (ISO-10303-21 / STEP) export.
...
def export_ifc(scene, parts, path):
    ...
    org = f.add("IFCORGANIZATION($,'etzhayyim','Giemon factory R0 (kami-engine)',$,$)")
    person = f.add("IFCPERSON($,'kami-app-giemon-factory',$,$,$,$,$,$)")
    pando = f.add(f"IFCPERSONANDORGANIZATION(#{person},#{org},$)")
    app = f.add(f"IFCAPPLICATION(#{org},'R0','kotoba-giemon-factory','giemon-factory')")
    ...
    proj = f.add(f"IFCPROJECT('{ifc_guid('project')}',#{owner},'giemon-factory-r0',"
                 f"'Giemon robot manufacturing plant R0',$,$,$,(#{ctx}),#{units})")
        ...
        if part:
            ...
            if props:
                ps = f.add(f"IFCPROPERTYSET('{ifc_guid('ps-' + eid)}',#{owner},"
                           f"'Pset_GiemonProcurement',$,{f.reflist(props)})")
    ...
    header = (
        ...
        "'kotoba-giemon-factory','kami-app-giemon-factory','');\n"
        "FILE_SCHEMA(('IFC4'));\n"
        "ENDSEC;\n"
        "DATA;\n"
    )
