#!/usr/bin/env python3
"""giemon-factory-r0 — engineering passes (a step up from "representative boxes").
...
_EQUIP_KW = {
    "bld:F1-cnc": 15.0, "bld:F2-lathe": 11.0, "bld:F3-cmm": 2.0,
    "bld:F5-conveyor": 3.7, "bld:F7-packing": 2.0, "bld:F8-cell": 0.5,
    "bld:E1-lighting": 0.15, "bld:E3-air": 15.0, "bld:E4-hvac": 20.0,
    "bld:G10-wiringdev": 0.0, "bld:H3-pump": 2.2, "bld:H5-heater": 5.0,
}
_DEMAND_FACTOR = 0.65
_SYS_V = 415.0   # 3φ line voltage
_PF = 0.85
_CV150_R = 0.124  # Ω/km, 150sq Cu
_CV150_X = 0.099  # Ω/km
_VD_LIMIT_PCT = 3.0
...
def size_electrical(parts):
    ...
    # feeder length from the CV cable line item
    length = 220.0
    for p in parts:
        if p["part/id"] == "bld:G6-cvcable":
            length = float(p.get("part/length-m", 220) or 220)
    vd = math.sqrt(3) * current * (_CV150_R * _PF + _CV150_X * math.sin(math.acos(_PF))) * (length / 1000.0)
    vd_pct = vd / _SYS_V * 100.0
    # cubicle / transformer capacity from rating
    tx_kva = 300.0
    cubicle_ok = demand_kva <= tx_kva
    return {
        "discipline": "電気 (低圧幹線)",
        ...
        "checks": [
            f"需要 {round(demand_kva,1)}kVA <= 変圧器 {tx_kva}kVA: {'OK' if cubicle_ok else 'NG'}",
...
def _entities(of, clashes, sizings, code, build):
    ...
    for c in clashes:
        ents.append({
            "id": c["id"], "type": "GiemonFactoryClash", "labelEn": c["note"],
            "claims": [
    ...
    for s in sizings:
        ...
        ents.append({"id": f"sizing:{s['discipline']}", "type": "GiemonFactorySizing",
                     "labelEn": s["discipline"], "claims": claims})
    for r in code["rows"]:
        ents.append({
            "id": f"code:{r['rule']}", "type": "GiemonFactoryCodeCheck", "labelEn": r["rule"],
            "claims": [
    ...
    for b in build:
        ents.append({
            "id": f"build:{b['step']}", "type": "GiemonFactoryBuildability", "labelEn": b["name"],
            "claims": [
...
def run_engineering(scene, parts, meta, steps=None, robots=None):
    of = meta.get("bom/of", "giemon-factory-r0")
