#!/usr/bin/env python3
"""giemon-factory-r0 — generate SBOM + kotoba ingest + 4D order from the EDN SSoT.
...
giemon-kabitori `sbom_gen.py`) and emits:
  ...
  - construction.order.json — flat ordered step list (seq / reveals / duration)
                              consumed by kami-app-giemon-factory's 4D build
                              viewer (run_giemon_factory_build_v1).
...
def part_entities(meta, parts):
    ...
    bom_of = meta.get("bom/of", "giemon-factory-r0")
    out = []
    for p in parts:
...
def to_cyclonedx(meta, parts):
    ...
    for p in parts:
        ...
        props += [{"name": f"giemon:{_claim_pred(k).replace('part/', '')}", "value": str(v)}
    ...
    return {
        ...
        "metadata": {
            "component": {
                "type": "device",
                "name": meta.get("bom/title", "Giemon factory"),
                "version": meta.get("bom/revision", "v1"),
            },
            "properties": [
                {"name": "giemon:bomOf", "value": meta.get("bom/of", "giemon-factory-r0")},
...
def step_entities(meta, steps):
    proc_of = meta.get("proc/of", "giemon-factory-r0")
    out = []
    for s in steps:
...
def order_json(meta, steps):
    ...
    return {"of": meta.get("proc/of", "giemon-factory-r0"), "steps": rows}
...
def robot_entities(robots, of):
    ...
    for r in robots:
        ...
        out.append({"id": r["robot/id"], "type": "GiemonConstructionRobot",
...
# ── render-element id universe (must match kami-app-giemon-factory scene.rs) ────
...
def main():
    ...
    (sdir / "process.json").write_text(
        json.dumps({"of": b_meta.get("bom/of", "giemon-factory-r0"), "ops": proc["ops"]},
    ...
    ingest = {"entities": part_entities(b_meta, parts)
              ...
              + robot_entities(robots, b_meta.get("bom/of", "giemon-factory-r0"))
