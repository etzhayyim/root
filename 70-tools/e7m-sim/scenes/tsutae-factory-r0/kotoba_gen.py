#!/usr/bin/env python3
"""tsutae-factory-r0 — generate SBOM + kotoba ingest + 4D order from the EDN SSoT.
...
giemon-kabitori `sbom_gen.py`) and emits:
  ...
  - construction.order.json — flat ordered step list (seq / reveals / duration)
                              consumed by kami-app-tsutae-factory's 4D build
                              viewer (run_tsutae_factory_build_v1).
...
def part_entities(meta, parts):
    ...
    bom_of = meta.get("bom/of", "tsutae-factory-r0")
    out = []
    for p in parts:
...
def to_cyclonedx(meta, parts):
    ...
    return {
        ...
        "metadata": {
            ...
            "properties": [
                {"name": "sarutahiko:bomOf", "value": meta.get("bom/of", "tsutae-factory-r0")},
...
def step_entities(meta, steps):
    proc_of = meta.get("proc/of", "tsutae-factory-r0")
    out = []
    for s in steps:
...
def order_json(meta, steps):
    ...
    return {"of": meta.get("proc/of", "tsutae-factory-r0"), "steps": rows}
...
# ── render-element id universe (must match kami-app-tsutae-factory scene.rs) ────
...
def main():
    ...
    (sdir / "process.json").write_text(
        json.dumps({"of": b_meta.get("bom/of", "tsutae-factory-r0"), "ops": proc["ops"]},
    ...
    ingest = {"entities": part_entities(b_meta, parts)
              ...
              + robot_entities(robots, b_meta.get("bom/of", "tsutae-factory-r0"))
