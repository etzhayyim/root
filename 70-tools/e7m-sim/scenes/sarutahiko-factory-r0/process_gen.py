#!/usr/bin/env python3
"""sarutahiko-factory-r0 — robotics process expansion (建築手順 → ロボット操作列).
...
def run_process(steps, parts, robots):
    ...
    for o in all_ops:
        claims = [
            {"pred": "op/of", "value": "sarutahiko-factory-r0"},
        ...
        ents.append({"id": o["id"], "type": "SarutahikoRobotOp", "labelEn": o["label"], "claims": claims})
...
def main():
    ...
    (here / "process.json").write_text(
        json.dumps({"of": "sarutahiko-factory-r0", "ops": res["ops"]}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8")
