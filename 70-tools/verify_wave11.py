...
def write_report():
    """Emit a MATURITY.md scorecard for the cohort (runs bb tests for honest green/red)."""
    rows = []
    tot_ent = tot_ep = tot_green = 0
    for h in HANDLES:
        n, errs = check(h)
        ok, _ = run_bb_test(h)
        man = json.load(open(os.path.join(ACTORS_DIR, f"{h}-compat", "manifest.json")))
        cat = clj.COHORT[h][1]
        rows.append((h, n, 5 * n, man["tier"], "✅" if (ok and not errs) else "❌", cat))
        tot_ent += n; tot_ep += 5 * n; tot_green += 1 if (ok and not errs) else 0
    lines = [
        ...
        "",
        f"**{len(HANDLES)} actors · {tot_ent} entities · {tot_ep} endpoints · "
        f"{tot_green}/{len(HANDLES)} green · all L4.**",
        "",
    ...
    for h, n, ep, tier, tests, cat in rows:
        lines.append(f"| `{h}` | {n} | {ep} | {tier} | {tests} | {cat} |")
    lines += [
        "",
        "## Per-actor surface",
        "Each actor: `DatomPort` DI seam (kotoba-kqe prod adapter / `in-memory-datom` test"
        " fake) · Datomic-shaped tx-data · CRUD + filtering + cursor pagination + relation"
        " expansion · runnable `dispatch` · 8 tests / 64 assertions on `bb`.",
        "",
        "## Verify",
        "```",
    ...
    out = os.path.join(ROOT, "70-tools", "WAVE11-MATURITY.md")
    with open(out, "w") as f:
        f.write("\n".join(lines))
    return out, tot_green, len(HANDLES)
...
def main():
    ...
    if do_report:
        out, green, total = write_report()
        print(f"REPORT WRITTEN: {green}/{total} green → {os.path.relpath(out, ROOT)}")
