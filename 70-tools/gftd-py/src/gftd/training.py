...
def training_list(pds: str | None, json_out: bool, filter_status: str) -> None:
    ...
    try:
        resp = httpx.get(f"{pds_url}/xrpc/ai.gftd.training.listJobs",
...
def training_get(job_id: str, pds: str | None, json_out: bool) -> None:
    ...
    pds_url = (pds or resolve_pds()).rstrip("/")
    try:
        resp = httpx.get(f"{pds_url}/xrpc/ai.gftd.training.getJob",
...
_TRAINING_NSID: dict[str, str] = {
    "sft": "ai.gftd.apps.training.runSft",
    "lora": "ai.gftd.apps.training.runLora",
    "distill": "ai.gftd.apps.training.runDistill",
}
...
def training_run(kind: str, base_model: str, student_base: str, dataset: str,
    ...
    """Start a training run (sft | lora | distill) via ai.gftd.apps.training.*."""
    if not dataset:
...
def training_start(job_type: str, model: str, dataset: str, pds: str | None, json_out: bool) -> None:
    ...
    try:
        resp = httpx.post(f"{pds_url}/xrpc/ai.gftd.training.startJob",
...
def training_cancel(job_id: str, pds: str | None) -> None:
    ...
    pds_url = (pds or resolve_pds()).rstrip("/")
    try:
        resp = httpx.post(f"{pds_url}/xrpc/ai.gftd.training.cancelJob",
...
def training_promote(checkpoint_id: str, alias: str, target: str, by: str,
    ...
    if rationale:
        payload["rationale"] = rationale
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.promote", payload, json_out)
...
def training_eval(checkpoint_id: str, bench: str, eval_dataset: str,
    ...
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.runEval", payload, json_out)
...
def training_list_runs(kind: str, filter_status: str, limit: int,
    ...
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.listRuns", payload, json_out)
...
def training_list_checkpoints(run: str, only_final: bool, limit: int,
    ...
    if run:
        payload["runId"] = run
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.listCheckpoints", payload, json_out)
...
def training_list_snapshots(dataset: str, filter_status: str, limit: int,
    ...
    if filter_status:
        payload["status"] = filter_status
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.listSnapshots", payload, json_out)
...
def training_coverage(pds: str | None, json_out: bool) -> None:
    ...
    pds_url = (pds or resolve_pds()).rstrip("/")
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.coverage", {}, json_out)
...
def training_serving(alias: str, pds: str | None, json_out: bool) -> None:
    ...
    if alias:
        payload["alias"] = alias
    _post_training_xrpc(pds_url, "ai.gftd.apps.training.serving", payload, json_out)
