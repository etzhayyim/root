...
def _dodaf_seed_tv1(now: str) -> list[dict]:
    return [
        {"id": "cf-wasm-no-dynamic-compile", "view": "TV-1", "title": "CF Workers: WebAssembly.compile() blocked at runtime", "standard_ref": "Cloudflare Workers V8 embedder policy", "rule": "WebAssembly.compile(bytes) called at request time returns CompileError: 'Wasm code generation disallowed by embedder'. Only static WASM imports via CompiledWasm wrangler rule are supported.", "severity": "critical", "permitted": False, "scope_folders": ["60-apps/", "_archive/30-graph/kagami-live-260414/wasm/"], "scope_tags": ["cloudflare", "wasm", "assemblyscript"], "scope_exts": [".ts", ".wasm", ".jsonc"], "evidence": "h0g3t3st.etzhayyim.com/xrpc/app.etzhayyim.apps.hoge.wasmEval — validated 2026-04-08", "status": "[PRODUCTION]", "source": "60-apps/ai-gftd-project-hoge/appview/src/index.ts", "alternative": "", "created_at": now},
        {"id": "cf-wasm-static-import-ok", "view": "TV-1", "title": "CF Workers: static WASM import via CompiledWasm rule works", "standard_ref": "Cloudflare Workers CompiledWasm rule", "rule": "Static WASM import with CompiledWasm wrangler rule works correctly. new WebAssembly.Instance(MODULE, imports) per-request instantiation is ~0ms.", "severity": "info", "permitted": True, "scope_folders": ["60-apps/", "_archive/30-graph/kagami-live-260414/wasm/"], "scope_tags": ["cloudflare", "wasm", "assemblyscript"], "scope_exts": [".ts", ".jsonc"], "evidence": "h0g3t3st.etzhayyim.com/xrpc/app.etzhayyim.apps.hoge.wasmTest — validated 2026-04-08", "status": "[PRODUCTION]", "source": "60-apps/ai-gftd-project-hoge/appview/src/index.ts", "alternative": "", "created_at": now},
...
def dodaf_seed(workspace_dir: str | None, pds_url: str, dry_run: bool) -> None:
    ...
    for row in rows:
        record = {
            "$type": "app.etzhayyim.dodaf.tv1Standard",
            "dodaf_id": row.get("id"),
            "title": row.get("title"),
            "rule": row.get("rule"),
        ...
        payload = json.dumps({
            "repo": "",
            "collection": "app.etzhayyim.dodaf.tv1Standard",
            "record": record,
        }).encode()
        try:
