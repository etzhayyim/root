...
def identity_migrate_paths(
    ...
    """Migrate path DIDs (legacy-nanoids) to did:gftd via PDS XRPC ai.gftd.identity.submitOp."""
    import subprocess
    import hashlib
    import base64
    ...
    for entry in entries:
        ...
        try:
            resp = httpx.post(
                f"{pds_url}/xrpc/ai.gftd.identity.submitOp",
...
def identity_audit(pds: str | None, json_out: bool) -> None:
    ...
    try:
        resp = httpx.get(
            f"{pds_url}/xrpc/ai.gftd.identity.auditIdentities",
