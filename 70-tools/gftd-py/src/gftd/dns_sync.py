...
Records managed (comment prefix gftd:adr-0013:):
...
Only records matching the gftd-managed comment prefix are affected; manual records
...
_DNS_COMMENT_PREFIX = "gftd:adr-0013:"
...
def _emit_routing_map_ts(legacies: list[_IdLegacy]) -> str:
    ...
    lines = [
        ...
        "//\n",
        "// Auto-generated by `gftd dns-sync --emit-routing-map`. DO NOT EDIT BY HAND.\n",
...
def _patch_wrangler_bindings(path: Path, actors: list[_IdActor]) -> tuple[str, int]:
    ...
    parts = [
        '"services": [\n',
        '    { "binding": "PDS_WORKER",    "service": "ai-gftd-pds-2603241700" },\n',
        '    { "binding": "PLC_DIRECTORY", "service": "ai-gftd-plc-directory" }',
    ]
    emitted = 2
...
@click.option(
    "--yoro-mirror-path",
    default="60-apps/ai-gftd-project-yoro/appview/yoro-ui-g00h5zto/svelte/src/lib/server/legacy-nanoid-map.ts",
...
def dns_sync(
    ...
    if no_cf:
        ...
        if json_out:
            ...
            click.echo("gftd dns-sync — offline mode (no Cloudflare API)")
            click.echo("================================================")
    ...
    click.echo("gftd dns-sync — ADR-0013 Phase 3")
    click.echo("=================================")
