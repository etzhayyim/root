...
DB_URL = "postgres://root@<vendor-rw-host>:4566/dev"
...
def statute_row(law: dict[str, Any], articles: list[dict[str, Any]]) -> tuple[Any, ...]:
    ...
    current = now_iso()
    vertex_id = f"at://{JPN_PATH_DID}/ai.gftd.apps.houbun.statute/{law_id}"
    return (
        vertex_id,
        current[:10],
        JPN_PATH_DID,
...
def article_rows(law: dict[str, Any], statute_ref: str, articles: list[dict[str, Any]]) -> tuple[list[tuple[Any, ...]], list[tuple[Any, ...]]]:
    ...
    for idx, article in enumerate(articles):
        ...
        vertex_id = f"at://{article_did}/ai.gftd.apps.houbun.article/{h}"
        rows.append(
            (
                vertex_id,
                current[:10],
...
def insert_social_contracts(conn: Any) -> int:
    ...
    for path in sorted(base.glob("*.jsonld")):
        ...
        rows.append(
            (
                f"at://{CONTRACTS_DID}/ai.gftd.apps.contracts.socialContract/{rkey}",
                current[:10],
                CONTRACTS_DID,
                rkey,
                doc.get("name"),
...
def insert_wikidata_untc_treaties(conn: Any, *, limit: int, offset: int) -> dict[str, int]:
    ...
    for b in bindings:
        ...
        treaty_rows.append(
            (
                f"at://{ACTOR_DID}:int:un-treaty/ai.gftd.apps.houbun.treaty/{rkey}",
                current[:10],
        ...
        social_rows.append(
            (
                f"at://{CONTRACTS_DID}/ai.gftd.apps.contracts.socialContract/{srkey}",
                current[:10],
                CONTRACTS_DID,
                srkey,
                title,
...
def insert_cfr_title(conn: Any, *, year: int, title: int, max_sections: int) -> dict[str, int]:
    ...
    for link in xml_links:
        try:
            ...
            statute_ref = f"at://{path_did}/ai.gftd.apps.houbun.statute/{package}"
            srow = (
                statute_ref,
                current[:10],
                path_did,
...
def insert_eurlex_celex(conn: Any, *, celex: str, max_articles: int) -> dict[str, int]:
    ...
    statute_ref = f"at://{path_did}/ai.gftd.apps.houbun.statute/{celex}"
    srow = (
        statute_ref,
        current[:10],
        path_did,
