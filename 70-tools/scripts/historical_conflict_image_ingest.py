...
def collect(limit_conflicts: int, limit_treaties: int, offset: int) -> dict[str, Any]:
    ...
    for row in sparql(conflict_query(limit_conflicts, offset)):
        ...
        conflict_vid = f"at://{conflict_actor.did}/ai.gftd.apps.history.conflict/{slug(qid, 64)}"
        ...
        if treaty_qid:
            ...
            treaty_vid = f"at://{treaty_actor.did}/ai.gftd.apps.houbun.treaty/{slug(treaty_qid, 64)}"
        ...
        if image:
            ...
            image_vid = f"at://{ACTOR_DID}/ai.gftd.apps.history.sourceImage/{image_id}"
            images[image_vid] = (
                image_vid,
                int(time.time() * 1000),
                today(),
    ...
    for row in sparql(treaty_actor_query(limit_treaties, offset)):
        ...
        if participant_qid:
            ...
            treaty_vid = f"at://{treaty_actor.did}/ai.gftd.apps.houbun.treaty/{slug(qid, 64)}"
...
def image_artifact_rows(image_rows: list[tuple[Any, ...]]) -> list[tuple[Any, ...]]:
    ...
    for row in image_rows:
        ...
        rows.append(
            (
                f"at://{ACTOR_DID}/ai.gftd.apps.ingest.artifact/{digest('image', uri, size=6)}",
                int(time.time() * 1000),
                today(),
                1,
                ACTOR_DID,
