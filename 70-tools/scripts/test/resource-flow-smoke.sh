...
code=$(call GET app.etzhayyim.apps.resourceFlow.getSankey 'flowClass=currency&domain=hospitality&limit=20')
assert "getSankey currency hospitality" "$code" "$(cat /tmp/rf-smoke.body)" "200"

# 2. getSankey — service
code=$(call GET app.etzhayyim.apps.resourceFlow.getSankey 'flowClass=service&domain=hospitality&limit=20')
assert "getSankey service hospitality" "$code" "$(cat /tmp/rf-smoke.body)" "200"

# 3. listFlows — currency
code=$(call GET app.etzhayyim.apps.resourceFlow.listFlows 'flowClass=currency&limit=10')
...
code=$(call GET app.etzhayyim.apps.resourceFlow.getActorLabels 'dids=did:web:yadoya.etzhayyim.com&dids=did:web:hospitality.etzhayyim.com:actor:chain:hilton')
...
code=$(call GET app.etzhayyim.apps.resourceFlow.listAnomalies 'limit=10&reviewed=open')
assert "listAnomalies reviewed=open" "$code" "$(cat /tmp/rf-smoke.body)" "200"

# 3c2. listAnomalies — closed queue (Phase 16')
code=$(call GET app.etzhayyim.apps.resourceFlow.listAnomalies 'limit=10&reviewed=closed')
assert "listAnomalies reviewed=closed" "$code" "$(cat /tmp/rf-smoke.body)" "200"

# 3c3. listAnomalies — severity filter
code=$(call GET app.etzhayyim.apps.resourceFlow.listAnomalies 'limit=10&reviewed=any&severity=high')
...
if [ -n "$BEARER" ]; then
  body='{"anomalyId":"at://did:web:resource-flow.etzhayyim.com/app.etzhayyim.apps.resourceFlow.anomaly/does-not-exist","action":"acknowledge","post":false}'
  code=$(call POST app.etzhayyim.apps.resourceFlow.reviewAnomaly '' "$body")
...
if [ -n "$BEARER" ]; then
  ...
  code=$(call POST app.etzhayyim.apps.resourceFlow.registerEmitter '' "$body")
  assert "registerEmitter yadoya" "$code" "$(cat /tmp/rf-smoke.body)" "200"

  # 5. projectFlow — synthetic record
  body='{"flowClass":"currency","recordUri":"at://did:web:yadoya.etzhayyim.com/app.etzhayyim.apps.resourceFlow.legalEntityCurrencyFlow/smoke-001","observedAt":"2026-04-28T18:00:00Z","record":{"sourceDid":"did:web:yadoya.etzhayyim.com","counterpartyDid":"did:web:hospitality.etzhayyim.com:actor:chain:hilton","fiscalPeriod":"2026-04","flowType":"revenue","amount":0,"amountBucket":"lt-60k","currency":"JPY","industryCode":"I5510","sourceUrl":"https://yadoya.etzhayyim.com/about","sourceLicense":"gftd-internal-aggregate-v1"}}'
  code=$(call POST app.etzhayyim.apps.resourceFlow.projectFlow '' "$body")
  ...
  code=$(call POST app.etzhayyim.apps.resourceFlow.detectAnomaly '' "$body")
