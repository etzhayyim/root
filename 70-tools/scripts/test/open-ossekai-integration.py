...
Env:
  ...
  GFTD_TOKEN             Service Auth JWT for CF Worker calls (from `gftd agent-token`)
...
READ_NSIDS = [
    "ai.gftd.apps.openOssekai.getOssekaiStatus",
    "ai.gftd.apps.openOssekai.listIntelBriefs",
    "ai.gftd.apps.openOssekai.listArbitrageOpportunities",
]
...
def test_generate_intel_brief() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.generateIntelBrief", method="POST",
...
def test_propose_arbitrage_brief() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.proposeArbitrageBrief", method="POST",
...
def test_request_consent() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.requestOssekaiConsent", method="POST",
...
def test_score_jocho() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.scoreJocho", method="POST",
                       body={"targetDid": TARGET_DID})
    assert status == 200, f"got {status}: {body!r}"
    result = (body or {}).get("result") or {}
    assert result.get("asyncStarted") is True, f"asyncStarted not true: {result!r}"
...
def test_generate_wellbecoming_plan() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.generateWellBecomingPlan", method="POST",
                       body={"targetDid": TARGET_DID})
    assert status == 200, f"got {status}: {body!r}"
    result = (body or {}).get("result") or {}
    assert result.get("asyncStarted") is True, f"asyncStarted not true: {result!r}"
...
def test_get_ossekai_status() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.getOssekaiStatus",
                       body={"targetDid": TARGET_DID})
    assert status == 200, f"got {status}: {body!r}"
    assert isinstance(body, dict), f"not a dict: {body!r}"
...
def test_list_intel_briefs() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.listIntelBriefs",
                       body={"limit": 5})
    assert status == 200, f"got {status}: {body!r}"
    assert isinstance(body, dict), f"not a dict: {body!r}"
...
def test_list_arbitrage_opportunities() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.listArbitrageOpportunities",
                       body={"limit": 5})
    assert status == 200, f"got {status}: {body!r}"
    assert isinstance(body, dict), f"not a dict: {body!r}"
...
def test_unknown_nsid_worker_404() -> None:
    status, body = _cf("ai.gftd.apps.openOssekai.nonexistentMethod", method="POST", body={})
...
def test_unknown_nsid_dispatcher_404(skip: bool = False) -> None:
    if skip:
        return
    status, body = _disp("ai.gftd.apps.openOssekai.nonexistentMethod", {})
    assert status == 404, f"expected 404, got {status}: {body!r}"
...
def test_rw_all_5_bindings_active() -> None:
    ...
    count = _rw_count(
        ...
        "WHERE nsid LIKE 'ai.gftd.apps.openOssekai.%' AND status='active'"
    )
