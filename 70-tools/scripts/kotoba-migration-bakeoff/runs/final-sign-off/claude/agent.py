...
Port of the original FinalSignOffCell onto the WASM-native `kotoba_langgraph`
API so it compiles to a kotoba-node component.

Build:
    bash ../../../../../../40-engine/kotoba/scripts/build-pywasm.sh agent.py agent.wasm
"""
...
def _validate_inspections(state: dict[str, Any]) -> dict[str, Any]:
    return {"signoff_state": {**state.get("signoff_state", {}), "phase": "inspections_validated", "completionPct": 40}, "next_node": "request"}


def _request_authority_signature(state: dict[str, Any]) -> dict[str, Any]:
...
def _emit_occupancy_clearance(state: dict[str, Any]) -> dict[str, Any]:
