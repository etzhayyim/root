...
"""
Validate every `60-apps/<app>/magatama.jsonld` against
`90-docs/_registry/schemas/magatama.schema.json`.
...
deferred follow-on; landed in cycle 53). Parallel pattern to:
  ...
  - **cycle 53: magatama manifest validation (axis 5; this script)**
...
Usage:
    70-tools/scripts/docs/validate-magatama-manifests.py
    70-tools/scripts/docs/validate-magatama-manifests.py --strict
        # exit 2 if jsonschema unavailable
    70-tools/scripts/docs/validate-magatama-manifests.py --json
...
APPS_GLOB = REPO / "60-apps" / "*" / "magatama.jsonld"
SCHEMA = REPO / "90-docs" / "_registry" / "schemas" / "magatama.schema.json"
...
def main() -> int:
    ...
    try:
        ...
        msg = (
            "validate-magatama-manifests: jsonschema package not installed.\n"
    ...
    if not manifests:
        msg = "validate-magatama-manifests: no manifests found under 60-apps/*/magatama.jsonld"
        if args.json:
    ...
    if args.json:
        ...
        print(
            f"magatama manifests: {clean}/{total} valid"
