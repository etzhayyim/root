"""Tests for validate-magatama-manifests.py (cycle 56).

The validator checks every 60-apps/<app>/magatama.jsonld against the
canonical magatama.schema.json. Tests cover:
...
_SCRIPT = Path(__file__).resolve().parent / "validate-magatama-manifests.py"
...
def test_imports_cleanly():
    spec = importlib.util.spec_from_file_location("validate_magatama_manifests", _SCRIPT)
...
def test_canonical_paths():
    ...
    assert "magatama.jsonld" in str(mod.APPS_GLOB)
    assert mod.SCHEMA.name == "magatama.schema.json"
...
def test_live_repo_baseline_42_42_valid():
    """Cycle 53 baseline: 42/42 magatama manifests clean. Regression guard."""
    try:
