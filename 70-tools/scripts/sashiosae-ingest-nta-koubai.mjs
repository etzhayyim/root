 ...
 *   vertex_page (ai.gftd.apps.site.page) に格納している。本 script は
 *   それら page を読み、public-auction 情報 (auctionId / scheduledAt /
 *   propertyType / estimatedValue / venueUrl) を抽出し、
 *   ai.gftd.apps.sashiosae.recordKankaResult XRPC を呼ぶ。
 *
 * Phase 1 scope: 国税庁 (did:web:jpn-state.etzhayyim.com:mof:nta:choushuu:kanka) のみ。
 * Phase 2 で 47 都道府県 + 指定都市の独自公売システムを追加。
 *
 * Auth: authority Service Auth JWT (lxm=ai.gftd.apps.sashiosae.recordKankaResult)
 *   env GFTD_AUTHORITY_JWT で渡す (60s TTL、gftd agent-token --lxm で取得)。
 *
 * Tier rule (ADR-0032): auctionId / scheduledAt / estimatedValue / venueUrl は
 *   Tier 1 public (政府公開情報)。落札者 / 正確落札額 は取得対象外 (Tier 3)。
 *
 * Usage:
 *   export GFTD_AUTHORITY_JWT=$(gftd agent-token --lxm ai.gftd.apps.sashiosae.recordKankaResult --did did:web:jpn-state.etzhayyim.com:mof:nta:choushuu:kanka)
...
const JWT = process.env.GFTD_AUTHORITY_JWT ?? "";
...
async function postKankaResult(payload) {
  ...
  if (!JWT) throw new Error("GFTD_AUTHORITY_JWT env required (gftd agent-token --lxm ai.gftd.apps.sashiosae.recordKankaResult)");
  const res = await fetch(`${XRPC_HOST}/xrpc/ai.gftd.apps.sashiosae.recordKankaResult`, {
    method: "POST",
    headers: {
