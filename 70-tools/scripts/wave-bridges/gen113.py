#!/usr/bin/env python3
"""Wave 113: N.Africa+E.Africa (DZ/TN/RW/UG/AO) + NATO partner + EU CSDP +
都市局 + SCO + HK sanctions. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateDzRelations",
     "vertex_open_us_state_dept", "bureau", "nearEastern",
     "action_kind", "dzDiplomacy", "action_id", "issued_at",
     "US State: アルジェリア (MAE DZ) 二国間 (LNG)"),
    ("open-us-state-dept", "usStateDept", "coordinateTnRelations",
     "vertex_open_us_state_dept", "bureau", "nearEastern",
     "action_kind", "tnDiplomacy", "action_id", "issued_at",
     "US State: チュニジア (MAE TN) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateRwRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "rwDiplomacy", "action_id", "issued_at",
     "US State: ルワンダ (MINAFFET RW) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateUgRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "ugDiplomacy", "action_id", "issued_at",
     "US State: ウガンダ (MOFA UG) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateAoRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "aoDiplomacy", "action_id", "issued_at",
     "US State: アンゴラ (MIREX AO) 二国間 (Lobito 回廊)"),
    ("open-jp-mofa", "jpMofa", "coordinateNatoPartnership",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "natoPartner", "action_id", "issued_at",
     "外務省: NATO Global Partner (IP4: 日豪韓NZ)"),
    ("open-jp-mofa", "jpMofa", "coordinateEuCsdp",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "euCsdpFramework", "action_id", "issued_at",
     "外務省: EU CSDP (Common Security and Defence Policy) 連携"),
    ("open-jp-mlit", "jpMlit", "regulateCityPlanning",
     "vertex_open_jp_mlit", "bureau", "city_planning",
     "action_kind", "cityPlanningOversight", "action_id", "issued_at",
     "国土交通省: 都市局 (都市計画 / 立地適正化計画)"),
    ("open-cn-mofa", "cnMofa", "coordinateSco",
     "vertex_open_cn_mofa", "department_kind", "treaty",
     "action_kind", "scoCooperation", "action_id", "issued_at",
     "中国外交部: SCO (上海協力機構) 首脳・外相会議"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionHkEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "hkSanction", "action_id", "issued_at",
     "US Treasury OFAC: 香港 (NSL related) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind113.sql", "w") as f:
