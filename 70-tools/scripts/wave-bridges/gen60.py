...
Bridges Wave 59:
- civil-liability ↔ csdddDirective.flagDueDiligenceGap
- fpic-consent ↔ soyMoratorium.flagMoratoriumBreach
- beneficial-ownership-registry ↔ debarmentList.flagPhoenixEntity
- export-credit-agency ↔ sovereignGuarantee.flagCallEvent
- shadow-fleet-insurance ↔ priceCapCoalition.flagCapBreach
"""
import json
...
def gen_lexicon(actor, method):
    nsid = f"ai.gftd.apps.{actor['app']}.{method['name']}"
    props={}; required=[]
...
for i, a in enumerate(ACTORS, start=1):
    bd=REPO/f"00-contracts/bpmn/ai/gftd/open-{a['slug']}"
    ld=REPO/f"00-contracts/lexicons/ai/gftd/apps/{a['app']}"
    bd.mkdir(parents=True,exist_ok=True); ld.mkdir(parents=True,exist_ok=True)
    for m in a["methods"]:
        (ld/f"{m['name']}.json").write_text(json.dumps(gen_lexicon(a,m),indent=2,ensure_ascii=False))
        (bd/f"{m['name']}.bpmn").write_text(gen_bpmn(a,m))
    ddl = gen_ddl(a)
    out = Path(f"/tmp/wave13/w60_{i:02d}.sql")
    out.write_text(ddl)
    print(f"wrote {out}")
