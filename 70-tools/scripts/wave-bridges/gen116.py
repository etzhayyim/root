#!/usr/bin/env python3
"""Wave 116: South Asia (LK/BD/NP/MV/BT) + 議連 + 高校無償化 + 子ども家庭
+ BF/NE Sahel junta sanctions. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateLkRelations",
     "vertex_open_us_state_dept", "bureau", "southCentralAsian",
     "action_kind", "lkDiplomacy", "action_id", "issued_at",
     "US State: スリランカ (MFA LK) 二国間 (IMF / debt)"),
    ("open-us-state-dept", "usStateDept", "coordinateBdRelations",
     "vertex_open_us_state_dept", "bureau", "southCentralAsian",
     "action_kind", "bdDiplomacy", "action_id", "issued_at",
     "US State: バングラデシュ (MFA BD) 二国間 (RMG textile)"),
    ("open-us-state-dept", "usStateDept", "coordinateNpRelations",
     "vertex_open_us_state_dept", "bureau", "southCentralAsian",
     "action_kind", "npDiplomacy", "action_id", "issued_at",
     "US State: ネパール (MoFA NP) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateMvRelations",
     "vertex_open_us_state_dept", "bureau", "southCentralAsian",
     "action_kind", "mvDiplomacy", "action_id", "issued_at",
     "US State: モルディブ (MoFA MV) 二国間 (India-First / China)"),
    ("open-us-state-dept", "usStateDept", "coordinateBtRelations",
     "vertex_open_us_state_dept", "bureau", "southCentralAsian",
     "action_kind", "btDiplomacy", "action_id", "issued_at",
     "US State: ブータン (MoFA BT) 二国間 (GNH / 中印境界)"),
    ("open-jp-mofa", "jpMofa", "coordinateInterParliament",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "interParliamentary", "action_id", "issued_at",
     "外務省: 議員連盟 (parliamentary friendship 議連)"),
    ("open-jp-mext", "jpMext", "fundHighSchool",
     "vertex_open_jp_mext", "bureau", "highered",
     "action_kind", "highSchoolFree", "action_id", "issued_at",
     "文科省: 高校無償化 (就学支援金)"),
    ("open-jp-mhlw", "jpMhlw", "supportChildFamily",
     "vertex_open_jp_mhlw", "bureau", "longTermCare",
     "action_kind", "childFamilySupport", "action_id", "issued_at",
     "厚労省: 子ども家庭支援 (児相 / 児童虐待 / 母子父子)"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionBfEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "bfSanction", "action_id", "issued_at",
     "US Treasury OFAC: ブルキナファソ (Junta) entity 制裁"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionNeEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "neSanction", "action_id", "issued_at",
     "US Treasury OFAC: ニジェール (CNSP Junta 2026) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind116.sql", "w") as f:
