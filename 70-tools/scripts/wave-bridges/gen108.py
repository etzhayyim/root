#!/usr/bin/env python3
"""Wave 108: Southern EU (PT/ES/IT/GR/CY) + arms control NPT/CWC + JP road bureau
+ research integrity + MM Myanmar sanction. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinatePtRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "ptDiplomacy", "action_id", "issued_at",
     "US State: ポルトガル (MNE PT) 二国間 (Lajes基地)"),
    ("open-us-state-dept", "usStateDept", "coordinateEsRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "esDiplomacy", "action_id", "issued_at",
     "US State: スペイン (MAEC ES) 二国間 (Rota基地)"),
    ("open-us-state-dept", "usStateDept", "coordinateItRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "itDiplomacy", "action_id", "issued_at",
     "US State: イタリア (MAECI IT) 二国間 (NATO 南欧flank)"),
    ("open-us-state-dept", "usStateDept", "coordinateGrRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "grDiplomacy", "action_id", "issued_at",
     "US State: ギリシャ (MFA GR) 二国間関係調整"),
    ("open-us-state-dept", "usStateDept", "coordinateCyRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "cyDiplomacy", "action_id", "issued_at",
     "US State: キプロス (MFA CY) 二国間関係調整"),
    ("open-jp-mofa", "jpMofa", "coordinateNpt",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "nptReview", "action_id", "issued_at",
     "外務省: NPT (核兵器不拡散条約) 運用検討会議"),
    ("open-jp-mofa", "jpMofa", "coordinateCwc",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "cwcOpcw", "action_id", "issued_at",
     "外務省: CWC / OPCW (化学兵器禁止条約)"),
    ("open-jp-mlit", "jpMlit", "regulateRoad",
     "vertex_open_jp_mlit", "bureau", "road",
     "action_kind", "roadBureauOversight", "action_id", "issued_at",
     "国土交通省: 道路局 (NEXCO 高速 / 国道) 監督"),
    ("open-jp-mext", "jpMext", "enforceResearchIntegrity",
     "vertex_open_jp_mext", "bureau", "highered",
     "action_kind", "researchIntegrity", "action_id", "issued_at",
     "文科省: 研究公正 (捏造・改ざん・盗用) 対応 / 不正調査"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionMmEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "mmSanction", "action_id", "issued_at",
     "US Treasury OFAC: ミャンマー (Tatmadaw) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind108.sql", "w") as f:
