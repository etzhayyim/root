#!/usr/bin/env python3
"""Wave 120: Africa rest (NA/BW/LS/SZ/CV) + OPCW/IAEA + 不動産 + 産業保安
+ Hamas leadership sanction. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateNaRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "naDiplomacy", "action_id", "issued_at",
     "US State: ナミビア (MIRCO NA) 二国間 (Uranium)"),
    ("open-us-state-dept", "usStateDept", "coordinateBwRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "bwDiplomacy", "action_id", "issued_at",
     "US State: ボツワナ (MIA BW) 二国間 (diamond)"),
    ("open-us-state-dept", "usStateDept", "coordinateLsRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "lsDiplomacy", "action_id", "issued_at",
     "US State: レソト (MFAIR LS) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateSzRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "szDiplomacy", "action_id", "issued_at",
     "US State: エスワティニ (MFA SZ) 二国間 (Taiwan recognition)"),
    ("open-us-state-dept", "usStateDept", "coordinateCvRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "cvDiplomacy", "action_id", "issued_at",
     "US State: カーボベルデ (MNEC CV) 二国間"),
    ("open-jp-mofa", "jpMofa", "coordinateOpcw",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "opcwHague", "action_id", "issued_at",
     "外務省: OPCW Hague 化学兵器禁止機関 (CWC実施)"),
    ("open-jp-mofa", "jpMofa", "coordinateIaea",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "iaeaSafeguards", "action_id", "issued_at",
     "外務省: IAEA 国際原子力機関 (保障措置 / 福島ALPS)"),
    ("open-jp-mlit", "jpMlit", "regulateRealEstate",
     "vertex_open_jp_mlit", "bureau", "city_planning",
     "action_kind", "realEstateOversight", "action_id", "issued_at",
     "国土交通省: 不動産取引 (宅建業 / 重要事項説明)"),
    ("open-jp-meti", "jpMeti", "regulateIndustrialSafety",
     "vertex_open_jp_meti", "bureau", "manufacturing",
     "action_kind", "industrialSafety", "action_id", "issued_at",
     "経産省: 産業保安 (高圧ガス / 火薬類 / 電力安全)"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionHamasLeadership",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "hamasLeadership", "action_id", "issued_at",
     "US Treasury OFAC: Hamas 指導部 (SDGT) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind120.sql", "w") as f:
