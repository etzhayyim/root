#!/usr/bin/env python3
"""Wave 112: West Africa (CI/GH) + LatAm (UY/PY/BO) + OECD/DAC + 自動車局
+ ZW/BY/NI sanctions. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateCiRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "ciDiplomacy", "action_id", "issued_at",
     "US State: コートジボワール (MAE CI) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateGhRelations",
     "vertex_open_us_state_dept", "bureau", "africa",
     "action_kind", "ghDiplomacy", "action_id", "issued_at",
     "US State: ガーナ (MFARI GH) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateUyRelations",
     "vertex_open_us_state_dept", "bureau", "westernHemisphere",
     "action_kind", "uyDiplomacy", "action_id", "issued_at",
     "US State: ウルグアイ (MRREE UY) 二国間 (Mercosur)"),
    ("open-us-state-dept", "usStateDept", "coordinatePyRelations",
     "vertex_open_us_state_dept", "bureau", "westernHemisphere",
     "action_kind", "pyDiplomacy", "action_id", "issued_at",
     "US State: パラグアイ (MRE PY) 二国間 (Taiwan 残存承認)"),
    ("open-us-state-dept", "usStateDept", "coordinateBoRelations",
     "vertex_open_us_state_dept", "bureau", "westernHemisphere",
     "action_kind", "boDiplomacy", "action_id", "issued_at",
     "US State: ボリビア (MRE BO) 二国間 (リチウム三角地帯)"),
    ("open-jp-mofa", "jpMofa", "coordinateOecd",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "oecdDac", "action_id", "issued_at",
     "外務省: OECD / DAC ODA 配分 (CRS 統計)"),
    ("open-jp-mlit", "jpMlit", "regulateVehicle",
     "vertex_open_jp_mlit", "bureau", "road",
     "action_kind", "vehicleBureauOversight", "action_id", "issued_at",
     "国土交通省: 自動車局 (リコール / 型式指定 / 国交省適合)"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionZwEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "zwSanction", "action_id", "issued_at",
     "US Treasury OFAC: ジンバブエ entity 制裁 (ZIDERA)"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionByEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "bySanction", "action_id", "issued_at",
     "US Treasury OFAC: ベラルーシ (Lukashenka) entity 制裁"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionNiEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "niSanction", "action_id", "issued_at",
     "US Treasury OFAC: ニカラグア (Ortega) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind112.sql", "w") as f:
