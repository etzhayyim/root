#!/usr/bin/env python3
"""Wave 124: Caucasus+Levant (AM/AZ/GE/JO/PS) + OECD subgroup + 自動車安全
+ 救急 + cn KR mediation + DRC LRA. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateAmRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "amDiplomacy", "action_id", "issued_at",
     "US State: アルメニア (MFA AM) 二国間 (Nagorno-Karabakh)"),
    ("open-us-state-dept", "usStateDept", "coordinateAzRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "azDiplomacy", "action_id", "issued_at",
     "US State: アゼルバイジャン (MFA AZ) 二国間 (Caspian energy)"),
    ("open-us-state-dept", "usStateDept", "coordinateGeRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "geDiplomacy", "action_id", "issued_at",
     "US State: ジョージア (MFA GE) 二国間 (NATO aspirant)"),
    ("open-us-state-dept", "usStateDept", "coordinateJoRelations",
     "vertex_open_us_state_dept", "bureau", "nearEastern",
     "action_kind", "joDiplomacy", "action_id", "issued_at",
     "US State: ヨルダン (MFA JO) 二国間 (Hashemite)"),
    ("open-us-state-dept", "usStateDept", "coordinatePsRelations",
     "vertex_open_us_state_dept", "bureau", "nearEastern",
     "action_kind", "psDiplomacy", "action_id", "issued_at",
     "US State: パレスチナ (PLO) 二国間 (PA / two-state)"),
    ("open-jp-mofa", "jpMofa", "coordinateOecdSubgroup",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "oecdSubgroup", "action_id", "issued_at",
     "外務省: OECD 委員会 (BEPS / DAC / 環境) subgroup"),
    ("open-jp-mlit", "jpMlit", "regulateVehicleSafety",
     "vertex_open_jp_mlit", "bureau", "road",
     "action_kind", "vehicleSafetyOversight", "action_id", "issued_at",
     "国土交通省: 自動車安全 (NCAP / 衝突安全 / 自動運転)"),
    ("open-jp-mhlw", "jpMhlw", "regulateEmergencyMedicine",
     "vertex_open_jp_mhlw", "bureau", "longTermCare",
     "action_kind", "emergencyMedicine", "action_id", "issued_at",
     "厚労省: 救急医療 (3次救急 / DPC / 救命救急センター)"),
    ("open-cn-mofa", "cnMofa", "mediateKoreaPeninsula",
     "vertex_open_cn_mofa", "department_kind", "treaty",
     "action_kind", "krMediation", "action_id", "issued_at",
     "中国外交部: 朝鮮半島和平調停 / 6者協議"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionLra",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "lraSanction", "action_id", "issued_at",
     "US Treasury OFAC: LRA (Lord's Resistance Army) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open(str(pathlib.Path(__file__).parent / "bind124.sql"), "w") as f:
