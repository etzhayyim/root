#!/usr/bin/env python3
"""Wave 111: Pacific islands (FJ/PG/SB/TV/VU) + cn FOCAC + 港湾局
+ Libya/Iraq/Yemen sanctions. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateFjRelations",
     "vertex_open_us_state_dept", "bureau", "eastAsianPacific",
     "action_kind", "fjDiplomacy", "action_id", "issued_at",
     "US State: フィジー (MFA FJ) 二国間 (Pacific Islands Forum)"),
    ("open-us-state-dept", "usStateDept", "coordinatePgRelations",
     "vertex_open_us_state_dept", "bureau", "eastAsianPacific",
     "action_kind", "pgDiplomacy", "action_id", "issued_at",
     "US State: パプアニューギニア (MFA PG) 二国間"),
    ("open-us-state-dept", "usStateDept", "coordinateSbRelations",
     "vertex_open_us_state_dept", "bureau", "eastAsianPacific",
     "action_kind", "sbDiplomacy", "action_id", "issued_at",
     "US State: ソロモン諸島 (MFA SB) 二国間 (PRC pivot)"),
    ("open-us-state-dept", "usStateDept", "coordinateTvRelations",
     "vertex_open_us_state_dept", "bureau", "eastAsianPacific",
     "action_kind", "tvDiplomacy", "action_id", "issued_at",
     "US State: ツバル (MFA TV) 二国間 (Taiwan recognition)"),
    ("open-us-state-dept", "usStateDept", "coordinateVuRelations",
     "vertex_open_us_state_dept", "bureau", "eastAsianPacific",
     "action_kind", "vuDiplomacy", "action_id", "issued_at",
     "US State: バヌアツ (MFA VU) 二国間"),
    ("open-cn-mofa", "cnMofa", "coordinateFocac",
     "vertex_open_cn_mofa", "department_kind", "treaty",
     "action_kind", "focacAfrica", "action_id", "issued_at",
     "中国外交部: FOCAC (中国アフリカ協力フォーラム)"),
    ("open-jp-mlit", "jpMlit", "regulatePorts",
     "vertex_open_jp_mlit", "bureau", "ports",
     "action_kind", "portsBureauOversight", "action_id", "issued_at",
     "国土交通省: 港湾局 (国際戦略港湾 / 国際拠点港湾)"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionLyEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "lySanction", "action_id", "issued_at",
     "US Treasury OFAC: リビア (Haftar / GNU 関連) entity 制裁"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionIqEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "iqSanction", "action_id", "issued_at",
     "US Treasury OFAC: イラク (PMF / IRGC 系) entity 制裁"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionYeEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "yeSanction", "action_id", "issued_at",
     "US Treasury OFAC: イエメン (Houthi Ansar Allah) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind111.sql", "w") as f:
