#!/usr/bin/env python3
"""Wave 109: Nordic/Baltic (DK/SE/NO/FI/EE) + HCCH Hague + 観光庁 + 環保部
+ AF/LB sanctions. NO DDL.
"""
...
ENTRIES_NEW = [
    ("open-us-state-dept", "usStateDept", "coordinateDkRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "dkDiplomacy", "action_id", "issued_at",
     "US State: デンマーク (UM DK) 二国間 (Greenland 関連)"),
    ("open-us-state-dept", "usStateDept", "coordinateSeRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "seDiplomacy", "action_id", "issued_at",
     "US State: スウェーデン (UD SE) 二国間 (NATO新規加盟)"),
    ("open-us-state-dept", "usStateDept", "coordinateNoRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "noDiplomacy", "action_id", "issued_at",
     "US State: ノルウェー (UD NO) 二国間 (Arctic / NATO)"),
    ("open-us-state-dept", "usStateDept", "coordinateFiRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "fiDiplomacy", "action_id", "issued_at",
     "US State: フィンランド (UM FI) 二国間 (NATO新規加盟)"),
    ("open-us-state-dept", "usStateDept", "coordinateEeRelations",
     "vertex_open_us_state_dept", "bureau", "european",
     "action_kind", "eeDiplomacy", "action_id", "issued_at",
     "US State: エストニア (MFA EE) 二国間 (Baltic NATO)"),
    ("open-jp-mofa", "jpMofa", "coordinateHcch",
     "vertex_open_jp_mofa", "bureau", "international",
     "action_kind", "hcchConvention", "action_id", "issued_at",
     "外務省: HCCH (Hague 国際私法会議) 条約調整"),
    ("open-jp-mlit", "jpMlit", "promoteTourism",
     "vertex_open_jp_mlit", "bureau", "tourism_jnto",
     "action_kind", "jntoTourism", "action_id", "issued_at",
     "国土交通省: 観光庁 / JNTO インバウンド振興"),
    ("open-cn-state-council", "cnStateCouncil", "regulateEcology",
     "vertex_open_cn_state_council", "organ_kind", "ministry",
     "topic", "ecologyEnvironment", "action_id", "issued_at",
     "国务院: 生態環境部 (環保部) 環境規制 / 排出基準"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionAfEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "afSanction", "action_id", "issued_at",
     "US Treasury OFAC: アフガニスタン (Taliban) entity 制裁"),
    ("open-us-treasury-dept", "usTreasuryDept", "sanctionLbEntity",
     "vertex_open_us_treasury_dept", "sub_agency", "ofac",
     "action_kind", "lbSanction", "action_id", "issued_at",
     "US Treasury OFAC: レバノン (Hezbollah) entity 制裁"),
]
ENTRIES = ENTRIES_NEW
...
with open("/tmp/wave13/bind109.sql", "w") as f:
