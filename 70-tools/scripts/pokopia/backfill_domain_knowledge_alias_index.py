    ...
    with psycopg.connect(get_rw_url()) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                WHERE document_vid LIKE 'at://did:web:llm.etzhayyim.com/ai.gftd.apps.llm.domainKnowledge/pokemon-pokopia-%'
                """
            )
            ...
            for entity in entities:
                ...
                if slug_tail.startswith(("pokemon-", "item-", "habitat-", "building-")):
                    ...
                    doc_vid = f"at://{OWNER_DID}/ai.gftd.apps.llm.domainKnowledge/pokemon-pokopia-chigo-berry"
                    chunk_vid = chunk_by_doc.get(doc_vid, f"{doc_vid}/chunk-1")
                    doc_suffix = ""
                else:
                    doc_suffix = f"{kind}-{slug_tail}"
                if doc_suffix:
                    doc_vid = f"at://{OWNER_DID}/ai.gftd.apps.llm.domainKnowledge/pokemon-pokopia-{doc_suffix}"
