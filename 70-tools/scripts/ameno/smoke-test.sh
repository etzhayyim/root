...
#   1. POST /xrpc/ai.gftd.apps.ameno.saveResult         — Worker → bpmn-dispatcher
#                                                          → ameno-langserver pod
#                                                          → INSERT vertex_ameno_inferenceresult
#   2. GET  /xrpc/ai.gftd.apps.ameno.listHistory        — same path, SELECT
#                                                          (asserts row from #1 is back)
#   3. GET  /xrpc/ai.gftd.apps.ameno.subscribeBriefs    — SSE stream, must
#                                                          emit `event: ready`
#                                                          (proves NATS path)
#
...
# ── 1. saveResult ─────────────────────────────────────────────────────────────
header "1. saveResult — POST /xrpc/ai.gftd.apps.ameno.saveResult"
...
status=$(curl -sS --max-time 30 -L \
  -X POST "${BASE}/xrpc/ai.gftd.apps.ameno.saveResult" \
...
# ── 2. listHistory ────────────────────────────────────────────────────────────
header "2. listHistory — GET /xrpc/ai.gftd.apps.ameno.listHistory"
status=$(curl -sS --max-time 15 -L -G \
  "${BASE}/xrpc/ai.gftd.apps.ameno.listHistory" \
...
# ── 3. subscribeBriefs ────────────────────────────────────────────────────────
header "3. subscribeBriefs — GET /xrpc/ai.gftd.apps.ameno.subscribeBriefs (SSE)"
...
curl -sS -N --max-time "${SSE_TIMEOUT_SEC}" -L \
  "${BASE}/xrpc/ai.gftd.apps.ameno.subscribeBriefs?collection=app.bsky.feed.post&maxEvents=2&idleTimeoutSec=${SSE_TIMEOUT_SEC}" \
