...
#
#   process_def  at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/{slug}-v{N}
#   binding      at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.binding/{ns-action}-v{N}
#
# - `slug` in process_def is the kebab-case form of `bpmn_process_id`
#   (underscores → hyphens).
# - `{ns-action}` in binding is `{second-to-last}-{last}` segments of the
#   NSID (e.g. `ai.gftd.apps.bot.ping` → `bot-ping`).
#
...
def print_report(d: dict[str, Any], only_drift: bool = False, strict: bool = False) -> int:
    ...
    if strict:
        ...
        if nc_def or nc_bind:
            ...
            print(f"     at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.{{processDef|binding}}/{{slug}}-v{{N}}")
            for d_ in nc_def:
