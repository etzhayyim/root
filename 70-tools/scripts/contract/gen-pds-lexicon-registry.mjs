...
// Collision-aware name resolver. When two NSIDs (e.g. ai.gftd.convo.sendMessage
// vs chat.bsky.convo.sendMessage) would produce the same `NSID_*` constant,
// disambiguate by prefixing the namespace segments (ai.gftd → AI_GFTD,
...
function generate(lexicons) {
  ...
  lines.push(" * Usage: const input = parseXrpcBody<\"ai.gftd.convo.send\">(body);");
  lines.push(" */");
  ...
  lines.push(" * Usage: return typedResponse<\"ai.gftd.convo.send\">(data);");
  lines.push(" */");
