"""Submit N images to ComfyUI Hunyuan3D-2, poll until all complete."""
...
from .workflow import hunyuan3d_workflow


def main() -> None:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--comfy-url", default=os.environ.get("BGP_COMFY_URL", "http://192.168.1.70:8188"))
    p.add_argument("--images", required=True, help="comma-separated image filenames (must already be in ComfyUI input/)")
    ...
    for img in images:
        ...
        jobs.append({"image": img, "slug": slug, "prompt_id": pid})
        print(f"  [{img}] -> {pid}")
