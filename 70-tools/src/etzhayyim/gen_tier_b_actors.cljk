...
;;
;; Scans flat west actor checkouts for `Tier-B` actors NOT already hand-authored in
...
(def ^:private actors-dir
  (or (System/getenv "ETZHAYYIM_WEST_ACTORS_DIR") "orgs/etzhayyim"))
...
(defn- read-record [a hand]
  (let [dir   (io/file actors-dir (str "com-etzhayyim-" a))
    ...
    (when (and m (= (:tier m) "Tier-B")
               (not= (:status m) :compatibility-redirect))
...
(defn collect []
  (let [hand (hand-authored)]
    (->> (sort (keep #(let [n (.getName %)]
                        (when (and (.isDirectory %)
                                   (str/starts-with? n "com-etzhayyim-"))
                          (subs n (count "com-etzhayyim-"))))
                      (.listFiles (io/file actors-dir))))
         (keep #(try (read-record % hand) (catch Exception _ nil)))
         ;; Compatibility repositories may temporarily expose the canonical
         ;; actor id until their redirect tombstone pin lands. Keep the first
         ;; flat path (hyphen sorts before underscore) for each public handle.
         (reduce (fn [by-handle entry]
                   (if (contains? by-handle (:handle entry))
                     by-handle
                     (assoc by-handle (:handle entry) entry)))
                 (sorted-map))
         vals
         (sort-by :handle)
         vec)))
