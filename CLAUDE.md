...
| tsutae 伝え | handheld comms device (≤200g, open SoC). **Uplift to sarutahiko-class substrate (ADR-2606023600)**: 8 Pregel cells implemented (8/8 state-machine tests, 7 in-code gate guards G9 open-SoC/G6 mic-killswitch/G3 repair/G2 bootloader/G7 blob/G8 anti-addiction/G4 quorum/G10 take-back) + `agent.py` 11/11 + `tsutae-factory-r0` cleanroom scene with **63-part building.edn incl. a 20-part device BOM = the smartphone supply chain** (SoC/PMIC/PCB/battery/display/camera/USB-C/cellular/chassis…, internal silicon/igata/hikari vs honest `:representative` external) + 15-step 4D + 12-station line + 6-script toolchain (SBOM/kotoba/IFC) + self-contained `tsutae-factory.htm` viewer. Cells stay Council-gated (`.solve()`→RuntimeError). **Deferred**: kami-app WASM physics crate (kami-engine submodule source absent; specced in KAMI_APP_SPEC.md) | 🟢 R0 substrate | 2605261300 / 2606023600 | 06-02 |
...
| aratame 改め | authorized GitHub-repo **static-source vulnerability inspection** — the THIRD leg of the security triad: akuma 悪魔 (runtime/blackbox diagnosis) + **aratame (source/static diagnosis)** + tsukuroi 繕い (remediation). owner+authority dual-signed `InspectionMandate` (pinned repo@ref) → **READ-ONLY** clone into egress-restricted `aratame-scan` sandbox → OSS **SAST** (Semgrep OSS/CodeQL OSS) + **SCA** (OSV-Scanner/Trivy + reuse giemon purl→CVE) + **secret scan** (gitleaks/trufflehog OSS) → Murakumo-only triage (**gemma4-26b-a4b**, Gemma 4 MoE ~26B/~4B active, NON-adjudicating) → emit tsukuroi-compatible `vulnFinding` + `inspectionReport`. **READ-ONLY + STATIC-ONLY** (never executes target code, never writes repo, no platform key); remediation is tsukuroi's (propose-only). 8 cells / 6 lexicons / 14 gates / 12 non-goals. kotoba-native. **NEW model gemma4-26b-a4b needs Murakumo fleet registration** (LiteLLM route reserved, NOT RunPod; `ollama create` on ≥128 GB node) before R1 | 🟡 R0 | 2606024000 | 06-02 |

| **worldwide civic-knowledge directories** (danjo · kokoro · manabi · kazaori · shidemori · hagukumi · kataribe) | per-actor `unverified-seed` worldwide seed registry: fiscal-transparency sources · crisis-support lines · open-education resources · civilian disaster agencies · death-registration authorities · public care-support programs · official gazettes. ~1,382 net-new entries up to 63 jurisdictions (danjo 304 · kazaori 255 · kataribe 226 · hagukumi 172 · manabi 168 · shidemori 130 · kokoro 127), each with a fail-closed invariants suite + VERIFICATION.md; built atop the civic-concierge worldwide set (toritsugi/kurashimori/moushibumi/himotoki/chigiri/musubi 363) + 9 R1 pure compute cores/resolvers. Corpus ~1,745 entries / 13 actors, all `unverified-seed` (G14), R0 ceiling (cells import-time RuntimeError). Honest: seed only, mid-tier entries low/medium-confidence-flagged for R1 verify | 🟡 R0 | 2606030600 / 2606023000 | 06-03 |
