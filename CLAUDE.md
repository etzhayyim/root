...
| **kami-genesis maturation (PhysX/Isaac gap)** | NVIDIA との差を WASM クリーンルームで縮小 (ADR-2606010030 v6): **②narrow-phase** `convex.rs`(GJK距離+交差) + EPA貫入(深さ/法線) + `ccd.rs`(conservative-advancement TOI + 解析sphere-plane, トンネリング防止); **③連続体** `mpm.rs`(2D MLS-MPM = 弾性/砂・コンクリgranular/流体, DepositField/MoldFieldの本物化); **③熱** `thermal.rs`(transient熱伝導PDE 2D-FDM+移動Gaussian源, WeldFieldの本物化); **①GPU並列env** `wgpu_backend` が cartpole/DP を実Metal device で **1024-env compute dispatch + CPU一致検証**(既存を確認)。genesis **111 tests** (+17新, `--features gpu` 115), wasm32 build green。honest: 2D/explicit/CPU/f32, manifold/persistent-contact未, 一般articulation GPU未; アルゴリズム同クラスをテスト検証, NVIDIAビット一致ではない | 🟢 | 2606010030 | 05-31 |
