...
| 62. **kotoba storage substrate unification R0** — single content-addressed Datalog + Pregel + Signal + WASM substrate engine subsuming 7+ storage components (ipfs-pinner / nats-jetstream-* / mst-projector / lancedb-wasm / tonbo / etzhayyim-xrpc-proxy / libsignal wrappers) under `40-engine/kotoba` git subrepo (17 crates, Apache-2.0, Charter Rider applied); Phase 0 = subrepo import + ADR; Phases 1-7 gated by per-phase Council Lv6+ ≥3 attestation; `kotoba-llm` local-inference constitutionally disabled per ADR-2605215000; existing impls remain ≥1 R-cycle as cold backup | 🟡 2026-05-26 (ADR-2605262100; subrepo at 40-engine/kotoba; `.gitrepo` metadata committed) |
