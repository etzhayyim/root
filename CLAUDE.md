...
| sukashi 透かし | ad-tech supply-chain + delivery-infra + fraud-network observatory — datafies the programmatic ad ecosystem from PUBLIC IAB files (ads.txt/app-ads.txt/sellers.json): :adtech/:adauth.edge (authorization handshake; declared&&!confirmed = spoofed-inventory surface)/:adcreative/:addelivery.edge (creative→IP/ASN/WHOIS, reusing tadori's ip-network+passive-dns)/:adfraud.signal (non-adjudicating, routed to akashi-malak/kurashimori/tasuke/danjo). ad-tech-supply-chain+fraud sibling of akashi 証 (ad-library disclosure). Fraud-PROTECTION observatory, NEVER an ad network/buying/targeting/detection-evasion tool (広告排除 invariant served); G4 real firms carry no fraud signal, every fraud example :synthesized on a FICTIONAL entity; 16 tests green | 🟡 R0 | 2606071600 | 06-07 |
