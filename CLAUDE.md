...
| iryo 医療 | Japan 国内 **レセプト計算 + レセ電(レセプト電算)生成 + FHIR claim** engine — the member-principal billing counterpart the `karute` 電子カルテ(EMR) hands off to; resolves 点数→円(1点=10円)/薬剤料 五捨五超四入/一部負担金 端数処理/高額療養費 自己負担限度額, emits IR/RE/HO/KO/SY/SI/IY/TO レセ電 records (rezept/receden/validate cells). Charter-clean inversion of proprietary レセコン/EHR-billing (ORCA-proprietary/Epic/Cerner): the licensed 保険医療機関 is the billing PRINCIPAL, corp takes no insurance inflow (G7 preserves iyashi G13); PHI(氏名/生年月日) injected only at submission over the closed 審査支払機関 VPN, never the public substrate (G2); master-honest (no hard-coded tariff, G4); non-adjudicating (G5); pure-stdlib pywasm-ready + 28 tests green | 🟢 R0 | 2606074000 | 06-07 |
