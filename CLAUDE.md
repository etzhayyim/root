...
| **giemon factory R0** | the FACTORY that builds the giemon line — whole-plant **layout + 建材 + 全MEP(電気/給排水/ガス/圧空/通信/消防) + 外構/外部動線 + 原材料 + 4D 建築手順**, all in kami-engine + kotoba Datomic. `factory.scene.json` (walls/columns/beams/zones/machines as `Obstacle::Aabb` + **routed `utilities`配管/配線** 色分け + `service_nodes`引込 + `fixtures`照明 + `site_*`外構) + **4 arm6 work-cells** + **2 four-DOF AGV**; `building.edn` (**72 parts A-L**)→CycloneDX SBOM + `kg/claim/part/*`; `construction.edn` (**23-step 4D**, 引込→埋設→基礎→鉄骨→外装→配線→照明→機械→ロボット→外構→試運転, depends-on/consumes/reveals)→`kg/claim/step/*` (critical path via VAET) + `construction.order.json` replayed by `run_giemon_factory_build_v1`. crate `kami-app-giemon-factory` (8 tests green, wasm ok); `giemon-factory.htm`. No FEM/構造解析・MEP/CFD・mega-press(N1); `:sourcing :representative` | 🟢 | 2606010030 | 05-31 |
