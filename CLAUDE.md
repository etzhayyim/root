...
**Legend**: ✅ shipped · 🟢 landed (substrate, tests green) · 🟡 R0 / proposed scaffold · ⏳ blocked/pending. **This table is a one-line index only** — full detail (tests, file paths, gates, honest-R0 caveats) lives in each row's ADR; see [`90-docs/adr/README.md`](90-docs/adr/README.md).
...
| Basic High Income doctrine | imputed-income flow + commons-asset stock; high income in-kind, cash≡0 | 🟡 | 2605301020 | 05-30 |
| Mission-funding revenue arm | vendor surplus → donation → Public Fund; non-profit = means, not end | 🟡 | 2605301036 | 05-30 |
| Social Security for Humanity | Charter §1.16 人類の社会保障; covenantal-universal, conversion-gated | 🟡 | 2605302357 | 05-30 |
| Social Security delivery pipeline | §1.16 outreach→vow→compute→openmail→atproto→social; outward-gated (G11) | 🟡 | 2605302358 | 05-30 |
...
| manimani kotoba-native | personal knowledge router on kotoba EAVT + StateGraph + Murakumo; Gmail/PC ingest | 🟡 | 2605291100 | 05-29 |
| kotoba v0.1.0 + brew tap | first tag + GH Release + homebrew-kotoba; `brew install kotoba` green | ✅ | 2605292100 | 05-29 |
| kotoba actor deploy + Murakumo live | WASM + Python-LangGraph actors in-WASM :8077; kotoba→Murakumo `llm.infer` | ✅ (see 2605302355) | 2605301625 | 05-30 |
| kotoba LangGraph LLM verified + durable routing | EMPIRICAL: in-WASM LangGraph actor does LLM inference; durable fix = loopback LiteLLM :4000 | ✅ | 2605302355 | 05-30 |
| **Donation-funded operation + compute-node donation** | etzhayyim.com /donate + donation.json; in-kind compute donation (ameno/e7m/kotoba node class) | 🟡 R0 | 2606012100 | 06-01 |
| Spirit-in-Physics kotoba datafication | 霊性 data → datomic kotoba; edge-primary `:spirit.bond/*` + Wellbecoming as-of history | 🟡 R0 | 2606011500 | 06-01 |
| **kotoba hybrid web search** | Google-shaped legs over Common Crawl, datom-native: BM25 + PageRank + RRF fusion | 🟢 | 2606012300 | 06-01 |
| **Actor profile + dynamic did.json** | actor-profile SSoT EDN + apex Worker issues did.json dynamically (KV→kotoba→fallback) | 🟢 | 2606013800 | 06-01 |
| **kotoba passkey-rooted secrecy** | WebAuthn PRF → Account Root Key (ARK) → HKDF purpose-keys; CACAO = authz plane, Signal = user↔user msg plane. **LANDED**: D2 AEAD-AAD + `SecureVault::{put,get}_bound` + `#![deny(unsafe_code)]`; D1 `key_tree` (PRF→ARK wrap/recover, multi-device, Shamir-GF(256) guardian recovery); D4 `signal::binding` (DID↔Signal verify + substitution guard + ratchet key-wrap) + kotoba-server `ai.gftd.signal.{publish,resolve}.identity` (resolver-verified: did:key trustless + did:web/plc via DID-document key = ERC725 mirror; unresolvable never vouched); EmailIngestor body bound to owning email CID. kotoba-crypto 79 / kse 137 / signal 114 / ingest 89 / server signal_xrpc 28 green. **Client L0**: yoro `$lib/auth/key-tree.ts`+`prf.ts` (WebCrypto PRF→ARK→purpose-keys, byte-for-byte Rust-interop via KAT) + signUp/signIn PRF wiring; yoro vitest 13 green, svelte-check clean. **L1/C-2/DID-doc landed**: kotoba-server `ai.gftd.account.{put,get}.wrapped.ark` (opaque per-passkey ARK store, server can't read) + yoro `session-key.ts` (deterministic Ed25519 from k_session, zero-dep pkcs8, did:key + registerSigningKey) + apex `ed25519VerificationMethod`/`toDidDoc` emit member key as verificationMethod+authentication. account_xrpc 4 green. **Client wired E2E**: yoro `key-hierarchy.ts` (enroll/recover/add-device, correct multi-device) + `account.ts` transport + signIn recovers-or-enrolls hierarchy from S_prf & registers session key. yoro auth vitest 26 green, svelte-check clean. **did:web chain closed**: publisher `--signing-key` + apex `withVerificationMethod` → did.json verificationMethod+auth+assert → resolve.identity verifies. **Add-device** (same-device `addDevicePasskey`) + **cross-device ARK transfer** (`device-transfer.ts`: HPKE X25519→HKDF→AES-GCM, never plaintext to channel/server) + **C-3 PoP** client (`signSessionPoP`) + **server verify** (`session-pop.ts` + `verifySessionPoP` endpoint, additive/read-only; client↔worker interop cross-checked in Node) landed. yoro auth vitest 31 green. **C-3 landed (additive)**: `verifySessionPoP` + `createSessionFromPoP` login (worker verifies client PoP, no server signing-key custody). **C-4 instrumented**: `logKekRead` at all 3 KEK sites + 30-day zero-read quarantine runbook. **C-4 enablement wired (all 3 KEK sites)**: `SS_KEY_CUSTODY_MODE=client_self_custody` → sign-up + sub-DID persist public-key-only; service-auth returns 409 for client-custody keys — none read the KEK (default-OFF). Remaining = operator KEK deletion (irreversible, 30-day zero-read gated). PDS-side PoP now landed in kotoba-server (ADR-2606015000) not the absent TS worker | 🟢 R0 | 2606014000 / 2606014500 | 06-02 |
| **PDS refactor → kotoba-server** | retire separate TS PDS (src absent from tree); serve AT Protocol PDS XRPC on kotoba-server over canonical Datom state. **D1 landed**: `pds_session.rs::verify_session_pop` (Rust zero-access PoP: did:key trustless + did:web via ERC725-mirror doc; expired/tampered/unresolvable rejected) + `POST ai.gftd.pds.session.verify`. pds_session 5 green, server builds. D2 (repo/sync/identity XRPC port) phased; full wasm32 edge gated on `kotoba-runtime-web` (kotoba-server native today) | 🟢 R0 | 2606015000 | 06-02 |
| **ai-gftd-auth → Proton zero-access** | shift live auth Worker off T1 server-KEK custody toward client-self-custody. **LANDED (C-2 server side)**: `app.etzhayyim.auth.registerSigningKey` (public-key-only, no KEK, ownership-checked) + `key_custody_tier` column + `SS_KEY_CUSTODY_MODE` flag (default legacy = non-breaking); Prohibited-Patterns amended. C-3 session PoP + C-4 KEK deletion gated; yoro client wiring + wrangler typecheck pending (no local TS harness) | 🟡 R0 | 2606014500 | 06-01 |
| **kotoba-git — git ↔ datomic + CID** | new crate `kotoba-git`: git object DAG (blob/tree/commit/tag/ref) ↔ kotoba, byte-exact round-trip. Lossless anchor = framed bytes as `KotobaCid` block; Datom projection = `:git/*` schema with SHA↔CID bridge (`:git/oid` ↔ `:git.object/cid`) + queryable commit DAG/refs. **Pure-Rust packfile decoder** (idx v2 + pack v2 + OFS/REF delta) → loose + packed (`import_repo`) repos round-trip, no `git`/libgit2. Verified vs real git oid vectors + `git` CLI loose+packed repo import/materialize. 27 tests green (incl. real-Datalog query + annotated-tag round-trip), clippy clean. R0: thin packs unsupported, parent-order only in block, sha256-repos out | 🟢 R0 | 2606015000 | 06-01 |
...
| kami-genesis physics maturation | PlanarChain N-link articulation + isaacsim.core.api clean-room + conservation-law validation | 🟢 | 2605311500 | 05-31 |
| kami-genesis 3-D spatial + contact | 3-D reduced-coord solver (Featherstone RNEA+CRBA+LDLᵀ) + rigid contact (PGS); 6-DOF arm6 | 🟢 | 2605311800 | 05-31 |
| Shibuya digital-twin (iter 1) | real OSM Shibuya → city-mesh → kami-genesis multi-agent physics sim | 🟢 | 2605311900 | 05-31 |
| Shibuya digital-twin iter 2 (kotoba + 3DGS) | point assets → 501 kotoba EAVT entities + Mapillary acquisition front | 🟢 | 2605312200 | 05-31 |
| Shibuya 3DGS (Mapillary SfM + splat-physics) | Mapillary → pycolmap SfM → 2,235-pt 渋谷 .splat → kami GsplatAdapter + physics | 🟢 | 2605312600 | 05-31 |
| giemon kabitori mold-removal sim | steerable rotary-brush mold-removal probe; Obstacle::Plane/Aabb contact + MoldField erosion | 🟢 | 2605312300 | 05-31 |
| giemon part graph (SBOM↔kotoba) | per-part SBOM ledger EDN → CycloneDX → kotoba EAVT; cross-robot fleet + CVE vuln-match | 🟢 | 2605312330 | 05-31 |
| **giemon factory R0** | 4D-BIM plant: layout + 建材 + full MEP + 外構 + robotics construction (tatekata); 272 kotoba entities | 🟢 | 2606010030 | 05-31 |
| **sarutahiko truck factory R0** | 4D-BIM Class-8 truck plant + production line + 積込ロボット; 286 kotoba entities | 🟢 | 2606013100 / 2605252500 | 06-01 |
| **kami-genesis maturation (PhysX/Isaac gap)** | narrow-phase GJK/EPA/CCD + MPM continuum + thermal PDE + OBB manifold + GPU planar-chain; 118 tests | 🟢 | 2606010030 | 05-31 |
| **kami-autodrive GNC autonomy** | perception→plan→control GNC; 4 vehicle classes (car/ship/drone/aircraft) w/ real physics plants | 🟢 | 2606010600 | 05-31 |
| **iwakura/fuigo ternary RTL → sky130 GDSII** | ternary-silicon RTL→verify→synth→P&R GDSII on sky130 (DRC/LVS 0, f_max ~93–211 MHz) | 🟢 | 2605242515 / 2606012800 | 06-01 |
...
| danjo 弾正 | public-accountability oversight — ingests JP 国会/予算/政府調達, non-adjudicating observations | 🟡 R0 | 2605301600 | 05-30 |
| tadori 辿 | authorized on-chain tx tracing + actor attribution (kotoba-EAVT-native) | 🟡 R0 | 2605301400 | 05-30 |
| warifu 割符 | open zero-fee card (credit+debit), Stripe/EMV/NFC-compatible; on-chain USDC settle; 0% credit | 🟡 R0 | 2605302000 | 05-30 |
| himotoki 繙き | ACTIVE disclosure-request filer — consent-bound DSAR (APPI/GDPR/CCPA) + FOIA; own-data-only | 🟡 R0 | 2605302130 | 05-30 |
| kanae 鼎 | global government fiscal-flow VISUALIZATION → kotoba fundFlowEdge; danjo finds, kanae renders | 🟡 R0 | 2605302300 | 05-30 |
| toritsugi 取次 | citizen government-procedure CONCIERGE; guide+draft-assist, member self-submits (UPL boundary) | 🟡 R0 | 2605312030 | 05-31 |
| moushibumi 申文 | citizen democratic-participation CONCIERGE — 選挙情報/請願/パブコメ; politically neutral (公選法) | 🟡 R0 | 2605312400 | 05-31 |
| kurashimori 暮らし守 | citizen consumer-protection CONCIERGE — クーリングオフ/返金/エスカレーション; UPL boundary | 🟡 R0 | 2605312500 | 05-31 |
| haraedo 祓戸 | global bulky-waste (粗大ゴミ) disposal — citizen intake + operator logistics dispatch | 🟡 R0 | 2606010200 | 05-31 |
| kizashi 兆 | non-invasive multimodal body-scan / sign-sensing; non-diagnostic (kizashi→mitate→iyashi) | 🟡 R0 | 2605312700 | 05-31 |
| tsumugi 紡ぎ | Engi Knowledge Graph intel weaver — Spirit-in-Physics over public power-entities, 取-concentration→release | 🟡 R0 | 2606011800 | 06-01 |
| okaimono 御買物 | charter-aligned anti-Amazon provisioning commons — 3 rings (commons-first/internal/external) | 🟢 R0+R1+R2+R3 | 2606012100 | 06-01 |
...
| ooyake 公 | world government ATLAS — kotoba-Datomic structural tree of every gov unit (国→省→庁→局→課→窓口) with 住所/窓口/書式/手続き/BPMN; read-side SSoT danjo/kanae/tsumugi/toritsugi/himotoki consume; observational MIRROR + civic wayfinding, NEVER an official channel/target-list; amends 2605242330 §2 | 🟡 R0 | 2606021600 | 06-02 |
| kabuto 兜 | world PUBLIC-COMPANY supply-chain KG — listed companies + 登記本社住所 + 公開連絡先 + first-class supplier→customer SUPPLY edges + BPMN; surfaces single-source/sector/jurisdiction concentration routed to redundancy + accountability; atproto social via kotoba-server; browser-native kotoba-wasm render on /search; resilience + transparency map, NEVER a target-list (tsumugi/watatsuna/danjo sibling, shares org.corp.* id space) | 🟡 R0 | 2606022000 | 06-02 |
