...
| kotoba LangGraph LLM verified + durable routing | EMPIRICAL re-verify on live :8077: Python LangGraph actor runs in-WASM + does LLM inference e2e; durable fix = route via loopback Murakumo LiteLLM (corrects 2605301625) | ✅ | 2605302355 | 05-30 |
...
| **WASM-actor SBOM attestation** | CycloneDX SBOM generator for deployed wasm (recomputes kotoba program CID); purl→CVE match unchanged; wired into deploy.sh + manifest G11 | 🟢 R0 | 2606036000 | 06-03 |
...
| sumitsubo 墨壺 | cleanroom CAD interop (Vectorworks/Autodesk/AutoCAD) — @etzhayyim/sumitsubo-cad geometry kernel + ModelOp vocab + DXF/SVG/OBJ/glTF/IFC4/STEP exporters; kotoba EAVT bridge; published-API-shape adapters only | 🟢 R0 | 2606033600 | 06-03 |
...
| tsubasa 翼 | flight-route/fare discovery commons (Skyscanner inversion) — honest fare meta-search, affiliate-stripped onward links, member self-books (no inflow); ranks by true total cost with CO₂ surfaced (G4); no-ads/no-urgency/no-person-fare-tracking; sibling of watari (position) | 🟢 R0+R1 | 2606072800 | 06-07 |
