...
| rasen 螺旋 | public-genetics (公開遺伝) KG mirror — the gene/variant-scale sibling of inochi 命 (which stops at the species scale); hosts PUBLIC reference genetics only (genes/variants/phenotypes/pathways + population-AGGREGATE allele frequencies) across human/elephant/rice/bacteria in the kotoba Datom log, runs **edge-primary clinical/functional evidence (care-priority = Σ incident variant-association via :located-in + gene-linkage × DISCLOSED clinsig) routed to CARE & RESEARCH**. **R1 live PUBLIC ingest**: MyGene.info + MyVariant.info → ClinVar significance + gnomAD SUPER-POPULATION AGGREGATE freq → genome-ontology EDN/Datom, content-addressed to a kotoba-IPFS CIDv1 (raw/sha2-256, `methods/cid.py`, byte-identical to `ipfs add`, no daemon) + ipfs-pinned (CID match verified). **+ live GO pathway ingest** (PR #1591): public Gene-Ontology/EBI-GOA via MyGene `go.BP` → deduped/best-evidence/capped (G5) `:participates-in` edges feeding the pleiotropy/cascade readout; GO-enabled live run **192 nodes/220 縁** (117 GO edges), 0 errors. G1 = CARE/RESEARCH map, **NEVER an individual-genotype registry** (no individual/sample/family genotypes; aggregate `af_*` only; coarse cytoband never precise coords — enforced by tests + by construction); non-adjudicating (ClinVar/GO DISCLOSED, not re-judged); ingest scope-expansion + WASM deploy G7-gated; individual/clinical genome ingest explicitly out-of-scope (→ separate himotoki-enveloped consent-bound actor); 17 tests green | 🟢 R1 | 2606101000 | 06-10 |
