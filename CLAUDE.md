...
| **kami-autodrive GNC autonomy** | the missing **自律ループ** wiring kami-engine の既存ピースを knit: `kami-vehicle`(BeamNG実車) + `kami-sensor-sim`(LiDAR/cam/IMU) + `kami-pathfind`(A*) を **知覚→計画→制御** に接続。新crate `kami-autodrive`: `perception`(LiDAR sweep→`OccupancyGrid`+C-space inflation+`forward_clearance`)・`planner`(inflated grid 上 A*→LOS簡約 polyline)・`control`(`PurePursuit`横+`SpeedController` PID縦+曲率速度制限)・`plant`(`Plant` trait + `BicycleModel`)・`autopilot`(`DriveState` Idle/Cruise/Slow/Stop/Blocked/Arrived; ingest→緊急停止→replan→pursuit→速度min)。**4車種** `VehicleClass{Car,Ship,Drone,Aircraft}`は同一GNCを共有、**車のみ高忠実度プラント**(`SoftBodyCar`アダプタ=同一Autopilotで実ソフトボディsedanを駆動, `soft-body-car` feature)、船/ドローン/飛行機は planar-kinematic(正直境界)。Omniverse/Isaac Sim でいう DRIVE スタック層に相当、ただし Apache/MIT + CPU稼働 + nv-compat `isaacsim`。**6 tests green**(壁迂回・緊急停止・船広旋回・実車waypoint到達)、example `drive_to_goal`(壁x18-22を y≈6.7迂回→(40,0)到達)。honest: 単体GNC・実シーン(渋谷splat)/カメラ知覚/multi-agent/流体・空力プラント未; wadachi R1(ADR-2605242000)はCouncil承認後に本crateを呼ぶ設計基盤(SAE L4上限・Transparent Force gated) | 🟢 | 2606010600 | 05-31 |
