...
| 61. **NVIDIA Omniverse stack API-compat layer R1.0 + §D10 viability-gate fallback + §D11 PhysX 10th compat target** — drop-in compat facade across all **10 NVIDIA components** (Omniverse Kit / Nucleus / Isaac Sim / Isaac Lab / OptiX / RTX Renderer / Replicator / DriveSim / Omniverse Cloud / **PhysX**), backed by KAMI-native canonical impls (amenominaka 天之御中 / yatachain-nucleus / e7m-sim / e7m-shugyo 修行 / hikari-rt 光 / kami-rtx / utsushimi 写身 / wadachi-sim / murakumo-render / **kami-physx**); physics primary = **Genesis (Apache-2.0)** 5-solver via Taichi→wgpu; Mitsuba 3 upstream-PR-only 90-day hold; **§D10 contingent fallback policy** — if upstream Genesis / Mitsuba 3 / tinyusdz WASM+WebGPU viability gate fails (R1.1 / R1.2 / R1.8 deadlines), religious-corp implements that capability from scratch in KAMI namespace under Council Lv6+ ≥3 attestation, with nv-compat facade unchanged (§D10.3 invariant); trademark boundary via Google v. Oracle 2021 API fair use + nv-compat namespace localization | ✅ 2026-05-26 (ADR-2605261800; **12 new Rust crates** under `40-engine/kami-engine/` — 9 primary {kami-rt, kami-pbrt, kami-usd, kami-genesis, kami-articulated, kami-sensor-sim, kami-replicator, kami-app-amenominaka, kami-shugyo} + **3 contingent fallback** {kami-physics-solvers, kami-rtx-native, kami-usd-native} all `cargo check` clean; 2 SDK nv-compat namespaces `20-actors/etzhayyim-sdk/src/nv-compat/` + `20-actors/magatama/py/src/pymagatama/nv_compat/`; deps.toml [[adrs]] + **14 [[modules]] entries**; kami-engine workspace members appended; CHARTER-RIDER §6.1 NVIDIA trademark notice (Omniverse / Isaac / OptiX / RTX / Nucleus / DriveSim / **PhysX®**); R1.1–R1.9 sub-phase plan with G5 ≥0.75 quality gate + viability gate per backend; honest scoping: from-scratch fallback = 5-10× upstream-bind effort, commits to multi-year robotics R&D; ADR-2605261800 §D8 reordering: D1..D8, D10, D11; 70-tools/e7m-sim/scenes/cartpole/ R1.1 PoC assets (URDF + scene.yaml + README)) |
