...
| **giemon factory R0** | the FACTORY that builds the giemon line — whole-plant **layout + 建材 + 4D 建築手順**, all in kami-engine + kotoba Datomic. `factory.scene.json` (walls/columns/beams/zones/machines/conveyors as `Obstacle::Aabb` + CAD) + **4 arm6 work-cells** (fixed-base `Articulation3d`) + **2 four-DOF AGV** (collide vs building); `building.edn` (27 parts A-F)→CycloneDX SBOM + `kg/claim/part/*`; `construction.edn` (13-step 4D, depends-on/consumes/reveals)→`kg/claim/step/*` (critical path via VAET) + `construction.order.json` replayed by `run_giemon_factory_build_v1` (基礎→鉄骨→…→試運転). New crate `kami-app-giemon-factory` (7 tests green, wasm ok); `giemon-factory.htm`. No FEM/MEP/mega-press (N1); `:sourcing :representative` | 🟢 | 2606010030 | 05-31 |
