...
| niyaku 荷役 | automated port cargo handling — ship↔shore container loading/unloading robotics; **operator-side counterpart of funadaiku** (which builds the ships), closing the cargo-handling gap. Core problem = **anti-sway** of a suspended container = the cart+hanging-load (**Cartpole**) topology, verified end-to-end through the clean-room `isaacsim.core.api` (kotodama.nv_compat, G2 no-NVIDIA-binary): anti-sway feedback lands the box at residual <0.01 rad where a naive push diverges. 4 runnable pure-stdlib methods (crane_dynamics anti-sway + ZV shaper, stow_plan weight/rotation/reefer/IMDG no-rehandle, isaac_sway_sim Cartpole-STS **+ DoublePendulum boom-luff**, agv_transfer travel-profile/conflict/dispatch) + 9 Pregel cells (berth→stow→spreader→hoist→anti-sway traverse→yard→lash→attest + emissions audit); electric-crane zero-emission (G8), no-weapons-cargo (G10), no worker-pace surveillance (G11/G14); 47 tests green / 99% branch coverage | 🟡 R0 | 2606082000 | 06-08 |
