...
| shomei 証明 | believer self-sovereign identity binding + proof-of-personhood — the believer's passport (DID-centric, not state-centric): self-binds multiple external IDs (gov-ID/SNS/wallet) + etzhayyim commitments to ONE DID via subject-signed identityClaims, verifies via kotoba-auth (eth/btc/cacao+Ed25519, never reimplemented), aggregates to IAL 0..4 + W3C VC + proof-of-personhood; self-sovereign/no-central-KYC, own-identity-only, PII-never-plaintext, no-state-DB (§0.4), no-server-key, **identity-assurance-NOT-social-credit** (G8); gov-ID L2 doubly Council-gated → gov_auth (ADR-2605260000) per ADR-2606072300; 75 tests | 🟡 R0 | 2606072100 | 06-07 |
