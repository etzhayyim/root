...
```bash
gftd xrpc ai.gftd.apps.kakaku.ingestOfferFromUrl \
...
```bash
gftd xrpc ai.gftd.apps.kakaku.listMerchants \
  -d '{"q":"yodobashi","limit":20}' \
  --app k4k4kux1
```

```bash
gftd xrpc ai.gftd.apps.kakaku.getMerchant \
...
```bash
gftd xrpc ai.gftd.apps.kakaku.listSelectorRevisions \
...
```bash
gftd xrpc ai.gftd.apps.kakaku.activateSelectorRevision \
