...
gftd cohort snapshot --out-dir data/staging-cohort-baseline/
```
...
```bash
gftd cohort list --pcfL1 3-market-sell
# 1 row 表示
```
...
```bash
gftd cohort evidence --cohort "$COHORT" --min-posterior 0.95 --judge true
...
```bash
gftd cohort lineage --did did:plc:pending-AABBCCDD
# lineage (2 hop):
#   ├─ did:plc:pending-AABBCCDD  agent-...  kind=fissioned
#   └─ did:plc:pending-X1Y2Z3W4  cohort-...  kind=cohort

gftd cohort lineage-stats --pcfL1 3-market-sell
...
| edge_cohort_derived missing | `gftd cohort repair-edge --did <fissioned> --dry-run=false` |
...
- `70-tools/gftd/CLAUDE.md` §gftd cohort
