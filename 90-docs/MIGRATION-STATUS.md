...
|--------|------:|---------|
| **A — DONE** | 75 | has a `rw-free/` on-chain reference impl |
| **B — CLEAN** | 208 | no `rw-free`, no TODO, no prohibited imports — compliant or thin stub |
| **C — NEEDS-CODEMOD** | 28 | still imports prohibited substrate → the real active backlog |
| **D — TODO-PENDING** | 55 | has `MIGRATION-TODO.md` (seed copied, codemod pending) |
| **V — VENDOR-RESIDENT** | 22 | judged correctly gftd-resident (regulated-infra axis) — no migration |

**Real remaining scope ≈ 83 apps** (C + D = 28 + 55; the 8 Tier-2 commerce apps
...
editor, gov (mixed split — public gov reference front),
...
## Bucket C — NEEDS-CODEMOD (28) — active backlog
...
email-service-adapter (stripe), hc, **hospitality (RW in
