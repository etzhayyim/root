---
id: 260320-magatama-cloudflare-worker-rpc-optimization
title: Magatama Cloudflare Worker RPC Optimization
status: active
doc_type: reference
...
# Magatama Cloudflare Worker RPC Optimization
...
## Why This Split Fits Magatama

`magatama` は guest から見れば capability-based runtime であり、Worker backend ではこれを TypeScript host layer が受ける。
...
よって Worker backend の host adapter は、`magatama capability` ごとに default transport を持つべきである。
...
MiniApp / Front Door Worker
  -> Service Binding
     -> Backend Worker
        ...
        -> TypeScript magatama host adapter
           -> capability registry
           -> generated JS glue
           -> component.wasm
```
...
## Relationship to Shared Magatama Design
...
- shared capability registry: `20-actors/magatama/magatama-capability-registry`
...
4. `/_magatama/capabilities` を TS host generator の入力として使う
