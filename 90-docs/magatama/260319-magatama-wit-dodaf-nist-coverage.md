---
id: 260319-magatama-wit-dodaf-nist-coverage
...
|---|---|---|---|
| `magatama:agent@1.0.0` | agent, identity, capability, governance, dependency, skill, traceability | CV-1/2/3, SV-5, StdV-1/2, DIV-1/2 | GV, PR-AC, PR-DS, GV.SC |
| `magatama:dm2@1.0.0` | types, performer, person, organization, system, service | AV-2, OV-4, CV-4, PV-1, SV-1/2 | GOV, ID.AM, PR-AC |
| `magatama:core@1.0.0` | log, outbound-http, config, cypher, vector-search, remote-call, serve, resilience | SV-1, SV-4, SV-6 | PR-PT |
| `magatama:auth@1.0.0` | passkey, authn, authz, crypto | — | PR-AC |
| `magatama:messaging@1.0.0` | signal, signal-session, conversation, smtp, w-protocol | OV-2 | PR-DP |
| `magatama:observability@1.0.0` | telemetry, access-log, ocel, secrets, audit-trail, anomaly, incident。`pubsub` は `[DESIGN]` (world.wit 未 import) | OV-6c, SV-7, SV-10c | DE, RS, RC, GV |
| `magatama:workflow@1.0.0` | timer, actor-state, reminder, workflow, activity, activity-parallel, lock, virtual-actor | OV-5a/5b, OV-6b | — |
| `magatama:storage@1.0.0` | ipfs, storage, cdn, static-site | — | — |
| `magatama:div@1.0.0` | information, documents, materiel | DIV-3 | PR-DS |
| `magatama:forms@1.0.0` | forms, bpmn, dmn — 全て `[DESIGN]` (world.wit 未 import) | — | — |
| `magatama:wproto@1.0.0` (actor visibility) | follow-request, actor-sensitivity, resolve-visibility | OV-4, OV-6a, SV-10a | GV.RM-1, PR-AC-1/3/5, PR-DS-1/3, DE.AE-1 |

## Guest SDK Coverage

Date: 2026-03-20
Scope: `00-contracts/wit/world.wit` `magatama-component`
...
| `magatama:core` | 7 | 7/7 |
| `magatama:auth` | 4 | 4/4 |
| `magatama:messaging` | 5 | 5/5 |
| `magatama:storage` | 4 | 4/4 |
| `magatama:agent` | 9 | 9/9 |
| `magatama:dm2` | 6 | 6/6 |
| `magatama:workflow` | 8 | 8/8 |
| `magatama:observability` | 8 | 8/8 |
| `magatama:forms` | 3 | 0/3 (`[DESIGN]`, world.wit 未 import) |
| `magatama:browser` | 3 | 3/3 |

### Verification

- `20-actors/magatama/magatama-ts/test/wit-coverage.test.ts` asserts that every `world.wit` import is mirrored in `src/imports.ts`.
- `20-actors/magatama/magatama-ts/test/wit-coverage.test.ts` asserts that every `world.wit` export is mirrored in `src/mod.ts`.
- `20-actors/magatama/magatama-ts/test/cypher-helpers.test.ts` covers the TypeScript Cypher encoding/decoding helpers, including `query-json`.
- `20-actors/magatama/magatama-ts/src/cbor.ts` + `src/remote.ts` provide CBOR-first typed remote-call/serve helpers with JSON fallback for legacy payloads.
