---
id: 260320-magatama-cloudflare-containers-evaluation
title: Magatama vs Cloudflare Containers Evaluation
status: active
...
# Magatama vs Cloudflare Containers Evaluation

Date: 2026-03-20

## Goal

`magatama` の現行 default である **split Workers**

- frontend: Svelte / AppShell Worker
- backend: TS native Worker

と、Cloudflare の `Static Frontend, Container Backend` 例に近い **frontend Worker + container backend** を比較し、`magatama` 全体設計としてどちらを標準に置くべきかを評価する。

この文書は「Cloudflare Containers を使うべきか」を一般論で語るものではない。`magatama` の app topology と traffic shape を前提に判断する。
...
`magatama` 全体の標準としては、**split Workers を維持するべき**である。

Cloudflare Containers は有用だが、`magatama` の primary workload である次の特性とは相性がよくない。
...
1. `magatama` の通常 app は control-plane heavy であり、Workers RPC / Service Binding の locality を活かしやすい
...
| Axis | Split Workers | Worker + Container backend | Verdict for `magatama` |
|---|---|---|---|
...
`magatama` default path は、主に次の課金で成立する。
...
したがって、`magatama` のような app 数の多い platform では、container backend を標準にすると **per-app の固定的なコスト要素が増えやすい**。
...
small payload の internal command/query を高頻度で行う `magatama` では、**split Workers の方が基本的に速い**。container backend は、low-latency typed internal API の標準には向かない。
...
- container backend は `magatama-server` 系 runtime 用 fallback として維持する
