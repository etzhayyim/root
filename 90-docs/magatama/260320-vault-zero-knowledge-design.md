...
- `magatama:secrets/vault` WIT interface の設計 (既存 `magatama:secrets/secrets` とは独立)
...
## Existing `magatama:secrets` との関係

| Interface | 役割 | Server trust |
|---|---|---|
| `magatama:secrets/secrets` | Flat KV secrets (host-encrypted AES-256-GCM) | **server-trusted** — host が平文を見る |
| `magatama:secrets/vault` (新規) | 1Password-style zero-knowledge vault | **zero-knowledge** — server は暗号文のみ |
...
```
User → kaiyaku:
  1. magatama:consent request (GNAP) → User 承認
...
Delegation (cross-principal item sharing) は `magatama:consent` が gate:
...
- Audit trail は `magatama:consent` graph に記録

## WIT Interface: `magatama:secrets/vault`

```wit
// magatama:secrets@1.0.0 package に追加
...
| Phase | 内容 | 依存 |
|---|---|---|
| **P0** | WIT interface (`magatama:secrets/vault`) + D1 schema + B2 storage | — |
| **P1** | Agent crypto (host-side HKDF + AES-GCM + X25519) | P0 |
| **P2** | Human crypto (AppShell JS SubtleCrypto + SRP) | P0 |
| **P3** | Consent-gated delegation (U2A, cross-actor) | P1 + `magatama:consent` |
