...
| Tailmesh peer disconnects | One node cannot reach others via `.mesh.gftd.ai` (pre-Step 8) or `.mesh.etzhayyim.com` (post-Step 8) | Restart mesh daemon: `pkill -f murakumo-mesh; ~/.gftd/mesh/run-mesh.sh &` (or `~/.etzhayyim/mesh/run-mesh.sh` post-Step 8) |
...
**Binary name change at cutover**: `gftd-murakumo` → `etzhayyim-murakumo`, `~/.gftd/` → `~/.etzhayyim/`, `.mesh.gftd.ai` → `.mesh.etzhayyim.com`.
