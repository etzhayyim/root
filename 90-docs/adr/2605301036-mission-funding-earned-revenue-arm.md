...
- **No new payment enum.** Vendor→religious-corp uses existing `donation`/`grant` (ADR-2605192130). Confirms `app.etzhayyim.gftd.apps.payment.sent` purpose enum is sufficient.
- **New lexicon** (`00-contracts/lexicons/app/etzhayyim/give/`): `vendorMissionDonationAttestation` — records a vendor surplus-donation event (period, gross surplus disclosed voluntarily, donated amount, recipient program, TitheRouter tx CID) for transparency; aggregate-only, no customer PII.
...
- `00-contracts/lexicons/app/etzhayyim/gftd/apps/payment/sent.json` (purpose enum — unchanged)
