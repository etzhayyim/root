...
- `curl -X POST 127.0.0.1:29030/xrpc/ai.gftd.apps.etzhayyim.kuniUmi.defineDeploymentSite` runs the full SiteSurveyCell LangGraph (8 nodes) and returns `ok=true, latencyMs=6.74, state.jurisdiction_ok=true, state.charter_rider_ok=true, state.submission_at_uri=at://...`.
