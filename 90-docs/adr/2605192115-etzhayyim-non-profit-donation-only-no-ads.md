...
- internal-purchase の record (`ai.gftd.apps.payment.sent` with purpose=`internal-purchase`) は **必ず両者の SBT tokenId を含む**
...
- 役員報酬 (もしあれば) — `ai.gftd.apps.etzhayyim.officer-compensation` AT Record として MST に公開 (current value: 0 — 役員は無報酬)
