...
title: "ADR-2605231630: LangGraph + yatachain + langserver canonical agent runtime; Goose retired; K8s/k3s/GPU-pod reintroduction approved"
status: proposed
doc_type: adr
...
axis: architecture
weight: 0.60
priority_note: "operator directive 2026-05-23 — replaces the Goose+RunPod-only stack with a LangGraph+yatachain+langserver+k3s religious-corp runtime"
authoritative_for:
...
# ADR-2605231630: LangGraph + yatachain + langserver canonical agent runtime; Goose retired; K8s/k3s/GPU-pod reintroduction approved
...
**(1) Canonical agent runtime — etzhayyim/* scope.** The triple `(LangGraph, yatachain, langserver)` is the single canonical agent runtime for religious-corp activity in `etzhayyim/root`. Specifically:
...
- `roles/goose/` Ansible role: marked retired; new `purge_goose.yml` idempotent task removes `~/.config/goose/`, the wrapper script, and `~/.gftd/goose-cron-wrapper.sh` on next apply.
...
- **k3s** on Lima VMs across the Mac mini fleet — production substrate for religious-corp cells per ADR-2605232100. Ansible playbook `60-apps/ai-gftd-project-murakumo/ansible/k8s-gpu-cluster.yml` (Stage 3) is the canonical bootstrap path.
...
**(4) Murakumo CLAUDE.md update.** `60-apps/ai-gftd-project-murakumo/CLAUDE.md` is updated in the same change set as this ADR to:
...
- Single canonical agent runtime — operators and contributing agents do not have to decide whether a new pipeline lives in a Goose recipe vs a LangGraph cell vs a launchd plist. It's always a LangGraph cell wrapped by langserver, anchored to yatachain.
...
| Keep K8s 禁止; run religious-corp cells as launchd daemons (ADR-2605191346 §3 original path) | ADR-2605232100 documented why launchd cannot express the declarative placement, rolling update, sidecar dependency, swarm Lease, and quorum invariants that yatachain cells need. Re-arguing the same case repeatedly is unproductive. |
...
- ADR-2605231400 — yatachain Holochain-iso substrate (defines `chain` in "langgraph + chain + server")
...
- `60-apps/ai-gftd-project-murakumo/CLAUDE.md` — Murakumo project SoT; edited in the same commit as this ADR
- `60-apps/ai-gftd-project-murakumo/ansible/k8s-gpu-cluster.yml` — Stage 3 bootstrap playbook
