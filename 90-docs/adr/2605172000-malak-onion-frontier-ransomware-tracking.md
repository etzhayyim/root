...
authoritative_for:
  ...
  - RisingWave-compatible publish pattern for ransomware_actor_activity
priority: 8.4
...
- `crawler-resident` and the CronJob parsed run IDs with brittle grep
  patterns.
- `ransomware_actor_activity` attempted `ON CONFLICT`, which RisingWave does
...
5. `ransomware_actor_activity` consumes onion metadata and publishes Yabai
   rows with RisingWave-compatible `DELETE -> INSERT` writes instead of
   `ON CONFLICT`.
