...
authoritative_for:
  - ai.gftd.apps.tsukuru.industryActor.*
  - tsukuru-isic-bpmn-actors
  - tsukuru-euv-bpmn-actors
  - ai.gftd.apps.tsukuru.euv.*
  - ai.gftd.apps.tsukuru.supplierExchange.*
related:
  - 0056-bpmn-as-actor
...
- tsukuru public XRPC queries:
  - `ai.gftd.apps.tsukuru.industryActor.getIndustryActor`
  - `ai.gftd.apps.tsukuru.industryActor.listIndustryActors`
...
|---|---|---|
| `tsukuru_euv_lithography_manufacturing_flow` | `ai.gftd.apps.tsukuru.euv.designManufacturingFlow` | Design EUV lithography manufacturing phases and CAD/CAM handoff gates |
| `tsukuru_normalize_supplier_exchange_package` | `ai.gftd.apps.tsukuru.supplierExchange.normalizePackage` | Normalize AutoCAD/Fusion/STEP/IGES/glTF artifacts into a supplier exchange envelope |
| `tsukuru_validate_supplier_exchange_package` | `ai.gftd.apps.tsukuru.supplierExchange.validatePackage` | Validate EUV supplier readiness and return `ready/blockers` without writing records |
| `tsukuru_prepare_euv_order_package` | `ai.gftd.apps.tsukuru.euv.prepareOrderPackage` | Compose EUV flow and supplier exchange package in one call |
| `tsukuru_get_euv_implementation_coverage` | `ai.gftd.apps.tsukuru.euv.getImplementationCoverage` | Report implementation coverage, manifest checks, score, and required capabilities |
...
`ai.gftd.apps.tsukuru.euv.getImplementationCoverage` is the runtime-readable
