...
4. **Lexicon namespace** — `org.etzhayyim.authz.*` (new) vs reusing `ai.gftd.authz.*` with prefix.
...
| NSID | Type | Replaces |
|---|---|---|
| `org.etzhayyim.authz.beginRootProvision` | procedure | vendor `ai.gftd.authz.linkEthereumBegin` |
| `org.etzhayyim.authz.completeRootProvision` | procedure | vendor `ai.gftd.authz.linkEthereumVerify` |
...
The `org.etzhayyim.*` namespace mirrors the existing `org/etzhayyim/yobel/` pattern in this repo. Vendor's `ai.gftd.authz.*` remains valid during the migration window; only **new** etzhayyim-scope identity ops use the `org.etzhayyim.*` namespace.
...
6. **Keep `ai.gftd.authz.*` namespace and add `etzhayyim:` prefix.** Rejected: AT NSIDs are namespace-purpose-method triples; bolting on a prefix breaks tooling that splits by dots. `org.etzhayyim.authz.*` is the clean separation and matches the existing `org/etzhayyim/yobel/` precedent.
