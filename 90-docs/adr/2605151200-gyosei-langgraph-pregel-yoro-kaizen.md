...
The existing infrastructure relied on static BPMN task definitions (`xrpc.ai.gftd.govXXX.*`) which did not scale gracefully to hierarchical agency discovery, nor did they natively support conversational intake or continuous improvement (Kaizen) loops for the legal schemas.
...
3. **Continuous Evaluation Loop (`gyosei-procedure-kaizen-pregel`)**
   - Implemented an autonomous quality assurance graph that periodically compares the GFTD modeled schema (`current_schema`) against real-world scraped texts (`official_source_data`).
   - If discrepancies (gaps) are detected (e.g., score < 100), the LLM generates JSON-Patch directives.
   - Outputs are persisted to a new `ai.gftd.apps.gyosei.kaizenReport` lexicon record for audit and application.
