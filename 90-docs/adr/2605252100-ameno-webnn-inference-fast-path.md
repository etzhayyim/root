...
status_note: "R0 scaffold. WebNN is a W3C Candidate Recommendation Draft (2026-05-21) for **neural network INFERENCE acceleration**; training (backward / gradient / optimizer) is explicitly out of scope. ameno adopts WebNN as the inference fast path that maps to OS-native NPU stacks (CoreML / DirectML / NNAPI → TFLite delegate / Qualcomm Hexagon via NNAPI-QNN delegate). Federated training (ADR-2605242600 / 2605242630) MUST remain on WebGPU — WebNN cannot carry it by design. Safari/WebKit has no WebNN signal, so WebGPU is the mandatory universal fallback."
authoritative_for:
