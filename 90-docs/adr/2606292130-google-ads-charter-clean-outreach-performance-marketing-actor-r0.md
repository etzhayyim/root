...
- **Outward registration (owner-authorized 2026-06-29, standing-auth per CLAUDE.md «Actors»)**: ✅ split repo `github.com/etzhayyim/com-google-ads` (private, HEAD `e9ffefb`) created + pushed; ✅ west entry — `repos.edn` `:extra-projects` + `west.yml` block (pin `e9ffefb` == child HEAD) via API single-entry commit `d57f86f` on `com-junkawasaki/root` main (survived a concurrent rebase to tip `640a8f9`); ✅ `actor-profile-seed.kotoba.edn` SSoT entry added (apex `did:web:etzhayyim.com:actor:com-google-ads`, glyph 広). ⏳ signed RAD identity journal (`:rad/repo "github.com/etzhayyim/com-google-ads"`, `:rad/did-web "did:web:etzhayyim.github.io:com-google-ads"`, `:rad/name "com-google-ads"`, `:rad/aozora-collection "com.etzhayyim.apps.googleads"`) — operator-key step via `70-tools/src/etzhayyim/kotoba_rad_sign.clj` (no-server-key; the seed entry is the precursor). Landed in an isolated root worktree + PR per root CLAUDE.md (the shared root checkout is raced).
