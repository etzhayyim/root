...
authoritative_for:
  - donation 時の successor steward 事前指定 procedure (`ai.gftd.apps.etzhayyim.steward-succession-declaration`)
  - donor 死亡時の succession trigger + activation flow (`ai.gftd.apps.etzhayyim.steward-succession-event`)
...
寄付者は寄付完了時に以下を含む successor 指定 record (`ai.gftd.apps.etzhayyim.steward-succession-declaration`) を作成:

```json
{
  "$type": "ai.gftd.apps.etzhayyim.steward-succession-declaration",
  "landId": 1234,
  "primarySuccessor": {
...
- 各 successor は事前に `ai.gftd.apps.etzhayyim.steward-succession-pre-acceptance` で承諾 sign 済み
...
`ai.gftd.apps.etzhayyim.steward-succession-event`:

```json
{
  "$type": "ai.gftd.apps.etzhayyim.steward-succession-event",
  "landId": 1234,
...
```
[死亡 / トリガー event]
  → ai.gftd.apps.etzhayyim.steward-succession-event record 起票
