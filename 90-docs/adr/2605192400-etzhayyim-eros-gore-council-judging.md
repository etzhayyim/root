...
authoritative_for:
  ...
  - precedent registry Lexicon (`ai.gftd.apps.etzhayyim.eros-gore-precedent`)
...
Layer 2: Council Lv6+ deliberation (3 名以上 multisig)
   ↓ (precedent setting)
Layer 3: Precedent Registry (`ai.gftd.apps.etzhayyim.eros-gore-precedent`)
```
...
```
ai.gftd.apps.etzhayyim.eros-gore-judging request
...
Council 判決は precedent として `ai.gftd.apps.etzhayyim.eros-gore-precedent` に永続記録される:

```json
{
  "$type": "ai.gftd.apps.etzhayyim.eros-gore-precedent",
...
```
ai.gftd.apps.etzhayyim.eros-gore-appeal
