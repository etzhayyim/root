...
magatama host-sdk に **2 つの加法的 Hono サブアプリ**を追加する。XRPC (`/xrpc/:nsid`) は完全に無変更で残す (AT-native client 向け、ADR-0022 Service Auth 経路のまま)。

## D1. Per-actor MCP server at `/mcp`

各 magatama actor Worker に **MCP Streamable HTTP endpoint** を実装:
...
```
00-contracts/lexicons/ai/gftd/apps/**/*.json
...
**Positive:**
- LangGraph / OpenAI Apps SDK / Claude Desktop が **per-actor で magatama の tool を bind できる**。scope の最小化 = Shannon η 向上。
...
- `20-actors/magatama/sdk/magatama-host-sdk/src/app-metadata.ts` — `buildActorCardFromCommands()` (tool export SSoT)
- `20-actors/magatama/sdk/magatama-host-sdk/src/host-web-router.ts` — Hono router 現行 12 ルート
