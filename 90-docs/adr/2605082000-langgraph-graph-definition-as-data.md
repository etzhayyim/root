...
```jsonc
{
  ...
  "condition_ref": "dmn:ai.gftd.policies.intent.escalation@1.0.0"
}
```
...
- `vertex_langgraph_graph_def` への write は L4 MCP tool (`ai.gftd.tools.langgraph.publish`)
  経由のみ許可 (生 SQL 禁止)
...
- `ai.gftd.tools.time.now` — 壁掛け時計 (`iso` / `epoch_s` / `epoch_ms`、
...
- `ai.gftd.tools.crypto.hash` — 内容アドレス hash (`sha256` / `sha1` /
...
[supervisor → state.<key>_input]
    ↓ kind=mcp_tool ref=mcp://ai.gftd.tools.llm.chat
...
**`ai.gftd.tools.sql.insert_row`** — 動的 row → INSERT。
