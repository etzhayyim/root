---
id: adr-2605232101-etzhayyim-organism-vertical-implementation
renumbered_from: "2605232100"
title: "ADR-2605232101: etzhayyim organism vertical — UNSPSC corpus + kuni-umi runtime + libp2p transport (11-iteration kaizen session)"
status: accepted
doc_type: adr
...
authoritative_for:
  ...
  - 6 kuni-umi phase cell.py implementations (canonical at 40-engine/kotoba/crates/kotoba-kotodama/cells/*; 20-actors/kuni-umi/cells/* are symlinks)
  ...
  - kotodama.transport.libp2p Python wrapper around etzhayyim-libp2p shell scripts
  - cell-runner KUBO_LIBP2P auto-expose hook for lan-api cells
  - kotodama.adapters.open_utility 14 CIM stubs
...
# ADR-2605232101: etzhayyim organism vertical — UNSPSC corpus + kuni-umi runtime + libp2p transport
...
Canonical path: `40-engine/kotoba/crates/kotoba-kotodama/cells/{site_survey,deployment_planning,…}/`.
ADR-2605201400 actor topology path `20-actors/kuni-umi/cells/*/cell.py` is now a
symlink set pointing into the kotodama path. Older 152-185 LOC stubs preserved
...
- **KuniUmiApiCell** (`40-engine/kotoba/crates/kotoba-kotodama/cells/kuni_umi_api/cell.py`, 663 LOC) — aiohttp LAN service on naphtali:13030 exposing all 6 lexicon endpoints as `/xrpc/{nsid}` + `/api/{short}` + `/api/invoke` aliases. Witness quorum pre-check (HTTP 400), GeoJSON validation, GraphRecursionError → HTTP 202 AwaitingWitnessQuorum, `KUNI_UMI_API_DEV_MODE` env knob.
...
`kotodama.adapters.open_utility` (714 LOC) — 14 stub functions for the open-*
...
`40-engine/kotoba/crates/kotoba-kotodama/py/tests/test_kuni_umi_e2e.py` (~1000 LOC, **28 tests across
...
`40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/transport/libp2p.py` (551 LOC, stdlib-only):
...
`kotodama/cell_runner_main.py` (+182 LOC):
...
def _ensure_libp2p_tunnel(shard) -> str | None:
    # dial via kotodama.transport.libp2p.dial_peer, cache, return URL
...
4. **MST production path not exercised** — `handle_mst_event` is wired but the cell-runner mst-listener subscriber requires `etzhayyim_sdk.cursor` which isn't installed in kotodama venv. Dev path (HTTP gateway) bypasses MST entirely.
...
| Use FastAPI / starlette for KuniUmiApiCell | aiohttp matches the existing UnispscAgentExecutorCell pattern, already in kotodama venv. No new dep. |
...
- `40-engine/kotoba/crates/kotoba-kotodama/py/tests/test_kuni_umi_e2e.py` — 28-test corpus invariant
