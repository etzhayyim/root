...
`bankruptcy.gftd.ai` (84 jurisdictions, ~170 process types) は **個別の法的負債清算**を網羅する — Chapter 7/11/13、自己破産、IVA、Restschuldbefreiung、UNCITRAL Model Law。`lawfirm.gftd.ai` (ADR-0016) は代理人レイヤを提供する。
...
これらは:
1. **法的破産手続 (bankruptcy.gftd.ai) と本質的に異なる** — voluntary creditor opt-in / 教義的権威 / 集合的契機
...
**`yobel.gftd.ai`** を新規 actor として設計する。

| 項目 | 値 |
|---|---|
| **Name** | yobel (יובל, ram's horn / Jubilee の語源) |
| **Operating entity** | **etzhayyim** (運営法人。Gftd Japan vendor の関与なし) |
| **Destination repo** | `github.com/etzhayyim/root` (Apache 2.0、open religious-corp monorepo) |
| **License** | Apache 2.0 |
| **DID** | `did:web:yobel.etzhayyim.com` (primary) + `did:web:yobel.gftd.ai` (federation alias only, AT layer) |
...
| **NSID** | `org.etzhayyim.yobel.*` (canonical, etzhayyim/root cutover 後) / `ai.gftd.apps.yobel.*` (transitional alias 維持) |
...
```
yobel.gftd.ai (etzhayyim, voluntary rite)
   ├── falls back to → bankruptcy.gftd.ai  (formal legal procedure, jurisdictionally required)
   ├── represented by → lawfirm.gftd.ai    (creditor consent letters, court filings)
   ├── eligibility   ← trust.gftd.ai       (kyu/dan + Well-Becoming filter, optional)
...
|---|---|---|
| `ai.gftd.apps.bankruptcy.recordYobelRiteReference` | vendor:bankruptcy ← (queries) → etzhayyim/yobel | Attach a yobel rite reference to a formal bankruptcy case. Handler resolves rite state via `YobelRiteRegistry.getRite()` on Base L2 (no trust required — independent chain verification). |
...
- Tax advice delegated to vendor:lawfirm.gftd.ai (vendor handles secular tax procedure)
...
- bankruptcy.gftd.ai (mandatory legal) と yobel.gftd.ai (voluntary doctrinal) の責務分離が明確
...
- **税務影響** — debt forgiveness は jurisdiction によって課税所得を生む (e.g. US IRC §61(a)(11) cancellation-of-debt income)。actor は jurisdiction-specific `jurisdictionNotes` を保持し、warning を返すが、税務 advice は提供しない (→ lawfirm.gftd.ai に delegate)
- **法的拘束力の限界** — voluntary opt-in のため secular creditor は無視可能。formal binding が必要な場合は bankruptcy.gftd.ai fallback
...
- NSID `org.etzhayyim.yobel.*` を canonical、`ai.gftd.apps.yobel.*` を transitional alias とする (ADR-2605152100 GitHub org split に整合)

# Alternatives Considered

**A. Extend bankruptcy.gftd.ai with a `voluntary_collective_release` process type** — 却下。bankruptcy actor は**法的強制力ある手続**の intelligence + 代行を扱う。voluntary doctrinal rite を同居させると domain confusion + vendor (Gftd Japan) と etzhayyim の責任境界が曖昧になる (3-axis split 違反)。

**B. 4-actor cluster (`shmita.gftd.ai` + `yobel.gftd.ai` + `tokusei.gftd.ai` + `amnesty.gftd.ai`)** — 却下。データモデル (debt registration + voluntary opt-in + release tx) はすべて同一。rite type discriminator で 1 actor に統合する方が Shannon-optimal (η 高、operational overhead 低)。

**C. Vendor placement (`ai.gftd.apps.yobel.*` only, this repo)** — 却下。user 明示で「非営利 → etzhayyim」。3-axis split: liability (religious doctrine) / custody (collective ritual records) / settlement (USDC on Base L2, no fiat) すべて etzhayyim clean。
