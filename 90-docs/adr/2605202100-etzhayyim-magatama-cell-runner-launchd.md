---
id: adr-2605202100-etzhayyim-magatama-cell-runner-launchd
title: "ADR-2605202100: magatama-cell-runner launchd LaunchAgent — operationalising Tier 1 常駐稼働 on the Murakumo fleet"
status: proposed
doc_type: adr
topic: magatama-cell-runner-launchd
authoritative: true
last_verified: 2026-05-20
priority: 6.0
axis: operations
weight: 0.60
priority_note: "ADR-2605192100 / 2605192415 / 2605201400-2605201900 で religious-corp daemon + kuni-umi roadmap が spec として完成。一方 `pymagatama.cell_runner_main` は CLI として存在するが、Murakumo 10 Mac mini fleet 上で 常駐起動する OS-level boot path が未整備だった。本 ADR は launchd plist template + per-host installer + uninstaller を 50-infra/cluster/murakumo/cell-runner/ に固定し、cell.py 完成度とは independent に 'fleet が常駐起動できる' 状態を closure する。`start_cell` の subprocess spawn / MST listener / healthz / swarm heartbeat は引き続き staged maturity (TODO comment 残置)。"
authoritative_for:
  - launchd plist template for magatama-cell-runner
  - Per-host installer (`install.sh --node <tribe>`) + uninstaller
  - pyproject `[project.scripts]` magatama-cell-runner entry
...
# ADR-2605202100: magatama-cell-runner launchd LaunchAgent
...
ADR-2605192415 §7.1 で religious-corp daemon の Tier 1 常駐稼働 substrate を Murakumo Mac mini fleet + launchd と確定し、`pymagatama.cell_runner_main` CLI を実装した。ADR-2605201400 〜 2605201900 で kuni-umi 6 cell + roadmap S0-S5 も追加した。fleet.toml には religious-corp 15 cell + kuni-umi 6 cell の per-node 割り当てが書かれている。

しかし **launchd で常駐起動する OS-level boot path が未整備**:

- `[project.scripts] magatama-cell-runner` が pyproject.toml に存在しなかった → CLI は `uv run python -m pymagatama.cell_runner_main` 経由でしか起動できない
- `com.etzhayyim.magatama-cell-runner.plist` template が存在しなかった (ameno-daemon / litellm の plist はあるが cell-runner 用は欠落)
...
`20-actors/magatama/py/pyproject.toml` に追加:
...
これで `uv run magatama-cell-runner --node <tribe>` が CLI として呼べる。`uv pip install -e .` した venv では `magatama-cell-runner` が PATH に直接出る。launchd の `ProgramArguments` も `uv run` 経由で呼ぶ — `uv` が venv resolution + dependency sync を担当。

## 2. launchd plist template

`50-infra/cluster/murakumo/cell-runner/com.etzhayyim.magatama-cell-runner.plist` を **per-host placeholder template** として固定 (literal install ファイルではない):
...
- `ProgramArguments`: `uv run --project @@REPO_PATH@@/20-actors/magatama/py magatama-cell-runner --node @@NODE_NAME@@ --log-level INFO`
...
- Stdout / stderr separate logs in `@@LOG_DIR@@/magatama-cell-runner.{stdout,stderr}.log`
...
3. `uv sync` to ensure pymagatama venv ready
4. Run `magatama-cell-runner --node <tribe> --health` as pre-flight config readback (no side effects)
...
- MST listener 起動 — `pymagatama.listener.MstListener` の per-cell wiring
...
- `magatama-cell-runner` が PATH-resolvable な CLI として exposed → 開発者が `uv run magatama-cell-runner --node naphtali --health` で local dry-run 可能
...
- pyproject `[project.scripts]` 追加は `pymagatama` ホイールビルドに `magatama-cell-runner` console-script が含まれる → 既存 pymagatama 利用箇所 (worker pods / langserver etc.) の wheel rebuild が必要 (但し既存 import path は無変更)
...
4. **`magatama-cell-runner` console-script 追加による wheel rebuild scope** — 既存 pymagatama 利用 worker pod は impact 0 想定だが、wheel cache invalidation 順序を CI で confirm。Decision: 本 ADR 範囲外、次回 worker pod deploy で観測
...
- `50-infra/cluster/murakumo/cell-runner/com.etzhayyim.magatama-cell-runner.plist` — plist template
...
- `20-actors/magatama/py/pyproject.toml` `[project.scripts]` — magatama-cell-runner CLI entry
- `20-actors/magatama/py/src/pymagatama/cell_runner_main.py` — CLI implementation (scaffold)
