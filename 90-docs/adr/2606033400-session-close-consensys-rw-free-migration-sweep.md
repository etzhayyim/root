...
# ADR-2606033400: Session close — Consensys-pattern rw-free migration sweep
...
|--------|------:|---------|
| A — DONE | 105 | has a committed `rw-free/src/index.ts` |
...
either a committed `rw-free/src/index.ts` OR a unique `^- **<app>**` Bucket V
...
1. **6 Bucket-A phantoms** — `6ir`, `air-sched`, `analytics`, `bim`,
   ...
   `rw-free/src` (only stray `node_modules`) and no source in either repo.
...
The loop's STEP-1 PICK rule was *"skip any app with an existing `rw-free/src` OR
already in Bucket V."* The implementation checked `rw-free/src` but **not Bucket V
...
check Bucket V membership in PICK**, not just `rw-free/src`. (This is honest
