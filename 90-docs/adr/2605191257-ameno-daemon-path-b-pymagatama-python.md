---
id: 2605191257-ameno-daemon-path-b-pymagatama-python
...
ADR-2605191229 で Path A(Bun/Node + Hono + LangGraph TS)を投入した。これは速い ROI のための **dev / single-machine** 用途には十分だが、Tier 1 = Murakumo Mac mini fleet が稼働する `pymagatama` Python LangGraph stack(ADR-2605182312, ADR-2605180900)とはコード資産が分かれている。

artificial-organism ecosystem 全体の **正本(Source of Truth)** は Python `pymagatama` 側にある:

- `pymagatama.local_llm` — Ollama / MLX 統合済の Async HTTP client
- `pymagatama.projects.uhl_right_neural` — 既存 LangGraph Pregel project の良いテンプレート
- `pymagatama.langserver_compat` + `langgraph_server_app` — FastAPI / Granian server runtime
- `pymagatama.checkpointer` — checkpoint 抽象、MstCheckpointSaver sidecar (ADR-2605171800) と連動
- `pymagatama.ameno_handlers` — 既存の PDS XRPC saveResult/listHistory 永続化(server-side persistence layer)
...
**`pymagatama.projects.ameno` を新設し、既存 Python LangGraph 資産で Path A と同じ agent loop を再実装する。**

### Module 構成

```
20-actors/magatama/py/src/pymagatama/projects/ameno/
├── __init__.py
├── __main__.py              # python -m pymagatama.projects.ameno
...
├── kustomization.yaml
└── lg-ameno.yaml            # 2-container topology (pymagatama + checkpointer sidecar)
                              # ADR-2605171800 と同 shape
```

`pymagatama.ameno_handlers`(既存 XRPC saveResult ハンドラ)と **同 pod 同居** が可能 — `ameno_handlers` は PDS write-side(従来の svelte → CF Worker → langserver の bare metal 受け口)、 新規 `projects.ameno` は agent loop。互いに独立、同 K8s namespace で並走。
...
- ameno が **Tier 1 (Python pymagatama) の正本** を持つ。Murakumo fleet の lg-* pod 群と同 shape で deploy 可能 → K8s で水平スケール
...
2. **TS daemon (Path A) を CLI として pymagatama から呼ぶ** — multi-language IPC のオーバーヘッド + 2 つの runtime を pod に置く重さ。reject
...
- `pymagatama.local_llm`, `pymagatama.ameno_handlers`(既存資産)
