---
id: adr-0066-kotodama-organizer-shinka-shannon-bayes
title: "ADR-0038: Kotodama Organizer — joucho Shinka × Shannon Optimization × Bayesian Evaluation"
status: accepted
doc_type: adr
topic: kotodama-organizer
authoritative: true
...
# ADR-0038: Kotodama Organizer — joucho Shinka × Shannon Optimization × Bayesian Evaluation
...
The kotodama platform (`kotodama.etzhayyim.com`) surfaces goose agent activity from
...
1. **joucho shinka heartbeat** (`kotodama:kotodama/shinka@1.0.0`) — a
...
Introduce a **repo-wide organizer** inside the `kotodama.etzhayyim.com` Worker
...
```
 ...
 │  R2 kotodama/plan-current.json + hourly history snapshots              │
 └───────────────────────────────────────────────────────────────────────┘
             ↓   (next crontab tick)
 ┌─ goose cron wrapper ──────────────────────────────────────────────────┐
...
The existing `etzhayyim:kotodama/shinka@1.0.0` WIT produces a per-actor
...
(`x-kotodama-verified: true` internal bypass; model `gemma3-1b`; 120
...
- `kotodama/plan-current.json` — latest plan, replaced every 5 min
- `kotodama/plan-history/{YYYY-MM-DDTHH}.json` — hourly snapshot for trend
...
- ADR-0034 — agent-cron-goose-kotoba-direct
- `50-infra/cloudflare/workers/kotodama/src/worker.ts` — implementation
- `60-apps/etzhayyim-project-kotodama/CLAUDE.md` — operator runbook
