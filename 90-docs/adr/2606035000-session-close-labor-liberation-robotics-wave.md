...
4. **RisingWave-free reaffirmed.** Per owner directive + ADR-2605172000: the legacy `60-apps/ai-gftd-project-recap/` (psycopg/RisingWave, with the `rw_…` placeholder credential class of ADR-2605173100) was **excluded then removed** from the tree. The credential is a documented placeholder (real RW root already rotated, ADR-2605173100 §61/§18) — no rotation needed. A clean manako/unspsc subset was committed+pushed separately on `feat/sumitsubo-cad-interop`.
