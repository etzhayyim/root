...
One BPMN file per stage. Filenames: `etzhayyim-root/00-contracts/bpmn/ai/gftd/animeka/{stage}.bpmn`. Each stage registers as one `vertex_bpmn_process_def` row + one `vertex_bpmn_lexicon_binding` row (per ADR-0056 canonical vertex_id scheme).
...
- Reference BPMN patterns: `etzhayyim-root/00-contracts/bpmn/ai/gftd/{llm/chat,yabai/triagePoc,bot/reviewAndPost}.bpmn`
