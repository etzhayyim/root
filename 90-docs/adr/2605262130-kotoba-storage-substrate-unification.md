...
**ADR hierarchy**: Substrate-engine charter. Names a single Rust workspace (`40-engine/kotoba`) as the canonical storage substrate for religious-corp `etzhayyim/root`. Supersedes the prior "yatachain composition spec + projection escape-hatch" framing (ADRs 2605231400, 2605231500, 2605232300, 2605232400, 2605101000). Does **not** modify constitutional invariants: the on-chain land / SBT / Council / Public Fund substrate (ADR-2605192245 + ADR-2605192300), the `app.etzhayyim.encrypted.*` wire format (ADR-2605181100 + ADR-2605181200), the Murakumo-only inference invariant (ADR-2605215000), and the RW-free + payments-on-chain invariants (ADR-2605172000 + ADR-2605172100) are all preserved bit-identically. What changes is **which Rust workspace computes the CID, walks the MST, indexes the Datalog, terminates the libp2p stream, and applies the XChaCha20 envelope**.
...
3. Strengthens the **no-RisingWave invariant** to also cover the projection backend slot: RisingWave / Postgres / Lance / DuckDB / SQLite are now prohibited as **read backends** as well as primary write stores. The only authorized hot-path read engine is `kotoba-kqe` arrangements over content-addressed blocks.
...
- ADR-2605101000 designed a BigQuery → RisingWave projection. This ADR retires that projection design; BigQuery ingest (if it survives at all) routes to `kotoba-kqe` directly, with the same MST-anchored receipts but no RW side store.
