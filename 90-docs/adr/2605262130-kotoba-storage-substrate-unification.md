...
`50-infra/ipfs-pinner/` (Phase 1 cold backup) + `50-infra/mst-projector/` (Phase 2 / 2.5) + `50-infra/nats-jetstream-{kv-resp, objectstore-s3}/` + `50-infra/nats-tiered-storage/` (Phase 3) + `50-infra/lancedb-wasm/` (Phase 4, formally retired as no-active-use vendored snapshot) + `50-infra/etzhayyim-xrpc-proxy/` (Phase 6) + `50-infra/tonbo/` (Phase 2 retire window) all REMAIN in tree, REMAIN listed in `CLAUDE.md` `## Repo Layout`, and REMAIN deployable. Deletion (if ever) is a separate ADR; this ADR does not authorize it.
