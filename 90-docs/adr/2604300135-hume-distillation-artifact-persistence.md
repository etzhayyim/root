...
title: Hume Distillation Artifact Persistence to IPFS and RisingWave
status: active
doc_type: adr
...
- Trainers can discover available artifacts through RisingWave without loading
  large JSONL/model bodies into the database.
- The first implementation uses the existing ingest spine, so no RisingWave DDL
...
- Store full JSONL/model bodies in RisingWave. Rejected because it mixes large
