...
4. **Routing Gateway Exception**: Since the dashboard is hosted on Cloudflare Pages (`ai-gftd-project-itonami.pages.dev`), we updated the `routing-gateway` Cloudflare Worker at `*.etzhayyim.com` to passthrough requests for `itonami` while stripping the `Host` header to prevent Cloudflare 522 infinite loop timeouts.
