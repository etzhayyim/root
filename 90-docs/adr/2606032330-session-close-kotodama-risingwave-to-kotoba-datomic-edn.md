---
id: adr-2606032330-session-close-kotodama-kotoba-to-kotoba-datomic-edn
title: "ADR-2606032330: Session close — kotodama Kotoba/Datomic → kotoba Datomic API/EDN refactor (substrate clients + LangGraph savers + sqlmesh view registry)"
status: active
doc_type: adr
topic: session-close-kotodama-rw-to-kotoba
authoritative: false
...
# ADR-2606032330: Session close — kotodama Kotoba/Datomic → kotoba Datomic API/EDN
...
This is the **kotodama application-layer** counterpart to the kotoba-engine-internal
...
(Datom log = first-class canonical state). This session ported the kotodama Python
...
(1) the `kotodama/py` worker framework (LangGraph persistence + `rw_async_pool`/
...
- **kotoba Datomic substrate client — TypeScript** `40-engine/kotoba/crates/kotoba-kotodama/sdk/kotodama-host-sdk/src/kotoba-datomic.ts`.
...
- **sqlmesh materialized-view → kotoba view registry** `40-engine/kotoba/crates/kotoba-kotodama/py/sqlmesh/sqlmesh_to_kotoba.py`
...
- **Substrate clients + LangGraph savers + view registry: LANDED** (standalone, tested;
  ...
  **untracked** in the working tree and were authored against the legacy `kotodama/py`
