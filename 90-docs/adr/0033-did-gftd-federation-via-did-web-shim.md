...
related:
  ...
  - 90-docs/adr/0029-did-gftd-recursive-hash-merkle.md
  - 90-docs/adr/0030-did-gftd-recursive-adoption-rollout.md
supersedes:
...
ADR-0029 で採用した `did:gftd` recursive Merkle method は、数千億 (10^11) order の
...
成立せず、`did:gftd` の federation 経路を **plc.directory に一切依存しない形** で
再設計する必要がある。
...
`did:gftd` を 3rd-party で実装 / PR させるのは ecosystem outreach cost が大きい。
...
- **Resolution**: `GET /{s0}/.../did.json` → D1 / RisingWave から `vertex_gftd_identity`
...
## Why did:web shim instead of native did:gftd PR to @atproto/identity?
...
## C. `@atproto/identity` に did:gftd resolver PR (将来 option、非採用)
...
2 identity 並走 (did:gftd primary + did:plc shadow) の運用 cost が高い。しかも
...
- migration: `vertex_gftd_identity` に `federated BOOLEAN DEFAULT false` + index
- `70-tools/gftd/gftd/actor_federate.go` — `gftd actor federate --did {did}` CLI
...
- ADR-0030 — did:gftd rollout (adoption schedule)
