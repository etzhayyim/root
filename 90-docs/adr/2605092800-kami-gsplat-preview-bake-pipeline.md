...
- `vertex_maps_gsplat_asset` — splat 1 ファイル = 1 行
  - PK: `vertex_id` (`at://{authority}/ai.gftd.apps.maps.gsplatAsset/{rkey}`)
...
`ai.gftd.apps.maps.*` 名前空間に 3 メソッドを追加する。

| NSID | 種別 | 役割 |
|---|---|---|
| `ai.gftd.apps.maps.getGsplatAsset` | query | 単一 asset 取得 (preview 配信) |
| `ai.gftd.apps.maps.listGsplatAssets` | query | tile_h3 / bbox / source_did で一覧 |
| `ai.gftd.apps.maps.bakeGsplatAsset` | procedure | bake job を BPMN へ enqueue |

`getGsplatAsset` は B2 signed URL + metadata を返す (binary は B2 直配信)。
`bakeGsplatAsset` は `sdk.zeebe.publishMessage({ name: "ai.gftd.apps.maps.bakeGsplatAsset", correlationKey: vertex_id, ... })` で k8s pod に委譲する (ADR-2604251830 L7)。
...
`ai.gftd.apps.maps.getGsplatCostSummary` (query, no params) が 3 つの
...
| NSID | Type | 用途 |
|---|---|---|
| `ai.gftd.apps.maps.getGsplatJobStatus` | query | `mv_maps_gsplat_job_latest` から jobId 一発 |
| `ai.gftd.apps.maps.listGsplatJobs` | query | tile / kind / status filter + pagination |
