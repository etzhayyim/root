...
| 4 | `DATABASE_URL` (Kotoba/Datomic creds) | bulk-ingest pods + Worker `HYPERDRIVE` | Direct INSERT into `vertex_spatial` and friends | Community operator publishes signed AT records → etzhayyim relay subscribes → kotoba-datomic-projection rebuilds the read cache (ADR-2605231500). |
