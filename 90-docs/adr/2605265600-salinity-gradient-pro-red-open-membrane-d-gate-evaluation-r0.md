...
| Phase | Scope | Cap |
|---|---|---|
| **R0** | This ADR; **LANDED 2026-06-16** as runnable methods — `20-actors/funamori/` actor instantiated: `methods/salinity_gradient.cljc` (PRO/RED physics + §1 gates as throwing `ex-info` assertions) + `methods/stack_robotics.cljc` (install/maintenance robotics design — boustrophedon anti-fouling coverage sweep per §5 + EOL membrane-module swap, over the shared kuni-umi substrate of ADR-2606091800; all dry-run/no-server-key); **36 cljc tests / 88 assertions green** via babashka; `kotoba/{schema,seed}.edn` (`:funamori.salinity.*` + `:funamori.robotics.*` EAVT), 3 lexicons (§6), `manifest.edn` (15 gates). Physics validated vs the Table (PRO 2.1 W/m², RED ~1.2 W/m², seawater π≈29 bar). Design only — no hardware. | None |
