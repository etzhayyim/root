...
7. App-level manual gather commands (e.g. `gatherSubDID`) SHOULD append `ai.gftd.liveData.status` with at least `status`, `sourceId`, `recordsCreated`, `updatedAt`.
8. `gftd monitor shinka` SHOULD expose sub-DID freshness and stale-count based on `--freshness-hours` threshold.
9. `ai.gftd.liveData.status` SHOULD include `actorDid`; if app omits it, host/PDS MUST backfill with write repo DID.
