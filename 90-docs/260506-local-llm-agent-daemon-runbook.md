...
Use `ops/local-agent/ai.gftd.agent-daemon.plist.example` as the template.
...
script directly through the `ops/local-agent/ai.gftd.agent-zeebe-worker.plist.example`
...
```bash
tail -f /tmp/ai.gftd.agent-daemon.out.log /tmp/ai.gftd.agent-daemon.err.log
```
...
Use `ops/local-agent/ai.gftd.agent-status-web.plist.example` when the WebUI
...
```bash
launchctl stop ai.gftd.agent-daemon
launchctl start ai.gftd.agent-daemon
```
