...
- 既存で coverage 系 pipeline を持つ。
  ...
  - xrpc `ai.gftd.apps.mediagamers.coverage.get`: 最新 snapshot + freshness 返却
...
2. `xrpc` pipeline (`ai.gftd.apps.<segment>.coverage.get`)
- 最新 snapshot
- freshnessRate (24h)
...
- `topCollections`
- `timestamp_ms`
- `collection` (`ai.gftd.apps.<segment>.coverageSnapshot`)
