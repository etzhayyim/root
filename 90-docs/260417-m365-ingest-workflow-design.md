...
| before | after |
|---|---|
| 手動 `python3 m365_mail_ingest.py upn` を UPN ごとに実行 | `cron */15 *` が `delta-sync-all-users` を自動起動、`ai.gftd.apps.m365Ingest.syncUser` XRPC で任意 UPN を on-demand |
| sync state = ローカル JSON (`~/.gftd/sync_state.json`) | sync state = graph (`vertex_m365_sync_state`) |
| routing なし (DB 書き込みのみ) | `ai.gftd.apps.kyber.inbox.emailSignal` emit → kyber-inbox / yabai が subscribeRepos で後処理 |
...
| `host.<group>.<method>` | lexicon input schema | lexicon output schema | Host capability (`ai.gftd.host.*`) |
...
テナント内の `@gftd.co.jp` ユーザ一覧を `vertex_m365_user` にキャッシュ (1日1回)。

```
token  ←  host.m365.acquireAppToken
users  ←  host.m365.enumerateUsers(token=$token.access_token, upnDomainSuffix='@gftd.co.jp')
...
paginate ←  loop (while=$next.nextLink != null || $iteration == 0, maxIterations=500, do = [
              ...
              classify-and-write ←  pipeline.map(items=$page.messages, emit={ collection: 'ai.gftd.apps.kyber.inbox.emailSignal', record: {... signalEncrypt, folderSignalClass, classifySenderKind ...} }),
            ])
...
|---|---|---|
| `kyber-inbox` | `ai.gftd.apps.kyber.inbox.emailSignal` | signal/noise classification → `vertex_email_message` INSERT + `kyber_dept` routing edge |
| `yabai` | `ai.gftd.apps.kyber.inbox.emailSignal` (where `signalClass='yabai'`) | `yabai.entity (Email)` + `yabai.evidence (FraudSignal)` + `yabai.risk (monitor)` |

新規 routing rule は downstream actor の manifest に追加するだけで、`m365-ingest` には触らない (open/closed 原則、Shannon η 維持)。

## Host capability: `ai.gftd.host.m365.*`

4 lexicons + 1 TS module。完全な contract SSoT (`00-contracts/lexicons/ai/gftd/host/m365/*.json`)。

| NSID | 方向 | 用途 |
|---|---|---|
| `ai.gftd.host.m365.acquireAppToken` | `procedure` | Client credentials flow、token cache (55min skew) |
| `ai.gftd.host.m365.enumerateUsers` | `query` | `/users?$filter=endsWith(upn,...)`, auto-pagination |
| `ai.gftd.host.m365.fetchMailFolders` | `query` | `/mailFolders` recursive (childFolders) |
| `ai.gftd.host.m365.fetchMessagesPage` | `query` | `/messages` 1 page (caller が `nextLink` ループ) |
...
| upn_domain | VARCHAR | Indexed for `@gftd.co.jp` filter |
...
| 5 | Actor manifest 登録: `gftd xrpc ai.gftd.actor.migrate -d '{"manifestPath":"20-actors/m365-ingest/actor-manifest.jsonld"}'` | ⏳ |
