#!/usr/bin/env bb
;; evidence-fusion.bb — fuse DISCLOSED evidence signals into the ECL objective function.
;;
;;   bb evidence-fusion.bb [evidence.edn]   # fuse → scores → J → route + provenance digest
;;   bb evidence-fusion.bb --edn [file]     # machine-readable verdict + fused scorecard
;;
...
(defn fuse-scores [signals]
  ...
  (doseq [s signals]
    (when-not (valid-dims (:dim s))
      (binding [*out* *err*] (println "FATAL: unknown dim" (:dim s))) (System/exit 2)))
  (into {} (for [d dims
                 :let [raw (reduce + 0.0
