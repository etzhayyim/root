...
│                                                                     │
│  ① ai.gftd.pds.invoke        L917   XRPC dispatch (no LLM)        │
│  ② ai.gftd.agent.chat        L1017  agentInfer                   ✓│
...
|---|---|---|---|---|---|---|---|
| ① | `ai.gftd.pds.invoke` | Worker RPC | なし (XRPC 転送) | - | - | - | LLM 不使用。統合不要 |
| ② | `ai.gftd.agent.chat` | MCP/直接 chat | `agentInfer` | ✓ | ✓ | ✓ | **統合済** |
...
│                    PDS Handler (index.ts)                           │
│                                                                     │
│  ① ai.gftd.pds.invoke        → XRPC dispatch (LLM 不使用)         │
│                                                                     │
│  ②③④ → agentInfer()                                               │
...
| やりたいこと | 使う NSID | 説明 |
|---|---|---|
| **プロジェクト会話で agent と対話** | `ai.gftd.projector.sendProjectMessage` | PM + member agent tools + reflexion。**推奨 default** |
| **agent に直接 DM** | `ai.gftd.agent.chat` | 1:1 会話。convoSystemPrompt で応答 |
| **convo で DM (Bluesky 互換)** | `chat.bsky.convo.sendMessage` | AT Protocol 標準。agent auto-reply |
| **XRPC 直接呼出** | `ai.gftd.pds.invoke` | tool 単体実行 (LLM なし) |
| **外部 platform から** | os-messaging webhook → `ai.gftd.convo.send` | Discord/Telegram/Slack/LINE/WhatsApp |
