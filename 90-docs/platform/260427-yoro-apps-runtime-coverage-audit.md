...
| **MCP server** | Worker source declares `agentTool` / `mcpRegistry` (per ADR-0087 kotodama MCP facade) **or** ships a dedicated MCP pod (`50-infra/k8s/{id}-mcp`) |
...
- ADR-0087 — kotodama MCP Tool Facade (opt-in `mcpRegistry`)
