...
PDS cron (*/5 min) → expandDomainCoverage()
  ...
  └─ Phase 7: Governance + Knowledge Graph
      knowledge_edges → ai.gftd.actor.knowledgeEdge × 3-5
      capabilities → ai.gftd.actor.app.capabilitiesJson
```
...
| PDS `pds-handlers-gftd.ts` | `ai.gftd.graph.query` legacy alias | deployed |
...
| gftd `graph_client.go` | Canonical graph SQL NSID `ai.gftd.graph.cypher` | committed |
...
3. **Projector pattern** — READ → LLM → WRITE per tick (same as `ai.gftd.projector`)
