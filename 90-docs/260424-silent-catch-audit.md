...
|---|---|---|
| `handlers/gftd/index.ts` | 31 | Bootstrap / projector / agent-chat / XRPC-dispatch path. Bug 3 (mergeVertexNode) lives here |
...
Plus **3 `Promise.allSettled`** call sites (`mcp-adapter.ts`, `handlers/appview/feed.ts`, `handlers/gftd/index.ts`) — these double-swallow individual task failures.
...
| H3 | `handlers/gftd/index.ts: mergeVertexNode` (line 86, called from `bootstrapKnowledgeNodes` / `bootstrapGovernanceNodes`) | Bug 3 (already surfaced) | knowledge-graph + Profile bootstrap missing. |
...
| M4 | `handlers/gftd/index.ts` (many) | bootstrap follow / tools / governance individual catches | Each sub-operation can fail independently; main bootstrap returns 200 regardless. Caller sees "registered" but subresources missing. |
...
4. **M4 (gftd/index.ts bootstrap)** — each sub-task catch: change to `Promise.allSettled` + aggregate errors into response metadata so caller can see partial-success.
