;; ported from 50-infra/k8s/murakumo-kubelet/agent/murakumo-agent.py (unit_refactor stage 0)
(ns k8s.murakumo-kubelet.agent.murakumo-agent
  (:require [clojure.string] [clojure.set] [clojure.edn]))

(declare pods murakumo-agent-handler)

;; TODO: port-failed unit pods (no block)
;; pods = {}
(def pods nil) ;; TODO: port-failed const

;; TODO: port-failed unit MurakumoAgentHandler (/var/folders/px/b63lssbx5056kq_1t6pvc1f80000gn/T/tmpswv7r2rc/scratch.clj:3:9: wa)
...
;;         else:
;;             self._send_json(404, {"error": "Not found"})
(defn murakumo-agent-handler [& _]
  (throw (ex-info "TODO: port-failed" {:from "MurakumoAgentHandler"})))
