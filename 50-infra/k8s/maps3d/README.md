...
| 2 | BPMN flow integration — POST `/xrpc/com.etzhayyim.apps.maps3d.processTile`, poll `vertex_maps3d_tile` until `status=done`, assert `mesh_uri` populated + audit row recorded. | dispatcher.etzhayyim.com + AgentGateway MCP + 4 pods + Kotoba/Datomic (RW_URL via Keychain) | `70-tools/scripts/test/maps3d-bpmn-integration.py` |
