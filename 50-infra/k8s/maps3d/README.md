...
`00-contracts/bpmn/ai/gftd/maps3d/processTile.bpmn`. All four pods run
...
```bash
curl -X POST http://dispatcher.etzhayyim.com:8080/xrpc/app.etzhayyim.apps.maps3d.processTile \
...
| 2 | BPMN flow integration — POST `/xrpc/app.etzhayyim.apps.maps3d.processTile`, poll `vertex_maps3d_tile` until `status=done`, assert `mesh_uri` populated + audit row recorded. | dispatcher.etzhayyim.com + AgentGateway MCP + 4 pods + RisingWave (RW_URL via Keychain) | `70-tools/scripts/test/maps3d-bpmn-integration.py` |
