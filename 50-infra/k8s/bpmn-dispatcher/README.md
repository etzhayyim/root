...
kotodama package living in etzhayyim; kotodama was subsequently migrated
to `etzhayyim/40-engine/kotoba/crates/kotoba-kotodama/py/`, making the dispatcher infrastructure
...
| File | Purpose |
|---|---|
| `deployment-dispatcher.yaml` | Python aiohttp Deployment + ClusterIP Service for the bpmn-dispatcher pod (image runs `python -m kotodama.dispatcher_main`) |
...
| `configmap-kotodama-cache-fix.yaml` | Hot-patch ConfigMap mounting an alternative `dispatcher_main.py` (cache fix variant) over the image's baked source |
| `configmap-kotodama-sse-fix.yaml` | Hot-patch ConfigMap mounting an alternative `dispatcher_main.py` (SSE fix variant) |
...
reconciled with the canonical `kotodama.dispatcher_main` source in
...
3. **Image**: replace `REPLACE_ME_IMAGE_REF` in `deployment-dispatcher.yaml`
   with the kotodama image built from `etzhayyim/40-engine/kotoba/crates/kotoba-kotodama/py/`.
...
kotodama's vendor-tier handlers. Per etzhayyim ADR-2605172100 hard
rule "MUST NOT integrate fiat payment processors":

| Env var (inherited) | Why preserved | Cleanup path |
|---|---|---|
| `STRIPE_US_API_KEY` / `STRIPE_JP_API_KEY` / `STRIPE_IN_API_KEY` / `STRIPE_PUBLIC_KEY` / `STRIPE_WEBHOOK_SECRET` | `kotodama.primitives.lawfirm_checkout` / `lawfirm_billing` / `ingest.stripe` still reference them | kotodama Stripe extraction (track in `deps.toml [[migrations]]`); after extraction drop the entire STRIPE_* block |
| `KOTOBA_URL` | `kotodama.db_sync` uses Kotoba/Datomic for `vertex_bpmn_lexicon_binding` lookup | Migrate binding registry to AT MST records or IPFS; after migration drop `KOTOBA_URL` + `RW_*_GUARD` block |

These were already present in kotodama at the time of its etzhayyim
migration; this iter does not introduce new violations. Substrate purity
cleanup is a separate workstream tracked against kotodama, not this
Deployment.
