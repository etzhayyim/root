...
def call_llm_json(system: str, user: str, *, max_tokens: int = 400) -> dict[str, Any] | None:
    ...
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (compatible; GftdIntelWorker/1.0)",
    }
    if api_key:
...
class IntelStore:
    ...
    def create_run(self, scope: dict[str, Any], trigger_kind: str, dry_run: bool) -> dict[str, Any]:
        ...
        vertex_id = f"at://{OWNER_DID}/ai.gftd.apps.intel.inferenceRun/{run_id}"
        with self.connect() as conn, conn.cursor() as cur:
            try:
                ...
                if not is_missing_relation_error(exc):
                    raise
                legacy_vertex_id = f"at://{OWNER_DID}/ai.gftd.apps.intel.inferenceChain/{run_id}"
                cur.execute(
                    """
    ...
    def materialize(self, run_id: str, candidates: list[Candidate], dry_run: bool) -> dict[str, Any]:
        ...
        with self.connect() as conn, conn.cursor() as cur:
            try:
                ...
                run_vertex_id = f"at://{OWNER_DID}/ai.gftd.apps.intel.inferenceRun/{run_id}"
                cur.execute(
                    """
    ...
    def materialize_legacy(self, cur: Any, run_id: str, candidates: list[Candidate]) -> tuple[int, int]:
        ...
        for c in candidates:
            ...
            vertex_id = f"at://{OWNER_DID}/ai.gftd.apps.intel.inferredCohort/{cohort_id}"
            cur.execute(
                """
        ...
        chain_vertex_id = f"at://{OWNER_DID}/ai.gftd.apps.intel.inferenceChain/{run_id}"
        cur.execute(
            """
