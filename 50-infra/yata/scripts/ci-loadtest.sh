...
for i in $(seq 1 $NUM_WRITES); do
  ...
  if [ -n "$ENTRY" ] && [ "$ENTRY" != "null" ]; then
    ...
    curl -s -X POST "$READ0_URL/xrpc/com.etzhayyim.yata.walApply" \
      -H 'Content-Type: application/json' -d "{\"entries\":[$ENTRY]}" > /dev/null &
    curl -s -X POST "$READ1_URL/xrpc/com.etzhayyim.yata.walApply" \
      -H 'Content-Type: application/json' -d "{\"entries\":[$ENTRY]}" > /dev/null &
    wait
...
# WAL tail
T0=$(python3 -c "import time; print(int(time.time()*1000))")
TAIL=$(curl -s -X POST "$WRITE_URL/xrpc/com.etzhayyim.yata.walTail" \
...
# WAL flush segment
T0=$(python3 -c "import time; print(int(time.time()*1000))")
FLUSH=$(curl -s -X POST "$WRITE_URL/xrpc/com.etzhayyim.yata.walFlushSegment" \
...
T0=$(python3 -c "import time; print(int(time.time()*1000))")
CP=$(curl -s -X POST "$WRITE_URL/xrpc/com.etzhayyim.yata.walCheckpoint" \
  -H 'Content-Type: application/json' -d '{}')
T1=$(python3 -c "import time; print(int(time.time()*1000))")
...
T0=$(python3 -c "import time; print(int(time.time()*1000))")
COLD=$(curl -s -X POST "$READ1_URL/xrpc/com.etzhayyim.yata.walColdStart" \
  -H 'Content-Type: application/json' -d '{}')
T1=$(python3 -c "import time; print(int(time.time()*1000))")
...
CATCH_TAIL=$(curl -s -X POST "$WRITE_URL/xrpc/com.etzhayyim.yata.walTail" \
