...
**Role (2026-04-03)**: Query index (materialized view)。R2 (CACHE_R2 = `etzhayyim-cache`) が authoritative source of truth。kagami は fire-and-forget で index 更新される。Container crash/restart でもデータは R2 に永続化済み。Graceful shutdown で final compaction → R2 flush。AUTO_COMPACT_FRAGMENT_THRESHOLD=8。
