...
pub fn router<G: GraphQueryExecutor>(state: YataRestState<G>) -> Router {
    Router::new()
        ...
        .route("/xrpc/com.etzhayyim.yata.cypher", post(xrpc_cypher::<G>))
        .route("/xrpc/com.etzhayyim.yata.cypherBatch", post(xrpc_cypher_batch::<G>))
        // LanceDB lifecycle
        .route("/xrpc/com.etzhayyim.yata.coldStart", post(cold_start_handler::<G>))
        .route("/xrpc/com.etzhayyim.yata.compact", post(compact_handler::<G>))
        .route("/xrpc/com.etzhayyim.yata.repair", post(repair_handler::<G>))
        .route("/xrpc/com.etzhayyim.yata.stats", get(stats_handler::<G>))
        // Phase 5: Distributed GIE fragment execution
        .route("/xrpc/com.etzhayyim.yata.executeFragment", post(execute_fragment_handler::<G>))
        .with_state(state)
}
...
/// GET /xrpc/com.etzhayyim.yata.stats — CPM metrics (read/mutation/mergeRecord counters).
...
/// POST /xrpc/com.etzhayyim.yata.cypher — unified Cypher read+write.
...
/// POST /xrpc/com.etzhayyim.yata.cypherBatch — execute multiple Cypher statements in one HTTP round-trip.
...
/// POST /xrpc/com.etzhayyim.yata.coldStart — Open LanceDB table from R2.
...
/// POST /xrpc/com.etzhayyim.yata.compact — L1 Compaction: PK-dedup Lance fragments.
...
/// POST /xrpc/com.etzhayyim.yata.repair — Repair corrupted Lance table by version rollback.
...
mod tests {
    ...
    async fn test_stats() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .uri("/xrpc/com.etzhayyim.yata.stats")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
