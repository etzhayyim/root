...
- **NOT FX arbitrage / market-making.** Mid-market rate is locked at
  `deposit()` time; spread profit is **zero** (G5 const).
- **NOT a Uniswap-style AMM.** No constant-product curve, no LP tokens,
  no slippage by design. Rebalance is a one-off Council-attested swap
  via Aerodrome-on-Base (or comparable), executed by `councilSafe`
  only.
...
- **G4 Mid-market band** — `deposit()` reverts when
  ...
  (= 50 bps = ±0.5%).
- **G5 No spread profit** — rate locked at `deposit()` time; reserve
  buffer absorbs subsequent oracle drift; the pool itself NEVER takes
  the spread.
