...
The `joinAttestationCid` is the keccak hash of the IPFS CID pointing at the creed-acceptance document. The document itself follows ADR-2605172600 oath conventions: signed by the founder's DID key, written to their PDS as `ai.gftd.apps.etzhayyim.oath`. See [the reconciliation ADR-2605172700](../../90-docs/adr/2605172700-membership-layering-shinto-adherent.md) for the layering between 信者 (172600) and Adherent (172300 S0) tiers — every founder is by definition both.
