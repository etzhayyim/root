...
// Also emit a flat handle→record projection of the SAME 28 records as a typed
// gen module the Worker bundles. This is the Worker's KV-FREE did:web
// compatibility source (resolveActorRecordTiered): did:web requires a
// server-served did.json, so until the canonical DID moves to content-addressed
// resolution (step 3), the Worker reads this static, seed-derived map instead of
// CF KV. The canonical state remains the Datom blocks above; this is a
// convenience projection of them, not a second source of truth.
const recordsMap = Object.fromEntries(records.map((r) => [r.handle, r]));
const genTs =
  ...
  JSON.stringify(recordsMap, null, 2) +
  ` as const;\n`;
writeFileSync(resolve(here, '../src/registry/actors-v1-records.gen.ts'), genTs);
console.log(
  `✓ wrote ${records.length} records → src/registry/actors-v1-records.gen.ts (Worker did:web compat, KV-free)`,
);
