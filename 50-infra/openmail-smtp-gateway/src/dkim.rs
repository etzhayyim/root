...
/// `a=` value for the only algorithm implemented in R0.
...
/// Sign `message` (full RFC 5322 bytes, headers + blank line + body) and return the
/// `DKIM-Signature:` header to prepend. `key` is the member's Ed25519 signing key
/// (held client-side; never by the gateway).
pub fn sign(message: &str, params: &SignParams, key: &SigningKey) -> String {
    ...
    // DKIM-Signature value with an empty b= (the signature target). Tag order
    // mirrors RFC 8463 Appendix A; h= is ":"-joined.
    let mut value = format!("v=1; a={ALG_ED25519_SHA256}; c=relaxed/relaxed; d={}; ", params.domain);
...
/// (the `p=` value from the signer's DNS TXT record). Returns `Ok(true)` only if both
/// the body hash and the Ed25519 signature check out.
