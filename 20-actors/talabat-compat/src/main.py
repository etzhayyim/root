"""
Py Kotodama WASM entrypoint for Talabat Cognitive Actor.
...
schema = load_schema("../schema/talabat.kotoba")
db = DatomicClient.connect()
app = Runtime("talabat-compat")
...
@app.route("/{api_version}/talabat/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_talabat_" + uuid.uuid4().hex[:12]
    db.transact([{
        "talabat.CognitiveRecord/id": internal_record_id,
        "talabat.CognitiveRecord/apiVersion": api_version,
        "talabat.CognitiveRecord/originalPayload": str(raw_data),
        "talabat.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "talabat.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_talabat_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "talabat_entity",
