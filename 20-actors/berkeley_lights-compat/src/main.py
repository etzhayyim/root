"""
Py Kotodama WASM entrypoint for Berkeley_lights Cognitive Actor.
...
schema = load_schema("../schema/berkeley_lights.kotoba")
db = DatomicClient.connect()
app = Runtime("berkeley_lights-compat")
...
@app.route("/{api_version}/berkeley_lights/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_berkeley_lights_" + uuid.uuid4().hex[:12]
    db.transact([{
        "berkeley_lights.CognitiveRecord/id": internal_record_id,
        "berkeley_lights.CognitiveRecord/apiVersion": api_version,
        "berkeley_lights.CognitiveRecord/originalPayload": str(raw_data),
        "berkeley_lights.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "berkeley_lights.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_berkeley_lights_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "berkeley_lights_entity",
