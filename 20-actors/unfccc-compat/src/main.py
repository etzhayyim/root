"""
Py Kotodama WASM entrypoint for Unfccc Cognitive Actor.
...
schema = load_schema("../schema/unfccc.kotoba")
db = DatomicClient.connect()
app = Runtime("unfccc-compat")
...
@app.route("/{api_version}/unfccc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_unfccc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "unfccc.CognitiveRecord/id": internal_record_id,
        "unfccc.CognitiveRecord/apiVersion": api_version,
        "unfccc.CognitiveRecord/originalPayload": str(raw_data),
        "unfccc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "unfccc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_unfccc_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "unfccc_entity",
