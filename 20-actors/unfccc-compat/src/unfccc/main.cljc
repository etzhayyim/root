(ns unfccc.main
  "Kotodama WASM entrypoint for the Unfccc clean-room actor (L4) — Clojure port.
  ...
  facts (`unfccc.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "unfccc")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"unfccc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "unfccc-compat" :tier tier :entities entities} 200])
...
(defn start! [] :unfccc-compat/ready)
