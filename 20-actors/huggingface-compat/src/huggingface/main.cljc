(ns huggingface.main
  "Kotodama WASM entrypoint for the Huggingface clean-room actor (L4) — Clojure port.
  ...
  facts (`huggingface.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "huggingface")
(def tier "L4")
...
(def entity-specs
  [{:entity "ModelInfo"       :plural "modelinfos"       :id-prefix "huggingf_mod"
    :fields [:author :downloads :likes :private :gated :pipelineTag :libraryName :sha :inference]
    :required [:author :downloads]
    :coerce {:downloads :int :likes :int :private :bool}
    :refs {}}
   {:entity "DatasetInfo"     :plural "datasetinfos"     :id-prefix "huggingf_dat"
    :fields [:author :downloads :likes :private :gated :sha :description :mainSize :citation]
    :required [:author :downloads]
    :coerce {:downloads :int :likes :int :private :bool :mainSize :int}
    :refs {}}
   {:entity "SpaceInfo"       :plural "spaceinfos"       :id-prefix "huggingf_spa"
    :fields [:author :sha :private :sdk :likes :host :subdomain :gated]
    :required [:author :sha]
    :coerce {:private :bool :likes :int}
    :refs {}}
   {:entity "TransformersInfo" :plural "transformersinfos" :id-prefix "huggingf_tra"
    :fields [:autoModel :customClass :pipelineTag :processor]
    :required [:autoModel :customClass]
    :coerce {}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"huggingface.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "huggingface-compat" :tier tier :entities entities} 200])
...
(defn start! [] :huggingface-compat/ready)
