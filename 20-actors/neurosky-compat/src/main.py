"""
Py Kotodama WASM entrypoint for Neurosky Cognitive Actor.
...
schema = load_schema("../schema/neurosky.kotoba")
db = DatomicClient.connect()
app = Runtime("neurosky-compat")
...
@app.route("/{api_version}/neurosky/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_neurosky_" + uuid.uuid4().hex[:12]
    db.transact([{
        "neurosky.CognitiveRecord/id": internal_record_id,
        "neurosky.CognitiveRecord/apiVersion": api_version,
        "neurosky.CognitiveRecord/originalPayload": str(raw_data),
        "neurosky.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "neurosky.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_neurosky_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "neurosky_entity",
