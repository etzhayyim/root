"""
Py Kotodama WASM entrypoint for Moneyforward Cognitive Actor.
...
@app.route("/{api_version}/moneyforward/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_moneyforward_" + uuid.uuid4().hex[:12]
    db.transact([{
        "moneyforward.CognitiveRecord/id": internal_record_id,
        "moneyforward.CognitiveRecord/apiVersion": api_version,
        "moneyforward.CognitiveRecord/originalPayload": str(raw_data),
        "moneyforward.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "moneyforward.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_moneyforward_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "moneyforward_entity",
