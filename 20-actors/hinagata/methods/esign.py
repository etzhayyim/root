...
ADR-2606111954. This is the missing electronic-contract flow (gap #4): it turns a published
template into an executable, signable contract document and wires it to the EXISTING
religious-corp e-signature substrate (`com.etzhayyim.esign.*` lexicons, ADR-2605231230 +
2605181100) — the document body is content-addressed (kotoba IPFS CIDv1 + SHA-256), the
envelope rosters DID signers, and each signer authenticates with their OWN WebAuthn passkey
(ES256/EdDSA) bound to their DID.
...
Pure stdlib (no network). Usage:
    python3 esign.py [seed.edn] --template tmpl.nda-mutual \\
        --requester did:web:etzhayyim.com:actor:abc \\
        --signer did:plc:alice --signer did:plc:bob [--out OUTDIR]
"""
...
def verify_signature(envelope: dict, signature: dict) -> tuple[bool, list[str]]:
    ...
    reasons = []
    if signature.get("signerDid") not in envelope.get("signers", []):
        reasons.append("signerDid not in envelope.signers roster")
    if signature.get("documentSha256") != envelope.get("documentSha256"):
        reasons.append("documentSha256 mismatch (document tampered between request and sign)")
    if signature.get("webauthnAlgorithm") not in ("ES256", "EdDSA"):
        reasons.append(f"unsupported webauthn algorithm: {signature.get('webauthnAlgorithm')}")
    if not signature.get("assertionEnvelope"):
        reasons.append("missing assertionEnvelope (encrypted WebAuthn assertion)")
