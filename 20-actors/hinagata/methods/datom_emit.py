...
first-class canonical state (NOT a projection cache). Two strata:
  ...
  GROUND (durable, op :add) — one datom per (entity, attribute, value): the template / clause /
    statute / jurisdiction / concept / license nodes and the :en/* 縁 (including every
    :cites-statute / :mandated-by edge that binds a clause to actual public law). This IS the
    Datom log: the template↔clause↔statute graph as durable, content-addressable state.
  ...
  DERIVED (transient, :bond/is-transient true) — the edge-primary groundedness / reusability /
    ...
    datoms; they are emitted in a clearly-flagged transient block so a reader can materialise
    them without mistaking them for persisted state.

Pure stdlib — runnable inside the hinagata kotoba pywasm actor (componentize-py).
Usage:
...
NODE_ATTRS = [":lt/kind", ":lt/label", ":lt/sourcing", ":lt/links",
...
EDGE_ATTRS = [":en/from", ":en/to", ":en/kind", ":en/binding-load", ":en/force", ":en/sourcing"]
