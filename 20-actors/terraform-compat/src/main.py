"""
Py Kotodama WASM entrypoint for Terraform Cognitive Actor.
...
@app.route("/{api_version}/terraform/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_terraform_" + uuid.uuid4().hex[:12]
    db.transact([{
        "terraform.CognitiveRecord/id": internal_record_id,
        "terraform.CognitiveRecord/apiVersion": api_version,
        "terraform.CognitiveRecord/originalPayload": str(raw_data),
        "terraform.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "terraform.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_terraform_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "terraform_entity",
