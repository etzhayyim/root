"""
Py Kotodama WASM entrypoint for the Appdynamics clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"appdynamics.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"appdynamics.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_service(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Service", "id": eid})
...
def delete_error(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Error", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trace(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Trace", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_metric(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Metric", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_alert(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Alert", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_incident(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"appdynamics.Incident", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "appdynamics-compat", "entities": ['Service', 'Error', 'Trace', 'Metric', 'Alert', 'Incident']}, 200
