"""
Py Kotodama WASM entrypoint for the Yang_ming_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"yang_ming_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"yang_ming_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_trade_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.TradeItem", "id": eid})
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_document(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.Document", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_container(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.Container", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_party(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.Party", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_shipment(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"yang_ming_api.Shipment", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "yang_ming_api-compat", "entities": ['TradeItem', 'Event', 'Document', 'Container', 'Party', 'Shipment']}, 200
