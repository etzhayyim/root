"""
Py Kotodama WASM entrypoint for Yang_ming_api Cognitive Actor.
...
schema = load_schema("../schema/yang_ming_api.kotoba")
db = DatomicClient.connect()
app = Runtime("yang_ming_api-compat")
...
@app.route("/{api_version}/yang_ming_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yang_ming_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yang_ming_api.CognitiveRecord/id": internal_record_id,
        "yang_ming_api.CognitiveRecord/apiVersion": api_version,
        "yang_ming_api.CognitiveRecord/originalPayload": str(raw_data),
        "yang_ming_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yang_ming_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yang_ming_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "yang_ming_api_entity",
