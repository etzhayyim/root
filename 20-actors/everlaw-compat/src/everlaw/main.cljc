(ns everlaw.main
  "Kotodama WASM entrypoint for the Everlaw clean-room actor (L4) — Clojure port.
  ...
  facts (`everlaw.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "everlaw")
(def tier "L4")
...
(def entity-specs
  [{:entity "DataSubject" :plural "datasubjects" :id-prefix "everlaw_dat"
    :fields [:email :jurisdiction :status]              :required [:email :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "ConsentRecord" :plural "consentrecords" :id-prefix "everlaw_con"
    :fields [:subjectId :purpose :granted :recordedAt] :required [:purpose :granted]
    :coerce {:granted :bool}          :refs {:subjectId "DataSubject"}}
   {:entity "Request" :plural "requests" :id-prefix "everlaw_req"
    :fields [:subjectId :type :status :dueAt]        :required [:type :status]
    :coerce {}                       :refs {:subjectId "DataSubject"}}
   {:entity "Control" :plural "controls" :id-prefix "everlaw_con"
    :fields [:framework :name :status]          :required [:framework :name]
    :coerce {}                       :refs {}}
   {:entity "Evidence" :plural "evidences" :id-prefix "everlaw_evi"
    :fields [:controlId :name :contentRef]          :required [:name]
    :coerce {}                       :refs {:controlId "Control"}}
   {:entity "Assessment" :plural "assessments" :id-prefix "everlaw_ass"
...
(defn emit-facts
  "EAVT facts for one record: {\"everlaw.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "everlaw-compat" :tier tier :entities entities} 200])
...
(defn start! [] :everlaw-compat/ready)
