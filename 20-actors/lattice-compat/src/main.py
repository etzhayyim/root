"""
Py Kotodama WASM entrypoint for Lattice Cognitive Actor.
...
@app.route("/{api_version}/lattice/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lattice_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lattice.CognitiveRecord/id": internal_record_id,
        "lattice.CognitiveRecord/apiVersion": api_version,
        "lattice.CognitiveRecord/originalPayload": str(raw_data),
        "lattice.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lattice.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lattice_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lattice_entity",
