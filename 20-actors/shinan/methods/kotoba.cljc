#!/usr/bin/env bb
;; shinan 指南 — coverage-ledger kotoba Datom-log writer (content-addressed).
(ns shinan.methods.kotoba
  "kotoba.cljc — shinan 指南 append-only COVERAGE LEDGER writer
  (ADR-2606291501, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the kanmon/kafun/busshi/meisai family).

  Persists the open-learning coverage assessments (topic/resource routes) to a local
  append-only EDN log of content-addressed transactions. Each tx CID = 'b' +
  sha256-hex over the canonical JSON {\"datoms\":[…],\"prev\":…} (keys stay ':…'
  STRINGS; EAVT = [op entity attr value], op :db/add only). prev-cid chaining makes
  the log tamper-evident (verify-chain). Deterministic: caller supplies tx-id + as-of
  (no wall clock) → resume-safe.

  No-server-key: this writer holds no key and performs NO network I/O — it only appends
  to a local file. The ledger is a record of OPEN-learning coverage (where scaffolds
  exist + where the gap is), never a record of any learner — there is no learner in
  the model (学習解放)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; shinan 指南 — COVERAGE LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of open-learning coverage assessments). Generated; DO NOT "
                        "hand-edit. A record of where open scaffolds exist + where the gap is, never "
                        "a record of any learner. ADR-2606291501.\n")))
