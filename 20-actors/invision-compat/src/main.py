"""
Py Kotodama WASM entrypoint for Invision Cognitive Actor.
...
schema = load_schema("../schema/invision.kotoba")
db = DatomicClient.connect()
app = Runtime("invision-compat")
...
@app.route("/{api_version}/invision/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_invision_" + uuid.uuid4().hex[:12]
    db.transact([{
        "invision.CognitiveRecord/id": internal_record_id,
        "invision.CognitiveRecord/apiVersion": api_version,
        "invision.CognitiveRecord/originalPayload": str(raw_data),
        "invision.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "invision.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_invision_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "invision_entity",
