"""
Py Kotodama WASM entrypoint for Fujitsu_msp Cognitive Actor.
...
schema = load_schema("../schema/fujitsu_msp.kotoba")
db = DatomicClient.connect()
app = Runtime("fujitsu_msp-compat")
...
@app.route("/{api_version}/fujitsu_msp/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fujitsu_msp_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fujitsu_msp.CognitiveRecord/id": internal_record_id,
        "fujitsu_msp.CognitiveRecord/apiVersion": api_version,
        "fujitsu_msp.CognitiveRecord/originalPayload": str(raw_data),
        "fujitsu_msp.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fujitsu_msp.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fujitsu_msp_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fujitsu_msp_entity",
