(ns synchron.main
  "Kotodama WASM entrypoint for the Synchron clean-room actor (L4) — Clojure port.
  ...
  facts (`synchron.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "synchron")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "synchron_dev"
    :fields [:model :channels :samplingHz :firmware] :required [:model :channels]
    :coerce {:channels :int :samplingHz :int}   :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "synchron_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"synchron.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "synchron-compat" :tier tier :entities entities} 200])
...
(defn start! [] :synchron-compat/ready)
