"""
Py Kotodama WASM entrypoint for Boj Cognitive Actor.
...
schema = load_schema("../schema/boj.kotoba")
db = DatomicClient.connect()
app = Runtime("boj-compat")
...
@app.route("/{api_version}/boj/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_boj_" + uuid.uuid4().hex[:12]
    db.transact([{
        "boj.CognitiveRecord/id": internal_record_id,
        "boj.CognitiveRecord/apiVersion": api_version,
        "boj.CognitiveRecord/originalPayload": str(raw_data),
        "boj.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "boj.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_boj_" + uuid.uuid4().hex[:8])
