"""
Py Kotodama WASM entrypoint for Deliveroo Cognitive Actor.
...
@app.route("/{api_version}/deliveroo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_deliveroo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "deliveroo.CognitiveRecord/id": internal_record_id,
        "deliveroo.CognitiveRecord/apiVersion": api_version,
        "deliveroo.CognitiveRecord/originalPayload": str(raw_data),
        "deliveroo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "deliveroo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_deliveroo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "deliveroo_entity",
