(ns first-light-fusion.main-test
  "Contract + behavioral test for the first_light_fusion-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "first_light_fusion-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'first-light-fusion.main-test)]
