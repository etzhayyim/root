(ns first-light-fusion.main
  "Kotodama WASM entrypoint for the first_light_fusion clean-room actor (L4) — Clojure port.
  ...
  facts (`first_light_fusion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "first_light_fusion")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"first_light_fusion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "first_light_fusion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :first_light_fusion-compat/ready)
