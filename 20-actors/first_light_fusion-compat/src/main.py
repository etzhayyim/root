"""
Py Kotodama WASM entrypoint for First_light_fusion Cognitive Actor.
...
schema = load_schema("../schema/first_light_fusion.kotoba")
db = DatomicClient.connect()
app = Runtime("first_light_fusion-compat")
...
@app.route("/{api_version}/first_light_fusion/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_first_light_fusion_" + uuid.uuid4().hex[:12]
    db.transact([{
        "first_light_fusion.CognitiveRecord/id": internal_record_id,
        "first_light_fusion.CognitiveRecord/apiVersion": api_version,
        "first_light_fusion.CognitiveRecord/originalPayload": str(raw_data),
        "first_light_fusion.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "first_light_fusion.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_first_light_fusion_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "first_light_fusion_entity",
