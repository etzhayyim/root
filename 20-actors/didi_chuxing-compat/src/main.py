"""
Py Kotodama WASM entrypoint for the Didi_chuxing clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"didi_chuxing.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"didi_chuxing.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_driver(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Driver", "id": eid})
...
def delete_rider(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Rider", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trip(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_vehicle(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Vehicle", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_fare(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Fare", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_rating(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"didi_chuxing.Rating", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "didi_chuxing-compat", "entities": ['Driver', 'Rider', 'Trip', 'Vehicle', 'Fare', 'Rating']}, 200
