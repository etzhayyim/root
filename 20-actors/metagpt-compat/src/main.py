"""
Py Kotodama WASM entrypoint for Metagpt Cognitive Actor.
...
schema = load_schema("../schema/metagpt.kotoba")
db = DatomicClient.connect()
app = Runtime("metagpt-compat")
...
@app.route("/{api_version}/metagpt/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_metagpt_" + uuid.uuid4().hex[:12]
    db.transact([{
        "metagpt.CognitiveRecord/id": internal_record_id,
        "metagpt.CognitiveRecord/apiVersion": api_version,
        "metagpt.CognitiveRecord/originalPayload": str(raw_data),
        "metagpt.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "metagpt.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_metagpt_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "metagpt_entity",
