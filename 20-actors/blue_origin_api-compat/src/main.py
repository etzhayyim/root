"""
Py Kotodama WASM entrypoint for the Blue_origin_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"blue_origin_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"blue_origin_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_object(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Object", "id": eid})
...
def delete_position(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Position", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_observation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Observation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_float(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Float", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_station(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"blue_origin_api.Station", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "blue_origin_api-compat", "entities": ['Object', 'Position', 'Observation', 'Scene', 'Float', 'Station']}, 200
