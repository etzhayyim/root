"""
Py Kotodama WASM entrypoint for Flashpoint Cognitive Actor.
...
schema = load_schema("../schema/flashpoint.kotoba")
db = DatomicClient.connect()
app = Runtime("flashpoint-compat")
...
@app.route("/{api_version}/flashpoint/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flashpoint_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flashpoint.CognitiveRecord/id": internal_record_id,
        "flashpoint.CognitiveRecord/apiVersion": api_version,
        "flashpoint.CognitiveRecord/originalPayload": str(raw_data),
        "flashpoint.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flashpoint.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flashpoint_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "flashpoint_entity",
