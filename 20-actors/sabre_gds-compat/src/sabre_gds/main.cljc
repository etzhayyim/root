(ns sabre_gds.main
  "Kotodama WASM entrypoint for the Sabre_gds clean-room actor (L4) — Clojure port.
  ...
  facts (`sabre_gds.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sabre_gds")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sabre_gds.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sabre_gds-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sabre_gds-compat/ready)
