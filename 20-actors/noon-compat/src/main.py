"""
Py Kotodama WASM entrypoint for Noon Cognitive Actor.
...
schema = load_schema("../schema/noon.kotoba")
db = DatomicClient.connect()
app = Runtime("noon-compat")
...
@app.route("/{api_version}/noon/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_noon_" + uuid.uuid4().hex[:12]
    db.transact([{
        "noon.CognitiveRecord/id": internal_record_id,
        "noon.CognitiveRecord/apiVersion": api_version,
        "noon.CognitiveRecord/originalPayload": str(raw_data),
        "noon.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "noon.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_noon_" + uuid.uuid4().hex[:8])
