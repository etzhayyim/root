(ns cma.main
  "Kotodama WASM entrypoint for the Cma clean-room actor (L4) — Clojure port.
  ...
  facts (`cma.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cma.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cma-compat" :tier tier :entities entities} 200])
