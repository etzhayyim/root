"""
Py Kotodama WASM entrypoint for Spacex_telemetry Cognitive Actor.
...
schema = load_schema("../schema/spacex_telemetry.kotoba")
db = DatomicClient.connect()
app = Runtime("spacex_telemetry-compat")
...
@app.route("/{api_version}/spacex_telemetry/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_spacex_telemetry_" + uuid.uuid4().hex[:12]
    db.transact([{
        "spacex_telemetry.CognitiveRecord/id": internal_record_id,
        "spacex_telemetry.CognitiveRecord/apiVersion": api_version,
        "spacex_telemetry.CognitiveRecord/originalPayload": str(raw_data),
        "spacex_telemetry.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "spacex_telemetry.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_spacex_telemetry_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "spacex_telemetry_entity",
