"""
Py Kotodama WASM entrypoint for Openrov Cognitive Actor.
...
schema = load_schema("../schema/openrov.kotoba")
db = DatomicClient.connect()
app = Runtime("openrov-compat")
...
@app.route("/{api_version}/openrov/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_openrov_" + uuid.uuid4().hex[:12]
    db.transact([{
        "openrov.CognitiveRecord/id": internal_record_id,
        "openrov.CognitiveRecord/apiVersion": api_version,
        "openrov.CognitiveRecord/originalPayload": str(raw_data),
        "openrov.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "openrov.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_openrov_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "openrov_entity",
