"""
Py Kotodama WASM entrypoint for Nvidia_nvml Cognitive Actor.
...
schema = load_schema("../schema/nvidia_nvml.kotoba")
db = DatomicClient.connect()
app = Runtime("nvidia_nvml-compat")
...
@app.route("/{api_version}/nvidia_nvml/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nvidia_nvml_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nvidia_nvml.CognitiveRecord/id": internal_record_id,
        "nvidia_nvml.CognitiveRecord/apiVersion": api_version,
        "nvidia_nvml.CognitiveRecord/originalPayload": str(raw_data),
        "nvidia_nvml.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nvidia_nvml.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nvidia_nvml_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nvidia_nvml_entity",
