"""
Py Kotodama WASM entrypoint for the Nvidia_nvml clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"nvidia_nvml.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"nvidia_nvml.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.Device", "id": eid})
...
def delete_kernel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.Kernel", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_alloc(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.MemoryAlloc", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inference_job(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.InferenceJob", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_engine(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.Engine", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_utilization(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_nvml.Utilization", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "nvidia_nvml-compat", "entities": ['Device', 'Kernel', 'MemoryAlloc', 'InferenceJob', 'Engine', 'Utilization']}, 200
