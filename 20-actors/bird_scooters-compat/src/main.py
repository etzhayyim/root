"""
Py Kotodama WASM entrypoint for Bird_scooters Cognitive Actor.
...
@app.route("/{api_version}/bird_scooters/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bird_scooters_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bird_scooters.CognitiveRecord/id": internal_record_id,
        "bird_scooters.CognitiveRecord/apiVersion": api_version,
        "bird_scooters.CognitiveRecord/originalPayload": str(raw_data),
        "bird_scooters.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bird_scooters.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bird_scooters_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bird_scooters_entity",
