"""
Py Kotodama WASM entrypoint for Ice_api Cognitive Actor.
...
schema = load_schema("../schema/ice_api.kotoba")
db = DatomicClient.connect()
app = Runtime("ice_api-compat")
...
@app.route("/{api_version}/ice_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ice_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ice_api.CognitiveRecord/id": internal_record_id,
        "ice_api.CognitiveRecord/apiVersion": api_version,
        "ice_api.CognitiveRecord/originalPayload": str(raw_data),
        "ice_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ice_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ice_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ice_api_entity",
