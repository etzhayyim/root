"""
Py Kotodama WASM entrypoint for Hubspot Cognitive Actor.
...
@app.route("/{api_version}/hubspot/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hubspot_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hubspot.CognitiveRecord/id": internal_record_id,
        "hubspot.CognitiveRecord/apiVersion": api_version,
        "hubspot.CognitiveRecord/originalPayload": str(raw_data),
        "hubspot.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hubspot.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hubspot_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hubspot_entity",
