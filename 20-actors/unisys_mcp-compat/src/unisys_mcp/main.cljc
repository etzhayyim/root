(ns unisys_mcp.main
  "Kotodama WASM entrypoint for the Unisys_mcp clean-room actor (L4) — Clojure port.
  ...
  facts (`unisys_mcp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "unisys_mcp")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"unisys_mcp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "unisys_mcp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :unisys_mcp-compat/ready)
