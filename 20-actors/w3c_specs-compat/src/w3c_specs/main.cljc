(ns w3c_specs.main
  "Kotodama WASM entrypoint for the W3C Specs clean-room actor (L4) — Clojure port.
  ...
  facts (`w3c_specs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "w3c_specs")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "w3cspecs_wor"
    :fields [:doi :title :type :publishedAt]  :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"    :plural "authors"    :id-prefix "w3cspecs_aut"
    :fields [:orcid :givenName :familyName]   :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "w3cspecs_cit"
    :fields [:workId :citedDoi]                :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "w3cspecs_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "w3cspecs_rec"
    :fields [:accession :database :title]     :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "w3cspecs_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"w3c_specs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "w3c_specs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :w3c_specs-compat/ready)
