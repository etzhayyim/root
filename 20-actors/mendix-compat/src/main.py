"""
Py Kotodama WASM entrypoint for Mendix Cognitive Actor.
...
schema = load_schema("../schema/mendix.kotoba")
db = DatomicClient.connect()
app = Runtime("mendix-compat")
...
@app.route("/{api_version}/mendix/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mendix_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mendix.CognitiveRecord/id": internal_record_id,
        "mendix.CognitiveRecord/apiVersion": api_version,
        "mendix.CognitiveRecord/originalPayload": str(raw_data),
        "mendix.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mendix.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mendix_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mendix_entity",
