"""
Py Kotodama WASM entrypoint for Firebase_games Cognitive Actor.
...
schema = load_schema("../schema/firebase_games.kotoba")
db = DatomicClient.connect()
app = Runtime("firebase_games-compat")
...
@app.route("/{api_version}/firebase_games/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_firebase_games_" + uuid.uuid4().hex[:12]
    db.transact([{
        "firebase_games.CognitiveRecord/id": internal_record_id,
        "firebase_games.CognitiveRecord/apiVersion": api_version,
        "firebase_games.CognitiveRecord/originalPayload": str(raw_data),
        "firebase_games.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "firebase_games.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_firebase_games_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "firebase_games_entity",
