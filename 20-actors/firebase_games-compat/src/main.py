"""
Py Kotodama WASM entrypoint for the Firebase_games clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"firebase_games.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"firebase_games.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_player(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Player", "id": eid})
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_match(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Match", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_leaderboard(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Leaderboard", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inventory(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Inventory", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_achievement(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"firebase_games.Achievement", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "firebase_games-compat", "entities": ['Player', 'Session', 'Match', 'Leaderboard', 'Inventory', 'Achievement']}, 200
