"""
Py Kotodama WASM entrypoint for Shipbob Cognitive Actor.
...
schema = load_schema("../schema/shipbob.kotoba")
db = DatomicClient.connect()
app = Runtime("shipbob-compat")
...
@app.route("/{api_version}/shipbob/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shipbob_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shipbob.CognitiveRecord/id": internal_record_id,
        "shipbob.CognitiveRecord/apiVersion": api_version,
        "shipbob.CognitiveRecord/originalPayload": str(raw_data),
        "shipbob.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shipbob.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shipbob_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shipbob_entity",
