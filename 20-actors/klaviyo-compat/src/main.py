"""
Py Kotodama WASM entrypoint for Klaviyo Cognitive Actor.
...
@app.route("/{api_version}/klaviyo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_klaviyo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "klaviyo.CognitiveRecord/id": internal_record_id,
        "klaviyo.CognitiveRecord/apiVersion": api_version,
        "klaviyo.CognitiveRecord/originalPayload": str(raw_data),
        "klaviyo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "klaviyo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_klaviyo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "klaviyo_entity",
