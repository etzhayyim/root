"""
Py Kotodama WASM entrypoint for Procore_construction Cognitive Actor.
...
schema = load_schema("../schema/procore_construction.kotoba")
db = DatomicClient.connect()
app = Runtime("procore_construction-compat")
...
@app.route("/{api_version}/procore_construction/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_procore_construction_" + uuid.uuid4().hex[:12]
    db.transact([{
        "procore_construction.CognitiveRecord/id": internal_record_id,
        "procore_construction.CognitiveRecord/apiVersion": api_version,
        "procore_construction.CognitiveRecord/originalPayload": str(raw_data),
        "procore_construction.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "procore_construction.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_procore_construction_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "procore_construction_entity",
