"""
Py Kotodama WASM entrypoint for Improbable Cognitive Actor.
...
schema = load_schema("../schema/improbable.kotoba")
db = DatomicClient.connect()
app = Runtime("improbable-compat")
...
@app.route("/{api_version}/improbable/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_improbable_" + uuid.uuid4().hex[:12]
    db.transact([{
        "improbable.CognitiveRecord/id": internal_record_id,
        "improbable.CognitiveRecord/apiVersion": api_version,
        "improbable.CognitiveRecord/originalPayload": str(raw_data),
        "improbable.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "improbable.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_improbable_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "improbable_entity",
