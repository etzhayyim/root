"""
Py Kotodama WASM entrypoint for Braze Cognitive Actor.
...
@app.route("/{api_version}/braze/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_braze_" + uuid.uuid4().hex[:12]
    db.transact([{
        "braze.CognitiveRecord/id": internal_record_id,
        "braze.CognitiveRecord/apiVersion": api_version,
        "braze.CognitiveRecord/originalPayload": str(raw_data),
        "braze.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "braze.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_braze_" + uuid.uuid4().hex[:8])
