"""
Py Kotodama WASM entrypoint for Hawkeye360 Cognitive Actor.
...
@app.route("/{api_version}/hawkeye360/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hawkeye360_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hawkeye360.CognitiveRecord/id": internal_record_id,
        "hawkeye360.CognitiveRecord/apiVersion": api_version,
        "hawkeye360.CognitiveRecord/originalPayload": str(raw_data),
        "hawkeye360.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hawkeye360.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hawkeye360_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hawkeye360_entity",
