"""
Py Kotodama WASM entrypoint for Vts Cognitive Actor.
...
schema = load_schema("../schema/vts.kotoba")
db = DatomicClient.connect()
app = Runtime("vts-compat")
...
@app.route("/{api_version}/vts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vts.CognitiveRecord/id": internal_record_id,
        "vts.CognitiveRecord/apiVersion": api_version,
        "vts.CognitiveRecord/originalPayload": str(raw_data),
        "vts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vts_" + uuid.uuid4().hex[:8])
