"""
Py Kotodama WASM entrypoint for Bcel_pay Cognitive Actor.
...
schema = load_schema("../schema/bcel_pay.kotoba")
db = DatomicClient.connect()
app = Runtime("bcel_pay-compat")
...
@app.route("/{api_version}/bcel_pay/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bcel_pay_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bcel_pay.CognitiveRecord/id": internal_record_id,
        "bcel_pay.CognitiveRecord/apiVersion": api_version,
        "bcel_pay.CognitiveRecord/originalPayload": str(raw_data),
        "bcel_pay.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bcel_pay.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bcel_pay_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bcel_pay_entity",
