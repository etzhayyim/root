"""
Py Kotodama WASM entrypoint for Ieee_xplore Cognitive Actor.
...
schema = load_schema("../schema/ieee_xplore.kotoba")
db = DatomicClient.connect()
app = Runtime("ieee_xplore-compat")
...
@app.route("/{api_version}/ieee_xplore/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ieee_xplore_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ieee_xplore.CognitiveRecord/id": internal_record_id,
        "ieee_xplore.CognitiveRecord/apiVersion": api_version,
        "ieee_xplore.CognitiveRecord/originalPayload": str(raw_data),
        "ieee_xplore.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ieee_xplore.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ieee_xplore_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ieee_xplore_entity",
