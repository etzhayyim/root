"""
Py Kotodama WASM entrypoint for Disco_legal Cognitive Actor.
...
schema = load_schema("../schema/disco_legal.kotoba")
db = DatomicClient.connect()
app = Runtime("disco_legal-compat")
...
@app.route("/{api_version}/disco_legal/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_disco_legal_" + uuid.uuid4().hex[:12]
    db.transact([{
        "disco_legal.CognitiveRecord/id": internal_record_id,
        "disco_legal.CognitiveRecord/apiVersion": api_version,
        "disco_legal.CognitiveRecord/originalPayload": str(raw_data),
        "disco_legal.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "disco_legal.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_disco_legal_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "disco_legal_entity",
