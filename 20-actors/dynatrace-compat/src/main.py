"""
Py Kotodama WASM entrypoint for Dynatrace Cognitive Actor.
...
schema = load_schema("../schema/dynatrace.kotoba")
db = DatomicClient.connect()
app = Runtime("dynatrace-compat")
...
@app.route("/{api_version}/dynatrace/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dynatrace_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dynatrace.CognitiveRecord/id": internal_record_id,
        "dynatrace.CognitiveRecord/apiVersion": api_version,
        "dynatrace.CognitiveRecord/originalPayload": str(raw_data),
        "dynatrace.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dynatrace.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dynatrace_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "dynatrace_entity",
