"""
Py Kotodama WASM entrypoint for Ironclad Cognitive Actor.
...
schema = load_schema("../schema/ironclad.kotoba")
db = DatomicClient.connect()
app = Runtime("ironclad-compat")
...
@app.route("/{api_version}/ironclad/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ironclad_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ironclad.CognitiveRecord/id": internal_record_id,
        "ironclad.CognitiveRecord/apiVersion": api_version,
        "ironclad.CognitiveRecord/originalPayload": str(raw_data),
        "ironclad.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ironclad.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ironclad_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ironclad_entity",
