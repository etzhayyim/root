"""
Py Kotodama WASM entrypoint for Square Cognitive Actor.
...
@app.route("/{api_version}/square/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_square_" + uuid.uuid4().hex[:12]
    db.transact([{
        "square.CognitiveRecord/id": internal_record_id,
        "square.CognitiveRecord/apiVersion": api_version,
        "square.CognitiveRecord/originalPayload": str(raw_data),
        "square.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "square.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_square_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "square_entity",
