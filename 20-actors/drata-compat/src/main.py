"""
Py Kotodama WASM entrypoint for Drata Cognitive Actor.
...
schema = load_schema("../schema/drata.kotoba")
db = DatomicClient.connect()
app = Runtime("drata-compat")
...
@app.route("/{api_version}/drata/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_drata_" + uuid.uuid4().hex[:12]
    db.transact([{
        "drata.CognitiveRecord/id": internal_record_id,
        "drata.CognitiveRecord/apiVersion": api_version,
        "drata.CognitiveRecord/originalPayload": str(raw_data),
        "drata.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "drata.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_drata_" + uuid.uuid4().hex[:8])
