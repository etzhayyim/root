#!/usr/bin/env bb
;; kanmon 関門 — observation-ledger kotoba Datom-log writer (content-addressed).
(ns kanmon.methods.kotoba
  "kotoba.cljc — kanmon 関門 append-only OBSERVATION LEDGER writer
  (ADR-2606291500, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the kafun/busshi/ugachi/meisai family).

  Persists the barrier-load → OPENING route assessments to a local append-only EDN
  log of content-addressed transactions. Each tx CID = 'b' + sha256-hex over the
  canonical JSON {\"datoms\":[…],\"prev\":…} (keys stay ':…' STRINGS; EAVT =
  [op entity attr value], op :db/add only). prev-cid chaining makes the log
  tamper-evident (verify-chain). Deterministic: caller supplies tx-id + as-of
  (no wall clock) → resume-safe.

  No-server-key: this writer holds no key and performs NO network I/O — it only
  appends to a local file. Routes are :representative at R0; the ledger is auditable
  (an OPENING record over exam GATES, never a target-list and never a per-student
  record — the data is opening priorities, not people)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; kanmon 関門 — OBSERVATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of 入試 barrier-load → OPENING route assessments). Generated; "
                        "DO NOT hand-edit. An OPENING record over exam GATES, never a target-list or a "
                        "per-student record. ADR-2606291500.\n")))
