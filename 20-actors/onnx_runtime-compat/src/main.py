"""
Py Kotodama WASM entrypoint for Onnx_runtime Cognitive Actor.
...
@app.route("/{api_version}/onnx_runtime/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_onnx_runtime_" + uuid.uuid4().hex[:12]
    db.transact([{
        "onnx_runtime.CognitiveRecord/id": internal_record_id,
        "onnx_runtime.CognitiveRecord/apiVersion": api_version,
        "onnx_runtime.CognitiveRecord/originalPayload": str(raw_data),
        "onnx_runtime.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "onnx_runtime.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_onnx_runtime_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "onnx_runtime_entity",
