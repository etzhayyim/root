#!/usr/bin/env bb
;; sarutahiko 猿田彦 — kotoba-clj WASM Component build (ADR-2606230001, 2026-06-23)
;;
...
;;   bb 20-actors/sarutahiko/deploy/build_wasm.clj
;;
;; Or via the bb.edn task:
;;   bb sarutahiko:build-wasm
;;
;; Prerequisites:
...
(defn -main [& _args]
  (let [script-dir (-> (System/getProperty "babashka.file") (fs/parent) (fs/absolutize))
    ...
    (println "==> sarutahiko kotoba-clj WASM build")
    ...
    (let [triple     (host-triple)
      ...
      (if (which? "wasmtime")
        (let [{:keys [out exit]} (sh "wasmtime" "run" "--wasm" "component-model" agent-wasm)]
          ...
          (if (and (str/includes? (or out "") "sarutahiko:")
