#!/usr/bin/env python3
"""sarutahiko 猿田彦 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605252500, R0 scaffold. Flattens each seed entity map into
...
datoms only (no writes, no LLM; G16 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.sarutahiko] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.sarutahiko")
    ...
    if args.dry-run or not os.environ.get("KOTOBA_TOKEN"):
