(ns din_standards.main
  "Kotodama WASM entrypoint for the Din_standards clean-room actor (L4) — Clojure port.
  ...
  facts (`din_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "din_standards")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"din_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "din_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :din_standards-compat/ready)
