...
describe("Crew Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:crew.gftd.ai"); });
  ...
  it("xrpc covers seafarer, certs, manifest, crew changes", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.crew.management.getSeafarer");
    expect(nsids).toContain("ai.gftd.apps.crew.management.listCertificates");
    expect(nsids).toContain("ai.gftd.apps.crew.management.getCrewManifest");
    expect(nsids).toContain("ai.gftd.apps.crew.management.listCrewChanges");
  });
