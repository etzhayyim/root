"""
Py Kotodama WASM entrypoint for Sitecore Cognitive Actor.
...
schema = load_schema("../schema/sitecore.kotoba")
db = DatomicClient.connect()
app = Runtime("sitecore-compat")
...
@app.route("/{api_version}/sitecore/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sitecore_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sitecore.CognitiveRecord/id": internal_record_id,
        "sitecore.CognitiveRecord/apiVersion": api_version,
        "sitecore.CognitiveRecord/originalPayload": str(raw_data),
        "sitecore.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sitecore.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sitecore_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sitecore_entity",
