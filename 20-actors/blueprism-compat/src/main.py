"""
Py Kotodama WASM entrypoint for Blueprism Cognitive Actor.
...
schema = load_schema("../schema/blueprism.kotoba")
db = DatomicClient.connect()
app = Runtime("blueprism-compat")
...
@app.route("/{api_version}/blueprism/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_blueprism_" + uuid.uuid4().hex[:12]
    db.transact([{
        "blueprism.CognitiveRecord/id": internal_record_id,
        "blueprism.CognitiveRecord/apiVersion": api_version,
        "blueprism.CognitiveRecord/originalPayload": str(raw_data),
        "blueprism.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "blueprism.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_blueprism_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "blueprism_entity",
