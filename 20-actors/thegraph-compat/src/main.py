"""
Py Kotodama WASM entrypoint for Thegraph Cognitive Actor.
...
@app.route("/{api_version}/thegraph/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_thegraph_" + uuid.uuid4().hex[:12]
    db.transact([{
        "thegraph.CognitiveRecord/id": internal_record_id,
        "thegraph.CognitiveRecord/apiVersion": api_version,
        "thegraph.CognitiveRecord/originalPayload": str(raw_data),
        "thegraph.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "thegraph.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_thegraph_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "thegraph_entity",
