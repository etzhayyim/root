"""
Py Kotodama WASM entrypoint for Mulesoft Cognitive Actor.
...
schema = load_schema("../schema/mulesoft.kotoba")
db = DatomicClient.connect()
app = Runtime("mulesoft-compat")
...
@app.route("/{api_version}/mulesoft/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mulesoft_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mulesoft.CognitiveRecord/id": internal_record_id,
        "mulesoft.CognitiveRecord/apiVersion": api_version,
        "mulesoft.CognitiveRecord/originalPayload": str(raw_data),
        "mulesoft.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mulesoft.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mulesoft_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mulesoft_entity",
