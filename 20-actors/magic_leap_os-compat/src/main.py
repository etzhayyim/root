"""
Py Kotodama WASM entrypoint for the Magic_leap_os clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"magic_leap_os.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"magic_leap_os.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_anchor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Anchor", "id": eid})
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_mesh(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Mesh", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_asset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Asset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"magic_leap_os.Device", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "magic_leap_os-compat", "entities": ['Anchor', 'Scene', 'Mesh', 'Session', 'Asset', 'Device']}, 200
