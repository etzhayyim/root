"""
Py Kotodama WASM entrypoint for Sendgrid Cognitive Actor.
...
@app.route("/{api_version}/sendgrid/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sendgrid_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sendgrid.CognitiveRecord/id": internal_record_id,
        "sendgrid.CognitiveRecord/apiVersion": api_version,
        "sendgrid.CognitiveRecord/originalPayload": str(raw_data),
        "sendgrid.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sendgrid.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sendgrid_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sendgrid_entity",
