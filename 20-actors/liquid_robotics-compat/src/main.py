"""
Py Kotodama WASM entrypoint for Liquid_robotics Cognitive Actor.
...
schema = load_schema("../schema/liquid_robotics.kotoba")
db = DatomicClient.connect()
app = Runtime("liquid_robotics-compat")
...
@app.route("/{api_version}/liquid_robotics/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_liquid_robotics_" + uuid.uuid4().hex[:12]
    db.transact([{
        "liquid_robotics.CognitiveRecord/id": internal_record_id,
        "liquid_robotics.CognitiveRecord/apiVersion": api_version,
        "liquid_robotics.CognitiveRecord/originalPayload": str(raw_data),
        "liquid_robotics.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "liquid_robotics.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_liquid_robotics_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "liquid_robotics_entity",
