"""
Py Kotodama WASM entrypoint for Postman Cognitive Actor.
...
schema = load_schema("../schema/postman.kotoba")
db = DatomicClient.connect()
app = Runtime("postman-compat")
...
@app.route("/{api_version}/postman/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_postman_" + uuid.uuid4().hex[:12]
    db.transact([{
        "postman.CognitiveRecord/id": internal_record_id,
        "postman.CognitiveRecord/apiVersion": api_version,
        "postman.CognitiveRecord/originalPayload": str(raw_data),
        "postman.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "postman.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_postman_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "postman_entity",
