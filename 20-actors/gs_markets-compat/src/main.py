"""
Py Kotodama WASM entrypoint for Gs_markets Cognitive Actor.
...
schema = load_schema("../schema/gs_markets.kotoba")
db = DatomicClient.connect()
app = Runtime("gs_markets-compat")
...
@app.route("/{api_version}/gs_markets/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gs_markets_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gs_markets.CognitiveRecord/id": internal_record_id,
        "gs_markets.CognitiveRecord/apiVersion": api_version,
        "gs_markets.CognitiveRecord/originalPayload": str(raw_data),
        "gs_markets.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gs_markets.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gs_markets_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gs_markets_entity",
