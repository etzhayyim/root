"""
Py Kotodama WASM entrypoint for Zillow_group Cognitive Actor.
...
schema = load_schema("../schema/zillow_group.kotoba")
db = DatomicClient.connect()
app = Runtime("zillow_group-compat")
...
@app.route("/{api_version}/zillow_group/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zillow_group_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zillow_group.CognitiveRecord/id": internal_record_id,
        "zillow_group.CognitiveRecord/apiVersion": api_version,
        "zillow_group.CognitiveRecord/originalPayload": str(raw_data),
        "zillow_group.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zillow_group.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zillow_group_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zillow_group_entity",
