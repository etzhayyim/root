"""
Py Kotodama WASM entrypoint for Jwplayer Cognitive Actor.
...
schema = load_schema("../schema/jwplayer.kotoba")
db = DatomicClient.connect()
app = Runtime("jwplayer-compat")
...
@app.route("/{api_version}/jwplayer/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_jwplayer_" + uuid.uuid4().hex[:12]
    db.transact([{
        "jwplayer.CognitiveRecord/id": internal_record_id,
        "jwplayer.CognitiveRecord/apiVersion": api_version,
        "jwplayer.CognitiveRecord/originalPayload": str(raw_data),
        "jwplayer.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "jwplayer.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_jwplayer_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "jwplayer_entity",
