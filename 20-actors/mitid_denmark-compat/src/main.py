"""
Py Kotodama WASM entrypoint for Mitid_denmark Cognitive Actor.
...
schema = load_schema("../schema/mitid_denmark.kotoba")
db = DatomicClient.connect()
app = Runtime("mitid_denmark-compat")
...
@app.route("/{api_version}/mitid_denmark/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mitid_denmark_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mitid_denmark.CognitiveRecord/id": internal_record_id,
        "mitid_denmark.CognitiveRecord/apiVersion": api_version,
        "mitid_denmark.CognitiveRecord/originalPayload": str(raw_data),
        "mitid_denmark.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mitid_denmark.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mitid_denmark_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mitid_denmark_entity",
