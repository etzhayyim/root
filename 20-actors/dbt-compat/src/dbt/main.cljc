(ns dbt.main
  "Kotodama WASM entrypoint for the Dbt clean-room actor (L4) — Clojure port.
  ...
  facts (`dbt.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Node"      :plural "nodes"      :id-prefix "dbt_nod"
    :fields [:database :schema :name :packageName :uniqueId :alias :description :compiled :rawCode :resourceType]
    :required [:database :schema]
    :coerce {:compiled :bool}           :refs {}}
   {:entity "Source"    :plural "sources"    :id-prefix "dbt_sou"
    :fields [:database :schema :name :sourceName :sourceDescription :loader :identifier :uniqueId :resourceType]
    :required [:database :schema]
    :coerce {}                          :refs {}}
   {:entity "RunResult" :plural "runresults" :id-prefix "dbt_run"
    :fields [:threadId :executionTime :message :failures :uniqueId :compiled :compiledCode :status]
    :required [:executionTime :message]
    :coerce {:executionTime :float :failures :int :compiled :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"dbt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dbt-compat" :tier tier :entities entities} 200])
