"""
Py Kotodama WASM entrypoint for Dbt Cognitive Actor.
...
@app.route("/{api_version}/dbt/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dbt_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dbt.CognitiveRecord/id": internal_record_id,
        "dbt.CognitiveRecord/apiVersion": api_version,
        "dbt.CognitiveRecord/originalPayload": str(raw_data),
        "dbt.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dbt.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dbt_" + uuid.uuid4().hex[:8])
