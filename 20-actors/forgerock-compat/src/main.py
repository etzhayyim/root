"""
Py Kotodama WASM entrypoint for Forgerock Cognitive Actor.
...
schema = load_schema("../schema/forgerock.kotoba")
db = DatomicClient.connect()
app = Runtime("forgerock-compat")
...
@app.route("/{api_version}/forgerock/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_forgerock_" + uuid.uuid4().hex[:12]
    db.transact([{
        "forgerock.CognitiveRecord/id": internal_record_id,
        "forgerock.CognitiveRecord/apiVersion": api_version,
        "forgerock.CognitiveRecord/originalPayload": str(raw_data),
        "forgerock.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "forgerock.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_forgerock_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "forgerock_entity",
