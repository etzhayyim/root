"""
Py Kotodama WASM entrypoint for Sap_plm Cognitive Actor.
...
schema = load_schema("../schema/sap_plm.kotoba")
db = DatomicClient.connect()
app = Runtime("sap_plm-compat")
...
@app.route("/{api_version}/sap_plm/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sap_plm_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sap_plm.CognitiveRecord/id": internal_record_id,
        "sap_plm.CognitiveRecord/apiVersion": api_version,
        "sap_plm.CognitiveRecord/originalPayload": str(raw_data),
        "sap_plm.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sap_plm.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sap_plm_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sap_plm_entity",
