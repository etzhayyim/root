"""
Py Kotodama WASM entrypoint for Yango Cognitive Actor.
...
schema = load_schema("../schema/yango.kotoba")
db = DatomicClient.connect()
app = Runtime("yango-compat")
...
@app.route("/{api_version}/yango/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yango_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yango.CognitiveRecord/id": internal_record_id,
        "yango.CognitiveRecord/apiVersion": api_version,
        "yango.CognitiveRecord/originalPayload": str(raw_data),
        "yango.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yango.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yango_" + uuid.uuid4().hex[:8])
