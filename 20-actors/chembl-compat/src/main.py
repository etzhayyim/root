"""
Py Kotodama WASM entrypoint for Chembl Cognitive Actor.
...
@app.route("/{api_version}/chembl/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_chembl_" + uuid.uuid4().hex[:12]
    db.transact([{
        "chembl.CognitiveRecord/id": internal_record_id,
        "chembl.CognitiveRecord/apiVersion": api_version,
        "chembl.CognitiveRecord/originalPayload": str(raw_data),
        "chembl.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "chembl.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_chembl_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "chembl_entity",
