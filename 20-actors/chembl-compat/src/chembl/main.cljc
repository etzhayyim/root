(ns chembl.main
  "Kotodama WASM entrypoint for the Chembl clean-room actor (L4) — Clojure port.
  ...
  facts (`chembl.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "chembl")
(def tier "L4")
...
(def entity-specs
  [{:entity "Molecule"  :plural "molecules"  :id-prefix "chembl_mol"
    :fields [:chemblId :prefName :moleculeType :blackBoxWarning :therapeuticFlag :firstApproval]
    :required [:prefName :moleculeType]
    :coerce {:blackBoxWarning :int :therapeuticFlag :bool :firstApproval :int}
    :refs {}}
   {:entity "Assay"     :plural "assays"     :id-prefix "chembl_ass"
    :fields [:chemblId :assayType :description :confidenceScore :targetChemblId :documentChemblId]
    :required [:assayType :description]
    :coerce {:confidenceScore :int}
    :refs {}}
   {:entity "Target"    :plural "targets"    :id-prefix "chembl_tar"
    :fields [:chemblId :prefName :targetType :organism :speciesGroupFlag]
    :required [:prefName :targetType]
    :coerce {:speciesGroupFlag :bool}
    :refs {}}
   {:entity "Activity"  :plural "activities" :id-prefix "chembl_act"
    :fields [:activityId :moleculeChemblId :assayChemblId :standardValue :standardType :documentYear]
    :required [:standardValue :standardType]
    :coerce {:activityId :int :documentYear :int}
    :refs {:activityId "Activity"}}
   {:entity "Document"  :plural "documents"  :id-prefix "chembl_doc"
    :fields [:chemblId :title :docType :year :pubmedId :doi]
    :required [:title :docType]
    :coerce {:year :int :pubmedId :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"chembl.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "chembl-compat" :tier tier :entities entities} 200])
...
(defn start! [] :chembl-compat/ready)
