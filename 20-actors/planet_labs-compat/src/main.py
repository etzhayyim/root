"""
Py Kotodama WASM entrypoint for Planet_labs Cognitive Actor.
...
schema = load_schema("../schema/planet_labs.kotoba")
db = DatomicClient.connect()
app = Runtime("planet_labs-compat")
...
@app.route("/{api_version}/planet_labs/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_planet_labs_" + uuid.uuid4().hex[:12]
    db.transact([{
        "planet_labs.CognitiveRecord/id": internal_record_id,
        "planet_labs.CognitiveRecord/apiVersion": api_version,
        "planet_labs.CognitiveRecord/originalPayload": str(raw_data),
        "planet_labs.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "planet_labs.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_planet_labs_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "planet_labs_entity",
