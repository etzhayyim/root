"""
Py Kotodama WASM entrypoint for Censys Cognitive Actor.
...
schema = load_schema("../schema/censys.kotoba")
db = DatomicClient.connect()
app = Runtime("censys-compat")
...
@app.route("/{api_version}/censys/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_censys_" + uuid.uuid4().hex[:12]
    db.transact([{
        "censys.CognitiveRecord/id": internal_record_id,
        "censys.CognitiveRecord/apiVersion": api_version,
        "censys.CognitiveRecord/originalPayload": str(raw_data),
        "censys.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "censys.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_censys_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "censys_entity",
