"""
Py Kotodama WASM entrypoint for the Censys clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"censys.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_indicator(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.Indicator", "id": eid})
...
def delete_vulnerability(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.Vulnerability", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_threat_actor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.ThreatActor", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_campaign(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.Campaign", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_host(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.Host", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_technique(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"censys.Technique", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "censys-compat", "entities": ['Indicator', 'Vulnerability', 'ThreatActor', 'Campaign', 'Host', 'Technique']}, 200
