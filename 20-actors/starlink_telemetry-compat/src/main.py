"""
Py Kotodama WASM entrypoint for Starlink_telemetry Cognitive Actor.
...
schema = load_schema("../schema/starlink_telemetry.kotoba")
db = DatomicClient.connect()
app = Runtime("starlink_telemetry-compat")
...
@app.route("/{api_version}/starlink_telemetry/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_starlink_telemetry_" + uuid.uuid4().hex[:12]
    db.transact([{
        "starlink_telemetry.CognitiveRecord/id": internal_record_id,
        "starlink_telemetry.CognitiveRecord/apiVersion": api_version,
        "starlink_telemetry.CognitiveRecord/originalPayload": str(raw_data),
        "starlink_telemetry.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "starlink_telemetry.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_starlink_telemetry_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "starlink_telemetry_entity",
