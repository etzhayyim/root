"""
Py Kotodama WASM entrypoint for Bipm_utc Cognitive Actor.
...
@app.route("/{api_version}/bipm_utc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bipm_utc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bipm_utc.CognitiveRecord/id": internal_record_id,
        "bipm_utc.CognitiveRecord/apiVersion": api_version,
        "bipm_utc.CognitiveRecord/originalPayload": str(raw_data),
        "bipm_utc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bipm_utc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bipm_utc_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bipm_utc_entity",
