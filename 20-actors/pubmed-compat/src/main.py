"""
Py Kotodama WASM entrypoint for Pubmed Cognitive Actor.
...
schema = load_schema("../schema/pubmed.kotoba")
db = DatomicClient.connect()
app = Runtime("pubmed-compat")
...
@app.route("/{api_version}/pubmed/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pubmed_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pubmed.CognitiveRecord/id": internal_record_id,
        "pubmed.CognitiveRecord/apiVersion": api_version,
        "pubmed.CognitiveRecord/originalPayload": str(raw_data),
        "pubmed.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pubmed.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pubmed_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pubmed_entity",
