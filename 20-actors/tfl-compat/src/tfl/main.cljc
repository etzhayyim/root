(ns tfl.main
  "Kotodama WASM entrypoint for the Tfl clean-room actor (L4) — Clojure port.
  ...
  facts (`tfl.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"tfl.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tfl-compat" :tier tier :entities entities} 200])
