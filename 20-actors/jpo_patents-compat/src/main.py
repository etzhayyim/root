"""
Py Kotodama WASM entrypoint for Jpo_patents Cognitive Actor.
...
schema = load_schema("../schema/jpo_patents.kotoba")
db = DatomicClient.connect()
app = Runtime("jpo_patents-compat")
...
@app.route("/{api_version}/jpo_patents/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_jpo_patents_" + uuid.uuid4().hex[:12]
    db.transact([{
        "jpo_patents.CognitiveRecord/id": internal_record_id,
        "jpo_patents.CognitiveRecord/apiVersion": api_version,
        "jpo_patents.CognitiveRecord/originalPayload": str(raw_data),
        "jpo_patents.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "jpo_patents.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_jpo_patents_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "jpo_patents_entity",
