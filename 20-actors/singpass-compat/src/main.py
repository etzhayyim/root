"""
Py Kotodama WASM entrypoint for Singpass Cognitive Actor.
...
schema = load_schema("../schema/singpass.kotoba")
db = DatomicClient.connect()
app = Runtime("singpass-compat")
...
@app.route("/{api_version}/singpass/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_singpass_" + uuid.uuid4().hex[:12]
    db.transact([{
        "singpass.CognitiveRecord/id": internal_record_id,
        "singpass.CognitiveRecord/apiVersion": api_version,
        "singpass.CognitiveRecord/originalPayload": str(raw_data),
        "singpass.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "singpass.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_singpass_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "singpass_entity",
