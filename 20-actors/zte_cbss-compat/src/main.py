"""
Py Kotodama WASM entrypoint for Zte_cbss Cognitive Actor.
...
schema = load_schema("../schema/zte_cbss.kotoba")
db = DatomicClient.connect()
app = Runtime("zte_cbss-compat")
...
@app.route("/{api_version}/zte_cbss/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zte_cbss_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zte_cbss.CognitiveRecord/id": internal_record_id,
        "zte_cbss.CognitiveRecord/apiVersion": api_version,
        "zte_cbss.CognitiveRecord/originalPayload": str(raw_data),
        "zte_cbss.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zte_cbss.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zte_cbss_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zte_cbss_entity",
