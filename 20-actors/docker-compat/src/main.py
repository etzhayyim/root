"""
Py Kotodama WASM entrypoint for Docker Cognitive Actor.
...
@app.route("/{api_version}/docker/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_docker_" + uuid.uuid4().hex[:12]
    db.transact([{
        "docker.CognitiveRecord/id": internal_record_id,
        "docker.CognitiveRecord/apiVersion": api_version,
        "docker.CognitiveRecord/originalPayload": str(raw_data),
        "docker.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "docker.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_docker_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "docker_entity",
