"""
Py Kotodama WASM entrypoint for the Autogpt_forge clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"autogpt_forge.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"autogpt_forge.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_agent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Agent", "id": eid})
...
def delete_task(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Task", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_tool(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Tool", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Memory", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_plan(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"autogpt_forge.Plan", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "autogpt_forge-compat", "entities": ['Agent', 'Task', 'Message', 'Tool', 'Memory', 'Plan']}, 200
