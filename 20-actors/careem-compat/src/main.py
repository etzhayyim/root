"""
Py Kotodama WASM entrypoint for Careem Cognitive Actor.
...
schema = load_schema("../schema/careem.kotoba")
db = DatomicClient.connect()
app = Runtime("careem-compat")
...
@app.route("/{api_version}/careem/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_careem_" + uuid.uuid4().hex[:12]
    db.transact([{
        "careem.CognitiveRecord/id": internal_record_id,
        "careem.CognitiveRecord/apiVersion": api_version,
        "careem.CognitiveRecord/originalPayload": str(raw_data),
        "careem.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "careem.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_careem_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "careem_entity",
