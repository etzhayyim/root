"""
Py Kotodama WASM entrypoint for Subcom_telemetry Cognitive Actor.
...
schema = load_schema("../schema/subcom_telemetry.kotoba")
db = DatomicClient.connect()
app = Runtime("subcom_telemetry-compat")
...
@app.route("/{api_version}/subcom_telemetry/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_subcom_telemetry_" + uuid.uuid4().hex[:12]
    db.transact([{
        "subcom_telemetry.CognitiveRecord/id": internal_record_id,
        "subcom_telemetry.CognitiveRecord/apiVersion": api_version,
        "subcom_telemetry.CognitiveRecord/originalPayload": str(raw_data),
        "subcom_telemetry.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "subcom_telemetry.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_subcom_telemetry_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "subcom_telemetry_entity",
