(ns cmic.main
  "Kotodama WASM entrypoint for the Cmic clean-room actor (L4) — Clojure port.
  ...
  facts (`cmic.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cmic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cmic-compat" :tier tier :entities entities} 200])
