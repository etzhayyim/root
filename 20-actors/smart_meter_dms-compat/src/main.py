"""
Py Kotodama WASM entrypoint for the Smart_meter_dms clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"smart_meter_dms.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"smart_meter_dms.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_meter(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.Meter", "id": eid})
...
def delete_reading(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.Reading", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_node(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.Node", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_load_forecast(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.LoadForecast", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_market_bid(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.MarketBid", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_tag(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"smart_meter_dms.Tag", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "smart_meter_dms-compat", "entities": ['Meter', 'Reading', 'Node', 'LoadForecast', 'MarketBid', 'Tag']}, 200
