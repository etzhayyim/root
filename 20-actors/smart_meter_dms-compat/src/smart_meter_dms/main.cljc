(ns smart-meter-dms.main
  "Kotodama WASM entrypoint for the Smart_meter_dms clean-room actor (L4) — Clojure port.
  ...
  facts (`smart_meter_dms.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "smart_meter_dms")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"smart_meter_dms.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "smart_meter_dms-compat" :tier tier :entities entities} 200])
...
(defn start! [] :smart_meter_dms-compat/ready)
