"""
Py Kotodama WASM entrypoint for Huawei_bss Cognitive Actor.
...
schema = load_schema("../schema/huawei_bss.kotoba")
db = DatomicClient.connect()
app = Runtime("huawei_bss-compat")
...
@app.route("/{api_version}/huawei_bss/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_huawei_bss_" + uuid.uuid4().hex[:12]
    db.transact([{
        "huawei_bss.CognitiveRecord/id": internal_record_id,
        "huawei_bss.CognitiveRecord/apiVersion": api_version,
        "huawei_bss.CognitiveRecord/originalPayload": str(raw_data),
        "huawei_bss.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "huawei_bss.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_huawei_bss_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "huawei_bss_entity",
