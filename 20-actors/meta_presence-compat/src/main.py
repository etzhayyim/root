"""
Py Kotodama WASM entrypoint for the Meta_presence clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"meta_presence.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"meta_presence.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_anchor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Anchor", "id": eid})
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_mesh(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Mesh", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_asset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Asset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"meta_presence.Device", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "meta_presence-compat", "entities": ['Anchor', 'Scene', 'Mesh', 'Session', 'Asset', 'Device']}, 200
