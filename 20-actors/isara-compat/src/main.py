"""
Py Kotodama WASM entrypoint for Isara Cognitive Actor.
...
schema = load_schema("../schema/isara.kotoba")
db = DatomicClient.connect()
app = Runtime("isara-compat")
...
@app.route("/{api_version}/isara/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_isara_" + uuid.uuid4().hex[:12]
    db.transact([{
        "isara.CognitiveRecord/id": internal_record_id,
        "isara.CognitiveRecord/apiVersion": api_version,
        "isara.CognitiveRecord/originalPayload": str(raw_data),
        "isara.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "isara.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_isara_" + uuid.uuid4().hex[:8])
