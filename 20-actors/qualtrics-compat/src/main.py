"""
Py Kotodama WASM entrypoint for Qualtrics Cognitive Actor.
...
schema = load_schema("../schema/qualtrics.kotoba")
db = DatomicClient.connect()
app = Runtime("qualtrics-compat")
...
@app.route("/{api_version}/qualtrics/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_qualtrics_" + uuid.uuid4().hex[:12]
    db.transact([{
        "qualtrics.CognitiveRecord/id": internal_record_id,
        "qualtrics.CognitiveRecord/apiVersion": api_version,
        "qualtrics.CognitiveRecord/originalPayload": str(raw_data),
        "qualtrics.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "qualtrics.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_qualtrics_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "qualtrics_entity",
