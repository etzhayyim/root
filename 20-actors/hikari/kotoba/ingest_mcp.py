#!/usr/bin/env python3
"""hikari 光 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605261100, migration plan Phase 3. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G8). --dry-run parses + counts
datoms only (no writes, no LLM; G5 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.hikari] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.hikari")
    ...
    print("   live ingest requested — implement MCP kotoba_datom_create wiring before use (G10).")
    return 0
