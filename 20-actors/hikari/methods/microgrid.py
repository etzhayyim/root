...
droop + secondary-PI loop drives dispatchable generation until the frequency
error integrates back to zero, and an anti-islanding ROCOF guard trips on an
abnormal rate-of-change of frequency.

This is the floating-point twin of the open-ot field-tier cells
(DROOP_P_F / ANTI_ISLANDING_ROCOF / SOC_KALMAN, Rust WASM). hikari constitutional
gates apply: no nuclear/fossil/rare-earth source ever enters the dispatch model
(G4/G5/G8 — the dispatch command is just kW; the *source* is enforced upstream by
the sourcing audit, never represented as fossil here), inference stays Murakumo
(not used in this deterministic loop), and live grid control is consent-gated
(G10 — this module is offline sim only; cell.py .solve() stays Council-gated).
"""
...
from _substrate import PID, Droop, DroopPI, MicrogridPlant, assert_civilian, simulate

# hikari grid civilian-use allowlist (closed-world, N1). A "source" is never an
# enum here; fossil/nuclear are excluded by sourcing audit upstream (G4/G5).
PERMITTED_USES = ("grid-control", "island", "black-start", "dispatch", "load-shed")
