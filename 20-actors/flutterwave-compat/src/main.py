"""
Py Kotodama WASM entrypoint for Flutterwave Cognitive Actor.
...
schema = load_schema("../schema/flutterwave.kotoba")
db = DatomicClient.connect()
app = Runtime("flutterwave-compat")
...
@app.route("/{api_version}/flutterwave/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flutterwave_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flutterwave.CognitiveRecord/id": internal_record_id,
        "flutterwave.CognitiveRecord/apiVersion": api_version,
        "flutterwave.CognitiveRecord/originalPayload": str(raw_data),
        "flutterwave.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flutterwave.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flutterwave_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "flutterwave_entity",
