"""
Py Kotodama WASM entrypoint for Curemd Cognitive Actor.
...
schema = load_schema("../schema/curemd.kotoba")
db = DatomicClient.connect()
app = Runtime("curemd-compat")
...
@app.route("/{api_version}/curemd/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_curemd_" + uuid.uuid4().hex[:12]
    db.transact([{
        "curemd.CognitiveRecord/id": internal_record_id,
        "curemd.CognitiveRecord/apiVersion": api_version,
        "curemd.CognitiveRecord/originalPayload": str(raw_data),
        "curemd.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "curemd.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_curemd_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "curemd_entity",
