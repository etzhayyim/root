"""
Py Kotodama WASM entrypoint for Nemetschek Cognitive Actor.
...
schema = load_schema("../schema/nemetschek.kotoba")
db = DatomicClient.connect()
app = Runtime("nemetschek-compat")
...
@app.route("/{api_version}/nemetschek/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nemetschek_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nemetschek.CognitiveRecord/id": internal_record_id,
        "nemetschek.CognitiveRecord/apiVersion": api_version,
        "nemetschek.CognitiveRecord/originalPayload": str(raw_data),
        "nemetschek.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nemetschek.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nemetschek_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nemetschek_entity",
