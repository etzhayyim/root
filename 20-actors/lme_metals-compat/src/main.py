"""
Py Kotodama WASM entrypoint for Lme_metals Cognitive Actor.
...
schema = load_schema("../schema/lme_metals.kotoba")
db = DatomicClient.connect()
app = Runtime("lme_metals-compat")
...
@app.route("/{api_version}/lme_metals/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lme_metals_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lme_metals.CognitiveRecord/id": internal_record_id,
        "lme_metals.CognitiveRecord/apiVersion": api_version,
        "lme_metals.CognitiveRecord/originalPayload": str(raw_data),
        "lme_metals.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lme_metals.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lme_metals_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lme_metals_entity",
