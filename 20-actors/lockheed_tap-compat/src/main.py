"""
Py Kotodama WASM entrypoint for Lockheed_tap Cognitive Actor.
...
schema = load_schema("../schema/lockheed_tap.kotoba")
db = DatomicClient.connect()
app = Runtime("lockheed_tap-compat")
...
@app.route("/{api_version}/lockheed_tap/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lockheed_tap_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lockheed_tap.CognitiveRecord/id": internal_record_id,
        "lockheed_tap.CognitiveRecord/apiVersion": api_version,
        "lockheed_tap.CognitiveRecord/originalPayload": str(raw_data),
        "lockheed_tap.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lockheed_tap.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lockheed_tap_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lockheed_tap_entity",
