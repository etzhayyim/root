...
describe("Oil Distribution Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:oil-distribution.gftd.ai"); });
  ...
  it("subscribes to oilRefining.yieldSnapshot", () => {
    ...
    expect(sub.trigger.collections).toContain("ai.gftd.apps.oilRefining.yieldSnapshot");
  });
  it("xrpc covers terminals, hubs, product balance, health", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.oilDistribution.network.getProductTerminal");
    expect(nsids).toContain("ai.gftd.apps.oilDistribution.network.listProductTerminals");
    expect(nsids).toContain("ai.gftd.apps.oilDistribution.network.listWholesaleHubs");
    expect(nsids).toContain("ai.gftd.apps.oilDistribution.analytics.getProductBalance");
    expect(nsids).toContain("ai.gftd.apps.oilDistribution.health");
  });
