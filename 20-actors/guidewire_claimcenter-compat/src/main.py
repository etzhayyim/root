"""
Py Kotodama WASM entrypoint for Guidewire_claimcenter Cognitive Actor.
...
schema = load_schema("../schema/guidewire_claimcenter.kotoba")
db = DatomicClient.connect()
app = Runtime("guidewire_claimcenter-compat")
...
@app.route("/{api_version}/guidewire_claimcenter/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_guidewire_claimcenter_" + uuid.uuid4().hex[:12]
    db.transact([{
        "guidewire_claimcenter.CognitiveRecord/id": internal_record_id,
        "guidewire_claimcenter.CognitiveRecord/apiVersion": api_version,
        "guidewire_claimcenter.CognitiveRecord/originalPayload": str(raw_data),
        "guidewire_claimcenter.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "guidewire_claimcenter.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_guidewire_claimcenter_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "guidewire_claimcenter_entity",
