"""
Py Kotodama WASM entrypoint for Discord Cognitive Actor.
...
@app.route("/{api_version}/discord/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_discord_" + uuid.uuid4().hex[:12]
    db.transact([{
        "discord.CognitiveRecord/id": internal_record_id,
        "discord.CognitiveRecord/apiVersion": api_version,
        "discord.CognitiveRecord/originalPayload": str(raw_data),
        "discord.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "discord.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_discord_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "discord_entity",
