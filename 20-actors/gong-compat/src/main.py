"""
Py Kotodama WASM entrypoint for Gong Cognitive Actor.
...
schema = load_schema("../schema/gong.kotoba")
db = DatomicClient.connect()
app = Runtime("gong-compat")
...
@app.route("/{api_version}/gong/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gong_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gong.CognitiveRecord/id": internal_record_id,
        "gong.CognitiveRecord/apiVersion": api_version,
        "gong.CognitiveRecord/originalPayload": str(raw_data),
        "gong.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gong.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gong_" + uuid.uuid4().hex[:8])
