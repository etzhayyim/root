"""
Py Kotodama WASM entrypoint for Cips_china Cognitive Actor.
...
schema = load_schema("../schema/cips_china.kotoba")
db = DatomicClient.connect()
app = Runtime("cips_china-compat")
...
@app.route("/{api_version}/cips_china/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cips_china_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cips_china.CognitiveRecord/id": internal_record_id,
        "cips_china.CognitiveRecord/apiVersion": api_version,
        "cips_china.CognitiveRecord/originalPayload": str(raw_data),
        "cips_china.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cips_china.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cips_china_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cips_china_entity",
