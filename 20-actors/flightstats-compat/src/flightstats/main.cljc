(ns flightstats.main
  "Kotodama WASM entrypoint for the Flightstats clean-room actor (L4) — Clojure port.
  ...
  facts (`flightstats.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flightstats")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"flightstats.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flightstats-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flightstats-compat/ready)
