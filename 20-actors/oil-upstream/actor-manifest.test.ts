...
describe("Oil Upstream Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:oil-upstream.gftd.ai"); });
  ...
  it("subscribes to oilMidstream.flow", () => {
    ...
    expect(sub.trigger.collections).toContain("ai.gftd.apps.oilMidstream.flow");
  });
  it("xrpc covers field, basins, production pressure, health", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.oilUpstream.registry.getField");
    expect(nsids).toContain("ai.gftd.apps.oilUpstream.registry.listFields");
    expect(nsids).toContain("ai.gftd.apps.oilUpstream.registry.listBasins");
    expect(nsids).toContain("ai.gftd.apps.oilUpstream.analytics.getProductionPressure");
    expect(nsids).toContain("ai.gftd.apps.oilUpstream.health");
  });
