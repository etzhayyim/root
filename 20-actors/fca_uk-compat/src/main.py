"""
Py Kotodama WASM entrypoint for Fca_uk Cognitive Actor.
...
@app.route("/{api_version}/fca_uk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fca_uk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fca_uk.CognitiveRecord/id": internal_record_id,
        "fca_uk.CognitiveRecord/apiVersion": api_version,
        "fca_uk.CognitiveRecord/originalPayload": str(raw_data),
        "fca_uk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fca_uk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fca_uk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fca_uk_entity",
