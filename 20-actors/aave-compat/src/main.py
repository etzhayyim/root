"""
Py Kotodama WASM entrypoint for Aave Cognitive Actor.
...
@app.route("/{api_version}/aave/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aave_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aave.CognitiveRecord/id": internal_record_id,
        "aave.CognitiveRecord/apiVersion": api_version,
        "aave.CognitiveRecord/originalPayload": str(raw_data),
        "aave.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aave.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aave_" + uuid.uuid4().hex[:8])
