"""
Py Kotodama WASM entrypoint for Cdiscount Cognitive Actor.
...
schema = load_schema("../schema/cdiscount.kotoba")
db = DatomicClient.connect()
app = Runtime("cdiscount-compat")
...
@app.route("/{api_version}/cdiscount/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cdiscount_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cdiscount.CognitiveRecord/id": internal_record_id,
        "cdiscount.CognitiveRecord/apiVersion": api_version,
        "cdiscount.CognitiveRecord/originalPayload": str(raw_data),
        "cdiscount.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cdiscount.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cdiscount_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cdiscount_entity",
