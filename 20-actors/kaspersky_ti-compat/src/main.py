"""
Py Kotodama WASM entrypoint for Kaspersky_ti Cognitive Actor.
...
schema = load_schema("../schema/kaspersky_ti.kotoba")
db = DatomicClient.connect()
app = Runtime("kaspersky_ti-compat")
...
@app.route("/{api_version}/kaspersky_ti/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_kaspersky_ti_" + uuid.uuid4().hex[:12]
    db.transact([{
        "kaspersky_ti.CognitiveRecord/id": internal_record_id,
        "kaspersky_ti.CognitiveRecord/apiVersion": api_version,
        "kaspersky_ti.CognitiveRecord/originalPayload": str(raw_data),
        "kaspersky_ti.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "kaspersky_ti.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_kaspersky_ti_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "kaspersky_ti_entity",
