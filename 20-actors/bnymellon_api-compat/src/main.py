"""
Py Kotodama WASM entrypoint for the Bnymellon_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"bnymellon_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"bnymellon_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Account", "id": eid})
...
def delete_loan(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Loan", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_policy(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Policy", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_claim(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Claim", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_transaction(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Transaction", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"bnymellon_api.Customer", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "bnymellon_api-compat", "entities": ['Account', 'Loan', 'Policy', 'Claim', 'Transaction', 'Customer']}, 200
