"""
Py Kotodama WASM entrypoint for Bnymellon_api Cognitive Actor.
...
schema = load_schema("../schema/bnymellon_api.kotoba")
db = DatomicClient.connect()
app = Runtime("bnymellon_api-compat")
...
@app.route("/{api_version}/bnymellon_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bnymellon_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bnymellon_api.CognitiveRecord/id": internal_record_id,
        "bnymellon_api.CognitiveRecord/apiVersion": api_version,
        "bnymellon_api.CognitiveRecord/originalPayload": str(raw_data),
        "bnymellon_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bnymellon_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bnymellon_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bnymellon_api_entity",
