"""
Py Kotodama WASM entrypoint for Nhsdigital Cognitive Actor.
...
schema = load_schema("../schema/nhsdigital.kotoba")
db = DatomicClient.connect()
app = Runtime("nhsdigital-compat")
...
@app.route("/{api_version}/nhsdigital/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nhsdigital_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nhsdigital.CognitiveRecord/id": internal_record_id,
        "nhsdigital.CognitiveRecord/apiVersion": api_version,
        "nhsdigital.CognitiveRecord/originalPayload": str(raw_data),
        "nhsdigital.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nhsdigital.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nhsdigital_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nhsdigital_entity",
