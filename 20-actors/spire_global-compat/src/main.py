"""
Py Kotodama WASM entrypoint for Spire_global Cognitive Actor.
...
schema = load_schema("../schema/spire_global.kotoba")
db = DatomicClient.connect()
app = Runtime("spire_global-compat")
...
@app.route("/{api_version}/spire_global/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_spire_global_" + uuid.uuid4().hex[:12]
    db.transact([{
        "spire_global.CognitiveRecord/id": internal_record_id,
        "spire_global.CognitiveRecord/apiVersion": api_version,
        "spire_global.CognitiveRecord/originalPayload": str(raw_data),
        "spire_global.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "spire_global.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_spire_global_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "spire_global_entity",
