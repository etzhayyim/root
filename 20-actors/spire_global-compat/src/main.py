"""
Py Kotodama WASM entrypoint for the Spire_global clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"spire_global.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"spire_global.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_object(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Object", "id": eid})
...
def delete_position(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Position", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_observation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Observation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_float(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Float", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_station(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"spire_global.Station", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "spire_global-compat", "entities": ['Object', 'Position', 'Observation', 'Scene', 'Float', 'Station']}, 200
