...
def register_with_open_ot(state: CommissioningState, deps: CellDeps) -> CommissioningState:
    ...
    """
    raise NotImplementedError(
        "Requires deps.sdk + open-ot XRPC (ai.gftd.apps.openOt.defineDevice/defineCell/defineLoop)."
    )
...
def commission(state: CommissioningState, deps: CellDeps) -> CommissioningState:
    ...
    raise NotImplementedError(
        "Requires deps.sdk for MST write of ai.gftd.apps.etzhayyim.kuniUmi.commissionDeployment."
    )
...
def healthz_extra(deps: CellDeps) -> dict:
    return {
        ...
        "trigger_nsid": "ai.gftd.apps.etzhayyim.kuniUmi.recordConstructionProgress",
