#!/bin/bash
# kuni-umi 国産 — kotoba deploy
# ADR-2605201400 · Phase 2 scaffold
#
# Ingests schema-shaped seed datoms (Giemon fleet + example site) into a
# running kotoba node and (optionally) builds the langgraph WASM actor (6-handler
...
# dry-run. Replaces the legacy Python cell.py path.
#
# Usage:
...
GRAPH="${KUNIUMI_GRAPH:-com.etzhayyim.kuniumi}"
ACTOR_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> kuni-umi kotoba deploy -> ${KOTOBA_URL} (graph ${GRAPH})"
...
# 0.5 Giemon fleet seed ingestion
echo "--> Giemon fleet registry ingest (:robot/* datoms)"
