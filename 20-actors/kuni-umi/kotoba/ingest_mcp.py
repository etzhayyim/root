#!/usr/bin/env python3
"""kuni-umi 国産 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605201400, Phase 2 scaffold. Flattens each seed entity map into
...
Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.kuniumi] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.kuniumi")
    ...
    # provisioned (G1 gates real outward writes). Kept explicit to avoid silent no-op.
    print("   live ingest requested — implement MCP kotoba_datom_create wiring before use (G1).")
    return 0
