"""
Py Kotodama WASM entrypoint for Nhtsa Cognitive Actor.
...
schema = load_schema("../schema/nhtsa.kotoba")
db = DatomicClient.connect()
app = Runtime("nhtsa-compat")
...
@app.route("/{api_version}/nhtsa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nhtsa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nhtsa.CognitiveRecord/id": internal_record_id,
        "nhtsa.CognitiveRecord/apiVersion": api_version,
        "nhtsa.CognitiveRecord/originalPayload": str(raw_data),
        "nhtsa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nhtsa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nhtsa_" + uuid.uuid4().hex[:8])
