"""
Py Kotodama WASM entrypoint for Kakao_cert Cognitive Actor.
...
schema = load_schema("../schema/kakao_cert.kotoba")
db = DatomicClient.connect()
app = Runtime("kakao_cert-compat")
...
@app.route("/{api_version}/kakao_cert/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_kakao_cert_" + uuid.uuid4().hex[:12]
    db.transact([{
        "kakao_cert.CognitiveRecord/id": internal_record_id,
        "kakao_cert.CognitiveRecord/apiVersion": api_version,
        "kakao_cert.CognitiveRecord/originalPayload": str(raw_data),
        "kakao_cert.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "kakao_cert.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_kakao_cert_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "kakao_cert_entity",
