...
def analyze(adtech, auth, creatives, delivery, fraud):
    ...
    # MATURITY — seller betweenness centrality (the supply-chain "depends/follow" graph).
    # In the bipartite publisher↔seller authorization graph, a seller lies on the path between
    # every PAIR of publishers that both authorize it, so its betweenness ∝ C(fan-in, 2) =
    # fan_in·(fan_in−1)/2. High = a seller that structurally bridges much of the supply graph —
    # where supply-chain power concentrates. A transparency signal, never a target (G2).
    seller_betweenness = sorted(
        ((s, len(ps), len(ps) * (len(ps) - 1) // 2) for s, ps in seller_fan_out.items() if len(ps) >= 2),
        key=lambda r: -r[2])
    ...
    return dict(
        ...
        seller_betweenness=seller_betweenness,
...
def render_report(adtech, auth, creatives, delivery, fraud, a):
    ...
    # ── seller betweenness centrality (the depends/follow graph) ──
    P("## Seller betweenness centrality — supply-chain bridge ranking")
    P("")
    P("In the publisher↔seller authorization graph, a seller bridges every PAIR of publishers "
    ...
    P("|---|---:|---:|")
    for s, fan, btw in a['seller_betweenness'][:15]:
        P(f"| {aname(adtech, s)} | {fan} | {btw} |")
    if not a['seller_betweenness']:
...
def render_datoms(a):
    ...
    for s, fan, btw in a['seller_betweenness']:
        P(f' {{:adsupply/seller {edn_str(s)} :adsupply/seller-betweenness {btw} '
          f':adsupply/seller-fan-in {fan} :adsupply/derived true}}')
    for asn, load, n in a['infra_rank']:
