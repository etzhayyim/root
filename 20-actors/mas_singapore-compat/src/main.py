"""
Py Kotodama WASM entrypoint for Mas_singapore Cognitive Actor.
...
schema = load_schema("../schema/mas_singapore.kotoba")
db = DatomicClient.connect()
app = Runtime("mas_singapore-compat")
...
@app.route("/{api_version}/mas_singapore/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mas_singapore_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mas_singapore.CognitiveRecord/id": internal_record_id,
        "mas_singapore.CognitiveRecord/apiVersion": api_version,
        "mas_singapore.CognitiveRecord/originalPayload": str(raw_data),
        "mas_singapore.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mas_singapore.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mas_singapore_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mas_singapore_entity",
