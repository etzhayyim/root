"""
Py Kotodama WASM entrypoint for Ge_proficy Cognitive Actor.
...
schema = load_schema("../schema/ge_proficy.kotoba")
db = DatomicClient.connect()
app = Runtime("ge_proficy-compat")
...
@app.route("/{api_version}/ge_proficy/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ge_proficy_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ge_proficy.CognitiveRecord/id": internal_record_id,
        "ge_proficy.CognitiveRecord/apiVersion": api_version,
        "ge_proficy.CognitiveRecord/originalPayload": str(raw_data),
        "ge_proficy.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ge_proficy.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ge_proficy_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ge_proficy_entity",
