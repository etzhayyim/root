"""
Py Kotodama WASM entrypoint for Nintex Cognitive Actor.
...
schema = load_schema("../schema/nintex.kotoba")
db = DatomicClient.connect()
app = Runtime("nintex-compat")
...
@app.route("/{api_version}/nintex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nintex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nintex.CognitiveRecord/id": internal_record_id,
        "nintex.CognitiveRecord/apiVersion": api_version,
        "nintex.CognitiveRecord/originalPayload": str(raw_data),
        "nintex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nintex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nintex_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nintex_entity",
