"""
Py Kotodama WASM entrypoint for the Pjm_interconnection clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"pjm_interconnection.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"pjm_interconnection.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_meter(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.Meter", "id": eid})
...
def delete_reading(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.Reading", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_node(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.Node", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_load_forecast(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.LoadForecast", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_market_bid(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.MarketBid", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_tag(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"pjm_interconnection.Tag", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "pjm_interconnection-compat", "entities": ['Meter', 'Reading', 'Node', 'LoadForecast', 'MarketBid', 'Tag']}, 200
