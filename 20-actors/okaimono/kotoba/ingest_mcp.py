#!/usr/bin/env python3
"""okaimono 御買物 — ingest seed.edn into a live kotoba node via MCP.

ADR-2606012100. Flattens each seed entity map into (graph, subject, predicate,
object) datoms and asserts them via the `kotoba_datom_create` MCP tool; `kotoba
commit` seals them. Writes require an operator AT-session JWT (no-server-key
...
def main() -> int:
    ...
    # crude attribute count: number of ` :ns/attr ` tokens across entities
    datoms = sum(e.count(" :") + (1 if e.startswith("{:") else 0) for e in entities)

    print(f"   parsed {len(entities)} entities (~{datoms} datoms) from seed.edn")
    ...
    # NOTE: R0 scaffold — wire to the live MCP endpoint when the operator session is
