(ns epa.main
  "Kotodama WASM entrypoint for the Epa clean-room actor (L4) — Clojure port.
  ...
  facts (`epa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"epa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epa-compat" :tier tier :entities entities} 200])
