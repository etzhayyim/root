"""
Py Kotodama WASM entrypoint for Epa Cognitive Actor.
...
schema = load_schema("../schema/epa.kotoba")
db = DatomicClient.connect()
app = Runtime("epa-compat")
...
@app.route("/{api_version}/epa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_epa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "epa.CognitiveRecord/id": internal_record_id,
        "epa.CognitiveRecord/apiVersion": api_version,
        "epa.CognitiveRecord/originalPayload": str(raw_data),
        "epa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "epa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_epa_" + uuid.uuid4().hex[:8])
