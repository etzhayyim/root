(ns europol.main
  "Kotodama WASM entrypoint for the Europol clean-room actor (L4) — Clojure port.
  ...
  facts (`europol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "europol")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "europol_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "europol_par"
    :fields [:caseId :role :name]            :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "europol_doc"
    :fields [:caseId :type :contentRef]      :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "europol_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "europol_wat"
    :fields [:name :program :entries]        :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "europol_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"europol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "europol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :europol-compat/ready)
