"""
Py Kotodama WASM entrypoint for the One_network clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"one_network.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"one_network.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_trade_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.TradeItem", "id": eid})
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_document(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.Document", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_container(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.Container", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_party(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.Party", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_shipment(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"one_network.Shipment", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "one_network-compat", "entities": ['TradeItem', 'Event', 'Document', 'Container', 'Party', 'Shipment']}, 200
