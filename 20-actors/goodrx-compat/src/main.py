"""
Py Kotodama WASM entrypoint for Goodrx Cognitive Actor.
...
schema = load_schema("../schema/goodrx.kotoba")
db = DatomicClient.connect()
app = Runtime("goodrx-compat")
...
@app.route("/{api_version}/goodrx/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_goodrx_" + uuid.uuid4().hex[:12]
    db.transact([{
        "goodrx.CognitiveRecord/id": internal_record_id,
        "goodrx.CognitiveRecord/apiVersion": api_version,
        "goodrx.CognitiveRecord/originalPayload": str(raw_data),
        "goodrx.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "goodrx.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_goodrx_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "goodrx_entity",
