"""
Py Kotodama WASM entrypoint for Sonarqube Cognitive Actor.
...
@app.route("/{api_version}/sonarqube/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sonarqube_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sonarqube.CognitiveRecord/id": internal_record_id,
        "sonarqube.CognitiveRecord/apiVersion": api_version,
        "sonarqube.CognitiveRecord/originalPayload": str(raw_data),
        "sonarqube.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sonarqube.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sonarqube_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sonarqube_entity",
