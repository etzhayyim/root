"""
Py Kotodama WASM entrypoint for Interactivebrokers Cognitive Actor.
...
schema = load_schema("../schema/interactivebrokers.kotoba")
db = DatomicClient.connect()
app = Runtime("interactivebrokers-compat")
...
@app.route("/{api_version}/interactivebrokers/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_interactivebrokers_" + uuid.uuid4().hex[:12]
    db.transact([{
        "interactivebrokers.CognitiveRecord/id": internal_record_id,
        "interactivebrokers.CognitiveRecord/apiVersion": api_version,
        "interactivebrokers.CognitiveRecord/originalPayload": str(raw_data),
        "interactivebrokers.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "interactivebrokers.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_interactivebrokers_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "interactivebrokers_entity",
