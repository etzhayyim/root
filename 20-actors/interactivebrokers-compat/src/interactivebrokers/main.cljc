(ns interactivebrokers.main
  "Kotodama WASM entrypoint for the Interactivebrokers clean-room actor (L4) — Clojure port.
  ...
  facts (`interactivebrokers.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "interactivebrokers")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"interactivebrokers.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "interactivebrokers-compat" :tier tier :entities entities} 200])
...
(defn start! [] :interactivebrokers-compat/ready)
