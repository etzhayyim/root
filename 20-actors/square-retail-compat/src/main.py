"""
Py Kotodama WASM entrypoint for Square-retail Cognitive Actor.
...
schema = load_schema("../schema/square-retail.kotoba")
db = DatomicClient.connect()
app = Runtime("square-retail-compat")
...
@app.route("/{api_version}/square-retail/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_square-retail_" + uuid.uuid4().hex[:12]
    db.transact([{
        "square-retail.CognitiveRecord/id": internal_record_id,
        "square-retail.CognitiveRecord/apiVersion": api_version,
        "square-retail.CognitiveRecord/originalPayload": str(raw_data),
        "square-retail.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "square-retail.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_square-retail_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "square-retail_entity",
