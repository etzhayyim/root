"""
Py Kotodama WASM entrypoint for Greenhouse Cognitive Actor.
...
@app.route("/{api_version}/greenhouse/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_greenhouse_" + uuid.uuid4().hex[:12]
    db.transact([{
        "greenhouse.CognitiveRecord/id": internal_record_id,
        "greenhouse.CognitiveRecord/apiVersion": api_version,
        "greenhouse.CognitiveRecord/originalPayload": str(raw_data),
        "greenhouse.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "greenhouse.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_greenhouse_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "greenhouse_entity",
