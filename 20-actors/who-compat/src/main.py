"""
Py Kotodama WASM entrypoint for Who Cognitive Actor.
...
schema = load_schema("../schema/who.kotoba")
db = DatomicClient.connect()
app = Runtime("who-compat")
...
@app.route("/{api_version}/who/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_who_" + uuid.uuid4().hex[:12]
    db.transact([{
        "who.CognitiveRecord/id": internal_record_id,
        "who.CognitiveRecord/apiVersion": api_version,
        "who.CognitiveRecord/originalPayload": str(raw_data),
        "who.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "who.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_who_" + uuid.uuid4().hex[:8])
