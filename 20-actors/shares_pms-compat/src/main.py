"""
Py Kotodama WASM entrypoint for Shares_pms Cognitive Actor.
...
schema = load_schema("../schema/shares_pms.kotoba")
db = DatomicClient.connect()
app = Runtime("shares_pms-compat")
...
@app.route("/{api_version}/shares_pms/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shares_pms_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shares_pms.CognitiveRecord/id": internal_record_id,
        "shares_pms.CognitiveRecord/apiVersion": api_version,
        "shares_pms.CognitiveRecord/originalPayload": str(raw_data),
        "shares_pms.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shares_pms.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shares_pms_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shares_pms_entity",
