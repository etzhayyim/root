"""
Py Kotodama WASM entrypoint for Optiva Cognitive Actor.
...
schema = load_schema("../schema/optiva.kotoba")
db = DatomicClient.connect()
app = Runtime("optiva-compat")
...
@app.route("/{api_version}/optiva/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_optiva_" + uuid.uuid4().hex[:12]
    db.transact([{
        "optiva.CognitiveRecord/id": internal_record_id,
        "optiva.CognitiveRecord/apiVersion": api_version,
        "optiva.CognitiveRecord/originalPayload": str(raw_data),
        "optiva.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "optiva.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_optiva_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "optiva_entity",
