// Package magatama — Declaration-First App SDK.
//
...
//	    magatama.AsAgentTool("Paint a cell with a color number"),  // → ActorCard.tools + ActorCapability
//	    magatama.WithCapabilityTags("game", "coloring"),           // → discovery tags
//	    magatama.WithWLexicon("paint"),                            // → W Protocol Firehose routing
//	    magatama.WithSignalEncrypt("canvas_id"),                   // → E2E encryption
//	    magatama.Responsible(magatama.AssigneeOrgRole, "player"),  // → RACI governance
//	    magatama.WithOCELEvent("cell.painted"),                    // → OCEL process mining
//	)
...
// Evolution is handled by magatama-evolver (Rust runtime).
...
// AppDef is the top-level declaration for a magatama WASM application.
type AppDef struct {
	ID          string
	Name        string
	Description string
...
func (a *App) OnDailyEvolution(fn func(*AppContext) map[string]float64) *App {
	return a // no-op — evolution is handled by magatama-evolver (Rust runtime)
}

// Serve wires up HTTP handler and W Protocol commit handler via magatama WIT exports.
...
func (a *App) autoRegisterIdentity() {
	...
	if errMsg := IdentityRegister(card); errMsg != "" {
		...
		fmt.Println("magatama: auto-register identity failed:", errMsg)
	}
...
func (a *App) autoRegisterCapabilities() {
	for _, cmd := range a.commands {
		...
		if errMsg := CapabilityDeclare(cap); errMsg != "" {
			fmt.Println("magatama: auto-declare capability failed:", cmd.name, errMsg)
		}
	}
}
...
func (a *App) registerGovernanceManifest() {
	...
	if err != nil {
		fmt.Println("magatama: governance manifest marshal:", err)
		return
	}
	if errMsg := GovernanceRegisterManifest(string(manifestJSON)); errMsg != "" {
		fmt.Println("magatama: governance register manifest:", errMsg)
	}
}
...
func (a *App) handleHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Timer callback dispatch: POST /{MethodName} (bare path from magatama runtime)
