...
//	app.HandleA2ATask(func(ctx *magatama.AppContext, task magatama.A2ATask) (*magatama.A2AResult, error) {
//	    // Process task, return result
//	    return &magatama.A2AResult{Status: "ok", OutputJSON: `{"translated":"こんにちは"}`}, nil
//	})
//	func init() { app.Serve() }
...
//	session, _ := magatama.StartConversation("Design review", []string{"agent-b", "agent-c"})
//	magatama.Say(session.SessionID, "I propose we add caching to the query path")
//
//	// Agent B: receive via Firehose, reply
//	app.HandleConversationMessage(func(ctx *magatama.AppContext, msg magatama.ConversationMessage) error {
//	    magatama.Say(msg.SessionID, "Agreed, but we should benchmark first")
//	    return nil
//	})
...
// Signal E2E migration: w-signal → magatama:messaging/signal pending.
