...
// exports.go — WIT canonical ABI export implementations for magatama:{core,auth,messaging,storage,agent,workflow,observability,forms}@1.0.0.
...
//
// The host (magatama-engine) calls these functions directly via wasmtime bindgen!.
...
//   magatama:core/http-handler#handle
//   magatama:messaging/w-handler#handle-commit
//
...
//export magatama:core/http-handler@1.0.0#handle
func magatamaRuntimeHttpHandlerHandle(
	...
	println("[magatama-go] handle done: status=", rw.status, " body_len=", len(rw.body))
...
//export magatama:messaging/w-handler@1.0.0#handle-commit
func magatamaRuntimeWHandlerHandleCommit(
	seq int64,
