...
// ErrNotAvailable is returned by host functions when running outside of magatama (native mode).
var ErrNotAvailable = errors.New("magatama: host function not available in native mode")

// ErrCypherNotAvailable is returned by CypherQuery when running outside of magatama (native mode).
...
func Serve() {
	if err := ServeAt(listenAddr()); err != nil {
		log.Fatalf("magatama: serve: %v", err)
	}
}
...
func ServeAt(addr string) error {
	h := registeredHandler
	if h == nil {
		return fmt.Errorf("magatama: no handler registered — call Handle() in init()")
	}
	fmt.Printf("magatama: listening on %s\n", addr)
