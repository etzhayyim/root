"""
Py Kotodama WASM entrypoint for Mckesson Cognitive Actor.
...
schema = load_schema("../schema/mckesson.kotoba")
db = DatomicClient.connect()
app = Runtime("mckesson-compat")
...
@app.route("/{api_version}/mckesson/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mckesson_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mckesson.CognitiveRecord/id": internal_record_id,
        "mckesson.CognitiveRecord/apiVersion": api_version,
        "mckesson.CognitiveRecord/originalPayload": str(raw_data),
        "mckesson.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mckesson.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mckesson_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mckesson_entity",
