...
from live_gate import LiveGate, require, LiveGateRefused

# Calculate Chokepoint Index (CI)
def calculate_ci(traits):
    score = 0
    weights = {
    ...
    for trait, active in traits.items():
        if active and trait in weights:
            score += weights[trait]
