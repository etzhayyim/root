(ns infineon_qkd.main-test
  "Contract + behavioral test for the infineon_qkd-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "infineon_qkd-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'infineon_qkd.main-test)]
