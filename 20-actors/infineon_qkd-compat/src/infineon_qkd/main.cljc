(ns infineon_qkd.main
  "Kotodama WASM entrypoint for the Infineon_qkd clean-room actor (L4) — Clojure port.
  ...
  facts (`infineon_qkd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "infineon_qkd")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"infineon_qkd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "infineon_qkd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :infineon_qkd-compat/ready)
