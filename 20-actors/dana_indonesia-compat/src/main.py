"""
Py Kotodama WASM entrypoint for Dana_indonesia Cognitive Actor.
...
schema = load_schema("../schema/dana_indonesia.kotoba")
db = DatomicClient.connect()
app = Runtime("dana_indonesia-compat")
...
@app.route("/{api_version}/dana_indonesia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dana_indonesia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dana_indonesia.CognitiveRecord/id": internal_record_id,
        "dana_indonesia.CognitiveRecord/apiVersion": api_version,
        "dana_indonesia.CognitiveRecord/originalPayload": str(raw_data),
        "dana_indonesia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dana_indonesia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dana_indonesia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "dana_indonesia_entity",
