"""
Py Kotodama WASM entrypoint for Docusign Cognitive Actor.
...
@app.route("/{api_version}/docusign/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_docusign_" + uuid.uuid4().hex[:12]
    db.transact([{
        "docusign.CognitiveRecord/id": internal_record_id,
        "docusign.CognitiveRecord/apiVersion": api_version,
        "docusign.CognitiveRecord/originalPayload": str(raw_data),
        "docusign.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "docusign.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_docusign_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "docusign_entity",
