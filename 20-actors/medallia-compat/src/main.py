"""
Py Kotodama WASM entrypoint for Medallia Cognitive Actor.
...
schema = load_schema("../schema/medallia.kotoba")
db = DatomicClient.connect()
app = Runtime("medallia-compat")
...
@app.route("/{api_version}/medallia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_medallia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "medallia.CognitiveRecord/id": internal_record_id,
        "medallia.CognitiveRecord/apiVersion": api_version,
        "medallia.CognitiveRecord/originalPayload": str(raw_data),
        "medallia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "medallia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_medallia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "medallia_entity",
