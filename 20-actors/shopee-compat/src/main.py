"""
Py Kotodama WASM entrypoint for Shopee Cognitive Actor.
...
@app.route("/{api_version}/shopee/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shopee_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shopee.CognitiveRecord/id": internal_record_id,
        "shopee.CognitiveRecord/apiVersion": api_version,
        "shopee.CognitiveRecord/originalPayload": str(raw_data),
        "shopee.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shopee.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shopee_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shopee_entity",
