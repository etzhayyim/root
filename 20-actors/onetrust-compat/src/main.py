"""
Py Kotodama WASM entrypoint for Onetrust Cognitive Actor.
...
schema = load_schema("../schema/onetrust.kotoba")
db = DatomicClient.connect()
app = Runtime("onetrust-compat")
...
@app.route("/{api_version}/onetrust/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_onetrust_" + uuid.uuid4().hex[:12]
    db.transact([{
        "onetrust.CognitiveRecord/id": internal_record_id,
        "onetrust.CognitiveRecord/apiVersion": api_version,
        "onetrust.CognitiveRecord/originalPayload": str(raw_data),
        "onetrust.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "onetrust.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_onetrust_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "onetrust_entity",
