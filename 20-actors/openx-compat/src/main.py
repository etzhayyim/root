"""
Py Kotodama WASM entrypoint for Openx Cognitive Actor.
...
schema = load_schema("../schema/openx.kotoba")
db = DatomicClient.connect()
app = Runtime("openx-compat")
...
@app.route("/{api_version}/openx/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_openx_" + uuid.uuid4().hex[:12]
    db.transact([{
        "openx.CognitiveRecord/id": internal_record_id,
        "openx.CognitiveRecord/apiVersion": api_version,
        "openx.CognitiveRecord/originalPayload": str(raw_data),
        "openx.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "openx.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_openx_" + uuid.uuid4().hex[:8])
