"""
Py Kotodama WASM entrypoint for Splunk Cognitive Actor.
...
schema = load_schema("../schema/splunk.kotoba")
db = DatomicClient.connect()
app = Runtime("splunk-compat")
...
@app.route("/{api_version}/splunk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_splunk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "splunk.CognitiveRecord/id": internal_record_id,
        "splunk.CognitiveRecord/apiVersion": api_version,
        "splunk.CognitiveRecord/originalPayload": str(raw_data),
        "splunk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "splunk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_splunk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "splunk_entity",
