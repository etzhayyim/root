"""
Py Kotodama WASM entrypoint for the Amd_rocm clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"amd_rocm.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.Device", "id": eid})
...
def delete_kernel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.Kernel", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_alloc(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.MemoryAlloc", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inference_job(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.InferenceJob", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_engine(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.Engine", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_utilization(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"amd_rocm.Utilization", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "amd_rocm-compat", "entities": ['Device', 'Kernel', 'MemoryAlloc', 'InferenceJob', 'Engine', 'Utilization']}, 200
