"""
Py Kotodama WASM entrypoint for Sap Cognitive Actor.
...
schema = load_schema("../schema/sap.kotoba")
db = DatomicClient.connect()
app = Runtime("sap-compat")
...
@app.route("/{api_version}/sap/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sap_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sap.CognitiveRecord/id": internal_record_id,
        "sap.CognitiveRecord/apiVersion": api_version,
        "sap.CognitiveRecord/originalPayload": str(raw_data),
        "sap.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sap.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sap_" + uuid.uuid4().hex[:8])
