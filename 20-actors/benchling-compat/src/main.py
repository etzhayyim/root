"""
Py Kotodama WASM entrypoint for Benchling Cognitive Actor.
...
@app.route("/{api_version}/benchling/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_benchling_" + uuid.uuid4().hex[:12]
    db.transact([{
        "benchling.CognitiveRecord/id": internal_record_id,
        "benchling.CognitiveRecord/apiVersion": api_version,
        "benchling.CognitiveRecord/originalPayload": str(raw_data),
        "benchling.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "benchling.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_benchling_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "benchling_entity",
