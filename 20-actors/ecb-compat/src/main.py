"""
Py Kotodama WASM entrypoint for Ecb Cognitive Actor.
...
schema = load_schema("../schema/ecb.kotoba")
db = DatomicClient.connect()
app = Runtime("ecb-compat")
...
@app.route("/{api_version}/ecb/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ecb_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ecb.CognitiveRecord/id": internal_record_id,
        "ecb.CognitiveRecord/apiVersion": api_version,
        "ecb.CognitiveRecord/originalPayload": str(raw_data),
        "ecb.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ecb.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ecb_" + uuid.uuid4().hex[:8])
