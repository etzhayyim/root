"""
Py Kotodama WASM entrypoint for Siteminder Cognitive Actor.
...
schema = load_schema("../schema/siteminder.kotoba")
db = DatomicClient.connect()
app = Runtime("siteminder-compat")
...
@app.route("/{api_version}/siteminder/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_siteminder_" + uuid.uuid4().hex[:12]
    db.transact([{
        "siteminder.CognitiveRecord/id": internal_record_id,
        "siteminder.CognitiveRecord/apiVersion": api_version,
        "siteminder.CognitiveRecord/originalPayload": str(raw_data),
        "siteminder.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "siteminder.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_siteminder_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "siteminder_entity",
