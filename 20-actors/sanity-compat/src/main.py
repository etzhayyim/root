"""
Py Kotodama WASM entrypoint for Sanity Cognitive Actor.
...
schema = load_schema("../schema/sanity.kotoba")
db = DatomicClient.connect()
app = Runtime("sanity-compat")
...
@app.route("/{api_version}/sanity/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sanity_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sanity.CognitiveRecord/id": internal_record_id,
        "sanity.CognitiveRecord/apiVersion": api_version,
        "sanity.CognitiveRecord/originalPayload": str(raw_data),
        "sanity.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sanity.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sanity_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sanity_entity",
