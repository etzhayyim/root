"""
Py Kotodama WASM entrypoint for Xroad Cognitive Actor.
...
@app.route("/{api_version}/xroad/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_xroad_" + uuid.uuid4().hex[:12]
    db.transact([{
        "xroad.CognitiveRecord/id": internal_record_id,
        "xroad.CognitiveRecord/apiVersion": api_version,
        "xroad.CognitiveRecord/originalPayload": str(raw_data),
        "xroad.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "xroad.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_xroad_" + uuid.uuid4().hex[:8])
