...
set -euo pipefail
cd "$(dirname "$0")/../.."
exec bb -e '(require (quote clojure.test) (quote hokorobi.methods.test-datom-emit) (quote hokorobi.tests.test-analyze) (quote hokorobi.tests.test-coverage))(let [r (apply clojure.test/run-tests (quote [hokorobi.methods.test-datom-emit hokorobi.tests.test-analyze hokorobi.tests.test-coverage]))](System/exit (if (zero? (+ (:fail r) (:error r))) 0 1)))'
