"""
Py Kotodama WASM entrypoint for Freefly Cognitive Actor.
...
@app.route("/{api_version}/freefly/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_freefly_" + uuid.uuid4().hex[:12]
    db.transact([{
        "freefly.CognitiveRecord/id": internal_record_id,
        "freefly.CognitiveRecord/apiVersion": api_version,
        "freefly.CognitiveRecord/originalPayload": str(raw_data),
        "freefly.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "freefly.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_freefly_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "freefly_entity",
