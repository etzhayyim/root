"""
Py Kotodama WASM entrypoint for Freshworks Cognitive Actor.
...
schema = load_schema("../schema/freshworks.kotoba")
db = DatomicClient.connect()
app = Runtime("freshworks-compat")
...
@app.route("/{api_version}/freshworks/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_freshworks_" + uuid.uuid4().hex[:12]
    db.transact([{
        "freshworks.CognitiveRecord/id": internal_record_id,
        "freshworks.CognitiveRecord/apiVersion": api_version,
        "freshworks.CognitiveRecord/originalPayload": str(raw_data),
        "freshworks.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "freshworks.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_freshworks_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "freshworks_entity",
