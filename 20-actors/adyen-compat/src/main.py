"""
Py Kotodama WASM entrypoint for Adyen Cognitive Actor.
...
@app.route("/{api_version}/adyen/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_adyen_" + uuid.uuid4().hex[:12]
    db.transact([{
        "adyen.CognitiveRecord/id": internal_record_id,
        "adyen.CognitiveRecord/apiVersion": api_version,
        "adyen.CognitiveRecord/originalPayload": str(raw_data),
        "adyen.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "adyen.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_adyen_" + uuid.uuid4().hex[:8])
