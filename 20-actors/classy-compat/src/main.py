"""
Py Kotodama WASM entrypoint for Classy Cognitive Actor.
...
@app.route("/{api_version}/classy/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_classy_" + uuid.uuid4().hex[:12]
    db.transact([{
        "classy.CognitiveRecord/id": internal_record_id,
        "classy.CognitiveRecord/apiVersion": api_version,
        "classy.CognitiveRecord/originalPayload": str(raw_data),
        "classy.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "classy.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_classy_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "classy_entity",
