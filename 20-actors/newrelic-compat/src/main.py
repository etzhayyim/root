"""
Py Kotodama WASM entrypoint for the Newrelic clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"newrelic.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_service(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Service", "id": eid})
...
def delete_error(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Error", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trace(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Trace", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_metric(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Metric", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_alert(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Alert", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_incident(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"newrelic.Incident", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "newrelic-compat", "entities": ['Service', 'Error', 'Trace', 'Metric', 'Alert', 'Incident']}, 200
