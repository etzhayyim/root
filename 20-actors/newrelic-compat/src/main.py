"""
Py Kotodama WASM entrypoint for Newrelic Cognitive Actor.
...
schema = load_schema("../schema/newrelic.kotoba")
db = DatomicClient.connect()
app = Runtime("newrelic-compat")
...
@app.route("/{api_version}/newrelic/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_newrelic_" + uuid.uuid4().hex[:12]
    db.transact([{
        "newrelic.CognitiveRecord/id": internal_record_id,
        "newrelic.CognitiveRecord/apiVersion": api_version,
        "newrelic.CognitiveRecord/originalPayload": str(raw_data),
        "newrelic.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "newrelic.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_newrelic_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "newrelic_entity",
