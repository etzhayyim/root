"""
Py Kotodama WASM entrypoint for Wmo_gts Cognitive Actor.
...
schema = load_schema("../schema/wmo_gts.kotoba")
db = DatomicClient.connect()
app = Runtime("wmo_gts-compat")
...
@app.route("/{api_version}/wmo_gts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_wmo_gts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "wmo_gts.CognitiveRecord/id": internal_record_id,
        "wmo_gts.CognitiveRecord/apiVersion": api_version,
        "wmo_gts.CognitiveRecord/originalPayload": str(raw_data),
        "wmo_gts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "wmo_gts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_wmo_gts_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "wmo_gts_entity",
