"""
Py Kotodama WASM entrypoint for the Aws_inferentia clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"aws_inferentia.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"aws_inferentia.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.Device", "id": eid})
...
def delete_kernel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.Kernel", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_alloc(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.MemoryAlloc", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inference_job(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.InferenceJob", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_engine(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.Engine", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_utilization(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aws_inferentia.Utilization", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "aws_inferentia-compat", "entities": ['Device', 'Kernel', 'MemoryAlloc', 'InferenceJob', 'Engine', 'Utilization']}, 200
