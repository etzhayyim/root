(ns firstdata_network-compat.main
  "Kotodama WASM entrypoint for the Firstdata_network clean-room actor (L4) — Clojure port.
  ...
  facts (`firstdata_network.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "firstdata_network")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"firstdata_network.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "firstdata_network-compat" :tier tier :entities entities} 200])
...
(defn start! [] :firstdata_network-compat/ready)
