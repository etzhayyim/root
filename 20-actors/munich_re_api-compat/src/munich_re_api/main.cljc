(ns munich_re_api.main
  "Kotodama WASM entrypoint for the Munich_re_api clean-room actor (L4) — Clojure port.
  ...
  facts (`munich_re_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "munich_re_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "munichre_acc"
    :fields [:holderId :type :balance :currency :status]        :required [:type :balance]
    :coerce {:balance :float}                :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "munichre_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"munich_re_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "munich_re_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :munich_re_api-compat/ready)
