"""
Py Kotodama WASM entrypoint for Ncts Cognitive Actor.
...
schema = load_schema("../schema/ncts.kotoba")
db = DatomicClient.connect()
app = Runtime("ncts-compat")
...
@app.route("/{api_version}/ncts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ncts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ncts.CognitiveRecord/id": internal_record_id,
        "ncts.CognitiveRecord/apiVersion": api_version,
        "ncts.CognitiveRecord/originalPayload": str(raw_data),
        "ncts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ncts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ncts_" + uuid.uuid4().hex[:8])
