"""
Py Kotodama WASM entrypoint for Kinaxis Cognitive Actor.
...
schema = load_schema("../schema/kinaxis.kotoba")
db = DatomicClient.connect()
app = Runtime("kinaxis-compat")
...
@app.route("/{api_version}/kinaxis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_kinaxis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "kinaxis.CognitiveRecord/id": internal_record_id,
        "kinaxis.CognitiveRecord/apiVersion": api_version,
        "kinaxis.CognitiveRecord/originalPayload": str(raw_data),
        "kinaxis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "kinaxis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_kinaxis_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "kinaxis_entity",
