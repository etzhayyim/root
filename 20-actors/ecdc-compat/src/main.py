"""
Py Kotodama WASM entrypoint for Ecdc Cognitive Actor.
...
@app.route("/{api_version}/ecdc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ecdc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ecdc.CognitiveRecord/id": internal_record_id,
        "ecdc.CognitiveRecord/apiVersion": api_version,
        "ecdc.CognitiveRecord/originalPayload": str(raw_data),
        "ecdc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ecdc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ecdc_" + uuid.uuid4().hex[:8])
