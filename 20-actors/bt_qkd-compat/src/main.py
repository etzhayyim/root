"""
Py Kotodama WASM entrypoint for Bt_qkd Cognitive Actor.
...
schema = load_schema("../schema/bt_qkd.kotoba")
db = DatomicClient.connect()
app = Runtime("bt_qkd-compat")
...
@app.route("/{api_version}/bt_qkd/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bt_qkd_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bt_qkd.CognitiveRecord/id": internal_record_id,
        "bt_qkd.CognitiveRecord/apiVersion": api_version,
        "bt_qkd.CognitiveRecord/originalPayload": str(raw_data),
        "bt_qkd.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bt_qkd.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bt_qkd_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bt_qkd_entity",
