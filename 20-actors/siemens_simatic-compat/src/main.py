"""
Py Kotodama WASM entrypoint for Siemens_simatic Cognitive Actor.
...
schema = load_schema("../schema/siemens_simatic.kotoba")
db = DatomicClient.connect()
app = Runtime("siemens_simatic-compat")
...
@app.route("/{api_version}/siemens_simatic/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_siemens_simatic_" + uuid.uuid4().hex[:12]
    db.transact([{
        "siemens_simatic.CognitiveRecord/id": internal_record_id,
        "siemens_simatic.CognitiveRecord/apiVersion": api_version,
        "siemens_simatic.CognitiveRecord/originalPayload": str(raw_data),
        "siemens_simatic.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "siemens_simatic.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_siemens_simatic_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "siemens_simatic_entity",
