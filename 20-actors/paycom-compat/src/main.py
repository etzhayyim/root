"""
Py Kotodama WASM entrypoint for Paycom Cognitive Actor.
...
schema = load_schema("../schema/paycom.kotoba")
db = DatomicClient.connect()
app = Runtime("paycom-compat")
...
@app.route("/{api_version}/paycom/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_paycom_" + uuid.uuid4().hex[:12]
    db.transact([{
        "paycom.CognitiveRecord/id": internal_record_id,
        "paycom.CognitiveRecord/apiVersion": api_version,
        "paycom.CognitiveRecord/originalPayload": str(raw_data),
        "paycom.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "paycom.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_paycom_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "paycom_entity",
