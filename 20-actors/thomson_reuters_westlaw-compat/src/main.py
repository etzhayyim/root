"""
Py Kotodama WASM entrypoint for Thomson_reuters_westlaw Cognitive Actor.
...
schema = load_schema("../schema/thomson_reuters_westlaw.kotoba")
db = DatomicClient.connect()
app = Runtime("thomson_reuters_westlaw-compat")
...
@app.route("/{api_version}/thomson_reuters_westlaw/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_thomson_reuters_westlaw_" + uuid.uuid4().hex[:12]
    db.transact([{
        "thomson_reuters_westlaw.CognitiveRecord/id": internal_record_id,
        "thomson_reuters_westlaw.CognitiveRecord/apiVersion": api_version,
        "thomson_reuters_westlaw.CognitiveRecord/originalPayload": str(raw_data),
        "thomson_reuters_westlaw.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "thomson_reuters_westlaw.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_thomson_reuters_westlaw_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "thomson_reuters_westlaw_entity",
