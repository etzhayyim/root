"""
Py Kotodama WASM entrypoint for Github Cognitive Actor.
...
@app.route("/{api_version}/github/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_github_" + uuid.uuid4().hex[:12]
    db.transact([{
        "github.CognitiveRecord/id": internal_record_id,
        "github.CognitiveRecord/apiVersion": api_version,
        "github.CognitiveRecord/originalPayload": str(raw_data),
        "github.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "github.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_github_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "github_entity",
