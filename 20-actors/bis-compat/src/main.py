"""
Py Kotodama WASM entrypoint for Bis Cognitive Actor.
...
schema = load_schema("../schema/bis.kotoba")
db = DatomicClient.connect()
app = Runtime("bis-compat")
...
@app.route("/{api_version}/bis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bis.CognitiveRecord/id": internal_record_id,
        "bis.CognitiveRecord/apiVersion": api_version,
        "bis.CognitiveRecord/originalPayload": str(raw_data),
        "bis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bis_" + uuid.uuid4().hex[:8])
