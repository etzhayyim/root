...
set -euo pipefail
cd "$(dirname "$0")/../.."
exec bb -e '(require (quote clojure.test) (quote hoshimori.methods.test-datom-emit) (quote hoshimori.methods.test-ingest) (quote hoshimori.tests.test-analyze) (quote hoshimori.tests.test-coverage))(let [r (apply clojure.test/run-tests (quote [hoshimori.methods.test-datom-emit hoshimori.methods.test-ingest hoshimori.tests.test-analyze hoshimori.tests.test-coverage]))](System/exit (if (zero? (+ (:fail r) (:error r))) 0 1)))'
