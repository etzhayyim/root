"""
Py Kotodama WASM entrypoint for Zengin_system Cognitive Actor.
...
schema = load_schema("../schema/zengin_system.kotoba")
db = DatomicClient.connect()
app = Runtime("zengin_system-compat")
...
@app.route("/{api_version}/zengin_system/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zengin_system_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zengin_system.CognitiveRecord/id": internal_record_id,
        "zengin_system.CognitiveRecord/apiVersion": api_version,
        "zengin_system.CognitiveRecord/originalPayload": str(raw_data),
        "zengin_system.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zengin_system.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zengin_system_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zengin_system_entity",
