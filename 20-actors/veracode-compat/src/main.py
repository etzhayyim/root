"""
Py Kotodama WASM entrypoint for Veracode Cognitive Actor.
...
schema = load_schema("../schema/veracode.kotoba")
db = DatomicClient.connect()
app = Runtime("veracode-compat")
...
@app.route("/{api_version}/veracode/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_veracode_" + uuid.uuid4().hex[:12]
    db.transact([{
        "veracode.CognitiveRecord/id": internal_record_id,
        "veracode.CognitiveRecord/apiVersion": api_version,
        "veracode.CognitiveRecord/originalPayload": str(raw_data),
        "veracode.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "veracode.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_veracode_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "veracode_entity",
