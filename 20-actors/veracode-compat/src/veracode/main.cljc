(ns veracode.main
  "Kotodama WASM entrypoint for the Veracode clean-room actor (L4) — Clojure port.
  ...
  facts (`veracode.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "veracode")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"veracode.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "veracode-compat" :tier tier :entities entities} 200])
...
(defn start! [] :veracode-compat/ready)
