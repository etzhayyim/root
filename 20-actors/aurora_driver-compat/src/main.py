"""
Py Kotodama WASM entrypoint for Aurora_driver Cognitive Actor.
...
schema = load_schema("../schema/aurora_driver.kotoba")
db = DatomicClient.connect()
app = Runtime("aurora_driver-compat")
...
@app.route("/{api_version}/aurora_driver/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aurora_driver_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aurora_driver.CognitiveRecord/id": internal_record_id,
        "aurora_driver.CognitiveRecord/apiVersion": api_version,
        "aurora_driver.CognitiveRecord/originalPayload": str(raw_data),
        "aurora_driver.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aurora_driver.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aurora_driver_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "aurora_driver_entity",
