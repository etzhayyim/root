"""
Py Kotodama WASM entrypoint for the Aurora_driver clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"aurora_driver.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"aurora_driver.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_vehicle(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.Vehicle", "id": eid})
...
def delete_signal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.Signal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.ChargeSession", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_connector(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.Connector", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trip(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_command(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"aurora_driver.Command", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "aurora_driver-compat", "entities": ['Vehicle', 'Signal', 'ChargeSession', 'Connector', 'Trip', 'Command']}, 200
