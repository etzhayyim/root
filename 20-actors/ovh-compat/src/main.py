"""
Py Kotodama WASM entrypoint for Ovh Cognitive Actor.
...
@app.route("/{api_version}/ovh/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ovh_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ovh.CognitiveRecord/id": internal_record_id,
        "ovh.CognitiveRecord/apiVersion": api_version,
        "ovh.CognitiveRecord/originalPayload": str(raw_data),
        "ovh.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ovh.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ovh_" + uuid.uuid4().hex[:8])
