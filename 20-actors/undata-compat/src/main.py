"""
Py Kotodama WASM entrypoint for Undata Cognitive Actor.
...
schema = load_schema("../schema/undata.kotoba")
db = DatomicClient.connect()
app = Runtime("undata-compat")
...
@app.route("/{api_version}/undata/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_undata_" + uuid.uuid4().hex[:12]
    db.transact([{
        "undata.CognitiveRecord/id": internal_record_id,
        "undata.CognitiveRecord/apiVersion": api_version,
        "undata.CognitiveRecord/originalPayload": str(raw_data),
        "undata.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "undata.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_undata_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "undata_entity",
