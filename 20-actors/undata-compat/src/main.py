"""
Py Kotodama WASM entrypoint for the Undata clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"undata.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_indicator(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.Indicator", "id": eid})
...
def delete_observation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.Observation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_country(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.Country", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_series(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.Series", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_report(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.Report", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_exchange_rate(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"undata.ExchangeRate", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "undata-compat", "entities": ['Indicator', 'Observation', 'Country', 'Series', 'Report', 'ExchangeRate']}, 200
