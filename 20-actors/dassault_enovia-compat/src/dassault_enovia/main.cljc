(ns dassault-enovia.main
  "Kotodama WASM entrypoint for the Dassault_enovia clean-room actor (L4) — Clojure port.
  ...
  facts (`dassault_enovia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dassault_enovia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dassault_enovia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dassault_enovia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dassault_enovia-compat/ready)
