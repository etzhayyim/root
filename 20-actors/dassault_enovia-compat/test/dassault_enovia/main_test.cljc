(ns dassault-enovia.main-test
  "Contract + behavioral test for the dassault-enovia L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "dassault_enovia-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'dassault-enovia.main-test)]
