"""
Py Kotodama WASM entrypoint for Paypal Cognitive Actor.
...
@app.route("/{api_version}/paypal/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_paypal_" + uuid.uuid4().hex[:12]
    db.transact([{
        "paypal.CognitiveRecord/id": internal_record_id,
        "paypal.CognitiveRecord/apiVersion": api_version,
        "paypal.CognitiveRecord/originalPayload": str(raw_data),
        "paypal.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "paypal.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_paypal_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "paypal_entity",
