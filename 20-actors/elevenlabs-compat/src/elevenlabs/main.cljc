(ns elevenlabs.main
  "Kotodama WASM entrypoint for the Elevenlabs clean-room actor (L5) — Clojure port.
  ...
  facts (`elevenlabs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "elevenlabs")
(def tier "L5")
...
(def entity-specs
  [{:entity "Voice"        :plural "voices"        :id-prefix "elevenla_voi"
    :fields [:voiceId :name :category :description :previewUrl :createdAtUnix :isOwner] :required [:name :category]
    :coerce {:createdAtUnix :int :isOwner :bool} :refs {}}
   {:entity "Sample"       :plural "samples"       :id-prefix "elevenla_sam"
    :fields [:sampleId :fileName :mimeType :sizeBytes :durationSecs :hash :removeBackgroundNoise] :required [:fileName :mimeType]
    :coerce {:sizeBytes :int :durationSecs :float :removeBackgroundNoise :bool} :refs {}}
   {:entity "HistoryItem"  :plural "historyitems"  :id-prefix "elevenla_his"
    :fields [:historyItemId :voiceId :modelId :text :dateUnix :state :characterCountChangeTo :contentType] :required [:text :dateUnix]
    :coerce {:dateUnix :int :characterCountChangeTo :int} :refs {:voiceId "Voice" :modelId "Model"}}
   {:entity "Model"        :plural "models"        :id-prefix "elevenla_mod"
    :fields [:modelId :name :canBeFinetune :canDoTextToSpeech :canUseStyle :tokenCostFactor :description] :required [:name :canBeFinetune]
    :coerce {:canBeFinetune :bool :canDoTextToSpeech :bool :canUseStyle :bool :tokenCostFactor :float} :refs {}}
   {:entity "Subscription" :plural "subscriptions" :id-prefix "elevenla_sub"
    :fields [:tier :characterCount :characterLimit :voiceSlotsUsed :voiceLimit :status :canExtendCharacterLimit] :required [:tier :characterCount]
    :coerce {:characterCount :int :characterLimit :int :voiceSlotsUsed :int :voiceLimit :int :canExtendCharacterLimit :bool} :refs {}}
   {:entity "Project"      :plural "projects"      :id-prefix "elevenla_pro"
    :fields [:projectId :name :title :author :createDateUnix :state :accessLevel :canBeDownloaded] :required [:name :title]
    :coerce {:createDateUnix :int :canBeDownloaded :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"elevenlabs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "elevenlabs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :elevenlabs-compat/ready)
