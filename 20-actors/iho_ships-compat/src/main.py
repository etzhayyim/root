"""
Py Kotodama WASM entrypoint for Iho_ships Cognitive Actor.
...
schema = load_schema("../schema/iho_ships.kotoba")
db = DatomicClient.connect()
app = Runtime("iho_ships-compat")
...
@app.route("/{api_version}/iho_ships/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iho_ships_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iho_ships.CognitiveRecord/id": internal_record_id,
        "iho_ships.CognitiveRecord/apiVersion": api_version,
        "iho_ships.CognitiveRecord/originalPayload": str(raw_data),
        "iho_ships.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iho_ships.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iho_ships_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iho_ships_entity",
