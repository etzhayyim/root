"""
Py Kotodama WASM entrypoint for Iceye Cognitive Actor.
...
schema = load_schema("../schema/iceye.kotoba")
db = DatomicClient.connect()
app = Runtime("iceye-compat")
...
@app.route("/{api_version}/iceye/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iceye_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iceye.CognitiveRecord/id": internal_record_id,
        "iceye.CognitiveRecord/apiVersion": api_version,
        "iceye.CognitiveRecord/originalPayload": str(raw_data),
        "iceye.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iceye.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iceye_" + uuid.uuid4().hex[:8])
