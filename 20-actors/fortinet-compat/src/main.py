"""
Py Kotodama WASM entrypoint for Fortinet Cognitive Actor.
...
schema = load_schema("../schema/fortinet.kotoba")
db = DatomicClient.connect()
app = Runtime("fortinet-compat")
...
@app.route("/{api_version}/fortinet/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fortinet_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fortinet.CognitiveRecord/id": internal_record_id,
        "fortinet.CognitiveRecord/apiVersion": api_version,
        "fortinet.CognitiveRecord/originalPayload": str(raw_data),
        "fortinet.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fortinet.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fortinet_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fortinet_entity",
