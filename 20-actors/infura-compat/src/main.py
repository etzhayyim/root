"""
Py Kotodama WASM entrypoint for Infura Cognitive Actor.
...
schema = load_schema("../schema/infura.kotoba")
db = DatomicClient.connect()
app = Runtime("infura-compat")
...
@app.route("/{api_version}/infura/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_infura_" + uuid.uuid4().hex[:12]
    db.transact([{
        "infura.CognitiveRecord/id": internal_record_id,
        "infura.CognitiveRecord/apiVersion": api_version,
        "infura.CognitiveRecord/originalPayload": str(raw_data),
        "infura.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "infura.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_infura_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "infura_entity",
