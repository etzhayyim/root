"""
Py Kotodama WASM entrypoint for Bentley_projectwise Cognitive Actor.
...
schema = load_schema("../schema/bentley_projectwise.kotoba")
db = DatomicClient.connect()
app = Runtime("bentley_projectwise-compat")
...
@app.route("/{api_version}/bentley_projectwise/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bentley_projectwise_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bentley_projectwise.CognitiveRecord/id": internal_record_id,
        "bentley_projectwise.CognitiveRecord/apiVersion": api_version,
        "bentley_projectwise.CognitiveRecord/originalPayload": str(raw_data),
        "bentley_projectwise.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bentley_projectwise.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bentley_projectwise_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bentley_projectwise_entity",
