"""
Py Kotodama WASM entrypoint for Barclays_marl Cognitive Actor.
...
schema = load_schema("../schema/barclays_marl.kotoba")
db = DatomicClient.connect()
app = Runtime("barclays_marl-compat")
...
@app.route("/{api_version}/barclays_marl/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_barclays_marl_" + uuid.uuid4().hex[:12]
    db.transact([{
        "barclays_marl.CognitiveRecord/id": internal_record_id,
        "barclays_marl.CognitiveRecord/apiVersion": api_version,
        "barclays_marl.CognitiveRecord/originalPayload": str(raw_data),
        "barclays_marl.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "barclays_marl.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_barclays_marl_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "barclays_marl_entity",
