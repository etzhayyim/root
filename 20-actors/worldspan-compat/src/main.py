"""
Py Kotodama WASM entrypoint for Worldspan Cognitive Actor.
...
schema = load_schema("../schema/worldspan.kotoba")
db = DatomicClient.connect()
app = Runtime("worldspan-compat")
...
@app.route("/{api_version}/worldspan/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_worldspan_" + uuid.uuid4().hex[:12]
    db.transact([{
        "worldspan.CognitiveRecord/id": internal_record_id,
        "worldspan.CognitiveRecord/apiVersion": api_version,
        "worldspan.CognitiveRecord/originalPayload": str(raw_data),
        "worldspan.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "worldspan.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_worldspan_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "worldspan_entity",
