"""
Py Kotodama WASM entrypoint for Irs Cognitive Actor.
...
schema = load_schema("../schema/irs.kotoba")
db = DatomicClient.connect()
app = Runtime("irs-compat")
...
@app.route("/{api_version}/irs/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_irs_" + uuid.uuid4().hex[:12]
    db.transact([{
        "irs.CognitiveRecord/id": internal_record_id,
        "irs.CognitiveRecord/apiVersion": api_version,
        "irs.CognitiveRecord/originalPayload": str(raw_data),
        "irs.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "irs.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_irs_" + uuid.uuid4().hex[:8])
