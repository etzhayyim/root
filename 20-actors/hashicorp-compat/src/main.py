"""
Py Kotodama WASM entrypoint for Hashicorp Cognitive Actor.
...
@app.route("/{api_version}/hashicorp/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hashicorp_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hashicorp.CognitiveRecord/id": internal_record_id,
        "hashicorp.CognitiveRecord/apiVersion": api_version,
        "hashicorp.CognitiveRecord/originalPayload": str(raw_data),
        "hashicorp.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hashicorp.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hashicorp_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hashicorp_entity",
