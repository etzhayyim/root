"""
Py Kotodama WASM entrypoint for Rhino Cognitive Actor.
...
schema = load_schema("../schema/rhino.kotoba")
db = DatomicClient.connect()
app = Runtime("rhino-compat")
...
@app.route("/{api_version}/rhino/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rhino_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rhino.CognitiveRecord/id": internal_record_id,
        "rhino.CognitiveRecord/apiVersion": api_version,
        "rhino.CognitiveRecord/originalPayload": str(raw_data),
        "rhino.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rhino.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rhino_" + uuid.uuid4().hex[:8])
