(ns compliancequest.main
  "Kotodama WASM entrypoint for the Compliancequest clean-room actor (L4) — Clojure port.
  ...
  facts (`compliancequest.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "compliancequest")
(def tier "L4")
...
(def entity-specs
  [{:entity "DataSubject"   :plural "datasubjects"   :id-prefix "complian_dat"
    :fields [:email :jurisdiction :status]              :required [:email :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "ConsentRecord"  :plural "consentrecords"  :id-prefix "complian_con"
    :fields [:subjectId :purpose :granted :recordedAt] :required [:purpose :granted]
    :coerce {:granted :bool}          :refs {}}
   {:entity "Request"        :plural "requests"        :id-prefix "complian_req"
    :fields [:subjectId :type :status :dueAt]        :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Control"        :plural "controls"        :id-prefix "complian_con"
    :fields [:framework :name :status]          :required [:framework :name]
    :coerce {}                       :refs {}}
   {:entity "Evidence"       :plural "evidences"       :id-prefix "complian_evi"
    :fields [:controlId :name :contentRef]          :required [:name]
    :coerce {}                       :refs {:controlId "Control"}}
   {:entity "Assessment"     :plural "assessments"     :id-prefix "complian_ass"
...
(defn emit-facts
  "EAVT facts for one record: {\"compliancequest.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "compliancequest-compat" :tier tier :entities entities} 200])
...
(defn start! [] :compliancequest-compat/ready)
