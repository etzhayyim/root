"""
Py Kotodama WASM entrypoint for Csg_international Cognitive Actor.
...
schema = load_schema("../schema/csg_international.kotoba")
db = DatomicClient.connect()
app = Runtime("csg_international-compat")
...
@app.route("/{api_version}/csg_international/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_csg_international_" + uuid.uuid4().hex[:12]
    db.transact([{
        "csg_international.CognitiveRecord/id": internal_record_id,
        "csg_international.CognitiveRecord/apiVersion": api_version,
        "csg_international.CognitiveRecord/originalPayload": str(raw_data),
        "csg_international.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "csg_international.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_csg_international_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "csg_international_entity",
