(ns csg-international.main
  "Kotodama WASM entrypoint for the csg_international clean-room actor (L4) — Clojure port.
  ...
  facts (`csg_international.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "csg_international")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"csg_international.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "csg_international-compat" :tier tier :entities entities} 200])
...
(defn start! [] :csg_international-compat/ready)
