"""
Py Kotodama WASM entrypoint for Rigetti Cognitive Actor.
...
schema = load_schema("../schema/rigetti.kotoba")
db = DatomicClient.connect()
app = Runtime("rigetti-compat")
...
@app.route("/{api_version}/rigetti/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rigetti_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rigetti.CognitiveRecord/id": internal_record_id,
        "rigetti.CognitiveRecord/apiVersion": api_version,
        "rigetti.CognitiveRecord/originalPayload": str(raw_data),
        "rigetti.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rigetti.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rigetti_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rigetti_entity",
