"""
Py Kotodama WASM entrypoint for Vipps_norway Cognitive Actor.
...
schema = load_schema("../schema/vipps_norway.kotoba")
db = DatomicClient.connect()
app = Runtime("vipps_norway-compat")
...
@app.route("/{api_version}/vipps_norway/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vipps_norway_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vipps_norway.CognitiveRecord/id": internal_record_id,
        "vipps_norway.CognitiveRecord/apiVersion": api_version,
        "vipps_norway.CognitiveRecord/originalPayload": str(raw_data),
        "vipps_norway.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vipps_norway.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vipps_norway_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "vipps_norway_entity",
