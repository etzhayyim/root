"""
Py Kotodama WASM entrypoint for Iter_data Cognitive Actor.
...
@app.route("/{api_version}/iter_data/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iter_data_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iter_data.CognitiveRecord/id": internal_record_id,
        "iter_data.CognitiveRecord/apiVersion": api_version,
        "iter_data.CognitiveRecord/originalPayload": str(raw_data),
        "iter_data.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iter_data.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iter_data_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iter_data_entity",
