"""
Py Kotodama WASM entrypoint for Zipline Cognitive Actor.
...
schema = load_schema("../schema/zipline.kotoba")
db = DatomicClient.connect()
app = Runtime("zipline-compat")
...
@app.route("/{api_version}/zipline/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zipline_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zipline.CognitiveRecord/id": internal_record_id,
        "zipline.CognitiveRecord/apiVersion": api_version,
        "zipline.CognitiveRecord/originalPayload": str(raw_data),
        "zipline.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zipline.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zipline_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zipline_entity",
