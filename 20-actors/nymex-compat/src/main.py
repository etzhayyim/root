"""
Py Kotodama WASM entrypoint for Nymex Cognitive Actor.
...
schema = load_schema("../schema/nymex.kotoba")
db = DatomicClient.connect()
app = Runtime("nymex-compat")
...
@app.route("/{api_version}/nymex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nymex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nymex.CognitiveRecord/id": internal_record_id,
        "nymex.CognitiveRecord/apiVersion": api_version,
        "nymex.CognitiveRecord/originalPayload": str(raw_data),
        "nymex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nymex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nymex_" + uuid.uuid4().hex[:8])
