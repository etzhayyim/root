"""
Py Kotodama WASM entrypoint for Niantic_lightship Cognitive Actor.
...
schema = load_schema("../schema/niantic_lightship.kotoba")
db = DatomicClient.connect()
app = Runtime("niantic_lightship-compat")
...
@app.route("/{api_version}/niantic_lightship/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_niantic_lightship_" + uuid.uuid4().hex[:12]
    db.transact([{
        "niantic_lightship.CognitiveRecord/id": internal_record_id,
        "niantic_lightship.CognitiveRecord/apiVersion": api_version,
        "niantic_lightship.CognitiveRecord/originalPayload": str(raw_data),
        "niantic_lightship.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "niantic_lightship.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_niantic_lightship_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "niantic_lightship_entity",
