"""
Py Kotodama WASM entrypoint for Pulumi Cognitive Actor.
...
@app.route("/{api_version}/pulumi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pulumi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pulumi.CognitiveRecord/id": internal_record_id,
        "pulumi.CognitiveRecord/apiVersion": api_version,
        "pulumi.CognitiveRecord/originalPayload": str(raw_data),
        "pulumi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pulumi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pulumi_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pulumi_entity",
