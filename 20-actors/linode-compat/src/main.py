"""
Py Kotodama WASM entrypoint for Linode Cognitive Actor.
...
@app.route("/{api_version}/linode/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_linode_" + uuid.uuid4().hex[:12]
    db.transact([{
        "linode.CognitiveRecord/id": internal_record_id,
        "linode.CognitiveRecord/apiVersion": api_version,
        "linode.CognitiveRecord/originalPayload": str(raw_data),
        "linode.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "linode.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_linode_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "linode_entity",
