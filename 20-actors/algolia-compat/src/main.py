"""
Py Kotodama WASM entrypoint for Algolia Cognitive Actor.
...
@app.route("/{api_version}/algolia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_algolia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "algolia.CognitiveRecord/id": internal_record_id,
        "algolia.CognitiveRecord/apiVersion": api_version,
        "algolia.CognitiveRecord/originalPayload": str(raw_data),
        "algolia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "algolia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_algolia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "algolia_entity",
