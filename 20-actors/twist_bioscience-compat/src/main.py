"""
Py Kotodama WASM entrypoint for Twist_bioscience Cognitive Actor.
...
schema = load_schema("../schema/twist_bioscience.kotoba")
db = DatomicClient.connect()
app = Runtime("twist_bioscience-compat")
...
@app.route("/{api_version}/twist_bioscience/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_twist_bioscience_" + uuid.uuid4().hex[:12]
    db.transact([{
        "twist_bioscience.CognitiveRecord/id": internal_record_id,
        "twist_bioscience.CognitiveRecord/apiVersion": api_version,
        "twist_bioscience.CognitiveRecord/originalPayload": str(raw_data),
        "twist_bioscience.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "twist_bioscience.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_twist_bioscience_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "twist_bioscience_entity",
