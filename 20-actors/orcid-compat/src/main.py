"""
Py Kotodama WASM entrypoint for Orcid Cognitive Actor.
...
@app.route("/{api_version}/orcid/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_orcid_" + uuid.uuid4().hex[:12]
    db.transact([{
        "orcid.CognitiveRecord/id": internal_record_id,
        "orcid.CognitiveRecord/apiVersion": api_version,
        "orcid.CognitiveRecord/originalPayload": str(raw_data),
        "orcid.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "orcid.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_orcid_" + uuid.uuid4().hex[:8])
