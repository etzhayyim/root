"""
Py Kotodama WASM entrypoint for Tga_australia Cognitive Actor.
...
schema = load_schema("../schema/tga_australia.kotoba")
db = DatomicClient.connect()
app = Runtime("tga_australia-compat")
...
@app.route("/{api_version}/tga_australia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tga_australia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tga_australia.CognitiveRecord/id": internal_record_id,
        "tga_australia.CognitiveRecord/apiVersion": api_version,
        "tga_australia.CognitiveRecord/originalPayload": str(raw_data),
        "tga_australia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tga_australia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tga_australia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tga_australia_entity",
