"""
Py Kotodama WASM entrypoint for Optimizely Cognitive Actor.
...
@app.route("/{api_version}/optimizely/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_optimizely_" + uuid.uuid4().hex[:12]
    db.transact([{
        "optimizely.CognitiveRecord/id": internal_record_id,
        "optimizely.CognitiveRecord/apiVersion": api_version,
        "optimizely.CognitiveRecord/originalPayload": str(raw_data),
        "optimizely.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "optimizely.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_optimizely_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "optimizely_entity",
