#!/usr/bin/env python3
"""funadaiku 船大工 — ingest seed.edn into a live kotoba node via MCP.

ADR-2606013400, migration plan Phase 3. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G12). --dry-run parses + counts
...
def main() -> int:
    ...
    # provisioned (G13 gates real outward writes). Kept explicit to avoid silent no-op.
    print("   live ingest requested — implement MCP kotoba_datom_create wiring before use (G13).")
    return 0
