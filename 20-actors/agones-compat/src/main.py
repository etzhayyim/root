"""
Py Kotodama WASM entrypoint for Agones Cognitive Actor.
...
@app.route("/{api_version}/agones/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_agones_" + uuid.uuid4().hex[:12]
    db.transact([{
        "agones.CognitiveRecord/id": internal_record_id,
        "agones.CognitiveRecord/apiVersion": api_version,
        "agones.CognitiveRecord/originalPayload": str(raw_data),
        "agones.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "agones.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_agones_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "agones_entity",
