"""
Py Kotodama WASM entrypoint for Greenway Cognitive Actor.
...
schema = load_schema("../schema/greenway.kotoba")
db = DatomicClient.connect()
app = Runtime("greenway-compat")
...
@app.route("/{api_version}/greenway/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_greenway_" + uuid.uuid4().hex[:12]
    db.transact([{
        "greenway.CognitiveRecord/id": internal_record_id,
        "greenway.CognitiveRecord/apiVersion": api_version,
        "greenway.CognitiveRecord/originalPayload": str(raw_data),
        "greenway.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "greenway.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_greenway_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "greenway_entity",
