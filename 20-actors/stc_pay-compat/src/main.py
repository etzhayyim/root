"""
Py Kotodama WASM entrypoint for Stc_pay Cognitive Actor.
...
schema = load_schema("../schema/stc_pay.kotoba")
db = DatomicClient.connect()
app = Runtime("stc_pay-compat")
...
@app.route("/{api_version}/stc_pay/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_stc_pay_" + uuid.uuid4().hex[:12]
    db.transact([{
        "stc_pay.CognitiveRecord/id": internal_record_id,
        "stc_pay.CognitiveRecord/apiVersion": api_version,
        "stc_pay.CognitiveRecord/originalPayload": str(raw_data),
        "stc_pay.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "stc_pay.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_stc_pay_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "stc_pay_entity",
