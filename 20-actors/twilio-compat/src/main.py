"""
Py Kotodama WASM entrypoint for Twilio Cognitive Actor.
...
@app.route("/{api_version}/twilio/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_twilio_" + uuid.uuid4().hex[:12]
    db.transact([{
        "twilio.CognitiveRecord/id": internal_record_id,
        "twilio.CognitiveRecord/apiVersion": api_version,
        "twilio.CognitiveRecord/originalPayload": str(raw_data),
        "twilio.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "twilio.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_twilio_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "twilio_entity",
