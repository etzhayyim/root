"""
Py Kotodama WASM entrypoint for Nuix Cognitive Actor.
...
schema = load_schema("../schema/nuix.kotoba")
db = DatomicClient.connect()
app = Runtime("nuix-compat")
...
@app.route("/{api_version}/nuix/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nuix_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nuix.CognitiveRecord/id": internal_record_id,
        "nuix.CognitiveRecord/apiVersion": api_version,
        "nuix.CognitiveRecord/originalPayload": str(raw_data),
        "nuix.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nuix.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nuix_" + uuid.uuid4().hex[:8])
