(ns synopsys_eda.main-test
  "Contract + behavioral test for the synopsys_eda-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "synopsys_eda-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'synopsys_eda.main-test)]
