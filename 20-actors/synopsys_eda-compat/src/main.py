"""
Py Kotodama WASM entrypoint for the Synopsys_eda clean-room actor.

Clean-room, API-shaped CRUD over a Datomic-backed Kotoba schema.
Generated by 70-tools/deepen_actors.py (ADR 260607 deepening phase).
No proprietary code or credentials; resource shapes only.
"""
from kotodama import Runtime
from kotoba import load_schema
from datomic import DatomicClient
import uuid
import datetime

schema = load_schema("../schema/synopsys_eda.kotoba")
db = DatomicClient.connect()
app = Runtime("synopsys_eda-compat")


def now():
    return datetime.datetime.utcnow().isoformat()


def new_id(prefix):
    return f"{prefix}_" + uuid.uuid4().hex[:16]


def _persist(entity, rec):
    """Transact a record into Datomic as namespaced EAVT facts."""
    facts = {}
    for k, v in rec.items():
        facts[f"synopsys_eda.{entity}/{k}"] = v
    db.transact([facts])
    return rec


def _query(entity, eid=None):
    """Read records of an entity from Datomic (optionally by id)."""
    pattern = {"entity": f"synopsys_eda.{entity}"}
    if eid is not None:
        pattern["id"] = eid
    return db.query(pattern)


def _require(data, fields):
    missing = [f for f in fields if not data.get(f)]
    if missing:
        return {"error": {"message": "Missing required fields: " + ", ".join(missing)}}
    return None


@app.route("/v1/registers", methods=["POST"])
def create_register(request):
    """Create a Register."""
    data = request.json or request.form or {}
    err = _require(data, ['name', 'widthBits'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_reg")}
    rec["name"] = data.get('name')
    rec["widthBits"] = int(data.get('widthBits') or 0)
    rec["role"] = data.get('role')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Register", rec)
    return rec, 201

@app.route("/v1/registers", methods=["GET"])
def list_registers(request):
    """List Registers."""
    items = _query("Register")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/registers/<eid>", methods=["GET"])
def get_register(request, eid):
    """Retrieve a Register by id."""
    rows = _query("Register", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/registers/<eid>", methods=["POST", "PATCH"])
def update_register(request, eid):
    """Update a Register."""
    rows = _query("Register", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Register", rec)
    return rec, 200

@app.route("/v1/registers/<eid>", methods=["DELETE"])
def delete_register(request, eid):
    """Delete a Register."""
    rows = _query("Register", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Register", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/instructions", methods=["POST"])
def create_instruction(request):
    """Create a Instruction."""
    data = request.json or request.form or {}
    err = _require(data, ['mnemonic', 'opcode'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_ins")}
    rec["mnemonic"] = data.get('mnemonic')
    rec["opcode"] = data.get('opcode')
    rec["operands"] = data.get('operands')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Instruction", rec)
    return rec, 201

@app.route("/v1/instructions", methods=["GET"])
def list_instructions(request):
    """List Instructions."""
    items = _query("Instruction")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/instructions/<eid>", methods=["GET"])
def get_instruction(request, eid):
    """Retrieve a Instruction by id."""
    rows = _query("Instruction", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/instructions/<eid>", methods=["POST", "PATCH"])
def update_instruction(request, eid):
    """Update a Instruction."""
    rows = _query("Instruction", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Instruction", rec)
    return rec, 200

@app.route("/v1/instructions/<eid>", methods=["DELETE"])
def delete_instruction(request, eid):
    """Delete a Instruction."""
    rows = _query("Instruction", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Instruction", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/qubits", methods=["POST"])
def create_qubit(request):
    """Create a Qubit."""
    data = request.json or request.form or {}
    err = _require(data, ['index', 't1Us'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_qub")}
    rec["index"] = int(data.get('index') or 0)
    rec["t1Us"] = float(data.get('t1Us') or 0)
    rec["t2Us"] = float(data.get('t2Us') or 0)
    rec["readoutError"] = float(data.get('readoutError') or 0)
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Qubit", rec)
    return rec, 201

@app.route("/v1/qubits", methods=["GET"])
def list_qubits(request):
    """List Qubits."""
    items = _query("Qubit")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/qubits/<eid>", methods=["GET"])
def get_qubit(request, eid):
    """Retrieve a Qubit by id."""
    rows = _query("Qubit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/qubits/<eid>", methods=["POST", "PATCH"])
def update_qubit(request, eid):
    """Update a Qubit."""
    rows = _query("Qubit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Qubit", rec)
    return rec, 200

@app.route("/v1/qubits/<eid>", methods=["DELETE"])
def delete_qubit(request, eid):
    """Delete a Qubit."""
    rows = _query("Qubit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Qubit", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/circuits", methods=["POST"])
def create_circuit(request):
    """Create a Circuit."""
    data = request.json or request.form or {}
    err = _require(data, ['name', 'qubitCount'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_cir")}
    rec["name"] = data.get('name')
    rec["qubitCount"] = int(data.get('qubitCount') or 0)
    rec["depth"] = int(data.get('depth') or 0)
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Circuit", rec)
    return rec, 201

@app.route("/v1/circuits", methods=["GET"])
def list_circuits(request):
    """List Circuits."""
    items = _query("Circuit")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/circuits/<eid>", methods=["GET"])
def get_circuit(request, eid):
    """Retrieve a Circuit by id."""
    rows = _query("Circuit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/circuits/<eid>", methods=["POST", "PATCH"])
def update_circuit(request, eid):
    """Update a Circuit."""
    rows = _query("Circuit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Circuit", rec)
    return rec, 200

@app.route("/v1/circuits/<eid>", methods=["DELETE"])
def delete_circuit(request, eid):
    """Delete a Circuit."""
    rows = _query("Circuit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Circuit", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/gates", methods=["POST"])
def create_gate(request):
    """Create a Gate."""
    data = request.json or request.form or {}
    err = _require(data, ['name', 'targets'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_gat")}
    rec["circuitId"] = data.get('circuitId')
    rec["name"] = data.get('name')
    rec["targets"] = data.get('targets')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Gate", rec)
    return rec, 201

@app.route("/v1/gates", methods=["GET"])
def list_gates(request):
    """List Gates."""
    items = _query("Gate")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/gates/<eid>", methods=["GET"])
def get_gate(request, eid):
    """Retrieve a Gate by id."""
    rows = _query("Gate", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/gates/<eid>", methods=["POST", "PATCH"])
def update_gate(request, eid):
    """Update a Gate."""
    rows = _query("Gate", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Gate", rec)
    return rec, 200

@app.route("/v1/gates/<eid>", methods=["DELETE"])
def delete_gate(request, eid):
    """Delete a Gate."""
    rows = _query("Gate", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Gate", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/jobs", methods=["POST"])
def create_job(request):
    """Create a Job."""
    data = request.json or request.form or {}
    err = _require(data, ['shots', 'status'])
    if err:
        return err, 400
    rec = {"id": new_id("synopsys_job")}
    rec["circuitId"] = data.get('circuitId')
    rec["shots"] = int(data.get('shots') or 0)
    rec["status"] = data.get('status')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Job", rec)
    return rec, 201

@app.route("/v1/jobs", methods=["GET"])
def list_jobs(request):
    """List Jobs."""
    items = _query("Job")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/jobs/<eid>", methods=["GET"])
def get_job(request, eid):
    """Retrieve a Job by id."""
    rows = _query("Job", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/jobs/<eid>", methods=["POST", "PATCH"])
def update_job(request, eid):
    """Update a Job."""
    rows = _query("Job", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Job", rec)
    return rec, 200

@app.route("/v1/jobs/<eid>", methods=["DELETE"])
def delete_job(request, eid):
    """Delete a Job."""
    rows = _query("Job", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"synopsys_eda.Job", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "synopsys_eda-compat", "entities": ['Register', 'Instruction', 'Qubit', 'Circuit', 'Gate', 'Job']}, 200


if __name__ == "__main__":
    app.start()
