(ns synopsys_eda.main
  "Kotodama WASM entrypoint for the Synopsys_eda clean-room actor (L4) — Clojure port.
  ...
  facts (`synopsys_eda.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "synopsys_eda")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"synopsys_eda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "synopsys_eda-compat" :tier tier :entities entities} 200])
...
(defn start! [] :synopsys_eda-compat/ready)
