"""
Py Kotodama WASM entrypoint for Pubmatic Cognitive Actor.
...
schema = load_schema("../schema/pubmatic.kotoba")
db = DatomicClient.connect()
app = Runtime("pubmatic-compat")
...
@app.route("/{api_version}/pubmatic/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pubmatic_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pubmatic.CognitiveRecord/id": internal_record_id,
        "pubmatic.CognitiveRecord/apiVersion": api_version,
        "pubmatic.CognitiveRecord/originalPayload": str(raw_data),
        "pubmatic.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pubmatic.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pubmatic_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pubmatic_entity",
