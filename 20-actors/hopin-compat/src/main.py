"""
Py Kotodama WASM entrypoint for Hopin Cognitive Actor.
...
schema = load_schema("../schema/hopin.kotoba")
db = DatomicClient.connect()
app = Runtime("hopin-compat")
...
@app.route("/{api_version}/hopin/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hopin_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hopin.CognitiveRecord/id": internal_record_id,
        "hopin.CognitiveRecord/apiVersion": api_version,
        "hopin.CognitiveRecord/originalPayload": str(raw_data),
        "hopin.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hopin.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hopin_" + uuid.uuid4().hex[:8])
