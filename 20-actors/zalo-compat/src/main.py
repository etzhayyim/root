"""
Py Kotodama WASM entrypoint for Zalo Cognitive Actor.
...
schema = load_schema("../schema/zalo.kotoba")
db = DatomicClient.connect()
app = Runtime("zalo-compat")
...
@app.route("/{api_version}/zalo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zalo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zalo.CognitiveRecord/id": internal_record_id,
        "zalo.CognitiveRecord/apiVersion": api_version,
        "zalo.CognitiveRecord/originalPayload": str(raw_data),
        "zalo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zalo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zalo_" + uuid.uuid4().hex[:8])
