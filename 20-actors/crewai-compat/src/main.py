"""
Py Kotodama WASM entrypoint for Crewai Cognitive Actor.
...
schema = load_schema("../schema/crewai.kotoba")
db = DatomicClient.connect()
app = Runtime("crewai-compat")
...
@app.route("/{api_version}/crewai/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_crewai_" + uuid.uuid4().hex[:12]
    db.transact([{
        "crewai.CognitiveRecord/id": internal_record_id,
        "crewai.CognitiveRecord/apiVersion": api_version,
        "crewai.CognitiveRecord/originalPayload": str(raw_data),
        "crewai.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "crewai.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_crewai_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "crewai_entity",
