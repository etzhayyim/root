"""
Py Kotodama WASM entrypoint for Mixpanel Cognitive Actor.
...
schema = load_schema("../schema/mixpanel.kotoba")
db = DatomicClient.connect()
app = Runtime("mixpanel-compat")
...
@app.route("/{api_version}/mixpanel/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mixpanel_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mixpanel.CognitiveRecord/id": internal_record_id,
        "mixpanel.CognitiveRecord/apiVersion": api_version,
        "mixpanel.CognitiveRecord/originalPayload": str(raw_data),
        "mixpanel.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mixpanel.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mixpanel_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mixpanel_entity",
