"""
Py Kotodama WASM entrypoint for Thoughtmachine Cognitive Actor.
...
schema = load_schema("../schema/thoughtmachine.kotoba")
db = DatomicClient.connect()
app = Runtime("thoughtmachine-compat")
...
@app.route("/{api_version}/thoughtmachine/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_thoughtmachine_" + uuid.uuid4().hex[:12]
    db.transact([{
        "thoughtmachine.CognitiveRecord/id": internal_record_id,
        "thoughtmachine.CognitiveRecord/apiVersion": api_version,
        "thoughtmachine.CognitiveRecord/originalPayload": str(raw_data),
        "thoughtmachine.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "thoughtmachine.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_thoughtmachine_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "thoughtmachine_entity",
