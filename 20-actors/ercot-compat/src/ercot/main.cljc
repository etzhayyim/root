(ns ercot.main
  "Kotodama WASM entrypoint for the Ercot clean-room actor (L4) — Clojure port.
  ...
  facts (`ercot.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ercot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ercot-compat" :tier tier :entities entities} 200])
