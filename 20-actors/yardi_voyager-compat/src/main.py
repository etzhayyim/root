"""
Py Kotodama WASM entrypoint for Yardi_voyager Cognitive Actor.
...
schema = load_schema("../schema/yardi_voyager.kotoba")
db = DatomicClient.connect()
app = Runtime("yardi_voyager-compat")
...
@app.route("/{api_version}/yardi_voyager/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yardi_voyager_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yardi_voyager.CognitiveRecord/id": internal_record_id,
        "yardi_voyager.CognitiveRecord/apiVersion": api_version,
        "yardi_voyager.CognitiveRecord/originalPayload": str(raw_data),
        "yardi_voyager.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yardi_voyager.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yardi_voyager_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "yardi_voyager_entity",
