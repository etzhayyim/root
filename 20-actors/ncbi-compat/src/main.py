"""
Py Kotodama WASM entrypoint for Ncbi Cognitive Actor.
...
schema = load_schema("../schema/ncbi.kotoba")
db = DatomicClient.connect()
app = Runtime("ncbi-compat")
...
@app.route("/{api_version}/ncbi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ncbi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ncbi.CognitiveRecord/id": internal_record_id,
        "ncbi.CognitiveRecord/apiVersion": api_version,
        "ncbi.CognitiveRecord/originalPayload": str(raw_data),
        "ncbi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ncbi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ncbi_" + uuid.uuid4().hex[:8])
