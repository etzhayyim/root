"""
Py Kotodama WASM entrypoint for Eclinicalworks Cognitive Actor.
...
@app.route("/{api_version}/eclinicalworks/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_eclinicalworks_" + uuid.uuid4().hex[:12]
    db.transact([{
        "eclinicalworks.CognitiveRecord/id": internal_record_id,
        "eclinicalworks.CognitiveRecord/apiVersion": api_version,
        "eclinicalworks.CognitiveRecord/originalPayload": str(raw_data),
        "eclinicalworks.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "eclinicalworks.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_eclinicalworks_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "eclinicalworks_entity",
