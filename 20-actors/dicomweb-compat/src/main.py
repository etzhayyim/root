"""
Py Kotodama WASM entrypoint for Dicomweb Cognitive Actor.
...
@app.route("/{api_version}/dicomweb/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dicomweb_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dicomweb.CognitiveRecord/id": internal_record_id,
        "dicomweb.CognitiveRecord/apiVersion": api_version,
        "dicomweb.CognitiveRecord/originalPayload": str(raw_data),
        "dicomweb.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dicomweb.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dicomweb_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "dicomweb_entity",
