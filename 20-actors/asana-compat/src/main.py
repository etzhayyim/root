"""
Py Kotodama WASM entrypoint for Asana Cognitive Actor.
...
@app.route("/{api_version}/asana/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_asana_" + uuid.uuid4().hex[:12]
    db.transact([{
        "asana.CognitiveRecord/id": internal_record_id,
        "asana.CognitiveRecord/apiVersion": api_version,
        "asana.CognitiveRecord/originalPayload": str(raw_data),
        "asana.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "asana.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_asana_" + uuid.uuid4().hex[:8])
