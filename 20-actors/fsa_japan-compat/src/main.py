"""
Py Kotodama WASM entrypoint for Fsa_japan Cognitive Actor.
...
schema = load_schema("../schema/fsa_japan.kotoba")
db = DatomicClient.connect()
app = Runtime("fsa_japan-compat")
...
@app.route("/{api_version}/fsa_japan/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fsa_japan_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fsa_japan.CognitiveRecord/id": internal_record_id,
        "fsa_japan.CognitiveRecord/apiVersion": api_version,
        "fsa_japan.CognitiveRecord/originalPayload": str(raw_data),
        "fsa_japan.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fsa_japan.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fsa_japan_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fsa_japan_entity",
