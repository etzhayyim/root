"""
Py Kotodama WASM entrypoint for Optumrx Cognitive Actor.
...
schema = load_schema("../schema/optumrx.kotoba")
db = DatomicClient.connect()
app = Runtime("optumrx-compat")
...
@app.route("/{api_version}/optumrx/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_optumrx_" + uuid.uuid4().hex[:12]
    db.transact([{
        "optumrx.CognitiveRecord/id": internal_record_id,
        "optumrx.CognitiveRecord/apiVersion": api_version,
        "optumrx.CognitiveRecord/originalPayload": str(raw_data),
        "optumrx.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "optumrx.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_optumrx_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "optumrx_entity",
