"""
Py Kotodama WASM entrypoint for Automationanywhere Cognitive Actor.
...
schema = load_schema("../schema/automationanywhere.kotoba")
db = DatomicClient.connect()
app = Runtime("automationanywhere-compat")
...
@app.route("/{api_version}/automationanywhere/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_automationanywhere_" + uuid.uuid4().hex[:12]
    db.transact([{
        "automationanywhere.CognitiveRecord/id": internal_record_id,
        "automationanywhere.CognitiveRecord/apiVersion": api_version,
        "automationanywhere.CognitiveRecord/originalPayload": str(raw_data),
        "automationanywhere.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "automationanywhere.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_automationanywhere_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "automationanywhere_entity",
