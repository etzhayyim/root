"""
Py Kotodama WASM entrypoint for Planview Cognitive Actor.
...
schema = load_schema("../schema/planview.kotoba")
db = DatomicClient.connect()
app = Runtime("planview-compat")
...
@app.route("/{api_version}/planview/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_planview_" + uuid.uuid4().hex[:12]
    db.transact([{
        "planview.CognitiveRecord/id": internal_record_id,
        "planview.CognitiveRecord/apiVersion": api_version,
        "planview.CognitiveRecord/originalPayload": str(raw_data),
        "planview.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "planview.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_planview_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "planview_entity",
