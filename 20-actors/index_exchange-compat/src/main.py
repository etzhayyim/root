"""
Py Kotodama WASM entrypoint for Index_exchange Cognitive Actor.
...
schema = load_schema("../schema/index_exchange.kotoba")
db = DatomicClient.connect()
app = Runtime("index_exchange-compat")
...
@app.route("/{api_version}/index_exchange/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_index_exchange_" + uuid.uuid4().hex[:12]
    db.transact([{
        "index_exchange.CognitiveRecord/id": internal_record_id,
        "index_exchange.CognitiveRecord/apiVersion": api_version,
        "index_exchange.CognitiveRecord/originalPayload": str(raw_data),
        "index_exchange.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "index_exchange.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_index_exchange_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "index_exchange_entity",
