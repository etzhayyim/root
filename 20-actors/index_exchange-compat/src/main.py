"""
Py Kotodama WASM entrypoint for the Index_exchange clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"index_exchange.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"index_exchange.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_campaign(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Campaign", "id": eid})
...
def delete_audience(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Audience", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_profile(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Profile", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_funnel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"index_exchange.Funnel", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "index_exchange-compat", "entities": ['Campaign', 'Audience', 'Event', 'Profile', 'Message', 'Funnel']}, 200
