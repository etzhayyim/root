"""
Py Kotodama WASM entrypoint for Opera_pms Cognitive Actor.
...
schema = load_schema("../schema/opera_pms.kotoba")
db = DatomicClient.connect()
app = Runtime("opera_pms-compat")
...
@app.route("/{api_version}/opera_pms/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_opera_pms_" + uuid.uuid4().hex[:12]
    db.transact([{
        "opera_pms.CognitiveRecord/id": internal_record_id,
        "opera_pms.CognitiveRecord/apiVersion": api_version,
        "opera_pms.CognitiveRecord/originalPayload": str(raw_data),
        "opera_pms.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "opera_pms.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_opera_pms_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "opera_pms_entity",
