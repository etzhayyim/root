...
describe("Cargo Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:cargo.gftd.ai"); });
  ...
  it("xrpc covers B/L, container, manifest", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.cargo.manifest.getBl");
    expect(nsids).toContain("ai.gftd.apps.cargo.manifest.trackContainer");
    expect(nsids).toContain("ai.gftd.apps.cargo.manifest.getVoyageManifest");
  });
  ...
  it("subscribes to vessel.voyage", () => {
    expect(m.triggers.subscribeRepos.collections).toContain("ai.gftd.apps.vessel.voyage");
  });
