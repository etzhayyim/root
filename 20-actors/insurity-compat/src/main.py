"""
Py Kotodama WASM entrypoint for Insurity Cognitive Actor.
...
schema = load_schema("../schema/insurity.kotoba")
db = DatomicClient.connect()
app = Runtime("insurity-compat")
...
@app.route("/{api_version}/insurity/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_insurity_" + uuid.uuid4().hex[:12]
    db.transact([{
        "insurity.CognitiveRecord/id": internal_record_id,
        "insurity.CognitiveRecord/apiVersion": api_version,
        "insurity.CognitiveRecord/originalPayload": str(raw_data),
        "insurity.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "insurity.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_insurity_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "insurity_entity",
