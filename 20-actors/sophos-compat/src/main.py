"""
Py Kotodama WASM entrypoint for Sophos Cognitive Actor.
...
schema = load_schema("../schema/sophos.kotoba")
db = DatomicClient.connect()
app = Runtime("sophos-compat")
...
@app.route("/{api_version}/sophos/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sophos_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sophos.CognitiveRecord/id": internal_record_id,
        "sophos.CognitiveRecord/apiVersion": api_version,
        "sophos.CognitiveRecord/originalPayload": str(raw_data),
        "sophos.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sophos.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sophos_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sophos_entity",
