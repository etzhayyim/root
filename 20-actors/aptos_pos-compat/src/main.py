"""
Py Kotodama WASM entrypoint for Aptos_pos Cognitive Actor.
...
@app.route("/{api_version}/aptos_pos/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aptos_pos_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aptos_pos.CognitiveRecord/id": internal_record_id,
        "aptos_pos.CognitiveRecord/apiVersion": api_version,
        "aptos_pos.CognitiveRecord/originalPayload": str(raw_data),
        "aptos_pos.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aptos_pos.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aptos_pos_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "aptos_pos_entity",
