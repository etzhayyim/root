"""
Py Kotodama WASM entrypoint for the Jpmorgan_onyx clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"jpmorgan_onyx.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"jpmorgan_onyx.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.Customer", "id": eid})
...
def delete_payment_intent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.PaymentIntent", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.Charge", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_refund(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.Refund", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payout(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.Payout", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payment_method(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"jpmorgan_onyx.PaymentMethod", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "jpmorgan_onyx-compat", "entities": ['Customer', 'PaymentIntent', 'Charge', 'Refund', 'Payout', 'PaymentMethod']}, 200
