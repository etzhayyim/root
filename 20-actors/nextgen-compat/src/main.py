"""
Py Kotodama WASM entrypoint for Nextgen Cognitive Actor.
...
schema = load_schema("../schema/nextgen.kotoba")
db = DatomicClient.connect()
app = Runtime("nextgen-compat")
...
@app.route("/{api_version}/nextgen/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nextgen_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nextgen.CognitiveRecord/id": internal_record_id,
        "nextgen.CognitiveRecord/apiVersion": api_version,
        "nextgen.CognitiveRecord/originalPayload": str(raw_data),
        "nextgen.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nextgen.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nextgen_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nextgen_entity",
