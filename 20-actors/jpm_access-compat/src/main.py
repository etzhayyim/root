"""
Py Kotodama WASM entrypoint for Jpm_access Cognitive Actor.
...
schema = load_schema("../schema/jpm_access.kotoba")
db = DatomicClient.connect()
app = Runtime("jpm_access-compat")
...
@app.route("/{api_version}/jpm_access/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_jpm_access_" + uuid.uuid4().hex[:12]
    db.transact([{
        "jpm_access.CognitiveRecord/id": internal_record_id,
        "jpm_access.CognitiveRecord/apiVersion": api_version,
        "jpm_access.CognitiveRecord/originalPayload": str(raw_data),
        "jpm_access.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "jpm_access.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_jpm_access_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "jpm_access_entity",
