(ns jpm-access.main
  "Kotodama WASM entrypoint for the Jpm_access clean-room actor (L4) — Clojure port.
  ...
  facts (`jpm_access.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jpm_access")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "jpmacces_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "jpmacces_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"jpm_access.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jpm_access-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jpm_access-compat/ready)
