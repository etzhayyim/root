"""
Py Kotodama WASM entrypoint for Mandiant Cognitive Actor.
...
schema = load_schema("../schema/mandiant.kotoba")
db = DatomicClient.connect()
app = Runtime("mandiant-compat")
...
@app.route("/{api_version}/mandiant/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mandiant_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mandiant.CognitiveRecord/id": internal_record_id,
        "mandiant.CognitiveRecord/apiVersion": api_version,
        "mandiant.CognitiveRecord/originalPayload": str(raw_data),
        "mandiant.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mandiant.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mandiant_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mandiant_entity",
