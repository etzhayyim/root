"""
Py Kotodama WASM entrypoint for Binance Cognitive Actor.
...
@app.route("/{api_version}/binance/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_binance_" + uuid.uuid4().hex[:12]
    db.transact([{
        "binance.CognitiveRecord/id": internal_record_id,
        "binance.CognitiveRecord/apiVersion": api_version,
        "binance.CognitiveRecord/originalPayload": str(raw_data),
        "binance.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "binance.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_binance_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "binance_entity",
