(ns fix-fast-protocol.main-test
  "Contract + behavioral test for the fix-fast-protocol L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "fix_fast_protocol-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'fix-fast-protocol.main-test)]
