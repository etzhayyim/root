(ns fix-fast-protocol.main
  "Kotodama WASM entrypoint for the Fix_fast_protocol clean-room actor (L4) — Clojure port.
  ...
  facts (`fix_fast_protocol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fix_fast_protocol")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "fixfastp_tra"
    :fields [:gtin :description :brand]              :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "fixfastp_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}          :refs {:itemId "TradeItem"}}
   {:entity "Document"  :plural "documents"  :id-prefix "fixfastp_doc"
    :fields [:type :senderId :receiverId :status]   :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers" :id-prefix "fixfastp_con"
    :fields [:bicCode :type :status]                :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"     :id-prefix "fixfastp_par"
    :fields [:gln :name :role]                      :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments"  :id-prefix "fixfastp_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"fix_fast_protocol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fix_fast_protocol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fix_fast_protocol-compat/ready)
