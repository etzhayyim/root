"""Fixture-only parser for public platform ad-library snapshots.

This covers social/search/video ad-library records such as Meta/Facebook/
Instagram and X/Twitter after a source-policy review has produced a local
fixture or public bulk/API sample. It intentionally performs no live fetching.
"""
...
PARSER_VERSION = "platform-ad-library-fixture-r1.0"
SOURCE_CODE_CID = "cid:akashi:platform-ad-library-fixture-parser:r1"
...
def parse_platform_ad_library_fixture(
    ...
    *,
    ...
    """Map a reviewed local platform ad-library fixture into akashi records."""
    ...
    method_note = {
        ...
        "limits": [
            "fixture-only parser; does not fetch remote ad libraries",
        ...
        "falsePositiveNotes": [
            "same page name or advertiser display name is not proof of common control",
    ...
    source_policy = {
        ...
        "accessMode": source.get("accessMode", "manual-review-only"),
        "collectionStatus": "manual-review",
        "methodNoteCid": method_note_cid,
        "attestingDid": attesting_did,
    }
    source_policy = {k: v for k, v in source_policy.items() if v is not None}
    ...
    for source_record in records:
        ...
        advertiser = {
            ...
            "pageUrl": source_record["advertiser"].get("pageUrl"),
        ...
        creative_text = source_record.get("creativeText")
        media = source_record.get("media", {})
        creative_record = {
            "createdAt": captured_at,
            "sourceSnapshotCid": snapshot_cid,
            "advertiserIdentityCid": advertiser_cid,
            "creativeTextCid": _cid("creative-text", creative_text)
            if creative_text is not None
            else None,
            "creativeTextSha256": hashlib.sha256(creative_text.encode()).hexdigest()
            if creative_text is not None
            else None,
            "mediaCid": media.get("cid"),
            "mediaSha256": media.get("sha256"),
        ...
        creative_record = {k: v for k, v in creative_record.items() if v is not None}
        creative_cid = _cid("creative", creative_record)
        creative.append(creative_record)

        targeting_summary = source_record.get("targetingSummary")
        delivery_record = {
            "createdAt": captured_at,
            "sourceSnapshotCid": snapshot_cid,
            ...
            "targetingSummaryCid": _cid("targeting-summary", targeting_summary)
            if targeting_summary is not None
            else None,
            "sourceLimited": True,
            "attestingDid": attesting_did,
        }
        delivery_record = {k: v for k, v in delivery_record.items() if v is not None}
...
__all__ = ["PARSER_VERSION", "parse_platform_ad_library_fixture"]
