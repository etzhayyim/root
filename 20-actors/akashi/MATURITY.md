# akashi 証 — Maturity Ledger

R0 maturity is intentionally modest: actor identity and schema are now present,
but collection and cell execution remain gated.

## Checklist

| # | Item | Status | Evidence |
|---|---|---|---|
| 1 | Master ADR | ✅ | `90-docs/adr/2606022300-akashi-public-ad-disclosure-kotoba-actor-r0.md` |
| 2 | Actor README + manifest | ✅ | `20-actors/akashi/README.md`, `manifest.jsonld` |
| 3 | Root actor index | ✅ | `CLAUDE.md` row |
| 4 | 10 Lexicon skeletons | ✅ | `00-contracts/lexicons/app/etzhayyim/akashi/` |
| 5 | Method note seed | ✅ | `20-actors/akashi/methods/v1-r0-seed.json` |
| 6 | Source coverage seed | ✅ | `20-actors/akashi/registry/source-catalog.seed.json` |
| 7 | Coverage matrix | ✅ | `20-actors/akashi/COVERAGE.md` |
| 8 | Docs registry / graph registry | ✅ | regenerated by docs scripts |
| 9 | Lexicon invariant tests | ✅ | `70-tools/scripts/audit/test_akashi_invariants.py` |
| 10 | Source adapter fixtures | 未 | R1: fixture-first adapters |
| 11 | Cell scaffold | 未 | R1: import-time RuntimeError scaffold |
| 12 | Live collection | 未 | R2+ only after source policy review |

## Maturity Score

| Axis | R0 score | Rationale |
|---|---:|---|
| Actor identity | 4/5 | DID, manifest, ADR, root index present |
| Schema surface | 4/5 | 10 lexicons present; R1 still needs closure and validators |
| Source coverage | 1/5 | seed catalog only; no fixtures/adapters |
| Constitutional gates | 4/5 | gates documented, schema anchors present, and invariants test locks R0 |
| Malak boundary | 4/5 | explicit candidate-only handoff in ADR, manifest, lexicon |
| Operational readiness | 1/5 | no cells, no adapters, no live jobs |

Overall R0 maturity: **21/30**. This is a usable design scaffold, not a running
collector.

## Next R1 Work

1. Add cell scaffold dirs under `20-actors/magatama/cells/akashi_*` with
   import-time R1 gate exceptions.
2. Add fixture-only parser for one regulator/bulk source before any platform
   page/API adapter.
3. Add source-policy review workflow that can disable an adapter without code
   changes.
