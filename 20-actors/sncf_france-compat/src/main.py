"""
Py Kotodama WASM entrypoint for the Sncf_france clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"sncf_france.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"sncf_france.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_stop(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.Stop", "id": eid})
...
def delete_route(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.Route", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trip(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_vehicle(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.Vehicle", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_service_alert(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.ServiceAlert", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_fare(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sncf_france.Fare", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "sncf_france-compat", "entities": ['Stop', 'Route', 'Trip', 'Vehicle', 'ServiceAlert', 'Fare']}, 200
