"""
Py Kotodama WASM entrypoint for Ng_aegis Cognitive Actor.
...
schema = load_schema("../schema/ng_aegis.kotoba")
db = DatomicClient.connect()
app = Runtime("ng_aegis-compat")
...
@app.route("/{api_version}/ng_aegis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ng_aegis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ng_aegis.CognitiveRecord/id": internal_record_id,
        "ng_aegis.CognitiveRecord/apiVersion": api_version,
        "ng_aegis.CognitiveRecord/originalPayload": str(raw_data),
        "ng_aegis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ng_aegis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ng_aegis_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ng_aegis_entity",
