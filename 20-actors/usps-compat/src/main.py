"""
Py Kotodama WASM entrypoint for Usps Cognitive Actor.
...
schema = load_schema("../schema/usps.kotoba")
db = DatomicClient.connect()
app = Runtime("usps-compat")
...
@app.route("/{api_version}/usps/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_usps_" + uuid.uuid4().hex[:12]
    db.transact([{
        "usps.CognitiveRecord/id": internal_record_id,
        "usps.CognitiveRecord/apiVersion": api_version,
        "usps.CognitiveRecord/originalPayload": str(raw_data),
        "usps.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "usps.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_usps_" + uuid.uuid4().hex[:8])
