"""
Py Kotodama WASM entrypoint for Rolls_royce_flight Cognitive Actor.
...
schema = load_schema("../schema/rolls_royce_flight.kotoba")
db = DatomicClient.connect()
app = Runtime("rolls_royce_flight-compat")
...
@app.route("/{api_version}/rolls_royce_flight/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rolls_royce_flight_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rolls_royce_flight.CognitiveRecord/id": internal_record_id,
        "rolls_royce_flight.CognitiveRecord/apiVersion": api_version,
        "rolls_royce_flight.CognitiveRecord/originalPayload": str(raw_data),
        "rolls_royce_flight.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rolls_royce_flight.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rolls_royce_flight_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rolls_royce_flight_entity",
