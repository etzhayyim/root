"""
Py Kotodama WASM entrypoint for Skydio Cognitive Actor.
...
schema = load_schema("../schema/skydio.kotoba")
db = DatomicClient.connect()
app = Runtime("skydio-compat")
...
@app.route("/{api_version}/skydio/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_skydio_" + uuid.uuid4().hex[:12]
    db.transact([{
        "skydio.CognitiveRecord/id": internal_record_id,
        "skydio.CognitiveRecord/apiVersion": api_version,
        "skydio.CognitiveRecord/originalPayload": str(raw_data),
        "skydio.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "skydio.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_skydio_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "skydio_entity",
