"""
Py Kotodama WASM entrypoint for Noaa_goes Cognitive Actor.
...
schema = load_schema("../schema/noaa_goes.kotoba")
db = DatomicClient.connect()
app = Runtime("noaa_goes-compat")
...
@app.route("/{api_version}/noaa_goes/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_noaa_goes_" + uuid.uuid4().hex[:12]
    db.transact([{
        "noaa_goes.CognitiveRecord/id": internal_record_id,
        "noaa_goes.CognitiveRecord/apiVersion": api_version,
        "noaa_goes.CognitiveRecord/originalPayload": str(raw_data),
        "noaa_goes.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "noaa_goes.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_noaa_goes_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "noaa_goes_entity",
