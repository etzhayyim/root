"""
Py Kotodama WASM entrypoint for Dingtalk Cognitive Actor.
...
schema = load_schema("../schema/dingtalk.kotoba")
db = DatomicClient.connect()
app = Runtime("dingtalk-compat")
...
@app.route("/{api_version}/dingtalk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dingtalk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dingtalk.CognitiveRecord/id": internal_record_id,
        "dingtalk.CognitiveRecord/apiVersion": api_version,
        "dingtalk.CognitiveRecord/originalPayload": str(raw_data),
        "dingtalk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dingtalk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dingtalk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "dingtalk_entity",
