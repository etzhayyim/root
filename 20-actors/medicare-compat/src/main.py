"""
Py Kotodama WASM entrypoint for Medicare Cognitive Actor.
...
schema = load_schema("../schema/medicare.kotoba")
db = DatomicClient.connect()
app = Runtime("medicare-compat")
...
@app.route("/{api_version}/medicare/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_medicare_" + uuid.uuid4().hex[:12]
    db.transact([{
        "medicare.CognitiveRecord/id": internal_record_id,
        "medicare.CognitiveRecord/apiVersion": api_version,
        "medicare.CognitiveRecord/originalPayload": str(raw_data),
        "medicare.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "medicare.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_medicare_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "medicare_entity",
