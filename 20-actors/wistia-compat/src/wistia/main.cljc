(ns wistia.main
  "Kotodama WASM entrypoint for the Wistia clean-room actor (L4) — Clojure port.
  ...
  facts (`wistia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wistia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"wistia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wistia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wistia-compat/ready)
