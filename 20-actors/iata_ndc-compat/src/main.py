"""
Py Kotodama WASM entrypoint for Iata_ndc Cognitive Actor.
...
schema = load_schema("../schema/iata_ndc.kotoba")
db = DatomicClient.connect()
app = Runtime("iata_ndc-compat")
...
@app.route("/{api_version}/iata_ndc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iata_ndc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iata_ndc.CognitiveRecord/id": internal_record_id,
        "iata_ndc.CognitiveRecord/apiVersion": api_version,
        "iata_ndc.CognitiveRecord/originalPayload": str(raw_data),
        "iata_ndc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iata_ndc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iata_ndc_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iata_ndc_entity",
