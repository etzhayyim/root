"""
Py Kotodama WASM entrypoint for the Photon_engine clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"photon_engine.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"photon_engine.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_player(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Player", "id": eid})
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_match(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Match", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_leaderboard(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Leaderboard", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inventory(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Inventory", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_achievement(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"photon_engine.Achievement", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "photon_engine-compat", "entities": ['Player', 'Session', 'Match', 'Leaderboard', 'Inventory', 'Achievement']}, 200
