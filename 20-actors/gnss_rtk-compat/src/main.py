"""
Py Kotodama WASM entrypoint for Gnss_rtk Cognitive Actor.
...
schema = load_schema("../schema/gnss_rtk.kotoba")
db = DatomicClient.connect()
app = Runtime("gnss_rtk-compat")
...
@app.route("/{api_version}/gnss_rtk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gnss_rtk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gnss_rtk.CognitiveRecord/id": internal_record_id,
        "gnss_rtk.CognitiveRecord/apiVersion": api_version,
        "gnss_rtk.CognitiveRecord/originalPayload": str(raw_data),
        "gnss_rtk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gnss_rtk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gnss_rtk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gnss_rtk_entity",
