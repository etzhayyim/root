#!/usr/bin/env bb
;; Working Clojure (babashka) port of methods/promote.py.
;; Replaces the broken promote.cljc stub (whose CLI edge is #?(:clj) gated but -main is
;; never wired for bb, so it can never run standalone under bb).
;; DO NOT edit the .cljc file — this .clj takes precedence under bb classpath resolution.
;;
;; Run:
;;   bb --classpath 20-actors 20-actors/mitooshi/methods/promote.clj \
;;      --scorecard 20-actors/mitooshi/data/persisted/chokepoint-backtest-scorecard.kotoba.edn \
...
(ns mitooshi.methods.promote
  ...
  Babashka (.clj) port of `methods/promote.py` (ADR-2606051800).
  ...
  PROOF this .clj loaded (not the .cljc): the var `promote-clj-loaded` is defined only here."
  (:require [clojure.string :as str]
            [clojure.java.io :as io]))

;; Sentinel var — present ONLY in this .clj (not in the .cljc).
;; Tests resolve it to confirm bb loaded this file and not the .cljc.
(def promote-clj-loaded true)

;; ── calibration_gate: review_promotion (copied verbatim from promote.cljc) ────────────
;; Single source of truth: cells/calibration_gate/state_machine.py.
;; promote.cljc has a faithful 1:1 port; we copy it here so this .clj is self-contained.
...
(defn review-promotion
  ...
  asserts no point (G1). 1:1 with review_promotion in cells/calibration_gate/state_machine.py."
  [state]
...
;; ── EDN reader (subset: [] {} :kw "str" num bool nil) — copied from promote.cljc ──────
...
(defn parse-edn
  "Parse the first EDN value from a token seq."
  [toks]
  (let [its (volatile! toks)]
    (parse-coll its)))

(defn load-edn-str
  "Parse an EDN string into the analyze.py value shape (string-keyed maps; :… atoms stay strings)."
  [s]
  (parse-edn (tokens s)))

(defn load-edn
  "Read + parse an EDN file (mirrors the Python load_edn)."
  [path]
...
(defn decide-from-scorecard
  ...
  Accepts either (rows) or (rows opts-map) where opts-map may contain:
  {:signed-by \"\" :deviation-max DEFAULT-DEVIATION-MAX :review-fn review-promotion}.
  The :review-fn injection is the explicit port of the Python importlib dynamic load — pass
  a stub to redirect the gate in tests; defaults to the in-namespace `review-promotion`."
...
(defn- num->str
  "Render a number the way Python str()/f-string does for these rounded values."
  [x]
  (let [d (double x)]
    (if (== d (Math/rint d))
      (str (long d))
    ...
    (if-not (some #{"--scorecard"} argv)
      ...
      (let [scorecard     (arg-after argv "--scorecard")
        ...
        (when-let [outdir (arg-after argv "--out")]
          (let [dir (io/file outdir)]
            (.mkdirs dir)
            (spit (io/file dir "chokepoint-promotion-decision.kotoba.edn")
