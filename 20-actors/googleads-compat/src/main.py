"""
Py Kotodama WASM entrypoint for Googleads Cognitive Actor.
...
schema = load_schema("../schema/googleads.kotoba")
db = DatomicClient.connect()
app = Runtime("googleads-compat")
...
@app.route("/{api_version}/googleads/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_googleads_" + uuid.uuid4().hex[:12]
    db.transact([{
        "googleads.CognitiveRecord/id": internal_record_id,
        "googleads.CognitiveRecord/apiVersion": api_version,
        "googleads.CognitiveRecord/originalPayload": str(raw_data),
        "googleads.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "googleads.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_googleads_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "googleads_entity",
