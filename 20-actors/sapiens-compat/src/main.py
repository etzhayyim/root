"""
Py Kotodama WASM entrypoint for Sapiens Cognitive Actor.
...
schema = load_schema("../schema/sapiens.kotoba")
db = DatomicClient.connect()
app = Runtime("sapiens-compat")
...
@app.route("/{api_version}/sapiens/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sapiens_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sapiens.CognitiveRecord/id": internal_record_id,
        "sapiens.CognitiveRecord/apiVersion": api_version,
        "sapiens.CognitiveRecord/originalPayload": str(raw_data),
        "sapiens.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sapiens.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sapiens_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sapiens_entity",
