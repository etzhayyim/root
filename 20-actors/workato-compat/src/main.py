"""
Py Kotodama WASM entrypoint for Workato Cognitive Actor.
...
@app.route("/{api_version}/workato/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_workato_" + uuid.uuid4().hex[:12]
    db.transact([{
        "workato.CognitiveRecord/id": internal_record_id,
        "workato.CognitiveRecord/apiVersion": api_version,
        "workato.CognitiveRecord/originalPayload": str(raw_data),
        "workato.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "workato.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_workato_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "workato_entity",
