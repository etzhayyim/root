"""
Py Kotodama WASM entrypoint for Covermymeds Cognitive Actor.
...
schema = load_schema("../schema/covermymeds.kotoba")
db = DatomicClient.connect()
app = Runtime("covermymeds-compat")
...
@app.route("/{api_version}/covermymeds/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_covermymeds_" + uuid.uuid4().hex[:12]
    db.transact([{
        "covermymeds.CognitiveRecord/id": internal_record_id,
        "covermymeds.CognitiveRecord/apiVersion": api_version,
        "covermymeds.CognitiveRecord/originalPayload": str(raw_data),
        "covermymeds.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "covermymeds.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_covermymeds_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "covermymeds_entity",
