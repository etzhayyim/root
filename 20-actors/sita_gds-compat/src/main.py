"""
Py Kotodama WASM entrypoint for Sita_gds Cognitive Actor.
...
schema = load_schema("../schema/sita_gds.kotoba")
db = DatomicClient.connect()
app = Runtime("sita_gds-compat")
...
@app.route("/{api_version}/sita_gds/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sita_gds_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sita_gds.CognitiveRecord/id": internal_record_id,
        "sita_gds.CognitiveRecord/apiVersion": api_version,
        "sita_gds.CognitiveRecord/originalPayload": str(raw_data),
        "sita_gds.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sita_gds.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sita_gds_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sita_gds_entity",
