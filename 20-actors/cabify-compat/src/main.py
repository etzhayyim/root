"""
Py Kotodama WASM entrypoint for Cabify Cognitive Actor.
...
schema = load_schema("../schema/cabify.kotoba")
db = DatomicClient.connect()
app = Runtime("cabify-compat")
...
@app.route("/{api_version}/cabify/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cabify_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cabify.CognitiveRecord/id": internal_record_id,
        "cabify.CognitiveRecord/apiVersion": api_version,
        "cabify.CognitiveRecord/originalPayload": str(raw_data),
        "cabify.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cabify.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cabify_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cabify_entity",
