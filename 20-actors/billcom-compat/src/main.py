"""
Py Kotodama WASM entrypoint for Billcom Cognitive Actor.
...
schema = load_schema("../schema/billcom.kotoba")
db = DatomicClient.connect()
app = Runtime("billcom-compat")
...
@app.route("/{api_version}/billcom/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_billcom_" + uuid.uuid4().hex[:12]
    db.transact([{
        "billcom.CognitiveRecord/id": internal_record_id,
        "billcom.CognitiveRecord/apiVersion": api_version,
        "billcom.CognitiveRecord/originalPayload": str(raw_data),
        "billcom.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "billcom.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_billcom_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "billcom_entity",
