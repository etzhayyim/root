"""
Py Kotodama WASM entrypoint for the Billcom clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"billcom.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_g_l_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.GLAccount", "id": eid})
...
def delete_journal_entry(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.JournalEntry", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_invoice(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.Invoice", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_purchase_order(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.PurchaseOrder", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_vendor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.Vendor", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_cost_center(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"billcom.CostCenter", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "billcom-compat", "entities": ['GLAccount', 'JournalEntry', 'Invoice', 'PurchaseOrder', 'Vendor', 'CostCenter']}, 200
