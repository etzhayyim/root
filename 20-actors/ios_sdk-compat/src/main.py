"""
Py Kotodama WASM entrypoint for Ios_sdk Cognitive Actor.
...
@app.route("/{api_version}/ios_sdk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ios_sdk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ios_sdk.CognitiveRecord/id": internal_record_id,
        "ios_sdk.CognitiveRecord/apiVersion": api_version,
        "ios_sdk.CognitiveRecord/originalPayload": str(raw_data),
        "ios_sdk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ios_sdk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ios_sdk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ios_sdk_entity",
