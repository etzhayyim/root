"""
Py Kotodama WASM entrypoint for Grab Cognitive Actor.
...
@app.route("/{api_version}/grab/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_grab_" + uuid.uuid4().hex[:12]
    db.transact([{
        "grab.CognitiveRecord/id": internal_record_id,
        "grab.CognitiveRecord/apiVersion": api_version,
        "grab.CognitiveRecord/originalPayload": str(raw_data),
        "grab.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "grab.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_grab_" + uuid.uuid4().hex[:8])
