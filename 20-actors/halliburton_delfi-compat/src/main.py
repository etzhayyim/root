"""
Py Kotodama WASM entrypoint for Halliburton_delfi Cognitive Actor.
...
schema = load_schema("../schema/halliburton_delfi.kotoba")
db = DatomicClient.connect()
app = Runtime("halliburton_delfi-compat")
...
@app.route("/{api_version}/halliburton_delfi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_halliburton_delfi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "halliburton_delfi.CognitiveRecord/id": internal_record_id,
        "halliburton_delfi.CognitiveRecord/apiVersion": api_version,
        "halliburton_delfi.CognitiveRecord/originalPayload": str(raw_data),
        "halliburton_delfi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "halliburton_delfi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_halliburton_delfi_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "halliburton_delfi_entity",
