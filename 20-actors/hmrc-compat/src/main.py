"""
Py Kotodama WASM entrypoint for Hmrc Cognitive Actor.
...
@app.route("/{api_version}/hmrc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hmrc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hmrc.CognitiveRecord/id": internal_record_id,
        "hmrc.CognitiveRecord/apiVersion": api_version,
        "hmrc.CognitiveRecord/originalPayload": str(raw_data),
        "hmrc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hmrc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hmrc_" + uuid.uuid4().hex[:8])
