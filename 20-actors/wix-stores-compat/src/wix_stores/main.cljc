(ns wix_stores.main
  "Kotodama WASM entrypoint for the Wix-stores clean-room actor (L4) — Clojure port.
  ...
  facts (`wix_stores.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wix_stores")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "wixstore_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "wixstore_cus"
    :fields [:email :firstName :lastName :phone]   :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection"    :plural "collections"    :id-prefix "wixstore_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"wix_stores.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wix-stores-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wix-stores-compat/ready)
