"""
Py Kotodama WASM entrypoint for Zendesk Cognitive Actor.
...
@app.route("/{api_version}/zendesk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zendesk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zendesk.CognitiveRecord/id": internal_record_id,
        "zendesk.CognitiveRecord/apiVersion": api_version,
        "zendesk.CognitiveRecord/originalPayload": str(raw_data),
        "zendesk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zendesk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zendesk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zendesk_entity",
