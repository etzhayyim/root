"""
Py Kotodama WASM entrypoint for Adobe Cognitive Actor.
...
@app.route("/{api_version}/adobe/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_adobe_" + uuid.uuid4().hex[:12]
    db.transact([{
        "adobe.CognitiveRecord/id": internal_record_id,
        "adobe.CognitiveRecord/apiVersion": api_version,
        "adobe.CognitiveRecord/originalPayload": str(raw_data),
        "adobe.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "adobe.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_adobe_" + uuid.uuid4().hex[:8])
