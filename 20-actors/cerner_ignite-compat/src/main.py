"""
Py Kotodama WASM entrypoint for Cerner_ignite Cognitive Actor.
...
schema = load_schema("../schema/cerner_ignite.kotoba")
db = DatomicClient.connect()
app = Runtime("cerner_ignite-compat")
...
@app.route("/{api_version}/cerner_ignite/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cerner_ignite_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cerner_ignite.CognitiveRecord/id": internal_record_id,
        "cerner_ignite.CognitiveRecord/apiVersion": api_version,
        "cerner_ignite.CognitiveRecord/originalPayload": str(raw_data),
        "cerner_ignite.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cerner_ignite.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cerner_ignite_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cerner_ignite_entity",
