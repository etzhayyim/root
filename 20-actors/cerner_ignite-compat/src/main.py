"""
Py Kotodama WASM entrypoint for the Cerner_ignite clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"cerner_ignite.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"cerner_ignite.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_patient(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.Patient", "id": eid})
...
def delete_observation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.Observation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_condition(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.Condition", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_diagnostic_report(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.DiagnosticReport", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_sequence(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.Sequence", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_specimen(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cerner_ignite.Specimen", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "cerner_ignite-compat", "entities": ['Patient', 'Observation', 'Condition', 'DiagnosticReport', 'Sequence', 'Specimen']}, 200
