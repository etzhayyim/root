"""
Py Kotodama WASM entrypoint for Ironsource Cognitive Actor.
...
schema = load_schema("../schema/ironSource.kotoba")
db = DatomicClient.connect()
app = Runtime("ironSource-compat")
...
@app.route("/{api_version}/ironSource/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ironSource_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ironSource.CognitiveRecord/id": internal_record_id,
        "ironSource.CognitiveRecord/apiVersion": api_version,
        "ironSource.CognitiveRecord/originalPayload": str(raw_data),
        "ironSource.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ironSource.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ironSource_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ironSource_entity",
