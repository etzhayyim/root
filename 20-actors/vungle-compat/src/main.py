"""
Py Kotodama WASM entrypoint for the Vungle clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"vungle.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_campaign(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Campaign", "id": eid})
...
def delete_audience(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Audience", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_profile(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Profile", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_funnel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"vungle.Funnel", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "vungle-compat", "entities": ['Campaign', 'Audience', 'Event', 'Profile', 'Message', 'Funnel']}, 200
