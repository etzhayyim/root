"""
Py Kotodama WASM entrypoint for Vungle Cognitive Actor.
...
schema = load_schema("../schema/vungle.kotoba")
db = DatomicClient.connect()
app = Runtime("vungle-compat")
...
@app.route("/{api_version}/vungle/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vungle_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vungle.CognitiveRecord/id": internal_record_id,
        "vungle.CognitiveRecord/apiVersion": api_version,
        "vungle.CognitiveRecord/originalPayload": str(raw_data),
        "vungle.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vungle.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vungle_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "vungle_entity",
