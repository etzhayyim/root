(ns vungle.main
  "Kotodama WASM entrypoint for the Vungle clean-room actor (L4) — Clojure port.
  ...
  facts (`vungle.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "vungle")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"vungle.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vungle-compat" :tier tier :entities entities} 200])
...
(defn start! [] :vungle-compat/ready)
