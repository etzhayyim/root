"""
Py Kotodama WASM entrypoint for Relativity Cognitive Actor.
...
@app.route("/{api_version}/relativity/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_relativity_" + uuid.uuid4().hex[:12]
    db.transact([{
        "relativity.CognitiveRecord/id": internal_record_id,
        "relativity.CognitiveRecord/apiVersion": api_version,
        "relativity.CognitiveRecord/originalPayload": str(raw_data),
        "relativity.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "relativity.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_relativity_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "relativity_entity",
