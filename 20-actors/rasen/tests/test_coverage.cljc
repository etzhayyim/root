(ns rasen.tests.test-coverage
  ...
  Pure stdlib / network-free. Loads the bounded PUBLIC seed and asserts the report's honesty
  disclosures, that taxa span life (not just humans), and that populations are aggregate."
...
(deftest test-taxa-span-life-not-just-humans
  (let [{:keys [nodes]} (load-seed)
        taxa (set (for [n (vals nodes) :when (= ":gene" (get n ":genome/kind"))]
                    (get n ":gene/taxon")))]
    (is (contains? taxa ":homo-sapiens") "human genetics missing")
    ;; at least one non-human taxon (life is broader than humans)
    (is (>= (count (disj taxa ":homo-sapiens")) 1) (str "only human taxa present: " taxa))))

(deftest test-populations-are-aggregate
  (let [{:keys [nodes]} (load-seed)
        pops (set (for [n (vals nodes) :when (= ":population" (get n ":genome/kind"))]
