"""
Py Kotodama WASM entrypoint for Rockwell_factorytalk Cognitive Actor.
...
schema = load_schema("../schema/rockwell_factorytalk.kotoba")
db = DatomicClient.connect()
app = Runtime("rockwell_factorytalk-compat")
...
@app.route("/{api_version}/rockwell_factorytalk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rockwell_factorytalk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rockwell_factorytalk.CognitiveRecord/id": internal_record_id,
        "rockwell_factorytalk.CognitiveRecord/apiVersion": api_version,
        "rockwell_factorytalk.CognitiveRecord/originalPayload": str(raw_data),
        "rockwell_factorytalk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rockwell_factorytalk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rockwell_factorytalk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rockwell_factorytalk_entity",
