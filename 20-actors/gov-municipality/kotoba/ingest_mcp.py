#!/usr/bin/env python3
"""gov-municipality 官 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605250800, migration plan Phase 3. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G18). --dry-run parses + counts
datoms only (no writes, no LLM; G15 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.govmunicipality] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.govmunicipality")
    ...
    # provisioned (G19 gates real outward writes). Kept explicit to avoid silent no-op.
    print("   live ingest requested — implement MCP kotoba_datom_create wiring before use (G19).")
    return 0
