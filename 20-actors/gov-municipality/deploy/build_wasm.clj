#!/usr/bin/env bb
;; gov_municipality 自治体 — kotoba-clj WASM Component build (ADR-2606230001, 2026-06-23)
;;
;; Builds deploy/agent.cljc → deploy/agent.wasm using the kotoba-clj compiler.
;; Then validates + smoke-runs the output Component under wasmtime.
;;
;; Usage (from repo root):
;;   bb 20-actors/gov_municipality/deploy/build_wasm.clj
;;
;; Or via the bb.edn task:
;;   bb gov_municipality:build-wasm
;;
;; Prerequisites:
...
(defn -main [& _args]
  (let [script-dir (-> (System/getProperty "babashka.file") (fs/parent) (fs/absolutize))
    ...
    (println "==> gov_municipality kotoba-clj WASM build")
    ...
    (let [triple     (host-triple)
      ...
      (if (which? "wasmtime")
        (let [{:keys [out exit]} (sh "wasmtime" "run" "--wasm" "component-model" agent-wasm)]
          ...
          (if (and (str/includes? (or out "") "gov_municipality:")
