"""
Py Kotodama WASM entrypoint for Mailchimp Cognitive Actor.
...
@app.route("/{api_version}/mailchimp/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mailchimp_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mailchimp.CognitiveRecord/id": internal_record_id,
        "mailchimp.CognitiveRecord/apiVersion": api_version,
        "mailchimp.CognitiveRecord/originalPayload": str(raw_data),
        "mailchimp.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mailchimp.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mailchimp_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mailchimp_entity",
