"""
Py Kotodama WASM entrypoint for Unionpay_cn Cognitive Actor.
...
schema = load_schema("../schema/unionpay_cn.kotoba")
db = DatomicClient.connect()
app = Runtime("unionpay_cn-compat")
...
@app.route("/{api_version}/unionpay_cn/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_unionpay_cn_" + uuid.uuid4().hex[:12]
    db.transact([{
        "unionpay_cn.CognitiveRecord/id": internal_record_id,
        "unionpay_cn.CognitiveRecord/apiVersion": api_version,
        "unionpay_cn.CognitiveRecord/originalPayload": str(raw_data),
        "unionpay_cn.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "unionpay_cn.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_unionpay_cn_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "unionpay_cn_entity",
