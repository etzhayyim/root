"""
Py Kotodama WASM entrypoint for the Unionpay_cn clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"unionpay_cn.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"unionpay_cn.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.Customer", "id": eid})
...
def delete_payment_intent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.PaymentIntent", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.Charge", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_refund(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.Refund", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payout(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.Payout", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payment_method(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"unionpay_cn.PaymentMethod", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "unionpay_cn-compat", "entities": ['Customer', 'PaymentIntent', 'Charge', 'Refund', 'Payout', 'PaymentMethod']}, 200
