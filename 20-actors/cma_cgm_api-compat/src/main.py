"""
Py Kotodama WASM entrypoint for Cma_cgm_api Cognitive Actor.
...
schema = load_schema("../schema/cma_cgm_api.kotoba")
db = DatomicClient.connect()
app = Runtime("cma_cgm_api-compat")
...
@app.route("/{api_version}/cma_cgm_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cma_cgm_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cma_cgm_api.CognitiveRecord/id": internal_record_id,
        "cma_cgm_api.CognitiveRecord/apiVersion": api_version,
        "cma_cgm_api.CognitiveRecord/originalPayload": str(raw_data),
        "cma_cgm_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cma_cgm_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cma_cgm_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cma_cgm_api_entity",
