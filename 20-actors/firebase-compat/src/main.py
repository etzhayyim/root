"""
Py Kotodama WASM entrypoint for Firebase Cognitive Actor.
...
schema = load_schema("../schema/firebase.kotoba")
db = DatomicClient.connect()
app = Runtime("firebase-compat")
...
@app.route("/{api_version}/firebase/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_firebase_" + uuid.uuid4().hex[:12]
    db.transact([{
        "firebase.CognitiveRecord/id": internal_record_id,
        "firebase.CognitiveRecord/apiVersion": api_version,
        "firebase.CognitiveRecord/originalPayload": str(raw_data),
        "firebase.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "firebase.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_firebase_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "firebase_entity",
