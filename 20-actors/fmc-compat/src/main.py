"""
Py Kotodama WASM entrypoint for Fmc Cognitive Actor.
...
@app.route("/{api_version}/fmc/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fmc_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fmc.CognitiveRecord/id": internal_record_id,
        "fmc.CognitiveRecord/apiVersion": api_version,
        "fmc.CognitiveRecord/originalPayload": str(raw_data),
        "fmc.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fmc.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fmc_" + uuid.uuid4().hex[:8])
