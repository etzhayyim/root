"""
Py Kotodama WASM entrypoint for Cybozu Cognitive Actor.
...
schema = load_schema("../schema/cybozu.kotoba")
db = DatomicClient.connect()
app = Runtime("cybozu-compat")
...
@app.route("/{api_version}/cybozu/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cybozu_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cybozu.CognitiveRecord/id": internal_record_id,
        "cybozu.CognitiveRecord/apiVersion": api_version,
        "cybozu.CognitiveRecord/originalPayload": str(raw_data),
        "cybozu.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cybozu.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cybozu_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cybozu_entity",
