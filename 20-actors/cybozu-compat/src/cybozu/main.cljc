(ns cybozu.main
  "Kotodama WASM entrypoint for the Cybozu clean-room actor (L4) — Clojure port.
  ...
  facts (`cybozu.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cybozu")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cybozu.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cybozu-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cybozu-compat/ready)
