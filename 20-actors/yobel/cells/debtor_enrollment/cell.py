...
def anchor_enrollment(state, anchor_bridge, anchor: bool):
    ...
    if anchor_bridge is None:
        return {
            ...
            "enrollment_vertex_uri": f"at://{state['debtor_did']}/ai.gftd.apps.etzhayyim.yobel.debtorEnrollment/{enrollment_id}",
        }
    result = anchor_bridge.write_and_anchor(
        collection="ai.gftd.apps.etzhayyim.yobel.debtorEnrollment",
        rkey=enrollment_id,
        payload={
            "rite_id": state["rite_id"],
            "debtor_did": state["debtor_did"],
