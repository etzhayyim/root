...
2. `signAndAppend` — sign triple with cell's rotating witness key; append to `auditLog` MST collection (`ai.gftd.apps.etzhayyim.yobel.auditEvent`)
...
- Continuous writes to `ai.gftd.apps.etzhayyim.yobel.auditEvent` MST collection (signed)
