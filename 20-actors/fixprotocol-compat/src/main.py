"""
Py Kotodama WASM entrypoint for Fixprotocol Cognitive Actor.
...
schema = load_schema("../schema/fixprotocol.kotoba")
db = DatomicClient.connect()
app = Runtime("fixprotocol-compat")
...
@app.route("/{api_version}/fixprotocol/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fixprotocol_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fixprotocol.CognitiveRecord/id": internal_record_id,
        "fixprotocol.CognitiveRecord/apiVersion": api_version,
        "fixprotocol.CognitiveRecord/originalPayload": str(raw_data),
        "fixprotocol.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fixprotocol.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fixprotocol_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fixprotocol_entity",
