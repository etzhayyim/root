"""
Py Kotodama WASM entrypoint for the Windows_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"windows_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"windows_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_process(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.Process", "id": eid})
...
def delete_thread(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.Thread", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_file_descriptor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.FileDescriptor", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_syscall(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.Syscall", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_region(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.MemoryRegion", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_signal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"windows_api.Signal", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "windows_api-compat", "entities": ['Process', 'Thread', 'FileDescriptor', 'Syscall', 'MemoryRegion', 'Signal']}, 200
