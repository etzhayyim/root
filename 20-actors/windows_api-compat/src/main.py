"""
Py Kotodama WASM entrypoint for Windows_api Cognitive Actor.
...
schema = load_schema("../schema/windows_api.kotoba")
db = DatomicClient.connect()
app = Runtime("windows_api-compat")
...
@app.route("/{api_version}/windows_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_windows_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "windows_api.CognitiveRecord/id": internal_record_id,
        "windows_api.CognitiveRecord/apiVersion": api_version,
        "windows_api.CognitiveRecord/originalPayload": str(raw_data),
        "windows_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "windows_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_windows_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "windows_api_entity",
