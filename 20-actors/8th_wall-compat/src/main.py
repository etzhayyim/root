"""
Py Kotodama WASM entrypoint for 8th_wall Cognitive Actor.
...
schema = load_schema("../schema/8th_wall.kotoba")
db = DatomicClient.connect()
app = Runtime("8th_wall-compat")
...
@app.route("/{api_version}/8th_wall/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_8th_wall_" + uuid.uuid4().hex[:12]
    db.transact([{
        "8th_wall.CognitiveRecord/id": internal_record_id,
        "8th_wall.CognitiveRecord/apiVersion": api_version,
        "8th_wall.CognitiveRecord/originalPayload": str(raw_data),
        "8th_wall.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "8th_wall.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_8th_wall_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "8th_wall_entity",
