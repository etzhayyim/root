"""
Py Kotodama WASM entrypoint for Yandex Cognitive Actor.
...
schema = load_schema("../schema/yandex.kotoba")
db = DatomicClient.connect()
app = Runtime("yandex-compat")
...
@app.route("/{api_version}/yandex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yandex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yandex.CognitiveRecord/id": internal_record_id,
        "yandex.CognitiveRecord/apiVersion": api_version,
        "yandex.CognitiveRecord/originalPayload": str(raw_data),
        "yandex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yandex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yandex_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "yandex_entity",
