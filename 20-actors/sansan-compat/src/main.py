"""
Py Kotodama WASM entrypoint for Sansan Cognitive Actor.
...
schema = load_schema("../schema/sansan.kotoba")
db = DatomicClient.connect()
app = Runtime("sansan-compat")
...
@app.route("/{api_version}/sansan/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sansan_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sansan.CognitiveRecord/id": internal_record_id,
        "sansan.CognitiveRecord/apiVersion": api_version,
        "sansan.CognitiveRecord/originalPayload": str(raw_data),
        "sansan.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sansan.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sansan_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sansan_entity",
