"""
Py Kotodama WASM entrypoint for Galileo Cognitive Actor.
...
schema = load_schema("../schema/galileo.kotoba")
db = DatomicClient.connect()
app = Runtime("galileo-compat")
...
@app.route("/{api_version}/galileo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_galileo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "galileo.CognitiveRecord/id": internal_record_id,
        "galileo.CognitiveRecord/apiVersion": api_version,
        "galileo.CognitiveRecord/originalPayload": str(raw_data),
        "galileo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "galileo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_galileo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "galileo_entity",
