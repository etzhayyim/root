"""
Py Kotodama WASM entrypoint for Monday Cognitive Actor.
...
@app.route("/{api_version}/monday/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_monday_" + uuid.uuid4().hex[:12]
    db.transact([{
        "monday.CognitiveRecord/id": internal_record_id,
        "monday.CognitiveRecord/apiVersion": api_version,
        "monday.CognitiveRecord/originalPayload": str(raw_data),
        "monday.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "monday.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_monday_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "monday_entity",
