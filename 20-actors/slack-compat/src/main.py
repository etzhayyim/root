"""
Py Kotodama WASM entrypoint for Slack Cognitive Actor.
...
@app.route("/{api_version}/slack/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_slack_" + uuid.uuid4().hex[:12]
    db.transact([{
        "slack.CognitiveRecord/id": internal_record_id,
        "slack.CognitiveRecord/apiVersion": api_version,
        "slack.CognitiveRecord/originalPayload": str(raw_data),
        "slack.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "slack.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_slack_" + uuid.uuid4().hex[:8])
