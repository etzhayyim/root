"""
Py Kotodama WASM entrypoint for Ratp_paris Cognitive Actor.
...
schema = load_schema("../schema/ratp_paris.kotoba")
db = DatomicClient.connect()
app = Runtime("ratp_paris-compat")
...
@app.route("/{api_version}/ratp_paris/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ratp_paris_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ratp_paris.CognitiveRecord/id": internal_record_id,
        "ratp_paris.CognitiveRecord/apiVersion": api_version,
        "ratp_paris.CognitiveRecord/originalPayload": str(raw_data),
        "ratp_paris.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ratp_paris.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ratp_paris_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ratp_paris_entity",
