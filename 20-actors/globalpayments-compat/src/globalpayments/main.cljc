(ns globalpayments.main
  "Kotodama WASM entrypoint for the Globalpayments clean-room actor (L4) — Clojure port.
  ...
  facts (`globalpayments.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "globalpayments")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"globalpayments.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "globalpayments-compat" :tier tier :entities entities} 200])
...
(defn start! [] :globalpayments-compat/ready)
