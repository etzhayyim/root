"""
Py Kotodama WASM entrypoint for the Globalpayments clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"globalpayments.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"globalpayments.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.Customer", "id": eid})
...
def delete_payment_intent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.PaymentIntent", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.Charge", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_refund(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.Refund", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payout(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.Payout", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payment_method(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"globalpayments.PaymentMethod", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "globalpayments-compat", "entities": ['Customer', 'PaymentIntent', 'Charge', 'Refund', 'Payout', 'PaymentMethod']}, 200
