"""
Py Kotodama WASM entrypoint for Globalpayments Cognitive Actor.
...
schema = load_schema("../schema/globalpayments.kotoba")
db = DatomicClient.connect()
app = Runtime("globalpayments-compat")
...
@app.route("/{api_version}/globalpayments/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_globalpayments_" + uuid.uuid4().hex[:12]
    db.transact([{
        "globalpayments.CognitiveRecord/id": internal_record_id,
        "globalpayments.CognitiveRecord/apiVersion": api_version,
        "globalpayments.CognitiveRecord/originalPayload": str(raw_data),
        "globalpayments.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "globalpayments.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_globalpayments_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "globalpayments_entity",
