"""
Py Kotodama WASM entrypoint for Bizzabo Cognitive Actor.
...
schema = load_schema("../schema/bizzabo.kotoba")
db = DatomicClient.connect()
app = Runtime("bizzabo-compat")
...
@app.route("/{api_version}/bizzabo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bizzabo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bizzabo.CognitiveRecord/id": internal_record_id,
        "bizzabo.CognitiveRecord/apiVersion": api_version,
        "bizzabo.CognitiveRecord/originalPayload": str(raw_data),
        "bizzabo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bizzabo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bizzabo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bizzabo_entity",
