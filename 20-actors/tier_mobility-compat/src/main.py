"""
Py Kotodama WASM entrypoint for Tier_mobility Cognitive Actor.
...
schema = load_schema("../schema/tier_mobility.kotoba")
db = DatomicClient.connect()
app = Runtime("tier_mobility-compat")
...
@app.route("/{api_version}/tier_mobility/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tier_mobility_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tier_mobility.CognitiveRecord/id": internal_record_id,
        "tier_mobility.CognitiveRecord/apiVersion": api_version,
        "tier_mobility.CognitiveRecord/originalPayload": str(raw_data),
        "tier_mobility.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tier_mobility.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tier_mobility_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tier_mobility_entity",
