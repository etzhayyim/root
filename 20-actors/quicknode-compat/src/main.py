"""
Py Kotodama WASM entrypoint for Quicknode Cognitive Actor.
...
schema = load_schema("../schema/quicknode.kotoba")
db = DatomicClient.connect()
app = Runtime("quicknode-compat")
...
@app.route("/{api_version}/quicknode/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_quicknode_" + uuid.uuid4().hex[:12]
    db.transact([{
        "quicknode.CognitiveRecord/id": internal_record_id,
        "quicknode.CognitiveRecord/apiVersion": api_version,
        "quicknode.CognitiveRecord/originalPayload": str(raw_data),
        "quicknode.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "quicknode.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_quicknode_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "quicknode_entity",
