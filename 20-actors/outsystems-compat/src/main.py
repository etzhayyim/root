"""
Py Kotodama WASM entrypoint for Outsystems Cognitive Actor.
...
schema = load_schema("../schema/outsystems.kotoba")
db = DatomicClient.connect()
app = Runtime("outsystems-compat")
...
@app.route("/{api_version}/outsystems/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_outsystems_" + uuid.uuid4().hex[:12]
    db.transact([{
        "outsystems.CognitiveRecord/id": internal_record_id,
        "outsystems.CognitiveRecord/apiVersion": api_version,
        "outsystems.CognitiveRecord/originalPayload": str(raw_data),
        "outsystems.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "outsystems.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_outsystems_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "outsystems_entity",
