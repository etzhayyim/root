"""
Py Kotodama WASM entrypoint for Pubchem Cognitive Actor.
...
schema = load_schema("../schema/pubchem.kotoba")
db = DatomicClient.connect()
app = Runtime("pubchem-compat")
...
@app.route("/{api_version}/pubchem/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pubchem_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pubchem.CognitiveRecord/id": internal_record_id,
        "pubchem.CognitiveRecord/apiVersion": api_version,
        "pubchem.CognitiveRecord/originalPayload": str(raw_data),
        "pubchem.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pubchem.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pubchem_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pubchem_entity",
