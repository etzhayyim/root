"""
Py Kotodama WASM entrypoint for Vtex Cognitive Actor.
...
schema = load_schema("../schema/vtex.kotoba")
db = DatomicClient.connect()
app = Runtime("vtex-compat")
...
@app.route("/{api_version}/vtex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vtex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vtex.CognitiveRecord/id": internal_record_id,
        "vtex.CognitiveRecord/apiVersion": api_version,
        "vtex.CognitiveRecord/originalPayload": str(raw_data),
        "vtex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vtex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vtex_" + uuid.uuid4().hex[:8])
