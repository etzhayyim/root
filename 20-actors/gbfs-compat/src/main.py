"""
Py Kotodama WASM entrypoint for Gbfs Cognitive Actor.
...
schema = load_schema("../schema/gbfs.kotoba")
db = DatomicClient.connect()
app = Runtime("gbfs-compat")
...
@app.route("/{api_version}/gbfs/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gbfs_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gbfs.CognitiveRecord/id": internal_record_id,
        "gbfs.CognitiveRecord/apiVersion": api_version,
        "gbfs.CognitiveRecord/originalPayload": str(raw_data),
        "gbfs.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gbfs.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gbfs_" + uuid.uuid4().hex[:8])
