(ns edifact_tty.main
  "Kotodama WASM entrypoint for the Edifact_tty clean-room actor (L4) — Clojure port.
  ...
  facts (`edifact_tty.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "edifact_tty")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"edifact_tty.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "edifact_tty-compat" :tier tier :entities entities} 200])
...
(defn start! [] :edifact_tty-compat/ready)
