"""
Py Kotodama WASM entrypoint for Signal_api Cognitive Actor.
...
schema = load_schema("../schema/signal_api.kotoba")
db = DatomicClient.connect()
app = Runtime("signal_api-compat")
...
@app.route("/{api_version}/signal_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_signal_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "signal_api.CognitiveRecord/id": internal_record_id,
        "signal_api.CognitiveRecord/apiVersion": api_version,
        "signal_api.CognitiveRecord/originalPayload": str(raw_data),
        "signal_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "signal_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_signal_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "signal_api_entity",
