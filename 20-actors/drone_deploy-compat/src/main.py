"""
Py Kotodama WASM entrypoint for the Drone_deploy clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"drone_deploy.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"drone_deploy.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_drone(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Drone", "id": eid})
...
def delete_mission(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Mission", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_waypoint(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Waypoint", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_telemetry(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Telemetry", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payload(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Payload", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_swarm(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"drone_deploy.Swarm", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "drone_deploy-compat", "entities": ['Drone', 'Mission', 'Waypoint', 'Telemetry', 'Payload', 'Swarm']}, 200
