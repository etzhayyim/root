(ns drone_deploy.main
  "Kotodama WASM entrypoint for the drone_deploy clean-room actor (L4) — Clojure port.
  ...
  facts (`drone_deploy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "drone_deploy")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "dronedep_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "dronedep_tel"
   ...
   {:entity "Payload"    :plural "payloads"    :id-prefix "dronedep_pay"
    :fields [:droneId :type :weightKg]                  :required [:type :weightKg]
    :coerce {:weightKg :float}            :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "dronedep_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"drone_deploy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "drone_deploy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :drone_deploy-compat/ready)
