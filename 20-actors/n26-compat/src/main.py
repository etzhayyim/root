"""
Py Kotodama WASM entrypoint for N26 Cognitive Actor.
...
schema = load_schema("../schema/n26.kotoba")
db = DatomicClient.connect()
app = Runtime("n26-compat")
...
@app.route("/{api_version}/n26/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_n26_" + uuid.uuid4().hex[:12]
    db.transact([{
        "n26.CognitiveRecord/id": internal_record_id,
        "n26.CognitiveRecord/apiVersion": api_version,
        "n26.CognitiveRecord/originalPayload": str(raw_data),
        "n26.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "n26.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_n26_" + uuid.uuid4().hex[:8])
