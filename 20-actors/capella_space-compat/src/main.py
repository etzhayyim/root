"""
Py Kotodama WASM entrypoint for Capella_space Cognitive Actor.
...
schema = load_schema("../schema/capella_space.kotoba")
db = DatomicClient.connect()
app = Runtime("capella_space-compat")
...
@app.route("/{api_version}/capella_space/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_capella_space_" + uuid.uuid4().hex[:12]
    db.transact([{
        "capella_space.CognitiveRecord/id": internal_record_id,
        "capella_space.CognitiveRecord/apiVersion": api_version,
        "capella_space.CognitiveRecord/originalPayload": str(raw_data),
        "capella_space.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "capella_space.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_capella_space_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "capella_space_entity",
