"""
Py Kotodama WASM entrypoint for Alipay Cognitive Actor.
...
schema = load_schema("../schema/alipay.kotoba")
db = DatomicClient.connect()
app = Runtime("alipay-compat")
...
@app.route("/{api_version}/alipay/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_alipay_" + uuid.uuid4().hex[:12]
    db.transact([{
        "alipay.CognitiveRecord/id": internal_record_id,
        "alipay.CognitiveRecord/apiVersion": api_version,
        "alipay.CognitiveRecord/originalPayload": str(raw_data),
        "alipay.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "alipay.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_alipay_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "alipay_entity",
