"""
Py Kotodama WASM entrypoint for Nubank Cognitive Actor.
...
schema = load_schema("../schema/nubank.kotoba")
db = DatomicClient.connect()
app = Runtime("nubank-compat")
...
@app.route("/{api_version}/nubank/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nubank_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nubank.CognitiveRecord/id": internal_record_id,
        "nubank.CognitiveRecord/apiVersion": api_version,
        "nubank.CognitiveRecord/originalPayload": str(raw_data),
        "nubank.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nubank.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nubank_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nubank_entity",
