"""
Py Kotodama WASM entrypoint for Zim_api Cognitive Actor.
...
schema = load_schema("../schema/zim_api.kotoba")
db = DatomicClient.connect()
app = Runtime("zim_api-compat")
...
@app.route("/{api_version}/zim_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zim_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zim_api.CognitiveRecord/id": internal_record_id,
        "zim_api.CognitiveRecord/apiVersion": api_version,
        "zim_api.CognitiveRecord/originalPayload": str(raw_data),
        "zim_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zim_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zim_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zim_api_entity",
