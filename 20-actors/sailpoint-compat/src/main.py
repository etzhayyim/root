"""
Py Kotodama WASM entrypoint for Sailpoint Cognitive Actor.
...
schema = load_schema("../schema/sailpoint.kotoba")
db = DatomicClient.connect()
app = Runtime("sailpoint-compat")
...
@app.route("/{api_version}/sailpoint/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sailpoint_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sailpoint.CognitiveRecord/id": internal_record_id,
        "sailpoint.CognitiveRecord/apiVersion": api_version,
        "sailpoint.CognitiveRecord/originalPayload": str(raw_data),
        "sailpoint.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sailpoint.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sailpoint_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sailpoint_entity",
