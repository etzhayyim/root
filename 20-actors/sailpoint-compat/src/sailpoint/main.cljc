(ns sailpoint.main
  "Kotodama WASM entrypoint for the Sailpoint clean-room actor (L4) — Clojure port.
  ...
  facts (`sailpoint.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sailpoint")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sailpoint.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sailpoint-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sailpoint-compat/ready)
