"""
Py Kotodama WASM entrypoint for Celonis Cognitive Actor.
...
@app.route("/{api_version}/celonis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_celonis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "celonis.CognitiveRecord/id": internal_record_id,
        "celonis.CognitiveRecord/apiVersion": api_version,
        "celonis.CognitiveRecord/originalPayload": str(raw_data),
        "celonis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "celonis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_celonis_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "celonis_entity",
