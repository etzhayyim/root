(ns euroclear.main
  "Kotodama WASM entrypoint for the Euroclear clean-room actor (L4) — Clojure port.
  ...
  facts (`euroclear.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "euroclear")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "euroclea_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "euroclea_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"euroclear.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "euroclear-compat" :tier tier :entities entities} 200])
...
(defn start! [] :euroclear-compat/ready)
