#!/usr/bin/env python3
"""tatekata 建方 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605250715, R0 scaffold. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G16). --dry-run parses + counts
datoms only (no writes, no LLM; G13 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.tatekata] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.tatekata")
