"""
Py Kotodama WASM entrypoint for Servicenow Cognitive Actor.
...
schema = load_schema("../schema/servicenow.kotoba")
db = DatomicClient.connect()
app = Runtime("servicenow-compat")
...
@app.route("/{api_version}/servicenow/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_servicenow_" + uuid.uuid4().hex[:12]
    db.transact([{
        "servicenow.CognitiveRecord/id": internal_record_id,
        "servicenow.CognitiveRecord/apiVersion": api_version,
        "servicenow.CognitiveRecord/originalPayload": str(raw_data),
        "servicenow.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "servicenow.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_servicenow_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "servicenow_entity",
