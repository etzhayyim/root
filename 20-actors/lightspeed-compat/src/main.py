"""
Py Kotodama WASM entrypoint for Lightspeed Cognitive Actor.
...
schema = load_schema("../schema/lightspeed.kotoba")
db = DatomicClient.connect()
app = Runtime("lightspeed-compat")
...
@app.route("/{api_version}/lightspeed/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lightspeed_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lightspeed.CognitiveRecord/id": internal_record_id,
        "lightspeed.CognitiveRecord/apiVersion": api_version,
        "lightspeed.CognitiveRecord/originalPayload": str(raw_data),
        "lightspeed.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lightspeed.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lightspeed_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lightspeed_entity",
