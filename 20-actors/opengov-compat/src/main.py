"""
Py Kotodama WASM entrypoint for Opengov Cognitive Actor.
...
schema = load_schema("../schema/opengov.kotoba")
db = DatomicClient.connect()
app = Runtime("opengov-compat")
...
@app.route("/{api_version}/opengov/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_opengov_" + uuid.uuid4().hex[:12]
    db.transact([{
        "opengov.CognitiveRecord/id": internal_record_id,
        "opengov.CognitiveRecord/apiVersion": api_version,
        "opengov.CognitiveRecord/originalPayload": str(raw_data),
        "opengov.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "opengov.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_opengov_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "opengov_entity",
