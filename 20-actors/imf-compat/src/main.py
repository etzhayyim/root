"""
Py Kotodama WASM entrypoint for Imf Cognitive Actor.
...
@app.route("/{api_version}/imf/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_imf_" + uuid.uuid4().hex[:12]
    db.transact([{
        "imf.CognitiveRecord/id": internal_record_id,
        "imf.CognitiveRecord/apiVersion": api_version,
        "imf.CognitiveRecord/originalPayload": str(raw_data),
        "imf.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "imf.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_imf_" + uuid.uuid4().hex[:8])
