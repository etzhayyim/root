"""
Py Kotodama WASM entrypoint for Oneweb Cognitive Actor.
...
schema = load_schema("../schema/oneweb.kotoba")
db = DatomicClient.connect()
app = Runtime("oneweb-compat")
...
@app.route("/{api_version}/oneweb/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_oneweb_" + uuid.uuid4().hex[:12]
    db.transact([{
        "oneweb.CognitiveRecord/id": internal_record_id,
        "oneweb.CognitiveRecord/apiVersion": api_version,
        "oneweb.CognitiveRecord/originalPayload": str(raw_data),
        "oneweb.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "oneweb.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_oneweb_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "oneweb_entity",
