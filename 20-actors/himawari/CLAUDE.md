...
All 7 cells have real `.solve()` logic (R0 RuntimeError stubs removed) + standalone test files (129 tests total across 13 files, all green; cell.py ≥93% line-coverage). Cell→lexicon + cell→composed-actor wiring:
...
**Standalone tests** (no pytest/langgraph dependency; each `python3 test_*.py`; 129 total green — solve() logic + kotoba write-path (fake-host) + graph-topology (fake-StateGraph); cell.py ≥93% line-coverage):
```bash
