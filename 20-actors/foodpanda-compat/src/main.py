"""
Py Kotodama WASM entrypoint for Foodpanda Cognitive Actor.
...
schema = load_schema("../schema/foodpanda.kotoba")
db = DatomicClient.connect()
app = Runtime("foodpanda-compat")
...
@app.route("/{api_version}/foodpanda/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_foodpanda_" + uuid.uuid4().hex[:12]
    db.transact([{
        "foodpanda.CognitiveRecord/id": internal_record_id,
        "foodpanda.CognitiveRecord/apiVersion": api_version,
        "foodpanda.CognitiveRecord/originalPayload": str(raw_data),
        "foodpanda.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "foodpanda.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_foodpanda_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "foodpanda_entity",
