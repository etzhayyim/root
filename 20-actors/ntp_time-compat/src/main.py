"""
Py Kotodama WASM entrypoint for Ntp_time Cognitive Actor.
...
@app.route("/{api_version}/ntp_time/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ntp_time_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ntp_time.CognitiveRecord/id": internal_record_id,
        "ntp_time.CognitiveRecord/apiVersion": api_version,
        "ntp_time.CognitiveRecord/originalPayload": str(raw_data),
        "ntp_time.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ntp_time.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ntp_time_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ntp_time_entity",
