(ns surveymonkey.main
  "Kotodama WASM entrypoint for the Surveymonkey clean-room actor (L4) — Clojure port.
  ...
  facts (`surveymonkey.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "surveymonkey")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"surveymonkey.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "surveymonkey-compat" :tier tier :entities entities} 200])
...
(defn start! [] :surveymonkey-compat/ready)
