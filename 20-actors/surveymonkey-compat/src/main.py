"""
Py Kotodama WASM entrypoint for Surveymonkey Cognitive Actor.
...
schema = load_schema("../schema/surveymonkey.kotoba")
db = DatomicClient.connect()
app = Runtime("surveymonkey-compat")
...
@app.route("/{api_version}/surveymonkey/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_surveymonkey_" + uuid.uuid4().hex[:12]
    db.transact([{
        "surveymonkey.CognitiveRecord/id": internal_record_id,
        "surveymonkey.CognitiveRecord/apiVersion": api_version,
        "surveymonkey.CognitiveRecord/originalPayload": str(raw_data),
        "surveymonkey.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "surveymonkey.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_surveymonkey_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "surveymonkey_entity",
