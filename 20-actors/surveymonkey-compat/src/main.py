"""
Py Kotodama WASM entrypoint for the Surveymonkey clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"surveymonkey.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"surveymonkey.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_ticket(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Ticket", "id": eid})
...
def delete_survey(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Survey", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_response(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Response", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Account", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_conversation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Conversation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_contact(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"surveymonkey.Contact", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "surveymonkey-compat", "entities": ['Ticket', 'Survey', 'Response', 'Account', 'Conversation', 'Contact']}, 200
