"""
Py Kotodama WASM entrypoint for Weatherford Cognitive Actor.
...
schema = load_schema("../schema/weatherford.kotoba")
db = DatomicClient.connect()
app = Runtime("weatherford-compat")
...
@app.route("/{api_version}/weatherford/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_weatherford_" + uuid.uuid4().hex[:12]
    db.transact([{
        "weatherford.CognitiveRecord/id": internal_record_id,
        "weatherford.CognitiveRecord/apiVersion": api_version,
        "weatherford.CognitiveRecord/originalPayload": str(raw_data),
        "weatherford.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "weatherford.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_weatherford_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "weatherford_entity",
