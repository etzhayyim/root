"""
Py Kotodama WASM entrypoint for Tylertech Cognitive Actor.
...
schema = load_schema("../schema/tylertech.kotoba")
db = DatomicClient.connect()
app = Runtime("tylertech-compat")
...
@app.route("/{api_version}/tylertech/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tylertech_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tylertech.CognitiveRecord/id": internal_record_id,
        "tylertech.CognitiveRecord/apiVersion": api_version,
        "tylertech.CognitiveRecord/originalPayload": str(raw_data),
        "tylertech.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tylertech.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tylertech_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tylertech_entity",
