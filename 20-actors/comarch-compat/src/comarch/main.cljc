(ns comarch.main
  "Kotodama WASM entrypoint for the Comarch clean-room actor (L4) — Clojure port.
  ...
  facts (`comarch.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "comarch")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"comarch.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "comarch-compat" :tier tier :entities entities} 200])
...
(defn start! [] :comarch-compat/ready)
