"""
Py Kotodama WASM entrypoint for Crossref Cognitive Actor.
...
schema = load_schema("../schema/crossref.kotoba")
db = DatomicClient.connect()
app = Runtime("crossref-compat")
...
@app.route("/{api_version}/crossref/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_crossref_" + uuid.uuid4().hex[:12]
    db.transact([{
        "crossref.CognitiveRecord/id": internal_record_id,
        "crossref.CognitiveRecord/apiVersion": api_version,
        "crossref.CognitiveRecord/originalPayload": str(raw_data),
        "crossref.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "crossref.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_crossref_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "crossref_entity",
