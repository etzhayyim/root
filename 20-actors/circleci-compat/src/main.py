"""
Py Kotodama WASM entrypoint for Circleci Cognitive Actor.
...
@app.route("/{api_version}/circleci/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_circleci_" + uuid.uuid4().hex[:12]
    db.transact([{
        "circleci.CognitiveRecord/id": internal_record_id,
        "circleci.CognitiveRecord/apiVersion": api_version,
        "circleci.CognitiveRecord/originalPayload": str(raw_data),
        "circleci.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "circleci.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_circleci_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "circleci_entity",
