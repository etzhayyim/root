"""
Py Kotodama WASM entrypoint for Atlassian Cognitive Actor.
...
@app.route("/{api_version}/atlassian/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_atlassian_" + uuid.uuid4().hex[:12]
    db.transact([{
        "atlassian.CognitiveRecord/id": internal_record_id,
        "atlassian.CognitiveRecord/apiVersion": api_version,
        "atlassian.CognitiveRecord/originalPayload": str(raw_data),
        "atlassian.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "atlassian.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_atlassian_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "atlassian_entity",
