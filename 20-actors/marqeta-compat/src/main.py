"""
Py Kotodama WASM entrypoint for Marqeta Cognitive Actor.
...
@app.route("/{api_version}/marqeta/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_marqeta_" + uuid.uuid4().hex[:12]
    db.transact([{
        "marqeta.CognitiveRecord/id": internal_record_id,
        "marqeta.CognitiveRecord/apiVersion": api_version,
        "marqeta.CognitiveRecord/originalPayload": str(raw_data),
        "marqeta.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "marqeta.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_marqeta_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "marqeta_entity",
