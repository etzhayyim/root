"""
Contract test for the marqeta-compat L4 actor.
...
SCHEMA = os.path.join(ACTOR, "schema", "marqeta.kotoba")
ENTITIES = ['Card', 'CardProduct', 'User', 'Transaction', 'FundingSource']
PLURALS = {'Card': 'cards', 'CardProduct': 'cardproducts', 'User': 'users', 'Transaction': 'transactions', 'FundingSource': 'fundingsources'}


class MarqetaContract(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_verified_enums_enforced(self):
        ...
        for field in ['fulfillmentStatus', 'state', 'status']:
