(ns enverus.main
  "Kotodama WASM entrypoint for the Enverus clean-room actor (L4) — Clojure port.
  ...
  facts (`enverus.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "enverus")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"enverus.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "enverus-compat" :tier tier :entities entities} 200])
...
(defn start! [] :enverus-compat/ready)
