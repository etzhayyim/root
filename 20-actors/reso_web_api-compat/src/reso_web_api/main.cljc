(ns reso_web_api.main
  "Kotodama WASM entrypoint for the Reso_web_api clean-room actor (L4) — Clojure port.
  ...
  facts (`reso_web_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "reso_web_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "resoweba_lis"
   ...
   {:entity "Tenant"      :plural "tenants"      :id-prefix "resoweba_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"reso_web_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "reso_web_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :reso_web_api-compat/ready)
