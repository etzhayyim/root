"""
Py Kotodama WASM entrypoint for Reso_web_api Cognitive Actor.
...
@app.route("/{api_version}/reso_web_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_reso_web_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "reso_web_api.CognitiveRecord/id": internal_record_id,
        "reso_web_api.CognitiveRecord/apiVersion": api_version,
        "reso_web_api.CognitiveRecord/originalPayload": str(raw_data),
        "reso_web_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "reso_web_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_reso_web_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "reso_web_api_entity",
