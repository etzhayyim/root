"""
Py Kotodama WASM entrypoint for Gojek Cognitive Actor.
...
schema = load_schema("../schema/gojek.kotoba")
db = DatomicClient.connect()
app = Runtime("gojek-compat")
...
@app.route("/{api_version}/gojek/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gojek_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gojek.CognitiveRecord/id": internal_record_id,
        "gojek.CognitiveRecord/apiVersion": api_version,
        "gojek.CognitiveRecord/originalPayload": str(raw_data),
        "gojek.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gojek.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gojek_" + uuid.uuid4().hex[:8])
