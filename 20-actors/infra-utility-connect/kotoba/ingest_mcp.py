#!/usr/bin/env python3
"""infra-utility-connect — ingest seed.edn into a live kotoba node via MCP.

ADR-2605250900, scaffold Phase 1. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G9). --dry-run parses + counts
datoms only (no writes, no LLM; G6 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.infrautilityconnect] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.infrautilityconnect")
    ...
    # provisioned (G10 gates real outward writes). Kept explicit to avoid silent no-op.
    print("   KOTOBA_TOKEN set; operator session ready. TODO: wire MCP calls.")
    return 0
