#!/usr/bin/env bb
;; Working Clojure port of py/agent.py (infra-utility-connect activation actor).
(ns infra-utility-connect.py.agent
  ...
  Run:  bb --classpath 20-actors 20-actors/infra-utility-connect/py/agent.clj"
...
;; ── main (smoke demo) ─────────────────────────────────────────────────────────
(defn main [& _]
  (println "service request (MEP OK):"
           (:provider (first (:service_requests
                              (handle_service_request
                               {"water" true "gas" true "electric" true "telecom" true}
                               "35.6595,139.7004")))))
  (println "provider approval (sig validated):"
           (:status (first (:approvals (handle_provider_approval ["req.water.001"])))))
  (println "meter install (calibrated):"
           (:status (first (:meters (handle_meter_install ["appr.water.001"])))))
  (println "activation (all live):"
           (:result (handle_activation_test [{:serial "WM-1"}])))
  (println "settlement (10% tithe):"
           (:titheMinor (build_settlement_intent 100000000))))
