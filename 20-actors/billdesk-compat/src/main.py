"""
Py Kotodama WASM entrypoint for Billdesk Cognitive Actor.
...
schema = load_schema("../schema/billdesk.kotoba")
db = DatomicClient.connect()
app = Runtime("billdesk-compat")
...
@app.route("/{api_version}/billdesk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_billdesk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "billdesk.CognitiveRecord/id": internal_record_id,
        "billdesk.CognitiveRecord/apiVersion": api_version,
        "billdesk.CognitiveRecord/originalPayload": str(raw_data),
        "billdesk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "billdesk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_billdesk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "billdesk_entity",
