"""
Py Kotodama WASM entrypoint for Scaleai Cognitive Actor.
...
schema = load_schema("../schema/scaleai.kotoba")
db = DatomicClient.connect()
app = Runtime("scaleai-compat")
...
@app.route("/{api_version}/scaleai/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_scaleai_" + uuid.uuid4().hex[:12]
    db.transact([{
        "scaleai.CognitiveRecord/id": internal_record_id,
        "scaleai.CognitiveRecord/apiVersion": api_version,
        "scaleai.CognitiveRecord/originalPayload": str(raw_data),
        "scaleai.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "scaleai.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_scaleai_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "scaleai_entity",
