"""
Py Kotodama WASM entrypoint for Rtgs_global Cognitive Actor.
...
schema = load_schema("../schema/rtgs_global.kotoba")
db = DatomicClient.connect()
app = Runtime("rtgs_global-compat")
...
@app.route("/{api_version}/rtgs_global/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rtgs_global_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rtgs_global.CognitiveRecord/id": internal_record_id,
        "rtgs_global.CognitiveRecord/apiVersion": api_version,
        "rtgs_global.CognitiveRecord/originalPayload": str(raw_data),
        "rtgs_global.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rtgs_global.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rtgs_global_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rtgs_global_entity",
