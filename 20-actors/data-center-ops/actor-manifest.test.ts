...
describe("Data Center Ops Actor Manifest", () => {
  ...
  it("xrpc coverage endpoints are present", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.infrastructure.getFacility");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.infrastructure.listFacilities");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.infrastructure.listRacks");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.infrastructure.getPowerZones");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.infrastructure.getSlaSummary");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.dependency.seedBaseline");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.dependency.collectGlobal");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.dependency.listNodes");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.dependency.listEdges");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.dependency.getReverseTopo");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.health");
    expect(nsids).toContain("ai.gftd.apps.dataCenterOps.coverage.get");
  });

  it("subscribeRepos pipeline tracks incident collection", () => {
    const sub = m.pipelines.find((p: any) => p.trigger.type === "subscribeRepos");
    expect(sub.trigger.collections).toContain("ai.gftd.apps.dataCenterOps.incident");
  });
  ...
  it("reverse topo query exists and reads the RW materialized view", () => {
    const p = m.pipelines.find((x: any) => x.trigger.nsid === "ai.gftd.apps.dataCenterOps.dependency.getReverseTopo");
    expect(p).toBeTruthy();
  ...
  it("global collection query exists and reads the global RW materialized view", () => {
    const p = m.pipelines.find((x: any) => x.trigger.nsid === "ai.gftd.apps.dataCenterOps.dependency.collectGlobal");
    expect(p).toBeTruthy();
