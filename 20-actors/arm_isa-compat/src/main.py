"""
Py Kotodama WASM entrypoint for Arm_isa Cognitive Actor.
...
@app.route("/{api_version}/arm_isa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_arm_isa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "arm_isa.CognitiveRecord/id": internal_record_id,
        "arm_isa.CognitiveRecord/apiVersion": api_version,
        "arm_isa.CognitiveRecord/originalPayload": str(raw_data),
        "arm_isa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "arm_isa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_arm_isa_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "arm_isa_entity",
