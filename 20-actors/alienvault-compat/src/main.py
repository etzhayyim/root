"""
Py Kotodama WASM entrypoint for Alienvault Cognitive Actor.
...
schema = load_schema("../schema/alienvault.kotoba")
db = DatomicClient.connect()
app = Runtime("alienvault-compat")
...
@app.route("/{api_version}/alienvault/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_alienvault_" + uuid.uuid4().hex[:12]
    db.transact([{
        "alienvault.CognitiveRecord/id": internal_record_id,
        "alienvault.CognitiveRecord/apiVersion": api_version,
        "alienvault.CognitiveRecord/originalPayload": str(raw_data),
        "alienvault.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "alienvault.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_alienvault_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "alienvault_entity",
