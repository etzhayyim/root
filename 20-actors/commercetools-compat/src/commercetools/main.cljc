(ns commercetools.main
  "Kotodama WASM entrypoint for the Commercetools clean-room actor (L4) — Clojure port.
  ...
  facts (`commercetools.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "commercetools")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"       :plural "orders"       :id-prefix "commerce_ord"
    :fields [:version :orderNumber :customerId :customerEmail :orderState :paymentState :shipmentState :createdAt :lastModifiedAt]
    :required [:version]
    :coerce {:version :int}         :refs {:customerId "Customer"}}
   {:entity "Cart"        :plural "carts"        :id-prefix "commerce_car"
    :fields [:version :customerId :cartState :createdAt :lastModifiedAt]
    :required [:version]
    :coerce {:version :int}         :refs {:customerId "Customer"}}
   {:entity "Payment"     :plural "payments"     :id-prefix "commerce_pay"
    :fields [:version :key :customerId :createdAt :lastModifiedAt]
    :required [:version]
    :coerce {:version :int}         :refs {:customerId "Customer"}}
   {:entity "Product"     :plural "products"     :id-prefix "commerce_pro"
    :fields [:version :key :priceMode :createdAt :lastModifiedAt]
    :required [:version]
    :coerce {:version :int}         :refs {}}
   {:entity "Customer"    :plural "customers"    :id-prefix "commerce_cus"
    :fields [:version :key :email :firstName :lastName :isEmailVerified :createdAt]
    :required [:version]
    :coerce {:version :int :isEmailVerified :bool} :refs {}}
   {:entity "Transaction" :plural "transactions" :id-prefix "commerce_tra"
    :fields [:timestamp :type :state :interactionId]
    :required [:timestamp]
    :coerce {}                     :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"commercetools.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "commercetools-compat" :tier tier :entities entities} 200])
...
(defn start! [] :commercetools-compat/ready)
