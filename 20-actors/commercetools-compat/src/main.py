"""
Py Kotodama WASM entrypoint for Commercetools Cognitive Actor.
...
@app.route("/{api_version}/commercetools/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_commercetools_" + uuid.uuid4().hex[:12]
    db.transact([{
        "commercetools.CognitiveRecord/id": internal_record_id,
        "commercetools.CognitiveRecord/apiVersion": api_version,
        "commercetools.CognitiveRecord/originalPayload": str(raw_data),
        "commercetools.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "commercetools.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_commercetools_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "commercetools_entity",
