(ns tsumugi.methods.test-autorun
  ...
  REAL Python beat over the committed seed (seed-nodes 626, seed-edges 635, dataset-triples
  5163, frontier-tips 523, harvested 99, country-coverage 20.5128, forage 'GROW …'); the cid
  ...
  (let [log (tmp) out (tmp-dir)]
    (try
      (let [b (autorun/beat log :out-dir out)
        ...
        (is (= 626 (get d ":tsumugi.cycle/seed-nodes")))
        (is (= 635 (get d ":tsumugi.cycle/seed-edges")))
        (is (= 5163 (get d ":tsumugi.cycle/dataset-triples")))
        (is (= 523 (get d ":tsumugi.cycle/frontier-tips")))
        (is (= 99 (get d ":tsumugi.cycle/harvested")))
        (is (= "GROW → next ring anchors on 523 frontier tips" (get d ":tsumugi.cycle/forage")))
