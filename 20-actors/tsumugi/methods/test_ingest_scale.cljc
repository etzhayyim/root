...
(deftest ingest-offline-lifts-two-new-from-fixture
  (let [[nodes ties new-nodes new-ties dropped]
    ...
    (is (= 626 (count nodes)))
    (is (= 635 (count ties)))
    (is (= 2 (count new-nodes)))         ; most fixture orgs already resolve into the seed
    (is (= 2 (count new-ties)))
    (is (= 0 (count dropped)))
    (is (= "org.ext.youtube" (get (first new-nodes) ":pwr/id")))
    (is (= "tie.ext.alphabet-inc.youtube" (get (first new-ties) ":tie/id")))
