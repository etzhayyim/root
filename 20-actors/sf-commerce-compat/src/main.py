"""
Py Kotodama WASM entrypoint for the Sf-commerce clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"sf_commerce.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"sf_commerce.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_product(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.Product", "id": eid})
...
def delete_variant(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.Variant", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_order(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.Order", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_line_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.LineItem", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.Customer", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_collection(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"sf_commerce.Collection", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "sf-commerce-compat", "entities": ['Product', 'Variant', 'Order', 'LineItem', 'Customer', 'Collection']}, 200
