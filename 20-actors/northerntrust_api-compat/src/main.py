"""
Py Kotodama WASM entrypoint for Northerntrust_api Cognitive Actor.
...
schema = load_schema("../schema/northerntrust_api.kotoba")
db = DatomicClient.connect()
app = Runtime("northerntrust_api-compat")
...
@app.route("/{api_version}/northerntrust_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_northerntrust_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "northerntrust_api.CognitiveRecord/id": internal_record_id,
        "northerntrust_api.CognitiveRecord/apiVersion": api_version,
        "northerntrust_api.CognitiveRecord/originalPayload": str(raw_data),
        "northerntrust_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "northerntrust_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_northerntrust_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "northerntrust_api_entity",
