...
#?(:clj
   (defn head-cid
     ...
     Extracts the top-level `:tx/cid \"b<hex>\"` from the LAST line by string scan — it is
     serialized (tx-to-edn) before the huge `:tx/datoms`, so this never re-parses the graph
     each tx embeds. Calling read-log (parses EVERY tx) once per heartbeat made the autonomous
     loop O(n²) in log size; over a grown corpus that degrades the resident daemon (same shape
     as kanjo, fixed in ADR-2606160842). Result == (get (last (read-log)) \":tx/cid\")."
     [log-path]
     (let [f (clojure.java.io/file (str log-path))]
       (if-not (.exists f)
         ""
