"""
Py Kotodama WASM entrypoint for Meituan Cognitive Actor.
...
schema = load_schema("../schema/meituan.kotoba")
db = DatomicClient.connect()
app = Runtime("meituan-compat")
...
@app.route("/{api_version}/meituan/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_meituan_" + uuid.uuid4().hex[:12]
    db.transact([{
        "meituan.CognitiveRecord/id": internal_record_id,
        "meituan.CognitiveRecord/apiVersion": api_version,
        "meituan.CognitiveRecord/originalPayload": str(raw_data),
        "meituan.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "meituan.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_meituan_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "meituan_entity",
