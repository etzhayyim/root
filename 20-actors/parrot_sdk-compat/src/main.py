"""
Py Kotodama WASM entrypoint for Parrot_sdk Cognitive Actor.
...
schema = load_schema("../schema/parrot_sdk.kotoba")
db = DatomicClient.connect()
app = Runtime("parrot_sdk-compat")
...
@app.route("/{api_version}/parrot_sdk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_parrot_sdk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "parrot_sdk.CognitiveRecord/id": internal_record_id,
        "parrot_sdk.CognitiveRecord/apiVersion": api_version,
        "parrot_sdk.CognitiveRecord/originalPayload": str(raw_data),
        "parrot_sdk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "parrot_sdk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_parrot_sdk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "parrot_sdk_entity",
