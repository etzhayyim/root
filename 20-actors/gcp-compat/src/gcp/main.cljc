(ns gcp.main
  "Kotodama WASM entrypoint for the GCP clean-room actor (L4) — Clojure port.
  ...
  facts (`gcp.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"gcp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gcp-compat" :tier tier :entities entities} 200])
