"""
Py Kotodama WASM entrypoint for Gcp Cognitive Actor.
...
@app.route("/{api_version}/gcp/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gcp_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gcp.CognitiveRecord/id": internal_record_id,
        "gcp.CognitiveRecord/apiVersion": api_version,
        "gcp.CognitiveRecord/originalPayload": str(raw_data),
        "gcp.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gcp.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gcp_" + uuid.uuid4().hex[:8])
