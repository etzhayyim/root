...
set -euo pipefail
cd "$(dirname "$0")/../.."
exec bb -e '(require (quote clojure.test) (quote tokigusuri.methods.test-datom-emit) (quote tokigusuri.tests.test-analyze) (quote tokigusuri.tests.test-coverage))(let [r (apply clojure.test/run-tests (quote [tokigusuri.methods.test-datom-emit tokigusuri.tests.test-analyze tokigusuri.tests.test-coverage]))](System/exit (if (zero? (+ (:fail r) (:error r))) 0 1)))'
