(ns corelogic.main
  "Kotodama WASM entrypoint for the Corelogic clean-room actor (L4) — Clojure port.
  ...
  facts (`corelogic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "corelogic")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"corelogic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "corelogic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :corelogic-compat/ready)
