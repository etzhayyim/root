"""
Py Kotodama WASM entrypoint for Jenkins Cognitive Actor.
...
@app.route("/{api_version}/jenkins/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_jenkins_" + uuid.uuid4().hex[:12]
    db.transact([{
        "jenkins.CognitiveRecord/id": internal_record_id,
        "jenkins.CognitiveRecord/apiVersion": api_version,
        "jenkins.CognitiveRecord/originalPayload": str(raw_data),
        "jenkins.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "jenkins.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_jenkins_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "jenkins_entity",
