"""
Py Kotodama WASM entrypoint for Deel Cognitive Actor.
...
@app.route("/{api_version}/deel/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_deel_" + uuid.uuid4().hex[:12]
    db.transact([{
        "deel.CognitiveRecord/id": internal_record_id,
        "deel.CognitiveRecord/apiVersion": api_version,
        "deel.CognitiveRecord/originalPayload": str(raw_data),
        "deel.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "deel.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_deel_" + uuid.uuid4().hex[:8])
