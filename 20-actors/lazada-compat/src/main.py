"""
Py Kotodama WASM entrypoint for Lazada Cognitive Actor.
...
schema = load_schema("../schema/lazada.kotoba")
db = DatomicClient.connect()
app = Runtime("lazada-compat")
...
@app.route("/{api_version}/lazada/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lazada_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lazada.CognitiveRecord/id": internal_record_id,
        "lazada.CognitiveRecord/apiVersion": api_version,
        "lazada.CognitiveRecord/originalPayload": str(raw_data),
        "lazada.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lazada.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lazada_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lazada_entity",
