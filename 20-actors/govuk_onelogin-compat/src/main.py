"""
Py Kotodama WASM entrypoint for the Govuk_onelogin clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"govuk_onelogin.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"govuk_onelogin.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_filing(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Filing", "id": eid})
...
def delete_citizen(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Citizen", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_service(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Service", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_application(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Application", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_dataset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Dataset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_identity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"govuk_onelogin.Identity", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "govuk_onelogin-compat", "entities": ['Filing', 'Citizen', 'Service', 'Application', 'Dataset', 'Identity']}, 200
