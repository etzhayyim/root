"""
Py Kotodama WASM entrypoint for Govuk_onelogin Cognitive Actor.
...
schema = load_schema("../schema/govuk_onelogin.kotoba")
db = DatomicClient.connect()
app = Runtime("govuk_onelogin-compat")
...
@app.route("/{api_version}/govuk_onelogin/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_govuk_onelogin_" + uuid.uuid4().hex[:12]
    db.transact([{
        "govuk_onelogin.CognitiveRecord/id": internal_record_id,
        "govuk_onelogin.CognitiveRecord/apiVersion": api_version,
        "govuk_onelogin.CognitiveRecord/originalPayload": str(raw_data),
        "govuk_onelogin.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "govuk_onelogin.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_govuk_onelogin_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "govuk_onelogin_entity",
