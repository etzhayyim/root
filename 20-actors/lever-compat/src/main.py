"""
Py Kotodama WASM entrypoint for Lever Cognitive Actor.
...
schema = load_schema("../schema/lever.kotoba")
db = DatomicClient.connect()
app = Runtime("lever-compat")
...
@app.route("/{api_version}/lever/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lever_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lever.CognitiveRecord/id": internal_record_id,
        "lever.CognitiveRecord/apiVersion": api_version,
        "lever.CognitiveRecord/originalPayload": str(raw_data),
        "lever.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lever.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lever_" + uuid.uuid4().hex[:8])
