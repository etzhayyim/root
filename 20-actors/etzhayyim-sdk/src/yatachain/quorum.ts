...
export function quorumState(
  ...
  if (quorumThreshold > quorumSize) {
    throw new Error(
      `yatachain: quorumThreshold ${quorumThreshold} exceeds quorumSize ${quorumSize}`,
    );
  }
  if (witnesses.length !== quorumSize) {
    throw new Error(
      `yatachain: witness set has ${witnesses.length} cells, expected quorumSize ${quorumSize}`,
    );
  }
