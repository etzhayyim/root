/**
 * yatachain production Ed25519 signer + verifier.
 *
 * Pairs with the Python implementation in
 * ``20-actors/magatama/py/src/pymagatama/yatachain.py`` (``make_ed25519_signer`` +
 ...
 */
export function makeEd25519CellSigner(privateKey: Uint8Array): CellSigner {
  if (privateKey.byteLength !== 32) {
    throw new Error(
      `Ed25519 private key must be 32 bytes, got ${privateKey.byteLength}`,
    );
  }
...
export function ed25519PublicKeyBytes(privateKey: Uint8Array): Uint8Array {
  if (privateKey.byteLength !== 32) {
    throw new Error(
      `Ed25519 private key must be 32 bytes, got ${privateKey.byteLength}`,
    );
