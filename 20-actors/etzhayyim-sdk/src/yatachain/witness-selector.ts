 ...
 */
export function fleetCell(node: string, cellId: string): FleetCell {
...
export async function selectWitnesses(
  ...
  if (fleet.length < quorumSize) {
    throw new Error(
      `yatachain: fleet has ${fleet.length} cells, cannot select quorum of ${quorumSize}`,
    );
  }
  if (quorumSize < 1) {
    throw new Error(`yatachain: quorumSize must be ≥1, got ${quorumSize}`);
  }
  ...
  for (let i = 0; selected.length < quorumSize; i++) {
    ...
    if (i >= sorted.length * 2) {
      throw new Error(
        `yatachain: witness selection loop did not converge — fleet may have duplicate keys`,
      );
    }
  }
  return selected;
...
export function flattenFleet(
  nodes: ReadonlyArray<{ name?: string; hostname?: string; cells: readonly string[] }>,
): FleetCell[] {
  const out: FleetCell[] = [];
  for (const node of nodes) {
    const nodeKey = node.hostname ?? node.name;
    if (!nodeKey) {
      throw new Error(`yatachain: fleet node missing both 'name' and 'hostname'`);
    }
