"""
Py Kotodama WASM entrypoint for Crowdstrike Cognitive Actor.
...
@app.route("/{api_version}/crowdstrike/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_crowdstrike_" + uuid.uuid4().hex[:12]
    db.transact([{
        "crowdstrike.CognitiveRecord/id": internal_record_id,
        "crowdstrike.CognitiveRecord/apiVersion": api_version,
        "crowdstrike.CognitiveRecord/originalPayload": str(raw_data),
        "crowdstrike.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "crowdstrike.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_crowdstrike_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "crowdstrike_entity",
