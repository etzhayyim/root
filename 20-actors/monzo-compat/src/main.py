"""
Py Kotodama WASM entrypoint for Monzo Cognitive Actor.
...
@app.route("/{api_version}/monzo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_monzo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "monzo.CognitiveRecord/id": internal_record_id,
        "monzo.CognitiveRecord/apiVersion": api_version,
        "monzo.CognitiveRecord/originalPayload": str(raw_data),
        "monzo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "monzo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_monzo_" + uuid.uuid4().hex[:8])
