(ns tokopedia.main
  "Kotodama WASM entrypoint for the Tokopedia clean-room actor (L4) — Clojure port.
  ...
  facts (`tokopedia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tokopedia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tokopedia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tokopedia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tokopedia-compat/ready)
