"""
Py Kotodama WASM entrypoint for Tokopedia Cognitive Actor.
...
schema = load_schema("../schema/tokopedia.kotoba")
db = DatomicClient.connect()
app = Runtime("tokopedia-compat")
...
@app.route("/{api_version}/tokopedia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tokopedia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tokopedia.CognitiveRecord/id": internal_record_id,
        "tokopedia.CognitiveRecord/apiVersion": api_version,
        "tokopedia.CognitiveRecord/originalPayload": str(raw_data),
        "tokopedia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tokopedia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tokopedia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tokopedia_entity",
