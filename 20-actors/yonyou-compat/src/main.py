"""
Py Kotodama WASM entrypoint for Yonyou Cognitive Actor.
...
schema = load_schema("../schema/yonyou.kotoba")
db = DatomicClient.connect()
app = Runtime("yonyou-compat")
...
@app.route("/{api_version}/yonyou/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yonyou_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yonyou.CognitiveRecord/id": internal_record_id,
        "yonyou.CognitiveRecord/apiVersion": api_version,
        "yonyou.CognitiveRecord/originalPayload": str(raw_data),
        "yonyou.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yonyou.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yonyou_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "yonyou_entity",
