"""
Py Kotodama WASM entrypoint for Moralis Cognitive Actor.
...
@app.route("/{api_version}/moralis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_moralis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "moralis.CognitiveRecord/id": internal_record_id,
        "moralis.CognitiveRecord/apiVersion": api_version,
        "moralis.CognitiveRecord/originalPayload": str(raw_data),
        "moralis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "moralis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_moralis_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "moralis_entity",
