"""
Py Kotodama WASM entrypoint for Coupang Cognitive Actor.
...
schema = load_schema("../schema/coupang.kotoba")
db = DatomicClient.connect()
app = Runtime("coupang-compat")
...
@app.route("/{api_version}/coupang/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_coupang_" + uuid.uuid4().hex[:12]
    db.transact([{
        "coupang.CognitiveRecord/id": internal_record_id,
        "coupang.CognitiveRecord/apiVersion": api_version,
        "coupang.CognitiveRecord/originalPayload": str(raw_data),
        "coupang.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "coupang.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_coupang_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "coupang_entity",
