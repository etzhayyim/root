(ns coupang.main
  "Kotodama WASM entrypoint for the Coupang clean-room actor (L4) — Clojure port.
  ...
  facts (`coupang.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "coupang")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "coupang_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "coupang_cus"
    :fields [:email :firstName :lastName :phone]            :required [:email :firstName]
    :coerce {}                                   :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "coupang_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"coupang.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "coupang-compat" :tier tier :entities entities} 200])
...
(defn start! [] :coupang-compat/ready)
