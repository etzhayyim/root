"""
Py Kotodama WASM entrypoint for Deutschebahn Cognitive Actor.
...
schema = load_schema("../schema/deutschebahn.kotoba")
db = DatomicClient.connect()
app = Runtime("deutschebahn-compat")
...
@app.route("/{api_version}/deutschebahn/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_deutschebahn_" + uuid.uuid4().hex[:12]
    db.transact([{
        "deutschebahn.CognitiveRecord/id": internal_record_id,
        "deutschebahn.CognitiveRecord/apiVersion": api_version,
        "deutschebahn.CognitiveRecord/originalPayload": str(raw_data),
        "deutschebahn.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "deutschebahn.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_deutschebahn_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "deutschebahn_entity",
