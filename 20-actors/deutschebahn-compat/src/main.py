"""
Py Kotodama WASM entrypoint for the Deutschebahn clean-room actor.

Clean-room, API-shaped CRUD over a Datomic-backed Kotoba schema.
Generated by 70-tools/deepen_actors.py (ADR 260607 deepening phase).
No proprietary code or credentials; resource shapes only.
"""
from kotodama import Runtime
from kotoba import load_schema
from datomic import DatomicClient
import uuid
import datetime

schema = load_schema("../schema/deutschebahn.kotoba")
db = DatomicClient.connect()
app = Runtime("deutschebahn-compat")


def now():
    return datetime.datetime.utcnow().isoformat()


def new_id(prefix):
    return f"{prefix}_" + uuid.uuid4().hex[:16]


def _persist(entity, rec):
    """Transact a record into Datomic as namespaced EAVT facts."""
    facts = {}
    for k, v in rec.items():
        facts[f"deutschebahn.{entity}/{k}"] = v
    db.transact([facts])
    return rec


def _query(entity, eid=None):
    """Read records of an entity from Datomic (optionally by id)."""
    pattern = {"entity": f"deutschebahn.{entity}"}
    if eid is not None:
        pattern["id"] = eid
    return db.query(pattern)


def _require(data, fields):
    missing = [f for f in fields if not data.get(f)]
    if missing:
        return {"error": {"message": "Missing required fields: " + ", ".join(missing)}}
    return None


@app.route("/v1/stops", methods=["POST"])
def create_stop(request):
    """Create a Stop."""
    data = request.json or request.form or {}
    err = _require(data, ['code', 'name'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_sto")}
    rec["code"] = data.get('code')
    rec["name"] = data.get('name')
    rec["lat"] = float(data.get('lat') or 0)
    rec["lon"] = float(data.get('lon') or 0)
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Stop", rec)
    return rec, 201

@app.route("/v1/stops", methods=["GET"])
def list_stops(request):
    """List Stops."""
    items = _query("Stop")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/stops/<eid>", methods=["GET"])
def get_stop(request, eid):
    """Retrieve a Stop by id."""
    rows = _query("Stop", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/stops/<eid>", methods=["POST", "PATCH"])
def update_stop(request, eid):
    """Update a Stop."""
    rows = _query("Stop", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Stop", rec)
    return rec, 200

@app.route("/v1/stops/<eid>", methods=["DELETE"])
def delete_stop(request, eid):
    """Delete a Stop."""
    rows = _query("Stop", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.Stop", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/routes", methods=["POST"])
def create_route(request):
    """Create a Route."""
    data = request.json or request.form or {}
    err = _require(data, ['shortName', 'longName'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_rou")}
    rec["shortName"] = data.get('shortName')
    rec["longName"] = data.get('longName')
    rec["mode"] = data.get('mode')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Route", rec)
    return rec, 201

@app.route("/v1/routes", methods=["GET"])
def list_routes(request):
    """List Routes."""
    items = _query("Route")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/routes/<eid>", methods=["GET"])
def get_route(request, eid):
    """Retrieve a Route by id."""
    rows = _query("Route", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/routes/<eid>", methods=["POST", "PATCH"])
def update_route(request, eid):
    """Update a Route."""
    rows = _query("Route", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Route", rec)
    return rec, 200

@app.route("/v1/routes/<eid>", methods=["DELETE"])
def delete_route(request, eid):
    """Delete a Route."""
    rows = _query("Route", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.Route", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/trips", methods=["POST"])
def create_trip(request):
    """Create a Trip."""
    data = request.json or request.form or {}
    err = _require(data, ['headsign'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_tri")}
    rec["routeId"] = data.get('routeId')
    rec["headsign"] = data.get('headsign')
    rec["serviceId"] = data.get('serviceId')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Trip", rec)
    return rec, 201

@app.route("/v1/trips", methods=["GET"])
def list_trips(request):
    """List Trips."""
    items = _query("Trip")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/trips/<eid>", methods=["GET"])
def get_trip(request, eid):
    """Retrieve a Trip by id."""
    rows = _query("Trip", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/trips/<eid>", methods=["POST", "PATCH"])
def update_trip(request, eid):
    """Update a Trip."""
    rows = _query("Trip", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Trip", rec)
    return rec, 200

@app.route("/v1/trips/<eid>", methods=["DELETE"])
def delete_trip(request, eid):
    """Delete a Trip."""
    rows = _query("Trip", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/vehicles", methods=["POST"])
def create_vehicle(request):
    """Create a Vehicle."""
    data = request.json or request.form or {}
    err = _require(data, ['label', 'lat'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_veh")}
    rec["routeId"] = data.get('routeId')
    rec["label"] = data.get('label')
    rec["lat"] = float(data.get('lat') or 0)
    rec["lon"] = float(data.get('lon') or 0)
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Vehicle", rec)
    return rec, 201

@app.route("/v1/vehicles", methods=["GET"])
def list_vehicles(request):
    """List Vehicles."""
    items = _query("Vehicle")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/vehicles/<eid>", methods=["GET"])
def get_vehicle(request, eid):
    """Retrieve a Vehicle by id."""
    rows = _query("Vehicle", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/vehicles/<eid>", methods=["POST", "PATCH"])
def update_vehicle(request, eid):
    """Update a Vehicle."""
    rows = _query("Vehicle", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Vehicle", rec)
    return rec, 200

@app.route("/v1/vehicles/<eid>", methods=["DELETE"])
def delete_vehicle(request, eid):
    """Delete a Vehicle."""
    rows = _query("Vehicle", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.Vehicle", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/servicealerts", methods=["POST"])
def create_service_alert(request):
    """Create a ServiceAlert."""
    data = request.json or request.form or {}
    err = _require(data, ['cause', 'effect'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_ser")}
    rec["routeId"] = data.get('routeId')
    rec["cause"] = data.get('cause')
    rec["effect"] = data.get('effect')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("ServiceAlert", rec)
    return rec, 201

@app.route("/v1/servicealerts", methods=["GET"])
def list_service_alerts(request):
    """List ServiceAlerts."""
    items = _query("ServiceAlert")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/servicealerts/<eid>", methods=["GET"])
def get_service_alert(request, eid):
    """Retrieve a ServiceAlert by id."""
    rows = _query("ServiceAlert", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/servicealerts/<eid>", methods=["POST", "PATCH"])
def update_service_alert(request, eid):
    """Update a ServiceAlert."""
    rows = _query("ServiceAlert", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("ServiceAlert", rec)
    return rec, 200

@app.route("/v1/servicealerts/<eid>", methods=["DELETE"])
def delete_service_alert(request, eid):
    """Delete a ServiceAlert."""
    rows = _query("ServiceAlert", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.ServiceAlert", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/fares", methods=["POST"])
def create_fare(request):
    """Create a Fare."""
    data = request.json or request.form or {}
    err = _require(data, ['price', 'currency'])
    if err:
        return err, 400
    rec = {"id": new_id("deutsche_far")}
    rec["routeId"] = data.get('routeId')
    rec["price"] = float(data.get('price') or 0)
    rec["currency"] = data.get('currency')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Fare", rec)
    return rec, 201

@app.route("/v1/fares", methods=["GET"])
def list_fares(request):
    """List Fares."""
    items = _query("Fare")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/fares/<eid>", methods=["GET"])
def get_fare(request, eid):
    """Retrieve a Fare by id."""
    rows = _query("Fare", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/fares/<eid>", methods=["POST", "PATCH"])
def update_fare(request, eid):
    """Update a Fare."""
    rows = _query("Fare", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Fare", rec)
    return rec, 200

@app.route("/v1/fares/<eid>", methods=["DELETE"])
def delete_fare(request, eid):
    """Delete a Fare."""
    rows = _query("Fare", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"deutschebahn.Fare", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "deutschebahn-compat", "entities": ['Stop', 'Route', 'Trip', 'Vehicle', 'ServiceAlert', 'Fare']}, 200


if __name__ == "__main__":
    app.start()
