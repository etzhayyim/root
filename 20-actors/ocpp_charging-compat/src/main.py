"""
Py Kotodama WASM entrypoint for the Ocpp_charging clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"ocpp_charging.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"ocpp_charging.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_vehicle(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.Vehicle", "id": eid})
...
def delete_signal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.Signal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.ChargeSession", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_connector(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.Connector", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trip(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_command(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"ocpp_charging.Command", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "ocpp_charging-compat", "entities": ['Vehicle', 'Signal', 'ChargeSession', 'Connector', 'Trip', 'Command']}, 200
