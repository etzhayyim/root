"""
Py Kotodama WASM entrypoint for Ocpp_charging Cognitive Actor.
...
schema = load_schema("../schema/ocpp_charging.kotoba")
db = DatomicClient.connect()
app = Runtime("ocpp_charging-compat")
...
@app.route("/{api_version}/ocpp_charging/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ocpp_charging_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ocpp_charging.CognitiveRecord/id": internal_record_id,
        "ocpp_charging.CognitiveRecord/apiVersion": api_version,
        "ocpp_charging.CognitiveRecord/originalPayload": str(raw_data),
        "ocpp_charging.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ocpp_charging.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ocpp_charging_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ocpp_charging_entity",
