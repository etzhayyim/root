"""
Py Kotodama WASM entrypoint for Costar Cognitive Actor.
...
schema = load_schema("../schema/costar.kotoba")
db = DatomicClient.connect()
app = Runtime("costar-compat")
...
@app.route("/{api_version}/costar/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_costar_" + uuid.uuid4().hex[:12]
    db.transact([{
        "costar.CognitiveRecord/id": internal_record_id,
        "costar.CognitiveRecord/apiVersion": api_version,
        "costar.CognitiveRecord/originalPayload": str(raw_data),
        "costar.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "costar.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_costar_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "costar_entity",
