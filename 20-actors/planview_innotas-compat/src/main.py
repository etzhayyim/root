"""
Py Kotodama WASM entrypoint for Planview_innotas Cognitive Actor.
...
schema = load_schema("../schema/planview_innotas.kotoba")
db = DatomicClient.connect()
app = Runtime("planview_innotas-compat")
...
@app.route("/{api_version}/planview_innotas/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_planview_innotas_" + uuid.uuid4().hex[:12]
    db.transact([{
        "planview_innotas.CognitiveRecord/id": internal_record_id,
        "planview_innotas.CognitiveRecord/apiVersion": api_version,
        "planview_innotas.CognitiveRecord/originalPayload": str(raw_data),
        "planview_innotas.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "planview_innotas.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_planview_innotas_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "planview_innotas_entity",
