"""
Py Kotodama WASM entrypoint for Universal_robots Cognitive Actor.
...
@app.route("/{api_version}/universal_robots/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_universal_robots_" + uuid.uuid4().hex[:12]
    db.transact([{
        "universal_robots.CognitiveRecord/id": internal_record_id,
        "universal_robots.CognitiveRecord/apiVersion": api_version,
        "universal_robots.CognitiveRecord/originalPayload": str(raw_data),
        "universal_robots.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "universal_robots.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_universal_robots_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "universal_robots_entity",
