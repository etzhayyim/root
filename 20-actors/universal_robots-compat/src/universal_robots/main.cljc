(ns universal-robots.main
  "Kotodama WASM entrypoint for the Universal_robots clean-room actor (L5) — Clojure port.
  ...
  facts (`universal_robots.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "universal_robots")
(def tier "L5")
...
(def entity-specs
  [{:entity "RtdeRobotState"     :plural "rtderobotstates"     :id-prefix "universa_rtd"
    :fields [:timestamp :robot_mode :robot_status_bits :safety_mode :safety_status_bits :actual_main_voltage :actual_robot_voltage :actual_robot_current]
    :required [:timestamp :robot_mode]
    :coerce {:timestamp :float :robot_mode :int :robot_status_bits :int :safety_mode :int :safety_status_bits :int :actual_main_voltage :float :actual_robot_voltage :float :actual_robot_current :float}
    :refs {}}
   {:entity "RtdeExecutionState"  :plural "rtdeexecutionstates"  :id-prefix "universa_rtd"
    :fields [:runtime_state :actual_execution_time :speed_scaling :target_speed_fraction]
    :required [:runtime_state :actual_execution_time]
    :coerce {:runtime_state :int :actual_execution_time :float :speed_scaling :float :target_speed_fraction :float}
    :refs {}}
   {:entity "RtdeToolState"       :plural "rtdetoolstates"       :id-prefix "universa_rtd"
    :fields [:tool_mode :tool_output_voltage :tool_output_current :tool_temperature]
    :required [:tool_mode :tool_output_voltage]
    :coerce {:tool_mode :int :tool_output_voltage :int :tool_output_current :float :tool_temperature :float}
    :refs {}}
   {:entity "DashboardRobotState" :plural "dashboardrobotstates" :id-prefix "universa_das"
    :fields [:robotmode :safetystatus :running]
    :required [:robotmode :safetystatus]
    :coerce {:running :bool}
    :refs {}}
   {:entity "DashboardProgram"    :plural "dashboardprograms"    :id-prefix "universa_das"
    :fields [:programName :state]
    :required [:programName :state]
    :coerce {}
    :refs {}}
   {:entity "DashboardInstallation" :plural "dashboardinstallations" :id-prefix "universa_das"
    :fields [:installationName]
...
(defn emit-facts
  "EAVT facts for one record: {\"universal_robots.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "universal_robots-compat" :tier tier :entities entities} 200])
...
(defn start! [] :universal_robots-compat/ready)
