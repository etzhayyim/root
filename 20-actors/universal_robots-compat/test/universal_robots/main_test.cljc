(ns universal-robots.main-test
  "Contract + behavioral test for the universal-robots-compat L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "universal_robots-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'universal-robots.main-test)]
