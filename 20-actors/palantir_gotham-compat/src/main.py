"""
Py Kotodama WASM entrypoint for Palantir_gotham Cognitive Actor.
...
schema = load_schema("../schema/palantir_gotham.kotoba")
db = DatomicClient.connect()
app = Runtime("palantir_gotham-compat")
...
@app.route("/{api_version}/palantir_gotham/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_palantir_gotham_" + uuid.uuid4().hex[:12]
    db.transact([{
        "palantir_gotham.CognitiveRecord/id": internal_record_id,
        "palantir_gotham.CognitiveRecord/apiVersion": api_version,
        "palantir_gotham.CognitiveRecord/originalPayload": str(raw_data),
        "palantir_gotham.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "palantir_gotham.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_palantir_gotham_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "palantir_gotham_entity",
