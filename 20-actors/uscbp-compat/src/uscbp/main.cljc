(ns uscbp.main
  "Kotodama WASM entrypoint for the Uscbp clean-room actor (L4) — Clojure port.
  ...
  facts (`uscbp.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"uscbp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "uscbp-compat" :tier tier :entities entities} 200])
