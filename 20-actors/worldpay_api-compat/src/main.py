"""
Py Kotodama WASM entrypoint for Worldpay_api Cognitive Actor.
...
schema = load_schema("../schema/worldpay_api.kotoba")
db = DatomicClient.connect()
app = Runtime("worldpay_api-compat")
...
@app.route("/{api_version}/worldpay_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_worldpay_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "worldpay_api.CognitiveRecord/id": internal_record_id,
        "worldpay_api.CognitiveRecord/apiVersion": api_version,
        "worldpay_api.CognitiveRecord/originalPayload": str(raw_data),
        "worldpay_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "worldpay_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_worldpay_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "worldpay_api_entity",
