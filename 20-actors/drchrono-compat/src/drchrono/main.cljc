(ns drchrono.main
  "Kotodama WASM entrypoint for the Drchrono clean-room actor (L5) — Clojure port.
  ...
  facts (`drchrono.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "drchrono")
(def tier "L5")
...
(def entity-specs
  [{:entity "Patient"      :plural "patients"      :id-prefix "drchrono_pat"
    :fields [:chartId :dateOfBirth :email :cellPhone :address :city :copay :defaultPharmacy :disableSmsMessages :doctor :emergencyContactName :emergencyContactPhone :employer :dateOfFirstAppointment :dateOfLastAppointment]
    :required [:dateOfBirth :email]
    :coerce {:disableSmsMessages :bool :doctor :int}
    :refs {}}
   {:entity "Appointment"  :plural "appointments"  :id-prefix "drchrono_app"
    :fields [:doctor :duration :examRoom :color :billingStatus :ins1Status :allowOverlapping :apptIsBreak :deletedFlag :baseRecurringAppointment :billingProvider :firstBilledDate]
    :required [:doctor :duration]
    :coerce {:doctor :int :duration :int :examRoom :int :allowOverlapping :bool :apptIsBreak :bool :deletedFlag :bool}
    :refs {}}
   {:entity "Doctor"       :plural "doctors"       :id-prefix "drchrono_doc"
    :fields [:firstName :lastName :email :cellPhone :officePhone :npiNumber :groupNpiNumber :specialty :jobTitle :practiceGroupName :isAccountSuspended]
    :required [:firstName :lastName]
    :coerce {:isAccountSuspended :bool}
    :refs {}}
   {:entity "Office"       :plural "offices"       :id-prefix "drchrono_off"
    :fields [:name :address :city :state :zipCode :phoneNumber :faxNumber :doctor :archived :onlineScheduling :startTime :endTime :taxIdNumberProfessional]
    :required [:name :address]
    :coerce {:archived :bool :onlineScheduling :bool}
    :refs {}}
   {:entity "ClinicalNote" :plural "clinicalnotes" :id-prefix "drchrono_cli"
    :fields [:patient :appointment :archived]
    :required [:patient :appointment]
    :coerce {:archived :bool}
    :refs {}}
   {:entity "LabResult"    :plural "labresults"    :id-prefix "drchrono_lab"
    :fields [:labTest :labOrder :observationCode :observationDescription :value :unit :normalRange :abnormalStatus :status :comments :groupCode :testPerformed :specimenReceived]
    :required [:labTest :labOrder]
    :coerce {:labTest :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"drchrono.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "drchrono-compat" :tier tier :entities entities} 200])
...
(defn start! [] :drchrono-compat/ready)
