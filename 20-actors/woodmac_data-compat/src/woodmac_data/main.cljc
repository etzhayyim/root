(ns woodmac_data.main
  "Kotodama WASM entrypoint for the Woodmac_data clean-room actor (L4) — Clojure port.
  ...
  facts (`woodmac_data.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "woodmac_data")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"woodmac_data.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "woodmac_data-compat" :tier tier :entities entities} 200])
...
(defn start! [] :woodmac_data-compat/ready)
