"""
Py Kotodama WASM entrypoint for Vercel Cognitive Actor.
...
@app.route("/{api_version}/vercel/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vercel_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vercel.CognitiveRecord/id": internal_record_id,
        "vercel.CognitiveRecord/apiVersion": api_version,
        "vercel.CognitiveRecord/originalPayload": str(raw_data),
        "vercel.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vercel.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vercel_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "vercel_entity",
