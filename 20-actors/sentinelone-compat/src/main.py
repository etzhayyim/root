"""
Py Kotodama WASM entrypoint for Sentinelone Cognitive Actor.
...
schema = load_schema("../schema/sentinelone.kotoba")
db = DatomicClient.connect()
app = Runtime("sentinelone-compat")
...
@app.route("/{api_version}/sentinelone/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sentinelone_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sentinelone.CognitiveRecord/id": internal_record_id,
        "sentinelone.CognitiveRecord/apiVersion": api_version,
        "sentinelone.CognitiveRecord/originalPayload": str(raw_data),
        "sentinelone.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sentinelone.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sentinelone_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sentinelone_entity",
