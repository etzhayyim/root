"""
Py Kotodama WASM entrypoint for Sepa_ct Cognitive Actor.
...
schema = load_schema("../schema/sepa_ct.kotoba")
db = DatomicClient.connect()
app = Runtime("sepa_ct-compat")
...
@app.route("/{api_version}/sepa_ct/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sepa_ct_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sepa_ct.CognitiveRecord/id": internal_record_id,
        "sepa_ct.CognitiveRecord/apiVersion": api_version,
        "sepa_ct.CognitiveRecord/originalPayload": str(raw_data),
        "sepa_ct.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sepa_ct.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sepa_ct_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sepa_ct_entity",
