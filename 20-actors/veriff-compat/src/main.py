"""
Py Kotodama WASM entrypoint for Veriff Cognitive Actor.
...
@app.route("/{api_version}/veriff/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_veriff_" + uuid.uuid4().hex[:12]
    db.transact([{
        "veriff.CognitiveRecord/id": internal_record_id,
        "veriff.CognitiveRecord/apiVersion": api_version,
        "veriff.CognitiveRecord/originalPayload": str(raw_data),
        "veriff.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "veriff.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_veriff_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "veriff_entity",
