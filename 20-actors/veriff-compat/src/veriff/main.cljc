(ns veriff.main
  "Kotodama WASM entrypoint for the Veriff clean-room actor (L4) — Clojure port.
  ...
  facts (`veriff.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "veriff")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"veriff.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "veriff-compat" :tier tier :entities entities} 200])
...
(defn start! [] :veriff-compat/ready)
