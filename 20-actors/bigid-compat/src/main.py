"""
Py Kotodama WASM entrypoint for Bigid Cognitive Actor.
...
schema = load_schema("../schema/bigid.kotoba")
db = DatomicClient.connect()
app = Runtime("bigid-compat")
...
@app.route("/{api_version}/bigid/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bigid_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bigid.CognitiveRecord/id": internal_record_id,
        "bigid.CognitiveRecord/apiVersion": api_version,
        "bigid.CognitiveRecord/originalPayload": str(raw_data),
        "bigid.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bigid.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bigid_" + uuid.uuid4().hex[:8])
