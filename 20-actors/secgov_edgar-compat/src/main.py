"""
Py Kotodama WASM entrypoint for the Secgov_edgar clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"secgov_edgar.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"secgov_edgar.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_filing(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.Filing", "id": eid})
...
def delete_entity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.Entity", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_dataset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.Dataset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_application(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.Application", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_public_record(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.PublicRecord", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_notice(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"secgov_edgar.Notice", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "secgov_edgar-compat", "entities": ['Filing', 'Entity', 'Dataset', 'Application', 'PublicRecord', 'Notice']}, 200
