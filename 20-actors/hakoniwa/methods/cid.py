...
Pure-stdlib re-implementation of the repo-canonical content-address used by the WASM loaders
(20-actors/*/wasm/verify.mjs, ADR-2605231525 / 2606014500) and by rasen's ingest: CIDv1, raw
codec (0x55), multihash sha2-256 (0x12 0x20), multibase base32-lower with 'b' prefix. This is
the SAME CID `ipfs add --cid-version=1 --raw-leaves` produces for a single raw block (< 256
KiB), so an ingested box's content-address is verifiable with or without the `ipfs` daemon.

Single-block only by design: hakoniwa ingests a BOUNDED public-entity slice (G6/G8), so the
EDN artifact fits one raw block.
"""
