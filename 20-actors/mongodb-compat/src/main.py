"""
Py Kotodama WASM entrypoint for Mongodb Cognitive Actor.
...
@app.route("/{api_version}/mongodb/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mongodb_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mongodb.CognitiveRecord/id": internal_record_id,
        "mongodb.CognitiveRecord/apiVersion": api_version,
        "mongodb.CognitiveRecord/originalPayload": str(raw_data),
        "mongodb.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mongodb.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mongodb_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mongodb_entity",
