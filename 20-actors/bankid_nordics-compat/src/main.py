"""
Py Kotodama WASM entrypoint for Bankid_nordics Cognitive Actor.
...
schema = load_schema("../schema/bankid_nordics.kotoba")
db = DatomicClient.connect()
app = Runtime("bankid_nordics-compat")
...
@app.route("/{api_version}/bankid_nordics/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bankid_nordics_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bankid_nordics.CognitiveRecord/id": internal_record_id,
        "bankid_nordics.CognitiveRecord/apiVersion": api_version,
        "bankid_nordics.CognitiveRecord/originalPayload": str(raw_data),
        "bankid_nordics.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bankid_nordics.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bankid_nordics_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bankid_nordics_entity",
