(ns msc_api.main
  "Kotodama WASM entrypoint for the msc_api clean-room actor (L4) — Clojure port.
  ...
  facts (`msc_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "msc_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "mscapi_tra"
    :fields [:gtin :description :brand]          :required [:gtin :description]
    :coerce {}                        :refs {}}
   {:entity "Event"     :plural "events"      :id-prefix "mscapi_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                        :refs {}}
   {:entity "Document"  :plural "documents"   :id-prefix "mscapi_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                        :refs {}}
   {:entity "Container" :plural "containers"  :id-prefix "mscapi_con"
    :fields [:bicCode :type :status]          :required [:bicCode :type]
    :coerce {}                        :refs {}}
   {:entity "Party"     :plural "parties"     :id-prefix "mscapi_par"
    :fields [:gln :name :role]                :required [:gln :name]
    :coerce {}                        :refs {}}
   {:entity "Shipment"  :plural "shipments"   :id-prefix "mscapi_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"msc_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "msc_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :msc_api-compat/ready)
