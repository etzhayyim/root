"""
Py Kotodama WASM entrypoint for Msc_api Cognitive Actor.
...
schema = load_schema("../schema/msc_api.kotoba")
db = DatomicClient.connect()
app = Runtime("msc_api-compat")
...
@app.route("/{api_version}/msc_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_msc_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "msc_api.CognitiveRecord/id": internal_record_id,
        "msc_api.CognitiveRecord/apiVersion": api_version,
        "msc_api.CognitiveRecord/originalPayload": str(raw_data),
        "msc_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "msc_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_msc_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "msc_api_entity",
