"""
Py Kotodama WASM entrypoint for Ola_cabs Cognitive Actor.
...
schema = load_schema("../schema/ola_cabs.kotoba")
db = DatomicClient.connect()
app = Runtime("ola_cabs-compat")
...
@app.route("/{api_version}/ola_cabs/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ola_cabs_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ola_cabs.CognitiveRecord/id": internal_record_id,
        "ola_cabs.CognitiveRecord/apiVersion": api_version,
        "ola_cabs.CognitiveRecord/originalPayload": str(raw_data),
        "ola_cabs.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ola_cabs.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ola_cabs_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ola_cabs_entity",
