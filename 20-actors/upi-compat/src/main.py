"""
Py Kotodama WASM entrypoint for Upi Cognitive Actor.
...
schema = load_schema("../schema/upi.kotoba")
db = DatomicClient.connect()
app = Runtime("upi-compat")
...
@app.route("/{api_version}/upi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_upi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "upi.CognitiveRecord/id": internal_record_id,
        "upi.CognitiveRecord/apiVersion": api_version,
        "upi.CognitiveRecord/originalPayload": str(raw_data),
        "upi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "upi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_upi_" + uuid.uuid4().hex[:8])
