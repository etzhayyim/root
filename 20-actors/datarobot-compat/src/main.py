"""
Py Kotodama WASM entrypoint for Datarobot Cognitive Actor.
...
schema = load_schema("../schema/datarobot.kotoba")
db = DatomicClient.connect()
app = Runtime("datarobot-compat")
...
@app.route("/{api_version}/datarobot/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_datarobot_" + uuid.uuid4().hex[:12]
    db.transact([{
        "datarobot.CognitiveRecord/id": internal_record_id,
        "datarobot.CognitiveRecord/apiVersion": api_version,
        "datarobot.CognitiveRecord/originalPayload": str(raw_data),
        "datarobot.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "datarobot.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_datarobot_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "datarobot_entity",
