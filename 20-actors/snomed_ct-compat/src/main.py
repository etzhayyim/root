"""
Py Kotodama WASM entrypoint for Snomed_ct Cognitive Actor.
...
schema = load_schema("../schema/snomed_ct.kotoba")
db = DatomicClient.connect()
app = Runtime("snomed_ct-compat")
...
@app.route("/{api_version}/snomed_ct/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_snomed_ct_" + uuid.uuid4().hex[:12]
    db.transact([{
        "snomed_ct.CognitiveRecord/id": internal_record_id,
        "snomed_ct.CognitiveRecord/apiVersion": api_version,
        "snomed_ct.CognitiveRecord/originalPayload": str(raw_data),
        "snomed_ct.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "snomed_ct.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_snomed_ct_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "snomed_ct_entity",
