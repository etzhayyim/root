"""
Py Kotodama WASM entrypoint for Bentley Cognitive Actor.
...
@app.route("/{api_version}/bentley/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bentley_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bentley.CognitiveRecord/id": internal_record_id,
        "bentley.CognitiveRecord/apiVersion": api_version,
        "bentley.CognitiveRecord/originalPayload": str(raw_data),
        "bentley.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bentley.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bentley_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bentley_entity",
