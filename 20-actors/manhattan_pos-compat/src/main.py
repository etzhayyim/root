"""
Py Kotodama WASM entrypoint for Manhattan_pos Cognitive Actor.
...
schema = load_schema("../schema/manhattan_pos.kotoba")
db = DatomicClient.connect()
app = Runtime("manhattan_pos-compat")
...
@app.route("/{api_version}/manhattan_pos/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_manhattan_pos_" + uuid.uuid4().hex[:12]
    db.transact([{
        "manhattan_pos.CognitiveRecord/id": internal_record_id,
        "manhattan_pos.CognitiveRecord/apiVersion": api_version,
        "manhattan_pos.CognitiveRecord/originalPayload": str(raw_data),
        "manhattan_pos.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "manhattan_pos.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_manhattan_pos_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "manhattan_pos_entity",
