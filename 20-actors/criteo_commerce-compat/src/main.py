"""
Py Kotodama WASM entrypoint for Criteo_commerce Cognitive Actor.
...
schema = load_schema("../schema/criteo_commerce.kotoba")
db = DatomicClient.connect()
app = Runtime("criteo_commerce-compat")
...
@app.route("/{api_version}/criteo_commerce/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_criteo_commerce_" + uuid.uuid4().hex[:12]
    db.transact([{
        "criteo_commerce.CognitiveRecord/id": internal_record_id,
        "criteo_commerce.CognitiveRecord/apiVersion": api_version,
        "criteo_commerce.CognitiveRecord/originalPayload": str(raw_data),
        "criteo_commerce.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "criteo_commerce.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_criteo_commerce_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "criteo_commerce_entity",
