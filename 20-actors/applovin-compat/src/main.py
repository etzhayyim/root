"""
Py Kotodama WASM entrypoint for Applovin Cognitive Actor.
...
schema = load_schema("../schema/applovin.kotoba")
db = DatomicClient.connect()
app = Runtime("applovin-compat")
...
@app.route("/{api_version}/applovin/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_applovin_" + uuid.uuid4().hex[:12]
    db.transact([{
        "applovin.CognitiveRecord/id": internal_record_id,
        "applovin.CognitiveRecord/apiVersion": api_version,
        "applovin.CognitiveRecord/originalPayload": str(raw_data),
        "applovin.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "applovin.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_applovin_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "applovin_entity",
