"""
Py Kotodama WASM entrypoint for Rtos Cognitive Actor.
...
schema = load_schema("../schema/rtos.kotoba")
db = DatomicClient.connect()
app = Runtime("rtos-compat")
...
@app.route("/{api_version}/rtos/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rtos_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rtos.CognitiveRecord/id": internal_record_id,
        "rtos.CognitiveRecord/apiVersion": api_version,
        "rtos.CognitiveRecord/originalPayload": str(raw_data),
        "rtos.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rtos.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rtos_" + uuid.uuid4().hex[:8])
