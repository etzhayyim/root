"""
Py Kotodama WASM entrypoint for Xero Cognitive Actor.
...
@app.route("/{api_version}/xero/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_xero_" + uuid.uuid4().hex[:12]
    db.transact([{
        "xero.CognitiveRecord/id": internal_record_id,
        "xero.CognitiveRecord/apiVersion": api_version,
        "xero.CognitiveRecord/originalPayload": str(raw_data),
        "xero.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "xero.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_xero_" + uuid.uuid4().hex[:8])
