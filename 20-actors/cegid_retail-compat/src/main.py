"""
Py Kotodama WASM entrypoint for the Cegid_retail clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"cegid_retail.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"cegid_retail.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_product(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.Product", "id": eid})
...
def delete_variant(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.Variant", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_order(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.Order", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_line_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.LineItem", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.Customer", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_collection(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cegid_retail.Collection", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "cegid_retail-compat", "entities": ['Product', 'Variant', 'Order', 'LineItem', 'Customer', 'Collection']}, 200
