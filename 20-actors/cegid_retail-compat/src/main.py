"""
Py Kotodama WASM entrypoint for Cegid_retail Cognitive Actor.
...
schema = load_schema("../schema/cegid_retail.kotoba")
db = DatomicClient.connect()
app = Runtime("cegid_retail-compat")
...
@app.route("/{api_version}/cegid_retail/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cegid_retail_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cegid_retail.CognitiveRecord/id": internal_record_id,
        "cegid_retail.CognitiveRecord/apiVersion": api_version,
        "cegid_retail.CognitiveRecord/originalPayload": str(raw_data),
        "cegid_retail.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cegid_retail.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cegid_retail_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cegid_retail_entity",
