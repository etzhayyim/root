"""
Py Kotodama WASM entrypoint for Retool Cognitive Actor.
...
schema = load_schema("../schema/retool.kotoba")
db = DatomicClient.connect()
app = Runtime("retool-compat")
...
@app.route("/{api_version}/retool/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_retool_" + uuid.uuid4().hex[:12]
    db.transact([{
        "retool.CognitiveRecord/id": internal_record_id,
        "retool.CognitiveRecord/apiVersion": api_version,
        "retool.CognitiveRecord/originalPayload": str(raw_data),
        "retool.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "retool.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_retool_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "retool_entity",
