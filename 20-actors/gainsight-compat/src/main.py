"""
Py Kotodama WASM entrypoint for Gainsight Cognitive Actor.
...
schema = load_schema("../schema/gainsight.kotoba")
db = DatomicClient.connect()
app = Runtime("gainsight-compat")
...
@app.route("/{api_version}/gainsight/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gainsight_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gainsight.CognitiveRecord/id": internal_record_id,
        "gainsight.CognitiveRecord/apiVersion": api_version,
        "gainsight.CognitiveRecord/originalPayload": str(raw_data),
        "gainsight.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gainsight.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gainsight_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gainsight_entity",
