(ns fanuc.main
  "Kotodama WASM entrypoint for the Fanuc clean-room actor (L4) — Clojure port.
  ...
  facts (`fanuc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fanuc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fanuc-compat" :tier tier :entities entities} 200])
