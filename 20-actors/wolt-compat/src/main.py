"""
Py Kotodama WASM entrypoint for Wolt Cognitive Actor.
...
@app.route("/{api_version}/wolt/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_wolt_" + uuid.uuid4().hex[:12]
    db.transact([{
        "wolt.CognitiveRecord/id": internal_record_id,
        "wolt.CognitiveRecord/apiVersion": api_version,
        "wolt.CognitiveRecord/originalPayload": str(raw_data),
        "wolt.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "wolt.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_wolt_" + uuid.uuid4().hex[:8])
