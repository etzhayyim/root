(ns wolt.main
  "Kotodama WASM entrypoint for the Wolt clean-room actor (L4) — Clojure port.
  ...
  facts (`wolt.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "wolt_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "wolt_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"wolt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wolt-compat" :tier tier :entities entities} 200])
