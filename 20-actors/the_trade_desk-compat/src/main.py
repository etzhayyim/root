"""
Py Kotodama WASM entrypoint for The_trade_desk Cognitive Actor.
...
schema = load_schema("../schema/the_trade_desk.kotoba")
db = DatomicClient.connect()
app = Runtime("the_trade_desk-compat")
...
@app.route("/{api_version}/the_trade_desk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_the_trade_desk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "the_trade_desk.CognitiveRecord/id": internal_record_id,
        "the_trade_desk.CognitiveRecord/apiVersion": api_version,
        "the_trade_desk.CognitiveRecord/originalPayload": str(raw_data),
        "the_trade_desk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "the_trade_desk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_the_trade_desk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "the_trade_desk_entity",
