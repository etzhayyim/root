"""
Py Kotodama WASM entrypoint for Neon Cognitive Actor.
...
schema = load_schema("../schema/neon.kotoba")
db = DatomicClient.connect()
app = Runtime("neon-compat")
...
@app.route("/{api_version}/neon/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_neon_" + uuid.uuid4().hex[:12]
    db.transact([{
        "neon.CognitiveRecord/id": internal_record_id,
        "neon.CognitiveRecord/apiVersion": api_version,
        "neon.CognitiveRecord/originalPayload": str(raw_data),
        "neon.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "neon.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_neon_" + uuid.uuid4().hex[:8])
