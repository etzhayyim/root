"""
Py Kotodama WASM entrypoint for Oag_flights Cognitive Actor.
...
schema = load_schema("../schema/oag_flights.kotoba")
db = DatomicClient.connect()
app = Runtime("oag_flights-compat")
...
@app.route("/{api_version}/oag_flights/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_oag_flights_" + uuid.uuid4().hex[:12]
    db.transact([{
        "oag_flights.CognitiveRecord/id": internal_record_id,
        "oag_flights.CognitiveRecord/apiVersion": api_version,
        "oag_flights.CognitiveRecord/originalPayload": str(raw_data),
        "oag_flights.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "oag_flights.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_oag_flights_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "oag_flights_entity",
