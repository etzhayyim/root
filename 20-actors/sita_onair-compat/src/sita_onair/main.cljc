(ns sita_onair.main
  "Kotodama WASM entrypoint for the Sita_onair clean-room actor (L4) — Clojure port.
  ...
  facts (`sita_onair.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sita_onair")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sita_onair.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sita_onair-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sita_onair-compat/ready)
