"""
Py Kotodama WASM entrypoint for Sita_onair Cognitive Actor.
...
schema = load_schema("../schema/sita_onair.kotoba")
db = DatomicClient.connect()
app = Runtime("sita_onair-compat")
...
@app.route("/{api_version}/sita_onair/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sita_onair_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sita_onair.CognitiveRecord/id": internal_record_id,
        "sita_onair.CognitiveRecord/apiVersion": api_version,
        "sita_onair.CognitiveRecord/originalPayload": str(raw_data),
        "sita_onair.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sita_onair.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sita_onair_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sita_onair_entity",
