"""
Py Kotodama WASM entrypoint for Nakama Cognitive Actor.
...
@app.route("/{api_version}/nakama/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nakama_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nakama.CognitiveRecord/id": internal_record_id,
        "nakama.CognitiveRecord/apiVersion": api_version,
        "nakama.CognitiveRecord/originalPayload": str(raw_data),
        "nakama.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nakama.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nakama_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nakama_entity",
