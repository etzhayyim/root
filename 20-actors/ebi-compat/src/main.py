"""
Py Kotodama WASM entrypoint for Ebi Cognitive Actor.
...
schema = load_schema("../schema/ebi.kotoba")
db = DatomicClient.connect()
app = Runtime("ebi-compat")
...
@app.route("/{api_version}/ebi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ebi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ebi.CognitiveRecord/id": internal_record_id,
        "ebi.CognitiveRecord/apiVersion": api_version,
        "ebi.CognitiveRecord/originalPayload": str(raw_data),
        "ebi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ebi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ebi_" + uuid.uuid4().hex[:8])
