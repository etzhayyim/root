(ns fastly.main
  "Kotodama WASM entrypoint for the Fastly clean-room actor (L4) — Clojure port.
  ...
  facts (`fastly.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fastly")
(def tier "L4")
...
(def entity-specs
  [{:entity "Service"    :plural "services"    :id-prefix "fastly_ser"
    :fields [:name :type :customerId :comment :paused] :required [:name]
    :coerce {:paused :bool}            :refs {}}
   {:entity "Version"    :plural "versions"    :id-prefix "fastly_ver"
    :fields [:serviceId :number :active :locked :deployed :staging :comment] :required [:number]
    :coerce {:number :int :active :bool :locked :bool :deployed :bool :staging :bool} :refs {:serviceId "Service"}}
   {:entity "Backend"    :plural "backends"    :id-prefix "fastly_bac"
    :fields [:name :serviceId :versionNumber :hostname :port :comment] :required [:name :versionNumber]
    :coerce {:versionNumber :int :port :int} :refs {:serviceId "Service"}}
   {:entity "Domain"     :plural "domains"     :id-prefix "fastly_dom"
    :fields [:serviceId :fqdn :description]   :required [:fqdn]
    :coerce {}                         :refs {:serviceId "Service"}}
   {:entity "Dictionary" :plural "dictionaries" :id-prefix "fastly_dic"
    :fields [:name :serviceId :versionNumber :description] :required [:name :versionNumber]
    :coerce {:versionNumber :int}      :refs {:serviceId "Service"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"fastly.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fastly-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fastly-compat/ready)
