"""
Py Kotodama WASM entrypoint for Fastly Cognitive Actor.
...
@app.route("/{api_version}/fastly/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fastly_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fastly.CognitiveRecord/id": internal_record_id,
        "fastly.CognitiveRecord/apiVersion": api_version,
        "fastly.CognitiveRecord/originalPayload": str(raw_data),
        "fastly.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fastly.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fastly_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fastly_entity",
