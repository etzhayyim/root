"""
Py Kotodama WASM entrypoint for Voi_technology Cognitive Actor.
...
schema = load_schema("../schema/voi_technology.kotoba")
db = DatomicClient.connect()
app = Runtime("voi_technology-compat")
...
@app.route("/{api_version}/voi_technology/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_voi_technology_" + uuid.uuid4().hex[:12]
    db.transact([{
        "voi_technology.CognitiveRecord/id": internal_record_id,
        "voi_technology.CognitiveRecord/apiVersion": api_version,
        "voi_technology.CognitiveRecord/originalPayload": str(raw_data),
        "voi_technology.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "voi_technology.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_voi_technology_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "voi_technology_entity",
