"""
Py Kotodama WASM entrypoint for Opensea Cognitive Actor.
...
@app.route("/{api_version}/opensea/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_opensea_" + uuid.uuid4().hex[:12]
    db.transact([{
        "opensea.CognitiveRecord/id": internal_record_id,
        "opensea.CognitiveRecord/apiVersion": api_version,
        "opensea.CognitiveRecord/originalPayload": str(raw_data),
        "opensea.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "opensea.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_opensea_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "opensea_entity",
