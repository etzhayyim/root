"""
Py Kotodama WASM entrypoint for Ginkgo_bioworks Cognitive Actor.
...
@app.route("/{api_version}/ginkgo_bioworks/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ginkgo_bioworks_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ginkgo_bioworks.CognitiveRecord/id": internal_record_id,
        "ginkgo_bioworks.CognitiveRecord/apiVersion": api_version,
        "ginkgo_bioworks.CognitiveRecord/originalPayload": str(raw_data),
        "ginkgo_bioworks.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ginkgo_bioworks.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ginkgo_bioworks_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ginkgo_bioworks_entity",
