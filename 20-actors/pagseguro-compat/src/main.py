"""
Py Kotodama WASM entrypoint for Pagseguro Cognitive Actor.
...
schema = load_schema("../schema/pagseguro.kotoba")
db = DatomicClient.connect()
app = Runtime("pagseguro-compat")
...
@app.route("/{api_version}/pagseguro/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pagseguro_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pagseguro.CognitiveRecord/id": internal_record_id,
        "pagseguro.CognitiveRecord/apiVersion": api_version,
        "pagseguro.CognitiveRecord/originalPayload": str(raw_data),
        "pagseguro.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pagseguro.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pagseguro_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pagseguro_entity",
