"""
Py Kotodama WASM entrypoint for Flipkart Cognitive Actor.
...
schema = load_schema("../schema/flipkart.kotoba")
db = DatomicClient.connect()
app = Runtime("flipkart-compat")
...
@app.route("/{api_version}/flipkart/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flipkart_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flipkart.CognitiveRecord/id": internal_record_id,
        "flipkart.CognitiveRecord/apiVersion": api_version,
        "flipkart.CognitiveRecord/originalPayload": str(raw_data),
        "flipkart.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flipkart.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flipkart_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "flipkart_entity",
