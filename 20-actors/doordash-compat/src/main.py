"""
Py Kotodama WASM entrypoint for Doordash Cognitive Actor.
...
schema = load_schema("../schema/doordash.kotoba")
db = DatomicClient.connect()
app = Runtime("doordash-compat")
...
@app.route("/{api_version}/doordash/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_doordash_" + uuid.uuid4().hex[:12]
    db.transact([{
        "doordash.CognitiveRecord/id": internal_record_id,
        "doordash.CognitiveRecord/apiVersion": api_version,
        "doordash.CognitiveRecord/originalPayload": str(raw_data),
        "doordash.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "doordash.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_doordash_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "doordash_entity",
