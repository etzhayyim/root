"""
Py Kotodama WASM entrypoint for Fugro_api Cognitive Actor.
...
schema = load_schema("../schema/fugro_api.kotoba")
db = DatomicClient.connect()
app = Runtime("fugro_api-compat")
...
@app.route("/{api_version}/fugro_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fugro_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fugro_api.CognitiveRecord/id": internal_record_id,
        "fugro_api.CognitiveRecord/apiVersion": api_version,
        "fugro_api.CognitiveRecord/originalPayload": str(raw_data),
        "fugro_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fugro_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fugro_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fugro_api_entity",
