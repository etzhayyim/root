"""
Py Kotodama WASM entrypoint for Evergreen_api Cognitive Actor.
...
schema = load_schema("../schema/evergreen_api.kotoba")
db = DatomicClient.connect()
app = Runtime("evergreen_api-compat")
...
@app.route("/{api_version}/evergreen_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_evergreen_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "evergreen_api.CognitiveRecord/id": internal_record_id,
        "evergreen_api.CognitiveRecord/apiVersion": api_version,
        "evergreen_api.CognitiveRecord/originalPayload": str(raw_data),
        "evergreen_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "evergreen_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_evergreen_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "evergreen_api_entity",
