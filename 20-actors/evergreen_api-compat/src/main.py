"""
Py Kotodama WASM entrypoint for the Evergreen_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"evergreen_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"evergreen_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_trade_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.TradeItem", "id": eid})
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_document(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.Document", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_container(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.Container", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_party(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.Party", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_shipment(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"evergreen_api.Shipment", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "evergreen_api-compat", "entities": ['TradeItem', 'Event', 'Document', 'Container', 'Party', 'Shipment']}, 200
