"""
Py Kotodama WASM entrypoint for the Wgs84_geodesy clean-room actor.

Clean-room, API-shaped CRUD over a Datomic-backed Kotoba schema.
Generated by 70-tools/deepen_actors.py (ADR 260607 deepening phase).
No proprietary code or credentials; resource shapes only.
"""
from kotodama import Runtime
from kotoba import load_schema
from datomic import DatomicClient
import uuid
import datetime

schema = load_schema("../schema/wgs84_geodesy.kotoba")
db = DatomicClient.connect()
app = Runtime("wgs84_geodesy-compat")


def now():
    return datetime.datetime.utcnow().isoformat()


def new_id(prefix):
    return f"{prefix}_" + uuid.uuid4().hex[:16]


def _persist(entity, rec):
    """Transact a record into Datomic as namespaced EAVT facts."""
    facts = {}
    for k, v in rec.items():
        facts[f"wgs84_geodesy.{entity}/{k}"] = v
    db.transact([facts])
    return rec


def _query(entity, eid=None):
    """Read records of an entity from Datomic (optionally by id)."""
    pattern = {"entity": f"wgs84_geodesy.{entity}"}
    if eid is not None:
        pattern["id"] = eid
    return db.query(pattern)


def _require(data, fields):
    missing = [f for f in fields if not data.get(f)]
    if missing:
        return {"error": {"message": "Missing required fields: " + ", ".join(missing)}}
    return None


@app.route("/v1/codes", methods=["POST"])
def create_code(request):
    """Create a Code."""
    data = request.json or request.form or {}
    err = _require(data, ['code', 'description'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_cod")}
    rec["code"] = data.get('code')
    rec["description"] = data.get('description')
    rec["scheme"] = data.get('scheme')
    rec["parentCode"] = data.get('parentCode')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Code", rec)
    return rec, 201

@app.route("/v1/codes", methods=["GET"])
def list_codes(request):
    """List Codes."""
    items = _query("Code")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/codes/<eid>", methods=["GET"])
def get_code(request, eid):
    """Retrieve a Code by id."""
    rows = _query("Code", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/codes/<eid>", methods=["POST", "PATCH"])
def update_code(request, eid):
    """Update a Code."""
    rows = _query("Code", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Code", rec)
    return rec, 200

@app.route("/v1/codes/<eid>", methods=["DELETE"])
def delete_code(request, eid):
    """Delete a Code."""
    rows = _query("Code", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Code", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/units", methods=["POST"])
def create_unit(request):
    """Create a Unit."""
    data = request.json or request.form or {}
    err = _require(data, ['symbol', 'name'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_uni")}
    rec["symbol"] = data.get('symbol')
    rec["name"] = data.get('name')
    rec["quantity"] = data.get('quantity')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Unit", rec)
    return rec, 201

@app.route("/v1/units", methods=["GET"])
def list_units(request):
    """List Units."""
    items = _query("Unit")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/units/<eid>", methods=["GET"])
def get_unit(request, eid):
    """Retrieve a Unit by id."""
    rows = _query("Unit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/units/<eid>", methods=["POST", "PATCH"])
def update_unit(request, eid):
    """Update a Unit."""
    rows = _query("Unit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Unit", rec)
    return rec, 200

@app.route("/v1/units/<eid>", methods=["DELETE"])
def delete_unit(request, eid):
    """Delete a Unit."""
    rows = _query("Unit", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Unit", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/references", methods=["POST"])
def create_reference(request):
    """Create a Reference."""
    data = request.json or request.form or {}
    err = _require(data, ['name', 'value'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_ref")}
    rec["name"] = data.get('name')
    rec["value"] = float(data.get('value') or 0)
    rec["uncertainty"] = float(data.get('uncertainty') or 0)
    rec["unit"] = data.get('unit')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Reference", rec)
    return rec, 201

@app.route("/v1/references", methods=["GET"])
def list_references(request):
    """List References."""
    items = _query("Reference")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/references/<eid>", methods=["GET"])
def get_reference(request, eid):
    """Retrieve a Reference by id."""
    rows = _query("Reference", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/references/<eid>", methods=["POST", "PATCH"])
def update_reference(request, eid):
    """Update a Reference."""
    rows = _query("Reference", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Reference", rec)
    return rec, 200

@app.route("/v1/references/<eid>", methods=["DELETE"])
def delete_reference(request, eid):
    """Delete a Reference."""
    rows = _query("Reference", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Reference", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/mappings", methods=["POST"])
def create_mapping(request):
    """Create a Mapping."""
    data = request.json or request.form or {}
    err = _require(data, ['fromCode', 'toScheme'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_map")}
    rec["fromCode"] = data.get('fromCode')
    rec["toScheme"] = data.get('toScheme')
    rec["toCode"] = data.get('toCode')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Mapping", rec)
    return rec, 201

@app.route("/v1/mappings", methods=["GET"])
def list_mappings(request):
    """List Mappings."""
    items = _query("Mapping")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/mappings/<eid>", methods=["GET"])
def get_mapping(request, eid):
    """Retrieve a Mapping by id."""
    rows = _query("Mapping", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/mappings/<eid>", methods=["POST", "PATCH"])
def update_mapping(request, eid):
    """Update a Mapping."""
    rows = _query("Mapping", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Mapping", rec)
    return rec, 200

@app.route("/v1/mappings/<eid>", methods=["DELETE"])
def delete_mapping(request, eid):
    """Delete a Mapping."""
    rows = _query("Mapping", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Mapping", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/locations", methods=["POST"])
def create_location(request):
    """Create a Location."""
    data = request.json or request.form or {}
    err = _require(data, ['unLocode', 'name'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_loc")}
    rec["unLocode"] = data.get('unLocode')
    rec["name"] = data.get('name')
    rec["country"] = data.get('country')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Location", rec)
    return rec, 201

@app.route("/v1/locations", methods=["GET"])
def list_locations(request):
    """List Locations."""
    items = _query("Location")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/locations/<eid>", methods=["GET"])
def get_location(request, eid):
    """Retrieve a Location by id."""
    rows = _query("Location", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/locations/<eid>", methods=["POST", "PATCH"])
def update_location(request, eid):
    """Update a Location."""
    rows = _query("Location", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Location", rec)
    return rec, 200

@app.route("/v1/locations/<eid>", methods=["DELETE"])
def delete_location(request, eid):
    """Delete a Location."""
    rows = _query("Location", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Location", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/v1/epoches", methods=["POST"])
def create_epoch(request):
    """Create a Epoch."""
    data = request.json or request.form or {}
    err = _require(data, ['name', 'utcOffset'])
    if err:
        return err, 400
    rec = {"id": new_id("wgs84geo_epo")}
    rec["name"] = data.get('name')
    rec["utcOffset"] = data.get('utcOffset')
    rec["asOf"] = data.get('asOf')
    rec["createdAt"] = now()
    rec["updatedAt"] = rec["createdAt"]
    _persist("Epoch", rec)
    return rec, 201

@app.route("/v1/epoches", methods=["GET"])
def list_epoches(request):
    """List Epoches."""
    items = _query("Epoch")
    return {"object": "list", "data": items, "count": len(items)}, 200

@app.route("/v1/epoches/<eid>", methods=["GET"])
def get_epoch(request, eid):
    """Retrieve a Epoch by id."""
    rows = _query("Epoch", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    return rows[0], 200

@app.route("/v1/epoches/<eid>", methods=["POST", "PATCH"])
def update_epoch(request, eid):
    """Update a Epoch."""
    rows = _query("Epoch", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    data = request.json or request.form or {}
    rec = rows[0]
    for k, v in data.items():
        if k in rec and k not in ("id", "createdAt"):
            rec[k] = v
    rec["updatedAt"] = now()
    _persist("Epoch", rec)
    return rec, 200

@app.route("/v1/epoches/<eid>", methods=["DELETE"])
def delete_epoch(request, eid):
    """Delete a Epoch."""
    rows = _query("Epoch", eid)
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"wgs84_geodesy.Epoch", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "wgs84_geodesy-compat", "entities": ['Code', 'Unit', 'Reference', 'Mapping', 'Location', 'Epoch']}, 200


if __name__ == "__main__":
    app.start()
