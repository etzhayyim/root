"""
Py Kotodama WASM entrypoint for Yotpo Cognitive Actor.
...
schema = load_schema("../schema/yotpo.kotoba")
db = DatomicClient.connect()
app = Runtime("yotpo-compat")
...
@app.route("/{api_version}/yotpo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yotpo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yotpo.CognitiveRecord/id": internal_record_id,
        "yotpo.CognitiveRecord/apiVersion": api_version,
        "yotpo.CognitiveRecord/originalPayload": str(raw_data),
        "yotpo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yotpo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yotpo_" + uuid.uuid4().hex[:8])
