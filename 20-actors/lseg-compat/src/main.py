"""
Py Kotodama WASM entrypoint for Lseg Cognitive Actor.
...
schema = load_schema("../schema/lseg.kotoba")
db = DatomicClient.connect()
app = Runtime("lseg-compat")
...
@app.route("/{api_version}/lseg/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lseg_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lseg.CognitiveRecord/id": internal_record_id,
        "lseg.CognitiveRecord/apiVersion": api_version,
        "lseg.CognitiveRecord/originalPayload": str(raw_data),
        "lseg.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lseg.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lseg_" + uuid.uuid4().hex[:8])
