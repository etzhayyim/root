"""
Py Kotodama WASM entrypoint for Donorperfect Cognitive Actor.
...
schema = load_schema("../schema/donorperfect.kotoba")
db = DatomicClient.connect()
app = Runtime("donorperfect-compat")
...
@app.route("/{api_version}/donorperfect/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_donorperfect_" + uuid.uuid4().hex[:12]
    db.transact([{
        "donorperfect.CognitiveRecord/id": internal_record_id,
        "donorperfect.CognitiveRecord/apiVersion": api_version,
        "donorperfect.CognitiveRecord/originalPayload": str(raw_data),
        "donorperfect.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "donorperfect.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_donorperfect_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "donorperfect_entity",
