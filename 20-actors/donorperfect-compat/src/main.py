"""
Py Kotodama WASM entrypoint for the Donorperfect clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"donorperfect.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"donorperfect.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Event", "id": eid})
...
def delete_ticket(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Ticket", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_attendee(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Attendee", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_donor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Donor", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_donation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Donation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_membership(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"donorperfect.Membership", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "donorperfect-compat", "entities": ['Event', 'Ticket', 'Attendee', 'Donor', 'Donation', 'Membership']}, 200
