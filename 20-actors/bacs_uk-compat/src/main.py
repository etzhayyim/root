"""
Py Kotodama WASM entrypoint for Bacs_uk Cognitive Actor.
...
schema = load_schema("../schema/bacs_uk.kotoba")
db = DatomicClient.connect()
app = Runtime("bacs_uk-compat")
...
@app.route("/{api_version}/bacs_uk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bacs_uk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bacs_uk.CognitiveRecord/id": internal_record_id,
        "bacs_uk.CognitiveRecord/apiVersion": api_version,
        "bacs_uk.CognitiveRecord/originalPayload": str(raw_data),
        "bacs_uk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bacs_uk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bacs_uk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bacs_uk_entity",
