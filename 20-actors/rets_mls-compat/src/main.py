"""
Py Kotodama WASM entrypoint for Rets_mls Cognitive Actor.
...
schema = load_schema("../schema/rets_mls.kotoba")
db = DatomicClient.connect()
app = Runtime("rets_mls-compat")
...
@app.route("/{api_version}/rets_mls/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rets_mls_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rets_mls.CognitiveRecord/id": internal_record_id,
        "rets_mls.CognitiveRecord/apiVersion": api_version,
        "rets_mls.CognitiveRecord/originalPayload": str(raw_data),
        "rets_mls.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rets_mls.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rets_mls_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rets_mls_entity",
