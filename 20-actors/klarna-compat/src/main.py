"""
Py Kotodama WASM entrypoint for Klarna Cognitive Actor.
...
schema = load_schema("../schema/klarna.kotoba")
db = DatomicClient.connect()
app = Runtime("klarna-compat")
...
@app.route("/{api_version}/klarna/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_klarna_" + uuid.uuid4().hex[:12]
    db.transact([{
        "klarna.CognitiveRecord/id": internal_record_id,
        "klarna.CognitiveRecord/apiVersion": api_version,
        "klarna.CognitiveRecord/originalPayload": str(raw_data),
        "klarna.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "klarna.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_klarna_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "klarna_entity",
