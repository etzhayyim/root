(ns klarna.main
  "Kotodama WASM entrypoint for the Klarna clean-room actor (L4) — Clojure port.
  ...
  facts (`klarna.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "klarna")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"klarna.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "klarna-compat" :tier tier :entities entities} 200])
...
(defn start! [] :klarna-compat/ready)
