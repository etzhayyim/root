"""
Py Kotodama WASM entrypoint for Ovo_indonesia Cognitive Actor.
...
@app.route("/{api_version}/ovo_indonesia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ovo_indonesia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ovo_indonesia.CognitiveRecord/id": internal_record_id,
        "ovo_indonesia.CognitiveRecord/apiVersion": api_version,
        "ovo_indonesia.CognitiveRecord/originalPayload": str(raw_data),
        "ovo_indonesia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ovo_indonesia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ovo_indonesia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ovo_indonesia_entity",
