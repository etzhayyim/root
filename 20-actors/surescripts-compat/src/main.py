"""
Py Kotodama WASM entrypoint for Surescripts Cognitive Actor.
...
schema = load_schema("../schema/surescripts.kotoba")
db = DatomicClient.connect()
app = Runtime("surescripts-compat")
...
@app.route("/{api_version}/surescripts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_surescripts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "surescripts.CognitiveRecord/id": internal_record_id,
        "surescripts.CognitiveRecord/apiVersion": api_version,
        "surescripts.CognitiveRecord/originalPayload": str(raw_data),
        "surescripts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "surescripts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_surescripts_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "surescripts_entity",
