"""
Py Kotodama WASM entrypoint for Coupa Cognitive Actor.
...
schema = load_schema("../schema/coupa.kotoba")
db = DatomicClient.connect()
app = Runtime("coupa-compat")
...
@app.route("/{api_version}/coupa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_coupa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "coupa.CognitiveRecord/id": internal_record_id,
        "coupa.CognitiveRecord/apiVersion": api_version,
        "coupa.CognitiveRecord/originalPayload": str(raw_data),
        "coupa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "coupa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_coupa_" + uuid.uuid4().hex[:8])
