(ns coupa.main
  "Kotodama WASM entrypoint for the Coupa clean-room actor (L4) — Clojure port.
  ...
  facts (`coupa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"coupa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "coupa-compat" :tier tier :entities entities} 200])
