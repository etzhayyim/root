"""
Py Kotodama WASM entrypoint for Supabase Cognitive Actor.
...
@app.route("/{api_version}/supabase/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_supabase_" + uuid.uuid4().hex[:12]
    db.transact([{
        "supabase.CognitiveRecord/id": internal_record_id,
        "supabase.CognitiveRecord/apiVersion": api_version,
        "supabase.CognitiveRecord/originalPayload": str(raw_data),
        "supabase.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "supabase.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_supabase_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "supabase_entity",
