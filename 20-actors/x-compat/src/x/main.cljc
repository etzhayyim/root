(ns x.main
  "Kotodama WASM entrypoint for the X clean-room actor (L5) — Clojure port.
  ...
  facts (`x.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Tweet"   :plural "tweets"   :id-prefix "x_twe"
    :fields [:text :lang :source :paidPartnership :possiblySensitive :replySettings :conversationId :authorId] :required [:text :lang]
    :coerce {:paidPartnership :bool :possiblySensitive :bool} :refs {}}
   {:entity "User"    :plural "users"    :id-prefix "x_use"
    :fields [:name :description :location :url :protected :verified :verifiedType :subscriptionType :profileImageUrl :profileBannerUrl :receivesYourDm] :required [:name :description]
    :coerce {:protected :bool :verified :bool :receivesYourDm :bool} :refs {}}
   {:entity "Space"   :plural "spaces"   :id-prefix "x_spa"
    :fields [:title :state :lang :isTicketed :participantCount :subscriberCount :scheduledStart :startedAt :endedAt] :required [:title :state]
    :coerce {:isTicketed :bool :participantCount :int :subscriberCount :int} :refs {}}
   {:entity "List"    :plural "lists"    :id-prefix "x_lis"
    :fields [:name :description :private :followerCount :memberCount] :required [:name :description]
    :coerce {:private :bool :followerCount :int :memberCount :int} :refs {}}
   {:entity "Media"   :plural "medias"   :id-prefix "x_med"
    :fields [:type] :required [:type]
    :coerce {} :refs {}}
   {:entity "Poll"    :plural "polls"    :id-prefix "x_pol"
    :fields [:durationMinutes :endDatetime :votingStatus] :required [:durationMinutes :endDatetime]
    :coerce {:durationMinutes :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"x.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "x-compat" :tier tier :entities entities} 200])
