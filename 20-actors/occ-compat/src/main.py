"""
Py Kotodama WASM entrypoint for Occ Cognitive Actor.
...
schema = load_schema("../schema/occ.kotoba")
db = DatomicClient.connect()
app = Runtime("occ-compat")
...
@app.route("/{api_version}/occ/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_occ_" + uuid.uuid4().hex[:12]
    db.transact([{
        "occ.CognitiveRecord/id": internal_record_id,
        "occ.CognitiveRecord/apiVersion": api_version,
        "occ.CognitiveRecord/originalPayload": str(raw_data),
        "occ.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "occ.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_occ_" + uuid.uuid4().hex[:8])
