#!/usr/bin/env bb
;; shinogi 鎬 — findings-ledger kotoba Datom-log writer (content-addressed).
(ns shinogi.methods.kotoba
  "kotoba.cljc — shinogi 鎬 append-only FINDINGS LEDGER writer
  (ADR-2606291200, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the junkan/ugachi/meisai family).

  Persists the exam-involution FINDINGS (driver facts + derived stock regimes +
  loop hypotheses + the failure-cycle read-off) to a local append-only EDN log of
  content-addressed transactions — shinogi's only output is structured, append-only
  findings (the ADR's defining property). Each tx CID = 'b' + sha256-hex over the
  ...
  actors — a relief/leverage MAP, NEVER a student/school/country shame-list (G7+G8)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; shinogi 鎬 — FINDINGS LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of exam-involution findings). Generated; DO NOT "
                        "hand-edit. A relief/leverage MAP, never a shame-list. ADR-2606291200.\n")))
