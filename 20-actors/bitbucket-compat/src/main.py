"""
Py Kotodama WASM entrypoint for Bitbucket Cognitive Actor.
...
@app.route("/{api_version}/bitbucket/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bitbucket_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bitbucket.CognitiveRecord/id": internal_record_id,
        "bitbucket.CognitiveRecord/apiVersion": api_version,
        "bitbucket.CognitiveRecord/originalPayload": str(raw_data),
        "bitbucket.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bitbucket.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bitbucket_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bitbucket_entity",
