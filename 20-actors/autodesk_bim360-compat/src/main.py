"""
Py Kotodama WASM entrypoint for Autodesk_bim360 Cognitive Actor.
...
schema = load_schema("../schema/autodesk_bim360.kotoba")
db = DatomicClient.connect()
app = Runtime("autodesk_bim360-compat")
...
@app.route("/{api_version}/autodesk_bim360/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_autodesk_bim360_" + uuid.uuid4().hex[:12]
    db.transact([{
        "autodesk_bim360.CognitiveRecord/id": internal_record_id,
        "autodesk_bim360.CognitiveRecord/apiVersion": api_version,
        "autodesk_bim360.CognitiveRecord/originalPayload": str(raw_data),
        "autodesk_bim360.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "autodesk_bim360.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_autodesk_bim360_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "autodesk_bim360_entity",
