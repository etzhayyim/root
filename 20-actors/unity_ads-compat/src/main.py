"""
Py Kotodama WASM entrypoint for Unity_ads Cognitive Actor.
...
schema = load_schema("../schema/unity_ads.kotoba")
db = DatomicClient.connect()
app = Runtime("unity_ads-compat")
...
@app.route("/{api_version}/unity_ads/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_unity_ads_" + uuid.uuid4().hex[:12]
    db.transact([{
        "unity_ads.CognitiveRecord/id": internal_record_id,
        "unity_ads.CognitiveRecord/apiVersion": api_version,
        "unity_ads.CognitiveRecord/originalPayload": str(raw_data),
        "unity_ads.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "unity_ads.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_unity_ads_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "unity_ads_entity",
