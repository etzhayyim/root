"""
Py Kotodama WASM entrypoint for Cruise_api Cognitive Actor.
...
schema = load_schema("../schema/cruise_api.kotoba")
db = DatomicClient.connect()
app = Runtime("cruise_api-compat")
...
@app.route("/{api_version}/cruise_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cruise_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cruise_api.CognitiveRecord/id": internal_record_id,
        "cruise_api.CognitiveRecord/apiVersion": api_version,
        "cruise_api.CognitiveRecord/originalPayload": str(raw_data),
        "cruise_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cruise_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cruise_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cruise_api_entity",
