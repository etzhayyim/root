"""
Py Kotodama WASM entrypoint for Materials_project Cognitive Actor.
...
@app.route("/{api_version}/materials_project/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_materials_project_" + uuid.uuid4().hex[:12]
    db.transact([{
        "materials_project.CognitiveRecord/id": internal_record_id,
        "materials_project.CognitiveRecord/apiVersion": api_version,
        "materials_project.CognitiveRecord/originalPayload": str(raw_data),
        "materials_project.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "materials_project.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_materials_project_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "materials_project_entity",
