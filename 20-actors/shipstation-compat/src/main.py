"""
Py Kotodama WASM entrypoint for Shipstation Cognitive Actor.
...
schema = load_schema("../schema/shipstation.kotoba")
db = DatomicClient.connect()
app = Runtime("shipstation-compat")
...
@app.route("/{api_version}/shipstation/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shipstation_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shipstation.CognitiveRecord/id": internal_record_id,
        "shipstation.CognitiveRecord/apiVersion": api_version,
        "shipstation.CognitiveRecord/originalPayload": str(raw_data),
        "shipstation.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shipstation.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shipstation_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shipstation_entity",
