"""
Py Kotodama WASM entrypoint for Flexport Cognitive Actor.
...
schema = load_schema("../schema/flexport.kotoba")
db = DatomicClient.connect()
app = Runtime("flexport-compat")
...
@app.route("/{api_version}/flexport/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flexport_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flexport.CognitiveRecord/id": internal_record_id,
        "flexport.CognitiveRecord/apiVersion": api_version,
        "flexport.CognitiveRecord/originalPayload": str(raw_data),
        "flexport.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flexport.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flexport_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "flexport_entity",
