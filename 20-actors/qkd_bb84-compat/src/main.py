"""
Py Kotodama WASM entrypoint for Qkd_bb84 Cognitive Actor.
...
schema = load_schema("../schema/qkd_bb84.kotoba")
db = DatomicClient.connect()
app = Runtime("qkd_bb84-compat")
...
@app.route("/{api_version}/qkd_bb84/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_qkd_bb84_" + uuid.uuid4().hex[:12]
    db.transact([{
        "qkd_bb84.CognitiveRecord/id": internal_record_id,
        "qkd_bb84.CognitiveRecord/apiVersion": api_version,
        "qkd_bb84.CognitiveRecord/originalPayload": str(raw_data),
        "qkd_bb84.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "qkd_bb84.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_qkd_bb84_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "qkd_bb84_entity",
