"""
Py Kotodama WASM entrypoint for Ncino Cognitive Actor.
...
schema = load_schema("../schema/ncino.kotoba")
db = DatomicClient.connect()
app = Runtime("ncino-compat")
...
@app.route("/{api_version}/ncino/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ncino_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ncino.CognitiveRecord/id": internal_record_id,
        "ncino.CognitiveRecord/apiVersion": api_version,
        "ncino.CognitiveRecord/originalPayload": str(raw_data),
        "ncino.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ncino.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ncino_" + uuid.uuid4().hex[:8])
