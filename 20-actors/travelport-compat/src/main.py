"""
Py Kotodama WASM entrypoint for Travelport Cognitive Actor.
...
schema = load_schema("../schema/travelport.kotoba")
db = DatomicClient.connect()
app = Runtime("travelport-compat")
...
@app.route("/{api_version}/travelport/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_travelport_" + uuid.uuid4().hex[:12]
    db.transact([{
        "travelport.CognitiveRecord/id": internal_record_id,
        "travelport.CognitiveRecord/apiVersion": api_version,
        "travelport.CognitiveRecord/originalPayload": str(raw_data),
        "travelport.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "travelport.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_travelport_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "travelport_entity",
