"""
Py Kotodama WASM entrypoint for Chips_clearing Cognitive Actor.
...
schema = load_schema("../schema/chips_clearing.kotoba")
db = DatomicClient.connect()
app = Runtime("chips_clearing-compat")
...
@app.route("/{api_version}/chips_clearing/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_chips_clearing_" + uuid.uuid4().hex[:12]
    db.transact([{
        "chips_clearing.CognitiveRecord/id": internal_record_id,
        "chips_clearing.CognitiveRecord/apiVersion": api_version,
        "chips_clearing.CognitiveRecord/originalPayload": str(raw_data),
        "chips_clearing.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "chips_clearing.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_chips_clearing_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "chips_clearing_entity",
