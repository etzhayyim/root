(ns tcp_ip.main
  "Kotodama WASM entrypoint for the tcp_ip clean-room actor (L4) — Clojure port.
  ...
  facts (`tcp_ip.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tcp_ip")
(def tier "L4")
...
(def entity-specs
  [{:entity "TcpSegment"   :plural "tcpsegments"   :id-prefix "tcpip_tcp"
    :fields [:sourcePort :destinationPort :sequenceNumber :acknowledgmentNumber :dataOffset :controlBits :window :urgentPointer] :required [:sourcePort :destinationPort]
    :coerce {:sourcePort :int :destinationPort :int :sequenceNumber :int :acknowledgmentNumber :int :dataOffset :int :window :int :urgentPointer :int} :refs {}}
   {:entity "TcpConnection" :plural "tcpconnections" :id-prefix "tcpip_tcp"
    :fields [:localPort :remotePort :state :sequenceSendNext :sequenceReceiveNext] :required [:localPort :remotePort]
    :coerce {:localPort :int :remotePort :int :sequenceSendNext :int :sequenceReceiveNext :int} :refs {}}
   {:entity "IpPacket"    :plural "ippackets"    :id-prefix "tcpip_ipp"
    :fields [:version :headerLength :totalLength :identification :flags :fragmentOffset :timeToLive :protocol :sourceAddress :destinationAddress] :required [:version :headerLength]
    :coerce {:version :int :headerLength :int :totalLength :int :identification :int :fragmentOffset :int :timeToLive :int :protocol :int} :refs {}}
   {:entity "IcmpMessage" :plural "icmpmessages" :id-prefix "tcpip_icm"
    :fields [:type :code :checksum :restOfHeader] :required [:type :code]
...
(defn emit-facts
  "EAVT facts for one record: {\"tcp_ip.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tcp_ip-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tcp_ip-compat/ready)
