"""
Py Kotodama WASM entrypoint for the Mercadolibre clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"mercadolibre.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"mercadolibre.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.Customer", "id": eid})
...
def delete_payment_intent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.PaymentIntent", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.Charge", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_refund(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.Refund", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payout(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.Payout", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payment_method(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"mercadolibre.PaymentMethod", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "mercadolibre-compat", "entities": ['Customer', 'PaymentIntent', 'Charge', 'Refund', 'Payout', 'PaymentMethod']}, 200
