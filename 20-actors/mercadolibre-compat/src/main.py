"""
Py Kotodama WASM entrypoint for Mercadolibre Cognitive Actor.
...
schema = load_schema("../schema/mercadolibre.kotoba")
db = DatomicClient.connect()
app = Runtime("mercadolibre-compat")
...
@app.route("/{api_version}/mercadolibre/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mercadolibre_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mercadolibre.CognitiveRecord/id": internal_record_id,
        "mercadolibre.CognitiveRecord/apiVersion": api_version,
        "mercadolibre.CognitiveRecord/originalPayload": str(raw_data),
        "mercadolibre.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mercadolibre.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mercadolibre_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mercadolibre_entity",
