"""
Py Kotodama WASM entrypoint for Ddbj Cognitive Actor.
...
schema = load_schema("../schema/ddbj.kotoba")
db = DatomicClient.connect()
app = Runtime("ddbj-compat")
...
@app.route("/{api_version}/ddbj/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ddbj_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ddbj.CognitiveRecord/id": internal_record_id,
        "ddbj.CognitiveRecord/apiVersion": api_version,
        "ddbj.CognitiveRecord/originalPayload": str(raw_data),
        "ddbj.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ddbj.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ddbj_" + uuid.uuid4().hex[:8])
