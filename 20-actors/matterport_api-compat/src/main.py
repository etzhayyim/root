"""
Py Kotodama WASM entrypoint for the Matterport_api clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"matterport_api.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"matterport_api.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_anchor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Anchor", "id": eid})
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_mesh(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Mesh", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_asset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Asset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matterport_api.Device", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "matterport_api-compat", "entities": ['Anchor', 'Scene', 'Mesh', 'Session', 'Asset', 'Device']}, 200
