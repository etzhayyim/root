...
- `registry/gov-units.world-countries.edn` — **all 197 UN-member sovereign-state
  COUNTRY units** as real data (the 全世界政府 breadth layer; G20 excluded as already
  seeded). Generated by a one-time maintainer pull of the Wikidata SPARQL endpoint
  (UN members `P463 Q1065` + ISO 3166-1 alpha-3 `P298` + official site `P856`),
  parsed deterministically (no summarizing model) → exact QIDs. 163/197 carry an
  official-portal URL; the rest cite Wikidata as provenance (ISO3↔QID verified).
  Gate `scripts/world_coverage.py` (**197 ≥ 193 UN members**).
