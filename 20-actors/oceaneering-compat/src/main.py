"""
Py Kotodama WASM entrypoint for the Oceaneering clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"oceaneering.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"oceaneering.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_object(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Object", "id": eid})
...
def delete_position(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Position", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_observation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Observation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_scene(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Scene", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_float(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Float", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_station(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"oceaneering.Station", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "oceaneering-compat", "entities": ['Object', 'Position', 'Observation', 'Scene', 'Float', 'Station']}, 200
