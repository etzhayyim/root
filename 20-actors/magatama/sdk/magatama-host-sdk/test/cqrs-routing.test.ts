...
describe("XRPC routing", () => {
  ...
  it("routes full NSID to command handler", async () => {
    ...
    app.command("ai.gftd.apps.news.generateArticle", handler);
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.news.generateArticle", [], new Uint8Array());
    expect(res.status).toBe(200);
    expect(handler).toHaveBeenCalledTimes(1);
  });
  ...
  it("XRPC unknown method returns 404", async () => {
    const app = makeApp();
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.news.nonexistent", [], new Uint8Array());
  ...
  it("XRPC handler error returns 500", async () => {
    const app = makeApp();
    app.command("ai.gftd.apps.news.failing", () => { throw new Error("handler boom"); });
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.news.failing", [], new Uint8Array());
  ...
  it("XRPC resolves context from authorization header", async () => {
    ...
    const app = new App(createMockAppDef(), mockHost);
    app.command("ai.gftd.apps.test.testCmd", (ctx, _payload) => {
      capturedCtx = ctx;
      return new TextEncoder().encode("{}");
    });
    await app.handleXRPC("/xrpc/ai.gftd.apps.test.testCmd", [
