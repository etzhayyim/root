...
describe("createWorkerExport pendingWrites drain", () => {
  it("drains writes via ctx.waitUntil when ctx is provided", async () => {
    ...
    const worker = createWorkerExport((sdk) => {
      writtenSdk = sdk;
      sdk.app.command("ai.gftd.apps.test123.ping", async () => {
        // Enqueue a fire-and-forget write via PDS
        sdk.pds?.createRecord("ai.gftd.apps.test123.log", { msg: "hello" });
    ...
    const req = new Request("http://localhost/xrpc/ai.gftd.apps.test123.ping", { method: "POST" });
  ...
  it("awaits writes inline when ctx is undefined (fallback path)", async () => {
    ...
    const worker = createWorkerExport((sdk) => {
      writtenSdk = sdk;
      sdk.app.command("ai.gftd.apps.test123.ping", async () => {
        sdk.pds?.createRecord("ai.gftd.apps.test123.log", { msg: "no-ctx" });
        return JSON.stringify({ ok: true });
      });
    });
  ...
  it("does not call waitUntil when ctx is undefined", async () => {
    ...
    const worker = createWorkerExport((sdk) => {
      sdk.app.command("ai.gftd.apps.test123.noop", async () => {
        return JSON.stringify({ ok: true });
      });
    });

    const env = createEnv(pds);
    const req = new Request("http://localhost/xrpc/ai.gftd.apps.test123.noop", { method: "POST" });
    // No ctx passed
  ...
  it("tracks failed writes in failedWrites[] for outbox archive", async () => {
    ...
    const worker = createWorkerExport((sdk) => {
      writtenSdk = sdk;
      sdk.app.command("ai.gftd.apps.test123.fail", async () => {
    ...
    const req = new Request("http://localhost/xrpc/ai.gftd.apps.test123.fail", { method: "POST" });
