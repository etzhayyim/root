...
describe("handleXRPC dispatch", () => {
  ...
  it("routes full NSID to the correct handler", async () => {
    ...
    app.command("ai.gftd.apps.myapp.processOrder", handler);
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.processOrder", [], new Uint8Array());
  ...
  it("routes full NSID with camelCase method", async () => {
    ...
    app.command("ai.gftd.apps.myapp.doSomething", handler);
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.doSomething", [], new Uint8Array());
    expect(res.status).toBe(200);
  });

  it("returns JSON error for unknown methods", async () => {
    const app = makeApp();
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.unknownMethod", [], new Uint8Array());
    expect(res.status).toBe(404);
    const body = JSON.parse(new TextDecoder().decode(res.body));
    expect(body.error).toContain("unknown xrpc method");
    expect(body.error).toContain("ai.gftd.apps.myapp.unknownMethod");
  ...
  it("returns 500 with error details on handler throw", async () => {
    const app = makeApp();
    app.command("ai.gftd.apps.myapp.explode", () => { throw new Error("kaboom"); });
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.explode", [], new Uint8Array());
  ...
  it("content-type header is application/json", async () => {
    const app = makeApp();
    app.command("ai.gftd.apps.myapp.test", (_ctx, _p) => new TextEncoder().encode("{}"));
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.test", [], new Uint8Array());
  ...
  it("passes body payload to handler", async () => {
    ...
    app.command("ai.gftd.apps.myapp.echo", (_ctx, payload) => {
      receivedBody = payload;
      return payload;
    });
    const inputBody = new TextEncoder().encode('{"msg":"hello"}');
    await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.echo", [], inputBody);
  ...
  it("emits ocel.v2 start/success for ocel-tagged command", async () => {
    ...
    app.command(
      "ai.gftd.apps.myapp.auditAction",
      () => ({ ok: true }),
      withOCELEvent("audit.action"),
    );
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.auditAction", [], new Uint8Array());
    expect(res.status).toBe(200);
  ...
  it("emits ocel.v2 start/success for untagged xrpc command", async () => {
    ...
    const host = createMockHostImports({ ocelEmitEvent });
    const app = new App(createMockAppDef({ id: "myapp" }), host);
    app.command("ai.gftd.apps.myapp.plainAction", () => ({ ok: true }));
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.plainAction", [], new Uint8Array());
    expect(res.status).toBe(200);
    expect(ocelEmitEvent).toHaveBeenCalledTimes(2);
    const startPayload = JSON.parse(ocelEmitEvent.mock.calls[0][0]);
    expect(startPayload.eventType).toBe("xrpc.ai.gftd.apps.myapp.plainAction");
    expect(startPayload.phase).toBe("start");
  });
  ...
  it("emits ocel.v2 error for unknown xrpc method", async () => {
    ...
    const host = createMockHostImports({ ocelEmitEvent });
    const app = new App(createMockAppDef({ id: "myapp" }), host);
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.unknownXrpc", [], new Uint8Array());
    expect(res.status).toBe(404);
    expect(ocelEmitEvent).toHaveBeenCalledTimes(2);
    const errorPayload = JSON.parse(ocelEmitEvent.mock.calls[1][0]);
    expect(errorPayload.eventType).toBe("xrpc.ai.gftd.apps.myapp.unknownXrpc");
  ...
  it("emits ocel.v2 error on command failure", async () => {
    ...
    const app = new App(createMockAppDef({ id: "myapp" }), host);
    app.command(
      "ai.gftd.apps.myapp.failingAuditAction",
      () => {
    ...
    const res = await app.handleXRPC("/xrpc/ai.gftd.apps.myapp.failingAuditAction", [], new Uint8Array());
...
describe("SDK handleRequest", () => {
  it("handles a request end-to-end", async () => {
    ...
    sdk.app.command("ai.gftd.apps.test.ping", () => new TextEncoder().encode('{"pong":true}'));
...
describe("NSID utilities", () => {
  it("witToCollection converts WIT kebab to AT collection", () => {
    expect(witToCollection("gftd:handotai", "article")).toBe("ai.gftd.apps.handotai.article");
  ...
  it("collectionToLabel converts AT collection to SQL label", () => {
    expect(collectionToLabel("ai.gftd.apps.handotai.article")).toBe("Article");
    expect(collectionToLabel("ai.gftd.apps.handotai.semiconductorCompany")).toBe("SemiconductorCompany");
    expect(collectionToLabel("app.bsky.feed.post")).toBe("Post");
  });

  it("nsidToMethod extracts method from NSID", () => {
    expect(nsidToMethod("ai.gftd.apps.handotai.article")).toBe("Article");
