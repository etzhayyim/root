...
export function createHostSDK(config: HostSDKConfig): HostSDK {
  ...
  // and live in magatama.jsonld component.env as plain M365_TENANT_ID / M365_CLIENT_ID.
...
/** Config for createMagatamaWorker — merged Worker entry factory.
 *  Used by deploy.go merged Worker path. Single-app entry typically uses createWorkerExport(). */
export interface MagatamaWorkerConfig {
  appId: string;
  projectName?: string;
  uiMode?: string;
  displayName?: string;
...
export function createMagatamaWorker(config: MagatamaWorkerConfig): { default: { fetch: ExportedHandlerFetchHandler } } {
  let _sdk: HostSDK | null = null;
