...
export class XrpcClient {
  ...
  async governanceRegisterManifest(manifestJson: string): Promise<void> {
    await this.rpc("ai.gftd.governance.registerManifest", { manifestJson });
  }
  ...
  // → com.atproto.repo.createRecord { collection: "ai.gftd.apps.handotai.article", record }

  domain(app: string) {
    const self = this;
    const col = (type: string) => `ai.gftd.apps.${app}.${type}`;
    return {
      ...
      call: <R = unknown>(method: string, params?: Record<string, unknown>) =>
        self.xrpc<R>(`ai.gftd.apps.${app}.${method}`, params ?? {}),
    };
  }
  ...
  async invoke(target: string, method: string, params: Record<string, unknown> = {}): Promise<unknown> {
    return this.xrpc("ai.gftd.pds.invoke", { target, method, params: JSON.stringify(params) });
  }
