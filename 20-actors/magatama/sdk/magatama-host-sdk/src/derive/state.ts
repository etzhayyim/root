 ...
 *   (a) a graph-backed lookup (`SELECT post_uri, post_cid FROM graphar.vertex_internal_derived_link WHERE source_uri = ? LIMIT 1`), populated via `comAtprotoRepoCreateRecord(env, repo, "ai.gftd.internal.derivedLink", …)` with deterministic rkey, or
