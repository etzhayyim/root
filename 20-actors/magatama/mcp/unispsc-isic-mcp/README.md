...
```json
{
  "mcpServers": {
    "etzhayyim-unispsc-isic": {
      "command": "node",
      "args": ["/path/to/repo/20-actors/magatama/mcp/unispsc-isic-mcp/src/cli.ts"],
      "env": {
...
back to the in-cluster Service DNS used by the Magatama actor wrapper
...
```
$ vitest run 20-actors/magatama/mcp/unispsc-isic-mcp/test/
