...
/// S3-compatible cold storage config from `[yata.s3]` section in magatama.toml.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn default_data_dir() -> PathBuf {
    PathBuf::from("./magatama-data")
}
fn default_graph_dir() -> String {
    "./magatama-data/graph".to_string()
}
...
"#;
        ...
        assert_eq!(cfg.yata.data_dir.to_str().unwrap(), "./magatama-data");
        assert_eq!(cfg.yata.graph_data_dir, "./magatama-data/graph");
...
"#;
    ...
    fn yata_default() {
        ...
        assert_eq!(def.data_dir.to_str().unwrap(), "./magatama-data");
...
"#;
        ...
        let cfg2: MagatamaConfig = toml::from_str(&serialized).unwrap();
