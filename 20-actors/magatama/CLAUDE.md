...
Canonical path: `20-actors/magatama`

Directory layout:
- `sdk/magatama-host-sdk`
- `hosts/magatama-desktop-host`
...
| **T3 TS Native (DEFAULT for custom)** | `src/app.ts` → `@gftd/magatama-host-sdk` → esbuild → CF Worker (V8)。WIT = design-time contract | esbuild のみ (<1s) | ~27 apps (WebGPU/ML/FIDO2)。η≈0.91 |
...
**SDK ロジックは `@gftd/magatama-host-sdk` (TypeScript) に統一。** App lifecycle, routing, conversation, governance 自動登録は全て TS host SDK の Single Source。Lexicon JSON (`00-contracts/lexicons/`) が contract SSoT、TS dispatcher が runtime binding SSoT、RisingWave vertex/edge が declaration/projection SSoT を担う。
...
- `@gftd/magatama-host-sdk` package が SDK ロジックの Single Source of Truth
...
│   ├─ @gftd/magatama-host-sdk           → App lifecycle, routing, auto-registration
...
- **Worker mode は TS native。** `magatama-engine` (wasmtime) は Worker deploy path では不使用
...
| Crate / Package | 言語 | 役割 |
|---|---|---|
| **`@gftd/xrpc`** | **TypeScript** | **XRPC Single Source** — NSID utilities (`collectionToLabel`/`expandCollection`/`nsidToFullMethod`)、Transport (`BindingTransport`/`BrowserTransport`/`SSRTransport`)、Auth (`ServiceAuth`/`SessionAuth`/`PublicAuth`)、Error (`WRPCError`/`parseResponse`)、Proxy factory (`procedure`/`query`)。0 外部依存。Lexicon JSON (`00-contracts/lexicons/`) → `70-tools/scripts/contract/gen-service-from-lexicon.mjs` → `service-generated.ts` (typed Connect-like funcs) を自動生成。Bluesky HTTP Reference 222/222 完全カバー |
| **`magatama-host-sdk`** | **TypeScript** | **SDK ロジック Single Source** — App lifecycle, command routing, conversation dispatch, Kysely query builder, governance 自動登録, PdsClient (direct async RPC), shared helpers, LLM module, embed HTML。app.ts が `createComponentHostSDK(env)` を export → generated entry が singleton cache + `sdk.handleRequest()` で全ルーティング (`/_commit`, `/_heartbeat`, `/_app/meta`, `?embed=1`, `/health`, XRPC) を処理。XRPC は `@gftd/xrpc` に委譲。**Hono policy (2026-04-03)**: router-level `onError` + `notFound` を標準採用（`all("*")` 404 代替は禁止）。**Database**: `createKyselyDb(env.HYPERDRIVE)` → Kysely type-safe queries via Hyperdrive → RisingWave。**Shared helpers**: `str`, `num`, `nowISO`, `stripHTML`, `truncateText`, `decodeJson`, `genID`, `rlsDefaults`, `firstRow`。**LLM module**: `llm.ts` (`agentConverseAsync`, `llmAsk`, `llmCall`, `llmJson`)。**app.ts にローカルコピー定義禁止** — SDK import を使用。**SQL / Drizzle archived 2026-04-13** |
| `magatama-config` | Rust | generated TOML parser (source is `magatama.jsonld`) |
...
- capability declaration
  `ai.gftd.agent.actorCapability` record → `vertex_capability`
- policy / governance declaration
  `ai.gftd.agent.governanceRule`, `ai.gftd.agent.roleBinding` → `vertex_policy`, `edge_actor_bound_role`, `edge_capability_requires_permission`
...
ai.gftd.host.secrets.get   ───►   (70-tools/scripts/contract/)         ───►    secretsGet(input: {key:string})
...
```typescript
import { hostClient } from "@gftd/magatama-host-sdk";
...
1. Add `hostClient` to the `@gftd/magatama-host-sdk` import block
...
| `LEXICON_NSID` | Frozen record。`LEXICON_NSID["ai.gftd.apps.foo.bar"] === "ai.gftd.apps.foo.bar"` |
...
**Archived (2026-04-13)**: `AssertCommandNSID` / `AssertQueryNSID` loose-guard types。経緯 + 復元方法は `_archive/20-actors/magatama/sdk/magatama-host-sdk-legacy-nsid-assert-260413/README.md`。
...
export default createWorkerExport((sdk) => {
  sdk.app.command(
    nsid("ai.gftd.apps.foo.bar"),                                   // ← compile error if lexicon missing
    async (ctx, body) => {
      const input = parseLexiconInput("ai.gftd.apps.foo.bar", body);
      //    ^^ typed as LexiconInput<"ai.gftd.apps.foo.bar"> + runtime schema validation
      //       (throws LexiconValidationError on missing/mistyped properties)
      const output: LexiconOutput<"ai.gftd.apps.foo.bar"> = {
...
| `gen-host-client-from-lexicon.mjs` | `ai.gftd.host.*` lexicons → `host-client.ts` (BindingTransport dispatcher) |
...
3. `src/app.ts` で `sdk.app.command(nsid("ai.gftd.apps.{app}.{action}"), handler)` を追加 (query は `sdk.app.query`)
...
## SDK Helper Modules (magatama-host-sdk)

**App 共通パターンを SDK helper として提供。app.ts でのボイラープレート排除。**

| Module | Factory | API |
|---|---|---|
| **`database.ts`** | `createKyselyDb(sql, env.HYPERDRIVE)` | `.selectFrom()`, `.insertInto()`, `.update()`, `.deleteFrom()` — type-safe graph queries (RisingWave PG). See `@gftd/graph-schema` for Database type + row types |
...
```typescript
import { createConsentHelper, createAgentLifecycle, createAuditHelper } from "@gftd/magatama-host-sdk";
...
export default createWorkerExport((sdk) => {
  ...
  sdk.app.command("ai.gftd.apps.myapp.approve", async (_ctx, body) => {
...
**Shinka ロジックは `@gftd/magatama-host-sdk` の `heartbeat-cadence.ts` に統一。** joucho 情緒 5 軸 (joy/calm/stress/gratitude/focus) が行動を駆動。`heartbeatCount % N` 固定タイマー禁止。
...
```typescript
import { resolveHeartbeatCadence, createInboxBuffer, createCadenceState } from "@gftd/magatama-host-sdk";
...
await db.insertInto("vertex_hr_journalEntry")
  .values({
    vertex_id: `at://${did}/ai.gftd.apps.hr.journalEntry/${rkey}`,
...
**禁止パターン**:
- `sdk.pds.createRecord("ai.gftd.apps.*", ...)` / `sdk.pds.dispatch({type:"com.atproto.repo.createRecord",...})` for domain → `db.insertInto('vertex_<actor>_<kind>').values(...).execute()` を使用 (ADR-0036)
...
**App コードはビジネスロジックのみ。** `src/app.ts` 1 ファイルに収める。infra は全て `@gftd/magatama-host-sdk` が提供。

SDK 関数一覧:

| 操作 | SDK 関数 | 備考 |
|---|---|---|
| **Graph query (DEFAULT)** | `magatama.G("Label").Match(Eq{...}).Return("prop").Query()` | squirrel 互換 SQL builder (47 methods) |
| Graph merge | `magatama.G("Label").Merge(Eq{...}).Set(Row{...}).Exec()` | MERGE + SET (upsert) |
| Graph create | `magatama.G("Label").Create(Row{...}).Exec()` | CREATE |
| Graph delete | `magatama.G("Label").Match(Eq{...}).Delete()` | MATCH + DELETE |
| Graph detach delete | `magatama.G("Label").Match(Eq{...}).DetachDelete()` | DETACH DELETE |
| Graph edge merge | `magatama.G("A").Edge("REL", Eq{...}, "B", Eq{...}).Set(Row{...}).Exec()` | MERGE edge + SET props |
| Graph edge query | `magatama.G("A").EdgeMatch("REL", Eq{...}, "B", nil).Return("prop").Query()` | MATCH edge → RETURN dst |
| Graph edge delete | `magatama.G("A").EdgeDelete("REL", Eq{...}, "B", Eq{...}).Exec()` | DELETE relationship |
| Graph traverse | `magatama.G("L").Match(Eq{...}).Traverse("REL",1,3,"L2").Return("p").Query()` | variable-hop 関係辿り |
| Graph optional match | `magatama.G("L").Match(Eq{...}).OptionalMatch().Return("p").Query()` | OPTIONAL MATCH (LEFT JOIN) |
...
| Graph WHERE STARTS WITH | `.WhereStartsWith("collection", "ai.gftd.apps")` | `WHERE n.collection STARTS WITH $p0` |
...
| Graph vector search | `magatama.G("Label").VectorSearch("embedding", vec, 10).Return("title").Query()` | `CALL db.index.vector.queryNodes` |
| Graph full-text | `magatama.G("Label").FullText("IndexName", "query").Return("title").Query()` | `CALL db.index.fulltext.queryNodes` |
| Graph CALL | `magatama.G("Label").Call("db.custom.proc", args...).Query()` | 任意の CALL procedure |
...
| Graph ToSql | `magatama.G("Label").Match(Eq{...}).ToSql()` | SQL 生成のみ (デバッグ用) |
| Graph write (PROHIBITED) | ~~`magatama.SqlExec()`~~ | **禁止** — G() builder or ComAtprotoRepoCreateRecord() を使用 |
| Graph read (PROHIBITED) | ~~`magatama.SqlQueryMap()`~~ | **禁止** — G() builder を使用 |
| KV | `magatama.KvGet/KvPut/KvDelete(bucket, key, ...)` | 小さな状態 |
| Outbound HTTP | `magatama.Send(req)` | WIT outbound-http |
...
| LLM ReAct | `magatama.AgentReact(task, options)` | ReAct loop (LLM→tool→observe→repeat) |
| Parallel activity | `magatama.ActivitySpawnParallel/ActivityAwaitAll` | fan-out/fan-in |
| Pub/Sub | `magatama.PubsubPublish/PubsubPull/PubsubAck` | topic-based at-least-once |
| Secrets | `magatama.SecretsGet/SecretsSet/SecretsDelete` | Secrets Store (CF) or AES-256-GCM (auto-selected) |
| Lock | `magatama.LockTryLock/LockUnlock/LockRenew` | distributed lock (TTL lease) |
| Virtual Actor | `magatama.VirtualActorRegister/VirtualActorInvoke` | actor lifecycle + reentrancy |
| Identity | `magatama.IdentityRegister/IdentityResolve/IdentityListActors` | agent card registry (graph) |
| Capability | `magatama.CapabilityDeclare/CapabilityDiscover/CapabilityListOwn` | CV-1 capability declaration + discovery |
| Invoke | `magatama.Invoke(did, method, params)` | DID-addressed RPC, governance-gated |
| Invoke (auto-discover) | `magatama.Invoke("", method, params)` | Host auto-discovers provider DID |
| Governance | `magatama.GovernanceRegisterManifest/GovernanceCheckPolicy` | RBAC/RACI/approval policy |
| Governance opts | `magatama.Responsible/Accountable/Consulted/Informed/RequireApproval` | command-level RACI + approval declaration |
| Invoke | `magatama.Invoke(did, method, params)` | DID-addressed, governance-gated |
| Invoke stream | `magatama.InvokeStream(did, method, params)` | Streaming invoke (wRPC, backpressure) |
...
| **Reactive input (DEFAULT)** | **`magatama.ComAtprotoSyncSubscribeRepos(handler)`** | **AT commit reactive input (Design D Layer 1): Follow-filtered, 空 handler 禁止** |
| **AppBskyFeedPost (DEFAULT)** | **`magatama.AppBskyFeedPost(did, text, opts)`** | **Single write = AT Record + social post (ComAtprotoRepoCreateRecord+AppBskyFeedPost 二重 write 禁止)** |
...
**`ActorRegistry` は全 multi-DID app (states, pachinko, ISCO 等) の共通ヘルパー。** ハードコード OrgDef[] 配列を graph-seeded data に置換。`20-actors/magatama/sdk/magatama-host-sdk/src/actor-registry.ts`

```typescript
import { ActorRegistry, type ActorDef } from "@gftd/magatama-host-sdk";
...
Signal/evolution/vector search が必要な app のみ。`magatama.jsonld` `"runtimeType": "container"`。

### Dockerfile.magatama (Container mode only)
...
COPY magatama.toml /app/magatama.toml  # generated TOML from magatama.jsonld
COPY svelte/build/ /app/static/
```

- init container は `sh -c "cp /app/component.wasm /wasm/component.wasm && cp /app/magatama.toml /wasm/magatama.toml && cp -r /app/static /wasm/static"` でコピーする (magatama.toml は generated TOML)
- svelte/build/ がない pure API component は `COPY svelte/build/` 行を省略する
- ファイルは `/app/` 配下に配置する（init container のコピー元パスと整合）

### magatama-server image
...
| `cdn` | `magatama-host-sdk` → PDS RPC delegate | `SPIN_VARIABLE_CDN_S3_*`, `SPIN_VARIABLE_CDN_PUBLIC_BASE_URL`, `SPIN_VARIABLE_CDN_DEFAULT_SUBDOMAIN` |
| `telemetry` | `magatama-host-sdk` → PDS RPC delegate | `SPIN_VARIABLE_OTEL_ENDPOINT`, `SPIN_VARIABLE_OTEL_SERVICE_NAME` |
| `access-log` | `magatama-host-sdk` → PDS RPC delegate | none (auto-collected) |
| `ocel` | `magatama-host-sdk` → PDS RPC delegate | none (PDS RPC) |
| `pubsub` | `magatama-host-sdk` → PDS RPC delegate | none (PDS RPC) |
| `secrets` | `magatama-host-sdk` → PDS RPC delegate | `SPIN_VARIABLE_SECRETS_MASTER_KEY` (AES fallback) |
| `lock` | `magatama-host-sdk` → PDS RPC delegate | none |
| `virtual-actor` | `magatama-host-sdk` → PDS RPC delegate | none |
| `agent` (route/react) | `magatama-host-sdk` → PDS RPC delegate | same as agent |
| `activity-parallel` | `magatama-host-sdk` → PDS RPC delegate | none |
| `identity` | `magatama-host-sdk` → PDS RPC delegate (ActorCard graph) | none |
| `capability` | `magatama-host-sdk` → PDS RPC delegate (CV-1 discovery) | none |
| `conversation` | `magatama-host-sdk` → PDS RPC delegate (W Protocol WChannel) | `SPIN_VARIABLE_AT_ACTOR_DID` |
| `governance` | `magatama-host-sdk` → PDS RPC delegate (RBAC/RACI/approval) | none |
| `invoke` | `magatama-host-sdk` → PDS RPC delegate (governance + W Protocol dispatch) | none |
| `serve` | `magatama-host-sdk` → PDS RPC delegate (inbound wrpc.call) | none |
...
**全 entity actor は conversation-aware agent として振る舞う。** 各 entity は `magatama.IdentityRegister()` + `magatama.CapabilityDeclare()` で graph に登録され、`magatama.Invoke("", method, params)` で他 agent から発見・呼び出し可能。
...
import { createWorkerExport, asAgentTool, withCapabilityTags, withCapabilityPhase,
  ...
  AssigneeOrgRole, DecisionClassC } from "@gftd/magatama-host-sdk";

export default createWorkerExport((sdk) => {
  sdk.app.command("ai.gftd.apps.i18n.translate", async (ctx, body) => {
    // translate handler
...
1. **Capability discovery**: `magatama.Invoke("", "contact.search", params)` → Host が Contact agent を自動発見して呼び出し
2. **Domain broadcast**: domain lead agent (Party, Capital, Ci, Project) は `magatama.Invoke("", method, params)` で domain 内 agent に通知
3. **Cross-domain call**: `agent.call` 権限を持つ agent は `magatama.Invoke(did, method, params)` で直接他 domain の agent を呼べる
