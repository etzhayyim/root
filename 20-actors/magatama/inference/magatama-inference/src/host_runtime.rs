...
impl HostRuntime {
    ...
    pub fn load_model_session(&self, model_id: &str) -> Result<LoadModelSessionResult, String> {
        let model_dir = self.resolve_model_storage_path(model_id).ok_or_else(|| {
            format!(
                "model not found: {}. Expected at ~/.cache/magatama/models/{}",
                model_id, model_id
            )
        })?;
    ...
    fn resolve_model_storage_override(&self, model_id: &str) -> Option<String> {
        ...
        let exact_key = format!("MAGATAMA_MODEL_STORAGE_URL_{}", slug.to_ascii_uppercase());
        if let Ok(url) = std::env::var(&exact_key) {
            if !url.trim().is_empty() {
                return Some(url);
            }
        }
        if let Ok(template) = std::env::var("MAGATAMA_MODEL_STORAGE_URL_TEMPLATE") {
    ...
    fn load_yata_catalog(&self) -> Result<HashMap<String, CatalogEntry>, String> {
        if let Ok(path) = std::env::var("MAGATAMA_YATA_CATALOG_PATH") {
            let body = std::fs::read_to_string(&path)
                .map_err(|e| format!("read MAGATAMA_YATA_CATALOG_PATH {path}: {e}"))?;
            return self.parse_yata_catalog(&body);
        }

        let url = std::env::var("MAGATAMA_YATA_CATALOG_URL")
            .unwrap_or_else(|_| "https://atproto.etzhayyim.com/xrpc/ai.gftd.kagami.catalog".to_string());
    ...
    fn materialize_storage_key(&self, storage_key: &str) -> Result<PathBuf, String> {
        ...
        if let Ok(root) = std::env::var("MAGATAMA_YATA_OBJECT_ROOT") {
        ...
        Err(format!(
            "storage key is not locally materialized: {storage_key}. Set MAGATAMA_YATA_OBJECT_ROOT or MAGATAMA_YATA_CATALOG_PATH"
        ))
    }
    ...
    fn object_cache_root(&self) -> Result<PathBuf, String> {
        if let Ok(root) = std::env::var("MAGATAMA_YATA_OBJECT_CACHE") {
    ...
    fn resolve_file_url(&self, storage_key: &str, file: &CatalogFile) -> Result<String, String> {
        ...
        let base_url = std::env::var("MAGATAMA_YATA_OBJECT_BASE_URL").map_err(|_| {
            format!(
                "cannot resolve remote file URL for storage_key {} without MAGATAMA_YATA_OBJECT_BASE_URL",
                storage_key
            )
        })?;
