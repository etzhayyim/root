...
/// Run: cargo test -p magatama-inference --test qwen35_load -- --nocapture
...
fn test_qwen35_hybrid_load() {
    ...
    let dn_blocks = model.blocks.iter()
        .filter(|b| matches!(b, magatama_inference::model::HybridBlock::DeltaNet(_)))
        .count();
    let attn_blocks = model.blocks.iter()
        .filter(|b| matches!(b, magatama_inference::model::HybridBlock::Attention(_)))
        .count();
    ...
    if let magatama_inference::model::HybridBlock::DeltaNet(dn) = &model.blocks[0] {
        println!("\nDeltaNet block 0:");
    ...
    if let magatama_inference::model::HybridBlock::Attention(attn) = &model.blocks[3] {
        println!("\nAttention block 3:");
