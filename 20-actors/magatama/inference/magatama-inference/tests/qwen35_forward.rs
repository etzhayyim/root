...
///
/// Run: cargo test -p magatama-inference --test qwen35_forward --release -- --nocapture
...
async fn test_qwen35_single_deltanet_forward() {
    ...
    let config = magatama_inference::loader::load_config(model_dir)
        .expect("failed to load config");

    println!("Initializing inference engine...");
    let engine = magatama_inference::engine::InferenceEngine::new()
        .await
    ...
    let model = magatama_inference::loader::load_hybrid_model(model_dir)
        .expect("failed to load model");

    let block = match &model.blocks[0] {
        magatama_inference::model::HybridBlock::DeltaNet(dn) => dn,
    ...
    let result = magatama_inference::transformer::forward_deltanet_block(
...
async fn test_qwen35_single_attention_forward() {
    ...
    let block = match &model.blocks[3] {
        magatama_inference::model::HybridBlock::Attention(attn) => attn,
    ...
    let result = magatama_inference::transformer::forward_block(
        &engine, block, &config, &input,
    )
    .await
...
async fn test_qwen35_hybrid_4_blocks() {
    ...
    let blocks: Vec<&magatama_inference::model::HybridBlock> = model.blocks[..4].iter().collect();
    ...
    let result = magatama_inference::transformer::forward_hybrid_shard(
