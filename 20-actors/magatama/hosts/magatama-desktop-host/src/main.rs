...
use magatama_desktop_host::DesktopHostConfig;
use std::fs;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("magatama-desktop-host: {err:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    eprintln!("magatama-desktop-host: app={}", config.app_name);
    eprintln!("magatama-desktop-host: runtime={}", config.runtime_mode);
    eprintln!(
        "magatama-desktop-host: resources={}",
        resources_dir.display()
    );
    eprintln!(
        "magatama-desktop-host: guest={}",
        config.guest_relative_path
    );
    eprintln!("magatama-desktop-host: startup={url}");
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    if let Err(err) = handle_client(stream, &root, &config) {
                        eprintln!("magatama-desktop-host: request error: {err:#}");
                    }
                });
            }
            Err(err) => {
                eprintln!("magatama-desktop-host: accept error: {err}");
...
fn handle_client(
    ...
    if path == "/health" {
        return respond_json(
            &mut stream,
            200,
            r#"{"status":"ok","host":"magatama-desktop-host"}"#,
        );
    }
...
<body>
  <main>
    <section class="panel">
      <h1>{name}</h1>
      <p>Magatama desktop host is running.</p>
