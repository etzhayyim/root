...
fn main() {
    ...
    let mut inference_bin = "magatama-inference".to_string();
        ...
        match arg.as_str() {
            ...
            "--artifact-dir" => {
                let Some(path) = args.next() else {
                    eprintln!("magatama-kami-host: --artifact-dir requires a path");
                    std::process::exit(2);
                };
            ...
            "--verify-golden" => {
                let Some(path) = args.next() else {
                    eprintln!("magatama-kami-host: --verify-golden requires a path");
                    std::process::exit(2);
                };
            ...
            "--update-golden" => {
                let Some(path) = args.next() else {
                    eprintln!("magatama-kami-host: --update-golden requires a path");
                    std::process::exit(2);
                };
            ...
            "--min-uiux-score" => {
                let Some(value) = args.next() else {
                    eprintln!("magatama-kami-host: --min-uiux-score requires a number");
                    std::process::exit(2);
                };
                min_uiux_score = value.parse().unwrap_or_else(|_| {
                    eprintln!("magatama-kami-host: invalid --min-uiux-score {}", value);
                    std::process::exit(2);
                });
            }
            "--listen" => {
                let Some(addr) = args.next() else {
                    eprintln!("magatama-kami-host: --listen requires an address");
                    std::process::exit(2);
                };
                listen = addr;
            }
            "--model" => {
                let Some(id) = args.next() else {
                    eprintln!("magatama-kami-host: --model requires a model id");
                    std::process::exit(2);
                };
                model = id;
            }
            "--guest" => {
                let Some(path) = args.next() else {
                    eprintln!("magatama-kami-host: --guest requires a path");
                    std::process::exit(2);
                };
            ...
            "--inference-bin" => {
                let Some(path) = args.next() else {
                    eprintln!("magatama-kami-host: --inference-bin requires a path");
                    std::process::exit(2);
                };
                inference_bin = path;
            }
            "--help" | "-h" => {
                eprintln!("usage: magatama-kami-host [--headless] [--artifact-dir PATH]");
                eprintln!(
                    "       magatama-kami-host --serve --model MODEL [--guest PATH] [--listen ADDR] [--inference-bin PATH]"
                );
                eprintln!(
                    "       magatama-kami-host --verify-golden DIR [--artifact-dir PATH]"
                );
                eprintln!(
                    "       magatama-kami-host --update-golden DIR [--artifact-dir PATH]"
                );
                ...
                eprintln!("  --serve: run local Magatama host with OpenAI-compatible endpoints");
                std::process::exit(0);
            }
            other => {
                eprintln!("magatama-kami-host: unknown arg {other}");
                std::process::exit(2);
            }
        }
    }
    ...
    let result = if serve {
        if model.is_empty() {
            eprintln!("magatama-kami-host: --serve requires --model");
            std::process::exit(2);
        }
        ...
        let dir = artifact_dir.unwrap_or_else(|| {
            magatama_kami_host::runtime::default_artifact_dir("kami-golden-verify")
                .expect("default artifact dir")
        });
        magatama_kami_host::runtime::verify_sample_headless_golden(
            &golden_dir,
            &dir,
            min_uiux_score,
        )
        ...
        let dir = artifact_dir.unwrap_or_else(|| {
            magatama_kami_host::runtime::default_artifact_dir("kami-golden-update")
                .expect("default artifact dir")
        });
        magatama_kami_host::runtime::update_sample_headless_golden(&golden_dir, &dir)
    } else if headless {
        let dir = match artifact_dir {
            Some(dir) => dir,
            None => match magatama_kami_host::runtime::default_artifact_dir("kami-headless") {
                Ok(dir) => dir,
                Err(err) => {
                    eprintln!("magatama-kami-host: {err:#}");
                    std::process::exit(1);
                }
            },
        };
        match magatama_kami_host::runtime::run_sample_headless(&dir) {
            Ok(path) => {
                eprintln!("magatama-kami-host headless transcript: {}", path.display());
                Ok(())
            }
            Err(err) => Err(err),
        }
...
fn serve_local(
    ...
    let probe = Command::new(inference_bin)
        .arg("--probe")
        .env("MAGATAMA_INFERENCE_BACKEND", "wgpu")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()?;
    if !probe.status.success() {
        anyhow::bail!(
            "magatama-inference probe failed: {}",
            String::from_utf8_lossy(&probe.stderr).trim()
        );
    }

    let listener = std::net::TcpListener::bind(listen)?;
    eprintln!("magatama-kami-host: serve");
    eprintln!("magatama-kami-host: model={model}");
    eprintln!(
        "magatama-kami-host: guest={}",
        guest
            .map(|p| p.display().to_string())
            .unwrap_or_else(|| "-".to_string())
    );
    eprintln!("magatama-kami-host: backend=webgpu");
    eprintln!("magatama-kami-host: listen={listen}");

    loop {
        let (mut stream, _) = listener.accept()?;
        if let Err(err) = handle_client(&mut stream, model, guest, inference_bin) {
            eprintln!("magatama-kami-host: request error: {err:#}");
        }
    }
}
