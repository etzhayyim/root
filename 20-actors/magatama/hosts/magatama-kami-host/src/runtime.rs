        ...
        Event::WindowEvent { event, window_id } if window_id == window.id() => match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                if let Ok(path) = default_screenshot_path("kami-host") {
                    match state.capture_screenshot_png(&path) {
                        Ok(artifact) => {
                            eprintln!("magatama-kami-host screenshot: {}", artifact.path)
                        }
                        Err(err) => eprintln!("magatama-kami-host screenshot error: {err:#}"),
                    }
                }
            }
            WindowEvent::KeyboardInput {
            ...
            WindowEvent::KeyboardInput {
                    ...
                    .run_automation_plan(&kami_devtools::sample_diskcleaner_plan(), &dir)
                {
                    Ok(transcript) => match write_transcript_json(&transcript, &dir) {
                        Ok(path) => {
                            eprintln!(
                                "magatama-kami-host automation transcript: {}",
                                path.display()
                            )
                        }
                        Err(err) => {
                            eprintln!("magatama-kami-host transcript write error: {err:#}")
                        }
                    },
                    Err(err) => eprintln!("magatama-kami-host automation error: {err:#}"),
                },
                Err(err) => eprintln!("magatama-kami-host artifact dir error: {err:#}"),
            },
            ...
            WindowEvent::RedrawRequested => {
                if let Err(err) = state.render() {
                    eprintln!("magatama-kami-host render error: {err:#}");
                    elwt.exit();
                }
            }
            _ => {}
...
pub fn run_sample_headless(artifact_dir: &Path) -> Result<PathBuf> {
    ...
    eprintln!(
        "magatama-kami-host headless uiux report: {}",
        report_path.display()
    );
    Ok(transcript_path)
}
...
impl RenderState {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("magatama-kami-host-device"),
    ...
    pub async fn new_headless(frame: KamiFrameModel) -> Result<Self> {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("magatama-kami-host-headless-device"),
                    required_features: wgpu::Features::empty(),
                    required_limits: wgpu::Limits::default(),
                    memory_hints: wgpu::MemoryHints::Performance,
                },
