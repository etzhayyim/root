"""AratameFindingEmitCell — aratame R0 Pregel cell.
...
Purpose: emit tsukuroi-compatible com.etzhayyim.aratame.vulnFinding plus an aggregate inspectionReport to kotoba datom + MST; remediation handoff to tsukuroi (propose-only).
...
gemma4-26b-a4b via judah LiteLLM 127.0.0.1:4000). Gates: G2 (append-only datom) + G13 (propose-only handoff).
Output Lexicon(s): com.etzhayyim.aratame.inspectionReport.
...
#
# class AratameFindingEmitCell(PregelCell):
#     process_step = "aratame_finding_emit"
#     pregel_tier = "B"
...
__all__ = ["AratameFindingEmitCell"]
