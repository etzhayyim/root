"""TsukuroiFindingIntakeCell — tsukuroi R0 Pregel cell.
...
Purpose: consume an owner-attested akuma vulnerability finding within an active RemediationMandate; reject any input lacking a resolvable finding_cid on the same owner+target.
...
#   2. akuma finding feed DID is registered (the only permitted vulnerability input source; G3 NO-PROBING)
#
# Any None below → import-time RuntimeError.

COUNCIL_CHARTER_ATTESTATION_TX_HASH: str | None = None
AKUMA_FINDING_FEED_DID: str | None = None

if (
    COUNCIL_CHARTER_ATTESTATION_TX_HASH is None
    or AKUMA_FINDING_FEED_DID is None
):
    raise RuntimeError(
        "tsukuroi R0 scaffold: activate via Council ADR-2605291500 "
        "post-ratification — Council has not attested the tsukuroi master "
        "charter (Lv6+ ≥3), and/or AKUMA_FINDING_FEED_DID is unset (akuma finding feed DID is registered (the only permitted vulnerability input source; G3 NO-PROBING)). "
...
#
# class TsukuroiFindingIntakeCell(PregelCell):
#     process_step = "tsukuroi_finding_intake"
#     pregel_tier = "B"
...
__all__ = ["TsukuroiFindingIntakeCell"]
