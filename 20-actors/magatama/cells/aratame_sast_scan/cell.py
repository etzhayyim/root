"""AratameSastScanCell — aratame R0 Pregel cell.
...
Purpose: run OSS SAST (Semgrep OSS / CodeQL OSS terms) over the cloned source and emit candidate SAST vulnFindings.
...
gemma4-26b-a4b via judah LiteLLM 127.0.0.1:4000). Gates: G3 (read-only) + G5 (OSS-tooling-only) + G9 (defensive-only).
...
#
# class AratameSastScanCell(PregelCell):
#     process_step = "aratame_sast_scan"
#     pregel_tier = "B"
