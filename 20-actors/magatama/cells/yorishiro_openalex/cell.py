"""
Yorishiro: openalex (kami: api.openalex.org)
...
`yorishiro regen openalex` — extend the kami OpenAPI spec instead.
"""
...
YORISHIRO_NAME = "openalex"
YORISHIRO_KAMI = "api.openalex.org"
YORISHIRO_BASE_URL = "https://api.openalex.org"
YORISHIRO_PURPOSES = tuple(["grant","kisha"])
USER_AGENT = f"etzhayyim-yorishiro-{YORISHIRO_NAME}/0.1"


class OpenalexState(TypedDict, total=False):
    # routing
    op: str
...
def search_works_node(state: dict[str, Any]) -> dict[str, Any]:
    """Full-text + structured search over the OpenAlex works index."""
...
def search_authors_node(state: dict[str, Any]) -> dict[str, Any]:
    """Free-text search over the OpenAlex authors index."""
    params = dict(state.get("params") or {})
    body = state.get("body") or None
    path = "/authors"
    for key in list(params.keys()):
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None):
    g = StateGraph(OpenalexState)
    g.add_node("searchWorks", search_works_node)
    g.add_node("searchAuthors", search_authors_node)

    def _router(state: OpenalexState) -> str:
        op = state.get("op") or "searchWorks"
        return op if op in {"searchWorks", "searchAuthors"} else "searchWorks"
...
def state_from_event(event: dict[str, Any]) -> OpenalexState:
