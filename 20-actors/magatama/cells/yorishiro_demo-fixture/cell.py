"""
Yorishiro: demo-fixture (kami: bin:demo-fixture)
...
`yorishiro regen demo-fixture`. Extend the kami manifest instead.
"""
...
YORISHIRO_NAME = "demo-fixture"
YORISHIRO_KAMI = "bin:demo-fixture"
YORISHIRO_BINARY = "demo-fixture"
YORISHIRO_PURPOSES = tuple(["grant"])


class DemoFixtureState(TypedDict, total=False):
    op: str
    args: dict[str, Any]
    exit_code: int
    stdout: str
...
def head_node(state: dict[str, Any]) -> dict[str, Any]:
    """Read up to MAX_LINES from INPUT_PATH and write to OUTPUT_PATH."""
    args = dict(state.get("args") or {})
    argv: list[str] = [YORISHIRO_BINARY]
    if args.get("max_lines") is not None and args.get("max_lines") != "":
        argv.append("--max-lines")
        argv.append(str(args["max_lines"]))
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None):
    g = StateGraph(DemoFixtureState)
    g.add_node("greet", greet_node)
    g.add_node("head", head_node)

    def _router(state: DemoFixtureState) -> str:
        op = state.get("op") or "greet"
        return op if op in {"greet", "head"} else "greet"
...
def state_from_event(event: dict[str, Any]) -> DemoFixtureState:
    return {
        "op": event.get("op", "greet"),
