"""AratameCharterRiderScanCell — aratame R0 Pregel cell.
...
Purpose: run the Charter Rider section 2(a)..(h) scan on every emitted text and reject any exploit/PoC content.
...
gemma4-26b-a4b via judah LiteLLM 127.0.0.1:4000). Gates: G1 (Charter Rider scan) + G9 (defensive-only / no exploit).
Output Lexicon(s): (gate cell — no new lexicon).
...
#
# class AratameCharterRiderScanCell(PregelCell):
#     process_step = "aratame_charter_rider_scan"
#     pregel_tier = "B"
...
__all__ = ["AratameCharterRiderScanCell"]
