"""TadoriTxTraceCell - tadori R0 Pregel cell.
...
Purpose: trace a target address (wallet deep-inspect) into tx/addr/cluster datoms; delegates the paginated fetch + classification compute to the malak wallet_deep_inspect_pursuit Pregel (ADR-2605152000).
...
+ kotoba-only store (ADR-2605262130). Gates: G9 (Murakumo-only) + G11 (kotoba-only).
Output Lexicon(s): app.etzhayyim.tadori.traceReport.
...
#   2. the malak wallet_deep_inspect_pursuit compute feed DID is registered (the durable trace report is persisted to kotoba by tadori, computed by malak).
#
# Any None below -> import-time RuntimeError.

COUNCIL_CHARTER_ATTESTATION_TX_HASH: str | None = None
MALAK_WALLET_PURSUIT_FEED_DID: str | None = None

if (
    COUNCIL_CHARTER_ATTESTATION_TX_HASH is None
    or MALAK_WALLET_PURSUIT_FEED_DID is None
):
    raise RuntimeError(
        "tadori R0 scaffold: activate via Council ADR-2605301400 "
        "post-ratification - Council has not attested the tadori master "
        "charter (Lv6+ >=3), and/or MALAK_WALLET_PURSUIT_FEED_DID is unset (the malak wallet_deep_inspect_pursuit compute feed DID is registered (the durable trace report is persisted to kotoba by tadori, computed by malak)). "
...
#
# class TadoriTxTraceCell(PregelCell):
#     process_step = "tadori_tx_trace"
#     pregel_tier = "B"
#     murakumo_node = "levi"
#
