"""
Yorishiro: huggingface-inference (kami: api-inference.huggingface.co)
...
`yorishiro regen huggingface-inference` — extend the kami OpenAPI spec instead.
"""
...
YORISHIRO_NAME = "huggingface-inference"
YORISHIRO_KAMI = "api-inference.huggingface.co"
YORISHIRO_BASE_URL = "https://api-inference.huggingface.co"
YORISHIRO_PURPOSES = tuple(["grant"])
USER_AGENT = f"etzhayyim-yorishiro-{YORISHIRO_NAME}/0.1"


class HuggingfaceInferenceState(TypedDict, total=False):
    # routing
    op: str
...
def extract_features_node(state: dict[str, Any]) -> dict[str, Any]:
    """POST text to the configured model and receive its hidden-state vector representation. Used by mst-projector embedders. The API key is supplied via Authorization: Bearer <key> by the caller and is NOT part of this lexicon — yorishiri never store credentials."""
    params = dict(state.get("params") or {})
    body = state.get("body") or None
    path = "/pipeline/feature-extraction/{model_id}"
    for key in list(params.keys()):
        token = "{" + key + "}"
        if token in path:
            path = path.replace(token, str(params.pop(key)))
    url = f"{YORISHIRO_BASE_URL}{path}"
    status, text = _http_call("POST", url, params, body if isinstance(body, dict) and body else None)
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None):
    g = StateGraph(HuggingfaceInferenceState)
    g.add_node("extractFeatures", extract_features_node)

    def _router(state: HuggingfaceInferenceState) -> str:
        op = state.get("op") or "extractFeatures"
        return op if op in {"extractFeatures"} else "extractFeatures"

    g.add_conditional_edges(START, _router, {
        "extractFeatures": "extractFeatures",
    })
    g.add_edge("extractFeatures", END)
...
def state_from_event(event: dict[str, Any]) -> HuggingfaceInferenceState:
    """Map an MST / XRPC event payload into the cell's TypedDict state."""
    return {
        "op": event.get("op", "extractFeatures"),
