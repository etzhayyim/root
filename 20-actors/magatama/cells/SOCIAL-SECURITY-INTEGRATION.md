...
config (MCP `magatama.jsonld`, openmail signer, projection target) is R1 work, gated
...
- **Substrate**: magatama MCP tool facade (ADR-0087); facade actors live under `20-actors/magatama/mcp/<name>/magatama.jsonld` and surface tools at `mcp.etzhayyim.com`.
- **Wire (R1, deployable artifact — NOT created at R0)**: a `magatama.jsonld` for an MCP facade exposing:
  ...
  - Target shape (R1):
    ...
    ```
- **Inert at R0**: `MCP_FACADE_REGISTRY_DID = None` + `COUNCIL_SS_PIPELINE_RATIFY_TX_HASH = None` → cell raises at import; no facade `magatama.jsonld` exists yet, so no deployable MCP surface.
...
| MCP | create `20-actors/magatama/mcp/social-security-mcp/magatama.jsonld` + set `MCP_FACADE_REGISTRY_DID` + `COUNCIL_SS_PIPELINE_RATIFY_TX_HASH` (Lv6+) | `mcp_facade` import gate (read tools only; write still vow-gated) |
