"""TsukuroiSilenReviewCell — tsukuroi R0 Pregel cell.
...
Purpose: quarterly Council audit emitting structural zero-counters (autonomousMergeCount / exploitArtifactCount / outOfScopeWriteCount / platformHeldKeyCount); any nonzero ⇒ cell halt + chigiri.disputeMediation.
...
inference (ADR-2605215000). Gates: G13 (silen review + Bonsai seed-tier prune on violation).
Output Lexicon(s): com.etzhayyim.tsukuroi.silenTsukuroiReview.
...
#   2. the quarterly silen-tsukuroi-review schedule CID is registered (G13 audit + Bonsai pruning hook)
#
# Any None below → import-time RuntimeError.

COUNCIL_CHARTER_ATTESTATION_TX_HASH: str | None = None
SILEN_TSUKUROI_REVIEW_SCHEDULE_CID: str | None = None

if (
    COUNCIL_CHARTER_ATTESTATION_TX_HASH is None
    or SILEN_TSUKUROI_REVIEW_SCHEDULE_CID is None
):
    raise RuntimeError(
        "tsukuroi R0 scaffold: activate via Council ADR-2605291500 "
        "post-ratification — Council has not attested the tsukuroi master "
        "charter (Lv6+ ≥3), and/or SILEN_TSUKUROI_REVIEW_SCHEDULE_CID is unset (the quarterly silen-tsukuroi-review schedule CID is registered (G13 audit + Bonsai pruning hook)). "
...
#
# class TsukuroiSilenReviewCell(PregelCell):
#     process_step = "tsukuroi_silen_tsukuroi_review"
#     pregel_tier = "B"
...
__all__ = ["TsukuroiSilenReviewCell"]
