"""
Yorishiro: argparse-sub (kami: bin:argparse-sub)
...
`yorishiro regen argparse-sub`. Extend the kami manifest instead.
"""
...
YORISHIRO_NAME = "argparse-sub"
YORISHIRO_KAMI = "bin:argparse-sub"
YORISHIRO_BINARY = "argparse-sub"
YORISHIRO_PURPOSES = tuple(["grant"])


class ArgparseSubState(TypedDict, total=False):
    op: str
    args: dict[str, Any]
    exit_code: int
    stdout: str
...
def encode_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if args.get("output") is not None and args.get("output") != "":
        argv.append("--output")
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None):
    g = StateGraph(ArgparseSubState)
    g.add_node("encode", encode_node)
    g.add_node("decode", decode_node)
    g.add_node("inspect", inspect_node)

    def _router(state: ArgparseSubState) -> str:
        op = state.get("op") or "encode"
        return op if op in {"encode", "decode", "inspect"} else "encode"
...
def state_from_event(event: dict[str, Any]) -> ArgparseSubState:
    return {
