"""
Yorishiro: clap-demo (kami: bin:clap-demo)
...
`yorishiro regen clap-demo`. Extend the kami manifest instead.
"""
...
YORISHIRO_NAME = "clap-demo"
YORISHIRO_KAMI = "bin:clap-demo"
YORISHIRO_BINARY = "clap-demo"
YORISHIRO_PURPOSES = tuple(["grant"])


class ClapDemoState(TypedDict, total=False):
    op: str
    args: dict[str, Any]
    exit_code: int
    stdout: str
...
def clap_demo_node(state: dict[str, Any]) -> dict[str, Any]:
    """Longer description of the clap demo CLI for the source-repo extractor."""
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None):
    g = StateGraph(ClapDemoState)
    g.add_node("clap-demo", clap_demo_node)

    def _router(state: ClapDemoState) -> str:
        op = state.get("op") or "clap-demo"
        return op if op in {"clap-demo"} else "clap-demo"

    g.add_conditional_edges(START, _router, {
        "clap-demo": "clap-demo",
    })
    g.add_edge("clap-demo", END)
...
def state_from_event(event: dict[str, Any]) -> ClapDemoState:
    return {
        "op": event.get("op", "clap-demo"),
