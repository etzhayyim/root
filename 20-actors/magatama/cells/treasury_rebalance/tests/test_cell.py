...
def test_emit_at_record_carries_full_payload():
    ...
    rec = port.records[0]
    assert rec["collection"] == "ai.gftd.apps.payment.treasury-rebalance-proposal"
...
def test_emit_skip_record_carries_reason():
    ...
    assert port.records[0]["collection"] == "ai.gftd.apps.payment.treasury-rebalance-skip"
...
def test_graph_routes_to_correct_terminal(balances, expects_proposal):
    ...
    if expects_proposal:
        ...
        assert pds.records[0]["collection"] == "ai.gftd.apps.payment.treasury-rebalance-proposal"
    else:
        assert final["needs_rebalance"] is False
        assert pds.records[0]["collection"] == "ai.gftd.apps.payment.treasury-rebalance-skip"
