...
|---|---|
| `ai.gftd.apps.payment.treasury-rebalance-proposal` | drift > threshold; carries `proposalId`, observed/target/drift bps, `descCid` |
| `ai.gftd.apps.payment.treasury-rebalance-skip` | within band or empty treasury; carries `reason` |
...
uv run pytest src/pymagatama/../../cells/treasury_rebalance/tests -v
```
...
```bash
magatama cell deploy --cell TreasuryRebalanceCell --schedule "0 0 1 * *"
magatama cell health --cell TreasuryRebalanceCell
```
