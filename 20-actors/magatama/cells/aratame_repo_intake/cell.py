"""AratameRepoIntakeCell — aratame R0 Pregel cell.
...
Purpose: validate the InspectionMandate (owner+authority dual-sig) and shallow READ-ONLY clone the attested repo@ref into the egress-restricted aratame-scan sandbox.
...
gemma4-26b-a4b via judah LiteLLM 127.0.0.1:4000). Gates: G3 (read-only/static-only) + G4 (dual-sig mandate) + G6 (no platform-held key) + G11 (egress-restricted sandbox).
Output Lexicon(s): com.etzhayyim.aratame.inspectionMandate.
...
#
# class AratameRepoIntakeCell(PregelCell):
#     process_step = "aratame_repo_intake"
#     pregel_tier = "B"
