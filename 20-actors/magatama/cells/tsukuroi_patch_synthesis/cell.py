"""TsukuroiPatchSynthesisCell — tsukuroi R0 Pregel cell.
...
Purpose: draft a candidate DEFENSIVE fix diff for the finding via Murakumo-only LLM (judah LiteLLM 127.0.0.1:4000); never emit exploit/PoC code.
...
inference (ADR-2605215000). Gates: G5 (defensive-only) + G6 (scoped write) + G10 (Murakumo-only).
Output Lexicon(s): app.etzhayyim.tsukuroi.patchProposal.
...
#   2. the Murakumo judah LiteLLM endpoint is registered (G10 Murakumo-only inference; no vendor API)
#
...
if (
    ...
    raise RuntimeError(
        ...
        "charter (Lv6+ ≥3), and/or MURAKUMO_JUDAH_ENDPOINT is unset (the Murakumo judah LiteLLM endpoint is registered (G10 Murakumo-only inference; no vendor API)). "
...
#
# class TsukuroiPatchSynthesisCell(PregelCell):
#     process_step = "tsukuroi_patch_synthesis"
#     pregel_tier = "B"
...
__all__ = ["TsukuroiPatchSynthesisCell"]
