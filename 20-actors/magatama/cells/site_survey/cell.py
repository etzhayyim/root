...
# uses pymagatama.transport.libp2p to dial the shard peer instead of
...
def _ensure_libp2p_tunnel(shard: int) -> str | None:
    ...
    try:
        from pymagatama.transport import libp2p as _l
...
def emit_at_record(state: SiteSurveyState) -> dict[str, Any]:
    ...
    submission_uri = (
        f"at://{site_did}/ai.gftd.apps.etzhayyim.kuniUmi.submitSiteSurvey/"
...
def state_from_event(event: dict[str, Any]) -> SiteSurveyState:
    ...
    The submitSiteSurvey lexicon (ai.gftd.apps.etzhayyim.kuniUmi.submitSiteSurvey)
