...
from pymagatama.db_sync import execute, fetch_one, fetch_all
...
def normalize_semiconductor() -> dict[str, Any]:
    ...
    stats["socNodes"] = execute(
        ...
        SELECT
          ...
          CURRENT_DATE,
          COALESCE(sensitivity_ord, 1),
          COALESCE(owner_did, 'did:web:jukyu.gftd.ai'),
          'did:web:jukyu.gftd.ai',
          'semiconductor',
          chip_id,
          'soc',
...
def normalize_entity_vessel_transport(domain: str | None = None) -> dict[str, Any]:
    ...
    stats["transportLegsFromOilCargo"] = execute(
        ...
        SELECT
          ...
          COALESCE(c.owner_did, 'did:web:jukyu.gftd.ai'),
          COALESCE(c.repo, 'did:web:jukyu.gftd.ai'),
