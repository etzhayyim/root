...
Triggered by `ai.gftd.apps.shinka.tickActor` UDF (see handlers/shinka.py),
...
def _kyumei_gather(state: ShinkaState) -> ShinkaState:
    ...
    execute(
        ...
        """,
        (
            f"at://{did}/ai.gftd.apps.standard.shinkaKnowledge/{rkey}",
            did,
            rkey,
            did,
            did,
...
def _write_heartbeat(state: ShinkaState) -> ShinkaState:
    ...
    did = state["actor_did"]
    now = state["now_ms"]
    collection = f"ai.gftd.apps.standard.heartbeat"
...
def _emit_evolution(state: ShinkaState) -> ShinkaState:
    ...
    execute(
        ...
        """,
        (
            f"at://{did}/ai.gftd.apps.standard.shinkaEvolution/{rkey}",
            did,
            rkey,
            did,
            did,
