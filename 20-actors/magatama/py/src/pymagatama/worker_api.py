"""FastAPI surface for pymagatama Python worker pods.
...
Run:
    granian --interface asgi --host 0.0.0.0 --port 8081 pymagatama.worker_api:app
    python -m pymagatama.worker_api
"""
...
_COMPONENT = os.environ.get("WORKER_API_COMPONENT", "pymagatama-worker")
...
if Gauge is not None:
    WORKER_UP = Gauge(
        "pymagatama_worker_api_up",
        "Whether the pymagatama worker API process is running.",
        ["component", "runtime_kind"],
    )
    WORKER_UP.labels(component=_COMPONENT, runtime_kind=_RUNTIME_KIND).set(1)

app = FastAPI(
    title="pymagatama worker api",
    version="1",
    docs_url=None,
    redoc_url=None,
    openapi_url=None,
...
async def invoke(req: InvokeRequest) -> dict[str, Any]:
    from pymagatama.mcp_dispatch import build_default_handlers, handle_envelope
...
def metrics() -> Response:
    ...
    body = (
        "# HELP pymagatama_worker_api_up Whether the pymagatama worker API process is running.\n"
        "# TYPE pymagatama_worker_api_up gauge\n"
        f'pymagatama_worker_api_up{{component="{_COMPONENT}",runtime_kind="{_RUNTIME_KIND}"}} 1\n'
    )
...
async def ameno_subscribe_briefs(request: Request) -> StreamingResponse:
    ...
    """
    from pymagatama.ameno_handlers import subscribe_briefs_sse
