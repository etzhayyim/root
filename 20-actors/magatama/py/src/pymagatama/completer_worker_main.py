...
async def task_evaluate(
    ...
    try:
        await conn.execute(
            ...
            """,
            f"at://did:web:completer.gftd.ai/ai.gftd.apps.completer.audit/{rkey}",
            int(time.time() * 1000),
            now[:10],
            0,
        ...
        for f in findings:
            ...
            await conn.execute(
                ...
                """,
                f"at://did:web:completer.gftd.ai/ai.gftd.apps.completer.finding/{frkey}",
                int(time.time() * 1000),
                now[:10],
                0,
                "did:web:completer.gftd.ai",
        ...
        if parsed:
            ...
            try:
                await conn.execute(
                    ...
                    """,
                    f"at://did:web:completer.gftd.ai/ai.gftd.apps.completer.remediation/{rid}",
                    int(time.time() * 1000),
                    now[:10],
                    0,
                    "did:web:completer.gftd.ai",
