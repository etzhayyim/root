...
def _compile_topology(
    ...
    for ce in spec.get("conditional_edges", []) or []:
        ...
        if router_path:
            ...
            # `pymagatama.langgraph_node_resolvers`.
    ...
    for row in deploy_rows:
        ...
        try:
            ...
            if kind == "py_factory":
                ...
                # "pymagatama.kobo_worker_main:task_bud_agent"). State envelope
                # is the same SingleTaskState used by _single_task_wrapper.py.
                if not factory_path:
                    raise ValueError("single_task row missing factory_path")
                from pymagatama.langgraph_graphs._single_task_wrapper import (
