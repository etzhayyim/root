...
def _http_json(url: str, *, method: str = "GET", headers: dict[str, str] | None = None, body: bytes | None = None, timeout: int = 30) -> Any:
    req = urllib.request.Request(url, data=body, method=method, headers={"accept": "application/json", "user-agent": "gftd-calendar-zeebe/1", **(headers or {})})
...
def _base(collection: str, rkey: str, created: str | None = None) -> dict[str, Any]:
    ...
    return {
        "vertex_id": f"at://{ACTOR_DID}/ai.gftd.apps.calendar.{collection}/{rkey}",
        "created_date": created_at[:10],
        "sensitivity_ord": 100,
        "owner_did": ACTOR_DID,
        "rkey": rkey,
        "repo": ACTOR_DID,
        "collection": f"ai.gftd.apps.calendar.{collection}",
