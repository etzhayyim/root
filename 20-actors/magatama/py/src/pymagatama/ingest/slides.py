...
def _http_json(url: str, *, method: str = "GET", body: bytes | None = None, headers: dict[str, str] | None = None) -> dict[str, Any]:
    req = urllib.request.Request(url, method=method, data=body, headers={"accept": "application/json", "user-agent": "gftd-slides-zeebe/1", **(headers or {})})
...
def _presentation_row(token: dict[str, Any], pres: dict[str, Any], file_id: str, modified_time: str) -> dict[str, Any]:
    ...
    return {
        "vertex_id": f"at://{actor}/ai.gftd.apps.slides.presentation/{presentation_id}",
...
def _slide_row(token: dict[str, Any], presentation_id: str, slide: dict[str, Any], slide_index: int) -> dict[str, Any]:
    ...
    return {
        "vertex_id": f"at://{actor}/ai.gftd.apps.slides.slide/{presentation_id}_{slide_object_id}",
        "_seq": int(time.time() * 1000),
        "created_date": now[:10],
        "sensitivity_ord": 100,
        "owner_did": actor,
