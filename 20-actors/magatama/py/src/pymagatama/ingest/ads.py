...
def create_campaign(name: str = "", description: str = "", advertiser: str = "", budgetJpy: Any = 0, **_: Any) -> dict[str, Any]:
    ...
    record = {
        "$type": "ai.gftd.apps.ads.campaign",
    ...
    _execute(
        ...
        VALUES (%s,%s,%s,%s,'ai.gftd.apps.ads.campaign',%s,%s,%s,%s,%s,%s,true,%s,'anon','anon',%s,%s,'anon')
        ON CONFLICT (vertex_id) DO NOTHING""",
        (f"at://{ACTOR}/ai.gftd.apps.ads.campaign/{_rkey(campaign_id)}", ACTOR, campaign_id, ACTOR, campaign_id, did, name, description or "", advertiser or "", _int(budgetJpy), created_at, ACTOR, ACTOR),
    )
...
def post_sponsored(campaignId: str = "", text: str = "", embedUri: str = "", embedTitle: str = "", embedDesc: str = "", embedThumb: str = "", langs: list[str] | None = None, **_: Any) -> dict[str, Any]:
    ...
    _execute(
        ...
        VALUES (%s,%s,%s,%s,'ai.gftd.apps.ads.sponsoredPost',%s,%s,%s,%s,%s,'anon','anon',%s,%s,'anon')
        ON CONFLICT (vertex_id) DO NOTHING""",
        (f"at://{ACTOR}/ai.gftd.apps.ads.sponsoredPost/{_rkey(campaignId + '-' + created_at)}", ACTOR, _rkey(campaignId + "-" + created_at), ACTOR, campaignId, uri, cid, text, created_at, ACTOR, ACTOR),
    )
    if posted.get("error"):
