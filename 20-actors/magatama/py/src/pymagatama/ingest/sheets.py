...
def _http_json(url: str, *, method: str = "GET", body: bytes | None = None, headers: dict[str, str] | None = None) -> dict[str, Any]:
    req = urllib.request.Request(url, method=method, data=body, headers={"accept": "application/json", "user-agent": "gftd-sheets-zeebe/1", **(headers or {})})
...
def _spreadsheet_row(token: dict[str, Any], ss: dict[str, Any], file_id: str, modified_time: str) -> dict[str, Any]:
    ...
    return {
        "vertex_id": f"at://{actor}/ai.gftd.apps.sheets.spreadsheet/{spreadsheet_id}",
...
def _sheet_row(token: dict[str, Any], spreadsheet_id: str, sheet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "vertex_id": f"at://{actor}/ai.gftd.apps.sheets.sheet/{spreadsheet_id}_{sheet_id}",
        "_seq": int(time.time() * 1000),
        "created_date": now[:10],
        "sensitivity_ord": 100,
        "owner_did": actor,
