...
def _insert_spatial(entity: str, props: dict[str, Any], rec_id: str | None = None) -> dict[str, Any]:
    ...
    collection = f"ai.gftd.apps.maps.{entity}"
...
def register_region(
    ...
    **_: Any,
    ...
    canonical_did = f"at://{OWNER_DID}/ai.gftd.apps.maps.adminArea/adminArea:{region_id}"
    ...
    aliases = 0
    for scheme, code in code_map.items():
        alias_did = f"at://{OWNER_DID}/ai.gftd.apps.maps.geoAlias/geoAlias:{scheme}:{code}"
        _insert_spatial(
            "geoAlias",
            {
                "scheme": scheme,
