...
def _vertex(collection: str, rkey: str) -> str:
    return f"at://{ACTOR}/ai.gftd.apps.shiharai.{collection}/{_rkey(rkey)}"
...
def _bpmn_call(method: str, params: dict[str, Any]) -> dict[str, Any]:
    # ADR-2604282300: ai.gftd.* must NOT route through CF Workers — use K8s ClusterIP.
    base = (
    ...
    req = urllib.request.Request(
        f"{base}/xrpc/ai.gftd.apps.bpmn.{method}",
        method="POST",
