...
def quote_vid(owner_did: str, venue: str, symbol: str, ts: str) -> str:
    ...
    return f"at://{owner_did}/ai.gftd.apps.arb.quote/{safe}"


def proposal_vid(owner_did: str, proposal_id: str) -> str:
    return f"at://{owner_did}/ai.gftd.apps.arb.proposal/{proposal_id}"
...
def score_proposal(proposal_id: str, model: str = "heuristic-v1") -> dict[str, Any]:
    ...
    vid = f"at://{owner}/ai.gftd.apps.arb.score/{proposal_id}"
    created_at = now_iso()
...
def publish_proposal(proposal_id: str, mention_cohort: str = "trader.gftd.ai", disclaimer: str = DISCLAIMER) -> dict[str, Any]:
    ...
    vid = f"at://{owner}/ai.gftd.apps.arb.publication/{proposal_id}"
    created_at = now_iso()
