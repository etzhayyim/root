...
COLLECTION_TABLES = {
    ...
    "ai.gftd.apps.i18n.translationMemory": "vertex_i18n_translation_memory",
    "ai.gftd.apps.i18n.graphNode": "vertex_i18n_text_node",
    "ai.gftd.apps.i18n.creditJob": "vertex_i18n_credit_job",
}
EDGE_COLLECTIONS = {"ai.gftd.apps.i18n.graphEdge"}
...
def _write_graph_edge(cur: Any, payload: dict[str, Any], created_at: str) -> None:
    ...
    if not src or not dst:
        return
    src_vid = _vertex_id("ai.gftd.apps.i18n.graphNode", src)
    if label == "HAS_LANG":
        dst_vid = f"at://{OWNER_DID}/ai.gftd.apps.i18n.language/{dst}"
        _write_edge(cur, "edge_i18n_text_language", src_vid, dst_vid, "has_language", payload, created_at)
    else:
        dst_vid = _vertex_id("ai.gftd.apps.i18n.graphNode", dst)
    ...
    if kind == "projectTranslation":
        ...
        if project_id:
            _write_edge(
                ...
                _vertex_id("ai.gftd.apps.i18n.project", project_id),
...
def _find_tm(source_text: str, target_lang: str) -> dict[str, Any] | None:
    ...
    for item in reversed(_list_records("ai.gftd.apps.i18n.translationMemory")):
...
def _persist_graph_translation(source_text: str, source_lang: str, target_text: str, target_lang: str) -> None:
    ...
    _record("ai.gftd.apps.i18n.graphNode", "graphNode", {"nodeId": src_node_id, "label": "TranslationText", "kind": "source", "lang": source_lang, "val": source_text, "createdAt": ts}, src_node_id)
    _record("ai.gftd.apps.i18n.graphNode", "graphNode", {"nodeId": dst_node_id, "label": "TranslationText", "kind": "translation", "lang": target_lang, "val": target_text, "createdAt": ts}, dst_node_id)
    _record("ai.gftd.apps.i18n.graphEdge", "graphEdge", {"edgeId": f"edge-{source_hash}-{target_lang}", "label": "TRANSLATED_TO", "src": src_node_id, "dst": dst_node_id, "lang": target_lang, "createdAt": ts})
    _record("ai.gftd.apps.i18n.graphEdge", "graphEdge", {"edgeId": f"lang-{source_hash}-{target_lang}", "label": "HAS_LANG", "src": dst_node_id, "dst": f"lang-{target_lang}", "lang": target_lang, "createdAt": ts})
...
def _upsert_tm(source_text: str, source_lang: str, target_lang: str, target_text: str, source: str, quality_score: float, context: str = "") -> dict[str, Any]:
    ...
    _record("ai.gftd.apps.i18n.translationMemory", "translationMemory", tm, _s(tm["id"]))
...
def _get_project(project_id: str) -> dict[str, Any] | None:
    for item in reversed(_list_records("ai.gftd.apps.i18n.project")):
...
def _get_project_translation(project_id: str, lang: str) -> dict[str, str] | None:
    for item in reversed(_list_records("ai.gftd.apps.i18n.projectTranslation")):
...
def translate_batch(projectId: Any = None, targetLangs: Any = None, **_: Any) -> dict[str, Any]:
    ...
    for lang in langs:
        ...
        _record("ai.gftd.apps.i18n.projectTranslation", "projectTranslation", {"projectId": project_id, "lang": lang, "messages": translated, "updatedAt": now_iso()}, f"{project_id}:{lang}")
...
def get_translation_status(projectId: Any = None, **_: Any) -> dict[str, Any]:
    ...
    for item in _list_records("ai.gftd.apps.i18n.projectTranslation"):
        if _s(item.get("projectId")) != project_id:
            continue
