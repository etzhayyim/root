...
COLLECTION_TABLES = {
    ...
    "ai.gftd.apps.gamePlayUploader.uploadSession": "vertex_game_play_upload_session",
    "ai.gftd.apps.gamePlayUploader.upload": "vertex_game_play_upload",
    "ai.gftd.apps.gamePlayUploader.review": "vertex_game_play_review",
    "ai.gftd.apps.gamePlayUploader.reward": "vertex_game_play_reward",
}
...
def _write_related_edges(cur: Any, collection: str, kind: str, record_id: str, payload: dict[str, Any], created_at: str) -> None:
    ...
    if kind == "uploadSession":
        ...
        if participant:
            _write_edge(cur, "edge_game_play_participant_session", _vertex_id("ai.gftd.apps.gamePlayUploader.participant", participant), vertex_id, "created_upload_session", payload, created_at)
    elif kind == "upload":
        session_id = _s(payload.get("sessionId"))
        if session_id:
            _write_edge(cur, "edge_game_play_session_upload", _vertex_id("ai.gftd.apps.gamePlayUploader.uploadSession", session_id), vertex_id, "has_upload", payload, created_at)
    elif kind == "review":
        upload_id = _s(payload.get("uploadId"))
        if upload_id:
            _write_edge(cur, "edge_game_play_upload_review", _vertex_id("ai.gftd.apps.gamePlayUploader.upload", upload_id), vertex_id, "reviewed_by", payload, created_at)
    elif kind == "reward":
        upload_id = _s(payload.get("uploadId"))
        if upload_id:
            _write_edge(cur, "edge_game_play_upload_reward", _vertex_id("ai.gftd.apps.gamePlayUploader.upload", upload_id), vertex_id, "earned_reward", payload, created_at)
...
def register_participant(participantDid: Any = None, displayName: Any = None, ageBand: Any = "unknown", payoutHandle: Any = None, guardianConsentRef: Any = None, **_: Any) -> dict[str, Any]:
    ...
    _record("ai.gftd.apps.gamePlayUploader.participant", "participant", {
...
def create_upload_session(participantDid: Any = None, gameTitle: Any = None, platform: Any = None, durationSec: Any = 0, captureStartedAt: Any = None, **_: Any) -> dict[str, Any]:
    ...
    _record("ai.gftd.apps.gamePlayUploader.uploadSession", "uploadSession", session, session_id)
...
def record_gameplay_upload(sessionId: Any = None, objectUri: Any = None, durationSec: Any = 0, sha256: Any = None, metadata: Any = None, **_: Any) -> dict[str, Any]:
    ...
    _record("ai.gftd.apps.gamePlayUploader.upload", "upload", upload, upload_id)
...
def review_upload(uploadId: Any = None, decision: Any = None, reviewerDid: Any = None, notes: Any = None, qualityScore: Any = None, **_: Any) -> dict[str, Any]:
    ...
    uploads = _list("ai.gftd.apps.gamePlayUploader.upload", "AND upload_id=%s", (upload_id,), 1)
    ...
    _record("ai.gftd.apps.gamePlayUploader.review", "review", review, review["reviewId"])
    if reward > 0:
        _record("ai.gftd.apps.gamePlayUploader.reward", "reward", {"uploadId": upload_id, "rewardJpy": reward, "status": "estimated"})
...
def get_campaign_status(participantDid: Any = None, **_: Any) -> dict[str, Any]:
    ...
    participants = _list("ai.gftd.apps.gamePlayUploader.participant", where, params)
    uploads = _list("ai.gftd.apps.gamePlayUploader.upload")
    reviews = _list("ai.gftd.apps.gamePlayUploader.review")
    ...
    return {
        "participants": len(participants) if participant_did else len(_list("ai.gftd.apps.gamePlayUploader.participant")),
        "uploads": len(uploads),
