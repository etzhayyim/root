...
from pymagatama.voxelforge.state import (
    Artifact,
    DesignKind,
    GenerateInput,
    TargetFormat,
...
def generate_trellis(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.runpod_client import call_trellis
...
def generate_comfy3d(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.runpod_client import call_comfyui_3d
...
def exec_cadquery(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.converters import cadquery_to_glb
...
def post_process_mesh(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.converters import decimate_glb
...
def voxelize(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.converters import glb_to_voxel_grid_json
...
def export_artifacts(state: VoxelforgeState) -> dict[str, Any]:
    from pymagatama.voxelforge.converters import (
        b2_put,
    ...
    if target in (TargetFormat.GLB, TargetFormat.BOTH) and state.mesh_b64:
        ...
        out.append(
            Artifact(
                vertex_id=f"at://{state.actor_did}/ai.gftd.apps.voxelforge.artifact/{sha[:16]}-glb",
    ...
    if target in (TargetFormat.VOX, TargetFormat.BOTH) and state.voxel_grid_b64:
        ...
        out.append(
            Artifact(
                vertex_id=f"at://{state.actor_did}/ai.gftd.apps.voxelforge.artifact/{sha_g[:16]}-grid",
                design_vertex_id=design_id,
                run_vertex_id=run_id,
        ...
        out.append(
            Artifact(
                vertex_id=f"at://{state.actor_did}/ai.gftd.apps.voxelforge.artifact/{sha_v[:16]}-vox",
                design_vertex_id=design_id,
                run_vertex_id=run_id,
                format="vox",
                b2_bucket=bucket,
    ...
    out.append(
        Artifact(
            vertex_id=f"at://{state.actor_did}/ai.gftd.apps.voxelforge.artifact/{sha_m[:16]}-mf",
            design_vertex_id=design_id,
            run_vertex_id=run_id,
...
def register_artifact(state: VoxelforgeState) -> dict[str, Any]:
    ...
    from pymagatama.voxelforge.converters import register_artifacts_to_rw
