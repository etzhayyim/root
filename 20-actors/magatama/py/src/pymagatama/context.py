...
Two handlers (`contracts.py`, `houbun.py`) reference `pymagatama.context.Context`
...
class _UnwiredDb:
    async def fetchrow(self, *_args: Any, **_kw: Any) -> None:
        raise NotImplementedError(
            "pymagatama.context.Context.db is not wired yet. "
...
class Context:
    ...
    def __init__(self, nsid: str) -> None:
        self.nsid = nsid
        self.db = _UnwiredDb()
        self._logger = logging.getLogger(f"pymagatama.ctx.{nsid}")
