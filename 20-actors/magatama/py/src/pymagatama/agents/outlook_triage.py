...
from pymagatama.agents.gmail_triage import (
    ALLOWLIST_DOMAINS,
    SCORE_GRAY_LOW,
    SCORE_SPAM,
    SCORE_TRASH,
...
async def _node_t1(state: OutlookTriageState) -> OutlookTriageState:
    ...
    for row in state.get("claimed") or []:
        ...
        if _allowlisted(from_addr):
            ...
            try:
                from pymagatama.agents.outlook_feedback import apply_sender_prior as _asp
...
async def _node_invoke_pregel(state: OutlookTriageState) -> OutlookTriageState:
    ...
    if not clean_rows:
        return state
    try:
        from pymagatama.pregel.graph import build_graph as _pregel_build
