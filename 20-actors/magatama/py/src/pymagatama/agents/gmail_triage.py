...
Task surface: invoked from ``pymagatama.ingest.gmail.triage`` (the BPMN
...
async def _node_register_yabai(state: GmailTriageState) -> GmailTriageState:
    ...
    with sync_cursor() as cur:
        for row in rows:
            ...
            entity_vid = f"at://{ACTOR_YABAI}/ai.gftd.apps.yabai.entity/{entity_id}"
            ...
            evidence_vid = f"at://{ACTOR_YABAI}/ai.gftd.apps.yabai.evidence/{evidence_id}"
