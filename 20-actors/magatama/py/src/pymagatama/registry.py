...
in-process table. On worker boot, `pymagatama.server.serve()` reads this
...
- Handlers are stateless per call. State lives in RisingWave via
  `pymagatama.db` (mirrors ADR-0036 Worker-direct Hyperdrive).
...
def udf(
    ...
    *,
    ...
    def decorator(fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        passthrough.__pymagatama_nsid__ = nsid  # type: ignore[attr-defined]
        passthrough.__pymagatama_entry__ = _HANDLERS[nsid]  # type: ignore[attr-defined]
        return passthrough
