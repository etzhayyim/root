...
Mirrors ``pymagatama.langgraph_graphs.unispsc_agents`` ``dynamic_runner``:
...
AGENT_PACKAGE = "pymagatama.phenotype_agents"
...
def load_agent(did: str) -> Optional[Any]:
    ...
    :mod:`pymagatama.eligibility.cell` when ``None`` is returned.
    """
    try:
