...
Run as a subprocess by `magatama-cell-runner` on a Murakumo Mac mini.
...
Lifecycle:
    1. Parse args
    2. Import cell module (20-actors/magatama/cells/<name>/cell.py)
...
_CELL_DIR_OVERRIDES: dict[str, tuple[Path, str]] = {
    "PhenotypeAgent": (MAGATAMA_CELLS_DIR, "phenotype_agent"),
}
...
def _cell_dir(cell_name: str) -> Path:
    ...
    kuni-umi cells live under 20-actors/kuni-umi/cells/ (ADR-2605201400 §1).
    Religious-corp cells live under 20-actors/magatama/cells/ (ADR-2605192415).
    """
    ...
    return MAGATAMA_CELLS_DIR / snake
...
def import_cell_module(cell_name: str) -> Any:
    ...
    module_name = f"pymagatama_cell_{cell_dir.name}"
...
def _build_checkpointer(cell_name: str):
    ...
    if sidecar_socket:
        try:
            from pymagatama.checkpointer import MstCheckpointSaver  # type: ignore
...
def run_mst_listener_loop(
    ...
    # TODO: wire pymagatama.listener.MstListener once it's wired into PDS subscribeRepos
