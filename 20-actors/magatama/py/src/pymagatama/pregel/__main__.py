...
Usage:
  python -m pymagatama.pregel               # smoke test (default)
  python -m pymagatama.pregel poll          # one-shot intent-analysis poll
  python -m pymagatama.pregel poll --loop   # continuous intent-analysis poll (60s)
  python -m pymagatama.pregel poll --since 2026-05-13T00:00:00Z
  python -m pymagatama.pregel triage        # one-shot: classify + RW ingest + folder move
  python -m pymagatama.pregel triage --loop --interval 300  # continuous (5min cycle)
  python -m pymagatama.pregel triage --dry-run              # classify + RW only, no moves
...
# ── Entry point ───────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(prog="python -m pymagatama.pregel")
