...
Subscribes to 3 Zeebe job types:
  ...
  webmk.create_ad_campaign  — XRPC call to ads.gftd.ai createCampaign
...
Env:
  ...
  RESEND_FROM         — sender address (default webmk@gftd.ai)
...
RESEND_FROM = os.environ.get("RESEND_FROM", "webmk@gftd.ai")
    ...
    def _run() -> None:
        with sync_cursor() as cur:
            cur.execute(
                ...
                """,
                (
                    f"at://{WEBMK_DID}/ai.gftd.apps.webmk.proposal/{state['proposal_id']}",
                    state["proposal_id"],
                    WEBMK_DID,
                    "proposal",
                    "generated",
...
<hr>
<p style="color:#888;font-size:12px">Generated by GFTD Web Marketing Agent · <a href="https://webmk.gftd.ai">webmk.gftd.ai</a></p>
"""
...
async def task_create_ad_campaign(
    ...
    **_: Any,
    ...
    """Call ads.gftd.ai createCampaign and link to proposal."""
    async with httpx.AsyncClient() as client:
        resp = await client.post(
    ...
    def _link() -> None:
        with sync_cursor() as cur:
            cur.execute(
                ...
                (
                    edge_id,
                    f"at://{WEBMK_DID}/ai.gftd.apps.webmk.proposal/{proposalId}",
                    f"at://{ADS_DID}/ai.gftd.apps.ads.campaign/{campaign_id}",
                    "has_campaign",
                    proposalId,
                    campaign_id,
                    campaign_did,
