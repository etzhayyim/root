"""pymagatama.substrate — Python-side seam for the etzhayyim RW-free substrate.
...
Substrate boundary:
  * Allowed substrate clients: ``httpx`` (this module). PDS HTTP endpoint
    is the canonical surface; pymagatama.substrate is the documented Python
...
Auth modes:
  1. **session_jwt**: User-side. Standard AT Protocol Bearer JWT obtained
     ...
     (pymagatama does not currently auto-refresh).
  2. **internal_token**: Service-to-service. Sends ``x-magatama-verified``
     header against magatama-host-sdk's internal trust path. Used by
...
class SubstrateError(Exception):
    """Base error for pymagatama.substrate operations."""
...
class Etzhayyim:
    ...
    def __init__(
        ...
        *,
        ...
        self._internal_token = internal_token or os.environ.get("MAGATAMA_INTERNAL_TOKEN")
    ...
    async def verify(self, uri: str) -> VerifyResult:
        ...
        return VerifyResult(
            uri=uri,
            included=False,
            error="verify() not yet implemented in pymagatama.substrate scaffold (parity with TS SDK 0.1.0-alpha)",
        )
    ...
    def _auth_headers(self) -> dict[str, str]:
        ...
        if self._internal_token:
            headers["x-magatama-verified"] = self._internal_token
        return headers
