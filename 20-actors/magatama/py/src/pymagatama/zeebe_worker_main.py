...
Subscribes to the four `ai.gftd.devstral.*` job types that map 1:1 to the
...
which engine implements them):
  ...
  ai.gftd.devstral.chat        → llm.call_tier (free-form chat)
  ai.gftd.devstral.classifyT3  → classify_t3 phishing JSON
  ai.gftd.devstral.translate   → news_translate semantics
  ai.gftd.devstral.storyboard  → mangaka_storyboard_from_prompt semantics
...
Env:
  ...
  VULTR_SERVERLESS_KEY — required by pymagatama.llm
"""
...
async def task_llm_knowledge_retrieve(
    ...
    from pymagatama.primitives import llm_knowledge
...
# ─── magatama.shinka.* tasks (Phase Z-α, BPMN heartbeat migration) ──────
#
# Each task wraps one node from pymagatama.shinka.__init__ (the existing
...
# pymagatama.shinka guarantees the BPMN run produces bit-identical writes
...
async def task_shinka_load_and_resolve(actorDid: str = "") -> dict:
    """Calls pymagatama.shinka._load_state + _resolve_cadence in one shot.
    Returns the merged state so the XOR gateway can branch on `shouldPost`."""
    if not actorDid:
        return {"error": "actorDid is required"}
    from pymagatama.shinka import _load_state, _resolve_cadence
...
async def task_shinka_write_heartbeat(actorDid: str = "", mood: str = "neutral",
    ...
    if not actorDid:
        return {"error": "actorDid is required"}
    from pymagatama.shinka import _write_heartbeat
    state = {
        "actor_did": actorDid,
        "now_ms": _now_ms(),
        "mood": mood,
...
async def task_shinka_emit_evolution(actorDid: str = "", mood: str = "neutral",
    ...
    if not actorDid:
        return {"error": "actorDid is required"}
    from pymagatama.shinka import _emit_evolution
    state = {
        "actor_did": actorDid,
        "now_ms": _now_ms(),
        "mood": mood,
...
#   - generic.llm.* uses the same pymagatama.llm path that already
...
# ai.gftd.* XRPC calls route to bpmn-dispatcher K8s ClusterIP directly.
...
async def _pds_dispatch_internal_xrpc(type: str, payload: dict, started: float) -> dict:
    """Route ai.gftd.* XRPC to bpmn-dispatcher K8s ClusterIP (no CF edge hop)."""
...
async def task_generic_pds_dispatch(type: str = "", payload: dict | None = None,
    ...
    - ai.gftd.* → internal K8s bpmn-dispatcher POST with x-internal-trust.
    ...
    if type.startswith(_GFTD_APP_NSID_PREFIX) and _BPMN_DISPATCHER_INTERNAL_URL:
...
_PD_COLOR_XRPC_NSIDS = {
    "ai.gftd.apps.storage.resolveSourceAsset",
    "ai.gftd.apps.copyright.license.inspect",
    "ai.gftd.apps.i18n.translateBatch",
}


async def _pd_color_add_json_manifest(kind: str, payload: dict) -> str:
    from pymagatama.primitives import ipfs_ingest  # noqa: PLC0415

    body = json.dumps(
        {
            "kind": kind,
            "schema": "ai.gftd.apps.publicDomainColorization.manifest.v1",
...
async def _pd_color_local_xrpc(nsid: str, payload: dict) -> dict:
    ...
    """
    if nsid == "ai.gftd.apps.storage.resolveSourceAsset":
        source_record = {
    ...
    if nsid == "ai.gftd.apps.copyright.license.inspect":
    ...
    if nsid == "ai.gftd.apps.i18n.translateBatch":
...
async def task_open_patent_generic_manufacturing_queue_seiyaku_batch_start(payload: dict | None = None) -> dict:
    ...
    batch_payload = p.get("batchPayload") if isinstance(p.get("batchPayload"), dict) else {}
    start_nsid = str(p.get("startNsid") or "ai.gftd.apps.openPatent.genericManufacturing.startSeiyakuBatch")
...
async def task_news_xrpc_analyze_intel(actor: str = "", input: dict | None = None) -> dict:
    ...
    return await _news_xrpc("ai.gftd.apps.news.analyzeIntel", payload)
...
async def task_news_xrpc_publish_intel(input: dict | None = None) -> dict:
    ...
    return await _news_xrpc("ai.gftd.apps.news.publishIntel", payload)
...
async def task_ind_efiling_submit(
    ...
    req = _u_req.Request(
        ...
        headers={
            ...
            "X-GFTD-Signature": "sha256=" + signature,
        },
    )
    started = time.monotonic()
    try:
...
async def task_generic_audit_emit(
    ...
    vertex_id = f"{actor_value}:ai.gftd.bpmn.audit:{rkey}:create"
    ...
    try:
        with sync_cursor() as cur:
            cur.execute(sql_text, (
                vertex_id, ts_ms, actor_value, "ai.gftd.bpmn.audit", rkey, "create",
...
async def task_kouza_sync_due_connections(maxConnections: int = 25,
    ...
    """
    from pymagatama.handlers.kouza import sync_due_connections_payload
...
async def task_ingest_run_mark_completed(
    ...
    try:
        from pymagatama.ingest.core import mark_run_finished
...
async def task_blockchain_head_ingest(
    ...
    try:
        from pymagatama.ingest.blockchain import ingest_head_delta
...
async def task_netintel_dns_delta(
    ...
    try:
        from pymagatama.ingest.netintel_dns import ingest_dns_delta
...
async def task_netintel_ip_enrich(
    ...
    try:
        from pymagatama.ingest.ip_enrich import ingest_ip_enrich
...
async def task_netintel_whois_delta(
    ...
    try:
        from pymagatama.ingest.whois_rdap import ingest_whois_delta
...
async def task_netintel_scan_banner(
    ...
    try:
        from pymagatama.ingest.scan_banner import ingest_scan_banner
...
async def task_netintel_fingerprint_delta(
    ...
    try:
        from pymagatama.ingest.fingerprint import ingest_fingerprint_delta
...
async def task_bluesky_ingest_actor(
    ...
    `/xrpc/ai.gftd.apps.bluesky.ingestActor` business logic.
    """
    try:
...
async def task_bluesky_refresh_stalest(
    ...
    """
    try:
        from pymagatama.ingest.bluesky import refresh_stalest
...
async def task_briefing_create_agenda(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import create_agenda
...
async def task_briefing_save_transcript(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import save_transcript
...
async def task_briefing_extract_action_items(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import extract_action_items
...
async def task_briefing_generate_summary(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import generate_summary
...
async def task_briefing_record_speaker_turn(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import record_speaker_turn
...
async def task_briefing_record_decision(**kwargs: Any) -> dict:
    from pymagatama.ingest.briefing import record_decision
...
async def task_arb_scout_quotes(assetClass: str = "") -> dict:
    try:
        from pymagatama.ingest.arbitrage import scout_quotes
...
async def task_arb_ingest_quote(**kwargs: Any) -> dict:
    try:
        from pymagatama.ingest.arbitrage import ingest_quote
...
async def task_arb_detect_spread(assetClass: str = "", minSpreadBps: float = 20) -> dict:
    try:
        from pymagatama.ingest.arbitrage import detect_spread
...
async def task_arb_propose_trade(**kwargs: Any) -> dict:
    try:
        from pymagatama.ingest.arbitrage import propose_trade
...
async def task_arb_score_proposal(proposalId: str = "", model: str = "heuristic-v1") -> dict:
    try:
        from pymagatama.ingest.arbitrage import score_proposal
...
async def task_arb_publish_proposal(
    ...
    try:
        from pymagatama.ingest.arbitrage import publish_proposal
...
async def task_arb_list_proposals(
    ...
    try:
        from pymagatama.ingest.arbitrage import list_proposals
...
async def task_arb_get_proposal(proposalId: str = "") -> dict:
    try:
        from pymagatama.ingest.arbitrage import get_proposal
...
async def _arms_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import arms
...
async def _collector_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import collector
...
async def _calendar_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import calendar
...
async def _animeka_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import animeka
...
async def _gworkspace_lite_call(app: str, fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import gworkspace_lite
...
async def _gworkspace_service_call(app: str, fn_name: str, kwargs: dict[str, Any]) -> dict:
    ...
    mod = importlib.import_module(f"pymagatama.ingest.{app}")
...
async def _gmail_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import gmail
...
async def _outlook_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import outlook
...
async def _mailer_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import mailer
...
async def _stripe_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import stripe
...
async def _ads_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import ads
...
async def _shiharai_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import shiharai
...
async def _kouza_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import kouza
...
async def _kaikei_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import kaikei
...
async def _moneyforward_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import moneyforward_ops
...
async def _ka_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import ka
...
async def _kg_curator_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import kg_curator
...
async def _demining_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import demining
...
async def _dns_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import dns
...
async def _kami_ketsu_gorilla_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import kami_ketsu_gorilla
...
async def _real_estate_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import real_estate
...
async def _mold_allergy_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import mold_allergy
...
async def _kami_eng_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import kami_eng
...
async def _i18n_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import i18n
...
async def _baminiku_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import baminiku
...
async def _game_play_uploader_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import game_play_uploader
...
async def _apps_directory_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import apps_directory
...
async def _nist_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import nist
...
async def _vehicle_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import vehicle
...
async def _vin_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import vin
...
async def _vessel_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import vessel
...
async def _port_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import port
...
async def _maps_collection_call(fn_name: str, kwargs: dict[str, Any]) -> dict:
    from pymagatama.ingest import maps_collection
...
# task_resource_flow_detect_anomaly moved to pymagatama/primitives/resource_flow.py
...
async def main() -> None:
    ...
    if worker_profile in {"yoro_actor_quality", "yoro-social", "yoro_social"}:
        from pymagatama.primitives import yoro_social  # noqa: E402
    ...
    if worker_profile in {"legal_entity", "legal-entity"}:
        from pymagatama.primitives import legal_entity  # noqa: E402
    ...
    if worker_profile in {"shosha", "sogo_shosha", "sogo-shosha"}:
        ...
        from pymagatama.primitives import shosha  # noqa: E402
    ...
    if worker_profile in {"iryo", "hospital", "iryo_hospital", "iryo-hospital"}:
        ...
        from pymagatama.primitives import iryo  # noqa: E402
    ...
    if worker_profile in {"lifehack", "lifehack_actor", "mitama_lifehack", "mitama-lifehack"}:
        ...
        from pymagatama.primitives import lifehack  # noqa: E402
    ...
    if worker_profile in {"otakiage", "otakiage_actor", "mitama_otakiage", "mitama-otakiage"}:
        ...
        from pymagatama.primitives import otakiage  # noqa: E402
    ...
    if worker_profile in {"training", "training_actor", "mitama_training", "mitama-training"}:
        ...
        from pymagatama.primitives import training_run  # noqa: E402
    ...
    if worker_profile in {"chat", "chat_agent", "chat-agent"}:
        ...
        from pymagatama.primitives import chat as _chat  # noqa: E402
    ...
    if worker_profile in {"shinshi"}:
        ...
        from pymagatama.primitives import shinshi_image  # noqa: E402
    ...
    if worker_profile in {"billing", "mitama_billing", "mitama-billing"}:
        ...
        from pymagatama.primitives import billing as billing_module  # noqa: E402
    ...
    if worker_profile in {"hume_emotion", "hume-emotion", "hume"}:
        ...
        from pymagatama.primitives import hume_distillation  # noqa: E402
        ...
        async def task_hume_generic_langgraph_run(
            ...
            from pymagatama.primitives import langgraph_registry  # noqa: E402
            import pymagatama.agents.hume_emotion  # noqa: E402, F401
        ...
        worker.task(
            task_type="ai.gftd.agent.hume.emotion",
            single_value=False,
            timeout_ms=timeout_ms,
        ...
        LOG.info(
            ...
            "hume.tts.synthesize,hume.distill.*,ai.gftd.agent.hume.emotion,generic.langgraph.run",
            worker_profile,
        )
    ...
    if worker_profile in {"webya"}:
        ...
        from pymagatama.primitives import webya  # noqa: E402
        from pymagatama.langgraph_graphs import webya_site_generation  # noqa: E402
    ...
    if worker_profile in {"ki", "ki_worker", "ki-worker"}:
        from pymagatama.ki_worker_main import (  # noqa: E402
    ...
    if worker_profile in {"saikin", "saikin_worker", "saikin-worker"}:
        from pymagatama.saikin_worker_main import (  # noqa: E402
    ...
    if worker_profile in {"completer", "completer_worker", "completer-worker"}:
        from pymagatama.completer_worker_main import (  # noqa: E402
        ...
        worker.task(task_type="ai.gftd.apps.completer.queryRules",        single_value=False, timeout_ms=completer_timeout_ms)(_completer_query_rules)
        worker.task(task_type="ai.gftd.apps.completer.matchRules",         single_value=False, timeout_ms=completer_timeout_ms)(_completer_match_rules)
        worker.task(task_type="ai.gftd.apps.completer.llmEvaluate",        single_value=False, timeout_ms=completer_llm_timeout_ms)(_completer_llm_evaluate)
        worker.task(task_type="ai.gftd.apps.completer.evaluate",           single_value=False, timeout_ms=completer_timeout_ms)(_completer_evaluate)
        worker.task(task_type="ai.gftd.apps.completer.evaluateRepoDids",   single_value=False, timeout_ms=completer_timeout_ms)(_completer_evaluate_repo_dids)
        worker.task(task_type="ai.gftd.apps.completer.remediate",          single_value=False, timeout_ms=completer_llm_timeout_ms)(_completer_remediate)
        worker.task(task_type="ai.gftd.apps.completer.getAuditReport",     single_value=False, timeout_ms=completer_timeout_ms)(_completer_get_audit_report)
        worker.task(task_type="ai.gftd.apps.completer.listFindings",       single_value=False, timeout_ms=completer_timeout_ms)(_completer_list_findings)
        worker.task(task_type="ai.gftd.apps.completer.listAudits",         single_value=False, timeout_ms=completer_timeout_ms)(_completer_list_audits)
        worker.task(task_type="ai.gftd.apps.completer.getComplianceScore", single_value=False, timeout_ms=completer_timeout_ms)(_completer_get_compliance_score)
        worker.task(task_type="generic.audit.emit", single_value=False, timeout_ms=60_000)(task_generic_audit_emit)
        LOG.info(
            "registered dedicated worker profile=%s task_types="
            "ai.gftd.apps.completer.{queryRules,matchRules,llmEvaluate,evaluate,evaluateRepoDids,"
    ...
    if worker_profile in {"newsletter", "newsletter_worker", "newsletter-worker"}:
        from pymagatama.newsletter_worker_main import (  # noqa: E402
    ...
    if worker_profile in {"webmk", "webmk_worker", "webmk-worker"}:
        from pymagatama.webmk_worker_main import (  # noqa: E402
    ...
    worker.task(task_type="ai.gftd.devstral.chat",       single_value=False, timeout_ms=LONG_TIMEOUT_MS)(task_chat)
    worker.task(task_type="ai.gftd.devstral.classifyT3", single_value=False, timeout_ms=LONG_TIMEOUT_MS)(task_classify_t3)
    worker.task(task_type="ai.gftd.devstral.translate",  single_value=False, timeout_ms=LONG_TIMEOUT_MS)(task_translate)
    worker.task(task_type="ai.gftd.devstral.storyboard", single_value=False, timeout_ms=LONG_TIMEOUT_MS)(task_storyboard)
    worker.task(task_type="llm.knowledge.retrieve", single_value=False, timeout_ms=60_000)(task_llm_knowledge_retrieve)
    ...
    # Phase Z-α — magatama.shinka.* heartbeat tasks (BPMN migration of the
    # LangGraph nodes). Shorter timeout; these are DB-bound + at most one
    # llm call inside compose.
    SHINKA_TIMEOUT_MS = 90_000
    worker.task(task_type="magatama.shinka.loadAndResolve", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_shinka_load_and_resolve)
    worker.task(task_type="magatama.shinka.compose",        single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_shinka_compose)
    worker.task(task_type="magatama.shinka.writeHeartbeat", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_shinka_write_heartbeat)
    worker.task(task_type="magatama.shinka.emitEvolution",  single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_shinka_emit_evolution)
    ...
    worker.task(task_type="ai.gftd.kouza.syncDueConnections", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_kouza_sync_due_connections)
    ...
    # per-service ingest modules in pymagatama.ingest.<app> (drive/contacts/
    ...
    from pymagatama.primitives import email_route as _email_route
    ...
    from pymagatama.primitives.oshinobi import register as _oshinobi_register  # noqa: E402
    ...
    # pymagatama.ingest.houbun; graph writes stay behind rw.health.probe.
    from pymagatama.ingest.houbun import (  # noqa: E402
    ...
    # pymagatama.ingest.fund; graph writes stay behind rw.health.probe.
    from pymagatama.ingest.fund.zeebe_tasks import (  # noqa: E402
    ...
    worker.task(task_type="xrpc.ai.gftd.apps.kakaku.upsertOffer", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_upsert_offer)
    worker.task(task_type="xrpc.ai.gftd.apps.kakaku.ingestOfferFromUrl", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_ingest_offer_from_url)
    worker.task(task_type="xrpc.ai.gftd.apps.kakaku.compareOffers", single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_compare_offers)
    ...
    worker.task(task_type="ai.gftd.shinka.tick", single_value=False, timeout_ms=SHINKA_TICK_TIMEOUT_MS)(task_shinka_tick)
    ...
    worker.task(task_type="ai.gftd.agent.plan",  single_value=False, timeout_ms=SHINKA_TIMEOUT_MS)(task_agent_plan)
    from pymagatama.agents.hume_emotion import task_agent_hume_emotion  # noqa: E402
    worker.task(task_type="ai.gftd.agent.hume.emotion", single_value=False,
    ...
    # retry → notify). Callers can use ai.gftd.agent.sbom.register
    # instead of the lower-level xrpc.ai.gftd.apps.sbom.registerArtifact
    ...
    worker.task(task_type="ai.gftd.agent.sbom.register", single_value=False,
    ...
    worker.task(task_type="ai.gftd.agent.gameka.studio", single_value=False,
    ...
    from pymagatama.handlers.gameka_codegen import (  # noqa: E402
    ...
    from pymagatama.handlers.gameka_avatar import (  # noqa: E402
    ...
    worker.task(task_type="ai.gftd.agent.gameka.visualCritic", single_value=False,
    ...
    async def task_generic_langgraph_run(
        ...
        import pymagatama.agents.gameka_studio  # noqa: E402, F401
        import pymagatama.agents.gameka_visual_critic  # noqa: E402, F401
        import pymagatama.agents.hume_emotion  # noqa: E402, F401
        import pymagatama.ingest.jp_corp_finance.langgraph_disclosure_extract  # noqa: E402, F401
        import pymagatama.primitives.apqc  # noqa: E402, F401
        graph = langgraph_registry.get(graph_id)
        if graph is None:
    ...
    from pymagatama.primitives import mangaka  # noqa: E402, lazy: skip Pillow import if unused
    ...
    from pymagatama.primitives import loading_robot  # noqa: E402
    ...
    from pymagatama.primitives import robotics  # noqa: E402
    ...
    from pymagatama.primitives import ma  # noqa: E402
    ...
    from pymagatama.primitives import business_person  # noqa: E402
    ...
    from pymagatama.primitives import telecom  # noqa: E402
    ...
    from pymagatama.primitives import projector  # noqa: E402
    ...
    from pymagatama.primitives import murakumo_fleet  # noqa: E402
    ...
    from pymagatama.primitives import graph_consumer  # noqa: E402
    graph_consumer.register(worker, timeout_ms=60_000)
    # Magatama organizer bridge. The CF dashboard keeps the classifier and R2
    # plan writes; Zeebe owns the 5-minute schedule.
    from pymagatama.primitives import magatama_organizer  # noqa: E402
    magatama_organizer.register(worker, timeout_ms=90_000)
    # PDS write-outbox replay bridge. PDS keeps XRPC dispatch and Hyperdrive;
    # Zeebe owns the replay schedule.
    from pymagatama.primitives import pds_outbox  # noqa: E402
    ...
    from pymagatama.primitives import pds_key_rotation  # noqa: E402
    ...
    from pymagatama.primitives import pds_mitama_cron  # noqa: E402
    ...
    from pymagatama.primitives import pds_heartbeat  # noqa: E402
    ...
    from pymagatama.primitives import pds_discover_cache  # noqa: E402
    ...
    from pymagatama.primitives import pds_domain_coverage  # noqa: E402
    ...
    from pymagatama.primitives import yard_ops as _yard_ops  # noqa: E402
    ...
    from pymagatama.primitives import open_lei  # noqa: E402
    ...
    from pymagatama.primitives import org_unit  # noqa: E402
    ...
    from pymagatama.primitives import public_malak_ads  # noqa: E402
    ...
    from pymagatama.primitives import malak as _malak_primitives  # noqa: E402
    ...
    from pymagatama.primitives import os_messaging_open_channels  # noqa: E402
    ...
    from pymagatama.primitives import gov_prk  # noqa: E402
    gov_prk.register(worker, timeout_ms=180_000)
    # Hong Kong SAR government actor migration.
    from pymagatama.primitives import gov_hkg  # noqa: E402
    ...
    # syncWetUpdates, shinka, heartbeatTick) under xrpc.ai.gftd.gov{Code}.*.
    ...
    from pymagatama.primitives import gov_are  # noqa: E402
    gov_are.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_arg  # noqa: E402
    gov_arg.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_atg  # noqa: E402
    gov_atg.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_aus  # noqa: E402
    gov_aus.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_aut  # noqa: E402
    gov_aut.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bel  # noqa: E402
    gov_bel.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bgd  # noqa: E402
    gov_bgd.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bgr  # noqa: E402
    gov_bgr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bhr  # noqa: E402
    gov_bhr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bih  # noqa: E402
    gov_bih.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_blr  # noqa: E402
    gov_blr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bol  # noqa: E402
    gov_bol.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bra  # noqa: E402
    gov_bra.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_brb  # noqa: E402
    gov_brb.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_brn  # noqa: E402
    gov_brn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_bwa  # noqa: E402
    gov_bwa.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_can  # noqa: E402
    gov_can.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_che  # noqa: E402
    gov_che.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_chl  # noqa: E402
    gov_chl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_chn  # noqa: E402
    gov_chn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_civ  # noqa: E402
    gov_civ.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cmr  # noqa: E402
    gov_cmr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cod  # noqa: E402
    gov_cod.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_col  # noqa: E402
    gov_col.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cri  # noqa: E402
    gov_cri.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cub  # noqa: E402
    gov_cub.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cyp  # noqa: E402
    gov_cyp.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_cze  # noqa: E402
    gov_cze.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_deu  # noqa: E402
    gov_deu.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_dma  # noqa: E402
    gov_dma.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_dnk  # noqa: E402
    gov_dnk.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_dom  # noqa: E402
    gov_dom.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_dza  # noqa: E402
    gov_dza.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ecu  # noqa: E402
    gov_ecu.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_egy  # noqa: E402
    gov_egy.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_esp  # noqa: E402
    gov_esp.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_est  # noqa: E402
    gov_est.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_eth  # noqa: E402
    gov_eth.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_fin  # noqa: E402
    gov_fin.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_fji  # noqa: E402
    gov_fji.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_fra  # noqa: E402
    gov_fra.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_gbr  # noqa: E402
    gov_gbr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_geo  # noqa: E402
    gov_geo.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_gha  # noqa: E402
    gov_gha.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_grc  # noqa: E402
    gov_grc.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_grd  # noqa: E402
    gov_grd.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_gtm  # noqa: E402
    gov_gtm.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_guy  # noqa: E402
    gov_guy.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_hnd  # noqa: E402
    gov_hnd.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_hrv  # noqa: E402
    gov_hrv.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_hti  # noqa: E402
    gov_hti.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_hun  # noqa: E402
    gov_hun.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_idn  # noqa: E402
    gov_idn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ind  # noqa: E402
    gov_ind.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_irl  # noqa: E402
    gov_irl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_irn  # noqa: E402
    gov_irn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_irq  # noqa: E402
    gov_irq.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_isl  # noqa: E402
    gov_isl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ita  # noqa: E402
    gov_ita.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_jam  # noqa: E402
    gov_jam.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_jor  # noqa: E402
    gov_jor.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_jpn  # noqa: E402
    gov_jpn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_kaz  # noqa: E402
    gov_kaz.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ken  # noqa: E402
    gov_ken.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_kgz  # noqa: E402
    gov_kgz.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_khm  # noqa: E402
    gov_khm.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_kwt  # noqa: E402
    gov_kwt.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lao  # noqa: E402
    gov_lao.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lbn  # noqa: E402
    gov_lbn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lby  # noqa: E402
    gov_lby.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lka  # noqa: E402
    gov_lka.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ltu  # noqa: E402
    gov_ltu.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lux  # noqa: E402
    gov_lux.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_lva  # noqa: E402
    gov_lva.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mar  # noqa: E402
    gov_mar.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mdg  # noqa: E402
    gov_mdg.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mex  # noqa: E402
    gov_mex.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mhl  # noqa: E402
    gov_mhl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mkd  # noqa: E402
    gov_mkd.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mlt  # noqa: E402
    gov_mlt.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mmr  # noqa: E402
    gov_mmr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mne  # noqa: E402
    gov_mne.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mng  # noqa: E402
    gov_mng.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_moz  # noqa: E402
    gov_moz.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_mys  # noqa: E402
    gov_mys.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_nga  # noqa: E402
    gov_nga.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_nic  # noqa: E402
    gov_nic.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_nld  # noqa: E402
    gov_nld.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_nor  # noqa: E402
    gov_nor.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_npl  # noqa: E402
    gov_npl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_nzl  # noqa: E402
    gov_nzl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_omn  # noqa: E402
    gov_omn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_pak  # noqa: E402
    gov_pak.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_pan  # noqa: E402
    gov_pan.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_per  # noqa: E402
    gov_per.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_phl  # noqa: E402
    gov_phl.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_png  # noqa: E402
    gov_png.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_pol  # noqa: E402
    gov_pol.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_prt  # noqa: E402
    gov_prt.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_pry  # noqa: E402
    gov_pry.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_pse  # noqa: E402
    gov_pse.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_qat  # noqa: E402
    gov_qat.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_rou  # noqa: E402
    gov_rou.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_rwa  # noqa: E402
    gov_rwa.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_sau  # noqa: E402
    gov_sau.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_sdn  # noqa: E402
    gov_sdn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_sen  # noqa: E402
    gov_sen.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_sgp  # noqa: E402
    gov_sgp.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_slv  # noqa: E402
    gov_slv.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_srb  # noqa: E402
    gov_srb.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ssd  # noqa: E402
    gov_ssd.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_sur  # noqa: E402
    gov_sur.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_svk  # noqa: E402
    gov_svk.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_svn  # noqa: E402
    gov_svn.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_swe  # noqa: E402
    gov_swe.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tha  # noqa: E402
    gov_tha.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tjk  # noqa: E402
    gov_tjk.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tkm  # noqa: E402
    gov_tkm.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tls  # noqa: E402
    gov_tls.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tur  # noqa: E402
    gov_tur.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_tza  # noqa: E402
    gov_tza.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_uga  # noqa: E402
    gov_uga.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ukr  # noqa: E402
    gov_ukr.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ury  # noqa: E402
    gov_ury.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_usa  # noqa: E402
    gov_usa.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_uzb  # noqa: E402
    gov_uzb.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_ven  # noqa: E402
    gov_ven.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_vnm  # noqa: E402
    gov_vnm.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_yem  # noqa: E402
    gov_yem.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_zmb  # noqa: E402
    gov_zmb.register(worker, timeout_ms=180_000)
    from pymagatama.primitives import gov_zwe  # noqa: E402
    ...
    from pymagatama.primitives import maps_sentinel  # noqa: E402
    ...
    from pymagatama.primitives import aircraft_live  # noqa: E402
    ...
    from pymagatama.primitives import satellite_live  # noqa: E402
    ...
    # accurate star background (R/P30D, ai.gftd.apps.maps.ingestCelestialCatalogs).
    ...
    from pymagatama.primitives import aismarine  # noqa: E402
    ...
    from pymagatama.ingest.flight_offer import (  # noqa: E402
    ...
    from pymagatama.ingest.curpus2skill import task_curpus2skill_extract_evidence  # noqa: E402
    ...
    from pymagatama.primitives import maps_building_3d  # noqa: E402
    ...
    from pymagatama.primitives import contribution_royalty  # noqa: E402
    ...
    from pymagatama.primitives import agent_economy  # noqa: E402
    ...
    from pymagatama.primitives import yukkuri  # noqa: E402
    ...
    from pymagatama.primitives import media_gamers_chart  # noqa: E402
    ...
    from pymagatama.primitives import (  # noqa: E402
        telecom_5g_security,
        telecom_5gcore,
        telecom_esim,
        telecom_ims,
    ...
    from pymagatama.primitives import onion_crawl  # noqa: E402
    ...
    from pymagatama.primitives import intel  # noqa: E402
    ...
    # seed.cases).
    from pymagatama.primitives import hanrei  # noqa: E402
    ...
    from pymagatama.primitives import isin  # noqa: E402
    ...
    from pymagatama.primitives import handotai  # noqa: E402
    ...
    from pymagatama.primitives import jp_fiscal  # noqa: E402
    ...
    from pymagatama.primitives import resource_flow  # noqa: E402
    ...
    from pymagatama.primitives import maps3d as _maps3d  # noqa: E402
    ...
    from pymagatama.ingest.site_common_crawl import (  # noqa: E402
    ...
    from pymagatama.primitives.owl_reasoner import register_owl_tasks  # noqa: E402
    ...
    from pymagatama.primitives import training_export  # noqa: E402
    ...
    from pymagatama.primitives import aria_signal  # noqa: E402
    ...
    from pymagatama.primitives import common_crawl  # noqa: E402
    ...
    LOG.info("registered tasks: chat, classifyT3, translate, storyboard, "
             ...
             "ai.gftd.agent.{plan,gameka.studio,gameka.visualCritic}, "
