...
This module moves the `did:web:prk-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:prk-state.gftd.ai"
DOMAIN_CODE = "prk"
...
def _http_post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    merged_headers = {
        "User-Agent": "gftd-pymagatama-gov-prk/0.1",
...
async def task_gov_prk_ingest_official_sources(
    ...
    """Queue official DPRK government sources through site.gftd.ai."""
    limit = max(1, min(int(limit or 10), 50))
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.ingestOfficialSources",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_ingest_official_sources)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_prk_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govPrk.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
