...
This module moves the `did:web:ind-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:ind-state.gftd.ai"
DOMAIN_CODE = "ind"
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govInd.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_ind_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govInd.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
