...
def _get_org(path: str) -> dict[str, Any] | None:
    # R0: fetch with single equality, filter by domain/owner
    rows = get_kotoba_client().select_where("vertex_gov_org", "path", path, limit=10)
    for row in rows:
        if row.get("domain_code") == DOMAIN_CODE and row.get("owner_did") == PRIMARY_DID:
            return row
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_irq_seed_orgs)
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_irq_register_dids)
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_irq_follow_site_deps)
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_irq_resolve_org_path)
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_irq_list_orgs)
    worker.task(
        task_type="xrpc.com.etzhayyim.govIrq.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
