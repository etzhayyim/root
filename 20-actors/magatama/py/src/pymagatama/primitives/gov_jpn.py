...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_jpn_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govJpn.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
