...
def _oai_checkpoint_vertex_id(
    ...
    if pages_seen is not None and records_seen is not None:
        ...
        return f"at://{_ACTOR}/ai.gftd.apps.ndl.oaiCheckpoint/{key}/{int(pages_seen)}-{int(records_seen)}-{token_key}"
    return f"at://{_ACTOR}/ai.gftd.apps.ndl.oaiCheckpoint/{key}"
...
def _b2_put_webp(pid: str, page_index: int, webp: bytes) -> tuple[str, str, str]:
    from pymagatama.primitives.isbn import _b2_put, _cidv1_raw_sha256
...
def _cursor_vertex_id(provider_id: str, query: str) -> str:
    ...
    return f"at://{_ACTOR}/ai.gftd.apps.ndl.ingestCursor/{provider_id}-{key}"
...
def _page_has_completed_ocr(pid: str, page_index: int) -> bool:
    row = _rw_one(
        ...
        LIMIT 1
        """,
        {"vertex_id": f"at://{_ACTOR}/ai.gftd.apps.ndl.digitalPage/{pid}-{page_index + 1:06d}"},
    )
...
def _item_insert_row(item: dict[str, str], now: str) -> dict[str, Any]:
    pid = item["pid"]
    return {
        "vertex_id": f"at://{_ACTOR}/ai.gftd.apps.ndl.digitalItem/{pid}",
...
def _write_run_and_cursor(
    *,
    ...
    _rw_exec(
        _INSERT_RUN,
        {
            "vertex_id": f"at://{_ACTOR}/ai.gftd.apps.ndl.ingestRun/{run_id}",
            "created_date": _today_date(),
            "owner_did": _ACTOR,
            "run_id": run_id,
            "provider_id": provider_id,
...
async def task_ndl_image_ocr_ingest(
    ...
    try:
        ...
        for item in selected_items:
            now = _now_iso()
            pid = item["pid"]
            item_vid = f"at://{_ACTOR}/ai.gftd.apps.ndl.digitalItem/{pid}"
            _rw_replace_vertex(
                "vertex_ndl_digital_item",
                item_vid,
                _INSERT_ITEM,
            ...
            for page_index, canvas in enumerate(canvases[:page_limit]):
                ...
                try:
                    ...
                    now = _now_iso()
                    page_vid = f"at://{_ACTOR}/ai.gftd.apps.ndl.digitalPage/{pid}-{page_index + 1:06d}"
                    page_row = {
                        "vertex_id": page_vid,
                        "created_date": _today_date(),
                        "owner_did": _ACTOR,
                    ...
                    if ocr:
                        ...
                        ocr_vid = f"at://{_ACTOR}/ai.gftd.apps.ndl.ocrText/{pid}-{page_index + 1:06d}"
                        ocr_row = {
                            "vertex_id": ocr_vid,
                            "created_date": _today_date(),
                            "owner_did": _ACTOR,
