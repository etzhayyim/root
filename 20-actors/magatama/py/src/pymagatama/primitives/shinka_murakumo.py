...
if "runpod" in os.environ.get("PATH", "").lower() or os.environ.get("RW_URL"):
    raise ImportError(
        ...
        "Use vendor pymagatama.primitives.shinka for paid SaaS workloads."
    )
...
class CadencePolicy:
    ...
    Vendor source: pymagatama/shinka/__init__.py _cadence_flags().
...
async def karma_hegemon_observation_cell(adherent_did: str) -> AdherentState:
    ...
    # Vendor _kyumei_gather reads from vertex_kyumei_signal via RW SELECT.
    # Religious-corp reads from MST via mst.query (stub until M3).
    kyumei_records: list[dict[str, Any]] = await _mst_mod.query(
        "app.etzhayyim.shinka.kyumeiSignal",
        filter_did=adherent_did,
        since=last_evolution_at,
        limit=50,
    )
...
async def shinka_tick(adherent_did: str | None = None) -> dict[str, Any]:
    ...
    In production, cells are dispatched by magatama-cell-runner, not called directly.
    ...
    Multi-adherent case (adherent_did is None):
      ...
      Full multi-adherent iteration requires the magatama-cell-runner tick loop
      (pending M3 fleet.toml wiring).

    Note: In production the Pregel super-step is cell-driven (magatama-cell-runner),
    ...
    if adherent_did is None:
        ...
        # Full iteration requires magatama-cell-runner tick loop (M3).
        _log.info(
            "shinka_tick: no adherent_did — would iterate over all adherents-due-for-tick. "
            "Full multi-adherent loop pending M3 magatama-cell-runner fleet.toml wiring."
        )
