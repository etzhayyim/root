...
This module moves the `did:web:aus-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:aus-state.gftd.ai"
DOMAIN_CODE = "aus"
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govAus.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_aus_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govAus.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
