...
async def task_gov_mdg_sync_wet_updates(limit: int = 10, postUpdates: bool = True) -> dict[str, Any]:
    ...
    with sync_cursor() as cur:
        cur.execute(
            f"""
            SELECT path, name_en, website, site_domain_slug, last_content_hash, owner_did
              FROM vertex_gov_org
             WHERE domain_code = %s
               AND site_domain_slug != ''
               AND (
                 last_content_hash = ''
                 OR last_content_hash IS NULL
                 OR
                 last_ingested_at = ''
    ...
    for r in rows:
        ...
        owner_did = str(r[5] or PRIMARY_DID)
