...
This module moves the `did:web:kor-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:kor-state.gftd.ai"
DOMAIN_CODE = "kor"
...
def _http_post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    merged_headers = {
        "User-Agent": "gftd-pymagatama-gov-kor/0.1",
...
async def task_gov_kor_ingest_official_sources(
    ...
    """Queue official South Korean government sources through site.gftd.ai."""
    limit = max(1, min(int(limit or 10), 50))
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govKor.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.ingestOfficialSources",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_ingest_official_sources)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_kor_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govKor.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
