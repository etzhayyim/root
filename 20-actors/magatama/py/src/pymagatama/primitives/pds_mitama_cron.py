"""PDS Mitama actor cron-trigger resync primitives for BPMN/LangServer."""
...
MITAMA_CRON_COLLECTION = "com.etzhayyim.apps.pds.mitamaCronResync"
...
def _http_post_json(url: str, payload: dict[str, Any], secret: str = "") -> dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "pymagatama-pds-mitama-cron/1",
    }
    if secret:
...
def call_pds_mitama_cron_resync(pds_url: str, secret: str) -> dict[str, Any]:
    url = pds_url.rstrip("/") + "/_internal/resync-mitama-cron-triggers"
    ...
    return {
        ...
        "scheduled": int(body.get("scheduled") or 0),
        "error": "" if ok else str(body.get("error") or f"http {http_status}")[:500],
    }


def write_mitama_cron_resync_tick(tick: dict[str, Any], *, flush: bool = True) -> dict[str, Any]:
    ts = str(tick.get("ts") or _utc_now_iso())
    rkey = "mitama-cron-resync-" + ts.replace("-", "").replace(":", "").replace(".", "")
    rkey = rkey.replace("T", "").replace("Z", "")
    uri = f"at://{PDS_DID}/{MITAMA_CRON_COLLECTION}/{rkey}"
    value = {
        "$type": MITAMA_CRON_COLLECTION,
        "v": 1,
        **tick,
    }
    row = {
        "vertex_id": uri,
        "tick_id": rkey,
        "operation_kind": "mitamaCronResync",
        "ok": bool(tick.get("ok")),
        "http_status": int(tick.get("httpStatus") or 0),
        "metric_primary": int(tick.get("scheduled") or 0),
        "metric_secondary": 0,
...
def task_pds_mitama_cron_triggers_resync(
    pdsUrl: str = "",
    flush: bool = True,
) -> dict[str, Any]:
    ...
    if not secret:
        ...
        result = call_pds_mitama_cron_resync(url, secret)
        tick = {
            "ts": ts,
            **result,
            "latencyMs": int((time.monotonic() - started) * 1000),
        }
    audit = write_mitama_cron_resync_tick(tick, flush=flush)
    return {
        "ok": tick["ok"],
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="pds.mitama.cronTriggers.resync",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_pds_mitama_cron_triggers_resync)
