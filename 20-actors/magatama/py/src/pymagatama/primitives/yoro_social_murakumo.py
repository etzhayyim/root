...
async def task_yoro_social_translate_post(
    ...
    LLM placeholder (M5 target):
      Real translation is deferred until M5 EVO-X2 LiteLLM integration.
      # TODO M5: replace with etzhayyim_sdk.llm.translate(source_text, target_lang)
      Current stub returns: f"[translated to {target_lang}] {source_text}"
    ...
    # Step 2: Translate (LLM stub for M4; real LLM wired in M5).
    # TODO M5: replace with etzhayyim_sdk.llm.translate(text, target_lang)
    translated_text = f"[translated to {targetLang}] {text}"
    source_method = "llm-stub"
