...
def classification_vertex_id(worker_did: str, isco_code: str, classified_at: str) -> str:
    return f"at://{OPEN_ISCO_DID}/ai.gftd.apps.openIsco.classification/{_digest(worker_did, isco_code, classified_at)}"
...
async def task_open_isco_classify_worker(
    ...
    edge_id = f"at://{OPEN_ISCO_DID}/ai.gftd.apps.openIsco.classificationOccupation/{_digest(vertex_id, code)}"
...
async def task_open_isco_record_concordance(
    ...
    vertex_id = f"at://{OPEN_ISCO_DID}/ai.gftd.apps.openIsco.concordance/{_digest(code, otherTaxonomy, otherCode, relation)}"
