...
async def task_patent_epo_ops_fill_citations(
    ...
    for row in rows:
        ...
        for cit in ops_cits:
            ...
            if not cited_num:
                continue
            cited_vertex_id = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.patent/{cited_cc}-{cited_num}"
        ...
        for member in members:
            ...
            if not member_num or not member_cc:
                continue
            member_vid = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.patent/{member_cc}-{member_num}"
...
async def task_patent_uspto_ingest_citation(
    ...
    for tsv_row in reader:
        ...
        citing_vid = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.patent/US-{pat_id}"
        cited_vid = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.patent/US-{cit_id}"
...
async def task_patent_uspto_ingest_patent(
    ...
    for tsv_row in reader:
        ...
        vertex_id = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.patent/US-{pat_number}"
        ...
        if pat_date and pat_date >= blobThresholdDate and filename:
            ...
            blob_vid = f"at://{_PATENT_ACTOR}/ai.gftd.apps.patent.blob/US-{pat_number}"
            blob_batch.append((
