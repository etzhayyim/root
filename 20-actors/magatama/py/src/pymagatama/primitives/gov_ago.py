...
This module moves the `did:web:ago-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:ago-state.gftd.ai"
DOMAIN_CODE = "ago"
...
def _http_post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    merged_headers = {
        "User-Agent": "gftd-pymagatama-gov-ago/0.1",
...
async def task_gov_ago_ingest_official_sources(
    ...
    """Queue official Angolan government sources through site.gftd.ai."""
    limit = max(1, min(int(limit or 10), 50))
