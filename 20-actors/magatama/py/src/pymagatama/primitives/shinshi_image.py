...
from pymagatama.primitives.yoro_social import (
    build_repo_record,
...
# The `x-magatama-verified` shortcut is only honored for intra-CF callers, so
...
def _pds_auth_headers(lxm: str, *, repo: str = "") -> dict[str, str]:
    ...
    Order: cached/fresh service-auth JWT → static PDS_SERVICE_AUTH_TOKEN
           → legacy x-magatama-verified (only if PDS_LEGACY_INTERNAL_TRUST=1).
    """
...
async def _upload_blob_to_pds(png: bytes, repo: str) -> str:
    ...
    internet (intra-CF `x-magatama-verified` shortcut not honored)."""
    auth = _pds_auth_headers("com.atproto.repo.uploadBlob", repo=repo)
    if (
        "Authorization" not in auth
        and "x-magatama-verified" not in auth
        and "x-magatama-repo" not in auth
    ):
