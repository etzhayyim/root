"""Magatama organizer primitives for BPMN/LangServer.
...
ORGANIZER_RUN_COLLECTION = "com.etzhayyim.apps.magatama.organizerRun"
...
def _http_post_json(url: str, payload: dict[str, Any], bearer: str = "") -> dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "pymagatama-organizer/1",
    }
    if bearer:
...
def write_organizer_run(tick: dict[str, Any], *, flush: bool = True) -> dict[str, Any]:
    ...
    uri = f"at://{MAGATAMA_DID}/{ORGANIZER_RUN_COLLECTION}/{rkey}"
    value = {
    ...
    row = {
        ...
        "created_at": ts,
        "updated_at": ts,
        "actor_did": MAGATAMA_DID,
        "org_did": "anon",
        "owner_did": MAGATAMA_DID,
        "sensitivity_ord": 2,
...
def task_magatama_organizer_run(
    ...
    url = organizerUrl or os.environ.get("MAGATAMA_ORGANIZER_URL") or ""
    ts = _utc_now_iso()
    if not url:
        tick = {
            "ts": ts,
            ...
            "fleetSaturation": 0.0,
            "error": "MAGATAMA_ORGANIZER_URL is required",
        }
    else:
        bearer = os.environ.get("MAGATAMA_ORGANIZER_BEARER") or ""
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="magatama.organizer.run",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_magatama_organizer_run)
