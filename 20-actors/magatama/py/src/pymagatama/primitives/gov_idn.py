...
This module moves the `did:web:idn-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:idn-state.gftd.ai"
DOMAIN_CODE = "idn"
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_idn_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govIdn.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
