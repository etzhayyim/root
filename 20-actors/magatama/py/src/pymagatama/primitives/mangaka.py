...
Implements 5 task types invoked by 00-contracts/bpmn/ai/gftd/mangaka/generateEpisode.bpmn:
...
async def _pds_upload_blob(session: aiohttp.ClientSession, png_bytes: bytes) -> tuple[str, int]:
    ...
    async with session.post(
        ...
        data=png_bytes,
        headers={"Content-Type": "image/png", "x-magatama-verified": PDS_TOKEN},
...
async def task_post_publish(workUri: str = "", charName: str = "", charSlug: str = "",
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            json=record,
            headers={"Content-Type": "application/json", "x-magatama-verified": PDS_TOKEN},
            timeout=aiohttp.ClientTimeout(total=60),
        ) as resp:
            body = await resp.json()
...
def register(worker, timeout_ms: int = 600_000) -> None:
    ...
    Call from zeebe_worker_main.py after generic.* primitives:
        ...
        from pymagatama.primitives import mangaka
