...
def _row_dict(rec: Any, *, prefix: str, key_fields: tuple[str, ...]) -> dict[str, Any]:
    raw = asdict(rec)
    vertex_id = raw.get("vertex_id") or _stable_vertex_id(
        prefix, {k: raw[k] for k in key_fields if k in raw}
    )
    raw["vertex_id"] = vertex_id
...
class AtIpfsLocalBeliefStore:
    ...
    def _put(self, table: str, row: dict[str, Any]) -> str:
        ...
        col_list = ",".join(cols)
        with self._lock, self._conn() as conn:
            conn.execute(f"DELETE FROM {table} WHERE vertex_id = ?", (row_to_persist["vertex_id"],))
            conn.execute(
                f"INSERT INTO {table} ({col_list}) VALUES ({placeholders})",
                tuple(row_to_persist[c] for c in cols),
            )
            conn.commit()
        return publish_uri or row_to_persist["vertex_id"]
