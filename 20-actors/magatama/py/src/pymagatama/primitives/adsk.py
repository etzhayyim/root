...
`trainingExport.bpmn` → B2 → `gftdcojp/gftd-corpus`) automatically
picks them up.
...
def _vertex_id(slug: str, record_id: str) -> str:
    ...
    return f"at://{_ADSK_ACTOR}/ai.gftd.apps.adsk.record/{safe_slug}--{safe_id}"
...
def _hf_load_streaming(slug: str, split: str | None) -> Iterable[dict[str, Any]]:
    ...
    Lazy import of `datasets` so the rest of pymagatama can run on
...
async def task_adsk_3d_blob_ingest(
    ...
    vertex_id = f"at://{_ADSK_ACTOR}/ai.gftd.apps.adsk.blob3d/{re.sub(r'[^a-zA-Z0-9._-]', '-', slug)}--{sha[:16]}"
