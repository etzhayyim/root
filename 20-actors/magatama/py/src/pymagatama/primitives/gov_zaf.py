...
This module moves the `did:web:zaf-state.gftd.ai` app actor off its
...
PRIMARY_DID = "did:web:zaf-state.gftd.ai"
DOMAIN_CODE = "zaf"
...
def _http_post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    merged_headers = {
        "User-Agent": "gftd-pymagatama-gov-zaf/0.1",
...
async def task_gov_zaf_ingest_official_sources(
    ...
    """Queue official South African government sources through site.gftd.ai."""
    limit = max(1, min(int(limit or 10), 50))
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.ingestOfficialSources",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_ingest_official_sources)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_zaf_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govZaf.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
