...
INSERT INTO vertex_celestial_catalog (
  vertex_id, label, did, rkey, repo, name, display_name, description,
  catalog_id, authority, version, frame, coverage_kind, metadata_json,
...
INSERT INTO vertex_celestial_object (
  vertex_id, label, did, rkey, repo, name, display_name, description, category,
  object_id, catalog_id, object_kind, ra_deg, dec_deg, distance_ly,
  spectral_class, render_priority, source_ref, status, metadata_json,
  actor_did, org_did, sensitivity_ord, owner_did, created_at
...
def _ensure_catalog(cur: Any, catalog_id: str, authority: str, version: str,
    ...
    cur.execute(
        _INSERT_CATALOG_SQL,
        (
            vid, "CelestialCatalog", did, catalog_id, did,
            name, display_name, description,
            catalog_id, authority, version, "ICRS J2000", "all-sky",
            "{}",
            ACTOR_DID, "anon", 1, DEFAULT_REPO, today,
        ),
    )
...
def task_celestial_hyg_refresh(mag_max: float = 6.5, max_rows: int = 12_000) -> dict[str, Any]:
    ...
    with sync_cursor() as cur:
        _ensure_catalog(
            cur, catalog_id, "Astronexus / HYG Database", "v41",
        ...
        for row in reader:
            ...
            try:
                cur.execute(
                    _INSERT_OBJECT_SQL,
                    (
                        vid, "CelestialObject", did, object_id, did,
                        ident, ident, f"V mag {mag:.2f}, spectral class {spect or '?'}",
                        "star",
                        object_id, catalog_id, "star", ra_deg, dec_deg, dist_ly,
                        spect, render_priority, HYG_URL, "active",
                        json.dumps({"mag": mag, "hip": hip, "hd": hd, "hr": hr}),
                        ACTOR_DID, "anon", 1, DEFAULT_REPO, today,
                    ),
                )
                written += 1
...
def task_celestial_ngc_refresh(max_rows: int = 5_000) -> dict[str, Any]:
    ...
    with sync_cursor() as cur:
        _ensure_catalog(
            cur, catalog_id, "OpenNGC project (Mattia Verga)", "2024-12",
            ...
            try:
                cur.execute(
                    ...
                    (
                        vid, "CelestialObject", did, object_id, did,
                        ident, ident,
                        f"{obj_kind} in {const}, V mag {v_mag if v_mag is not None else '?'}",
                        "deep_sky" if obj_kind != "star" else "star",
                        object_id, catalog_id, obj_kind, ra_deg, dec_deg, None,
                        None, render_priority, NGC_URL, "active",
                        json.dumps(metadata),
                        ACTOR_DID, "anon", 1, DEFAULT_REPO, today,
                    ),
                )
