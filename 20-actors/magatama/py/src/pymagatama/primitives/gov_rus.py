...
{"path":"subject:chelyabinsk","name":"Челябинская область","nameEn":"Chelyabinsk Oblast","website":"https://www.chelregion.ru/","contract":"Конституция Ст. 65","tags":["cofog:01","oblast"],"orgTier":"state"}
...
def _http_post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    merged_headers = {
        "User-Agent": "gftd-pymagatama-gov-rus/0.1",
...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_seed_orgs)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_register_dids)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_follow_site_deps)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.ingestOfficialSources",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_ingest_official_sources)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_resolve_org_path)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_list_orgs)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_sync_wet_updates)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_shinka)
    worker.task(
        task_type="xrpc.app.etzhayyim.govRus.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
