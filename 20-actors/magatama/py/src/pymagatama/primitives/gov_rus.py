...
def register(worker: Any, *, timeout_ms: int) -> None:
    worker.task(
        task_type="xrpc.ai.gftd.govRus.seedOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_seed_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.registerDIDs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_register_dids)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.followSiteDeps",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_follow_site_deps)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.ingestOfficialSources",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_ingest_official_sources)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.resolveOrgPath",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_resolve_org_path)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.listOrgs",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_list_orgs)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.syncWetUpdates",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_sync_wet_updates)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.shinka",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_gov_rus_shinka)
    worker.task(
        task_type="xrpc.ai.gftd.govRus.heartbeatTick",
        single_value=False,
        timeout_ms=timeout_ms,
