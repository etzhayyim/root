    ...
    with sync_cursor() as cur:
        ...
        if record_kind == "agent":
            ...
            cur.execute(
                ...
                (f"{OS_DID}:agent:{record.get('agentId')}:EVENT:{rkey}", f"at://{OS_DID}/app.etzhayyim.apps.os.agent/{record.get('agentId')}", vertex_id, str(record.get("agentId") or ""), str(record.get("event") or "EVENT"), now, OS_DID, 2),
            )
            ...
            cur.execute(
                ...
                (f"{OS_DID}:consent:{rkey}:RESPONSE:{vertex_id}", f"at://{OS_DID}/app.etzhayyim.apps.os.consentRequest/{rkey}", vertex_id, rkey, "RESPONDED_BY", now, OS_DID, 2),
            )
            ...
            cur.execute(
                ...
                (f"{OS_DID}:budget:{rkey}:FOR:{record.get('agentId')}", vertex_id, f"at://{OS_DID}/app.etzhayyim.apps.os.agent/{record.get('agentId')}", str(record.get("agentId") or ""), "ALLOCATED_TO", now, OS_DID, 2),
            )
            ...
            cur.execute(
                ...
                (f"{OS_DID}:agent:{record.get('agentId')}:AUDIT:{rkey}", f"at://{OS_DID}/app.etzhayyim.apps.os.agent/{record.get('agentId')}", vertex_id, str(record.get("agentId") or ""), str(record.get("event") or "AUDIT"), now, OS_DID, 2),
            )
        else:
...
def task_os_agent_spawn(appId: str = "", name: str = "", config: Any = None, **_: Any) -> dict[str, Any]:
    ...
    ts = _now()
    _insert("app.etzhayyim.apps.os.agent", {
...
def _agent_event(agentId: str, event: str, **extra: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.agentEvent", {"agentId": agentId, "event": event, **extra, "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label=event)
...
def task_os_agent_list(**_: Any) -> dict[str, Any]:
    return {"agents": _list("app.etzhayyim.apps.os.agent", limit=100)}
...
def task_os_consent_submit(agentDid: str = "", action: str = "", riskTier: str = "", estimatedCost: Any = 0, context: Any = None, **_: Any) -> dict[str, Any]:
    ...
    _insert("app.etzhayyim.apps.os.consentRequest", {
...
def task_os_consent_approve(requestId: str = "", **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.consentResponse", {"requestId": requestId, "verdict": "approved", "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label="approved")
    return {"requestId": requestId, "verdict": "approved"}


def task_os_consent_deny(requestId: str = "", reason: str = "", **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.consentResponse", {"requestId": requestId, "verdict": "denied", "reason": reason, "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label="denied")
    return {"requestId": requestId, "verdict": "denied"}


def task_os_consent_pending(**_: Any) -> dict[str, Any]:
    return {"pending": _filter("app.etzhayyim.apps.os.consentRequest", lambda r: r.get("status") == "pending", limit=50)}


def task_os_budget_allocate(agentId: str = "", amount: Any = 0, expiresAt: str = "", **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.budgetAllocation", {"agentId": agentId, "amount": amount, "expiresAt": expiresAt, "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label=agentId)
    return {"agentId": agentId, "allocated": amount}


def task_os_budget_balance(agentId: str = "", **_: Any) -> dict[str, Any]:
    rows = _filter("app.etzhayyim.apps.os.budgetAllocation", lambda r: str(r.get("agentId") or "") == agentId, limit=500)
...
def task_os_directory_search(tags: Any = None, limit: Any = 50, **_: Any) -> dict[str, Any]:
    ...
    rows = _list("app.etzhayyim.apps.os.directoryEntry", limit=500)
...
def task_os_directory_register(did: str = "", name: str = "", tags: Any = None, **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.directoryEntry", {"did": did, "name": name, "tags": json.dumps(tags if isinstance(tags, list) else []), "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label=name)
    return {"did": did, "registered": True}


def task_os_audit_trail(agentId: str = "", limit: Any = 50, **_: Any) -> dict[str, Any]:
    rows = _filter("app.etzhayyim.apps.os.auditEntry", lambda r: str(r.get("agentId") or "") == agentId, limit=int(limit or 50))
    return {"trail": rows}


def task_os_sync_push(path: str = "", data: Any = "", **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.syncEvent", {"direction": "push", "path": path, "dataSize": len(str(data)), "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label=path)
    return {"path": path, "direction": "push", "status": "synced"}


def task_os_sync_pull(path: str = "", **_: Any) -> dict[str, Any]:
    rows = _filter("app.etzhayyim.apps.os.syncEvent", lambda r: r.get("path") == path and r.get("direction") == "push", limit=1)
...
def task_os_window_open(appId: str = "", title: str = "", contentType: str = "", contentUrl: str = "", **_: Any) -> dict[str, Any]:
    window_id = _id("win")
    _insert("app.etzhayyim.apps.os.windowEvent", {"windowId": window_id, "event": "open", "appId": appId, "title": title, "contentType": contentType, "contentUrl": contentUrl, "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label=title)
    return {"windowId": window_id, "status": "opened"}


def task_os_window_close(windowId: str = "", **_: Any) -> dict[str, Any]:
    _insert("app.etzhayyim.apps.os.windowEvent", {"windowId": windowId, "event": "close", "org_id": "anon", "user_id": "anon", "actor_id": APP_ID, "created_at": _now()}, label="close")
...
def register(worker: Any, *, timeout_ms: int = 60_000) -> None:
    tasks = {
        "xrpc.app.etzhayyim.apps.os.agentList": task_os_agent_list,
        "xrpc.app.etzhayyim.apps.os.agentMigrate": task_os_agent_migrate,
        "xrpc.app.etzhayyim.apps.os.agentPause": task_os_agent_pause,
        "xrpc.app.etzhayyim.apps.os.agentResume": task_os_agent_resume,
        "xrpc.app.etzhayyim.apps.os.agentSpawn": task_os_agent_spawn,
        "xrpc.app.etzhayyim.apps.os.agentStop": task_os_agent_stop,
        "xrpc.app.etzhayyim.apps.os.auditTrail": task_os_audit_trail,
        "xrpc.app.etzhayyim.apps.os.budgetAllocate": task_os_budget_allocate,
        "xrpc.app.etzhayyim.apps.os.budgetBalance": task_os_budget_balance,
        "xrpc.app.etzhayyim.apps.os.consentApprove": task_os_consent_approve,
        "xrpc.app.etzhayyim.apps.os.consentDeny": task_os_consent_deny,
        "xrpc.app.etzhayyim.apps.os.consentPending": task_os_consent_pending,
        "xrpc.app.etzhayyim.apps.os.consentSubmit": task_os_consent_submit,
        "xrpc.app.etzhayyim.apps.os.directoryRegister": task_os_directory_register,
        "xrpc.app.etzhayyim.apps.os.directorySearch": task_os_directory_search,
        "xrpc.app.etzhayyim.apps.os.health": task_os_health,
        "xrpc.app.etzhayyim.apps.os.syncPull": task_os_sync_pull,
        "xrpc.app.etzhayyim.apps.os.syncPush": task_os_sync_push,
        "xrpc.app.etzhayyim.apps.os.windowClose": task_os_window_close,
        "xrpc.app.etzhayyim.apps.os.windowOpen": task_os_window_open,
    }
