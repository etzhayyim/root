...
def _patent_vid(patent_number: str) -> str:
    return f"at://{OWNER_DID}/ai.gftd.apps.openPatent.patent/US-{patent_number}"


def _citation_vid(citing: str, cited: str, seq: str) -> str:
    raw = f"{citing}\x1f{cited}\x1f{seq}"
    h = hashlib.sha256(raw.encode()).hexdigest()[:20]
    return f"at://{OWNER_DID}/ai.gftd.apps.openPatent.citation/{h}"
...
def _blob_vid(patent_number: str) -> str:
    return f"at://{OWNER_DID}/ai.gftd.apps.patentBlob.doc/US-{patent_number}"


def _foreign_patent_vid(cc: str, number: str) -> str:
    return f"at://{OWNER_DID}/ai.gftd.apps.openPatent.patent/{cc}-{number}"
...
def _stream_tsv_zip(
    ...
    """
    req = urllib.request.Request(url, headers={"User-Agent": "pymagatama/patent-ingest"})
