    ...
    for c in components:
        ...
        component_rows.append(
            (
                ...
                _pick_property(c, "cdx:gftd:vehicle:supplier_mpn"),
                _pick_property(c, "cdx:gftd:vehicle:parent"),
...
def register(worker: Any, *, timeout_ms: int = 600_000) -> None:
    ...
    worker.task(
        task_type="xrpc.ai.gftd.apps.sbom.registerArtifact",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_sbom_register_artifact)
    worker.task(
        task_type="xrpc.ai.gftd.apps.sbom.runVulnMatch",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_sbom_run_vuln_match)
    worker.task(
        task_type="xrpc.ai.gftd.apps.sbom.recall",
        single_value=False,
        timeout_ms=timeout_ms,
    )(task_sbom_recall)
    worker.task(
        task_type="xrpc.ai.gftd.apps.sbom.cveIngestOsv",
        single_value=False,
        timeout_ms=timeout_ms,
