...
# training pod's HTTP server (`pymagatama.training_http_server`).
...
def _delegate_to_runpod(kind: str, payload: dict) -> dict:
    ...
    `pymagatama.training_http_server` → `runpod_handler(event)` which:
...
def runpod_handler(event: dict) -> dict:
    """Entry point invoked from `pymagatama.runpod_trainer_handler`.
