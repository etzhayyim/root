...
`@udf` decorators in `pymagatama.handlers.*`) and binds gRPC/arrow-flight
...
from pymagatama.registry import registered
...
def serve(
    ...
    generated from `pymagatama.handlers.__init__.py` which eagerly imports
    every actor module.
    """
    ...
    # handler package.
    if preload is None:
        import pymagatama.handlers  # noqa: F401 — side-effect: register handlers
    else:
        import importlib
    ...
    # pymagatama.registry (name=nsid). The NSID becomes the remote_udf name.
    ...
    if start_http_server and Gauge:
        ...
        g = Gauge("pymagatama_handlers_registered", "Count of registered NSIDs")
        g.set(len(entries))
    ...
    log.info(
        "pymagatama serving arrow-flight on %s:%d with %d handlers",
        host,
        port,
        len(entries),
    )
