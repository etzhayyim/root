"""Generic-primitive worker for ai.gftd.tools.const.* (ADR-2605082000 §2.6 follow-up).

Hosts identity / no-op tools that let LangGraph topology nodes stay data-only
without per-actor Python code. Today's surface:

  ai.gftd.tools.const.echo({"constant": {...}}) → {...}

The dispatcher convention (`pymagatama.{actor}_worker_main:task_{snake}`) does
not apply here — the namespace is `ai.gftd.tools.const`, not
`ai.gftd.apps.<actor>`. Wire-up uses ``register_overrides`` in mcp_dispatch.
"""
...
async def task_echo(*, constant: Any | None = None, **_ignored: Any) -> dict[str, Any]:
    ...
    """
    if constant is None:
        return {"error": "ai.gftd.tools.const.echo: 'constant' is required"}
    if not isinstance(constant, dict):
        return {"error": "ai.gftd.tools.const.echo: 'constant' must be an object"}
