...
Startup via Granian:
  granian --interface asgi --host 0.0.0.0 --port 8000 \\
          pymagatama.langgraph_server_app:app
"""
...
async def _lifespan(application: FastAPI):
    ...
    watcher_task = None
    try:
        from pymagatama.langgraph_watcher import watch_forever
        import os as _os
...
async def registry_source():
    ...
    try:
        from pymagatama.langgraph_watcher import STATS as _watcher_stats
...
def _register_shosha_refresh_sanctions_list() -> None:
    ...
    from pymagatama.langgraph_graphs.shosha_refresh_sanctions_list import build_graph
...
def _register_gameya_quality_loop() -> None:
    ...
    from pymagatama.langgraph_graphs.gameya_quality_loop import build_graph
...
def _register_tsukuru_isic_pulse() -> None:
    ...
    from pymagatama.langgraph_graphs.tsukuru_isic_pulse import build_graph
...
def _register_open_isic_classify_entity() -> None:
    ...
    from pymagatama.langgraph_graphs.open_isic_classify_entity import build_graph
...
def _register_open_isic_hierarchical_classify() -> None:
    ...
    from pymagatama.langgraph_graphs.open_isic_hierarchical_classify import build_graph
...
def _register_os_messaging_crawl_open_channels() -> None:
    ...
    from pymagatama.langgraph_graphs.os_messaging_crawl_open_channels import build_graph
...
def _register_site_common_crawl_ingest() -> None:
    ...
    from pymagatama.langgraph_graphs.site_common_crawl_ingest import build_graph
...
def _register_onion_crawl_seeds() -> None:
    ...
    from pymagatama.langgraph_graphs.onion_crawl_seeds import build_graph
...
def _register_public_malak_crawl_ads() -> None:
    ...
    from pymagatama.langgraph_graphs.public_malak_crawl_ads import build_graph
...
def _register_ransomware_actor_activity() -> None:
    ...
    from pymagatama.langgraph_graphs.ransomware_actor_activity import build_graph
...
def _register_agent_runtime_lease_autopilot() -> None:
    ...
    from pymagatama.langgraph_graphs.agent_runtime_lease_autopilot import build_graph
...
def _register_coverage_gap_bridge() -> None:
    ...
    from pymagatama.langgraph_graphs.coverage_gap_bridge import build_graph
...
def _register_aria_graph(node: str) -> None:
    ...
    mod = importlib.import_module(f"pymagatama.langgraph_graphs.aria_{node}")
...
def _register_adsk_ingest_dataset() -> None:
    ...
    from pymagatama.langgraph_graphs.adsk_ingest_dataset import build_graph
...
def _register_isbn_ingest_aozora() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_aozora import build_graph
...
def _register_isbn_ingest_gutenberg() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_gutenberg import build_graph
...
def _register_isbn_ingest_ndl() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_ndl import build_graph
...
def _register_ndl_image_ocr_ingest() -> None:
    ...
    from pymagatama.langgraph_graphs.ndl_image_ocr_ingest import build_graph
...
def _register_biblio_open_data_ingest() -> None:
    ...
    from pymagatama.langgraph_graphs.biblio_open_data_ingest import build_graph
...
def _register_isbn_ingest_hathitrust() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_hathitrust import build_graph
...
def _register_isbn_ingest_open_library() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_open_library import build_graph
...
def _register_isbn_ingest_internet_archive() -> None:
    ...
    from pymagatama.langgraph_graphs.isbn_ingest_internet_archive import build_graph
...
def _register_yoro_product_ingest() -> None:
    ...
    from pymagatama.langgraph_graphs.yoro_product_ingest import build_graph
...
def _register_shinshi_seed_gap_fill() -> None:
    ...
    from pymagatama.langgraph_graphs.shinshi_seed_gap_fill import build_graph
...
def _register_yoro_platform_pulse() -> None:
    ...
    from pymagatama.langgraph_graphs.yoro_platform_pulse import build_graph
...
def _register_animeka_autopilot() -> None:
    ...
    from pymagatama.langgraph_graphs.animeka_autopilot import build_graph
...
def _register_shinka_cron_tick() -> None:
    ...
    from pymagatama.langgraph_graphs.shinka_cron_tick import build_graph
...
def _register_wellbecoming_process_mining() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_process_mining import build_graph
...
def _register_wellbecoming_detect_bottleneck() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_detect_bottleneck import build_graph
...
def _register_wellbecoming_proactive_connect() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_proactive_connect import build_graph
...
def _register_wellbecoming_floor_violation_alert() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_floor_violation_alert import build_graph
...
def _register_wellbecoming_minimax_sweep() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_minimax_sweep import build_graph
...
def _register_wellbecoming_belief_influence_propagate() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_belief_influence_propagate import build_graph
...
def _register_wellbecoming_belief_noise_inject() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_belief_noise_inject import build_graph
...
def _register_wellbecoming_belief_restoring_capture() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_belief_restoring_capture import build_graph
...
def _register_wellbecoming_trust_weight_update() -> None:
    ...
    from pymagatama.langgraph_graphs.wellbecoming_trust_weight_update import build_graph
...
def _register_ki_synthesis() -> None:
    ...
    from pymagatama.primitives.ki_synthesis_graph import _build_graph
...
def _register_ki_cycle() -> None:
    ...
    from pymagatama.langgraph_graphs.ki_cycle import build_graph
...
def _register_saikin_cycle() -> None:
    ...
    from pymagatama.langgraph_graphs.saikin_cycle import build_graph
...
def _register_newsletter_send_campaign() -> None:
    ...
    from pymagatama.newsletter_worker_main import build_newsletter_graph
...
def _register_koke_cycle() -> None:
    ...
    from pymagatama.langgraph_graphs.koke_cycle import build_graph
...
def _register_organism_single_task_chains() -> None:
    ...
    from pymagatama.langgraph_graphs._single_task_wrapper import build_single_task_graph
    from pymagatama.kobo_worker_main import (
        task_bud_agent,
        task_sporulate,
        task_germinate,
    )
...
def _register_shosha_market_intelligence() -> None:
    ...
    from pymagatama.langgraph_graphs.shosha_market_intelligence import build_graph
...
def _register_pregel_email_triage() -> None:
    ...
    from pymagatama.pregel.graph import build_graph
