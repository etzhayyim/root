...
def _envelope_content(state: dict, envelope_key: str, legacy_key: str) -> str:
    ...
    upstream node is mcp_tool ai.gftd.tools.llm.chat) and fall back to
...
def generate_scene_text(state: AnimakaAutopilotState) -> dict:
    ...
    from pymagatama.zeebe_worker_main import task_generic_llm_chat
    ...
    if sb_cid:
        try:
            asyncio.run(task_generic_db_insert(
                ...
                values={
                    "vertex_id": f"at://{_REPO}/ai.gftd.apps.animeka.storyboard/{cut_id}-c1",
                    "repo": _REPO,
                    "collection": "ai.gftd.apps.animeka.storyboard",
                    "rkey": f"{cut_id}-c1",
                    "blob_cid": sb_cid,
    ...
    if sb_cid:
        try:
            asyncio.run(task_generic_db_insert(
                table="vertex_animeka",
                values={
                    "vertex_id": f"at://{_REPO}/ai.gftd.apps.animeka.storyboard/{cut_id}-c2",
                    "repo": _REPO,
                    "collection": "ai.gftd.apps.animeka.storyboard",
                    "rkey": f"{cut_id}-c2",
                    "blob_cid": sb_cid,
    ...
    if ly_cid:
        try:
            asyncio.run(task_generic_db_insert(
                table="vertex_animeka",
                values={
                    "vertex_id": f"at://{_REPO}/ai.gftd.apps.animeka.layout/{cut_id}",
                    "repo": _REPO,
                    "collection": "ai.gftd.apps.animeka.layout",
                    "rkey": cut_id,
                    "blob_cid": ly_cid,
                    "cut_summary": cut_summary,
...
def generate_keyframe(state: AnimakaAutopilotState) -> dict:
    ...
    from pymagatama.zeebe_worker_main import task_generic_db_insert
    ...
    if kf_cid:
        try:
            asyncio.run(task_generic_db_insert(
                table="vertex_animeka",
                values={
                    "vertex_id": f"at://{_REPO}/ai.gftd.apps.animeka.keyframe/{cut_id}-f1",
                    "repo": _REPO,
                    "collection": "ai.gftd.apps.animeka.keyframe",
                    "rkey": f"{cut_id}-f1",
                    "blob_cid": kf_cid,
                    "cut_summary": cut_summary,
                    "frame_num": 1,
    ...
    if bg_cid:
        try:
            asyncio.run(task_generic_db_insert(
                table="vertex_animeka",
                values={
                    "vertex_id": f"at://{_REPO}/ai.gftd.apps.animeka.background/{cut_id}",
                    "repo": _REPO,
                    "collection": "ai.gftd.apps.animeka.background",
                    "rkey": cut_id,
                    "blob_cid": bg_cid,
                    "bg_cid": bg_cid,
                    "cut_summary": cut_summary,
...
def emit_audit(state: AnimakaAutopilotState) -> dict:
    ...
    from pymagatama.zeebe_worker_main import task_generic_audit_emit
