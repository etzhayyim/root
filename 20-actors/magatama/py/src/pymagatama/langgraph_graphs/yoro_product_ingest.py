...
def _fanout_one(retailer: str, query: str, max_items: int) -> list[dict]:
    from pymagatama.primitives.yoro_product import fetch_one
...
def write_research(state: YoroProductIngestState) -> dict:
    ...
    row = {
        "vertex_id": vertex_id,
        "actor_did": actor_did,
        "org_did": os.environ.get("YORO_ORG_DID", "did:erc725:gftd:260425:amanomibashira"),
        "at_did": actor_did,
        "created_at": now_iso,
        "query": state["query"],
...
def emit_audit(state: YoroProductIngestState) -> dict:
    try:
        from pymagatama.primitives.audit import emit_audit_event  # type: ignore
