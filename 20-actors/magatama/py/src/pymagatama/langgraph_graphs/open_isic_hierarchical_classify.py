...
options via the MCP tool `ai.gftd.apps.openIsic.getTaxonomy` and using the LLM
...
async def fetch_taxonomy(state: HierarchicalClassifyState) -> dict:
    ...
    envelope = {
        ...
        "method": "tools/call",
        "params": {
            "name": "ai.gftd.apps.openIsic.getTaxonomy",
            "arguments": {
                "level": level,
...
async def predict_level(state: HierarchicalClassifyState) -> dict:
    ...
    # In a real environment, this node dispatches an MCP call to `ai.gftd.tools.llm.chat`.
...
async def save_classification(state: HierarchicalClassifyState) -> dict:
    ...
    from pymagatama.langgraph_graphs.open_isic_classify_entity import call_mcp_tool
