...
def scan_upstream(state: ShoshaReactUpstreamState) -> dict:
    ...
    from pymagatama.primitives.shosha import task_shosha_reactive_scan_upstream
        ...
        with sync_cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    "com.etzhayyim.apps.shosha.reactToUpstream",
                    f"lg-{int(_time.time() * 1000)}",
                    "create",
                    int(_time.time() * 1000),
                    f'{{"reactionsEmitted":{state.get("reactionsEmitted",0)},'
                    f'"recordsScanned":{state.get("recordsScanned",0)},'
