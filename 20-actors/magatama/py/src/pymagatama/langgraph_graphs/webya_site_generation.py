"""
webya.gftd.ai — サイト生成 LangGraph StateGraph (ADR-2605080600 + ADR-2605072000).
...
def publisher(state: WebsiteGenerationState) -> WebsiteGenerationState:
    ...
    try:
        for slug, page in state.pages.items():
            ...
            vertex_id = f"at://did:web:webya.gftd.ai/ai.gftd.apps.webya.page/{page_id}"
            ...
            # edge: site → page
            edge_vid = f"at://did:web:webya.gftd.ai/ai.gftd.apps.webya.edge/site-page-{page_id}"
            site_vid = f"at://did:web:webya.gftd.ai/ai.gftd.apps.webya.site/{site_id}"
            _rw_execute(
