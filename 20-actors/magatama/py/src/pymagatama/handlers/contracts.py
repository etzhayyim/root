...
Phase 1 surface:
- `ai.gftd.apps.contracts.mintOrganizationDid`      (procedure, DB-only)
- `ai.gftd.apps.contracts.projectFromLegalEntity`    (procedure, batch / per-row)
- `ai.gftd.apps.contracts.ingestSocialContract`      (procedure, fresh HTTP crawl)
...
async def _insert_organization_row(pool: Any, row: dict[str, Any]) -> bool:
    ...
    vertex_id = f"at://{did}/ai.gftd.apps.contracts.organization/{rkey}"
...
@udf(
    nsid="ai.gftd.apps.contracts.mintOrganizationDid",
    io_threads=50,
    input_types=["VARCHAR"],
    result_type="VARCHAR",
...
async def mint_organization_did(params_json: str) -> str:
    ...
    ctx = Context(nsid="ai.gftd.apps.contracts.mintOrganizationDid")
...
@udf(
    nsid="ai.gftd.apps.contracts.projectFromLegalEntity",
    io_threads=100,
    input_types=["VARCHAR"],
    result_type="VARCHAR",
...
async def project_from_legal_entity(params_json: str) -> str:
    ...
    ctx = Context(nsid="ai.gftd.apps.contracts.projectFromLegalEntity")
    scanned = 0
    inserted = 0
    skipped = 0
    dids: list[str] = []
...
async def _insert_social_contract_row(pool: Any, row: dict[str, Any]) -> bool:
    ...
    vertex_id = f"at://{ACTOR_DID}/ai.gftd.apps.contracts.socialContract/{rkey}"
    now_iso = datetime.now(timezone.utc).isoformat()
    result = await pool.fetchval(
        _INSERT_SOCIAL_CONTRACT,
        vertex_id,  # $1
...
@udf(
    nsid="ai.gftd.apps.contracts.ingestSocialContract",
    io_threads=100,
    input_types=["VARCHAR"],
    result_type="VARCHAR",
...
async def ingest_social_contract(params_json: str) -> str:
    ...
    ctx = Context(nsid="ai.gftd.apps.contracts.ingestSocialContract")
...
@udf(
    nsid="ai.gftd.apps.contracts.resolveOrganization",
    io_threads=50,
    input_types=["VARCHAR"],
    result_type="VARCHAR",
...
async def resolve_organization(params_json: str) -> str:
    ...
    ctx = Context(nsid="ai.gftd.apps.contracts.resolveOrganization")
