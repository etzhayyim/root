...
`pymagatama` LangGraph stack so it can deploy as a Murakumo Tier-1
`lg-ameno` pod next to the existing `lg-uhl-right-neural` shape.

ADR: [`90-docs/adr/2605191257-ameno-daemon-path-b-pymagatama-python.md`](../../../../../../../90-docs/adr/2605191257-ameno-daemon-path-b-pymagatama-python.md).
...
- `pymagatama` installed (this repo's `20-actors/magatama/py` package)
...
python -m pymagatama.projects.ameno
```
...
| `LOCAL_LLM_MODEL` | `qwen3:14b` *(pymagatama default)* | model name; recommend `gemma3:4b` for parity with browser |
...
- **`pymagatama.ameno_handlers`** — receive-side XRPC saveResult / listHistory persistence (ADR-2605111200). Independent from this project.
