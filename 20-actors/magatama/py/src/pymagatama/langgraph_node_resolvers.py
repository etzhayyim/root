  ...
  mcp_tool      POST to an MCP endpoint with body = MCP ``tools/call`` envelope.
                ``ref`` can be one of:
                  ...
                  - ``mcp://<nsid>`` (registry-resolved per ADR-2605082000 §2.6 —
                    ...
                    ``https://{actor_host}/xrpc/ai.gftd.mcp.message``;
...
_MCP_ENVELOPE_PATH = "/xrpc/ai.gftd.mcp.message"
...
def _read_mcp_nsid_overrides() -> list[tuple[str, str]]:
    ...
    so the topology Pregel's `mcp://ai.gftd.apps.mangaka.tools.*` calls
...
async def _resolve_mcp_nsid(
    ...
    Returns the full POST URL (`https://{actor_host}/xrpc/ai.gftd.mcp.message`).
...
def make_mcp_tool_node(
    ...
    *,
    ...
    ``ai.gftd.tools.json.extract``: ``a.b[2].c`` / ``a.*``).
    """
    cfg = _parse_config(config)
    input_keys = list(cfg.get("input_keys") or [])
    ...
    # Use case: identity / no-op tools (ai.gftd.tools.const.echo) where the
    ...
    async def _node(state: dict) -> dict:
        ...
        # navigator as ai.gftd.tools.json.extract so the path grammar is
        # identical across the resolver and the JSON tool.
        if input_paths:
            try:
                from pymagatama.tools_json_worker_main import _parse_path, _walk
            except Exception:
...
def make_llm_node(ref: str, config: Any) -> NodeFn:
    ...
    ref = tier name (passed to pymagatama.llm.call_tier_json's ``tier`` arg —
...
def resolve_node(
    ...
    *,
    ...
    `kind="llm_vision"` to materialise `image_keys` into bytes. pymagatama
...
def make_llm_vision_node(
    ...
    *,
    ...
    ``ref``     model id / tier name (forwarded to
                ``pymagatama.llm.call_tier_vision_json`` — common values:
    ...
    async def _node(state: dict) -> dict:
        ...
        # monkeypatch `pymagatama.llm.call_tier_vision_json` before this
        # closure resolves it.
        from pymagatama.llm import call_tier_vision_json
...
def _parse_dmn_ref(ref: str) -> tuple[str, int]:
    """``dmn:ai.gftd.policies.foo.bar@1.0.0`` → (``ai.gftd.policies.foo.bar``, 1).
