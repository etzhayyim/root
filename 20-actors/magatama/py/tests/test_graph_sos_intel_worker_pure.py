...
def test_gen_id_prefix():
    from pymagatama.graph_sos_intel_worker_main import _gen_id
...
def test_now_is_iso():
    from pymagatama.graph_sos_intel_worker_main import _now
...
def test_inventory_catalog_counts():
    ...
    with patch(
        "pymagatama.graph_sos_intel_worker_main.fetch_all",
        side_effect=[mock_relations, mock_indexes],
    ):
        from pymagatama.graph_sos_intel_worker_main import task_inventory_catalog
...
def test_detect_findings_no_prev():
    ...
    with (
        patch("pymagatama.graph_sos_intel_worker_main.fetch_one", return_value=None),
        patch("pymagatama.graph_sos_intel_worker_main.sync_cursor"),
    ):
        from pymagatama.graph_sos_intel_worker_main import task_detect_findings
...
def test_detect_findings_large_delta():
    ...
    with (
        patch(
            "pymagatama.graph_sos_intel_worker_main.fetch_one",
            return_value=prev_row,
        ),
        patch(
            "pymagatama.graph_sos_intel_worker_main.sync_cursor",
...
def test_query_latest_snapshot_not_found():
    with patch("pymagatama.graph_sos_intel_worker_main.fetch_one", return_value=None):
        from pymagatama.graph_sos_intel_worker_main import task_query_latest_snapshot
...
def test_query_latest_snapshot_found():
    ...
    with patch("pymagatama.graph_sos_intel_worker_main.fetch_one", return_value=row):
        from pymagatama.graph_sos_intel_worker_main import task_query_latest_snapshot
...
def test_generate_briefing_no_snapshot():
    from pymagatama.graph_sos_intel_worker_main import task_generate_briefing
...
def test_generate_briefing_with_snapshot():
    ...
    with patch("pymagatama.llm.call_tier", mock_llm):
        from pymagatama.graph_sos_intel_worker_main import task_generate_briefing
...
def test_write_finding_empty_briefing():
    from pymagatama.graph_sos_intel_worker_main import task_write_finding
