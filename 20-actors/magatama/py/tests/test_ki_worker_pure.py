...
def test_uid_prefix():
    from pymagatama.ki_worker_main import _uid
...
def test_now_iso():
    from pymagatama.ki_worker_main import _now
...
def test_absorb_scan_empty():
    ...
    with patch("pymagatama.ki_worker_main.fetch_one", return_value=None):
...
def test_absorb_scan_returns_pending():
    ...
    row = ("at://ki.gftd.ai/ai.gftd.apps.ki.absorb/xyl-deadbeef",)
...
def test_absorb_by_content():
    ...
    with patch("pymagatama.ki_worker_main.sync_cursor", return_value=mock_cm):
        from pymagatama.ki_worker_main import task_absorb

        result = _run(task_absorb(
            sourceVertexId="at://koke.gftd.ai/ai.gftd.apps.koke.fixation/kox-abc",
            inputKind="text",
...
def test_synthesize_no_input():
    from pymagatama.ki_worker_main import task_synthesize
...
def test_synthesize_valid_langgraph_response():
    ...
    with (
        patch("pymagatama.primitives.ki_synthesis_graph.synthesize", mock_graph),
        patch("pymagatama.ki_worker_main.sync_cursor", return_value=mock_cm),
        patch("pymagatama.ki_worker_main.fetch_one", return_value=None),
    ):
        from pymagatama.ki_worker_main import task_synthesize
...
def test_bloom_no_artifact_id():
    from pymagatama.ki_worker_main import task_bloom
...
def test_bloom_low_confidence_skips():
    ...
    row = ("at://ki.gftd.ai/ai.gftd.apps.ki.artifact/art-abc", 0.3)
...
def test_bloom_success():
    ...
    row = ("at://ki.gftd.ai/ai.gftd.apps.ki.artifact/art-abc", 0.85)
...
def test_ring_creates_checkpoint():
    ...
    with (
        patch("pymagatama.ki_worker_main.fetch_one", return_value=(42,)),
        patch("pymagatama.ki_worker_main.sync_cursor", return_value=mock_cm),
    ):
        from pymagatama.ki_worker_main import task_ring
...
def test_ring_default_period():
    ...
    with (
        patch("pymagatama.ki_worker_main.fetch_one", return_value=(0,)),
        patch("pymagatama.ki_worker_main.sync_cursor", return_value=mock_cm),
    ):
        from pymagatama.ki_worker_main import task_ring
