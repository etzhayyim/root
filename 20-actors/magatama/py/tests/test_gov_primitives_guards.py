...
test file loads the real pymagatama.db_sync (which needs RW_URL), making any
...
# pymagatama package stub (needed for spec_from_file_location loads)
if "pymagatama" not in sys.modules:
    _pkg = types.ModuleType("pymagatama")
    _pkg.__path__ = [str(_py_src / "pymagatama")]  # type: ignore[attr-defined]
    _pkg.__package__ = "pymagatama"
    sys.modules["pymagatama"] = _pkg


# ── discover ISO3 codes ─────────────────────────────────────────────────────
_GOV_DIR = _py_src / "pymagatama" / "primitives"
_ISO3_CODES = sorted(
    f.stem.split("_", 1)[1]
...
def _load_gov(iso3: str):
    ...
    # Temporarily inject stub so the import-time `from pymagatama.db_sync import
    ...
    try:
        ...
        elif "pymagatama.db_sync" in sys.modules and sys.modules["pymagatama.db_sync"] is _db_stub:
            pass  # stub is fine
    return mod
