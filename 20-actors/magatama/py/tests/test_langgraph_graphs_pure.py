...
class TestShoshaAgentLoopGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import build_graph
        graph = build_graph()
    ...
    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import ShoshaAgentState
        import typing
    ...
    def test_fetch_context_returns_empty_on_db_error(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import fetch_context
        with patch("pymagatama.langgraph_graphs.shosha_agent_loop._rw_query", return_value=[]):
    ...
    def test_fetch_context_formats_rows(self):
        ...
        with patch("pymagatama.langgraph_graphs.shosha_agent_loop._rw_query", side_effect=fake_query):
            result = fetch_context({"prompt": "test"})
    ...
    def test_call_llm_returns_error_on_empty_prompt(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import call_llm
    ...
    def test_call_llm_strips_think_blocks(self):
        ...
        with patch("pymagatama.langgraph_graphs.shosha_agent_loop.llm") as mock_llm:
    ...
    def test_emit_audit_is_nonfatal(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import emit_audit
        with patch("pymagatama.langgraph_graphs.shosha_agent_loop.sync_cursor") as mock_cur:
    ...
    def test_strip_think_partial_block(self):
        from pymagatama.langgraph_graphs.shosha_agent_loop import _strip_think
...
class TestWebmkProposalGraph:
    def test_build_graph_compiles(self):
        try:
            from pymagatama.langgraph_graphs.webmk_proposal import build_graph
            graph = build_graph()
            assert graph is not None
        except ImportError:
...
class TestShoshaReactUpstreamGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.shosha_react_upstream import build_graph
        graph = build_graph()
        assert graph is not None

    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.shosha_react_upstream import ShoshaReactUpstreamState
        import typing
    ...
    def test_emit_audit_nonfatal_on_db_error(self):
        from pymagatama.langgraph_graphs.shosha_react_upstream import emit_audit
        from unittest.mock import patch
        with patch("pymagatama.langgraph_graphs.shosha_react_upstream.sync_cursor") as m:
...
class TestShoshaTradeBookRecomputeGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.shosha_trade_book_recompute import build_graph
        graph = build_graph()
        assert graph is not None

    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.shosha_trade_book_recompute import ShoshaTradeBookState
        import typing
    ...
    def test_emit_audit_nonfatal_on_db_error(self):
        from pymagatama.langgraph_graphs.shosha_trade_book_recompute import emit_audit
        from unittest.mock import patch
        with patch("pymagatama.langgraph_graphs.shosha_trade_book_recompute.sync_cursor") as m:
            m.side_effect = RuntimeError("DB down")
...
class TestShoshaTradeIdeaSynthesizeGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.shosha_trade_idea_synthesize import build_graph
        graph = build_graph()
        assert graph is not None

    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.shosha_trade_idea_synthesize import ShoshaTradeIdeaState
        import typing
    ...
    def test_emit_audit_nonfatal_on_db_error(self):
        from pymagatama.langgraph_graphs.shosha_trade_idea_synthesize import emit_audit
        from unittest.mock import patch
        with patch("pymagatama.langgraph_graphs.shosha_trade_idea_synthesize.sync_cursor") as m:
            m.side_effect = RuntimeError("DB down")
            result = emit_audit({"ok": True})
        assert result == {}
...
class TestShoshaDailyReportGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.shosha_daily_report import build_graph
        graph = build_graph()
        assert graph is not None

    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.shosha_daily_report import ShoshaDailyReportState
        import typing
    ...
    def test_emit_audit_nonfatal_on_db_error(self):
        from pymagatama.langgraph_graphs.shosha_daily_report import emit_audit
        from unittest.mock import patch
        with patch("pymagatama.langgraph_graphs.shosha_daily_report.sync_cursor") as m:
            m.side_effect = RuntimeError("DB down")
...
class TestCopyrightIngestGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.copyright_ingest import build_graph
        graph = build_graph()
        assert graph is not None
    ...
    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.copyright_ingest import CopyrightIngestState
        import typing
    ...
    def test_doi_rkey_replaces_slash(self):
        from pymagatama.langgraph_graphs.copyright_ingest import _doi_rkey
    ...
    def test_crossref_row_returns_none_without_doi(self):
        from pymagatama.langgraph_graphs.copyright_ingest import _crossref_row
    ...
    def test_datacite_row_returns_none_without_doi(self):
        from pymagatama.langgraph_graphs.copyright_ingest import _datacite_row
    ...
    def test_fetch_crossref_on_http_error_returns_empty(self):
        from pymagatama.langgraph_graphs.copyright_ingest import fetch_crossref
        import httpx
        with patch("pymagatama.langgraph_graphs.copyright_ingest.httpx.Client") as mock_client:
    ...
    def test_fetch_datacite_on_http_error_returns_empty(self):
        from pymagatama.langgraph_graphs.copyright_ingest import fetch_datacite
        import httpx
        with patch("pymagatama.langgraph_graphs.copyright_ingest.httpx.Client") as mock_client:
            mock_client.return_value.__enter__.return_value.get.side_effect = httpx.TimeoutException("timeout")
    ...
    def test_insert_crossref_skips_items_without_doi(self):
        from pymagatama.langgraph_graphs.copyright_ingest import insert_crossref
        state = {"crossrefItems": [{"title": ["No DOI here"]}]}
        with patch("pymagatama.langgraph_graphs.copyright_ingest._bulk_insert_vertex_work", return_value=0) as mock_insert:
    ...
    def test_insert_crossref_passes_valid_rows(self):
        ...
        with patch("pymagatama.langgraph_graphs.copyright_ingest._bulk_insert_vertex_work", return_value=1) as mock_insert:
            result = insert_crossref(state)
    ...
    def test_insert_datacite_sets_ok_true(self):
        from pymagatama.langgraph_graphs.copyright_ingest import insert_datacite
        state = {"dataciteItems": [{"id": "10.8/dc", "attributes": {"titles": [{"title": "T"}]}}]}
        with patch("pymagatama.langgraph_graphs.copyright_ingest._bulk_insert_vertex_work", return_value=1):
    ...
    def test_insert_datacite_sets_ok_false_on_error(self):
        ...
        with patch("pymagatama.langgraph_graphs.copyright_ingest._bulk_insert_vertex_work", side_effect=RuntimeError("db fail")):
            result = insert_datacite(state)
        assert result["ok"] is False
    ...
    def test_emit_audit_is_nonfatal(self):
        from pymagatama.langgraph_graphs.copyright_ingest import emit_audit
        with patch("pymagatama.langgraph_graphs.copyright_ingest.sync_cursor") as mock_cur:
            mock_cur.side_effect = RuntimeError("DB down")
...
class TestCopyrightFulltextGraph:
    def test_build_graph_compiles(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import build_graph
        graph = build_graph()
        assert graph is not None

    def test_state_typeddict_fields(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import CopyrightFulltextState
        import typing
    ...
    def test_query_oa_works_returns_empty_on_db_error(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import query_oa_works
        with patch("pymagatama.langgraph_graphs.copyright_fulltext.sync_cursor") as m:
            m.side_effect = RuntimeError("DB down")
    ...
    def test_fetch_fulltext_skips_non_oa_license(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import fetch_fulltext
        fake_unp = MagicMock()
        ...
        with patch("pymagatama.langgraph_graphs.copyright_fulltext.httpx.Client") as mock_client:
    ...
    def test_store_blobs_counts_stored_vs_failed(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import store_blobs
        blobs = [
        ...
        with patch("pymagatama.langgraph_graphs.copyright_fulltext.sync_cursor", return_value=mock_cur):
    ...
    def test_emit_audit_is_nonfatal(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import emit_audit
        with patch("pymagatama.langgraph_graphs.copyright_fulltext.sync_cursor") as m:
            m.side_effect = RuntimeError("DB down")
    ...
    def test_extract_text_from_html_strips_tags(self):
        from pymagatama.langgraph_graphs.copyright_fulltext import _extract_text_from_html
...
class TestWebYaSiteGenerationGraph:
    def test_build_graph_compiles(self):
        try:
            from pymagatama.langgraph_graphs.webya_site_generation import build_graph
            graph = build_graph()
            assert graph is not None
        except ImportError:
