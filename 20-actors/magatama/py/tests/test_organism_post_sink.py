"""Tests for pymagatama.organism.post_sink (ADR-2605240100)."""
...
from pymagatama.organism.inbox import InboundCommit
...
from pymagatama.organism.unispsc_organism import UnispscOrganism
...
def test_ndjson_sink_custom_lexicon(tmp_path: Path):
    queue = tmp_path / "queue.ndjson"
    sink = NdjsonQueuePostSink(queue, lexicon="ai.gftd.apps.etzhayyim.shinka.post")
    sink("x", ctx=_Ctx(), mood="neutral", content_source_kind="inbound")
    payload = json.loads(queue.read_text(encoding="utf-8").splitlines()[0])
    assert payload["lexicon"] == "ai.gftd.apps.etzhayyim.shinka.post"
...
def test_fleet_state_organisms_share_post_sink(tmp_path: Path):
    from pymagatama.organism.fleet_cell_main import FleetState
