"""Pure helper tests for os_messaging_open_channels and magatama_organizer primitives.
...
- magatama_organizer: _utc_now_iso / MAGATAMA_DID / ORGANIZER_RUN_COLLECTION constants
"""
...
from pymagatama.primitives import os_messaging_open_channels as OC
from pymagatama.primitives import magatama_organizer as MO
...
# ─── magatama_organizer — _utc_now_iso / constants ──────────────────────────
...
def test_mo_magatama_did_starts_with_did():
    assert MO.MAGATAMA_DID.startswith("did:")


def test_mo_magatama_did_contains_magatama():
    assert "magatama" in MO.MAGATAMA_DID
...
def test_mo_organizer_run_collection_starts_with_ai_gftd():
    assert MO.ORGANIZER_RUN_COLLECTION.startswith("ai.gftd.apps.")
