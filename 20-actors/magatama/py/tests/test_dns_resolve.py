...
(pymagatama.handlers.dns_resolve).
...
# Stub arrow_udf so pymagatama.registry imports cleanly without the
...
# `pymagatama.handlers` — that package's __init__.py eagerly loads
...
_src = _P(__file__).resolve().parents[1] / "src/pymagatama/handlers/dns_resolve.py"
