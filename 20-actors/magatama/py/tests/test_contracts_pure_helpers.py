...
if _MOD_NAME in sys.modules:
    ...
    try:
        from pymagatama import registry as _reg
    ...
    CO = _load_mod(_MOD_NAME, "pymagatama/handlers/contracts.py")
...
def test_row_to_resolve_dto_vertex_id_mapped_to_camel() -> None:
    dto = CO._row_to_resolve_dto(_make_resolve_row())
    assert dto["vertexId"] == "at://did:web:x/ai.gftd.apps.contracts.organization/rkey1"
...
def _make_le_row(**kwargs: object) -> dict:
    base: dict = {
        "vertex_id": "at://did:web:x/ai.gftd.apps.contracts.organization/rk1",
        "country": "JPN",
        "national_id": "12345",
        "name": "Acme Corp",
...
def test_row_to_projection_legal_entity_ref_from_vertex_id() -> None:
    result = CO._row_to_projection(_make_le_row())
    assert result["legal_entity_ref"] == "at://did:web:x/ai.gftd.apps.contracts.organization/rk1"
