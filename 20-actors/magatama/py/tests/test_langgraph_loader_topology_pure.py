...
async def test_topology_linear_compiles_and_runs():
    ...
    nodes_cursor = _cursor([
        ("echo",  "py_primitive", "pymagatama.primitives.demo_echo_chain:step_echo",  None),
        ("count", "py_primitive", "pymagatama.primitives.demo_echo_chain:step_count", None),
    ])
...
async def test_topology_conditional_routes_correctly():
    ...
    spec = json.dumps({
        ...
        "conditional_edges": [
            {
                "from": "count",
                "router": "pymagatama.primitives.demo_echo_chain:route_by_length",
                "paths": {"short": "short", "long": "long"},
            }
        ],
    })
    pkg = "pymagatama.primitives.demo_echo_chain"
...
async def test_topology_conditional_condition_ref_routes_via_dmn():
    ...
    from pymagatama.langgraph_loader import load_active_graphs
    from pymagatama.langgraph_node_resolvers import _DMN_REGISTRY_CACHE
...
async def test_ki_cycle_compiles_via_topology(monkeypatch):
    ...
    import pymagatama.ki_worker_main as kw
    ...
    pkg = "pymagatama.langgraph_graphs.ki_cycle"
    spec = json.dumps({
        "state_keys": [
...
async def test_topology_missing_node_binding_errors():
    ...
    nodes_cursor = _cursor([
        ("n1", "py_primitive", "pymagatama.primitives.demo_echo_chain:step_echo", None),
        # n2 binding missing
    ])
    pool = _make_pool([deploy_cursor, nodes_cursor])
...
def fake_factory_module():
    mod = types.ModuleType("pymagatama_test_topology_factory")
    sentinel = object()
    mod.build_graph = lambda: sentinel  # type: ignore[attr-defined]
    sys.modules["pymagatama_test_topology_factory"] = mod
    yield sentinel
    sys.modules.pop("pymagatama_test_topology_factory", None)
...
async def test_py_factory_still_works(fake_factory_module):
    ...
    deploy_cursor = _cursor([
        ("legacy_factory", 1, "py_factory", "pymagatama_test_topology_factory", None, "active", "ts1"),
    ])
