...
def test_ingest_prices_success():
    from pymagatama.langgraph_graphs.shosha_market_intelligence import ingest_prices

    async def _fake(*args, **kwargs):
        return {"rows": 17, "skipped": ["ZC=F"]}

    with patch(
        "pymagatama.langgraph_graphs.shosha_market_intelligence.asyncio.run",
        side_effect=lambda coro: _fake().__await__() and {"rows": 17, "skipped": ["ZC=F"]},
    ):
        # asyncio.run is sync; patch it to return the dict directly
        with patch(
            "pymagatama.primitives.shosha.task_shosha_intel_ingest_prices",
            return_value=None,
        ):
    ...
    with patch(
        "pymagatama.langgraph_graphs.shosha_market_intelligence.asyncio"
    ) as mock_asyncio:
...
def test_ingest_freight_success():
    from pymagatama.langgraph_graphs.shosha_market_intelligence import ingest_freight
...
def test_synth_market_views_success():
    from pymagatama.langgraph_graphs.shosha_market_intelligence import synth_market_views
...
def test_emit_audit_non_fatal_on_db_error():
    from pymagatama.langgraph_graphs.shosha_market_intelligence import emit_audit

    with patch(
        "pymagatama.langgraph_graphs.shosha_market_intelligence.sync_cursor"
    ) as mock_ctx:
