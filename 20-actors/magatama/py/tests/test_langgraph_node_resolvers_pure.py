...
async def test_sql_udf_node_executes_and_returns_result_key():
    from pymagatama.langgraph_node_resolvers import make_sql_udf_node
...
async def test_mcp_tool_node_posts_envelope():
    ...
    assert sent["url"].endswith("/ai.gftd.mcp.message")
    assert sent["json"] == {
        "method": "tools/call",
        "params": {
            "name": "web_research",
...
async def test_mcp_tool_static_args_merged_into_arguments():
    ...
    node = make_mcp_tool_node("https://x.example/xrpc/ai.gftd.mcp.message", cfg)
    ...
    assert sent["json"]["params"]["name"] == "ai.gftd.tools.const.echo"
...
async def test_mcp_tool_input_paths_navigates_nested_state():
    ...
    cfg = {
        ...
        "result_key": "out",
        "args": {"name": "ai.gftd.tools.json.extract", "path": "message.items"},
    }
    node = make_mcp_tool_node("https://x.example/xrpc/ai.gftd.mcp.message", cfg)
...
async def test_mcp_tool_registry_ref_resolves_via_vertex_mcp_tool_def():
    ...
    from pymagatama import langgraph_node_resolvers as mod
...
async def test_llm_node_calls_call_tier_json():
    from pymagatama.langgraph_node_resolvers import make_llm_node
    ...
    with patch("pymagatama.llm.call_tier_json", _fake_call_tier_json):
...
async def test_llm_node_template_format_failure_falls_back_to_raw():
    ...
    def _fake(tier, system, user, **_):
        captured["user"] = user
        return {}
    with patch("pymagatama.llm.call_tier_json", _fake):
        await node({"a": "x"})
...
async def test_foreach_iterates_inner_node_over_items():
    ...
    from pymagatama.langgraph_node_resolvers import make_foreach_node
    ...
    with patch("pymagatama.llm.call_tier_json", side_effect=lambda *a, **kw: {"echoed": a[2]}):
...
async def test_foreach_via_resolve_node_dispatch():
    ...
    with patch("pymagatama.llm.call_tier_json", return_value={"ok": True}):
...
async def test_foreach_passes_outer_state_to_inner():
    ...
    with patch("pymagatama.llm.call_tier_json", side_effect=lambda *a, **kw: {"u": a[2]}):
        node = make_foreach_node({
            "items_path": "items",
            "result_key": "out",
            "node": {
        ...
        with patch("pymagatama.llm.call_tier_json", side_effect=_spy):
