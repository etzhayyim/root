"""Unit tests for pymagatama.handlers.kouza."""
...
_src = _P(__file__).resolve().parents[1] / "src/pymagatama/handlers/kouza.py"
...
def test_sync_due_connections_dry_run(monkeypatch):
    factory = _SyncCursorFactory([
        ("at://did:web:owner/ai.gftd.apps.kouza.institutionConnection/conn-a", "did:web:owner", "mock"),
    ])
...
def test_sync_due_connections_writes_pending_sync_run(monkeypatch):
    ...
    assert out["syncRunDids"][0].startswith("at://did:web:owner/ai.gftd.apps.kouza.syncRun/")
...
def test_sync_due_connections_delegates_to_kouza_core(monkeypatch):
    ...
    class _Resp:
        ...
        def read(self, _limit):
            return b'{"syncRunDid":"at://did:web:owner/ai.gftd.apps.kouza.syncRun/sync-core","status":"succeeded"}'
