...
from pymagatama import agent_status_main
...
def test_summarize_status_reports_repairing_organism() -> None:
    report = agent_status_main.summarize_status(
        ...
        launchd={
            "ai.gftd.agent-daemon": True,
            "ai.gftd.agent-zeebe-worker": True,
        },
        belief_rows=[
            {
...
def test_format_text_includes_process_status() -> None:
    text = agent_status_main.format_text(
        {
            ...
            "processes": {"ai.gftd.agent-daemon": True, "ai.gftd.agent-zeebe-worker": False},
        }
    )
    ...
    assert "ai.gftd.agent-daemon: running" in text
    assert "ai.gftd.agent-zeebe-worker: down" in text
