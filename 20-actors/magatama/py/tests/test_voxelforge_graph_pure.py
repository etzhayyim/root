...
def patched_converters(monkeypatch):
    from pymagatama.voxelforge import converters
...
def patched_runpod(monkeypatch):
    from pymagatama.voxelforge import runpod_client
...
def _drive_graph_sync(initial_state_dict):
    ...
    from pymagatama.voxelforge.graph import build_graph
...
def _make_initial_state(input_payload: dict):
    from pymagatama.voxelforge.state import GenerateInput, VoxelforgeState

    gi = GenerateInput.model_validate(input_payload)
    state = VoxelforgeState(
        input=gi,
        actor_did="did:web:voxelforge.gftd.ai",
        org_did="did:erc725:gftd:260425:test",
        design_vertex_id="at://did:web:voxelforge.gftd.ai/ai.gftd.apps.voxelforge.design/test",
        run_id="test-run-id",
...
def test_cad_path_completes_with_artifacts(patched_converters):
    ...
    assert {a["design_vertex_id"] for a in arts} == {
        "at://did:web:voxelforge.gftd.ai/ai.gftd.apps.voxelforge.design/test"
    }
...
def test_unwrap_envelope_accepts_both_shapes():
    from pymagatama.voxelforge.server import build_app  # noqa: F401
    ...
    payload, actor, thread = unwrap(
        {
            ...
            "actor_did": "did:web:caller.gftd.ai",
            "thread_id": "abc123",
        }
    )
    assert payload["kind"] == "cad"
    assert actor == "did:web:caller.gftd.ai"
