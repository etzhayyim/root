"""Pure unit tests for pymagatama.mcp_dispatch.handle_envelope (no aiohttp)."""
...
def test_camel_to_snake():
    from pymagatama.mcp_dispatch import _camel_to_snake
...
def test_register_actor_by_convention_resolves_task_functions(monkeypatch):
    """`com.etzhayyim.apps.{actor}.{method}` → `pymagatama.{actor}_worker_main:task_{snake}`."""
    import sys
    import types
    from pymagatama.mcp_dispatch import register_actor_by_convention

    fake_mod = types.ModuleType("pymagatama.fakeactor_worker_main")
    ...
    monkeypatch.setitem(sys.modules, "pymagatama.fakeactor_worker_main", fake_mod)
...
def test_register_actor_by_convention_override_module_and_fn_template(monkeypatch):
    ...
    where task lives in pymagatama.primitives.<actor> with a non-standard name)."""
    import sys
    import types
    from pymagatama.mcp_dispatch import register_actor_by_convention

    fake_mod = types.ModuleType("pymagatama.primitives.adsk")
    ...
    monkeypatch.setitem(sys.modules, "pymagatama.primitives.adsk", fake_mod)

    handlers = register_actor_by_convention(
        "adsk", ["datasetIngestAll"],
        module_template="pymagatama.primitives.adsk",
...
def test_register_actor_by_convention_missing_method_skipped(monkeypatch, caplog):
    ...
    fake_mod = types.ModuleType("pymagatama.partial_worker_main")
    async def task_present(**_): return {"ok": True}
    fake_mod.task_present = task_present
    # Note: no task_missing.
    monkeypatch.setitem(sys.modules, "pymagatama.partial_worker_main", fake_mod)
...
async def test_task_sql_exec_strict_guards():
    ...
    from pymagatama.tools_sql_worker_main import task_sql_exec
...
async def test_task_sql_exec_executemany_path(monkeypatch):
    ...
    fake_db = types.ModuleType("pymagatama.db_alchemy")
    fake_db.sa_executemany = _fake_executemany
    fake_db.sa_rowcount = lambda *a, **k: 0
    monkeypatch.setitem(sys.modules, "pymagatama.db_alchemy", fake_db)
...
async def test_task_sql_query_rejects_non_select():
    ...
    from pymagatama.tools_sql_worker_main import task_sql_query
...
async def test_task_sql_query_accepts_select_and_with(monkeypatch):
    ...
    fake_mod = types.ModuleType("pymagatama.db_alchemy")
    fake_mod.sa_query = _fake_sa_query
    monkeypatch.setitem(sys.modules, "pymagatama.db_alchemy", fake_mod)
...
async def test_task_llm_chat_forwards_kwargs(monkeypatch):
    ...
    fake_mod = types.ModuleType("pymagatama.zeebe_worker_main")
    fake_mod.task_generic_llm_chat = _fake_chat
    monkeypatch.setitem(sys.modules, "pymagatama.zeebe_worker_main", fake_mod)
...
async def test_task_llm_chat_renders_user_template(monkeypatch):
    ...
    fake = types.ModuleType("pymagatama.zeebe_worker_main")
    fake.task_generic_llm_chat = _fake_chat
    monkeypatch.setitem(sys.modules, "pymagatama.zeebe_worker_main", fake)
...
async def test_task_audit_emit_returns_vertex_id_and_rkey(monkeypatch):
    ...
    import pymagatama.tools_audit_worker_main as mod
    import sys, types
    ...
    fake_mod = types.ModuleType("pymagatama.db_sync")
...
async def test_yoro_canonical_actor_registers_all_methods():
    ...
    8 methods from pymagatama.primitives.yoro_social (multi-line import that
...
def test_register_actor_by_mapping_invalid_target_skipped(caplog):
    from pymagatama.mcp_dispatch import register_actor_by_mapping
    with caplog.at_level("WARNING", logger="mcp_dispatch"):
...
async def test_shosha_canonical_actor_registers_all_methods():
    ...
    surface is derived from `pymagatama.primitives.shosha:task_shosha_*`
    ...
    assert not missing, (
        ...
        "Check pymagatama.primitives.shosha for renamed task_shosha_* fns."
    )
...
async def test_aria_canonical_actor_registers_all_methods():
    ...
    regressions in pymagatama.primitives.aria_signal renames."""
    from pymagatama.mcp_dispatch import build_default_handlers
    handlers = build_default_handlers()
    expected = {
    ...
    assert not missing, (
        ...
        "Check pymagatama.primitives.aria_signal for renamed task_aria_* fns."
    )
