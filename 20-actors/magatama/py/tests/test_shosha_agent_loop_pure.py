...
def test_fetch_context_with_rows():
    ...
    with patch("pymagatama.langgraph_graphs.shosha_agent_loop._rw_query", side_effect=_mock_query):
        result = fetch_context(_make_state())
...
def test_fetch_context_commodity_focus():
    ...
    with patch("pymagatama.langgraph_graphs.shosha_agent_loop._rw_query", side_effect=_capturing_query):
...
def test_emit_audit_non_fatal_on_db_error():
    ...
    with patch("pymagatama.langgraph_graphs.shosha_agent_loop.sync_cursor") as mock_ctx:
...
async def test_graph_runs_end_to_end():
    ...
    with (
        patch("pymagatama.langgraph_graphs.shosha_agent_loop._rw_query", return_value=[]),
        patch("pymagatama.langgraph_graphs.shosha_agent_loop.llm") as mock_llm,
        patch("pymagatama.langgraph_graphs.shosha_agent_loop.sync_cursor") as mock_ctx,
    ):
        mock_llm.call_tier.return_value = mock_resp
        mock_llm.LlmError = Exception
        mock_cur = MagicMock()
