"""Unit tests for magatama organizer Zeebe primitive."""
...
from pymagatama.primitives import magatama_organizer as M  # noqa: E402
...
def test_call_organizer_summarizes_plan(monkeypatch):
    ...
    out = M.call_organizer("https://magatama.example/api/organizer/run", "tok")
...
def test_task_writes_organizer_run(monkeypatch):
    ...
    out = asyncio.run(M.task_magatama_organizer_run(
        organizerUrl="https://magatama.example/api/organizer/run",
    ))
...
def test_task_requires_organizer_url(monkeypatch):
    factory = _SyncCursorFactory()
    monkeypatch.setattr(M, "sync_cursor", factory)
    monkeypatch.delenv("MAGATAMA_ORGANIZER_URL", raising=False)

    out = asyncio.run(M.task_magatama_organizer_run())

    assert out["ok"] is False
    assert "MAGATAMA_ORGANIZER_URL" in out["error"]
