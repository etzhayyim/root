...
from pymagatama import agent_daemon_main
from pymagatama.local_llm import (
    LocalLlmConfig,
    build_chat_request,
    extract_json_object,
    parse_chat_response,
...
def test_build_homeostasis_belief_row_derives_runtime_health_posterior() -> None:
    result = agent_daemon_main.build_homeostasis_belief_row(
        ...
        metrics={
            ...
            "launchd": {"ai.gftd.agent-daemon": True},
    ...
    assert state_value["launchd"]["ai.gftd.agent-daemon"] is True
...
def test_execute_real_world_action_direct_records_email_receipt_observation(monkeypatch) -> None:
    ...
    monkeypatch.setattr(agent_daemon_main, "insert_direct_row", fake_insert)
    monkeypatch.setattr("pymagatama.ingest.mailer.send_email", fake_send_email)
...
def test_email_live_channel_blocker_distinguishes_resend_account_pending(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pymagatama.db_sync.sync_cursor", lambda: FakeSyncCursor())
...
def test_collect_local_homeostasis_metrics_maps_probe_failures(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda label: label == "ai.gftd.agent-daemon",
    )
    monkeypatch.setattr(
        agent_daemon_main,
    ...
    result = agent_daemon_main.collect_local_homeostasis_metrics(
        ...
        launchd_labels=("ai.gftd.agent-daemon", "ai.gftd.agent-zeebe-worker"),
        log_paths=(str(log_path),),
    )

    assert result["source"] == "measured"
    assert result["launchd"]["ai.gftd.agent-daemon"] is True
    assert result["launchd"]["ai.gftd.agent-zeebe-worker"] is False
...
def test_harden_runtime_viability_repairs_local_service_degraded() -> None:
    result = agent_daemon_main.harden_runtime_viability(
        ...
        metrics={
            "launchd": {"ai.gftd.agent-daemon": True, "ai.gftd.agent-zeebe-worker": False},
            "ollama": {"ok": True},
        },
    )
    ...
    assert result["failedLaunchdServices"] == ["ai.gftd.agent-zeebe-worker"]
...
def test_execute_local_self_repair_restarts_failed_non_daemon_services(monkeypatch) -> None:
    ...
    result = agent_daemon_main.execute_local_self_repair(
        ...
        current_label="ai.gftd.agent-daemon",
    )

    assert calls == ["ai.gftd.agent-zeebe-worker"]
    assert result["attempted"] == [{"ok": True, "label": "ai.gftd.agent-zeebe-worker"}]
    assert result["skipped"] == [
        {"label": "ai.gftd.agent-daemon", "reason": "current_daemon_not_self_restarted"}
    ]
...
def test_build_self_repair_observation_row_records_local_repair_payload() -> None:
    row = agent_daemon_main.build_self_repair_observation_row(
        ...
        local_repair={
            "ok": True,
            "attempted": [{"ok": True, "label": "ai.gftd.agent-zeebe-worker"}],
            "skipped": [],
        },
    ...
    assert payload["localRepair"]["attempted"][0]["label"] == "ai.gftd.agent-zeebe-worker"
...
def test_run_one_tick_dispatches_only_selected_real_world_effects(monkeypatch) -> None:
    ...
    monkeypatch.setattr(agent_daemon_main, "_run_process_async", fail_run_process)
    monkeypatch.setattr(
        "pymagatama.agent_authority_policy.load_delegated_authority_policy",
        load_policy,
    )
