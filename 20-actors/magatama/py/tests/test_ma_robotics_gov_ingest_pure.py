...
from pymagatama.primitives import ma as MA  # noqa: E402
from pymagatama.primitives import robotics as ROB  # noqa: E402
...
def test_gov_ingest_official_sources_returns_dict(code: str) -> None:
    import importlib
    mod = sys.modules.get(f"pymagatama.primitives.gov_{code}") or importlib.import_module(
        f"pymagatama.primitives.gov_{code}"
    )
    from pymagatama.db_sync import sync_cursor as _real_sc
    ...
    try:
        ...
        fresh = importlib.import_module(f"pymagatama.primitives.gov_{code}")
