...
def fake_factory_module():
    mod = types.ModuleType("pymagatama_test_fake_factory")
    sentinel = object()
    mod.build_graph = lambda: sentinel  # type: ignore[attr-defined]
    sys.modules["pymagatama_test_fake_factory"] = mod
    yield mod, sentinel
    sys.modules.pop("pymagatama_test_fake_factory", None)
...
async def test_loads_py_factory_and_registers(fake_factory_module):
    ...
    rows = [("ki_cycle_v2", 1, "py_factory", "pymagatama_test_fake_factory", None, "active", "ts1")]
...
async def test_skips_already_registered(fake_factory_module):
    ...
    rows = [("ki_cycle", 1, "py_factory", "pymagatama_test_fake_factory", None, "active", "ts1")]
    pool = _make_pool(rows)
    registered: dict[str, object] = {}
...
async def test_factory_path_with_explicit_attr(fake_factory_module):
    ...
    rows = [("explicit_attr", 1, "py_factory", "pymagatama_test_fake_factory:build_graph", None, "active", "ts1")]
    pool = _make_pool(rows)
    registered: dict[str, object] = {}
...
def test_resolve_checkpointer_none_returns_none():
    from pymagatama.langgraph_loader import _resolve_checkpointer
...
def test_resolve_checkpointer_rw_vertex_swallows_init_error(monkeypatch):
    ...
    fake = types.ModuleType("pymagatama.langgraph_checkpoint_rw")
    class _Boom:
        def __init__(self):
            raise RuntimeError("simulated init failure")
    fake.RisingWaveCheckpointSaver = _Boom
    monkeypatch.setitem(sys.modules, "pymagatama.langgraph_checkpoint_rw", fake)
