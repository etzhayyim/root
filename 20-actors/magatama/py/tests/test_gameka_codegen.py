...
Both tests stay offline:
  ...
  - studio tests stub `pymagatama.llm.call_tier_json` so the LangGraph
...
rather than the `pymagatama.handlers` / `pymagatama.agents` packages,
...
ROOT = _P(__file__).resolve().parents[1] / "src" / "pymagatama"
...
def _install_llm_stub(planner_payload: dict, critic_payload: dict) -> list:
    """Stub pymagatama.llm so the gameka_studio graph runs offline."""
    calls: list[dict] = []
    stub = types.ModuleType("pymagatama.llm")
    ...
    sys.modules["pymagatama.llm"] = stub
    # Also make `pymagatama` package available
    if "pymagatama" not in sys.modules:
        sys.modules["pymagatama"] = types.ModuleType("pymagatama")
    sys.modules["pymagatama"].llm = stub  # type: ignore[attr-defined]
    return calls
