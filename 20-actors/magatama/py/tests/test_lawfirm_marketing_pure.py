...
def _make_state(**kw):
    from pymagatama.langgraph_graphs.lawfirm_marketing_ops import MarketingState
...
class TestSupervisorPrefixRouting(unittest.TestCase):
    def _call(self, task_type: str) -> dict:
        from pymagatama.langgraph_graphs.lawfirm_marketing_ops import supervisor
...
class TestRouterFunction(unittest.TestCase):
    def test_route_returns_kind(self):
        from pymagatama.langgraph_graphs.lawfirm_marketing_ops import _route_kind
...
class TestGraphStructure(unittest.TestCase):
    def setUp(self):
        import pymagatama.langgraph_graphs.lawfirm_marketing_ops as mod
...
class TestStripeSignatureVerify(unittest.TestCase):
    def test_valid_signature(self):
        import hmac, hashlib
        from pymagatama.primitives.lawfirm_marketing import _verify_stripe_signature
        secret = "whsec_test"
        ts = "1700000000"
        body = '{"id":"evt_x"}'
...
class TestPrimitiveRegistration(unittest.TestCase):
    def test_marketing_module_importable(self):
        import pymagatama.primitives.lawfirm_marketing as m
...
class TestVidFormat(unittest.TestCase):
    def test_vid(self):
        ...
        self.assertTrue(v.startswith("at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.lawfirm.marketingAsset/"))
