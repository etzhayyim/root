...
# Ensure pymagatama is importable from the src tree.
_src = str(Path(__file__).resolve().parent.parent / "src")
if _src not in sys.path:
    sys.path.insert(0, _src)

from pymagatama.cell_runner_main import (
    _async_main,
    _cron_to_interval_s,
...
class TestSpawnCronCell:
    def test_spawn_cron_cell_invokes_entry(self) -> None:
        ...
        async def run():
            with patch(
                "pymagatama.cell_runner_main._import_cell_entry",
                return_value=fake_cell_fn,
            ):
                # Use a very small interval so the test doesn't wait 60s.
                with patch(
                    "pymagatama.cell_runner_main._cron_to_interval_s",
                    return_value=0.05,
                ):
...
class TestSpawnListenerCell:
        ...
        async def run():
            ...
            with patch(
                ...
                with patch.dict(
                    ...
                    with patch(
                        "pymagatama.cell_runner_main._spawn_listener_cell.__globals__"
                        if False else "pymagatama.cell_runner_main._spawn_listener_cell",
                        wraps=_spawn_listener_cell,
                    ):
                        pass
                    # Drive the cell directly, patching the cursor import.
                    import pymagatama.cell_runner_main as crm
    ...
    def test_spawn_listener_cell_no_collections_exits(self) -> None:
        ...
        async def run():
            with patch("pymagatama.cell_runner_main._import_cell_entry") as mock_import:
    ...
    def test_spawn_listener_cell_sdk_import_error_exits(self) -> None:
        ...
        async def run():
            ...
            with patch(
                "pymagatama.cell_runner_main._import_cell_entry", return_value=fake_fn
            ):
...
class TestSpawnCellsForNode:
    ...
    def test_spawn_cells_for_node_creates_tasks_for_cron(self) -> None:
        ...
        async def run():
            ...
            crm._cell_tasks.clear()
            with patch(
                "pymagatama.cell_runner_main._import_cell_entry", return_value=fake_cell_fn
            ):
                with patch(
                    "pymagatama.cell_runner_main._cron_to_interval_s", return_value=0.05
                ):
