"""Tests for all gov_* country primitives + langgraph_registry + magatama_organizer."""
...
from pymagatama.primitives import langgraph_registry as LGR  # noqa: E402
...
from pymagatama.primitives import magatama_organizer as MO  # noqa: E402


@pytest.fixture()
def _stub_mo_db():
    with patch("pymagatama.primitives.magatama_organizer.sync_cursor") as m:
        cur = MagicMock()
...
def test_organizer_no_url_returns_error(_stub_mo_db):
    result = asyncio.run(MO.task_magatama_organizer_run(organizerUrl="", flush=False))
    assert result["ok"] is False
    assert "MAGATAMA_ORGANIZER_URL" in result["error"]
...
def test_organizer_register_exposes_one_task():
    ...
    assert registered == ["magatama.organizer.run"]
...
def test_gov_register_exposes_tasks(country: str):
    ...
    # Each task type should follow the xrpc.ai.gftd.gov{CC}.* pattern
    cc = country
    camel_cc = cc[0].upper() + cc[1:]  # e.g. "jpn" → "Jpn"
    prefix = f"xrpc.ai.gftd.gov{camel_cc}."
    for t in registered:
