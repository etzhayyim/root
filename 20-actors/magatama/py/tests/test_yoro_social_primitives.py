...
from pymagatama.primitives import yoro_social as YS  # noqa: E402


@pytest.fixture(autouse=True)
def _stub_db():
    with patch("pymagatama.primitives.yoro_social.sync_cursor") as m:
        cur = MagicMock()
...
def test_translate_post_dry_run_uses_gemma_translation_path():
    with patch("pymagatama.primitives.yoro_social._fetch_source_post", return_value={
        ...
        "text": "こんにちは",
        "langs": ["ja"],
    }), patch("pymagatama.primitives.yoro_social._translate_social_text", return_value={
        "ok": True,
...
def test_translate_post_batch_uses_requested_languages():
    with patch("pymagatama.primitives.yoro_social.task_yoro_social_translate_post") as m:
