...
def _isolate_state():
    ...
    from pymagatama import langgraph_server_app as mod
    mod._RUNS.clear()
...
def client():
    ...
    from pymagatama.langgraph_server_app import app
    from pymagatama import langgraph_server_app as mod
...
def client_with_echo(client):
    ...
    from pymagatama.langgraph_server_app import register_graph
...
@patch("pymagatama.langgraph_server_app._rw_upsert_run", new_callable=AsyncMock)
@patch("pymagatama.langgraph_server_app._execute_graph", new_callable=AsyncMock)
...
def test_readyz_db_unreachable_503(client):
    with patch(
        "pymagatama.langgraph_server_app.ensure_rw_async_pool",
