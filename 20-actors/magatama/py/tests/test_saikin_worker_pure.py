...
def test_uid_prefix():
    from pymagatama.saikin_worker_main import _uid
...
def test_now_iso():
    from pymagatama.saikin_worker_main import _now
...
def test_probe_empty():
    with patch("pymagatama.saikin_worker_main.fetch_all", return_value=[]):
...
def test_probe_returns_signals():
    rows = [
        ("at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.signal/s41k-abc", "hash1", "text", "content A"),
        ("at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.signal/s41k-def", "hash2", "url", "https://x.com"),
    ]
...
def test_transfer_no_input():
    from pymagatama.saikin_worker_main import task_transfer_signal
...
def test_transfer_by_signal_id():
    ...
    with patch("pymagatama.saikin_worker_main.sync_cursor", return_value=mock_cm):
        from pymagatama.saikin_worker_main import task_transfer_signal
...
def test_form_colony_no_signals():
    from pymagatama.saikin_worker_main import task_form_colony
...
def test_lyse_no_signal_id():
    from pymagatama.saikin_worker_main import task_lyse
...
def test_lyse_not_found():
    with patch("pymagatama.saikin_worker_main.fetch_one", return_value=None):
        from pymagatama.saikin_worker_main import task_lyse
...
def test_lyse_success():
    ...
    row = ("at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.signal/s41k-abc",)

    with (
        patch("pymagatama.saikin_worker_main.fetch_one", return_value=row),
        patch("pymagatama.saikin_worker_main.sync_cursor", return_value=mock_cm),
    ):
        from pymagatama.saikin_worker_main import task_lyse
...
def test_handoff_to_ki_no_args():
    from pymagatama.saikin_worker_main import task_handoff_to_ki
...
def test_handoff_to_ki_via_colony():
    ...
    colony_row = (
        "at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.colony/col-abc",
        "knowledge colony",
        '{"signalIds":["s41k-001"]}',
    )

    with (
        patch("pymagatama.saikin_worker_main.fetch_one", return_value=colony_row),
        patch("pymagatama.saikin_worker_main.sync_cursor", return_value=mock_cm),
    ):
        from pymagatama.saikin_worker_main import task_handoff_to_ki
    ...
    assert result["sourceVertexId"] == "at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.colony/col-abc"
...
def test_handoff_to_ki_falls_back_to_signal():
    ...
    signal_row = (
        "at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.signal/s41k-xyz",
        "text",
    ...
    with (
        patch("pymagatama.saikin_worker_main.fetch_one", side_effect=[None, signal_row]),
        patch("pymagatama.saikin_worker_main.sync_cursor", return_value=mock_cm),
    ):
        from pymagatama.saikin_worker_main import task_handoff_to_ki
    ...
    assert result["kiAbsorbId"].startswith("xyl-")
    assert result["sourceVertexId"] == "at://saikin.etzhayyim.com/app.etzhayyim.apps.saikin.signal/s41k-xyz"
