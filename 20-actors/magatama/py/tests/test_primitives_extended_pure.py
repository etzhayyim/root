...
- primitives/magatama_organizer.py: task_magatama_organizer_run (no URL → error dict)
...
import pymagatama.primitives.pds_discover_cache as PDC
import pymagatama.primitives.pds_domain_coverage as PDO
import pymagatama.primitives.pds_heartbeat as PHB
import pymagatama.primitives.pds_key_rotation as PKR
import pymagatama.primitives.pds_mitama_cron as PMC
import pymagatama.primitives.pds_outbox as POB
import pymagatama.primitives.magatama_organizer as MO
import pymagatama.primitives.murakumo_fleet as MF
import pymagatama.primitives.vector_embedding as VE
import pymagatama.primitives.intel as IT
import pymagatama.primitives.projector as PR
...
def _patch(mod, mock_sc):
    from pymagatama.db_sync import sync_cursor as _real
...
def test_magatama_organizer_no_url_returns_dict() -> None:
    real = _patch(MO, _noop_cursor_mock())
    try:
        result = asyncio.run(MO.task_magatama_organizer_run())
        assert isinstance(result, dict)
    finally:
        _restore(MO, real)


def test_magatama_organizer_no_url_ok_false() -> None:
    real = _patch(MO, _noop_cursor_mock())
    try:
        result = asyncio.run(MO.task_magatama_organizer_run())
        assert result["ok"] is False
...
def test_magatama_organizer_no_url_has_error() -> None:
    real = _patch(MO, _noop_cursor_mock())
    try:
        result = asyncio.run(MO.task_magatama_organizer_run())
        assert "error" in result and result["error"]
...
def test_magatama_organizer_has_summary() -> None:
    real = _patch(MO, _noop_cursor_mock())
    try:
        result = asyncio.run(MO.task_magatama_organizer_run())
