...
Live-network tests (dispatcher.gftd.ai / *.badssl.com) are exercised
...
def _install_stubs() -> None:
    ...
    if "pymagatama.llm" not in sys.modules:
        llm = types.ModuleType("pymagatama.llm")
        ...
        sys.modules["pymagatama.llm"] = llm

    if "pymagatama.db_sync" not in sys.modules:
        dbsync = types.ModuleType("pymagatama.db_sync")
        ...
        sys.modules["pymagatama.db_sync"] = dbsync


_install_stubs()

from pymagatama.zeebe_worker_main import (  # noqa: E402
    _cert_cn,
    _cert_issuer_cn,
    _cert_san_dns,
    _decode_cert_bin,
...
def test_generic_xrpc_invoke_requires_nsid():
    r = asyncio.run(task_generic_xrpc_invoke(actor="did:web:tsukuru.gftd.ai"))
    assert r == {"error": "nsid required", "status": 400}


def test_generic_xrpc_invoke_uses_did_web_actor(monkeypatch):
    import pymagatama.zeebe_worker_main as worker_main
    ...
    assert calls[0][0] == "https://tsukuru.gftd.ai/xrpc/ai.gftd.apps.tsukuru.euv.designManufacturingFlow"
    assert calls[0][1]["actor"] == "did:web:tsukuru.gftd.ai"
