...
from pymagatama.primitives import pds_heartbeat as PH  # noqa: E402
from pymagatama.primitives import pds_domain_coverage as PDC  # noqa: E402
from pymagatama.primitives import pds_discover_cache as PDSC  # noqa: E402
from pymagatama.primitives import pds_key_rotation as PKR  # noqa: E402
from pymagatama.primitives import pds_mitama_cron as PMC  # noqa: E402
from pymagatama.primitives import pds_outbox as PO  # noqa: E402
...
class TestPdsHeartbeat:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_heartbeat.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
...
class TestPdsDomainCoverage:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_domain_coverage.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
...
class TestPdsDiscoverCache:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_discover_cache.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
...
class TestPdsKeyRotation:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_key_rotation.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
...
class TestPdsMitamaCron:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_mitama_cron.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
...
class TestPdsOutbox:
    @pytest.fixture()
    def _stub_db(self):
        with patch("pymagatama.primitives.pds_outbox.sync_cursor") as m:
            cur = MagicMock()
            m.return_value.__enter__ = MagicMock(return_value=cur)
            m.return_value.__exit__ = MagicMock(return_value=False)
            yield cur
