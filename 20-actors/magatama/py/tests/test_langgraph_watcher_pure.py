...
def fake_factory_module():
    mod = types.ModuleType("pymagatama_test_watcher_factory")
    ...
    sys.modules["pymagatama_test_watcher_factory"] = mod
    yield mod, sentinel_v1, sentinel_v2
    sys.modules.pop("pymagatama_test_watcher_factory", None)
...
async def test_add_active_row_registers(fake_factory_module):
    from pymagatama.langgraph_watcher import _reconcile_once

    mod, sentinel, _ = fake_factory_module
    deploy_cursor = _cursor([("aid_new", 1, "active", "ts1")])
    assistant_cursor = _cursor([("py_factory", "pymagatama_test_watcher_factory", None, "active")])
...
async def test_compile_failure_does_not_evict():
    ...
    assistant_cursor = _cursor([("py_factory", "pymagatama_test_watcher_NONEXISTENT", None, "active")])
    pool, _ = _pool_with_cursors([deploy_cursor, assistant_cursor])
...
async def test_concurrent_reconcile_last_write_wins(fake_factory_module):
    ...
    asst_a = _cursor([("py_factory", "pymagatama_test_watcher_factory", None, "active")])
    pool_a, _ = _pool_with_cursors([deploy_a, asst_a])

    # Second sees v2.
    mod._current = sentinel_v2
    deploy_b = _cursor([("aid_x", 2, "active", "ts2")])
    asst_b = _cursor([("py_factory", "pymagatama_test_watcher_factory", None, "active")])
...
async def test_stats_reload_count_increments(fake_factory_module):
    from pymagatama.langgraph_watcher import _reconcile_once, STATS

    STATS.reload_count = 0
    STATS.error_count = 0

    deploy_cursor = _cursor([("aid_y", 1, "active", "ts1")])
    asst_cursor = _cursor([("py_factory", "pymagatama_test_watcher_factory", None, "active")])
...
async def test_stats_error_count_increments_on_broken_row():
    ...
    asst_cursor = _cursor([("py_factory", "pymagatama_test_watcher_NONEXISTENT", None, "active")])
    pool, _ = _pool_with_cursors([deploy_cursor, asst_cursor])
