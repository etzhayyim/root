...
through `pymagatama.llm.call_tier_vision_json`.
...
def test_parse_dmn_ref_strips_version_suffix():
    ...
    assert _parse_dmn_ref("dmn:ai.gftd.policies.foo.bar@1.0.0") == (
        "ai.gftd.policies.foo.bar", 1,
    )
...
def test_eval_dmn_input_entry_matrix(entry, value, state, expected):
    from pymagatama.langgraph_node_resolvers import _eval_dmn_input_entry
...
def test_eval_dmn_rule_all_inputs_must_match():
    from pymagatama.langgraph_node_resolvers import _eval_dmn_rule
...
async def test_resolve_dmn_ref_caches_after_first_hit():
    ...
    ref = "dmn:ai.gftd.policies.mangaka.composeScene3dRefinement@1.0.0"

    out1 = await _resolve_dmn_ref(ref, lambda: _async_return(pool)())
    out2 = await _resolve_dmn_ref(ref, lambda: _async_return(pool)())

    assert out1["decision_key"] == "ai.gftd.policies.mangaka.composeScene3dRefinement"
...
async def test_dmn_router_matches_python_route_after_critique(
    ...
    router = make_dmn_condition_router(
        "dmn:ai.gftd.policies.mangaka.composeScene3dRefinement@1.0.0",
...
async def test_dmn_router_requires_pool_factory():
    from pymagatama.langgraph_node_resolvers import make_dmn_condition_router
...
async def test_llm_vision_node_resolves_single_image_path():
    from pymagatama.langgraph_node_resolvers import make_llm_vision_node
    ...
    with patch("pymagatama.llm.call_tier_vision_json", fake_vision):
        out = await node({
...
async def test_llm_vision_node_walks_array_path_for_many_images():
    ...
    with patch("pymagatama.llm.call_tier_vision_json", lambda *a, **k: {"ok": True}):
        await node(state)
...
async def test_mcp_nsid_override_short_circuits_db_lookup(monkeypatch):
    ...
    url = await _resolve_mcp_nsid(
        "ai.gftd.apps.mangaka.tools.loadPanelPlan", _explode_pool,
    )
    assert url == "http://localhost:8000/xrpc/ai.gftd.mcp.message"
...
async def test_mcp_nsid_override_longest_prefix_wins(monkeypatch):
    ...
    url = await _resolve_mcp_nsid(
        "ai.gftd.apps.mangaka.tools.loadPanelPlan", _unused_pool,
    )
...
async def test_mcp_nsid_override_does_not_match_prefix_substring(monkeypatch):
    """`ai.gftd.appsXYZ.tools.foo` must NOT match the
    `ai.gftd.apps` prefix — only exact segment boundaries count."""
    from pymagatama.langgraph_node_resolvers import (
        _MCP_REGISTRY_CACHE, _resolve_mcp_nsid,
    )
...
async def test_mcp_nsid_override_matches_exact_nsid(monkeypatch):
    ...
    assert url == "http://exact:7000/xrpc/ai.gftd.mcp.message"
...
async def test_llm_vision_dispatched_via_resolve_node():
    ...
    from pymagatama.langgraph_node_resolvers import resolve_node
    ...
    with patch("pymagatama.llm.call_tier_vision_json",
