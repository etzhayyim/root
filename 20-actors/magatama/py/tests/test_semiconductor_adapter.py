...
def _run_normalize(
    ...
    with (
        patch("pymagatama.jukyu.adapter.execute", return_value=execute_return) as mock_exec,
        patch("pymagatama.jukyu.adapter.fetch_one", return_value=fetch_one_return),
    ):
        from pymagatama.jukyu.adapter import normalize_semiconductor
...
class TestNormalizeSemiconductorDbCalls:
    def test_execute_called_multiple_times(self):
        with (
            patch("pymagatama.jukyu.adapter.execute", return_value=0) as mock_exec,
            patch("pymagatama.jukyu.adapter.fetch_one", return_value=(0,)),
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
    ...
    def test_fetch_one_called_three_times(self):
        with (
            patch("pymagatama.jukyu.adapter.execute", return_value=0),
            patch("pymagatama.jukyu.adapter.fetch_one", return_value=(0,)) as mock_fetch,
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
            normalize_semiconductor()
    ...
    def test_totals_use_semiconductor_domain_param(self):
        ...
        with (
            patch("pymagatama.jukyu.adapter.execute", return_value=0),
            patch("pymagatama.jukyu.adapter.fetch_one", side_effect=_mock_fetch),
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
            normalize_semiconductor()
...
class TestNormalizeSemiconductorIdempotency:
    def test_has_both_deletes_and_inserts(self):
        ...
        with (
            patch("pymagatama.jukyu.adapter.execute", side_effect=_mock_exec),
            patch("pymagatama.jukyu.adapter.fetch_one", return_value=(0,)),
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
            normalize_semiconductor()
...
class TestNormalizeSemiconductorErrors:
    def test_db_error_propagates(self):
        with (
            patch("pymagatama.jukyu.adapter.execute", side_effect=RuntimeError("DB down")),
            patch("pymagatama.jukyu.adapter.fetch_one", return_value=(0,)),
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
    ...
    def test_fetch_none_returns_zero_total(self):
        with (
            patch("pymagatama.jukyu.adapter.execute", return_value=0),
            patch("pymagatama.jukyu.adapter.fetch_one", return_value=None),
        ):
            from pymagatama.jukyu.adapter import normalize_semiconductor
            result = normalize_semiconductor()
