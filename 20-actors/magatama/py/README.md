...
- Handlers are stateless per call. State lives in RisingWave via `pymagatama.db`.
...
pip install -e '.[dev]'
python -m pymagatama.server    # binds :8815 arrow-flight, :9090 prometheus
```
...
async def classify(subject: str, body: str) -> int:
    # Access the shared asyncpg pool via pymagatama.context.Context when
    # invoked through the server. For direct unit tests, import db.get_pool().
    return compute_score(subject, body)
```

Add the module to `pymagatama/handlers/__init__.py` so it is imported at boot.
...
| `pymagatama.server` | arrow-flight UdfServer bootstrap + signal handling + metrics |
| `pymagatama.context` | per-invocation `Context` (db / pds / auth / logger) |
| `pymagatama.db` | asyncpg pool to RisingWave (Kysely-mirror writes) |
| `pymagatama.pds` | atproto XRPC client (sdk.pds.dispatch equivalent) |
| `pymagatama.saver` | LangGraph `KyselyMirrorSaver` (no SELECT FOR UPDATE) |
| `pymagatama.handlers.*` | per-actor handler modules; Phase B pilot: bpmn + playwright |
