"""
Py Kotodama WASM entrypoint for Myinfo Cognitive Actor.
...
schema = load_schema("../schema/myinfo.kotoba")
db = DatomicClient.connect()
app = Runtime("myinfo-compat")
...
@app.route("/{api_version}/myinfo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_myinfo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "myinfo.CognitiveRecord/id": internal_record_id,
        "myinfo.CognitiveRecord/apiVersion": api_version,
        "myinfo.CognitiveRecord/originalPayload": str(raw_data),
        "myinfo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "myinfo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_myinfo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "myinfo_entity",
