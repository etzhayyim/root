...
describe("ALL_PROJECTORS", () => {
  ...
  it("hanrei declares the three Phase E reference collections", () => {
    ...
    expect(collections).toContain("ai.gftd.hanrei.case");
    expect(collections).toContain("ai.gftd.hanrei.law");
    expect(collections).toContain("ai.gftd.hanrei.gazetteEntry");
  });
  ...
  it("ipaddress declares provider + scan with countryIso3 facet", () => {
    ...
    const provider = Object.values(cfg.collections).find(
      (c) => c.collection === "ai.gftd.ipaddress.provider",
    );
