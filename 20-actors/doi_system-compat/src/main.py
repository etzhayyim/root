"""
Py Kotodama WASM entrypoint for Doi_system Cognitive Actor.
...
schema = load_schema("../schema/doi_system.kotoba")
db = DatomicClient.connect()
app = Runtime("doi_system-compat")
...
@app.route("/{api_version}/doi_system/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_doi_system_" + uuid.uuid4().hex[:12]
    db.transact([{
        "doi_system.CognitiveRecord/id": internal_record_id,
        "doi_system.CognitiveRecord/apiVersion": api_version,
        "doi_system.CognitiveRecord/originalPayload": str(raw_data),
        "doi_system.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "doi_system.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_doi_system_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "doi_system_entity",
