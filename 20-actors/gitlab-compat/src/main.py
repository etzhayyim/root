"""
Py Kotodama WASM entrypoint for Gitlab Cognitive Actor.
...
@app.route("/{api_version}/gitlab/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gitlab_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gitlab.CognitiveRecord/id": internal_record_id,
        "gitlab.CognitiveRecord/apiVersion": api_version,
        "gitlab.CognitiveRecord/originalPayload": str(raw_data),
        "gitlab.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gitlab.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gitlab_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gitlab_entity",
