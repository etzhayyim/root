"""
Py Kotodama WASM entrypoint for Un_locode Cognitive Actor.
...
@app.route("/{api_version}/un_locode/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_un_locode_" + uuid.uuid4().hex[:12]
    db.transact([{
        "un_locode.CognitiveRecord/id": internal_record_id,
        "un_locode.CognitiveRecord/apiVersion": api_version,
        "un_locode.CognitiveRecord/originalPayload": str(raw_data),
        "un_locode.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "un_locode.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_un_locode_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "un_locode_entity",
