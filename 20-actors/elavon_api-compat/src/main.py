"""
Py Kotodama WASM entrypoint for Elavon_api Cognitive Actor.
...
schema = load_schema("../schema/elavon_api.kotoba")
db = DatomicClient.connect()
app = Runtime("elavon_api-compat")
...
@app.route("/{api_version}/elavon_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_elavon_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "elavon_api.CognitiveRecord/id": internal_record_id,
        "elavon_api.CognitiveRecord/apiVersion": api_version,
        "elavon_api.CognitiveRecord/originalPayload": str(raw_data),
        "elavon_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "elavon_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_elavon_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "elavon_api_entity",
