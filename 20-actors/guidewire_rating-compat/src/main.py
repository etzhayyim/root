"""
Py Kotodama WASM entrypoint for Guidewire_rating Cognitive Actor.
...
schema = load_schema("../schema/guidewire_rating.kotoba")
db = DatomicClient.connect()
app = Runtime("guidewire_rating-compat")
...
@app.route("/{api_version}/guidewire_rating/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_guidewire_rating_" + uuid.uuid4().hex[:12]
    db.transact([{
        "guidewire_rating.CognitiveRecord/id": internal_record_id,
        "guidewire_rating.CognitiveRecord/apiVersion": api_version,
        "guidewire_rating.CognitiveRecord/originalPayload": str(raw_data),
        "guidewire_rating.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "guidewire_rating.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_guidewire_rating_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "guidewire_rating_entity",
