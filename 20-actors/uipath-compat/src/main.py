"""
Py Kotodama WASM entrypoint for Uipath Cognitive Actor.
...
@app.route("/{api_version}/uipath/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_uipath_" + uuid.uuid4().hex[:12]
    db.transact([{
        "uipath.CognitiveRecord/id": internal_record_id,
        "uipath.CognitiveRecord/apiVersion": api_version,
        "uipath.CognitiveRecord/originalPayload": str(raw_data),
        "uipath.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "uipath.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_uipath_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "uipath_entity",
