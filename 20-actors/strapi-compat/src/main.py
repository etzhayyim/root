"""
Py Kotodama WASM entrypoint for Strapi Cognitive Actor.
...
schema = load_schema("../schema/strapi.kotoba")
db = DatomicClient.connect()
app = Runtime("strapi-compat")
...
@app.route("/{api_version}/strapi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_strapi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "strapi.CognitiveRecord/id": internal_record_id,
        "strapi.CognitiveRecord/apiVersion": api_version,
        "strapi.CognitiveRecord/originalPayload": str(raw_data),
        "strapi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "strapi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_strapi_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "strapi_entity",
