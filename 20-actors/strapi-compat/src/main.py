"""
Py Kotodama WASM entrypoint for the Strapi clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"strapi.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_content_entry(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.ContentEntry", "id": eid})
...
def delete_product(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.Product", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_shipment(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.Shipment", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_search_index(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.SearchIndex", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_webhook(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.Webhook", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_order(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"strapi.Order", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "strapi-compat", "entities": ['ContentEntry', 'Product', 'Shipment', 'SearchIndex', 'Webhook', 'Order']}, 200
