(ns cohere.main
  "Kotodama WASM entrypoint for the Cohere clean-room actor (L4) — Clojure port.
  ...
  facts (`cohere.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cohere")
(def tier "L4")
...
(def entity-specs
  [{:entity "Dataset"    :plural "datasets"    :id-prefix "cohere_dat"
    :fields [:name :validationStatus :datasetType] :required [:name]
    :coerce {}                       :refs {}}
   {:entity "DatasetPart" :plural "datasetparts" :id-prefix "cohere_dat"
    :fields [:name :sizeBytes :numRows]          :required [:name]
    :coerce {:sizeBytes :int :numRows :int}      :refs {}}
   {:entity "Model"      :plural "models"      :id-prefix "cohere_mod"
    :fields [:name :isDeprecated :contextLength :finetuned] :required [:name :isDeprecated]
    :coerce {:contextLength :int :isDeprecated :bool :finetuned :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"cohere.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cohere-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cohere-compat/ready)
