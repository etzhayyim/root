"""
Py Kotodama WASM entrypoint for Appfolio Cognitive Actor.
...
schema = load_schema("../schema/appfolio.kotoba")
db = DatomicClient.connect()
app = Runtime("appfolio-compat")
...
@app.route("/{api_version}/appfolio/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_appfolio_" + uuid.uuid4().hex[:12]
    db.transact([{
        "appfolio.CognitiveRecord/id": internal_record_id,
        "appfolio.CognitiveRecord/apiVersion": api_version,
        "appfolio.CognitiveRecord/originalPayload": str(raw_data),
        "appfolio.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "appfolio.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_appfolio_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "appfolio_entity",
