"""
Py Kotodama WASM entrypoint for Iea Cognitive Actor.
...
@app.route("/{api_version}/iea/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iea_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iea.CognitiveRecord/id": internal_record_id,
        "iea.CognitiveRecord/apiVersion": api_version,
        "iea.CognitiveRecord/originalPayload": str(raw_data),
        "iea.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iea.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iea_" + uuid.uuid4().hex[:8])
