"""
Py Kotodama WASM entrypoint for Aadhaar Cognitive Actor.
...
schema = load_schema("../schema/aadhaar.kotoba")
db = DatomicClient.connect()
app = Runtime("aadhaar-compat")
...
@app.route("/{api_version}/aadhaar/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aadhaar_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aadhaar.CognitiveRecord/id": internal_record_id,
        "aadhaar.CognitiveRecord/apiVersion": api_version,
        "aadhaar.CognitiveRecord/originalPayload": str(raw_data),
        "aadhaar.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aadhaar.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aadhaar_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "aadhaar_entity",
