"""
Py Kotodama WASM entrypoint for Dlocal Cognitive Actor.
...
schema = load_schema("../schema/dlocal.kotoba")
db = DatomicClient.connect()
app = Runtime("dlocal-compat")
...
@app.route("/{api_version}/dlocal/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dlocal_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dlocal.CognitiveRecord/id": internal_record_id,
        "dlocal.CognitiveRecord/apiVersion": api_version,
        "dlocal.CognitiveRecord/originalPayload": str(raw_data),
        "dlocal.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dlocal.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dlocal_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "dlocal_entity",
