"""
Py Kotodama WASM entrypoint for Rappi Cognitive Actor.
...
schema = load_schema("../schema/rappi.kotoba")
db = DatomicClient.connect()
app = Runtime("rappi-compat")
...
@app.route("/{api_version}/rappi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rappi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rappi.CognitiveRecord/id": internal_record_id,
        "rappi.CognitiveRecord/apiVersion": api_version,
        "rappi.CognitiveRecord/originalPayload": str(raw_data),
        "rappi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rappi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rappi_" + uuid.uuid4().hex[:8])
