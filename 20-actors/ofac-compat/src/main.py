"""
Py Kotodama WASM entrypoint for Ofac Cognitive Actor.
...
schema = load_schema("../schema/ofac.kotoba")
db = DatomicClient.connect()
app = Runtime("ofac-compat")
...
@app.route("/{api_version}/ofac/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ofac_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ofac.CognitiveRecord/id": internal_record_id,
        "ofac.CognitiveRecord/apiVersion": api_version,
        "ofac.CognitiveRecord/originalPayload": str(raw_data),
        "ofac.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ofac.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ofac_" + uuid.uuid4().hex[:8])
