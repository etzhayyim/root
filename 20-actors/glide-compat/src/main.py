"""
Py Kotodama WASM entrypoint for Glide Cognitive Actor.
...
schema = load_schema("../schema/glide.kotoba")
db = DatomicClient.connect()
app = Runtime("glide-compat")
...
@app.route("/{api_version}/glide/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_glide_" + uuid.uuid4().hex[:12]
    db.transact([{
        "glide.CognitiveRecord/id": internal_record_id,
        "glide.CognitiveRecord/apiVersion": api_version,
        "glide.CognitiveRecord/originalPayload": str(raw_data),
        "glide.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "glide.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_glide_" + uuid.uuid4().hex[:8])
