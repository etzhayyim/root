"""
Py Kotodama WASM entrypoint for the Triton clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"triton.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.Device", "id": eid})
...
def delete_kernel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.Kernel", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_alloc(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.MemoryAlloc", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inference_job(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.InferenceJob", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_engine(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.Engine", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_utilization(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"triton.Utilization", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "triton-compat", "entities": ['Device', 'Kernel', 'MemoryAlloc', 'InferenceJob', 'Engine', 'Utilization']}, 200
