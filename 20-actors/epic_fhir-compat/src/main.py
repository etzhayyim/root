"""
Py Kotodama WASM entrypoint for Epic_fhir Cognitive Actor.
...
schema = load_schema("../schema/epic_fhir.kotoba")
db = DatomicClient.connect()
app = Runtime("epic_fhir-compat")
...
@app.route("/{api_version}/epic_fhir/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_epic_fhir_" + uuid.uuid4().hex[:12]
    db.transact([{
        "epic_fhir.CognitiveRecord/id": internal_record_id,
        "epic_fhir.CognitiveRecord/apiVersion": api_version,
        "epic_fhir.CognitiveRecord/originalPayload": str(raw_data),
        "epic_fhir.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "epic_fhir.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_epic_fhir_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "epic_fhir_entity",
