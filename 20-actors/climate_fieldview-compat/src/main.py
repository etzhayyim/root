"""
Py Kotodama WASM entrypoint for Climate_fieldview Cognitive Actor.
...
schema = load_schema("../schema/climate_fieldview.kotoba")
db = DatomicClient.connect()
app = Runtime("climate_fieldview-compat")
...
@app.route("/{api_version}/climate_fieldview/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_climate_fieldview_" + uuid.uuid4().hex[:12]
    db.transact([{
        "climate_fieldview.CognitiveRecord/id": internal_record_id,
        "climate_fieldview.CognitiveRecord/apiVersion": api_version,
        "climate_fieldview.CognitiveRecord/originalPayload": str(raw_data),
        "climate_fieldview.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "climate_fieldview.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_climate_fieldview_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "climate_fieldview_entity",
