"""
Py Kotodama WASM entrypoint for Losant Cognitive Actor.
...
@app.route("/{api_version}/losant/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_losant_" + uuid.uuid4().hex[:12]
    db.transact([{
        "losant.CognitiveRecord/id": internal_record_id,
        "losant.CognitiveRecord/apiVersion": api_version,
        "losant.CognitiveRecord/originalPayload": str(raw_data),
        "losant.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "losant.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_losant_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "losant_entity",
