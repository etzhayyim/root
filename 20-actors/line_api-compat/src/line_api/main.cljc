(ns line-api.main
  "Kotodama WASM entrypoint for the Line_api clean-room actor (L4) — Clojure port.
  ...
  facts (`line_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "line_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Message"   :plural "messages"   :id-prefix "lineapi_mes"
    :fields [:type :text :originalContentUrl :previewImageUrl :duration] :required [:type :text]
    :coerce {:duration :int}          :refs {}}
   {:entity "Source"      :plural "sources"      :id-prefix "lineapi_sou"
    :fields [:type :userId :groupId :roomId] :required [:type]
    :coerce {}          :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "lineapi_eve"
    :fields [:type :mode :replyToken :timestamp :webhookEventId]        :required [:type :mode]
    :coerce {:timestamp :int} :refs {}}
   {:entity "Profile" :plural "profiles" :id-prefix "lineapi_pro"
    :fields [:displayName :userId :pictureUrl :statusMessage :language]          :required [:displayName :pictureUrl]
    :coerce {}                       :refs {}}
   {:entity "SentMessage"   :plural "sentmessages"   :id-prefix "lineapi_sen"
    :fields [:quoteToken]              :required [:quoteToken]
    :coerce {}          :refs {}}
   {:entity "NarrowcastProgress"    :plural "narrowcastprogresses"    :id-prefix "lineapi_nar"
    :fields [:phase :successCount :failureCount :targetCount]  :required [:phase :successCount]
    :coerce {:successCount :int :failureCount :int :targetCount :int}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"line_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "line_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :line_api-compat/ready)
