"""
Py Kotodama WASM entrypoint for the Line_api clean-room actor.
...
def delete_channel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"line_api.Channel", "id": eid})
...
def delete_user(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"line_api.User", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_room(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"line_api.Room", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_stream(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"line_api.Stream", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_webhook(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"line_api.Webhook", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "line_api-compat", "entities": ['Channel', 'Message', 'User', 'Room', 'Stream', 'Webhook']}, 200
