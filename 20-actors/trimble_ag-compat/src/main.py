"""
Py Kotodama WASM entrypoint for Trimble_ag Cognitive Actor.
...
schema = load_schema("../schema/trimble_ag.kotoba")
db = DatomicClient.connect()
app = Runtime("trimble_ag-compat")
...
@app.route("/{api_version}/trimble_ag/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_trimble_ag_" + uuid.uuid4().hex[:12]
    db.transact([{
        "trimble_ag.CognitiveRecord/id": internal_record_id,
        "trimble_ag.CognitiveRecord/apiVersion": api_version,
        "trimble_ag.CognitiveRecord/originalPayload": str(raw_data),
        "trimble_ag.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "trimble_ag.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_trimble_ag_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "trimble_ag_entity",
