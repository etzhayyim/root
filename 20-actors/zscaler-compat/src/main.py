"""
Py Kotodama WASM entrypoint for Zscaler Cognitive Actor.
...
schema = load_schema("../schema/zscaler.kotoba")
db = DatomicClient.connect()
app = Runtime("zscaler-compat")
...
@app.route("/{api_version}/zscaler/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zscaler_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zscaler.CognitiveRecord/id": internal_record_id,
        "zscaler.CognitiveRecord/apiVersion": api_version,
        "zscaler.CognitiveRecord/originalPayload": str(raw_data),
        "zscaler.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zscaler.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zscaler_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zscaler_entity",
