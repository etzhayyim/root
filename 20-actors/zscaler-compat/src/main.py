"""
Py Kotodama WASM entrypoint for the Zscaler clean-room actor (L4).
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"zscaler.{entity}/{k}"] = v
    db.transact([facts])
    return rec


def _query(entity, eid=None):
    pattern = {"entity": f"zscaler.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_identity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Identity", "id": eid})
...
def delete_policy(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Policy", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_detection(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Detection", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_incident(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Incident", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_asset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Asset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"zscaler.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "zscaler-compat", "tier": "L4",
