"""
Py Kotodama WASM entrypoint for Hkma Cognitive Actor.
...
@app.route("/{api_version}/hkma/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hkma_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hkma.CognitiveRecord/id": internal_record_id,
        "hkma.CognitiveRecord/apiVersion": api_version,
        "hkma.CognitiveRecord/originalPayload": str(raw_data),
        "hkma.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hkma.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hkma_" + uuid.uuid4().hex[:8])
