"""
Py Kotodama WASM entrypoint for Trendyol Cognitive Actor.
...
schema = load_schema("../schema/trendyol.kotoba")
db = DatomicClient.connect()
app = Runtime("trendyol-compat")
...
@app.route("/{api_version}/trendyol/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_trendyol_" + uuid.uuid4().hex[:12]
    db.transact([{
        "trendyol.CognitiveRecord/id": internal_record_id,
        "trendyol.CognitiveRecord/apiVersion": api_version,
        "trendyol.CognitiveRecord/originalPayload": str(raw_data),
        "trendyol.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "trendyol.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_trendyol_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "trendyol_entity",
