...
(ns meyasu.py.test-pipeline-cid
  ...
  (:require [meyasu.py.autorun :as autorun]
  ...
  (let [in-proc (in-process-head 3)
        child (try
                (sh "bb" "--classpath" "20-actors" "-e"
                    (str "(require (quote [meyasu.py.autorun :as a]))"
...
(when (= *file* (System/getProperty "babashka.file"))
  (let [{:keys [fail error]} (run-tests 'meyasu.py.test-pipeline-cid)]
