"""
Py Kotodama WASM entrypoint for Appsflyer Cognitive Actor.
...
schema = load_schema("../schema/appsflyer.kotoba")
db = DatomicClient.connect()
app = Runtime("appsflyer-compat")
...
@app.route("/{api_version}/appsflyer/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_appsflyer_" + uuid.uuid4().hex[:12]
    db.transact([{
        "appsflyer.CognitiveRecord/id": internal_record_id,
        "appsflyer.CognitiveRecord/apiVersion": api_version,
        "appsflyer.CognitiveRecord/originalPayload": str(raw_data),
        "appsflyer.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "appsflyer.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_appsflyer_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "appsflyer_entity",
