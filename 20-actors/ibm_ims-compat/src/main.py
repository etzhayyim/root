"""
Py Kotodama WASM entrypoint for Ibm_ims Cognitive Actor.
...
@app.route("/{api_version}/ibm_ims/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ibm_ims_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ibm_ims.CognitiveRecord/id": internal_record_id,
        "ibm_ims.CognitiveRecord/apiVersion": api_version,
        "ibm_ims.CognitiveRecord/originalPayload": str(raw_data),
        "ibm_ims.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ibm_ims.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ibm_ims_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ibm_ims_entity",
