"""
Py Kotodama WASM entrypoint for Particle Cognitive Actor.
...
@app.route("/{api_version}/particle/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_particle_" + uuid.uuid4().hex[:12]
    db.transact([{
        "particle.CognitiveRecord/id": internal_record_id,
        "particle.CognitiveRecord/apiVersion": api_version,
        "particle.CognitiveRecord/originalPayload": str(raw_data),
        "particle.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "particle.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_particle_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "particle_entity",
