"""
Py Kotodama WASM entrypoint for Zymergen Cognitive Actor.
...
schema = load_schema("../schema/zymergen.kotoba")
db = DatomicClient.connect()
app = Runtime("zymergen-compat")
...
@app.route("/{api_version}/zymergen/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zymergen_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zymergen.CognitiveRecord/id": internal_record_id,
        "zymergen.CognitiveRecord/apiVersion": api_version,
        "zymergen.CognitiveRecord/originalPayload": str(raw_data),
        "zymergen.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zymergen.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zymergen_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zymergen_entity",
