"""
Py Kotodama WASM entrypoint for Nih Cognitive Actor.
...
schema = load_schema("../schema/nih.kotoba")
db = DatomicClient.connect()
app = Runtime("nih-compat")
...
@app.route("/{api_version}/nih/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nih_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nih.CognitiveRecord/id": internal_record_id,
        "nih.CognitiveRecord/apiVersion": api_version,
        "nih.CognitiveRecord/originalPayload": str(raw_data),
        "nih.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nih.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nih_" + uuid.uuid4().hex[:8])
