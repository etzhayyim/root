"""
Py Kotodama WASM entrypoint for Checkout Cognitive Actor.
...
schema = load_schema("../schema/checkout.kotoba")
db = DatomicClient.connect()
app = Runtime("checkout-compat")
...
@app.route("/{api_version}/checkout/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_checkout_" + uuid.uuid4().hex[:12]
    db.transact([{
        "checkout.CognitiveRecord/id": internal_record_id,
        "checkout.CognitiveRecord/apiVersion": api_version,
        "checkout.CognitiveRecord/originalPayload": str(raw_data),
        "checkout.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "checkout.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_checkout_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "checkout_entity",
