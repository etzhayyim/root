"""
Py Kotodama WASM entrypoint for Tesla_api Cognitive Actor.
...
schema = load_schema("../schema/tesla_api.kotoba")
db = DatomicClient.connect()
app = Runtime("tesla_api-compat")
...
@app.route("/{api_version}/tesla_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tesla_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tesla_api.CognitiveRecord/id": internal_record_id,
        "tesla_api.CognitiveRecord/apiVersion": api_version,
        "tesla_api.CognitiveRecord/originalPayload": str(raw_data),
        "tesla_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tesla_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tesla_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tesla_api_entity",
