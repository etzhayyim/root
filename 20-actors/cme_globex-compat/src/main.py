"""
Py Kotodama WASM entrypoint for Cme_globex Cognitive Actor.
...
schema = load_schema("../schema/cme_globex.kotoba")
db = DatomicClient.connect()
app = Runtime("cme_globex-compat")
...
@app.route("/{api_version}/cme_globex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cme_globex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cme_globex.CognitiveRecord/id": internal_record_id,
        "cme_globex.CognitiveRecord/apiVersion": api_version,
        "cme_globex.CognitiveRecord/originalPayload": str(raw_data),
        "cme_globex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cme_globex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cme_globex_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cme_globex_entity",
