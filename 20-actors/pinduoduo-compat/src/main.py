"""
Py Kotodama WASM entrypoint for Pinduoduo Cognitive Actor.
...
schema = load_schema("../schema/pinduoduo.kotoba")
db = DatomicClient.connect()
app = Runtime("pinduoduo-compat")
...
@app.route("/{api_version}/pinduoduo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_pinduoduo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "pinduoduo.CognitiveRecord/id": internal_record_id,
        "pinduoduo.CognitiveRecord/apiVersion": api_version,
        "pinduoduo.CognitiveRecord/originalPayload": str(raw_data),
        "pinduoduo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "pinduoduo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_pinduoduo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "pinduoduo_entity",
