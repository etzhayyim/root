...
ingested merge) and emits AGGREGATE-FIRST observations:
  ...
  - out/intel-report.md  +  out/financial-metrics.kotoba.edn

NON-ADJUDICATING (G2) / NO ADVICE (G4): every number here is either a figure the
company disclosed or a transparent ratio of two disclosed figures. kanjō reports;
it never rates "good/bad", values a company, or recommends an action.
...
from concept_map import metric_inputs  # noqa: E402

# Fallback company meta (name / sector / country) for the seed cohort, used only when
# kabuto's :company graph is unavailable. kabuto (shared org.corp.* id space) is the SSoT —
# load_company_meta() joins it first so sector/country come from one place (G5).
COMPANY_META = {
...
def load(path):
    rows = kanjo_edn.read_file(path)
    filings = {r["fin.filing/id" if "fin.filing/id" in r else ":fin.filing/id"]: r
               for r in rows if ":fin.filing/id" in r}
    facts = [r for r in rows if ":fin.fact/id" in r]
    return filings, facts


def fact_key(f):
    return (f[":fin.fact/company"], int(f[":fin.fact/filing"].split(".")[-1]) if False else None)
