"""
Py Kotodama WASM entrypoint for Shopify Cognitive Actor.
...
@app.route("/{api_version}/shopify/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shopify_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shopify.CognitiveRecord/id": internal_record_id,
        "shopify.CognitiveRecord/apiVersion": api_version,
        "shopify.CognitiveRecord/originalPayload": str(raw_data),
        "shopify.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shopify.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shopify_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shopify_entity",
