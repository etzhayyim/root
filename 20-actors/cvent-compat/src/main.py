"""
Py Kotodama WASM entrypoint for Cvent Cognitive Actor.
...
schema = load_schema("../schema/cvent.kotoba")
db = DatomicClient.connect()
app = Runtime("cvent-compat")
...
@app.route("/{api_version}/cvent/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cvent_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cvent.CognitiveRecord/id": internal_record_id,
        "cvent.CognitiveRecord/apiVersion": api_version,
        "cvent.CognitiveRecord/originalPayload": str(raw_data),
        "cvent.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cvent.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cvent_" + uuid.uuid4().hex[:8])
