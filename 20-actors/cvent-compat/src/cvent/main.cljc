(ns cvent.main
  "Kotodama WASM entrypoint for the Cvent clean-room actor (L4) — Clojure port.
  ...
  facts (`cvent.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cvent.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cvent-compat" :tier tier :entities entities} 200])
