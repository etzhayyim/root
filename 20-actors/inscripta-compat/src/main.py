"""
Py Kotodama WASM entrypoint for Inscripta Cognitive Actor.
...
schema = load_schema("../schema/inscripta.kotoba")
db = DatomicClient.connect()
app = Runtime("inscripta-compat")
...
@app.route("/{api_version}/inscripta/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_inscripta_" + uuid.uuid4().hex[:12]
    db.transact([{
        "inscripta.CognitiveRecord/id": internal_record_id,
        "inscripta.CognitiveRecord/apiVersion": api_version,
        "inscripta.CognitiveRecord/originalPayload": str(raw_data),
        "inscripta.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "inscripta.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_inscripta_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "inscripta_entity",
