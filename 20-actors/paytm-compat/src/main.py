"""
Py Kotodama WASM entrypoint for Paytm Cognitive Actor.
...
schema = load_schema("../schema/paytm.kotoba")
db = DatomicClient.connect()
app = Runtime("paytm-compat")
...
@app.route("/{api_version}/paytm/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_paytm_" + uuid.uuid4().hex[:12]
    db.transact([{
        "paytm.CognitiveRecord/id": internal_record_id,
        "paytm.CognitiveRecord/apiVersion": api_version,
        "paytm.CognitiveRecord/originalPayload": str(raw_data),
        "paytm.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "paytm.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_paytm_" + uuid.uuid4().hex[:8])
