...
;; 20-actors); shells out to system binaries (cargo/wasm-tools/node/ipfs) via
;; babashka.process, which is permitted. Produces dist/maps-core.wasm + .cid and
;; asserts the place-not-person invariant on the actual wasm output.
...
;; content-addressed CIDv1 (raw) — byte-identical to `ipfs add --cid-version=1 --raw-leaves`
(def cid (str/trim (:out (shell {:out :string}
                                "ipfs" "add" "-Q" "--only-hash" "--cid-version=1" out))))
