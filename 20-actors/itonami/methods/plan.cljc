(ns itonami.methods.plan
  ...
  Translates OEE observations into ACTUAL output (units/day) through a SECOND lens: the
  takt-capacity bottleneck (uptime ÷ takt). The OEE-worst and throughput-worst stations are
  often DIFFERENT — surfacing that gap is the operations intelligence FOX sells.
    ...
    line-plan        — line throughput bottleneck + units/window + units/day
    relief-plan      — recover the throughput-bottleneck's idle/down → new line throughput

  CONSTITUTIONAL: G1 recommends a plan; G2 capacity relief is AVAILABILITY recovery WITHIN takt
  (never sub-takt speed-up); G3 read-time aggregate (operating-hours is documented, G5).
...
(defn- num
  ...
  (let [v (if (or (nil? v) (false? v)) 0 v)]
    (double (if (number? v) v (try (Double/parseDouble (str v)) (catch Exception _ 0.0))))))
  ...
  (reduce
   (fn [out sid]
     (let [takt (num (get-in stations [sid ":station/takt-s"] 0))
       ...
       (assoc out sid {"takt" takt "capacity_run" cap-run "capacity_planned" cap-planned
                       "actual_cycles" (get r "cycles") "quality" (get r "quality")})))
   {} (res-stations res)))

(defn- window-s
  [res]
  (let [ps (map #(get-in res [% "planned_s"]) (res-stations res))]
    (if (seq ps) (reduce max ps) 0.0)))

;; capacity must preserve station insertion order for the min argmin tie semantics
(defn- cap-order [stations res] (vec (res-stations res)))
...
(defn line-plan
  ...
  ([stations res hours]
   (let [cap (station-capacity stations res)
         sids (cap-order stations res)]
     (if (empty? sids)
       {}
       (let [w (window-s res)
             bn (reduce (fn [best s]
                          (if (< (get-in cap [s "capacity_run"]) (get-in cap [best "capacity_run"])) s best))
...
(defn relief-plan
  ...
  ([stations res plan hours]
   (let [plan (or plan (line-plan stations res hours))
         ...
         sids (get plan "_cap_order")
         bn-recovered (get-in cap [bn "capacity_planned"])
         others (vec (map #(get-in cap [% "capacity_run"]) (filter #(not= % bn) sids)))
         new-units-per-window (if (seq others) (reduce min (cons bn-recovered others)) bn-recovered)
     ...
     {"bottleneck" bn
      ...
      "recovered_units_per_window" new-units-per-window
      "uplift_frac" (if (> cur 0) (/ (- new-units-per-window cur) cur) 0.0)
      "current_units_per_day" (* cur day-scale)
      "recovered_units_per_day" (* new-units-per-window day-scale)
      "lever" "availability recovery (remove idle/down stops) within takt; per-cycle pace unchanged"
      "still_takt_limited" (and (<= new-units-per-window (+ (get-in cap [bn "capacity_planned"]) 1e-9))
                                (< (Math/abs (double (- new-units-per-window bn-recovered))) 1e-9))})))
...
(defn report-md
  ...
  (let [oee-bn (get-in res ["_recommend" "bottleneck" "station"])
    ...
    (P (str "\n**Two bottlenecks, two lenses:** the OEE-worst station is **"
            (label stations oee-bn) "**, but the THROUGHPUT-worst (takt-capacity) station is **"
            (label stations (get plan "throughput_bottleneck")) "** — relieve the right one "
            "for the right goal.\n"))
    ...
    (P "|---|---:|---:|---:|")
    (doseq [sid (sort-by (fn [s] (get-in plan ["capacity" s "capacity_run"])) (get plan "_cap_order"))]
      (let [c (get-in plan ["capacity" sid])]
...
(defn emit
  ...
  ([plan relief tx]
   (let [L (transient [";; itonami R5 throughput plan — TRANSIENT (:bond/is-transient true), G1/G3." "["])]
     (conj! L (str "[:line.sarutahiko-a :ops/throughput-bottleneck " (get plan "throughput_bottleneck")
                   " " tx " :derived] ;; :bond/is-transient true"))
     (conj! L (str "[:line.sarutahiko-a :ops/units-per-day-good " (fmt-g (get plan "units_per_day_good"))
                   " " tx " :derived] ;; :bond/is-transient true"))
     (conj! L (str "[:line.sarutahiko-a :ops/throughput-uplift-frac " (fmt-g (get relief "uplift_frac"))
                   " " tx " :derived] ;; :bond/is-transient true"))
     (conj! L "]")
     (str (str/join "\n" (persistent! L)) "\n"))))
