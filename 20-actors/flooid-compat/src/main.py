"""
Py Kotodama WASM entrypoint for Flooid Cognitive Actor.
...
schema = load_schema("../schema/flooid.kotoba")
db = DatomicClient.connect()
app = Runtime("flooid-compat")
...
@app.route("/{api_version}/flooid/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flooid_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flooid.CognitiveRecord/id": internal_record_id,
        "flooid.CognitiveRecord/apiVersion": api_version,
        "flooid.CognitiveRecord/originalPayload": str(raw_data),
        "flooid.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flooid.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flooid_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "flooid_entity",
