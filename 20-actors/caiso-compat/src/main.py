"""
Py Kotodama WASM entrypoint for Caiso Cognitive Actor.
...
schema = load_schema("../schema/caiso.kotoba")
db = DatomicClient.connect()
app = Runtime("caiso-compat")
...
@app.route("/{api_version}/caiso/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_caiso_" + uuid.uuid4().hex[:12]
    db.transact([{
        "caiso.CognitiveRecord/id": internal_record_id,
        "caiso.CognitiveRecord/apiVersion": api_version,
        "caiso.CognitiveRecord/originalPayload": str(raw_data),
        "caiso.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "caiso.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_caiso_" + uuid.uuid4().hex[:8])
