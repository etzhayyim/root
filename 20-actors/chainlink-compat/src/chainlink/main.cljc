(ns chainlink.main
  "Kotodama WASM entrypoint for the Chainlink clean-room actor (L4) — Clojure port.
  ...
  facts (`chainlink.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "chainlink")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"chainlink.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "chainlink-compat" :tier tier :entities entities} 200])
...
(defn start! [] :chainlink-compat/ready)
