"""
Py Kotodama WASM entrypoint for Chainlink Cognitive Actor.
...
@app.route("/{api_version}/chainlink/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_chainlink_" + uuid.uuid4().hex[:12]
    db.transact([{
        "chainlink.CognitiveRecord/id": internal_record_id,
        "chainlink.CognitiveRecord/apiVersion": api_version,
        "chainlink.CognitiveRecord/originalPayload": str(raw_data),
        "chainlink.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "chainlink.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_chainlink_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "chainlink_entity",
