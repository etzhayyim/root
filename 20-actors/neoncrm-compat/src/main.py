"""
Py Kotodama WASM entrypoint for Neoncrm Cognitive Actor.
...
schema = load_schema("../schema/neoncrm.kotoba")
db = DatomicClient.connect()
app = Runtime("neoncrm-compat")
...
@app.route("/{api_version}/neoncrm/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_neoncrm_" + uuid.uuid4().hex[:12]
    db.transact([{
        "neoncrm.CognitiveRecord/id": internal_record_id,
        "neoncrm.CognitiveRecord/apiVersion": api_version,
        "neoncrm.CognitiveRecord/originalPayload": str(raw_data),
        "neoncrm.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "neoncrm.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_neoncrm_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "neoncrm_entity",
