"""
Py Kotodama WASM entrypoint for Powerbi Cognitive Actor.
...
@app.route("/{api_version}/powerbi/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_powerbi_" + uuid.uuid4().hex[:12]
    db.transact([{
        "powerbi.CognitiveRecord/id": internal_record_id,
        "powerbi.CognitiveRecord/apiVersion": api_version,
        "powerbi.CognitiveRecord/originalPayload": str(raw_data),
        "powerbi.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "powerbi.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_powerbi_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "powerbi_entity",
