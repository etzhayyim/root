"""
Py Kotodama WASM entrypoint for Datadog Cognitive Actor.
...
schema = load_schema("../schema/datadog.kotoba")
db = DatomicClient.connect()
app = Runtime("datadog-compat")
...
@app.route("/{api_version}/datadog/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_datadog_" + uuid.uuid4().hex[:12]
    db.transact([{
        "datadog.CognitiveRecord/id": internal_record_id,
        "datadog.CognitiveRecord/apiVersion": api_version,
        "datadog.CognitiveRecord/originalPayload": str(raw_data),
        "datadog.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "datadog.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_datadog_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "datadog_entity",
