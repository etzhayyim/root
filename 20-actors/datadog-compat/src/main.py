"""
Py Kotodama WASM entrypoint for the Datadog clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"datadog.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_incident(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"datadog.Incident", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "datadog-compat", "entities": ['Service', 'Error', 'Trace', 'Metric', 'Alert', 'Incident']}, 200
