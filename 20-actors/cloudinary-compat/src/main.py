"""
Py Kotodama WASM entrypoint for Cloudinary Cognitive Actor.
...
@app.route("/{api_version}/cloudinary/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cloudinary_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cloudinary.CognitiveRecord/id": internal_record_id,
        "cloudinary.CognitiveRecord/apiVersion": api_version,
        "cloudinary.CognitiveRecord/originalPayload": str(raw_data),
        "cloudinary.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cloudinary.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cloudinary_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cloudinary_entity",
