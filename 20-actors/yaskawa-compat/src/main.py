"""
Py Kotodama WASM entrypoint for Yaskawa Cognitive Actor.
...
schema = load_schema("../schema/yaskawa.kotoba")
db = DatomicClient.connect()
app = Runtime("yaskawa-compat")
...
@app.route("/{api_version}/yaskawa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yaskawa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yaskawa.CognitiveRecord/id": internal_record_id,
        "yaskawa.CognitiveRecord/apiVersion": api_version,
        "yaskawa.CognitiveRecord/originalPayload": str(raw_data),
        "yaskawa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yaskawa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yaskawa_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "yaskawa_entity",
