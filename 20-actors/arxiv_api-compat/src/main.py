"""
Py Kotodama WASM entrypoint for Arxiv_api Cognitive Actor.
...
schema = load_schema("../schema/arxiv_api.kotoba")
db = DatomicClient.connect()
app = Runtime("arxiv_api-compat")
...
@app.route("/{api_version}/arxiv_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_arxiv_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "arxiv_api.CognitiveRecord/id": internal_record_id,
        "arxiv_api.CognitiveRecord/apiVersion": api_version,
        "arxiv_api.CognitiveRecord/originalPayload": str(raw_data),
        "arxiv_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "arxiv_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_arxiv_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "arxiv_api_entity",
