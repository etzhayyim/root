"""
Py Kotodama WASM entrypoint for Gepredix Cognitive Actor.
...
schema = load_schema("../schema/gepredix.kotoba")
db = DatomicClient.connect()
app = Runtime("gepredix-compat")
...
@app.route("/{api_version}/gepredix/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gepredix_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gepredix.CognitiveRecord/id": internal_record_id,
        "gepredix.CognitiveRecord/apiVersion": api_version,
        "gepredix.CognitiveRecord/originalPayload": str(raw_data),
        "gepredix.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gepredix.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gepredix_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gepredix_entity",
