"""
Py Kotodama WASM entrypoint for Shippo Cognitive Actor.
...
@app.route("/{api_version}/shippo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shippo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shippo.CognitiveRecord/id": internal_record_id,
        "shippo.CognitiveRecord/apiVersion": api_version,
        "shippo.CognitiveRecord/originalPayload": str(raw_data),
        "shippo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shippo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shippo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shippo_entity",
