"""
Py Kotodama WASM entrypoint for Canva Cognitive Actor.
...
@app.route("/{api_version}/canva/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_canva_" + uuid.uuid4().hex[:12]
    db.transact([{
        "canva.CognitiveRecord/id": internal_record_id,
        "canva.CognitiveRecord/apiVersion": api_version,
        "canva.CognitiveRecord/originalPayload": str(raw_data),
        "canva.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "canva.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_canva_" + uuid.uuid4().hex[:8])
