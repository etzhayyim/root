"""
Py Kotodama WASM entrypoint for Duckcreek Cognitive Actor.
...
schema = load_schema("../schema/duckcreek.kotoba")
db = DatomicClient.connect()
app = Runtime("duckcreek-compat")
...
@app.route("/{api_version}/duckcreek/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_duckcreek_" + uuid.uuid4().hex[:12]
    db.transact([{
        "duckcreek.CognitiveRecord/id": internal_record_id,
        "duckcreek.CognitiveRecord/apiVersion": api_version,
        "duckcreek.CognitiveRecord/originalPayload": str(raw_data),
        "duckcreek.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "duckcreek.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_duckcreek_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "duckcreek_entity",
