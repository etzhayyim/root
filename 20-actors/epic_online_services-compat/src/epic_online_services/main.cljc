(ns epic_online_services.main
  "Kotodama WASM entrypoint for the epic_online_services clean-room actor (L4) — Clojure port.
  ...
  facts (`epic_online_services.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "epic_online_services")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"epic_online_services.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epic_online_services-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epic_online_services-compat/ready)
