"""
Py Kotodama WASM entrypoint for Discover_global Cognitive Actor.
...
schema = load_schema("../schema/discover_global.kotoba")
db = DatomicClient.connect()
app = Runtime("discover_global-compat")
...
@app.route("/{api_version}/discover_global/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_discover_global_" + uuid.uuid4().hex[:12]
    db.transact([{
        "discover_global.CognitiveRecord/id": internal_record_id,
        "discover_global.CognitiveRecord/apiVersion": api_version,
        "discover_global.CognitiveRecord/originalPayload": str(raw_data),
        "discover_global.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "discover_global.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_discover_global_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "discover_global_entity",
