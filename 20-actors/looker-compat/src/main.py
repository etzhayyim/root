"""
Py Kotodama WASM entrypoint for Looker Cognitive Actor.
...
@app.route("/{api_version}/looker/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_looker_" + uuid.uuid4().hex[:12]
    db.transact([{
        "looker.CognitiveRecord/id": internal_record_id,
        "looker.CognitiveRecord/apiVersion": api_version,
        "looker.CognitiveRecord/originalPayload": str(raw_data),
        "looker.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "looker.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_looker_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "looker_entity",
