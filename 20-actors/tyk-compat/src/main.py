"""
Py Kotodama WASM entrypoint for Tyk Cognitive Actor.
...
schema = load_schema("../schema/tyk.kotoba")
db = DatomicClient.connect()
app = Runtime("tyk-compat")
...
@app.route("/{api_version}/tyk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tyk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tyk.CognitiveRecord/id": internal_record_id,
        "tyk.CognitiveRecord/apiVersion": api_version,
        "tyk.CognitiveRecord/originalPayload": str(raw_data),
        "tyk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tyk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tyk_" + uuid.uuid4().hex[:8])
