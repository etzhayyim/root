"""
Py Kotodama WASM entrypoint for Bic_container_codes Cognitive Actor.
...
schema = load_schema("../schema/bic_container_codes.kotoba")
db = DatomicClient.connect()
app = Runtime("bic_container_codes-compat")
...
@app.route("/{api_version}/bic_container_codes/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bic_container_codes_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bic_container_codes.CognitiveRecord/id": internal_record_id,
        "bic_container_codes.CognitiveRecord/apiVersion": api_version,
        "bic_container_codes.CognitiveRecord/originalPayload": str(raw_data),
        "bic_container_codes.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bic_container_codes.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bic_container_codes_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bic_container_codes_entity",
