"""
Py Kotodama WASM entrypoint for Mavlink_drones Cognitive Actor.
...
schema = load_schema("../schema/mavlink_drones.kotoba")
db = DatomicClient.connect()
app = Runtime("mavlink_drones-compat")
...
@app.route("/{api_version}/mavlink_drones/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mavlink_drones_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mavlink_drones.CognitiveRecord/id": internal_record_id,
        "mavlink_drones.CognitiveRecord/apiVersion": api_version,
        "mavlink_drones.CognitiveRecord/originalPayload": str(raw_data),
        "mavlink_drones.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mavlink_drones.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mavlink_drones_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mavlink_drones_entity",
