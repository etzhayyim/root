"""
Py Kotodama WASM entrypoint for Hp_vos3 Cognitive Actor.
...
schema = load_schema("../schema/hp_vos3.kotoba")
db = DatomicClient.connect()
app = Runtime("hp_vos3-compat")
...
@app.route("/{api_version}/hp_vos3/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hp_vos3_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hp_vos3.CognitiveRecord/id": internal_record_id,
        "hp_vos3.CognitiveRecord/apiVersion": api_version,
        "hp_vos3.CognitiveRecord/originalPayload": str(raw_data),
        "hp_vos3.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hp_vos3.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hp_vos3_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hp_vos3_entity",
