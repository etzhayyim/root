...
describe("Bunker Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:bunker.gftd.ai"); });
  ...
  it("subscribes to bunker.fuelSample for compliance check", () => {
    ...
    expect(sub.trigger.collections).toContain("ai.gftd.apps.bunker.fuelSample");
  });
  ...
  it("xrpc covers delivery, samples, consumption, MARPOL", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.bunker.supply.getDelivery");
    expect(nsids).toContain("ai.gftd.apps.bunker.supply.listSamples");
    expect(nsids).toContain("ai.gftd.apps.bunker.supply.getConsumption");
    expect(nsids).toContain("ai.gftd.apps.bunker.supply.checkMarpolCompliance");
  });
