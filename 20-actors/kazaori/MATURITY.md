...
- 2026-06-02 registry deepening (mid-tier long-tail): `registry/agencies.seed.json` を中位 jurisdiction で拡充 — 計 255 entries / 63 distinct jurisdictions に到達。これに伴い `test_kazaori_registry_seed.py` の worldwide-coverage 閾値を `>= 12` → `>= 55` に引き上げ(他の assertion は不変)。8 test 緑維持。
