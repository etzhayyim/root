"""
Py Kotodama WASM entrypoint for Cartegraph Cognitive Actor.
...
schema = load_schema("../schema/cartegraph.kotoba")
db = DatomicClient.connect()
app = Runtime("cartegraph-compat")
...
@app.route("/{api_version}/cartegraph/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cartegraph_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cartegraph.CognitiveRecord/id": internal_record_id,
        "cartegraph.CognitiveRecord/apiVersion": api_version,
        "cartegraph.CognitiveRecord/originalPayload": str(raw_data),
        "cartegraph.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cartegraph.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cartegraph_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cartegraph_entity",
