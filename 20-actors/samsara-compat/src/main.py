"""
Py Kotodama WASM entrypoint for Samsara Cognitive Actor.
...
schema = load_schema("../schema/samsara.kotoba")
db = DatomicClient.connect()
app = Runtime("samsara-compat")
...
@app.route("/{api_version}/samsara/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_samsara_" + uuid.uuid4().hex[:12]
    db.transact([{
        "samsara.CognitiveRecord/id": internal_record_id,
        "samsara.CognitiveRecord/apiVersion": api_version,
        "samsara.CognitiveRecord/originalPayload": str(raw_data),
        "samsara.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "samsara.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_samsara_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "samsara_entity",
