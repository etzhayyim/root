(ns ejustice-eu.main
  "Kotodama WASM entrypoint for the Ejustice_eu clean-room actor (L4) — Clojure port.
  ...
  facts (`ejustice_eu.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ejustice_eu")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "ejustice_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "ejustice_par"
    :fields [:caseId :role :name]                    :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "ejustice_doc"
    :fields [:caseId :type :contentRef]              :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "ejustice_inc"
    :fields [:type :jurisdiction :occurredAt]       :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "ejustice_wat"
    :fields [:name :program :entries]                :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "ejustice_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"ejustice_eu.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ejustice_eu-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ejustice_eu-compat/ready)
