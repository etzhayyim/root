(ns ejustice-eu.main-test
  "Contract + behavioral test for the ejustice-eu-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ejustice_eu-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ejustice-eu.main-test)]
