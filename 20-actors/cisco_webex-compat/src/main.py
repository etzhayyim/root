"""
Py Kotodama WASM entrypoint for the Cisco_webex clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"cisco_webex.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"cisco_webex.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_subscriber(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.Subscriber", "id": eid})
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_number(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.Number", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_terminal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.Terminal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_usage_record(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"cisco_webex.UsageRecord", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "cisco_webex-compat", "entities": ['Subscriber', 'Session', 'Message', 'Number', 'Terminal', 'UsageRecord']}, 200
