"""
Py Kotodama WASM entrypoint for Openbankinguk Cognitive Actor.
...
schema = load_schema("../schema/openbankinguk.kotoba")
db = DatomicClient.connect()
app = Runtime("openbankinguk-compat")
...
@app.route("/{api_version}/openbankinguk/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_openbankinguk_" + uuid.uuid4().hex[:12]
    db.transact([{
        "openbankinguk.CognitiveRecord/id": internal_record_id,
        "openbankinguk.CognitiveRecord/apiVersion": api_version,
        "openbankinguk.CognitiveRecord/originalPayload": str(raw_data),
        "openbankinguk.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "openbankinguk.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_openbankinguk_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "openbankinguk_entity",
