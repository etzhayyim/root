"""
Py Kotodama WASM entrypoint for the Openbankinguk clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"openbankinguk.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"openbankinguk.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.Account", "id": eid})
...
def delete_consent(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.Consent", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_transaction(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.Transaction", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payment_initiation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.PaymentInitiation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_institution(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.Institution", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_filing(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"openbankinguk.Filing", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "openbankinguk-compat", "entities": ['Account', 'Consent', 'Transaction', 'PaymentInitiation', 'Institution', 'Filing']}, 200
