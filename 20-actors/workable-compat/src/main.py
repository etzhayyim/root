"""
Py Kotodama WASM entrypoint for Workable Cognitive Actor.
...
schema = load_schema("../schema/workable.kotoba")
db = DatomicClient.connect()
app = Runtime("workable-compat")
...
@app.route("/{api_version}/workable/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_workable_" + uuid.uuid4().hex[:12]
    db.transact([{
        "workable.CognitiveRecord/id": internal_record_id,
        "workable.CognitiveRecord/apiVersion": api_version,
        "workable.CognitiveRecord/originalPayload": str(raw_data),
        "workable.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "workable.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_workable_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "workable_entity",
