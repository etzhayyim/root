"""
Py Kotodama WASM entrypoint for Contentful Cognitive Actor.
...
@app.route("/{api_version}/contentful/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_contentful_" + uuid.uuid4().hex[:12]
    db.transact([{
        "contentful.CognitiveRecord/id": internal_record_id,
        "contentful.CognitiveRecord/apiVersion": api_version,
        "contentful.CognitiveRecord/originalPayload": str(raw_data),
        "contentful.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "contentful.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_contentful_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "contentful_entity",
