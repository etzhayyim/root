"""
Py Kotodama WASM entrypoint for Intel_hip Cognitive Actor.
...
schema = load_schema("../schema/intel_hip.kotoba")
db = DatomicClient.connect()
app = Runtime("intel_hip-compat")
...
@app.route("/{api_version}/intel_hip/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_intel_hip_" + uuid.uuid4().hex[:12]
    db.transact([{
        "intel_hip.CognitiveRecord/id": internal_record_id,
        "intel_hip.CognitiveRecord/apiVersion": api_version,
        "intel_hip.CognitiveRecord/originalPayload": str(raw_data),
        "intel_hip.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "intel_hip.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_intel_hip_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "intel_hip_entity",
