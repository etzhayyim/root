"""
Py Kotodama WASM entrypoint for the Intel_hip clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"intel_hip.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_device(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.Device", "id": eid})
...
def delete_kernel(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.Kernel", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_memory_alloc(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.MemoryAlloc", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_inference_job(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.InferenceJob", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_engine(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.Engine", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_utilization(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"intel_hip.Utilization", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "intel_hip-compat", "entities": ['Device', 'Kernel', 'MemoryAlloc', 'InferenceJob', 'Engine', 'Utilization']}, 200
