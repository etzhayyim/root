"""
Py Kotodama WASM entrypoint for the Pingidentity clean-room actor (L4).
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"pingidentity.{entity}/{k}"] = v
    db.transact([facts])
    return rec


def _query(entity, eid=None):
    pattern = {"entity": f"pingidentity.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_identity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Identity", "id": eid})
...
def delete_policy(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Policy", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_detection(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Detection", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_incident(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Incident", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_asset(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Asset", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"pingidentity.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "pingidentity-compat", "tier": "L4",
