"""
Py Kotodama WASM entrypoint for Bubble Cognitive Actor.
...
schema = load_schema("../schema/bubble.kotoba")
db = DatomicClient.connect()
app = Runtime("bubble-compat")
...
@app.route("/{api_version}/bubble/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bubble_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bubble.CognitiveRecord/id": internal_record_id,
        "bubble.CognitiveRecord/apiVersion": api_version,
        "bubble.CognitiveRecord/originalPayload": str(raw_data),
        "bubble.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bubble.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bubble_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bubble_entity",
