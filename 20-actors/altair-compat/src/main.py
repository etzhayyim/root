"""
Py Kotodama WASM entrypoint for Altair Cognitive Actor.
...
schema = load_schema("../schema/altair.kotoba")
db = DatomicClient.connect()
app = Runtime("altair-compat")
...
@app.route("/{api_version}/altair/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_altair_" + uuid.uuid4().hex[:12]
    db.transact([{
        "altair.CognitiveRecord/id": internal_record_id,
        "altair.CognitiveRecord/apiVersion": api_version,
        "altair.CognitiveRecord/originalPayload": str(raw_data),
        "altair.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "altair.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_altair_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "altair_entity",
