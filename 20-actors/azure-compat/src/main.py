"""
Py Kotodama WASM entrypoint for Azure Cognitive Actor.
...
@app.route("/{api_version}/azure/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_azure_" + uuid.uuid4().hex[:12]
    db.transact([{
        "azure.CognitiveRecord/id": internal_record_id,
        "azure.CognitiveRecord/apiVersion": api_version,
        "azure.CognitiveRecord/originalPayload": str(raw_data),
        "azure.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "azure.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_azure_" + uuid.uuid4().hex[:8])
