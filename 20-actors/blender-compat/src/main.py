"""
Py Kotodama WASM entrypoint for Blender Cognitive Actor.
...
@app.route("/{api_version}/blender/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_blender_" + uuid.uuid4().hex[:12]
    db.transact([{
        "blender.CognitiveRecord/id": internal_record_id,
        "blender.CognitiveRecord/apiVersion": api_version,
        "blender.CognitiveRecord/originalPayload": str(raw_data),
        "blender.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "blender.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_blender_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "blender_entity",
