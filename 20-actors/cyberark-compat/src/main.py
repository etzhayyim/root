"""
Py Kotodama WASM entrypoint for Cyberark Cognitive Actor.
...
schema = load_schema("../schema/cyberark.kotoba")
db = DatomicClient.connect()
app = Runtime("cyberark-compat")
...
@app.route("/{api_version}/cyberark/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cyberark_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cyberark.CognitiveRecord/id": internal_record_id,
        "cyberark.CognitiveRecord/apiVersion": api_version,
        "cyberark.CognitiveRecord/originalPayload": str(raw_data),
        "cyberark.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cyberark.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cyberark_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cyberark_entity",
