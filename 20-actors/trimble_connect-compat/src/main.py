"""
Py Kotodama WASM entrypoint for Trimble_connect Cognitive Actor.
...
schema = load_schema("../schema/trimble_connect.kotoba")
db = DatomicClient.connect()
app = Runtime("trimble_connect-compat")
...
@app.route("/{api_version}/trimble_connect/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_trimble_connect_" + uuid.uuid4().hex[:12]
    db.transact([{
        "trimble_connect.CognitiveRecord/id": internal_record_id,
        "trimble_connect.CognitiveRecord/apiVersion": api_version,
        "trimble_connect.CognitiveRecord/originalPayload": str(raw_data),
        "trimble_connect.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "trimble_connect.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_trimble_connect_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "trimble_connect_entity",
