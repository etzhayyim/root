"""
Py Kotodama WASM entrypoint for Idnow Cognitive Actor.
...
schema = load_schema("../schema/idnow.kotoba")
db = DatomicClient.connect()
app = Runtime("idnow-compat")
...
@app.route("/{api_version}/idnow/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_idnow_" + uuid.uuid4().hex[:12]
    db.transact([{
        "idnow.CognitiveRecord/id": internal_record_id,
        "idnow.CognitiveRecord/apiVersion": api_version,
        "idnow.CognitiveRecord/originalPayload": str(raw_data),
        "idnow.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "idnow.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_idnow_" + uuid.uuid4().hex[:8])
