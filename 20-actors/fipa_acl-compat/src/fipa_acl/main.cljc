(ns fipa_acl.main
  "Kotodama WASM entrypoint for the Fipa_acl clean-room actor (L4) — Clojure port.
  ...
  facts (`fipa_acl.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fipa_acl")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fipa_acl.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fipa_acl-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fipa_acl-compat/ready)
