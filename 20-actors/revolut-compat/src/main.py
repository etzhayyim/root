"""
Py Kotodama WASM entrypoint for Revolut Cognitive Actor.
...
schema = load_schema("../schema/revolut.kotoba")
db = DatomicClient.connect()
app = Runtime("revolut-compat")
...
@app.route("/{api_version}/revolut/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_revolut_" + uuid.uuid4().hex[:12]
    db.transact([{
        "revolut.CognitiveRecord/id": internal_record_id,
        "revolut.CognitiveRecord/apiVersion": api_version,
        "revolut.CognitiveRecord/originalPayload": str(raw_data),
        "revolut.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "revolut.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_revolut_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "revolut_entity",
