"""
Py Kotodama WASM entrypoint for Helium Cognitive Actor.
...
@app.route("/{api_version}/helium/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_helium_" + uuid.uuid4().hex[:12]
    db.transact([{
        "helium.CognitiveRecord/id": internal_record_id,
        "helium.CognitiveRecord/apiVersion": api_version,
        "helium.CognitiveRecord/originalPayload": str(raw_data),
        "helium.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "helium.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_helium_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "helium_entity",
