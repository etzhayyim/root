"""
Py Kotodama WASM entrypoint for Duckcreek_rating Cognitive Actor.
...
schema = load_schema("../schema/duckcreek_rating.kotoba")
db = DatomicClient.connect()
app = Runtime("duckcreek_rating-compat")
...
@app.route("/{api_version}/duckcreek_rating/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_duckcreek_rating_" + uuid.uuid4().hex[:12]
    db.transact([{
        "duckcreek_rating.CognitiveRecord/id": internal_record_id,
        "duckcreek_rating.CognitiveRecord/apiVersion": api_version,
        "duckcreek_rating.CognitiveRecord/originalPayload": str(raw_data),
        "duckcreek_rating.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "duckcreek_rating.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_duckcreek_rating_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "duckcreek_rating_entity",
