"""
Py Kotodama WASM entrypoint for Calendly Cognitive Actor.
...
schema = load_schema("../schema/calendly.kotoba")
db = DatomicClient.connect()
app = Runtime("calendly-compat")
...
@app.route("/{api_version}/calendly/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_calendly_" + uuid.uuid4().hex[:12]
    db.transact([{
        "calendly.CognitiveRecord/id": internal_record_id,
        "calendly.CognitiveRecord/apiVersion": api_version,
        "calendly.CognitiveRecord/originalPayload": str(raw_data),
        "calendly.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "calendly.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_calendly_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "calendly_entity",
