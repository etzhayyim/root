"""
Py Kotodama WASM entrypoint for Aftership Cognitive Actor.
...
@app.route("/{api_version}/aftership/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aftership_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aftership.CognitiveRecord/id": internal_record_id,
        "aftership.CognitiveRecord/apiVersion": api_version,
        "aftership.CognitiveRecord/originalPayload": str(raw_data),
        "aftership.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aftership.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aftership_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "aftership_entity",
