"""
Py Kotodama WASM entrypoint for Mynumber Cognitive Actor.
...
schema = load_schema("../schema/mynumber.kotoba")
db = DatomicClient.connect()
app = Runtime("mynumber-compat")
...
@app.route("/{api_version}/mynumber/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mynumber_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mynumber.CognitiveRecord/id": internal_record_id,
        "mynumber.CognitiveRecord/apiVersion": api_version,
        "mynumber.CognitiveRecord/originalPayload": str(raw_data),
        "mynumber.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mynumber.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mynumber_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mynumber_entity",
