"""
Py Kotodama WASM entrypoint for Itsme_norway Cognitive Actor.
...
schema = load_schema("../schema/itsme_norway.kotoba")
db = DatomicClient.connect()
app = Runtime("itsme_norway-compat")
...
@app.route("/{api_version}/itsme_norway/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_itsme_norway_" + uuid.uuid4().hex[:12]
    db.transact([{
        "itsme_norway.CognitiveRecord/id": internal_record_id,
        "itsme_norway.CognitiveRecord/apiVersion": api_version,
        "itsme_norway.CognitiveRecord/originalPayload": str(raw_data),
        "itsme_norway.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "itsme_norway.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_itsme_norway_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "itsme_norway_entity",
