...
describe("Oil Coverage Actor Manifest", () => {
  ...
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:oil-coverage.gftd.ai"); });
  ...
  it("xrpc covers get, listTargets, listBackbone", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.oil.coverage.get");
    expect(nsids).toContain("ai.gftd.apps.oil.coverage.listTargets");
    expect(nsids).toContain("ai.gftd.apps.oil.coverage.listBackbone");
  });
