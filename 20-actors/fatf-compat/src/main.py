"""
Py Kotodama WASM entrypoint for Fatf Cognitive Actor.
...
schema = load_schema("../schema/fatf.kotoba")
db = DatomicClient.connect()
app = Runtime("fatf-compat")
...
@app.route("/{api_version}/fatf/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fatf_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fatf.CognitiveRecord/id": internal_record_id,
        "fatf.CognitiveRecord/apiVersion": api_version,
        "fatf.CognitiveRecord/originalPayload": str(raw_data),
        "fatf.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fatf.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fatf_" + uuid.uuid4().hex[:8])
