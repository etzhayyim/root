"""
Py Kotodama WASM entrypoint for Nasa_dsn Cognitive Actor.
...
schema = load_schema("../schema/nasa_dsn.kotoba")
db = DatomicClient.connect()
app = Runtime("nasa_dsn-compat")
...
@app.route("/{api_version}/nasa_dsn/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nasa_dsn_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nasa_dsn.CognitiveRecord/id": internal_record_id,
        "nasa_dsn.CognitiveRecord/apiVersion": api_version,
        "nasa_dsn.CognitiveRecord/originalPayload": str(raw_data),
        "nasa_dsn.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nasa_dsn.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nasa_dsn_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nasa_dsn_entity",
