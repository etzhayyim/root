"""
Py Kotodama WASM entrypoint for General_fusion Cognitive Actor.
...
schema = load_schema("../schema/general_fusion.kotoba")
db = DatomicClient.connect()
app = Runtime("general_fusion-compat")
...
@app.route("/{api_version}/general_fusion/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_general_fusion_" + uuid.uuid4().hex[:12]
    db.transact([{
        "general_fusion.CognitiveRecord/id": internal_record_id,
        "general_fusion.CognitiveRecord/apiVersion": api_version,
        "general_fusion.CognitiveRecord/originalPayload": str(raw_data),
        "general_fusion.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "general_fusion.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_general_fusion_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "general_fusion_entity",
