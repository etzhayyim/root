"""
Py Kotodama WASM entrypoint for Allscripts Cognitive Actor.
...
schema = load_schema("../schema/allscripts.kotoba")
db = DatomicClient.connect()
app = Runtime("allscripts-compat")
...
@app.route("/{api_version}/allscripts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_allscripts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "allscripts.CognitiveRecord/id": internal_record_id,
        "allscripts.CognitiveRecord/apiVersion": api_version,
        "allscripts.CognitiveRecord/originalPayload": str(raw_data),
        "allscripts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "allscripts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_allscripts_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "allscripts_entity",
