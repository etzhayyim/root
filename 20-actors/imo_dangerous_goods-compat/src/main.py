"""
Py Kotodama WASM entrypoint for Imo_dangerous_goods Cognitive Actor.
...
schema = load_schema("../schema/imo_dangerous_goods.kotoba")
db = DatomicClient.connect()
app = Runtime("imo_dangerous_goods-compat")
...
@app.route("/{api_version}/imo_dangerous_goods/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_imo_dangerous_goods_" + uuid.uuid4().hex[:12]
    db.transact([{
        "imo_dangerous_goods.CognitiveRecord/id": internal_record_id,
        "imo_dangerous_goods.CognitiveRecord/apiVersion": api_version,
        "imo_dangerous_goods.CognitiveRecord/originalPayload": str(raw_data),
        "imo_dangerous_goods.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "imo_dangerous_goods.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_imo_dangerous_goods_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "imo_dangerous_goods_entity",
