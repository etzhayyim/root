...
//     Data also written to s3://gftd-intel/geoip/{ip}-{YYYYMMDD}.json
...
//     Results → POST /xrpc/ai.gftd.apps.ipaddress.collectRirDelegations
//
...
//   Results → POST /xrpc/ai.gftd.apps.ipaddress.ingestScanResult
...
function tableForCollection(collection: string): string {
  switch (collection) {
    case "ai.gftd.apps.ipaddress.ipAddress":
      return "vertex_ip_address";
    case "ai.gftd.apps.ipaddress.ipRange":
      return "vertex_ipaddress_range";
    case "ai.gftd.apps.ipaddress.asn":
      return "vertex_ipaddress_asn";
    case "ai.gftd.apps.ipaddress.scanJob":
      return "vertex_ipaddress_scan_job";
    case "ai.gftd.apps.ipaddress.scanResult":
...
function write(sdk: HostSDK, collection: string, rec: Record<string, unknown>): void {
  const nsid = `ai.gftd.apps.ipaddress.${collection}`;
...
async function fetchJson<T>(url: string, init?: RequestInit): Promise<T | null> {
  try {
    const res = await fetch(url, { ...init, headers: { Accept: "application/json", "User-Agent": "gftd-ipaddress/1.0", ...(init?.headers ?? {}) } });
...
// Also caches results to s3://gftd-intel/geoip/{ip}-{YYYYMMDD}.json
...
async function cmdCollectScan(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  // via GET /xrpc/ai.gftd.apps.ipaddress.getScanJobs?status=queued
...
async function cmdGetScanJobs(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  const rows = await listByCollection("ai.gftd.apps.ipaddress.scanJob", (query) => {
    let next = query;
...
async function cmdIngestScanResult(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  const existing = await getFirstByCollection("ai.gftd.apps.ipaddress.ipAddress", (query) =>
...
async function cmdGetScanResults(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  const rows = await listByCollection("ai.gftd.apps.ipaddress.scanResult", (query) => {
    let next = query;
...
async function cmdGetAsn(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  const rows = req.asn
    ? await listByCollection("ai.gftd.apps.ipaddress.asn", (query) =>
...
async function cmdListAsns(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  const limit = Math.min(req.limit ?? 50, 100);
  const offset = req.offset ?? 0;
  const rows = await listByCollection("ai.gftd.apps.ipaddress.asn", (query) => {
    let next = query;
...
async function cmdGetRirStats(sdk: HostSDK, _payload: Uint8Array): Promise<unknown> {
  const [ipRanges, asns, ips, scans] = await Promise.all([
    listByCollection("ai.gftd.apps.ipaddress.ipRange"),
    listByCollection("ai.gftd.apps.ipaddress.asn"),
    listByCollection("ai.gftd.apps.ipaddress.ipAddress"),
    listByCollection("ai.gftd.apps.ipaddress.scanResult"),
  ]);
...
export async function handleComAtprotoSyncSubscribeReposCommit(sdk: HostSDK, commit: ComAtprotoSyncSubscribeReposCommit): Promise<{ ok: boolean }> {
  ...
  if (commit.collection === "ai.gftd.apps.yabai.entity") {
    try {
      const row = await getFirstByCollection("ai.gftd.apps.yabai.entity", (query) =>
  ...
  if (commit.collection === "ai.gftd.apps.malak.actorInfrastructure") {
    try {
      const row = await getFirstByCollection("ai.gftd.apps.malak.actorInfrastructure", (query) =>
...
export default createWorkerExport((sdk) => {
  ...
  sdk.app
    // IP analysis
    .command("ai.gftd.apps.ipaddress.analyzeIp", (_, b) => cmdAnalyzeIp(sdk, b),
      asAgentTool("Full IP analysis: GeoIP + WHOIS RDAP + PTR + ASN"), withCapabilityTags("ip", "analyze"), withOCELEvent("collector.run"))
    .command("ai.gftd.apps.ipaddress.lookupIp", (_, b) => cmdAnalyzeIp(sdk, b),
      asAgentTool("Lookup IP address with enrichment"), withCapabilityTags("ip", "lookup"))
    .command("ai.gftd.apps.ipaddress.reverseDns", (_, b) => cmdReverseDns(sdk, b),
      asAgentTool("Reverse DNS (PTR) lookup for IP"), withCapabilityTags("dns", "ptr"))
    .command("ai.gftd.apps.ipaddress.getIpReputation", (_, b) => cmdGetIpReputation(sdk, b),
      asAgentTool("Get IP reputation from yabai risk engine"), withCapabilityTags("ip", "reputation"))
    // RIR collection
    .command("ai.gftd.apps.ipaddress.collectRirDelegations", (_, b) => cmdCollectRirDelegations(sdk, b),
    ...
    // ASN management
    .command("ai.gftd.apps.ipaddress.getAsn", (_, b) => cmdGetAsn(sdk, b),
      asAgentTool("Get ASN details"), withCapabilityTags("asn", "query"))
    .command("ai.gftd.apps.ipaddress.listAsns", (_, b) => cmdListAsns(sdk, b),
      asAgentTool("List ASNs with optional RIR/country filter"), withCapabilityTags("asn", "query"))
    .command("ai.gftd.apps.ipaddress.seedAsns", (_, b) => cmdSeedAsns(sdk, b),
    ...
    .command("ai.gftd.apps.ipaddress.collectScan", (_, b) => cmdCollectScan(sdk, b),
      asAgentTool("Create scan job for CF Container or Linode (ZMap/Masscan)"), withCapabilityTags("scan", "queue"))
    .command("ai.gftd.apps.ipaddress.getScanJobs", (_, b) => cmdGetScanJobs(sdk, b),
      asAgentTool("List scan jobs (polled by CF Container / Linode)"), withCapabilityTags("scan", "query"))
    .command("ai.gftd.apps.ipaddress.ingestScanResult", (_, b) => cmdIngestScanResult(sdk, b),
      asAgentTool("Ingest port scan result from CF Container or Linode"), withCapabilityTags("scan", "ingest"))
    .command("ai.gftd.apps.ipaddress.getScanResults", (_, b) => cmdGetScanResults(sdk, b),
      asAgentTool("Query stored scan results"), withCapabilityTags("scan", "query"))
    // Stats
    .command("ai.gftd.apps.ipaddress.getRirStats", (_, b) => cmdGetRirStats(sdk, b),
      asAgentTool("Get RIR delegation and coverage statistics"), withCapabilityTags("rir", "stats"))
    .command("ai.gftd.apps.ipaddress.registerEntityProfiles", (_, b) => cmdRegisterEntityProfiles(sdk, b),
