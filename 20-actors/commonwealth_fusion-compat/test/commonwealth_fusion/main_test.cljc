(ns commonwealth_fusion.main-test
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [clojure.string :as str]
            [commonwealth_fusion.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "commonwealth_fusion-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))
