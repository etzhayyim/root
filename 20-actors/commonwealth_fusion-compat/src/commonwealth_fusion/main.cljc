(ns commonwealth_fusion.main
  "Kotodama WASM entrypoint for the commonwealth_fusion clean-room actor (L4) — Clojure port.
  ...
  facts (`commonwealth_fusion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "commonwealth_fusion")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"commonwealth_fusion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "commonwealth_fusion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :commonwealth_fusion-compat/ready)
