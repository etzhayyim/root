(ns coinbaseprime.main
  "Kotodama WASM entrypoint for the Coinbaseprime clean-room actor (L4) — Clojure port.
  ...
  facts (`coinbaseprime.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "coinbaseprime")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"coinbaseprime.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "coinbaseprime-compat" :tier tier :entities entities} 200])
...
(defn start! [] :coinbaseprime-compat/ready)
