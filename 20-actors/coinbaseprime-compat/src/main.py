"""
Py Kotodama WASM entrypoint for Coinbaseprime Cognitive Actor.
...
schema = load_schema("../schema/coinbaseprime.kotoba")
db = DatomicClient.connect()
app = Runtime("coinbaseprime-compat")
...
@app.route("/{api_version}/coinbaseprime/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_coinbaseprime_" + uuid.uuid4().hex[:12]
    db.transact([{
        "coinbaseprime.CognitiveRecord/id": internal_record_id,
        "coinbaseprime.CognitiveRecord/apiVersion": api_version,
        "coinbaseprime.CognitiveRecord/originalPayload": str(raw_data),
        "coinbaseprime.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "coinbaseprime.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_coinbaseprime_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "coinbaseprime_entity",
