"""
Py Kotodama WASM entrypoint for Asme_materials Cognitive Actor.
...
@app.route("/{api_version}/asme_materials/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_asme_materials_" + uuid.uuid4().hex[:12]
    db.transact([{
        "asme_materials.CognitiveRecord/id": internal_record_id,
        "asme_materials.CognitiveRecord/apiVersion": api_version,
        "asme_materials.CognitiveRecord/originalPayload": str(raw_data),
        "asme_materials.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "asme_materials.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_asme_materials_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "asme_materials_entity",
