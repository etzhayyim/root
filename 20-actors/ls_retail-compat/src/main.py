"""
Py Kotodama WASM entrypoint for Ls_retail Cognitive Actor.
...
schema = load_schema("../schema/ls_retail.kotoba")
db = DatomicClient.connect()
app = Runtime("ls_retail-compat")
...
@app.route("/{api_version}/ls_retail/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ls_retail_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ls_retail.CognitiveRecord/id": internal_record_id,
        "ls_retail.CognitiveRecord/apiVersion": api_version,
        "ls_retail.CognitiveRecord/originalPayload": str(raw_data),
        "ls_retail.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ls_retail.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ls_retail_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ls_retail_entity",
