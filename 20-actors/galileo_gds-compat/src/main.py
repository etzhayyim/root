"""
Py Kotodama WASM entrypoint for Galileo_gds Cognitive Actor.
...
schema = load_schema("../schema/galileo_gds.kotoba")
db = DatomicClient.connect()
app = Runtime("galileo_gds-compat")
...
@app.route("/{api_version}/galileo_gds/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_galileo_gds_" + uuid.uuid4().hex[:12]
    db.transact([{
        "galileo_gds.CognitiveRecord/id": internal_record_id,
        "galileo_gds.CognitiveRecord/apiVersion": api_version,
        "galileo_gds.CognitiveRecord/originalPayload": str(raw_data),
        "galileo_gds.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "galileo_gds.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_galileo_gds_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "galileo_gds_entity",
