"""
Py Kotodama WASM entrypoint for Razorpay Cognitive Actor.
...
@app.route("/{api_version}/razorpay/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_razorpay_" + uuid.uuid4().hex[:12]
    db.transact([{
        "razorpay.CognitiveRecord/id": internal_record_id,
        "razorpay.CognitiveRecord/apiVersion": api_version,
        "razorpay.CognitiveRecord/originalPayload": str(raw_data),
        "razorpay.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "razorpay.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_razorpay_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "razorpay_entity",
