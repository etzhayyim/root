"""
Py Kotodama WASM entrypoint for Vonage Cognitive Actor.
...
@app.route("/{api_version}/vonage/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_vonage_" + uuid.uuid4().hex[:12]
    db.transact([{
        "vonage.CognitiveRecord/id": internal_record_id,
        "vonage.CognitiveRecord/apiVersion": api_version,
        "vonage.CognitiveRecord/originalPayload": str(raw_data),
        "vonage.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "vonage.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_vonage_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "vonage_entity",
