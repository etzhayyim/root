"""
Py Kotodama WASM entrypoint for Statestreet_api Cognitive Actor.
...
schema = load_schema("../schema/statestreet_api.kotoba")
db = DatomicClient.connect()
app = Runtime("statestreet_api-compat")
...
@app.route("/{api_version}/statestreet_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_statestreet_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "statestreet_api.CognitiveRecord/id": internal_record_id,
        "statestreet_api.CognitiveRecord/apiVersion": api_version,
        "statestreet_api.CognitiveRecord/originalPayload": str(raw_data),
        "statestreet_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "statestreet_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_statestreet_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "statestreet_api_entity",
