"""
Py Kotodama WASM entrypoint for Shodan Cognitive Actor.
...
schema = load_schema("../schema/shodan.kotoba")
db = DatomicClient.connect()
app = Runtime("shodan-compat")
...
@app.route("/{api_version}/shodan/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_shodan_" + uuid.uuid4().hex[:12]
    db.transact([{
        "shodan.CognitiveRecord/id": internal_record_id,
        "shodan.CognitiveRecord/apiVersion": api_version,
        "shodan.CognitiveRecord/originalPayload": str(raw_data),
        "shodan.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "shodan.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_shodan_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "shodan_entity",
