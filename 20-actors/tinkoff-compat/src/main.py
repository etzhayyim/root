"""
Py Kotodama WASM entrypoint for Tinkoff Cognitive Actor.
...
schema = load_schema("../schema/tinkoff.kotoba")
db = DatomicClient.connect()
app = Runtime("tinkoff-compat")
...
@app.route("/{api_version}/tinkoff/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tinkoff_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tinkoff.CognitiveRecord/id": internal_record_id,
        "tinkoff.CognitiveRecord/apiVersion": api_version,
        "tinkoff.CognitiveRecord/originalPayload": str(raw_data),
        "tinkoff.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tinkoff.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tinkoff_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tinkoff_entity",
