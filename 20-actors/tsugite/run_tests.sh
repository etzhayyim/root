...
set -euo pipefail
cd "$(dirname "$0")/../.."
exec bb -e '(require (quote clojure.test) (quote tsugite.methods.test-datom-emit) (quote tsugite.tests.test-analyze) (quote tsugite.tests.test-coverage))(let [r (apply clojure.test/run-tests (quote [tsugite.methods.test-datom-emit tsugite.tests.test-analyze tsugite.tests.test-coverage]))](System/exit (if (zero? (+ (:fail r) (:error r))) 0 1)))'
