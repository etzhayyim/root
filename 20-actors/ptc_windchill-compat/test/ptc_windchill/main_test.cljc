(ns ptc_windchill.main-test
  "Contract + behavioral test for the ptc_windchill-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ptc_windchill-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ptc_windchill.main-test)]
