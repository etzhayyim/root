(ns ptc_windchill.main
  "Kotodama WASM entrypoint for the ptc_windchill clean-room actor (L4) — Clojure port.
  ...
  facts (`ptc_windchill.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ptc_windchill")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ptc_windchill.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ptc_windchill-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ptc_windchill-compat/ready)
