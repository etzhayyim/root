"""
Py Kotodama WASM entrypoint for Cerillion Cognitive Actor.
...
schema = load_schema("../schema/cerillion.kotoba")
db = DatomicClient.connect()
app = Runtime("cerillion-compat")
...
@app.route("/{api_version}/cerillion/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cerillion_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cerillion.CognitiveRecord/id": internal_record_id,
        "cerillion.CognitiveRecord/apiVersion": api_version,
        "cerillion.CognitiveRecord/originalPayload": str(raw_data),
        "cerillion.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cerillion.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cerillion_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "cerillion_entity",
