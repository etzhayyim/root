"""
Py Kotodama WASM entrypoint for Helion_energy Cognitive Actor.
...
schema = load_schema("../schema/helion_energy.kotoba")
db = DatomicClient.connect()
app = Runtime("helion_energy-compat")
...
@app.route("/{api_version}/helion_energy/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_helion_energy_" + uuid.uuid4().hex[:12]
    db.transact([{
        "helion_energy.CognitiveRecord/id": internal_record_id,
        "helion_energy.CognitiveRecord/apiVersion": api_version,
        "helion_energy.CognitiveRecord/originalPayload": str(raw_data),
        "helion_energy.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "helion_energy.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_helion_energy_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "helion_energy_entity",
