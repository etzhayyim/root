"""
Py Kotodama WASM entrypoint for Aws Cognitive Actor.
...
@app.route("/{api_version}/aws/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_aws_" + uuid.uuid4().hex[:12]
    db.transact([{
        "aws.CognitiveRecord/id": internal_record_id,
        "aws.CognitiveRecord/apiVersion": api_version,
        "aws.CognitiveRecord/originalPayload": str(raw_data),
        "aws.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "aws.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_aws_" + uuid.uuid4().hex[:8])
