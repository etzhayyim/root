#!/usr/bin/env bb
;; Clojure port of py/test_agent.py (wadachi autonomous-mobility agent gate tests).
(ns wadachi.py.test-agent
  ...
  Run: bb --classpath 20-actors 20-actors/wadachi/py/test_agent.clj"
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [wadachi.py.agent :as agent]))
...
(when (= *file* (System/getProperty "babashka.file"))
  (let [{:keys [fail error]} (clojure.test/run-tests 'wadachi.py.test-agent)]
