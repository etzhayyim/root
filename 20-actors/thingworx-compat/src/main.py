"""
Py Kotodama WASM entrypoint for Thingworx Cognitive Actor.
...
schema = load_schema("../schema/thingworx.kotoba")
db = DatomicClient.connect()
app = Runtime("thingworx-compat")
...
@app.route("/{api_version}/thingworx/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_thingworx_" + uuid.uuid4().hex[:12]
    db.transact([{
        "thingworx.CognitiveRecord/id": internal_record_id,
        "thingworx.CognitiveRecord/apiVersion": api_version,
        "thingworx.CognitiveRecord/originalPayload": str(raw_data),
        "thingworx.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "thingworx.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_thingworx_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "thingworx_entity",
