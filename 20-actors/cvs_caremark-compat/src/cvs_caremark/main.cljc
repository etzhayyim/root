(ns cvs_caremark.main
  "Kotodama WASM entrypoint for the CVS Caremark clean-room actor (L4) — Clojure port.
  ...
  facts (`cvs_caremark.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cvs_caremark")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "cvscarem_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "cvscarem_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "cvscarem_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "cvscarem_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "cvscarem_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "cvscarem_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"cvs_caremark.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cvs_caremark-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cvs_caremark-compat/ready)
