"""
Py Kotodama WASM entrypoint for Etax Cognitive Actor.
...
schema = load_schema("../schema/etax.kotoba")
db = DatomicClient.connect()
app = Runtime("etax-compat")
...
@app.route("/{api_version}/etax/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_etax_" + uuid.uuid4().hex[:12]
    db.transact([{
        "etax.CognitiveRecord/id": internal_record_id,
        "etax.CognitiveRecord/apiVersion": api_version,
        "etax.CognitiveRecord/originalPayload": str(raw_data),
        "etax.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "etax.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_etax_" + uuid.uuid4().hex[:8])
