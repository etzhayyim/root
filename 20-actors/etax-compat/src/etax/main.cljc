(ns etax.main
  "Kotodama WASM entrypoint for the Etax clean-room actor (L4) — Clojure port.
  ...
  facts (`etax.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"etax.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "etax-compat" :tier tier :entities entities} 200])
