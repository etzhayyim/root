"""
Py Kotodama WASM entrypoint for Rapyd Cognitive Actor.
...
schema = load_schema("../schema/rapyd.kotoba")
db = DatomicClient.connect()
app = Runtime("rapyd-compat")
...
@app.route("/{api_version}/rapyd/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rapyd_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rapyd.CognitiveRecord/id": internal_record_id,
        "rapyd.CognitiveRecord/apiVersion": api_version,
        "rapyd.CognitiveRecord/originalPayload": str(raw_data),
        "rapyd.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rapyd.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rapyd_" + uuid.uuid4().hex[:8])
