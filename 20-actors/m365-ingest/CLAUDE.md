...
Microsoft Graph API → `ai.gftd.apps.kyber.inbox.emailSignal` pipeline. **No UI, no OAuth.** Pure tenant-app ingest using client credentials flow (`Application.Mail.Read`).
...
| **Host capability** | `ai.gftd.host.m365.*` (4 methods — acquireAppToken, enumerateUsers, fetchMailFolders, fetchMessagesPage) |
| **Output collection** | `ai.gftd.apps.kyber.inbox.emailSignal` (derives `vertex_email_message`) |
...
Refresh `vertex_m365_user` from Graph `/users?$filter=endsWith(upn,'@gftd.co.jp')`.
...
### 3. `sync-single-user` (xrpc `ai.gftd.apps.m365Ingest.syncUser`)
...
- Emit `createRecord(ai.gftd.apps.kyber.inbox.emailSignal, ...)`
- Update watermark at end

## Lexicons

### Host (4)
- `ai.gftd.host.m365.acquireAppToken` — client credentials → Bearer JWT
- `ai.gftd.host.m365.enumerateUsers` — `/users` with filter
- `ai.gftd.host.m365.fetchMailFolders` — recursive folder tree
- `ai.gftd.host.m365.fetchMessagesPage` — one page of `/users/{upn}/messages`

### App (3 XRPC)
- `ai.gftd.apps.m365Ingest.syncUser` (procedure) — on-demand single-user ingest
- `ai.gftd.apps.m365Ingest.syncAllUsers` (procedure) — manual fan-out trigger
- `ai.gftd.apps.m365Ingest.syncStatus` (query) — read vertex_m365_sync_state
...
Reading employee mailboxes = high PII. Required consent records (`ai.gftd.apps.consent.grant`):
...
Audit trail: every ingest run writes `ai.gftd.apps.m365Ingest.run` event → ocel log.
...
```bash
gftd xrpc ai.gftd.actor.migrate -d '{"manifestPath":"20-actors/m365-ingest/actor-manifest.jsonld"}'
```
...
- Initial ingest per UPN via `~/.gftd/ingest/m365_mail_ingest.py`:
  - `legal@gftd.co.jp`: skipped (email not reachable via Graph)
  - `jinji@gftd.co.jp`: ingested, Amazon Business email classified IntelExtraction score=28/clean
  - `keiri@gftd.co.jp`: ingested, **BEC campaign detected** — 4 outlook.com actors sent CEO name (河崎純真) + company name spoofing, all registered to yabai (risk 75–80/monitor)
...
→ Step 4: Register manifest → `gftd xrpc ai.gftd.actor.migrate`: PENDING
