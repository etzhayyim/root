"""
Py Kotodama WASM entrypoint for Planet_api Cognitive Actor.
...
schema = load_schema("../schema/planet_api.kotoba")
db = DatomicClient.connect()
app = Runtime("planet_api-compat")
...
@app.route("/{api_version}/planet_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_planet_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "planet_api.CognitiveRecord/id": internal_record_id,
        "planet_api.CognitiveRecord/apiVersion": api_version,
        "planet_api.CognitiveRecord/originalPayload": str(raw_data),
        "planet_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "planet_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_planet_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "planet_api_entity",
