(ns dhs.main
  "Kotodama WASM entrypoint for the dhs clean-room actor (L4) — Clojure port.
  ...
  facts (`dhs.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"dhs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dhs-compat" :tier tier :entities entities} 200])
