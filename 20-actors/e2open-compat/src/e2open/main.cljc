(ns e2open.main
  "Kotodama WASM entrypoint for the e2open clean-room actor (L4) — Clojure port.
  ...
  facts (`e2open.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "e2open")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"e2open.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "e2open-compat" :tier tier :entities entities} 200])
...
(defn start! [] :e2open-compat/ready)
