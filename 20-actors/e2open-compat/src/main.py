"""
Py Kotodama WASM entrypoint for E2open Cognitive Actor.
...
schema = load_schema("../schema/e2open.kotoba")
db = DatomicClient.connect()
app = Runtime("e2open-compat")
...
@app.route("/{api_version}/e2open/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_e2open_" + uuid.uuid4().hex[:12]
    db.transact([{
        "e2open.CognitiveRecord/id": internal_record_id,
        "e2open.CognitiveRecord/apiVersion": api_version,
        "e2open.CognitiveRecord/originalPayload": str(raw_data),
        "e2open.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "e2open.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_e2open_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "e2open_entity",
