"""
Py Kotodama WASM entrypoint for Sendbird Cognitive Actor.
...
@app.route("/{api_version}/sendbird/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sendbird_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sendbird.CognitiveRecord/id": internal_record_id,
        "sendbird.CognitiveRecord/apiVersion": api_version,
        "sendbird.CognitiveRecord/originalPayload": str(raw_data),
        "sendbird.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sendbird.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sendbird_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "sendbird_entity",
