"""
Py Kotodama WASM entrypoint for Cowin Cognitive Actor.
...
schema = load_schema("../schema/cowin.kotoba")
db = DatomicClient.connect()
app = Runtime("cowin-compat")
...
@app.route("/{api_version}/cowin/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_cowin_" + uuid.uuid4().hex[:12]
    db.transact([{
        "cowin.CognitiveRecord/id": internal_record_id,
        "cowin.CognitiveRecord/apiVersion": api_version,
        "cowin.CognitiveRecord/originalPayload": str(raw_data),
        "cowin.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "cowin.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_cowin_" + uuid.uuid4().hex[:8])
