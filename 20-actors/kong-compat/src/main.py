"""
Py Kotodama WASM entrypoint for Kong Cognitive Actor.
...
@app.route("/{api_version}/kong/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_kong_" + uuid.uuid4().hex[:12]
    db.transact([{
        "kong.CognitiveRecord/id": internal_record_id,
        "kong.CognitiveRecord/apiVersion": api_version,
        "kong.CognitiveRecord/originalPayload": str(raw_data),
        "kong.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "kong.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_kong_" + uuid.uuid4().hex[:8])
