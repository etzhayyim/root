"""
Py Kotodama WASM entrypoint for Hapag_lloyd_api Cognitive Actor.
...
schema = load_schema("../schema/hapag_lloyd_api.kotoba")
db = DatomicClient.connect()
app = Runtime("hapag_lloyd_api-compat")
...
@app.route("/{api_version}/hapag_lloyd_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hapag_lloyd_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hapag_lloyd_api.CognitiveRecord/id": internal_record_id,
        "hapag_lloyd_api.CognitiveRecord/apiVersion": api_version,
        "hapag_lloyd_api.CognitiveRecord/originalPayload": str(raw_data),
        "hapag_lloyd_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hapag_lloyd_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hapag_lloyd_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hapag_lloyd_api_entity",
