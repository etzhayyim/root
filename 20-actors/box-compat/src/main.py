"""
Py Kotodama WASM entrypoint for Box Cognitive Actor.
...
@app.route("/{api_version}/box/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_box_" + uuid.uuid4().hex[:12]
    db.transact([{
        "box.CognitiveRecord/id": internal_record_id,
        "box.CognitiveRecord/apiVersion": api_version,
        "box.CognitiveRecord/originalPayload": str(raw_data),
        "box.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "box.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_box_" + uuid.uuid4().hex[:8])
