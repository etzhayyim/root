"""
Py Kotodama WASM entrypoint for Matillion Cognitive Actor.
...
@app.route("/{api_version}/matillion/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_matillion_" + uuid.uuid4().hex[:12]
    db.transact([{
        "matillion.CognitiveRecord/id": internal_record_id,
        "matillion.CognitiveRecord/apiVersion": api_version,
        "matillion.CognitiveRecord/originalPayload": str(raw_data),
        "matillion.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "matillion.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_matillion_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "matillion_entity",
