"""
Py Kotodama WASM entrypoint for Icertis Cognitive Actor.
...
schema = load_schema("../schema/icertis.kotoba")
db = DatomicClient.connect()
app = Runtime("icertis-compat")
...
@app.route("/{api_version}/icertis/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_icertis_" + uuid.uuid4().hex[:12]
    db.transact([{
        "icertis.CognitiveRecord/id": internal_record_id,
        "icertis.CognitiveRecord/apiVersion": api_version,
        "icertis.CognitiveRecord/originalPayload": str(raw_data),
        "icertis.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "icertis.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_icertis_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "icertis_entity",
