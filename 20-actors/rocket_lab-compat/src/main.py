"""
Py Kotodama WASM entrypoint for Rocket_lab Cognitive Actor.
...
schema = load_schema("../schema/rocket_lab.kotoba")
db = DatomicClient.connect()
app = Runtime("rocket_lab-compat")
...
@app.route("/{api_version}/rocket_lab/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rocket_lab_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rocket_lab.CognitiveRecord/id": internal_record_id,
        "rocket_lab.CognitiveRecord/apiVersion": api_version,
        "rocket_lab.CognitiveRecord/originalPayload": str(raw_data),
        "rocket_lab.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rocket_lab.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rocket_lab_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rocket_lab_entity",
