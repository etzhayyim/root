(ns hannover-re.main
  "Kotodama WASM entrypoint for the Hannover_re clean-room actor (L4) — Clojure port.
  ...
  facts (`hannover_re.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hannover_re")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "hannover_acc"
    :fields [:holderId :type :balance :currency :status]     :required [:type :balance]
    :coerce {:balance :float}         :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "hannover_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"hannover_re.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hannover_re-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hannover_re-compat/ready)
