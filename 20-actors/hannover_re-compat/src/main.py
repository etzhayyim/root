"""
Py Kotodama WASM entrypoint for Hannover_re Cognitive Actor.
...
schema = load_schema("../schema/hannover_re.kotoba")
db = DatomicClient.connect()
app = Runtime("hannover_re-compat")
...
@app.route("/{api_version}/hannover_re/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hannover_re_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hannover_re.CognitiveRecord/id": internal_record_id,
        "hannover_re.CognitiveRecord/apiVersion": api_version,
        "hannover_re.CognitiveRecord/originalPayload": str(raw_data),
        "hannover_re.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hannover_re.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hannover_re_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hannover_re_entity",
