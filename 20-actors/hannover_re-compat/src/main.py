"""
Py Kotodama WASM entrypoint for the Hannover_re clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"hannover_re.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"hannover_re.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Account", "id": eid})
...
def delete_loan(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Loan", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_policy(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Policy", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_claim(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Claim", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_transaction(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Transaction", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_customer(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"hannover_re.Customer", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "hannover_re-compat", "entities": ['Account', 'Loan', 'Policy', 'Claim', 'Transaction', 'Customer']}, 200
