"""
Py Kotodama WASM entrypoint for Morningstar Cognitive Actor.
...
schema = load_schema("../schema/morningstar.kotoba")
db = DatomicClient.connect()
app = Runtime("morningstar-compat")
...
@app.route("/{api_version}/morningstar/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_morningstar_" + uuid.uuid4().hex[:12]
    db.transact([{
        "morningstar.CognitiveRecord/id": internal_record_id,
        "morningstar.CognitiveRecord/apiVersion": api_version,
        "morningstar.CognitiveRecord/originalPayload": str(raw_data),
        "morningstar.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "morningstar.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_morningstar_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "morningstar_entity",
