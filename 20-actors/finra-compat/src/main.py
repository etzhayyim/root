"""
Py Kotodama WASM entrypoint for Finra Cognitive Actor.
...
schema = load_schema("../schema/finra.kotoba")
db = DatomicClient.connect()
app = Runtime("finra-compat")
...
@app.route("/{api_version}/finra/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_finra_" + uuid.uuid4().hex[:12]
    db.transact([{
        "finra.CognitiveRecord/id": internal_record_id,
        "finra.CognitiveRecord/apiVersion": api_version,
        "finra.CognitiveRecord/originalPayload": str(raw_data),
        "finra.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "finra.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_finra_" + uuid.uuid4().hex[:8])
