"""
Py Kotodama WASM entrypoint for the Zendesksell clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"zendesksell.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"zendesksell.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_ticket(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Ticket", "id": eid})
...
def delete_survey(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Survey", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_response(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Response", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Account", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_conversation(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Conversation", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_contact(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"zendesksell.Contact", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "zendesksell-compat", "entities": ['Ticket', 'Survey', 'Response', 'Account', 'Conversation', 'Contact']}, 200
