"""
Py Kotodama WASM entrypoint for Zendesksell Cognitive Actor.
...
schema = load_schema("../schema/zendesksell.kotoba")
db = DatomicClient.connect()
app = Runtime("zendesksell-compat")
...
@app.route("/{api_version}/zendesksell/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zendesksell_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zendesksell.CognitiveRecord/id": internal_record_id,
        "zendesksell.CognitiveRecord/apiVersion": api_version,
        "zendesksell.CognitiveRecord/originalPayload": str(raw_data),
        "zendesksell.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zendesksell.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zendesksell_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zendesksell_entity",
