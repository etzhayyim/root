#!/usr/bin/env python3
"""mitsuho 瑞穂 — ingest seed.edn into a live kotoba node via MCP.

ADR-2605261015, migration plan Phase 3. Flattens each seed entity map into
...
operator AT-session JWT (no-server-key posture, G4). --dry-run parses + counts
datoms only (no writes, no LLM; G5 untouched).

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.mitsuho] [--dry-run]
"""
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="com.etzhayyim.mitsuho")
    ...
    # provisioned (G5 gates real outward writes). Kept explicit to avoid silent no-op.
    print("   live ingest requested — implement MCP kotoba_datom_create wiring before use (G5).")
    return 0
