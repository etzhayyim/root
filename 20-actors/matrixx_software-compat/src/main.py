"""
Py Kotodama WASM entrypoint for Matrixx_software Cognitive Actor.
...
schema = load_schema("../schema/matrixx_software.kotoba")
db = DatomicClient.connect()
app = Runtime("matrixx_software-compat")
...
@app.route("/{api_version}/matrixx_software/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_matrixx_software_" + uuid.uuid4().hex[:12]
    db.transact([{
        "matrixx_software.CognitiveRecord/id": internal_record_id,
        "matrixx_software.CognitiveRecord/apiVersion": api_version,
        "matrixx_software.CognitiveRecord/originalPayload": str(raw_data),
        "matrixx_software.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "matrixx_software.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_matrixx_software_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "matrixx_software_entity",
