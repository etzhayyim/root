"""
Py Kotodama WASM entrypoint for the Matrixx_software clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"matrixx_software.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"matrixx_software.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_subscriber(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.Subscriber", "id": eid})
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_number(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.Number", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_terminal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.Terminal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_usage_record(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"matrixx_software.UsageRecord", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "matrixx_software-compat", "entities": ['Subscriber', 'Session', 'Message', 'Number', 'Terminal', 'UsageRecord']}, 200
