"""
Py Kotodama WASM entrypoint for the Meditech clean-room actor (L4).
...
def delete_medication_request(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"meditech.MedicationRequest", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_allergy_intolerance(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"meditech.AllergyIntolerance", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_condition(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"meditech.Condition", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "meditech-compat", "tier": "L4",
