"""
Py Kotodama WASM entrypoint for Ncr_retalix Cognitive Actor.
...
schema = load_schema("../schema/ncr_retalix.kotoba")
db = DatomicClient.connect()
app = Runtime("ncr_retalix-compat")
...
@app.route("/{api_version}/ncr_retalix/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ncr_retalix_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ncr_retalix.CognitiveRecord/id": internal_record_id,
        "ncr_retalix.CognitiveRecord/apiVersion": api_version,
        "ncr_retalix.CognitiveRecord/originalPayload": str(raw_data),
        "ncr_retalix.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ncr_retalix.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ncr_retalix_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ncr_retalix_entity",
