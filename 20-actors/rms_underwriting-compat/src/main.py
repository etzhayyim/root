"""
Py Kotodama WASM entrypoint for Rms_underwriting Cognitive Actor.
...
schema = load_schema("../schema/rms_underwriting.kotoba")
db = DatomicClient.connect()
app = Runtime("rms_underwriting-compat")
...
@app.route("/{api_version}/rms_underwriting/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_rms_underwriting_" + uuid.uuid4().hex[:12]
    db.transact([{
        "rms_underwriting.CognitiveRecord/id": internal_record_id,
        "rms_underwriting.CognitiveRecord/apiVersion": api_version,
        "rms_underwriting.CognitiveRecord/originalPayload": str(raw_data),
        "rms_underwriting.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "rms_underwriting.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_rms_underwriting_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "rms_underwriting_entity",
