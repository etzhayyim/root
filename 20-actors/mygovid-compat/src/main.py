"""
Py Kotodama WASM entrypoint for Mygovid Cognitive Actor.
...
@app.route("/{api_version}/mygovid/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mygovid_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mygovid.CognitiveRecord/id": internal_record_id,
        "mygovid.CognitiveRecord/apiVersion": api_version,
        "mygovid.CognitiveRecord/originalPayload": str(raw_data),
        "mygovid.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mygovid.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mygovid_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "mygovid_entity",
