"""
Py Kotodama WASM entrypoint for Spatial_os Cognitive Actor.
...
@app.route("/{api_version}/spatial_os/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_spatial_os_" + uuid.uuid4().hex[:12]
    db.transact([{
        "spatial_os.CognitiveRecord/id": internal_record_id,
        "spatial_os.CognitiveRecord/apiVersion": api_version,
        "spatial_os.CognitiveRecord/originalPayload": str(raw_data),
        "spatial_os.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "spatial_os.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_spatial_os_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "spatial_os_entity",
