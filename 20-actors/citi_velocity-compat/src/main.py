"""
Py Kotodama WASM entrypoint for Citi_velocity Cognitive Actor.
...
schema = load_schema("../schema/citi_velocity.kotoba")
db = DatomicClient.connect()
app = Runtime("citi_velocity-compat")
...
@app.route("/{api_version}/citi_velocity/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_citi_velocity_" + uuid.uuid4().hex[:12]
    db.transact([{
        "citi_velocity.CognitiveRecord/id": internal_record_id,
        "citi_velocity.CognitiveRecord/apiVersion": api_version,
        "citi_velocity.CognitiveRecord/originalPayload": str(raw_data),
        "citi_velocity.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "citi_velocity.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_citi_velocity_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "citi_velocity_entity",
