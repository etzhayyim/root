"""
Py Kotodama WASM entrypoint for the Remote clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"remote.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_employee(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.Employee", "id": eid})
...
def delete_candidate(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.Candidate", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_job_req(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.JobReq", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payroll_run(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.PayrollRun", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_time_off(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.TimeOff", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_review(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"remote.Review", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "remote-compat", "entities": ['Employee', 'Candidate', 'JobReq', 'PayrollRun', 'TimeOff', 'Review']}, 200
