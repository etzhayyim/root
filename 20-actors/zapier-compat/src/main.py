"""
Py Kotodama WASM entrypoint for Zapier Cognitive Actor.
...
schema = load_schema("../schema/zapier.kotoba")
db = DatomicClient.connect()
app = Runtime("zapier-compat")
...
@app.route("/{api_version}/zapier/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zapier_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zapier.CognitiveRecord/id": internal_record_id,
        "zapier.CognitiveRecord/apiVersion": api_version,
        "zapier.CognitiveRecord/originalPayload": str(raw_data),
        "zapier.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zapier.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zapier_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zapier_entity",
