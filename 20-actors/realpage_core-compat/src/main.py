"""
Py Kotodama WASM entrypoint for Realpage_core Cognitive Actor.
...
schema = load_schema("../schema/realpage_core.kotoba")
db = DatomicClient.connect()
app = Runtime("realpage_core-compat")
...
@app.route("/{api_version}/realpage_core/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_realpage_core_" + uuid.uuid4().hex[:12]
    db.transact([{
        "realpage_core.CognitiveRecord/id": internal_record_id,
        "realpage_core.CognitiveRecord/apiVersion": api_version,
        "realpage_core.CognitiveRecord/originalPayload": str(raw_data),
        "realpage_core.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "realpage_core.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_realpage_core_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "realpage_core_entity",
