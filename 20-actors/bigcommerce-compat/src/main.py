"""
Py Kotodama WASM entrypoint for Bigcommerce Cognitive Actor.
...
@app.route("/{api_version}/bigcommerce/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_bigcommerce_" + uuid.uuid4().hex[:12]
    db.transact([{
        "bigcommerce.CognitiveRecord/id": internal_record_id,
        "bigcommerce.CognitiveRecord/apiVersion": api_version,
        "bigcommerce.CognitiveRecord/originalPayload": str(raw_data),
        "bigcommerce.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "bigcommerce.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_bigcommerce_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "bigcommerce_entity",
