"""
Py Kotodama WASM entrypoint for Dott Cognitive Actor.
...
schema = load_schema("../schema/dott.kotoba")
db = DatomicClient.connect()
app = Runtime("dott-compat")
...
@app.route("/{api_version}/dott/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_dott_" + uuid.uuid4().hex[:12]
    db.transact([{
        "dott.CognitiveRecord/id": internal_record_id,
        "dott.CognitiveRecord/apiVersion": api_version,
        "dott.CognitiveRecord/originalPayload": str(raw_data),
        "dott.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "dott.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_dott_" + uuid.uuid4().hex[:8])
