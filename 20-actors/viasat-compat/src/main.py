"""
Py Kotodama WASM entrypoint for the Viasat clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"viasat.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_subscriber(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.Subscriber", "id": eid})
...
def delete_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.Session", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_message(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.Message", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_number(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.Number", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_terminal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.Terminal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_usage_record(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"viasat.UsageRecord", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "viasat-compat", "entities": ['Subscriber', 'Session', 'Message', 'Number', 'Terminal', 'UsageRecord']}, 200
