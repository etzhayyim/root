"""
Py Kotodama WASM entrypoint for Viasat Cognitive Actor.
...
schema = load_schema("../schema/viasat.kotoba")
db = DatomicClient.connect()
app = Runtime("viasat-compat")
...
@app.route("/{api_version}/viasat/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_viasat_" + uuid.uuid4().hex[:12]
    db.transact([{
        "viasat.CognitiveRecord/id": internal_record_id,
        "viasat.CognitiveRecord/apiVersion": api_version,
        "viasat.CognitiveRecord/originalPayload": str(raw_data),
        "viasat.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "viasat.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_viasat_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "viasat_entity",
