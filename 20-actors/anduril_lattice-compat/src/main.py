"""
Py Kotodama WASM entrypoint for Anduril_lattice Cognitive Actor.
...
schema = load_schema("../schema/anduril_lattice.kotoba")
db = DatomicClient.connect()
app = Runtime("anduril_lattice-compat")
...
@app.route("/{api_version}/anduril_lattice/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_anduril_lattice_" + uuid.uuid4().hex[:12]
    db.transact([{
        "anduril_lattice.CognitiveRecord/id": internal_record_id,
        "anduril_lattice.CognitiveRecord/apiVersion": api_version,
        "anduril_lattice.CognitiveRecord/originalPayload": str(raw_data),
        "anduril_lattice.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "anduril_lattice.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_anduril_lattice_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "anduril_lattice_entity",
