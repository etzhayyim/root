"""
Py Kotodama WASM entrypoint for Zap_energy Cognitive Actor.
...
schema = load_schema("../schema/zap_energy.kotoba")
db = DatomicClient.connect()
app = Runtime("zap_energy-compat")
...
@app.route("/{api_version}/zap_energy/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zap_energy_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zap_energy.CognitiveRecord/id": internal_record_id,
        "zap_energy.CognitiveRecord/apiVersion": api_version,
        "zap_energy.CognitiveRecord/originalPayload": str(raw_data),
        "zap_energy.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zap_energy.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zap_energy_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zap_energy_entity",
