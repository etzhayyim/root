"""
Py Kotodama WASM entrypoint for Gsma_opengateway Cognitive Actor.
...
schema = load_schema("../schema/gsma_opengateway.kotoba")
db = DatomicClient.connect()
app = Runtime("gsma_opengateway-compat")
...
@app.route("/{api_version}/gsma_opengateway/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gsma_opengateway_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gsma_opengateway.CognitiveRecord/id": internal_record_id,
        "gsma_opengateway.CognitiveRecord/apiVersion": api_version,
        "gsma_opengateway.CognitiveRecord/originalPayload": str(raw_data),
        "gsma_opengateway.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gsma_opengateway.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gsma_opengateway_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gsma_opengateway_entity",
