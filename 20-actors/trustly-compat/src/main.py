"""
Py Kotodama WASM entrypoint for Trustly Cognitive Actor.
...
schema = load_schema("../schema/trustly.kotoba")
db = DatomicClient.connect()
app = Runtime("trustly-compat")
...
@app.route("/{api_version}/trustly/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_trustly_" + uuid.uuid4().hex[:12]
    db.transact([{
        "trustly.CognitiveRecord/id": internal_record_id,
        "trustly.CognitiveRecord/apiVersion": api_version,
        "trustly.CognitiveRecord/originalPayload": str(raw_data),
        "trustly.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "trustly.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_trustly_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "trustly_entity",
