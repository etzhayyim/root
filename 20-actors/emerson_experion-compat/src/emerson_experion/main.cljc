(ns emerson_experion.main
  "Kotodama WASM entrypoint for the Emerson_experion clean-room actor (L4) — Clojure port.
  ...
  facts (`emerson_experion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "emerson_experion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "emersone_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "emersone_con"
    :fields [:name :setpoint :processValue :mode]            :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}          :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "emersone_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"emerson_experion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "emerson_experion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :emerson_experion-compat/ready)
