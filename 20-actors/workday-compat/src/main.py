"""
Py Kotodama WASM entrypoint for Workday Cognitive Actor.
...
schema = load_schema("../schema/workday.kotoba")
db = DatomicClient.connect()
app = Runtime("workday-compat")
...
@app.route("/{api_version}/workday/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_workday_" + uuid.uuid4().hex[:12]
    db.transact([{
        "workday.CognitiveRecord/id": internal_record_id,
        "workday.CognitiveRecord/apiVersion": api_version,
        "workday.CognitiveRecord/originalPayload": str(raw_data),
        "workday.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "workday.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_workday_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "workday_entity",
