"""
Py Kotodama WASM entrypoint for the Workday clean-room actor.
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"workday.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_employee(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.Employee", "id": eid})
...
def delete_candidate(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.Candidate", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_job_req(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.JobReq", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_payroll_run(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.PayrollRun", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_time_off(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.TimeOff", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_review(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"workday.Review", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "workday-compat", "entities": ['Employee', 'Candidate', 'JobReq', 'PayrollRun', 'TimeOff', 'Review']}, 200
