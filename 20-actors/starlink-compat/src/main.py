"""
Py Kotodama WASM entrypoint for Starlink Cognitive Actor.
...
schema = load_schema("../schema/starlink.kotoba")
db = DatomicClient.connect()
app = Runtime("starlink-compat")
...
@app.route("/{api_version}/starlink/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_starlink_" + uuid.uuid4().hex[:12]
    db.transact([{
        "starlink.CognitiveRecord/id": internal_record_id,
        "starlink.CognitiveRecord/apiVersion": api_version,
        "starlink.CognitiveRecord/originalPayload": str(raw_data),
        "starlink.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "starlink.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_starlink_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "starlink_entity",
