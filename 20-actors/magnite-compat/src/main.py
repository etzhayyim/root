"""
Py Kotodama WASM entrypoint for Magnite Cognitive Actor.
...
schema = load_schema("../schema/magnite.kotoba")
db = DatomicClient.connect()
app = Runtime("magnite-compat")
...
@app.route("/{api_version}/magnite/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_magnite_" + uuid.uuid4().hex[:12]
    db.transact([{
        "magnite.CognitiveRecord/id": internal_record_id,
        "magnite.CognitiveRecord/apiVersion": api_version,
        "magnite.CognitiveRecord/originalPayload": str(raw_data),
        "magnite.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "magnite.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_magnite_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "magnite_entity",
