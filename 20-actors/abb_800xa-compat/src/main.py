"""
Py Kotodama WASM entrypoint for Abb_800xa Cognitive Actor.
...
schema = load_schema("../schema/abb_800xa.kotoba")
db = DatomicClient.connect()
app = Runtime("abb_800xa-compat")
...
@app.route("/{api_version}/abb_800xa/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_abb_800xa_" + uuid.uuid4().hex[:12]
    db.transact([{
        "abb_800xa.CognitiveRecord/id": internal_record_id,
        "abb_800xa.CognitiveRecord/apiVersion": api_version,
        "abb_800xa.CognitiveRecord/originalPayload": str(raw_data),
        "abb_800xa.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "abb_800xa.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_abb_800xa_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "abb_800xa_entity",
