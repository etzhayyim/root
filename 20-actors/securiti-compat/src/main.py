"""
Py Kotodama WASM entrypoint for Securiti Cognitive Actor.
...
@app.route("/{api_version}/securiti/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_securiti_" + uuid.uuid4().hex[:12]
    db.transact([{
        "securiti.CognitiveRecord/id": internal_record_id,
        "securiti.CognitiveRecord/apiVersion": api_version,
        "securiti.CognitiveRecord/originalPayload": str(raw_data),
        "securiti.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "securiti.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_securiti_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "securiti_entity",
