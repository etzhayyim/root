(ns criteo.main
  "Kotodama WASM entrypoint for the Criteo clean-room actor (L4) — Clojure port.
  ...
  facts (`criteo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "criteo")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"criteo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "criteo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :criteo-compat/ready)
