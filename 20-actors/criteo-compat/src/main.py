"""
Py Kotodama WASM entrypoint for Criteo Cognitive Actor.
...
@app.route("/{api_version}/criteo/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_criteo_" + uuid.uuid4().hex[:12]
    db.transact([{
        "criteo.CognitiveRecord/id": internal_record_id,
        "criteo.CognitiveRecord/apiVersion": api_version,
        "criteo.CognitiveRecord/originalPayload": str(raw_data),
        "criteo.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "criteo.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_criteo_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "criteo_entity",
