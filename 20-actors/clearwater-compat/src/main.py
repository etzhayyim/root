"""
Py Kotodama WASM entrypoint for Clearwater Cognitive Actor.
...
@app.route("/{api_version}/clearwater/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_clearwater_" + uuid.uuid4().hex[:12]
    db.transact([{
        "clearwater.CognitiveRecord/id": internal_record_id,
        "clearwater.CognitiveRecord/apiVersion": api_version,
        "clearwater.CognitiveRecord/originalPayload": str(raw_data),
        "clearwater.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "clearwater.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_clearwater_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "clearwater_entity",
