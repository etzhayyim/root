"""
Py Kotodama WASM entrypoint for Ibm_zos Cognitive Actor.
...
schema = load_schema("../schema/ibm_zos.kotoba")
db = DatomicClient.connect()
app = Runtime("ibm_zos-compat")
...
@app.route("/{api_version}/ibm_zos/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ibm_zos_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ibm_zos.CognitiveRecord/id": internal_record_id,
        "ibm_zos.CognitiveRecord/apiVersion": api_version,
        "ibm_zos.CognitiveRecord/originalPayload": str(raw_data),
        "ibm_zos.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ibm_zos.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ibm_zos_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ibm_zos_entity",
