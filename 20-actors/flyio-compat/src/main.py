"""
Py Kotodama WASM entrypoint for Flyio Cognitive Actor.
...
schema = load_schema("../schema/flyio.kotoba")
db = DatomicClient.connect()
app = Runtime("flyio-compat")
...
@app.route("/{api_version}/flyio/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_flyio_" + uuid.uuid4().hex[:12]
    db.transact([{
        "flyio.CognitiveRecord/id": internal_record_id,
        "flyio.CognitiveRecord/apiVersion": api_version,
        "flyio.CognitiveRecord/originalPayload": str(raw_data),
        "flyio.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "flyio.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_flyio_" + uuid.uuid4().hex[:8])
