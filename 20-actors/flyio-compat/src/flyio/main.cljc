(ns flyio.main
  "Kotodama WASM entrypoint for the Flyio clean-room actor (L4) — Clojure port.
  ...
  facts (`flyio.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"flyio.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flyio-compat" :tier tier :entities entities} 200])
