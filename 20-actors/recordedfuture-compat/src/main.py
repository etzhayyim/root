"""
Py Kotodama WASM entrypoint for the Recordedfuture clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"recordedfuture.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"recordedfuture.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_indicator(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.Indicator", "id": eid})
...
def delete_vulnerability(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.Vulnerability", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_threat_actor(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.ThreatActor", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_campaign(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.Campaign", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_host(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.Host", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_technique(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"recordedfuture.Technique", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "recordedfuture-compat", "entities": ['Indicator', 'Vulnerability', 'ThreatActor', 'Campaign', 'Host', 'Technique']}, 200
