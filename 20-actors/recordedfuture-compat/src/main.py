"""
Py Kotodama WASM entrypoint for Recordedfuture Cognitive Actor.
...
schema = load_schema("../schema/recordedfuture.kotoba")
db = DatomicClient.connect()
app = Runtime("recordedfuture-compat")
...
@app.route("/{api_version}/recordedfuture/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_recordedfuture_" + uuid.uuid4().hex[:12]
    db.transact([{
        "recordedfuture.CognitiveRecord/id": internal_record_id,
        "recordedfuture.CognitiveRecord/apiVersion": api_version,
        "recordedfuture.CognitiveRecord/originalPayload": str(raw_data),
        "recordedfuture.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "recordedfuture.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_recordedfuture_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "recordedfuture_entity",
