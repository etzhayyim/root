(ns recordedfuture.main
  "Kotodama WASM entrypoint for the Recordedfuture clean-room actor (L4) — Clojure port.
  ...
  facts (`recordedfuture.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "recordedfuture")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"recordedfuture.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "recordedfuture-compat" :tier tier :entities entities} 200])
...
(defn start! [] :recordedfuture-compat/ready)
