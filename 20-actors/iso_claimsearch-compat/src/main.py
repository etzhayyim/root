"""
Py Kotodama WASM entrypoint for Iso_claimsearch Cognitive Actor.
...
schema = load_schema("../schema/iso_claimsearch.kotoba")
db = DatomicClient.connect()
app = Runtime("iso_claimsearch-compat")
...
@app.route("/{api_version}/iso_claimsearch/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iso_claimsearch_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iso_claimsearch.CognitiveRecord/id": internal_record_id,
        "iso_claimsearch.CognitiveRecord/apiVersion": api_version,
        "iso_claimsearch.CognitiveRecord/originalPayload": str(raw_data),
        "iso_claimsearch.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iso_claimsearch.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iso_claimsearch_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iso_claimsearch_entity",
