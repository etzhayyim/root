"""
Py Kotodama WASM entrypoint for Hmm_api Cognitive Actor.
...
schema = load_schema("../schema/hmm_api.kotoba")
db = DatomicClient.connect()
app = Runtime("hmm_api-compat")
...
@app.route("/{api_version}/hmm_api/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hmm_api_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hmm_api.CognitiveRecord/id": internal_record_id,
        "hmm_api.CognitiveRecord/apiVersion": api_version,
        "hmm_api.CognitiveRecord/originalPayload": str(raw_data),
        "hmm_api.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hmm_api.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hmm_api_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hmm_api_entity",
