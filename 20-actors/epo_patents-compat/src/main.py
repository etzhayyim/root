"""
Py Kotodama WASM entrypoint for Epo_patents Cognitive Actor.
...
schema = load_schema("../schema/epo_patents.kotoba")
db = DatomicClient.connect()
app = Runtime("epo_patents-compat")
...
@app.route("/{api_version}/epo_patents/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_epo_patents_" + uuid.uuid4().hex[:12]
    db.transact([{
        "epo_patents.CognitiveRecord/id": internal_record_id,
        "epo_patents.CognitiveRecord/apiVersion": api_version,
        "epo_patents.CognitiveRecord/originalPayload": str(raw_data),
        "epo_patents.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "epo_patents.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_epo_patents_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "epo_patents_entity",
