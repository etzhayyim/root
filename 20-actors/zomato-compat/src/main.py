"""
Py Kotodama WASM entrypoint for Zomato Cognitive Actor.
...
schema = load_schema("../schema/zomato.kotoba")
db = DatomicClient.connect()
app = Runtime("zomato-compat")
...
@app.route("/{api_version}/zomato/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_zomato_" + uuid.uuid4().hex[:12]
    db.transact([{
        "zomato.CognitiveRecord/id": internal_record_id,
        "zomato.CognitiveRecord/apiVersion": api_version,
        "zomato.CognitiveRecord/originalPayload": str(raw_data),
        "zomato.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "zomato.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_zomato_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "zomato_entity",
