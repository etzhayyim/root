"""
Py Kotodama WASM entrypoint for Elasticpath Cognitive Actor.
...
schema = load_schema("../schema/elasticpath.kotoba")
db = DatomicClient.connect()
app = Runtime("elasticpath-compat")
...
@app.route("/{api_version}/elasticpath/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_elasticpath_" + uuid.uuid4().hex[:12]
    db.transact([{
        "elasticpath.CognitiveRecord/id": internal_record_id,
        "elasticpath.CognitiveRecord/apiVersion": api_version,
        "elasticpath.CognitiveRecord/originalPayload": str(raw_data),
        "elasticpath.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "elasticpath.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_elasticpath_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "elasticpath_entity",
