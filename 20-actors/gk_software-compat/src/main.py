"""
Py Kotodama WASM entrypoint for Gk_software Cognitive Actor.
...
schema = load_schema("../schema/gk_software.kotoba")
db = DatomicClient.connect()
app = Runtime("gk_software-compat")
...
@app.route("/{api_version}/gk_software/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_gk_software_" + uuid.uuid4().hex[:12]
    db.transact([{
        "gk_software.CognitiveRecord/id": internal_record_id,
        "gk_software.CognitiveRecord/apiVersion": api_version,
        "gk_software.CognitiveRecord/originalPayload": str(raw_data),
        "gk_software.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "gk_software.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_gk_software_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "gk_software_entity",
