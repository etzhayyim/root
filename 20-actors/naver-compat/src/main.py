"""
Py Kotodama WASM entrypoint for Naver Cognitive Actor.
...
schema = load_schema("../schema/naver.kotoba")
db = DatomicClient.connect()
app = Runtime("naver-compat")
...
@app.route("/{api_version}/naver/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_naver_" + uuid.uuid4().hex[:12]
    db.transact([{
        "naver.CognitiveRecord/id": internal_record_id,
        "naver.CognitiveRecord/apiVersion": api_version,
        "naver.CognitiveRecord/originalPayload": str(raw_data),
        "naver.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "naver.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_naver_" + uuid.uuid4().hex[:8])
