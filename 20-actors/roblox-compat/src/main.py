"""
Py Kotodama WASM entrypoint for Roblox Cognitive Actor.
...
schema = load_schema("../schema/roblox.kotoba")
db = DatomicClient.connect()
app = Runtime("roblox-compat")
...
@app.route("/{api_version}/roblox/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_roblox_" + uuid.uuid4().hex[:12]
    db.transact([{
        "roblox.CognitiveRecord/id": internal_record_id,
        "roblox.CognitiveRecord/apiVersion": api_version,
        "roblox.CognitiveRecord/originalPayload": str(raw_data),
        "roblox.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "roblox.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_roblox_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "roblox_entity",
