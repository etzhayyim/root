"""
Py Kotodama WASM entrypoint for Brightcove Cognitive Actor.
...
@app.route("/{api_version}/brightcove/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_brightcove_" + uuid.uuid4().hex[:12]
    db.transact([{
        "brightcove.CognitiveRecord/id": internal_record_id,
        "brightcove.CognitiveRecord/apiVersion": api_version,
        "brightcove.CognitiveRecord/originalPayload": str(raw_data),
        "brightcove.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "brightcove.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_brightcove_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "brightcove_entity",
