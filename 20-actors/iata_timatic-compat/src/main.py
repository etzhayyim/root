"""
Py Kotodama WASM entrypoint for Iata_timatic Cognitive Actor.
...
schema = load_schema("../schema/iata_timatic.kotoba")
db = DatomicClient.connect()
app = Runtime("iata_timatic-compat")
...
@app.route("/{api_version}/iata_timatic/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iata_timatic_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iata_timatic.CognitiveRecord/id": internal_record_id,
        "iata_timatic.CognitiveRecord/apiVersion": api_version,
        "iata_timatic.CognitiveRecord/originalPayload": str(raw_data),
        "iata_timatic.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iata_timatic.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iata_timatic_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iata_timatic_entity",
