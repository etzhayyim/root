"""
Py Kotodama WASM entrypoint for the Iata_timatic clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"iata_timatic.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"iata_timatic.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_trade_item(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.TradeItem", "id": eid})
...
def delete_event(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.Event", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_document(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.Document", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_container(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.Container", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_party(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.Party", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_shipment(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"iata_timatic.Shipment", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "iata_timatic-compat", "entities": ['TradeItem', 'Event', 'Document', 'Container', 'Party', 'Shipment']}, 200
