(ns express_scripts.main
  "Kotodama WASM entrypoint for the Express_scripts clean-room actor (L4) — Clojure port.
  ...
  facts (`express_scripts.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "express_scripts")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "expresss_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "expresss_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "expresss_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "expresss_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "expresss_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "expresss_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"express_scripts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "express_scripts-compat" :tier tier :entities entities} 200])
...
(defn start! [] :express_scripts-compat/ready)
