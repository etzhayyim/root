"""
Py Kotodama WASM entrypoint for Express_scripts Cognitive Actor.
...
schema = load_schema("../schema/express_scripts.kotoba")
db = DatomicClient.connect()
app = Runtime("express_scripts-compat")
...
@app.route("/{api_version}/express_scripts/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_express_scripts_" + uuid.uuid4().hex[:12]
    db.transact([{
        "express_scripts.CognitiveRecord/id": internal_record_id,
        "express_scripts.CognitiveRecord/apiVersion": api_version,
        "express_scripts.CognitiveRecord/originalPayload": str(raw_data),
        "express_scripts.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "express_scripts.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_express_scripts_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "express_scripts_entity",
