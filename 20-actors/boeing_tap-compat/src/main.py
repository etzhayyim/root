"""
Py Kotodama WASM entrypoint for Boeing_tap Cognitive Actor.
...
schema = load_schema("../schema/boeing_tap.kotoba")
db = DatomicClient.connect()
app = Runtime("boeing_tap-compat")
...
@app.route("/{api_version}/boeing_tap/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_boeing_tap_" + uuid.uuid4().hex[:12]
    db.transact([{
        "boeing_tap.CognitiveRecord/id": internal_record_id,
        "boeing_tap.CognitiveRecord/apiVersion": api_version,
        "boeing_tap.CognitiveRecord/originalPayload": str(raw_data),
        "boeing_tap.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "boeing_tap.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_boeing_tap_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "boeing_tap_entity",
