"""
Py Kotodama WASM entrypoint for Typeform Cognitive Actor.
...
@app.route("/{api_version}/typeform/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_typeform_" + uuid.uuid4().hex[:12]
    db.transact([{
        "typeform.CognitiveRecord/id": internal_record_id,
        "typeform.CognitiveRecord/apiVersion": api_version,
        "typeform.CognitiveRecord/originalPayload": str(raw_data),
        "typeform.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "typeform.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_typeform_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "typeform_entity",
