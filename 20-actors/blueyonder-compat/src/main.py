"""
Py Kotodama WASM entrypoint for Blueyonder Cognitive Actor.
...
schema = load_schema("../schema/blueyonder.kotoba")
db = DatomicClient.connect()
app = Runtime("blueyonder-compat")
...
@app.route("/{api_version}/blueyonder/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_blueyonder_" + uuid.uuid4().hex[:12]
    db.transact([{
        "blueyonder.CognitiveRecord/id": internal_record_id,
        "blueyonder.CognitiveRecord/apiVersion": api_version,
        "blueyonder.CognitiveRecord/originalPayload": str(raw_data),
        "blueyonder.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "blueyonder.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_blueyonder_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "blueyonder_entity",
