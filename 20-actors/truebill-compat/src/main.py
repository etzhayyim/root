"""
Py Kotodama WASM entrypoint for Truebill Cognitive Actor.
...
schema = load_schema("../schema/truebill.kotoba")
db = DatomicClient.connect()
app = Runtime("truebill-compat")
...
@app.route("/{api_version}/truebill/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_truebill_" + uuid.uuid4().hex[:12]
    db.transact([{
        "truebill.CognitiveRecord/id": internal_record_id,
        "truebill.CognitiveRecord/apiVersion": api_version,
        "truebill.CognitiveRecord/originalPayload": str(raw_data),
        "truebill.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "truebill.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_truebill_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "truebill_entity",
