"""
Py Kotodama WASM entrypoint for Yoti Cognitive Actor.
...
schema = load_schema("../schema/yoti.kotoba")
db = DatomicClient.connect()
app = Runtime("yoti-compat")
...
@app.route("/{api_version}/yoti/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_yoti_" + uuid.uuid4().hex[:12]
    db.transact([{
        "yoti.CognitiveRecord/id": internal_record_id,
        "yoti.CognitiveRecord/apiVersion": api_version,
        "yoti.CognitiveRecord/originalPayload": str(raw_data),
        "yoti.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "yoti.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_yoti_" + uuid.uuid4().hex[:8])
