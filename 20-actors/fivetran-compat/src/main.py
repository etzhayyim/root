"""
Py Kotodama WASM entrypoint for Fivetran Cognitive Actor.
...
@app.route("/{api_version}/fivetran/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fivetran_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fivetran.CognitiveRecord/id": internal_record_id,
        "fivetran.CognitiveRecord/apiVersion": api_version,
        "fivetran.CognitiveRecord/originalPayload": str(raw_data),
        "fivetran.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fivetran.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fivetran_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fivetran_entity",
