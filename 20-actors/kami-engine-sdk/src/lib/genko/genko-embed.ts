...
async function saveToPDS(){
  ...
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.saveDocument',{docId:doc.docId,name:doc.name,document:json,convoId:activeProjectId||doc.convoId||''});
...
async function loadFromPDS(docId){
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.loadDocument',{docId});
...
async function listFromPDS(){
  try{return(await xrpc('app.etzhayyim.apps.mangaka.listDocuments',{limit:20})).items||[]}
...
async function loadProjects(){
  ...
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.listProjects',{limit:50});
...
projSelect.onchange=async()=>{
  ...
  let loaded=false;
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.listDocuments',{limit:1,convoId:val});
    const items=r.items||[];
    if(items.length>0){
      const d=items[0];
  ...
  try{
    ...
    const [wR,cR]=await Promise.allSettled([
      xrpc('app.etzhayyim.apps.mangaka.listWorks',{limit:20}),
      xrpc('app.etzhayyim.apps.mangaka.listCharacters',{limit:50}),
    ]);
...
document.getElementById('projNewOk').onclick=async()=>{
  ...
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.createProject',{name:nm,description:'Manga project: '+nm});
    if(r.convoId){
...
function buildProjectTocDoc(project){
  ...
  for(const [arc,epDocs] of arcMap){
    ...
    for(const d of epDocs){
      const linkNid=nid();
      const href='/at/'+appHost+'/app.etzhayyim.apps.mangaka.document/'+d.docId;
...
async function loadMembers(){
  ...
  if(activeProjectId){
    try{
      const r=await xrpc('app.etzhayyim.apps.mangaka.getMembers',{convoId:activeProjectId});
...
document.getElementById('mpAddBtn').onclick=async()=>{
  ...
  try{
    await xrpc('app.etzhayyim.apps.mangaka.addMember',{convoId:activeProjectId,memberDid:did,displayName:name});
...
async function sendChat(){
  ...
  if(isStoryRequest&&director){
    ...
    try{
      const r=await xrpc('app.etzhayyim.apps.mangaka.storyboard',{story:txt+buildContextStr()});
  ...
  try{
    ...
    const r=await xrpc('app.etzhayyim.apps.mangaka.generateImage',{prompt:txt+ctxStr,style:'manga'});
...
async function generateForPanel(oi,style,artistName){
  ...
  try{
    const r=await xrpc('app.etzhayyim.apps.mangaka.generateImage',{prompt:promptText,style});
    if(r.image){
...
async function generatePanelImage(panelIdx){
  ...
  try{
    ...
    const r=await xrpc('app.etzhayyim.apps.mangaka.generateImage',{
