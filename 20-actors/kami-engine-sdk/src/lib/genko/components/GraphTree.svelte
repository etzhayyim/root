<!--
  ...
  their pages. Data comes from:
    POST /xrpc/app.etzhayyim.apps.mangaka.listChapters { workId }
    POST /xrpc/app.etzhayyim.apps.mangaka.listPages    { chapterId }
...
<script lang="ts">
  ...
  async function loadChapters(workId: string) {
    ...
    errorMsg = '';
    try {
      const r = await fetch('/xrpc/app.etzhayyim.apps.mangaka.listChapters', {
        method: 'POST',
  ...
  async function loadPages(chapterId: string) {
    ...
    try {
      const r = await fetch('/xrpc/app.etzhayyim.apps.mangaka.listPages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  async function saveEdit() {
    ...
    saveError = '';
    try {
      const r = await fetch('/xrpc/app.etzhayyim.apps.mangaka.publishChapter', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({
  ...
  async function savePageEdit(chapterId: string) {
    ...
    pageSaveError = '';
    try {
      const r = await fetch('/xrpc/app.etzhayyim.apps.mangaka.updatePage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({
  ...
  async function submitCreateChapter() {
    ...
    try {
      const workAtUri = `at://mng4k4x1.etzhayyim.com/app.etzhayyim.apps.mangaka.work/${workId}`;
      const r = await fetch('/xrpc/app.etzhayyim.apps.mangaka.addChapter', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({
          workId: workAtUri,
