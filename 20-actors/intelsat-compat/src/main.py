"""
Py Kotodama WASM entrypoint for Intelsat Cognitive Actor.
...
@app.route("/{api_version}/intelsat/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_intelsat_" + uuid.uuid4().hex[:12]
    db.transact([{
        "intelsat.CognitiveRecord/id": internal_record_id,
        "intelsat.CognitiveRecord/apiVersion": api_version,
        "intelsat.CognitiveRecord/originalPayload": str(raw_data),
        "intelsat.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "intelsat.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_intelsat_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "intelsat_entity",
