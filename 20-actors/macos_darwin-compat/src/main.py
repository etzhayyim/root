"""
Py Kotodama WASM entrypoint for Macos_darwin Cognitive Actor.
...
@app.route("/{api_version}/macos_darwin/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_macos_darwin_" + uuid.uuid4().hex[:12]
    db.transact([{
        "macos_darwin.CognitiveRecord/id": internal_record_id,
        "macos_darwin.CognitiveRecord/apiVersion": api_version,
        "macos_darwin.CognitiveRecord/originalPayload": str(raw_data),
        "macos_darwin.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "macos_darwin.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_macos_darwin_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "macos_darwin_entity",
