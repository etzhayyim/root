"""
Py Kotodama WASM entrypoint for Mosip Cognitive Actor.
...
schema = load_schema("../schema/mosip.kotoba")
db = DatomicClient.connect()
app = Runtime("mosip-compat")
...
@app.route("/{api_version}/mosip/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_mosip_" + uuid.uuid4().hex[:12]
    db.transact([{
        "mosip.CognitiveRecord/id": internal_record_id,
        "mosip.CognitiveRecord/apiVersion": api_version,
        "mosip.CognitiveRecord/originalPayload": str(raw_data),
        "mosip.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "mosip.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_mosip_" + uuid.uuid4().hex[:8])
