"""
Py Kotodama WASM entrypoint for the Dynamics365 clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"dynamics365.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"dynamics365.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_account(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Account", "id": eid})
...
def delete_contact(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Contact", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_lead(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Lead", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_opportunity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Opportunity", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_activity(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Activity", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_pipeline(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"dynamics365.Pipeline", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "dynamics365-compat", "entities": ['Account', 'Contact', 'Lead', 'Opportunity', 'Activity', 'Pipeline']}, 200
