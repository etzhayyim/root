...
## XRPC (browser → atproto.gftd.ai)

人間のブラウザアクセスは **atproto.gftd.ai** の XRPC 経由:

- Convo: `atproto.gftd.ai/xrpc/ai.gftd.convo.{method}` (messaging/convos/DMs)
- Signal: `atproto.gftd.ai/xrpc/ai.gftd.signal.{method}` (E2E encryption)
- RTC: `atproto.gftd.ai/xrpc/ai.gftd.rtc.{method}` (VoIP/meetings)
- AT Standard: `atproto.gftd.ai/xrpc/{app.bsky|com.atproto|chat.bsky}.*`
- Stream: `atproto.gftd.ai/xrpc/com.atproto.sync.subscribeRepos` (SSE)

**yoro.gftd.ai 自体に data endpoint は公開しない。**
...
|---|---|---|
| **Actors** (default) | AT Protocol Actor 検索 (登録日/更新日ソート) | `searchActors("gftd", {limit:25})` (`app.bsky.actor.searchActors`) → PDS SQL `STARTS WITH` + `did:web:` server-side filter → `indexedAt` 付き response → client-side sort (re-fetch なし)。PDS 側: `:Profile` + `:App` SQL ノード (PascalCase) |
