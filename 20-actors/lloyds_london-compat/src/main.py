"""
Py Kotodama WASM entrypoint for Lloyds_london Cognitive Actor.
...
schema = load_schema("../schema/lloyds_london.kotoba")
db = DatomicClient.connect()
app = Runtime("lloyds_london-compat")
...
@app.route("/{api_version}/lloyds_london/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_lloyds_london_" + uuid.uuid4().hex[:12]
    db.transact([{
        "lloyds_london.CognitiveRecord/id": internal_record_id,
        "lloyds_london.CognitiveRecord/apiVersion": api_version,
        "lloyds_london.CognitiveRecord/originalPayload": str(raw_data),
        "lloyds_london.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "lloyds_london.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_lloyds_london_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "lloyds_london_entity",
