"""
Py Kotodama WASM entrypoint for Fao_stat Cognitive Actor.
...
schema = load_schema("../schema/fao_stat.kotoba")
db = DatomicClient.connect()
app = Runtime("fao_stat-compat")
...
@app.route("/{api_version}/fao_stat/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_fao_stat_" + uuid.uuid4().hex[:12]
    db.transact([{
        "fao_stat.CognitiveRecord/id": internal_record_id,
        "fao_stat.CognitiveRecord/apiVersion": api_version,
        "fao_stat.CognitiveRecord/originalPayload": str(raw_data),
        "fao_stat.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "fao_stat.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_fao_stat_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "fao_stat_entity",
