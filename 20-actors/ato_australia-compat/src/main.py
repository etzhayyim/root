"""
Py Kotodama WASM entrypoint for Ato_australia Cognitive Actor.
...
schema = load_schema("../schema/ato_australia.kotoba")
db = DatomicClient.connect()
app = Runtime("ato_australia-compat")
...
@app.route("/{api_version}/ato_australia/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_ato_australia_" + uuid.uuid4().hex[:12]
    db.transact([{
        "ato_australia.CognitiveRecord/id": internal_record_id,
        "ato_australia.CognitiveRecord/apiVersion": api_version,
        "ato_australia.CognitiveRecord/originalPayload": str(raw_data),
        "ato_australia.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "ato_australia.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_ato_australia_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "ato_australia_entity",
