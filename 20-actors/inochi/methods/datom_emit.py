...
Pure stdlib — runnable inside the inochi kotoba pywasm actor (componentize-py).
Usage:
...
EDGE_ATTRS = [":en/from", ":en/to", ":en/kind", ":en/grasping-load", ":en/sourcing"]
...
def emit(nodes: dict, edges: list, res: dict, tx: int = 1) -> str:
    L = []
    L.append(";; inochi 命 — GENERATED kotoba Datom log (ADR-2606073000). DO NOT hand-edit.")
    ...
    for nid, v in sorted(res["restoration"].items(), key=lambda kv: -kv[1]):
        L.append(f"[{_fmt(nid)} :bond/restoration-priority {v:g} {tx} :derived] ;; :bond/is-transient true")
...
def main(argv):
    ...
    out = outdir / "biosphere-datoms.kotoba.edn"
    out.write_text(emit(nodes, edges, res, tx), encoding="utf-8")
    ground = sum(1 for _ in nodes) + len(edges)
    print(f"inochi datom log → {out} ({len(nodes)} nodes + {len(edges)} 縁, tx={tx})")
    return 0
