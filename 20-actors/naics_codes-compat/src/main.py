"""
Py Kotodama WASM entrypoint for the Naics_codes clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"naics_codes.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"naics_codes.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_code(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Code", "id": eid})
...
def delete_unit(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Unit", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_reference(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Reference", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_mapping(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Mapping", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_location(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Location", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_epoch(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"naics_codes.Epoch", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "naics_codes-compat", "entities": ['Code', 'Unit', 'Reference', 'Mapping', 'Location', 'Epoch']}, 200
