"""
Py Kotodama WASM entrypoint for Sel4 Cognitive Actor.
...
schema = load_schema("../schema/sel4.kotoba")
db = DatomicClient.connect()
app = Runtime("sel4-compat")
...
@app.route("/{api_version}/sel4/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_sel4_" + uuid.uuid4().hex[:12]
    db.transact([{
        "sel4.CognitiveRecord/id": internal_record_id,
        "sel4.CognitiveRecord/apiVersion": api_version,
        "sel4.CognitiveRecord/originalPayload": str(raw_data),
        "sel4.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "sel4.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_sel4_" + uuid.uuid4().hex[:8])
