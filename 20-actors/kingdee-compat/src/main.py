"""
Py Kotodama WASM entrypoint for Kingdee Cognitive Actor.
...
schema = load_schema("../schema/kingdee.kotoba")
db = DatomicClient.connect()
app = Runtime("kingdee-compat")
...
@app.route("/{api_version}/kingdee/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_kingdee_" + uuid.uuid4().hex[:12]
    db.transact([{
        "kingdee.CognitiveRecord/id": internal_record_id,
        "kingdee.CognitiveRecord/apiVersion": api_version,
        "kingdee.CognitiveRecord/originalPayload": str(raw_data),
        "kingdee.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "kingdee.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_kingdee_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "kingdee_entity",
