"""
Py Kotodama WASM entrypoint for Relayhealth Cognitive Actor.
...
schema = load_schema("../schema/relayhealth.kotoba")
db = DatomicClient.connect()
app = Runtime("relayhealth-compat")
...
@app.route("/{api_version}/relayhealth/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_relayhealth_" + uuid.uuid4().hex[:12]
    db.transact([{
        "relayhealth.CognitiveRecord/id": internal_record_id,
        "relayhealth.CognitiveRecord/apiVersion": api_version,
        "relayhealth.CognitiveRecord/originalPayload": str(raw_data),
        "relayhealth.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "relayhealth.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_relayhealth_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "relayhealth_entity",
