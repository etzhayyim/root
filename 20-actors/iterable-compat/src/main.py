"""
Py Kotodama WASM entrypoint for Iterable Cognitive Actor.
...
@app.route("/{api_version}/iterable/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_iterable_" + uuid.uuid4().hex[:12]
    db.transact([{
        "iterable.CognitiveRecord/id": internal_record_id,
        "iterable.CognitiveRecord/apiVersion": api_version,
        "iterable.CognitiveRecord/originalPayload": str(raw_data),
        "iterable.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "iterable.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_iterable_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "iterable_entity",
