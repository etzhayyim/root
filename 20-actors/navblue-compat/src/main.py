"""
Py Kotodama WASM entrypoint for Navblue Cognitive Actor.
...
@app.route("/{api_version}/navblue/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_navblue_" + uuid.uuid4().hex[:12]
    db.transact([{
        "navblue.CognitiveRecord/id": internal_record_id,
        "navblue.CognitiveRecord/apiVersion": api_version,
        "navblue.CognitiveRecord/originalPayload": str(raw_data),
        "navblue.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "navblue.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_navblue_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "navblue_entity",
