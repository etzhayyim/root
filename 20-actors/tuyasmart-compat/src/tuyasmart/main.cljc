(ns tuyasmart.main
  "Kotodama WASM entrypoint for the Tuyasmart clean-room actor (L4) — Clojure port.
  ...
  facts (`tuyasmart.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tuyasmart")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tuyasmart.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tuyasmart-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tuyasmart-compat/ready)
