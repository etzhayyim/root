...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "tuyasmart-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'tuyasmart.main-test)]
