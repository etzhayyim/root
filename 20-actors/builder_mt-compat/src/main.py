"""
Py Kotodama WASM entrypoint for Builder_mt Cognitive Actor.
...
schema = load_schema("../schema/builder_mt.kotoba")
db = DatomicClient.connect()
app = Runtime("builder_mt-compat")
...
@app.route("/{api_version}/builder_mt/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_builder_mt_" + uuid.uuid4().hex[:12]
    db.transact([{
        "builder_mt.CognitiveRecord/id": internal_record_id,
        "builder_mt.CognitiveRecord/apiVersion": api_version,
        "builder_mt.CognitiveRecord/originalPayload": str(raw_data),
        "builder_mt.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "builder_mt.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_builder_mt_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "builder_mt_entity",
