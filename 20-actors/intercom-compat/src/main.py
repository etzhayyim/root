"""
Py Kotodama WASM entrypoint for Intercom Cognitive Actor.
...
@app.route("/{api_version}/intercom/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_intercom_" + uuid.uuid4().hex[:12]
    db.transact([{
        "intercom.CognitiveRecord/id": internal_record_id,
        "intercom.CognitiveRecord/apiVersion": api_version,
        "intercom.CognitiveRecord/originalPayload": str(raw_data),
        "intercom.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "intercom.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_intercom_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "intercom_entity",
