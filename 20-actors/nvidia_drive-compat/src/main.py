"""
Py Kotodama WASM entrypoint for the Nvidia_drive clean-room actor.
...
def _persist(entity, rec):
    ...
    for k, v in rec.items():
        facts[f"nvidia_drive.{entity}/{k}"] = v
    db.transact([facts])
    return rec
...
def _query(entity, eid=None):
    ...
    pattern = {"entity": f"nvidia_drive.{entity}"}
    if eid is not None:
        pattern["id"] = eid
...
def delete_vehicle(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.Vehicle", "id": eid})
...
def delete_signal(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.Signal", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_charge_session(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.ChargeSession", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_connector(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.Connector", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_trip(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.Trip", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_command(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found"}}, 404
    db.retract({"entity": f"nvidia_drive.Command", "id": eid})
    return {"id": eid, "deleted": True}, 200

@app.route("/healthz", methods=["GET"])
def healthz(request):
    return {"status": "ok", "actor": "nvidia_drive-compat", "entities": ['Vehicle', 'Signal', 'ChargeSession', 'Connector', 'Trip', 'Command']}, 200
