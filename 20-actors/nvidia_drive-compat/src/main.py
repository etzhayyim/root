"""
Py Kotodama WASM entrypoint for Nvidia_drive Cognitive Actor.
...
schema = load_schema("../schema/nvidia_drive.kotoba")
db = DatomicClient.connect()
app = Runtime("nvidia_drive-compat")
...
@app.route("/{api_version}/nvidia_drive/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_nvidia_drive_" + uuid.uuid4().hex[:12]
    db.transact([{
        "nvidia_drive.CognitiveRecord/id": internal_record_id,
        "nvidia_drive.CognitiveRecord/apiVersion": api_version,
        "nvidia_drive.CognitiveRecord/originalPayload": str(raw_data),
        "nvidia_drive.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "nvidia_drive.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_nvidia_drive_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "nvidia_drive_entity",
