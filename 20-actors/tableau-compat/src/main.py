"""
Py Kotodama WASM entrypoint for Tableau Cognitive Actor.
...
@app.route("/{api_version}/tableau/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_tableau_" + uuid.uuid4().hex[:12]
    db.transact([{
        "tableau.CognitiveRecord/id": internal_record_id,
        "tableau.CognitiveRecord/apiVersion": api_version,
        "tableau.CognitiveRecord/originalPayload": str(raw_data),
        "tableau.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "tableau.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_tableau_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "tableau_entity",
