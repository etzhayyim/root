"""
Py Kotodama WASM entrypoint for Webflow Cognitive Actor.
...
@app.route("/{api_version}/webflow/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_webflow_" + uuid.uuid4().hex[:12]
    db.transact([{
        "webflow.CognitiveRecord/id": internal_record_id,
        "webflow.CognitiveRecord/apiVersion": api_version,
        "webflow.CognitiveRecord/originalPayload": str(raw_data),
        "webflow.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "webflow.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_webflow_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "webflow_entity",
