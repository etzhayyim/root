(ns webflow.main
  "Kotodama WASM entrypoint for the Webflow clean-room actor (L4) — Clojure port.
  ...
  facts (`webflow.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "webflow")
(def tier "L4")
...
(def entity-specs
  [{:entity "Site"           :plural "sites"           :id-prefix "webflow_sit"
    :fields [:displayName :shortName :timeZone :createdOn :lastUpdated :lastPublished] :required [:displayName]
    :coerce {}                                         :refs {}}
   {:entity "Collection"     :plural "collections"     :id-prefix "webflow_col"
    :fields [:displayName :singularName :slug :createdOn :lastUpdated] :required [:displayName]
    :coerce {}                                         :refs {}}
   {:entity "CollectionItem" :plural "collectionitems" :id-prefix "webflow_col"
    :fields [:cmsLocaleId :isArchived :isDraft :createdOn :lastUpdated :lastPublished] :required [:isArchived]
    :coerce {:isArchived :bool :isDraft :bool}       :refs {}}
   {:entity "Page"           :plural "pages"           :id-prefix "webflow_pag"
    :fields [:siteId :title :slug :archived :draft :publishedPath :parentId :createdOn :lastUpdated] :required [:title]
    :coerce {:archived :bool :draft :bool}            :refs {:siteId "Site"}}
   {:entity "Form"           :plural "forms"           :id-prefix "webflow_for"
    :fields [:displayName :siteId :pageId :pageName :formElementId :createdOn :lastUpdated] :required [:displayName]
    :coerce {}                                         :refs {:siteId "Site" :pageId "Page"}}
   {:entity "Field"          :plural "fields"          :id-prefix "webflow_fie"
    :fields [:displayName :slug :type :isRequired]     :required [:displayName]
    :coerce {:isRequired :bool}                        :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"webflow.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "webflow-compat" :tier tier :entities entities} 200])
...
(defn start! [] :webflow-compat/ready)
