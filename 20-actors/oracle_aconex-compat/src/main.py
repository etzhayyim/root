"""
Py Kotodama WASM entrypoint for Oracle_aconex Cognitive Actor.
...
schema = load_schema("../schema/oracle_aconex.kotoba")
db = DatomicClient.connect()
app = Runtime("oracle_aconex-compat")
...
@app.route("/{api_version}/oracle_aconex/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_oracle_aconex_" + uuid.uuid4().hex[:12]
    db.transact([{
        "oracle_aconex.CognitiveRecord/id": internal_record_id,
        "oracle_aconex.CognitiveRecord/apiVersion": api_version,
        "oracle_aconex.CognitiveRecord/originalPayload": str(raw_data),
        "oracle_aconex.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "oracle_aconex.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_oracle_aconex_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "oracle_aconex_entity",
