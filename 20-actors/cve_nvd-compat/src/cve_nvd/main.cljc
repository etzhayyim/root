(ns cve_nvd.main
  "Kotodama WASM entrypoint for the Cve_nvd clean-room actor (L4) — Clojure port.
  ...
  facts (`cve_nvd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cve_nvd")
(def tier "L4")
...
(def entity-specs
  [{:entity "CveItem"       :plural "cveitems"       :id-prefix "cvenvd_cve"
    :fields [:cveId :sourceIdentifier :vulnStatus :published :lastModified] :required [:sourceIdentifier :vulnStatus]
    :coerce {}                       :refs {}}
   {:entity "CvssV3Metric"  :plural "cvssv3metrics"  :id-prefix "cvenvd_cvs"
    :fields [:baseScore :baseSeverity :exploitabilityScore :impactScore]      :required [:baseScore :baseSeverity]
    :coerce {:baseScore :float :exploitabilityScore :float :impactScore :float} :refs {}}
   {:entity "CvssV3Vector"  :plural "cvssv3vectors"  :id-prefix "cvenvd_cvs"
    :fields [:version :vectorString :attackVector :attackComplexity :privilegesRequired :userInteraction :scope :confidentialityImpact :integrityImpact :availabilityImpact] :required [:version :vectorString]
    :coerce {}                       :refs {}}
   {:entity "Reference"     :plural "references"     :id-prefix "cvenvd_ref"
    :fields [:url :source]                            :required [:url :source]
    :coerce {}                       :refs {}}
   {:entity "Weakness"      :plural "weaknesses"     :id-prefix "cvenvd_wea"
    :fields [:source :type :description]              :required [:source :type]
    :coerce {}                       :refs {}}
   {:entity "CveMetrics"    :plural "cvemetricses"   :id-prefix "cvenvd_cve"
    :fields [:source :type]                           :required [:source :type]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"cve_nvd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cve_nvd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cve_nvd-compat/ready)
