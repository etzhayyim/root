"""
Py Kotodama WASM entrypoint for Hl7_fhir Cognitive Actor.
...
schema = load_schema("../schema/hl7_fhir.kotoba")
db = DatomicClient.connect()
app = Runtime("hl7_fhir-compat")
...
@app.route("/{api_version}/hl7_fhir/action", methods=["POST", "GET"])
def process_strict_request(request, api_version: str):
    ...
    internal_record_id = f"cog_hl7_fhir_" + uuid.uuid4().hex[:12]
    db.transact([{
        "hl7_fhir.CognitiveRecord/id": internal_record_id,
        "hl7_fhir.CognitiveRecord/apiVersion": api_version,
        "hl7_fhir.CognitiveRecord/originalPayload": str(raw_data),
        "hl7_fhir.CognitiveRecord/internalRiskScore": final_state.get("risk_score"),
        "hl7_fhir.CognitiveRecord/computedAt": datetime.datetime.utcnow().isoformat()
    }])
    ...
    client_id = raw_data.get("id", f"ext_hl7_fhir_" + uuid.uuid4().hex[:8])

    compatibility_response = {
        "id": client_id,
        "object": "hl7_fhir_entity",
