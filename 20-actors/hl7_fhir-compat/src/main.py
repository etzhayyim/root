"""
Py Kotodama WASM entrypoint for the Hl7_fhir clean-room actor (L4).
...
def delete_encounter(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"hl7_fhir.Encounter", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_medication_request(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"hl7_fhir.MedicationRequest", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def delete_allergy_intolerance(request, eid):
    ...
    if not rows:
        return {"error": {"message": "Not found", "type": "not_found"}}, 404
    db.retract({"entity": f"hl7_fhir.AllergyIntolerance", "id": eid})
    return {"id": eid, "deleted": True}, 200
...
def healthz(request):
    return {"status": "ok", "actor": "hl7_fhir-compat", "tier": "L4",
