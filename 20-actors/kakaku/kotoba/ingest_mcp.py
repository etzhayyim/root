...
ADR-2605091200. Flattens each seed entity map into datoms and asserts them via the
`kotoba_datom_create` MCP tool; `kotoba commit` seals them. Writes require an operator
AT-session JWT (no-server-key posture). --dry-run parses + counts datoms only (no
writes, no LLM; G5 untouched). Live retailer ingest is a separate G11-gated path.

Usage:
    python3 ingest_mcp.py [--url http://127.0.0.1:8077] [--graph com.etzhayyim.kakaku] [--dry-run]
"""
