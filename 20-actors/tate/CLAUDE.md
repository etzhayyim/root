...
**Jurisdictions (R1)**: `:jp :us :eu :uk :de :kr :fr :au :ca :it :es :nl :br :tw :sg :in :cn :pl :se :at :pt :ie :ch :dk :fi :no :mx :be :ar :nz` — representative 30 of ~193, plus `us-states.edn` (45/50 州 — :us 通知に州が分かれば州規則を開示追記、州不明は honest degrade)
...
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (76 procs; :civil 36 + :labor 10 + :housing 8 + :enforcement 8 + :insolvency 6 + :family 8 — track×juris matrix; fake-guard 語彙 registry 自動導出; :dl/critical 期限先頭表示; **非civil全手続きに protective 選択肢必在**)
...
python3 tests/test_terms.py && python3 tests/test_respond.py \
  && python3 tests/test_coverage.py  # 75 green
```
