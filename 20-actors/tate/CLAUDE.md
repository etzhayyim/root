...
**Jurisdictions (R1)**: `:jp :us :eu :uk :de :kr :fr :au :ca :it :es :nl :br :tw :sg :in :cn :pl :se :at :pt :ie :ch :dk :fi :no :mx` — representative 27 of ~193, plus `us-states.edn` (10/50 州: CA/NY/TX/FL/WA/IL/PA/OH/GA/NC — :us 通知に州が分かれば州規則を開示追記、州不明は honest degrade)
...
│   ├── clause-patterns.edn        # jurisdiction-keyed clause registry (66 shapes, 27 juris)
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (56 procs; :civil 33 + :labor 6 + :housing 6 + :enforcement 5 + :insolvency 3 + :family 3 — track×juris matrix in coverage report)
...
python3 tests/test_terms.py && python3 tests/test_respond.py \
  && python3 tests/test_coverage.py  # 62 green
```
