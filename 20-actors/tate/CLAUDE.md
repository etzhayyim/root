...
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (181 procs; :civil 36 + :labor 29 + :housing 29 + :enforcement 29 + :insolvency 29 + :family 29 — track×juris matrix; fake-guard 語彙 registry 自動導出; :dl/critical 期限先頭表示; **非civil全手続きに protective 選択肢必在**; :dl/source-url 一次ソース URL を verified deadline-rule に記録 — proc-level provenance 96/181 (rule-level 121、過半数 53%)、:jp + :de + :uk + :fr + :kr + :it + :es + :nl + :au + :ca + :pl + :at + :ch 完備 (13 法域) + :in (CPC/ID Act/Gratuity/IBC) 進行、:tw は 全國法規資料庫 (勞動基準法 N0030001 / 民事訴訟法 B0010001) で裏取り開始 (残 家事事件法/強制執行法/破產法/租賃住宅條例)、法令単位で back-fill 継続)
