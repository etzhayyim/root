...
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (181 procs; :civil 36 + :labor 29 + :housing 29 + :enforcement 29 + :insolvency 29 + :family 29 — track×juris matrix; fake-guard 語彙 registry 自動導出; :dl/critical 期限先頭表示; **非civil全手続きに protective 選択肢必在**; :dl/source-url 一次ソース URL を verified deadline-rule に記録 — proc-level provenance 61/181 (rule-level 82)、:jp + :de + :uk + :fr + :kr + :it + :es + :nl 完備 (8 法域)、:au は federal 3法 (Fair Work Act/Corporations Act/Family Law Act, legislation.gov.au) 裏取り済み (残 NSW state acts on legislation.nsw.gov.au)、法令単位で back-fill 継続)
