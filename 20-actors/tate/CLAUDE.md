...
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (181 procs; :civil 36 + :labor 29 + :housing 29 + :enforcement 29 + :insolvency 29 + :family 29 — track×juris matrix; fake-guard 語彙 registry 自動導出; :dl/critical 期限先頭表示; **非civil全手続きに protective 選択肢必在**; :dl/source-url 一次ソース URL を verified deadline-rule に記録 — proc-level provenance 104/181 (rule-level 129、57%)、:jp + :de + :uk + :fr + :kr + :it + :es + :nl + :au + :ca + :pl + :at + :ch 完備 (13 法域) + :in/:tw/:sg/:be 進行、:cn は 国家法律法规数据库 (劳动合同法 flk.npc.gov.cn/detail?id=2c909…) で裏取り開始 (残 民事诉讼法/民法典/企业破产法/劳动争议法 各 flk id を逐次検証)、法令単位で back-fill 継続)
