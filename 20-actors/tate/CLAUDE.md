...
│   ├── procedure-registry.edn     # jurisdiction-keyed procedure registry (181 procs; :civil 36 + :labor 29 + :housing 29 + :enforcement 29 + :insolvency 29 + :family 29 — track×juris matrix; fake-guard 語彙 registry 自動導出; :dl/critical 期限先頭表示; **非civil全手続きに protective 選択肢必在**; :dl/source-url 一次ソース URL を verified deadline-rule に記録 — proc-level provenance 66/181 (rule-level 89)、:jp + :de + :uk + :fr + :kr + :it + :es + :nl + :au 完備 (9 法域)、:ca は Ontario e-Laws (ESA 2000 00e41 / RTA 2006 06r17) で裏取り開始 (残 BIA federal/ON Wages Act/Small Claims/Family Law Rules)、法令単位で back-fill 継続)
