(ns tate.methods.cid
  ...
  Vendored 1:1 from the repo-canonical helper (rasen/jinushi/hinagata/…); ADR-2606101000.
  ...
  Used here to content-address the PUBLIC anonymized coverage digest (coverage_publish) so it
  is mesh-distributable + verifiable. The digest holds AGGREGATE registry metadata only —
  never member documents (G1).
...
(defn base32
  "RFC4648 base32-lower, no padding. `data` = seq of byte/int values."
  [data]
