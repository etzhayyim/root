...
security add-generic-password -s gftd.litellm -a MASTER_KEY \
  -w "sk-litellm-$(openssl rand -hex 32)" -U

# 2. Clone repo + cd to litellm/
cd 60-apps/ai-gftd-project-murakumo/litellm
...
- **Logs**: `/var/log/gftd-litellm.{out,err}.log`
- **Restart**: `launchctl unload ~/Library/LaunchAgents/com.gftd.litellm.plist && launchctl load ~/Library/LaunchAgents/com.gftd.litellm.plist`
