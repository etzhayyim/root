...
```bash
docker build \
  -t ghcr.io/gftd-ai/murakumo-llama-vulkan:local \
  60-apps/ai-gftd-project-murakumo/containers/llama-vulkan
```

For local k3s, import the image into containerd:

```bash
docker save ghcr.io/gftd-ai/murakumo-llama-vulkan:local \
...
```bash
kubectl apply -k 60-apps/ai-gftd-project-murakumo/k8s/llama-vulkan
