...
// Usage:
//
//	murakumo-mesh                  # Start mesh + proxy (reads ~/.gftd/mesh/config.json)
...
func cmdInitFleet() {
	...
	fmt.Println("\nDistribute each mesh-<name>.json to ~/.gftd/mesh/config.json on the corresponding node.")
}
...
func cmdInitClient(name string) {
	...
	fmt.Printf("Then create ~/.gftd/mesh/config.json with init-fleet output.\n")
