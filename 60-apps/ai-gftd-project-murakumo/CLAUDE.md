# ai-gftd-project-murakumo — Server Inference Platform (Compute + Inference)
...
| Fleet CLI (Go) | `70-tools/gftd/murakumo.go` + `murakumo_fleet.go` |
...
| Role | Target | 内容 |
|---|---|---|
| **common** | fleet | .gftd dir, SSH keys, node.conf |
...
| LiteLLM | judah `~/litellm-venv/bin/litellm` 1.83.9 | `0.0.0.0:4000` | `crontab -l | grep litellm` — `@reboot` entry | `~/.gftd/litellm.log` |
| Config | `/Users/judah/litellm.yaml` | router_settings: `routing_strategy: simple-shuffle`, `num_retries: 2`, `timeout: 30` | — | — |
| Secret | `/Users/judah/.gftd/rw-url` (chmod 600) + macOS Keychain `gftd.rw ROOT_URL` | RisingWave PG URL for goose recipes (ADR-0034). LiteLLM master_key `sk-gftd-litellm-local` from ansible var | — | — |
...
crontab @ judah
  ...
  └─ */15m  yoro-mention-drain            (subscribeRepos substitute → reply to @yoro)
           │
           ▼ ~/.gftd/goose-cron-wrapper.sh (shinka throttle + deterministic side effects)
           │   1. resolve $RW_URL (Keychain → ~/.gftd/rw-url)
           ...
           │   5. optional AT post via `gftd agent-token --lxm com.atproto.repo.createRecord`
...
→ `gftd dodaf tv1 query --id ai-gftd-project-murakumo-yoro-human-participation-credits` / MCP `gftd.dodaf.tv1.query`

## Model Family: `gftd/gftd-moe-moe-kyun` (9-Node Mac Mini Cluster)

**Opus 4.6 (1M context) distilled specialist model family。** Naming: `gftd/gftd-moe-moe-kyun-{specialist}-{version}`。Cloud orchestration (CoordinatorDO + B2) + Mac Mini M4 × 9 ノード MLX LoRA。
...
|---|---|---|---|---|---|
| benjamin | gftd-moe-moe-kyun-general-v0 (Qwen2.5-3B) | 3.1B | 2.0 GB | 19.7 tok/s | 2026-03-24 |
...
| Node | Model ID (`gftd-moe-moe-kyun-*`) | Base | RAM |
|---|---|---|---|
...
- `gftd murakumo ingest --extract-raw` も同じ key を利用:
  1. `--extract-api-key`
  2. `MURAKUMO_API_KEY` env
  3. `~/.config/gftd/murakumo_api_key`

### B2 Key Layout (`gftd-moe-moe-kyun/`)
...
`60-apps/ai-gftd-project-murakumo/_archive/` or deleted; git history preserves.
...
| `70-tools/gftd/gftd/openclaw.go` CLI proxy | Unused shim around the retired `openclaw` binary. | Deleted 2026-04-20; `gftd agent-token --lxm` covers the scoped-auth use case. |
