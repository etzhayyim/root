...
// ────────────────────────────────────────────────────────────────────────────
// ai-gftd-project-cowork-graph — Cowork Graph Connector (c0w0rkg1)
//
...
let dbHandle: Kysely<Database> | null = null;
...
async function rwQuery<T>(db: Kysely<any>, sql: string, params: unknown[] = []): Promise<T[]> {
...
export default createWorkerExport((sdk: HostSDK) => {
  ...
  if (env.HYPERDRIVE && !dbHandle) {
    dbHandle = createKyselyDb(env.HYPERDRIVE) as Kysely<Database>;
  }
