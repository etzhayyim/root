// gmail.gftd.ai thin edge facade. Gmail OAuth/sync/send/triage run in AgentGateway MCP + pod-side LangServer.
...
const APP = "gmail";
const NSID_PREFIX = "ai.gftd.apps.gmail.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:gmail.gftd.ai", nanoid: env.APP_NANOID ?? "gm4il0x1", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "20-actors/magatama/py/src/pymagatama/ingest/gmail.py", bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/gmail" });
