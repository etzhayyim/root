...
#   3. mac Keychain に gftd.m365 credentials 設定済み (root CLAUDE.md 参照)
#
# Phase 1 (did:gftd bootstrap, 3 種) → Phase 3 (lawyer records, 3 種)
# Phase 4 (Teams sync) は sync_teams_channel.mjs を別途実行。
# Phase 5 (markdown) は kagoshima-univ.md を Phase 3 結果で手動更新。
#
# 実行順:
#   bash 60-apps/ai-gftd-project-lawyer/bootstrap.sh phase1  # mint 3 DIDs
#   bash 60-apps/ai-gftd-project-lawyer/bootstrap.sh phase3  # write 3 records
#
# 出力: ~/.local/gftd/bootstrap/lawyer-$(date +%Y%m%d).json (DID + record URI の台帳)

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT_DIR="${HOME}/.local/gftd/bootstrap"
...
require_gftd() { command -v gftd >/dev/null 2>&1 || die "gftd CLI not found"; }
require_jq()   { command -v jq >/dev/null 2>&1 || die "jq not found (brew install jq)"; }
require_auth() {
  gftd auth whoami >/dev/null 2>&1 || die "run 'gftd auth login' first"
}
save_kv() {
...
phase1() {
  ...
  etzhayyim_resp=$(gftd xrpc com.etzhayyim.auth.createGuestAccount \
  ...
  local lawyer_resp
  lawyer_resp=$(gftd xrpc com.etzhayyim.auth.createGuestAccount \
  ...
  save_kv "lawyer_did_gftd" "${lawyer_did}"
  save_kv "lawyer_bootstrap_username" "${lawyer_user}"
  security add-generic-password -s "gftd.lawyer.bootstrap" -a "${lawyer_user}" -w "${lawyer_pw}" -U 2>/dev/null || true
  echo "  ✓ ${lawyer_did}"
  ...
  # gftd CLI にはセッション切替 API がないため、直接 XRPC Bearer で呼ぶ
  local lawyer_access
  ...
  save_kv "bakshi_did_gftd" "${bakshi_did}"
