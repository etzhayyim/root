"""yatabase Lead Discovery actor — autonomous HN + GitHub + Reddit + Dev.to lead gen.

Runs every 6h via APScheduler (registered in server.py).
Pipeline: search_hn → search_github → search_reddit → search_devto → filter_new → ingest → report

Sources:
  - HN Algolia: stories mentioning graph DB keywords (past 25h window)
  - GitHub Search: users who recently starred neo4j/dgraph/memgraph repos
  - Reddit (public JSON): posts in r/LocalLLaMA, r/MachineLearning, r/programming
  - Dev.to public API: articles tagged graphdatabase, knowledgegraph, mcp

Output: new rows in vertex_lead (source='discovery'), summary in outbox.
...
_DEVTO_TAGS = ["graphdatabase", "knowledgegraph", "neo4j", "mcp"]
_MAX_PER_SOURCE = 30
    ...
    for kw in _HN_KEYWORDS:
        try:
            ...
            url = (
                ...
                f"?query={q}&tags=story&numericFilters=created_at_i>{since_ts}&hitsPerPage=10"
            )
            data = _http_get(url)
            for hit in (data.get("hits") or [])[:5]:
                ...
                leads.append({
                    ...
                    "contact_email": "",
                    "company": domain,
                    "signal": f"HN story score={hit.get('points',0)}: {hit.get('title','')[:120]}",
                    "tech_stack": "unknown",
                    "outreach_status": "new",
                })
        except Exception as e:
            _log.warning("[lead_discovery] HN kw=%r error: %s", kw, e)

    _log.info("[lead_discovery] HN: %d leads", len(leads))
    return {**state, "run_date": datetime.now(timezone.utc).date().isoformat(), "hn_leads": leads}
...
async def search_github(state: LeadDiscoveryState) -> LeadDiscoveryState:
    ...
    for repo in _GH_REPOS:
        try:
            url = f"https://api.github.com/repos/{repo}/stargazers?per_page=15&page=1"
            users = _http_get(url, headers)
            for user in (users or [])[:_MAX_PER_SOURCE // len(_GH_REPOS)]:
                login = user.get("login", "")
                if not login:
                    continue
                # Fetch public profile for email
                try:
            ...
            _log.warning("[lead_discovery] GitHub repo=%r error: %s", repo, e)

    _log.info("[lead_discovery] GitHub: %d leads", len(leads))
    return {**state, "gh_leads": leads}
        ...
        for kw in _REDDIT_KEYWORDS[:2]:  # limit calls: 2 kw × 3 subs = 6 requests
            try:
                ...
                for post in posts[:3]:
                    ...
                    # skip reddit self-posts and imgur/i.redd.it noise
                    if not domain or "reddit.com" in domain or "redd.it" in domain:
                ...
                _log.warning("[lead_discovery] Reddit sub=%r kw=%r error: %s", sub, kw, e)

    _log.info("[lead_discovery] Reddit: %d leads", len(leads))
...
async def search_devto(state: LeadDiscoveryState) -> LeadDiscoveryState:
    ...
    for tag in _DEVTO_TAGS:
        try:
            ...
            _log.warning("[lead_discovery] Dev.to tag=%r error: %s", tag, e)

    _log.info("[lead_discovery] Dev.to: %d leads", len(leads))
...
async def filter_new(state: LeadDiscoveryState) -> LeadDiscoveryState:
    ...
    # Cross-check DB: any existing vertex_id?
    if deduped:
        try:
            ids = [l["vertex_id"] for l in deduped]
            placeholders = ", ".join(f"${i+1}" for i in range(len(ids)))
            rows = await fetch(
                f"SELECT vertex_id FROM vertex_lead WHERE vertex_id IN ({placeholders}) LIMIT {len(ids)}",
                *ids,
            )
            existing = {r["vertex_id"] for r in rows}
            deduped = [l for l in deduped if l["vertex_id"] not in existing]
        except Exception as e:
            _log.warning("[lead_discovery] filter DB check failed: %s", e)

    _log.info("[lead_discovery] new leads after filter: %d", len(deduped))
    ...
    for lead in new_leads:
        try:
            await execute(
                ...
                """,
                lead["vertex_id"],
                "did:plc:yatabase-lead-discovery",
            ...
            _log.warning("[lead_discovery] insert failed: %s", e)
    _log.info("[lead_discovery] ingested %d new leads", ingested)
...
async def report(state: LeadDiscoveryState) -> LeadDiscoveryState:
    ...
    summary = (
        f"[{run_date}] lead_discovery: HN={hn} GitHub={gh} Reddit={reddit} Dev.to={devto}"
    ...
    if ingested > 0:
        try:
            ...
            _log.warning("[lead_discovery] report insert failed: %s", e)
    _log.info("[lead_discovery] %s", summary)
...
def _build() -> Any:
    ...
    sg.add_edge(START, "search_hn")
    sg.add_edge("search_hn", "search_github")
    sg.add_edge("search_github", "search_reddit")
