// web4.etzhayyim.ai — Decentralized Web4 Inference & Expert Network
// Thin-edge dispatcher: business logic in AgentGateway MCP + pod-side LangServer.
// 8 methods: registerExpert / listExperts / submitInference / getInferenceResult /
//            listJobs / getJobStatus / getClusterStats / updateExpert
...
const NSID_PREFIX = "com.etzhayyim.apps.web4.";
const ACTOR_DID = "did:web:web4.etzhayyim.ai";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "874frhoc",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "60-apps/etzhayyim-project-web4/bpmn",
        methods: [
          "registerExpert", "listExperts", "submitInference", "getInferenceResult",
          "listJobs", "getJobStatus", "getClusterStats", "updateExpert",
        ],
      });
    }
...
async function proxyToDispatcher(env: Env, nsid: string, body: Record<string, unknown>): Promise<Response> {
  const dispatcherUrl = env.DISPATCHER_URL ?? "https://dispatcher.etzhayyim.ai";
