> **DEPRECATED**: Actor migrated to `20-actors/i18n/actor-manifest.jsonld` (T1 MCP-Compose). This project wasm/*/src/app.ts is retained as T3 fallback only.
...
|---|---|
| **nanoid** | `251b9zaw` |
| **domain** | `i18n.etzhayyim.com` |
| **performer_id** | `251b9zaw` |
...
|---|---|---|
| `ai.gftd.command.i18n.translate` | `translate_batch` | プロジェクト+言語バッチ翻訳 |
| `ai.gftd.command.i18n.sync` | `sync_all` | 全プロジェクトスキャン→変更検出→翻訳 |
| `ai.gftd.command.i18n.translate_message` | `translate_message` | AT channel メッセージ翻訳 |
...
| Lexicon | Description |
|---|---|
| `ai.gftd.i18n.translation_completed` | バッチ翻訳完了イベント |
| `ai.gftd.i18n.translated_message` | メッセージ翻訳結果 (record_uri, channel_id, source/target_lang, translated) |
| `ai.gftd.apps.i18n.translatedPost` | yoro 投稿の自動翻訳結果 (source_uri, source/target_lang, model) |
...
User reads AT channel message (foreign language)
  → Widget UI shows "Translate" button
    ...
    → publish ai.gftd.i18n.translated_message AT record
...
yoro post create → PDS commit → i18n subscribeRepos (Follow-based)
  ...
  → ai.gftd.apps.i18n.translatedPost record × N languages
  → social announce (AppBskyFeedPost)
```

**Output record** (`ai.gftd.apps.i18n.translatedPost`):
