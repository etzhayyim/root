"""Sheets persistence stores.

``SheetStore`` is the narrow interface the handlers use. Two implementations:

- ``KotobaSheetStore`` — production, backed by kotoba datomic (graph
  ``sheets-v1``) via :class:`lg_calendar.kotoba_datomic.KotobaDatomic`.
- ``FakeSheetStore`` — in-memory dict, used by the unit/smoke tests so the
...
class SheetStore(Protocol):
    async def get_book_attrs(self, slug: str) -> dict[str, Any] | None: ...
    async def all_book_attrs(self) -> list[dict[str, Any]]: ...
...
class KotobaSheetStore:
    ...
    async def all_book_attrs(self) -> list[dict[str, Any]]:
        rows = await self._dm.q('[:find ?slug :where [?e :sheet/type "Spreadsheet"] [?e :sheet/slug ?slug]]')
        out: list[dict[str, Any]] = []
        for row in rows:
            if not row:
                continue
            attrs = await self.get_book_attrs(str(row[0]))
            if attrs:
                out.append(attrs)
        return out
    ...
    async def lookup_slug(self, attr: str, value: Any) -> str | None:
        ...
        query = f"[:find ?slug :where [?e :{bare} {encode(value)}] [?e :sheet/slug ?slug]]"
