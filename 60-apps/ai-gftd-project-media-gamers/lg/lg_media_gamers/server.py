"""OSS FastAPI server for lg-media-gamers (mirrors lg-animeka pattern).
...
tunnel layer, same pattern as lg-animeka).

media-gamers NSID namespace: 4 graphs.
"""
...
from lg_media_gamers.cron import start_cron, stop_cron
from lg_media_gamers.graphs.health import GRAPH as HEALTH
from lg_media_gamers.graphs.ingest_charts import GRAPH as INGEST_CHARTS
from lg_media_gamers.graphs.guide_generator import GRAPH as GUIDE_GENERATOR
from lg_media_gamers.graphs.autopilot import GRAPH as AUTOPILOT
from lg_media_gamers.graphs.pokopia_research import GRAPH as POKOPIA_RESEARCH

_log = logging.getLogger(__name__)

GRAPHS: dict[str, Any] = {
    "health":            HEALTH,
    "ingest_charts":     INGEST_CHARTS,
    "guide_generator":   GUIDE_GENERATOR,
    "autopilot":         AUTOPILOT,
    "pokopia_research":  POKOPIA_RESEARCH,
}
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    _log.info("lg-media-gamers server up: graphs=%s crons=%s",
...
app = FastAPI(title="lg-media-gamers OSS server", version="0.1.0", lifespan=_lifespan)
...
_NSID_TO_ASSISTANT: dict[str, str] = {
    "ai.gftd.apps.media_gamers.health":           "health",
    "ai.gftd.apps.media_gamers.ingestCharts":     "ingest_charts",
    "ai.gftd.apps.media_gamers.generateGuide":    "guide_generator",
    "ai.gftd.apps.media_gamers.autopilot":        "autopilot",
    "ai.gftd.apps.media_gamers.researchPokopia":  "pokopia_research",
}
...
async def xrpc_compat(nsid: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": f"lg-media-gamers {type(exc).__name__}",
