// vpn.gftd.ai — CF Worker portal (ADR-2605252200)
//
// L1 Edge: CF Worker proxies XRPC → vpn-provisioner pod (L8, Vultr VKE SJC)
// Auth: gftd_session JWT cookie or Bearer JWT → resolves caller DID
...
//   AUTHN_SERVICE_URL    — https://auth.gftd.ai
...
app.get("/_app/meta", (c) =>
  c.json({
    app: "ai-gftd-project-vpn",
    did: c.env.VPN_ACTOR_DID ?? "did:web:vpn.gftd.ai",
...
app.get("/.well-known/did.json", (c) => {
  const did = c.env.VPN_ACTOR_DID ?? "did:web:vpn.gftd.ai";
  return c.json({
    "@context": ["https://www.w3.org/ns/did/v1"],
    id: did,
    service: [
      { id: `${did}#xrpc`, type: "AtprotoPersonalDataServer", serviceEndpoint: "https://vpn.gftd.ai" },
    ],
  });
});
...
async function resolveViewer(c: any): Promise<Viewer | null> { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  if (!hasJwtBearer && !/(?:^|;\s*)gftd_session=/.test(cookie)) return null;
  ...
  const authnUrl = env.AUTHN_SERVICE_URL ?? "https://auth.gftd.ai";
  try {
