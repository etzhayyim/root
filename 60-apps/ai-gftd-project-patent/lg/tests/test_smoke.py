...
_EXPECTED_NSID_MAP = {
    "ai.gftd.apps.patent.blobConvert":       "blob_convert",
    "ai.gftd.apps.patent.ingestUsptoWeekly": "ingest_uspto_weekly",
}
...
def test_xrpc_unknown_nsid(client) -> None:
    r = client.post("/xrpc/ai.gftd.apps.patent.unknownMethod", json={})
    assert r.status_code == 404
...
def test_xrpc_blob_convert_dispatches(client) -> None:
    ...
    with patch.object(
        ...
        r = client.post("/xrpc/ai.gftd.apps.patent.blobConvert", json={"limit": 5})
