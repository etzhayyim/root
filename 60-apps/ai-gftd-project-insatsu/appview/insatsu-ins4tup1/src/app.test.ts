...
describe("insatsu command registration", () => {
  it("registers expected commands", () => {
    ...
    expect(nsids).toContain("ai.gftd.apps.insatsu.printPartner.registerPrintPartner");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printPartner.getPrintPartner");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printPartner.listPrintPartners");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printMailJob.quotePrintMailJob");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printMailJob.createPrintMailJob");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printMailJob.getPrintMailJob");
    expect(nsids).toContain("ai.gftd.apps.insatsu.printMailJob.listPrintMailJobs");
  });
});
...
describe("print partners", () => {
  ...
  it("lists seeded APAC partners", async () => {
    const handler = getHandler("ai.gftd.apps.insatsu.printPartner.listPrintPartners")!;
  ...
  it("registers a dynamic partner", async () => {
    const handler = getHandler("ai.gftd.apps.insatsu.printPartner.registerPrintPartner")!;
...
describe("print mail jobs", () => {
  it("quotes Japan route via yuubin handoff", async () => {
    const handler = getHandler("ai.gftd.apps.insatsu.printMailJob.quotePrintMailJob")!;
    const result = await dec(handler(null, enc({
