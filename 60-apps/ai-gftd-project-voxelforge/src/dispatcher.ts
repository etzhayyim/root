...
export async function dispatchVoxelforgeXrpc(
  ...
  const headers: Record<string, string> = {
    ...
    "x-gftd-actor-did": auth.activeDid ?? auth.did,
    "x-gftd-org-did": auth.orgDid,
    "x-gftd-trace-id": crypto.randomUUID(),
  };
  if (auth.productScope) headers["x-gftd-product-scope"] = auth.productScope;
...
function forwardedHeaders(src: Headers): Headers {
  ...
  for (const [k, v] of src.entries()) {
    ...
    if (lk === "content-type" || lk === "content-length" || lk.startsWith("x-gftd-")) {
      out.set(k, v);
    }
  }
  return out;
