...
//
// Actor 境界 (lightweight separation, chat.gftd.ai 同居のまま):
//   - actorDid  : did:web:gftd.ai:actor:shibuya   (path-based DID。PDS への live 登録は
//                 別途 worker `sdk.did.create` が必要 — 現状は識別子の宣言のみ)
//   - nsidPrefix: ai.gftd.apps.shibuya            (records/XRPC を足す際の名前空間。
//                 chat 一般 `ai.gftd.apps.chat` とは分離)
...
export const SHIBUYA: WardProfile = {
  wardCode: "13113",
  wardName: "渋谷区",
  actorDid: "did:web:gftd.ai:actor:shibuya",
  nsidPrefix: "ai.gftd.apps.shibuya",
  ...
  unofficialNotice:
    "本サービスは gftd.ai の非公式アシスタントです（AI Agent — unofficial）。渋谷区の公式サービスではなく、渋谷区とは無関係です。実際の申込は渋谷区 粗大ごみ受付センターで行ってください。",
};
