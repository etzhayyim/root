...
export async function listConversations(): Promise<Conversation[]> {
  const resp = await fetch("/api/xrpc/ai.gftd.apps.chat.listConversations", {
    credentials: "include",
  });
...
export async function getConversation(convId: string): Promise<ConversationDetail> {
  const resp = await fetch(
    `/api/xrpc/ai.gftd.apps.chat.getConversation?convId=${encodeURIComponent(convId)}`,
    { credentials: "include" },
  );
...
export async function deleteConversation(convId: string, purgeNow = false): Promise<void> {
  await fetch("/api/xrpc/ai.gftd.apps.chat.deleteConversation", {
    method: "POST",
    headers: { "content-type": "application/json" },
