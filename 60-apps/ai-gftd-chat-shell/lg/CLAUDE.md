# lg-chat — LangGraph Server for chat.gftd.ai
...
- **shibuya actor 境界 (lightweight, chat.gftd.ai 同居)**: 区固有値を ward プロファイルに集約。
  actorDid=`did:web:gftd.ai:actor:shibuya` / nsidPrefix=`ai.gftd.apps.shibuya`（chat 一般と分離・**宣言のみ、DID live登録は未実施**）。非公式ディスクレーマ表示。`WARD`/`RECEPTION_URL` 差替が多区化の seam
...
```bash
cd 60-apps/ai-gftd-chat-shell/lg
...
- CF Worker proxies `/lg/*` → `chat-agent.gftd.ai` (CF Tunnel)
...
ported from the archived `pymagatama.primitives.chat` module.
...
├── Dockerfile              # FROM python:3.11-slim, no pymagatama dep
├── lg_chat/
│   ├── __init__.py
...
ChatPanel.svelte  →  POST /lg/runs/stream
  CF Worker (chat.gftd.ai CF Worker)
    /lg/*  →  CF Tunnel chat-agent.gftd.ai (cedba8e6, remote_config v7)
...
| **Sprint 1** Docker image build + push | ✅ 2026-05-25 | `ghcr.io/gftdcojp/lg-chat:0.1.2-amd64` |
| **Sprint 1** `helm install lg-chat-pool` | ✅ 2026-05-25 | mitama-udf namespace (revision 4) |
| **Sprint 1** CF Tunnel update | ✅ 2026-05-25 | `chat-agent.gftd.ai` → `lg-chat.mitama-udf.svc:8000` (remote config v7) |
...
- **Image**: `ghcr.io/gftdcojp/lg-chat:0.1.2-amd64` (fixes: `response_model=None`, keiei-litellm URL, model name case)
- **Model name**: `gemma-4-E4B-it` — keiei-litellm model IDs are case-sensitive (`E4B` not `e4b`)
- **CF Tunnel remote config v7**: `chat-agent.gftd.ai` is first ingress rule in tunnel `cedba8e6`; cloudflared must be restarted to pick up remote config changes
- **DNS CNAME**: `chat-agent.gftd.ai` → `cedba8e6-8210-4de2-9859-ccf5f80b0cce.cfargotunnel.com` (proxied)
...
| | lg-shinshi | lg-chat |
|---|---|---|
| pymagatama dep | Yes (primitives reuse) | **No** — standalone |
