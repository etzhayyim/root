// gftd.ai apex chat shell — T3 CF Worker.
//
// Architecture (ADR-2604282300 + ADR-2604241038):
//   Browser  ──────▶  CF Worker (this file, gftd.ai/*)
//                       │
...
type Env = {
  ...
  // Service binding to ai-gftd-auth. Verifies HS256 session JWT inside the CF
...
app.get("/_app/meta", (c) =>
  c.json({
    app: "ai-gftd-chat-shell",
    did: c.env.CHAT_ACTOR_DID ?? "did:web:gftd.ai",
    layer: "chat-shell",
...
// ── DID document for did:web:gftd.ai ────────────────────────────────
// The apex agent identity. ADR-0019 / ADR-0023 : did:web spec compliant.
//
// Signing authority is delegated to authn.gftd.ai's ES256 keypair. External
// verifiers fetch the JWKS at the documented URL to validate Service Auth
// JWTs issued on behalf of did:web:gftd.ai (apex chat agent has no local
// keypair — Shannon-thin facade per ADR-2604282300).
app.get("/.well-known/did.json", (c) => {
  const did = c.env.CHAT_ACTOR_DID ?? "did:web:gftd.ai";
  const authnDid = "did:web:authn.gftd.ai";
  return c.json({
    "@context": [
    ...
    verificationMethod: [
      {
        ...
        type: "JsonWebKey2020",
        controller: authnDid,
        // Public keys live at authn.gftd.ai/.well-known/jwks.json (ES256,
        // P-256). No `publicKeyJwk` inline so rotation stays single-source.
        publicKeyJwk: {
          kty: "OKP",
          $ref: "https://authn.gftd.ai/.well-known/jwks.json",
        },
      },
    ],
    ...
    service: [
      {
        id: `${did}#chat`,
        type: "GftdChatAgent",
        serviceEndpoint: "https://gftd.ai",
      },
      {
        id: `${did}#mcp`,
        type: "ModelContextProtocol",
        serviceEndpoint: "https://gftd.ai/mcp",
      },
      {
        id: `${did}#authn`,
        type: "GftdAuthN",
        serviceEndpoint: "https://authn.gftd.ai",
      },
      {
        id: `${did}#jwks`,
        type: "JsonWebKeySet",
        serviceEndpoint: "https://authn.gftd.ai/.well-known/jwks.json",
      },
    ],
  });
});
...
async function resolveViewer(c: any): Promise<Viewer | null> {  // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const hasCookie = /(?:^|;\s*)gftd_session=/.test(cookie);
...
const CORS_ORIGIN_ALLOW = /^https:\/\/(?:[a-z0-9-]+\.)?gftd\.ai$/;
...
function resolveCorsOrigin(c: any): string {  // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  // (server-side, no cookie). Browser callers from non-gftd origins are
...
//
// Forwards https://gftd.ai/mcp to the chat-agent pod's /mcp endpoint.
// `did:web:gftd.ai` advertises this URL via its DID document service[].
// External MCP clients (Claude Desktop, Cursor, Aider, Continue) can
// add `https://gftd.ai/mcp` as an MCP server to call code_exec /
...
//
// Proxies https://gftd.ai/v1/{models,chat/completions,…} → chat-agent pod.
...
app.all("/api/xrpc/:nsid", async (c) => {
  ...
  if (!nsid.startsWith("ai.gftd.apps.chat.")) {
...
// resolved viewer DID + handle when the gftd_session cookie is valid,
// or `{ anon: true }` so the UI can render the [Sign in] button.
//
// `/api/auth/signin-url` — apex stays a thin facade. authn.gftd.ai owns
// passkey ceremonies; we just hand back the canonical sign-up URL with
// a redirectUrl back to the page the user was on.
//
// `/api/auth/signout` — clears the cross-subdomain `gftd_session` cookie.
...
app.get("/api/auth/signin-url", (c) => {
  const url = new URL(c.req.url);
  const redirectUrl = url.searchParams.get("redirectUrl") ?? "https://gftd.ai/";
  const authn = c.env.AUTHN_URL ?? "https://authn.gftd.ai";
  // sign-up = passkey-first zero-input flow per ai-gftd-project-auth/CLAUDE.md.
...
app.post("/api/auth/signout", async (c) => {
  const env = c.env as Env;
  const authn = env.AUTHN_URL ?? "https://authn.gftd.ai";
  ...
  // Clear the cross-subdomain cookie. Match authn's `Domain=.gftd.ai;
  // Path=/; Secure; SameSite=Lax` so the browser drops the same cookie
  // it accepted from authn.
  const headers = new Headers({
    "content-type": "application/json",
    "set-cookie": "gftd_session=; Domain=.gftd.ai; Path=/; Max-Age=0; Secure; SameSite=Lax",
    "access-control-allow-origin": resolveCorsOrigin(c),
