...
| **Data/Knowledge** | drive-sync + Redis KV + gitstate | `gftd:os/sync`, `wasi:keyvalue` |
| **Agent** | Agent runtime + Automaton (survival, memory, soul) | `gftd:os/agent-runtime`, `gftd:agent`, `gftd:automaton` |
| **Behavioral** | Consent UI + Directory + Natural language interface | `gftd:os/consent`, `gftd:os/directory` |
| **Governance** | Budget enforcement + Audit trail + Policy engine | `gftd:os/budget`, `gftd:os/audit`, `gftd:automaton/policy` |
...
│                                                                  │
│  ┌──────────────┐    ┌──────────────────────────────────────┐   │
│  │  Tauri Shell  │    │  OS Runtime Component (gftd-os-runtime) │
│  │  (Svelte UI)  │───▶│                                      │   │
│  │              │ gRPC│  exports:                             │   │
│  │  - Agent Dash │    │    gftd:os/agent-runtime              │   │
│  │  - Consent UI │    │    gftd:os/consent                    │   │
│  │  - Directory  │    │    gftd:os/directory                  │   │
│  │  - Budgets    │    │    gftd:os/budget                     │   │
│  │  - Audit Log  │    │    gftd:os/audit                      │   │
│  │  - Sync Status│    │    gftd:os/sync                       │   │
│  └──────────────┘    │                                      │   │
│                       │  imports:                             │   │
│                       │    gftd:agent, gftd:automaton/*       │   │
│                       │    gftd:web3/wallet                   │   │
│                       │    gftd:mesh/*, gftd:inference        │   │
│                       │    gftd:clerk/authn/authz             │   │
│                       └──────────┬───────────────────────────┘   │
│                                  │ wRPC/NATS                     │
│                       ┌──────────▼───────────────────────────┐   │
...
3. NATS JetStream publish
   Subject: gftd.os.migrate.{agent-id}
        │                                  │
        │                                  ▼
...
```
OS Runtime Component (gftd:os@0.1.0)
...
WASM Component (os-messaging-component)
  └── import gftd:native-bridge/imessage
        │
        │ wRPC link (WADM)
        ▼
...
|---|---|---|
| `gftd:native-bridge/imessage` | macOS | chat.db + AppleScript required |

NOT native (WASM-internal via WebLLM):
- Text generation: `gftd:inference/engine` (WebGPU LLM)
- Image generation: `gftd:inference/engine` (WebGPU Stable Diffusion)
...
|---|---|---|
| gftd-agent (core) | os-runtime-component | 6 OS interface exports |
| gftd-agent (chat/AI) | os-ai-component | WebLLM inference |
| gftd-agent (wellness) | os-wellbeing-component | automaton memory/soul |
| gftd-messaging | os-messaging-component | + iMessage native bridge |
| gftd-scheduler | os-scheduler-component | AI-driven scheduling |
| gftd-system (Go) | os-system-component | AV, privacy, system |
| gftd-imsg-legacy-runtime | imessage-native-provider | macOS capability provider |
| gftd-llm (Python MLX) | WebLLM (WebGPU) | WASM-internal, no native |
| gftd-agent-ops (XRPC) | os-runtime-component | Consolidated |
