...
architecture as implemented in the GFTD platform, demonstrate how Unix
...
\begin{verbatim}
/org/gftdcojp/                              # Organization root
  projects/ai-gftd-project-scheduler/       # Project (process group)
    app/                                    # Agent code
    capabilities.jsonld                     # Capability manifest
    PROJECT.jsonld                          # Process metadata
  projects/ai-gftd-project-os/             # Another project
    ui/                                     # Static interface
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Resource allocation}: How are compute credits (GCC---GFTD
...
uses \emph{GFTD Computing Credits (GCC)} as the internal unit of account:
...
\begin{itemize}[nosep]
  ...
  \item The credits system is managed by \texttt{ai-gftd-project-credits}.
\end{itemize}
...
corresponding to the GFTD platform deployment model:
...
\section{Case Study: GFTD Scheduler as AO Service}

The GFTD Scheduler (\texttt{scheduler.gftd.ai}) exemplifies AO principles:
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Governance}: Resource limits defined in
    \texttt{gftd.json}; scaling policy enforced by KEDA.
...
GFTD platform demonstrates this architecture in production with legacy runtime,
