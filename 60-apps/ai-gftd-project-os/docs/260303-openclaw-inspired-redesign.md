...
| シェル実行 | Sandboxed Runner (gftd-browserless 連携) | `os-runner-{nanoid}` |
| ブラウザ自動操作 | Browserless Playwright 連携 | `gftd-browserless` (既存) |
...
```
60-apps/ai-gftd-project-os/wasm/os-messaging-component/
...
```
60-apps/ai-gftd-project-os/wasm/ai-gftd-wasm-os-agent-<nanoid>/
...
```
60-apps/ai-gftd-project-os/wasm/ai-gftd-wasm-os-llm-<nanoid>/
├── main.go
...
```
60-apps/ai-gftd-project-os/wasm/ai-gftd-wasm-os-memory-<nanoid>/
├── main.go
...
```
60-apps/ai-gftd-project-os/wasm/ai-gftd-wasm-os-skills-<nanoid>/
...
```
60-apps/ai-gftd-project-os/wasm/ai-gftd-wasm-os-runner-<nanoid>/
├── main.go
├── shell.go        # gftd-browserless 経由のシェルコマンド実行
...
os-agent → tool_call: "run_shell" { command: "ls -la /project" }
  → os/consent.evaluate("run_shell", {command}, risk=dangerous)
    ...
    → Approved
  → os-runner → XRPC → gftd-browserless (magatama-runtime:8080)
...
| `os-runner` | 新規割当 | API-only | No | gftd-browserless |
