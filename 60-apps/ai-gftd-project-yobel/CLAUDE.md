# ai-gftd-project-yobel — Collective Debt Release Rites (etzhayyim activity)

集合的・教義的・政治的な債務免除 rite (`yobel.gftd.ai`) — שמיטה (shmita 7yr) / יובל (yobel 49yr) / 徳政令 / Catholic Jubilee / modern political amnesty。
...
| 項目 | 値 |
|---|---|
| Operating entity | **etzhayyim** (運営宗教法人。Gftd Japan vendor の関与なし) |
...
- Transitional alias (本 repo 期間): `ai.gftd.apps.yobel.*`
...
```
yobel.gftd.ai (voluntary doctrinal)
 ├─ fallback     → bankruptcy.gftd.ai  (mandatory legal procedure)
 ├─ represented  → lawfirm.gftd.ai     (creditor consent, court filing)
 ├─ eligibility  ← trust.gftd.ai       (kyu/dan filter, optional)
...
- **Voluntary opt-in only.** 強制力なし。secular creditor が無視した場合は `bankruptcy.gftd.ai` fallback
- **Religious-corp doctrinal authority のみ.** secular law を override する主張は出さない
- **Tax/regulatory warning.** debt forgiveness は jurisdiction によって課税所得 (e.g. US IRC §61(a)(11) cancellation-of-debt income)。`jurisdictionNotes` で warning を返す。税務 advice は提供しない (→ lawfirm.gftd.ai に delegate)
