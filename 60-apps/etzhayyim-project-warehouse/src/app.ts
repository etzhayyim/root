// warehouse.etzhayyim.com — WMS layer
// Thin-edge dispatcher: business logic in AgentGateway MCP + pod-side LangServer.
// 4 methods: registerSku / putaway / pick / getInventory
...
const NSID_PREFIX = "com.etzhayyim.apps.warehouse.";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "w4r3h0u5",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/warehouse",
        methods: ["registerSku", "putaway", "pick", "getInventory"],
      });
    }
