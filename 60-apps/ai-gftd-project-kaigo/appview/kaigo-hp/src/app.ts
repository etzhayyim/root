import { createWorkerExport, type HostSDK } from '@gftd/magatama-host-sdk';
...
export default createWorkerExport((sdk: HostSDK) => {
  ...
  sdk.app.query('ai.gftd.apps.kaigo.getProduct', async (_input, _ctx) => {
    return {
      name: 'Giemon Kaigo',
      version: '1.0.0',
  ...
  sdk.app.query('ai.gftd.apps.kaigo.calcHousingReformBenefit', async (input, _ctx) => {
  ...
  sdk.app.query('ai.gftd.apps.kaigo.estimateCareCost', async (input, _ctx) => {
