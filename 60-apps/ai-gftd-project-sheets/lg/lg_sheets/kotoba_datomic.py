...
Wraps ``ai.gftd.apps.kotoba.datomic.{transact,q,pull}`` — the canonical
write/read surface for calendar events (graph ``sheets-v1``). Adapted from
...
dedicated ``sheets-v1`` graph JWT-only (no CACAO) per ADR-2605302130 Option 3.
"""
...
_GRAPH_LABEL = os.environ.get("KOTOBA_GRAPH", "sheets-v1")
