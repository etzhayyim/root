# ai-gftd-project-sheets — Google Sheets v4 + Microsoft Graph (workbook) compatible spreadsheet API

**sheets.gftd.ai** — a spreadsheet API whose external surface is shaped like **both**
...
```
Google SDK  → sheets.gftd.ai/v4/spreadsheets/{id}/values/{range}                  ┐
MS Graph SDK→ sheets.gftd.ai/v1.0/me/drive/items/{id}/workbook/worksheets/{s}/range ┘
  CF Worker sheets-compat (50-infra/cloudflare/workers/sheets-compat)
   ├─ /xrpc/ai.gftd.apps.sheets.*  → pod (actor-worker passthrough)
   └─ provider REST → canonical XRPC → pod → reshape to provider JSON
  lg-sheets pod (60-apps/ai-gftd-project-sheets/lg, FastAPI server.py)
...
catch-all actor:true → pipethroughActorWorker → sheets.gftd.ai/xrpc → `SHEETS_POD_URL`
tunnel → pod; dedicated graph `sheets-v1`).

## Canonical XRPC methods (ai.gftd.apps.sheets.*)
...
Lexicons: `00-contracts/lexicons/ai/gftd/apps/sheets/`. **Storage model (ADR D6)**: one
...
## Test
```bash
cd 60-apps/ai-gftd-project-sheets/lg && python3 -m pytest tests/ -q
