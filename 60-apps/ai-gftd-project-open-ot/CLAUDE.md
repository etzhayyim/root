...
| Hardware reference spec (Mimi / Te / Atama) | `60-apps/ai-gftd-project-open-ot/cad-spec/` |
| NixOS module spec for Atama edge controller | `60-apps/ai-gftd-project-open-ot/nixos/atama/` |
| Lexicon × manifest CI validator | `70-tools/scripts/open-ot/validate-cell-abi.py` (12 tests) |
| Microgrid prototype scope | `60-apps/ai-gftd-project-open-ot/PROTOTYPE-MICROGRID.md` |
...
| Logic Tier 1 (greenfield) | **Rust FB API, IEC 61499 FBType-compatible** → `wasm32-wasi` | typed FB, no GC, gftd-native |
...
| Control plane | **gftd XRPC + MCP** → atproto records | config, lineage, audit, capability grants |
...
3. **First prototype vertical** — **community microgrid** (100 kW–10 MW) (ADR §R3). Cross-link with `ai-gftd-project-open-denki` for CIM config SSoT; open-ot adds control verbs. Building HVAC and water utility deferred to MVP+1 / MVP+2.
