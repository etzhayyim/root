...
| Item | Status |
|---|---|
| Build → sign → pin | Specced in SPEC §9: cargo build → `wamrc` AOT → builder DID Ed25519 sign → CID via `b3sum` → atproto record via `ai.gftd.apps.openOt.pinModule` |
...
| SL-2 gap (FR 2 role-based use control) requires SBT redesign | medium | low | SBT ↔ role binding already in `ai.gftd.apps.openOt` Lexicon set; gap is plumbing |
