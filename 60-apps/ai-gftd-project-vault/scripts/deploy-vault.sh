...
#   - ai-gftd-auth Worker already deployed (AUTH_SERVICE binding target).
#
# Usage:
#   cd 60-apps/ai-gftd-project-vault
...
say "Step 1/4: ensure D1 database 'ai-gftd-vault' exists"
D1_ID=$(wrangler d1 list --json 2>/dev/null \
  | jq -r '.[] | select(.name == "ai-gftd-vault") | .uuid // empty' | head -n1)

if [[ -z "$D1_ID" ]]; then
  say "  creating ai-gftd-vault ..."
  CREATE_OUT=$(wrangler d1 create ai-gftd-vault 2>&1)
...
( cd "$WORKER_DIR" && wrangler d1 migrations apply ai-gftd-vault --remote )
