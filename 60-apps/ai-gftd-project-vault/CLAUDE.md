...
- `memberDeviceKey` = HKDF(WebAuthn PRF) **client-side** (browser passkey) or macOS Keychain (gftd CLI)
...
- For programmatic agents (Claude Code), use `gftd agent-token --lxm com.etzhayyim.vault.getItem --ttl 60` to mint scoped Service Auth JWT
...
wrangler d1 create ai-gftd-vault              # capture id → wrangler.jsonc
wrangler d1 migrations apply ai-gftd-vault    # applies migrations/0001_init.sql

cd 60-apps/ai-gftd-project-vault/worker
