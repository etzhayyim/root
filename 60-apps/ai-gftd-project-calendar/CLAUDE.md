# ai-gftd-project-calendar — Google Calendar v3 + Microsoft Graph compatible calendar API

**calendar.gftd.ai** — a calendar API whose external surface is byte-shaped like **both**
...
```
Google SDK  → calendar.gftd.ai/calendar/v3/calendars/{cid}/events  ┐
MS Graph SDK→ calendar.gftd.ai/v1.0/me/events                       ┘ (two external skins)
  CF Worker calendar-compat (50-infra/cloudflare/workers/calendar-compat)
   ├─ /xrpc/ai.gftd.apps.calendar.*  → pod (actor-worker passthrough)
   └─ provider REST → canonical XRPC → pod → reshape to provider JSON
  lg-calendar pod (60-apps/ai-gftd-project-calendar/lg, FastAPI server.py)
...
- **Internal routing**: `atproto.gftd.ai/xrpc/ai.gftd.apps.calendar.*` → catch-all `actor:true`
  (no BPMN binding) → `pipethroughActorWorker` → `calendar.gftd.ai/xrpc/...` (this worker) →
...
## Canonical XRPC methods (ai.gftd.apps.calendar.*)
...
Lexicons: `00-contracts/lexicons/ai/gftd/apps/calendar/` (`defs.json` = shared event/attendee/
...
```
lg/
├── Dockerfile                  # ghcr.io/gftdcojp/lg-calendar (remote BuildKit gftd-vke)
├── pyproject.toml
├── langgraph.json
├── lg_calendar/
│   ├── server.py               # FastAPI XRPC surface (/xrpc/ai.gftd.apps.calendar.*)
...
cd 60-apps/ai-gftd-project-calendar/lg && python3 -m pytest tests/ -q
# compat skins (mappers + route integration)
cd 50-infra/cloudflare/workers/calendar-compat && node --test test/*.test.ts
# build + deploy (infra)
cd 60-apps/ai-gftd-project-calendar/lg && docker buildx build --builder gftd-vke --platform linux/amd64 -t ghcr.io/gftdcojp/lg-calendar:0.1.0-amd64 --push .
