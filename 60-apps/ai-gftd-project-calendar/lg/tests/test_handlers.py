...
async def test_create_get_roundtrip(store: FakeCalendarStore) -> None:
    ...
    assert res["did"].startswith("did:web:calendar.gftd.ai:event:")
    assert res["iCalUid"].endswith("@calendar.gftd.ai")
...
async def test_rsvp_updates_attendee(store: FakeCalendarStore) -> None:
    res = await handlers.create_event(store, {
        "summary": "party",
        "startsAt": "2026-06-02T09:00:00Z",
        "attendees": [{"did": "did:web:alice.gftd.ai", "responseStatus": "needsAction"}],
    })
    slug = res["event"]["uri"].split("/")[-1]
    r = await handlers.rsvp(store, {"eventId": slug, "respondentDid": "did:web:alice.gftd.ai", "response": "accepted"})
