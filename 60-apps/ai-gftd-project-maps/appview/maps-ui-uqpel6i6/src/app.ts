...
import { sql } from "@gftd/magatama-host-sdk";
...
function normalizeCollectionName(collection: string): string {
  ...
  return `ai.gftd.apps.maps.${collection}`;
}
...
async function upsertMapsSocialProfileDirect(
  ...
  const row = {
    vertex_id: `at://${did}/ai.gftd.apps.maps.socialProfile/self`,
    did,
 ...
 * Emits `ai.gftd.apps.maps.building` records carrying `geometry` (GeoJSON
...
async function cmdPlaceReverseGeocode(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.reverseGeocode", payload);
...
async function cmdWeatherAt(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.weatherAt", payload);
  if (req.lat == null || req.lng == null) return { error: "lat and lng required" };
...
async function cmdWeatherGrid(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.weatherGrid", payload);
...
async function cmdIpGeolocate(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.ipGeolocate", payload);
...
async function cmdActorLocations(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.actorLocations", normalizeQueryPayload(payload, ["limit"]));
...
/**
 * ai.gftd.apps.maps.nextDeparturesAtStop — Phase 2 (bus + train timetable).
 *
...
/**
 * ai.gftd.apps.maps.realtimeDelaysAtStop — Phase 3 (gated, RT layered on Phase 2).
 *
...
// ─── ai.gftd.apps.maps.{get,list,bake}GsplatAsset ──────────────────────────
//
  ...
  try {
    ...
    if (zeebe && typeof zeebe.publishMessage === "function") {
      await zeebe.publishMessage({
        name: "ai.gftd.apps.maps.trainGsplatFromMapillary",
...
async function cmdBakeGsplatAsset(sdk: HostSDK, body: ArrayBuffer | Uint8Array | string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    if (zeebe && typeof zeebe.publishMessage === "function") {
      await zeebe.publishMessage({
        name: "ai.gftd.apps.maps.bakeGsplatAsset",
        correlationKey: tileH3,
...
async function cmdDeviceBind(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.deviceBind", payload);
...
async function cmdTwinStateUpdate(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.twinStateUpdate", payload);
...
async function cmdTwinScene(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.twinScene", payload);
  if (req.lat == null || req.lng == null) return { error: "lat and lng required" };
...
async function cmdOccupancyUpdate(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.occupancyUpdate", payload);
...
async function cmdSensorIngest(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.sensorIngest", payload);
...
async function cmdSensorQuery(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.sensorQuery", payload);
...
async function cmdSensorLatest(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.sensorLatest", payload);
  if (!req.sensorId) return { error: "sensorId required" };
...
async function cmdSensorAlertSet(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.sensorAlertSet", payload);
...
async function cmdSimulationCreate(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.simulationCreate", payload);
...
async function cmdSimulationRun(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.simulationRun", payload);
...
async function cmdForecastGet(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.forecastGet", payload);
...
async function cmdHealthAssess(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.healthAssess", payload);
  if (!req.entityId) return { error: "entityId required" };
...
async function cmdMaintenancePlan(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.maintenancePlan", payload);
  if (!req.entityId) return { error: "entityId required" };
...
async function cmdWorldBeliefUpdate(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.worldBeliefUpdate", payload);
...
async function cmdWorldBeliefGet(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.worldBeliefGet", payload);
  if (!req.entityId) return { error: "entityId required" };
...
async function cmdLatentWorldModelRun(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.latentWorldModelRun", payload);
  const modelVersion = str(req.modelVersion) || "maps-blwm-v1";
...
async function cmdSpatialEventRecord(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialEventRecord", payload);
...
async function cmdSpatialEventQuery(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialEventQuery", payload);
...
async function cmdSpatialVersionQuery(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialVersionQuery", payload);
  if (!req.entityId) return { error: "entityId required" };
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdSpatialRelationWrite(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialRelationWrite", payload);
...
async function cmdSpatialRelationQuery(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialRelationQuery", payload);
  if (!req.entityId) return { error: "entityId required" };
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdTimeline(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.timeline", payload);
  if (!req.entityId) return { error: "entityId required" };
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdSpatialDiff(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.spatialDiff", payload);
  if (!req.entityId) return { error: "entityId required" };
...
async function cmdDisplayLayerDefine(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.displayLayerDefine", payload);
  if (!req.name) return { error: "name required" };
...
async function cmdListPostLocations(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.listPostLocations", payload);
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdMapralyImportPoi(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.mapralyImportPoi", payload);
...
async function cmdMapralyListPois(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.mapralyListPois", payload);
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdListVisionResults(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.listVisionResults", payload);
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdSatelliteIngest(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.satelliteIngest", payload);
...
async function cmdSatelliteImportScene(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.satelliteImportScene", payload);
...
async function cmdSatelliteAnalyze(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.satelliteAnalyze", payload);
...
async function cmdListSatelliteScenes(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.listSatelliteScenes", payload);
  const limit = Math.min(req.limit ?? 50, 100);
...
async function cmdGetDashboard(sdk: HostSDK, _payload: Uint8Array): Promise<unknown> {
  const pod = await callMapsLangserverRead("ai.gftd.apps.maps.getDashboard", _payload);
  if (pod) return pod;
...
async function cmdSeedSeismicFeed(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.maps.seedSeismicFeed", payload);
  const feedMap = {
...
async function cmdSeedWorldAdminAreas(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.maps.seedWorldAdminAreas", payload);
...
async function cmdSeedAdsb(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.maps.seedAdsb", payload);
...
async function cmdListLiveAircraft(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const pod = await callMapsLangserverRead("ai.gftd.apps.maps.listLiveAircraft", payload);
  if (pod) return pod;

  const req = parseLexiconInput("ai.gftd.apps.maps.listLiveAircraft", normalizeQueryPayload(payload, ["minLat", "maxLat", "minLon", "maxLon", "maxAgeSec", "limit"]));
...
async function cmdListLiveSatellites(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const pod = await callMapsLangserverRead("ai.gftd.apps.maps.listLiveSatellites", payload);
  if (pod) return pod;

  const req = parseLexiconInput("ai.gftd.apps.maps.listLiveSatellites", normalizeQueryPayload(payload, ["limit"]));
...
async function cmdSatellitePassQuery(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.satellitePassQuery", payload);
...
async function cmdAircraftTrack(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.aircraftTrack", payload);
...
async function cmdListCelestialObjects(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.listCelestialObjects", normalizeQueryPayload(payload, ["magMax", "limit"]));
...
async function cmdListWebCrawlGeoEntities(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.maps.listWebCrawlGeoEntities", payload);
...
export async function handleComAtprotoSyncSubscribeReposCommit(sdk: HostSDK, commit: ComAtprotoSyncSubscribeReposCommit): Promise<{ ok: true; detail: string }> {
  ...
  if (commit.collection.startsWith("ai.gftd.apps.maps.")) {
    // Murakumo LLM vision callback → parse entities → write + post per entity
    if (commit.collection === "ai.gftd.apps.maps.satelliteAnalysisResult") {
      try {
    ...
    const kind = commit.collection.replace("ai.gftd.apps.maps.", "");
  ...
  if (commit.collection === "ai.gftd.apps.ipaddress.ipGeo") {
    try {
      ...
      sdk.pds.dispatch({
        ...
        payload: {
          ...
          params: JSON.stringify({
            ...
            responseFormat: "json",
            callbackCollection: "ai.gftd.apps.maps.satelliteAnalysisResult",
...
async function cmdAismarineQueryVesselsBbox(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.aismarine.queryVesselsBbox", payload);
...
async function cmdAismarineGetVesselDetail(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.aismarine.getVesselDetail", payload);
...
async function cmdAismarineSearchVessels(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.aismarine.searchVessels", payload);
...
async function cmdAismarineGetVesselDensityTile(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.aismarine.getVesselDensityTile", payload);
  const parsed = _parseBbox4(req.bbox);
...
async function cmdGraphSearchNodes(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  ...
  for (const r of places) {
    ...
    nodes.push({
      ...
      sourceUrl: null, types: [r.label ?? "Place"], nsPrefix: "ai.gftd.apps.maps.spatial",
    });
  }
  for (const v of vessels) {
    ...
    nodes.push({
      ...
      sourceUrl: null, types: [v.type_class ?? "unknown"], nsPrefix: "ai.gftd.apps.maps.aismarine",
    });
  }
  for (const e of entities) {
    nodes.push({
      ...
      types: [e.entity_type ?? "LegalEntity"], nsPrefix: "ai.gftd.apps.legal_entity",
    });
  }
  for (const p of ports) {
    nodes.push({
      ...
      sourceUrl: null, types: ["Port", p.un_locode], nsPrefix: "ai.gftd.apps.maps.openPorts",
    });
  }
...
async function cmdSearchSemanticNodes(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.apps.maps.searchSemanticNodes" as any, payload);
...
const _innerExport = createWorkerExport((sdk) => {
  ...
  sdk.app.onHeartbeat(async (cadence) => {
    ...
    // /xrpc/ai.gftd.apps.maps.batchCoverageCycle guaranteed every 2 min —
  ...
  const a = sdk.app;
    // Spatial Intelligence
    a.command(nsid("ai.gftd.apps.maps.runtimeConfig"), (_, body) => cmdRuntimeConfig(sdk, body), asAgentTool("Get maps runtime configuration"), withCapabilityTags("config", "query"))
      .command(nsid("ai.gftd.apps.maps.kamiConfig"), (_, body) => cmdKamiConfig(sdk, body), asAgentTool("Get KAMI runtime configuration"), withCapabilityTags("config", "query"))
      .command(nsid("ai.gftd.apps.maps.tileGeoJson"), (_, body) => cmdTileGeoJson(sdk, body), asAgentTool("RisingWave-native vector tile: per-label GeoJSON for bbox"), withCapabilityTags("map", "vector", "geojson"))
      .command(nsid("ai.gftd.apps.maps.tileXyz"), (_, body) => cmdTileXyz(sdk, body), asAgentTool("Slippy-tile (z/x/y) vector endpoint — stable tile URL, lod-simplified features"), withCapabilityTags("map", "vector", "xyz", "tile"))
      .command(nsid("ai.gftd.apps.maps.getChunk"), (_, body) => cmdGetChunk(sdk, body), asAgentTool("Forward-topology H3 chunk reader: per-cell per-label GeoJSON (replaces XYZ pyramid)"), withCapabilityTags("map", "chunk", "h3", "forward-topology"))
      .command(nsid("ai.gftd.apps.maps.getChunkModels"), (_, body) => cmdGetChunkModels(sdk, body), asAgentTool("DB-driven 3D model instances: buildings (AABB), vegetation (TaxonomicProfile), atoms for H3 tiles"), withCapabilityTags("map", "model", "3d", "science", "h3"))
      .command(nsid("ai.gftd.apps.maps.seedBuildings"), (_, body) => cmdSeedBuildings(sdk, body), asAgentTool("Seed building polygons + heights from OSM Overpass for a bbox"), withCapabilityTags("seed", "building", "overpass"))
      .command(nsid("ai.gftd.apps.maps.reverseGeocode"), (_, body) => cmdPlaceReverseGeocode(sdk, body), asAgentTool("Reverse geocode lat/lng"), withCapabilityTags("place", "geocode"))
      .command(nsid("ai.gftd.apps.maps.weatherAt"), (_, body) => cmdWeatherAt(sdk, body), asAgentTool("Weather at location"), withCapabilityTags("weather", "query"))
      .command(nsid("ai.gftd.apps.maps.weatherGrid"), (_, body) => cmdWeatherGrid(sdk, body), asAgentTool("Weather grid query"), withCapabilityTags("weather", "query"))
      .command(nsid("ai.gftd.apps.maps.ipGeolocate"), (_, body) => cmdIpGeolocate(sdk, body), asAgentTool("IP geolocation lookup"), withCapabilityTags("ip", "query"));
    // Transport Intelligence
    a.command(nsid("ai.gftd.apps.maps.nextDeparturesAtStop"), (_, body) => cmdNextDeparturesAtStop(sdk, body), asAgentTool("Next scheduled departures at a Station / BusStop (GTFS-JP timetable, no realtime)"), withCapabilityTags("transit", "schedule", "query", "gtfs"))
      .command(nsid("ai.gftd.apps.maps.realtimeDelaysAtStop"), (_, body) => cmdRealtimeDelaysAtStop(sdk, body), asAgentTool("Next departures at a stop with GTFS-RT delays + active alerts (degrades to static when RT pipeline offline)"), withCapabilityTags("transit", "schedule", "query", "gtfs", "realtime"))
      .command(nsid("ai.gftd.apps.maps.crawlFlightPrices" as any), (_, body) => cmdCrawlFlightPrices(sdk, body), asAgentTool("Queue flight fare crawler (Skyscanner-like)"), withCapabilityTags("transport", "crawler", "flight", "price", "write"));
    // Gsplat preview / QC (ADR-2605092800)
    a.query(nsid("ai.gftd.apps.maps.getGsplatAsset"), (_, body) => cmdGetGsplatAsset(sdk, body), asAgentTool("Resolve a 3D Gaussian Splat preview asset by tile (H3) or vertex_id"), withCapabilityTags("gsplat", "preview", "qc", "query"))
      .query(nsid("ai.gftd.apps.maps.listGsplatAssets"), (_, body) => cmdListGsplatAssets(sdk, body), asAgentTool("List 3D Gaussian Splat preview assets (filter by tile / source_did)"), withCapabilityTags("gsplat", "preview", "list", "query"))
      .command(nsid("ai.gftd.apps.maps.bakeGsplatAsset"), (_, body) => cmdBakeGsplatAsset(sdk, body), asAgentTool("Enqueue a splat→mesh bake job for a tile (delegates to L8 k8s pod)"), withCapabilityTags("gsplat", "bake", "mesh", "write"))
      .command(nsid("ai.gftd.apps.maps.trainGsplatFromMapillary"), (_, body) => cmdTrainGsplatFromMapillary(sdk, body), asAgentTool("Train a 3D Gaussian Splat at lat/lng from Mapillary imagery (COLMAP + gsplat on RunPod L40S)"), withCapabilityTags("gsplat", "train", "mapillary", "colmap", "write"))
      .query(nsid("ai.gftd.apps.maps.getGsplatJobStatus"), (_, body) => cmdGetGsplatJobStatus(sdk, body), asAgentTool("Latest state of a single gsplat train / bake job by jobId"), withCapabilityTags("gsplat", "status", "query"))
      .query(nsid("ai.gftd.apps.maps.listGsplatJobs"), (_, body) => cmdListGsplatJobs(sdk, body), asAgentTool("List the latest state of gsplat train / bake jobs (filter by tile / kind / status)"), withCapabilityTags("gsplat", "status", "list", "query"))
      .query(nsid("ai.gftd.apps.maps.getGsplatCostSummary"), (_, body) => cmdGetGsplatCostSummary(sdk, body), asAgentTool("RunPod $ spend summary across train + bake (today UTC / last 7 / last 30 days)"), withCapabilityTags("gsplat", "cost", "rollup", "query"));
    // Digital Twin
    a.command(nsid("ai.gftd.apps.maps.twinScene"), (_, body) => cmdTwinScene(sdk, body), asAgentTool("Get KAMI 3D scene for area"), withCapabilityTags("twin", "scene"))
      .command(nsid("ai.gftd.apps.maps.worldBeliefUpdate"), (_, body) => cmdWorldBeliefUpdate(sdk, body), asAgentTool("Bayesian latent world-model belief update for a spatial entity"), withCapabilityTags("world-model", "bayesian", "belief", "write"))
      .command(nsid("ai.gftd.apps.maps.worldBeliefGet"), (_, body) => cmdWorldBeliefGet(sdk, body), asAgentTool("Get Bayesian latent world-model beliefs for a spatial entity"), withCapabilityTags("world-model", "bayesian", "belief", "query"))
      .command(nsid("ai.gftd.apps.maps.latentWorldModelRun"), (_, body) => cmdLatentWorldModelRun(sdk, body), asAgentTool("Run Bayesian latent world-model inference across twin, sensor, and spatial event state"), withCapabilityTags("world-model", "bayesian", "latent", "simulation"));
    // Operations dashboard — World Monitor-style summary surface for the maps UI.
    a.query(nsid("ai.gftd.apps.maps.getDashboard"), (_, body) => cmdGetDashboard(sdk, body), asAgentTool("Get maps operations dashboard summary"), withCapabilityTags("dashboard", "intel", "query"));
    a.query(nsid("ai.gftd.apps.maps.getWorldMonitorDashboard"), (_, body) => cmdMapsPodIntelRead("ai.gftd.apps.maps.getWorldMonitorDashboard", body), asAgentTool("Get World Monitor-style resident intelligence dashboard"), withCapabilityTags("dashboard", "intel", "world-monitor", "query"))
      .query(nsid("ai.gftd.apps.maps.listIntelEvents"), (_, body) => cmdMapsPodIntelRead("ai.gftd.apps.maps.listIntelEvents", body), asAgentTool("List resident intelligence graph events"), withCapabilityTags("event", "intel", "world-monitor", "query"))
      .query(nsid("ai.gftd.apps.maps.getRiskSnapshot"), (_, body) => cmdMapsPodIntelRead("ai.gftd.apps.maps.getRiskSnapshot", body), asAgentTool("Get resident intelligence risk snapshot"), withCapabilityTags("risk", "intel", "world-monitor", "query"))
      .query(nsid("ai.gftd.apps.maps.getLatestBrief"), (_, body) => cmdMapsPodIntelRead("ai.gftd.apps.maps.getLatestBrief", body), asAgentTool("Get latest resident intelligence brief"), withCapabilityTags("brief", "intel", "world-monitor", "query"))
      .query(nsid("ai.gftd.apps.maps.listIntelAlerts"), (_, body) => cmdMapsPodIntelRead("ai.gftd.apps.maps.listIntelAlerts", body), asAgentTool("List resident intelligence alerts"), withCapabilityTags("alert", "intel", "world-monitor", "query"));
    a.command(nsid("ai.gftd.apps.maps.timeline"), (_, body) => cmdTimeline(sdk, body), asAgentTool("Get spatial event timeline for an entity"), withCapabilityTags("timeline", "event", "query"))
      .command(nsid("ai.gftd.apps.maps.displayLayerDefine"), (_, body) => cmdDisplayLayerDefine(sdk, body), asAgentTool("Define a display layer"), withCapabilityTags("layer", "write"))
      .query(nsid("ai.gftd.apps.maps.listDisplayLayers"), (_, body) => cmdListDisplayLayers(sdk, body), asAgentTool("List display layers"), withCapabilityTags("layer", "query"));
    // Sensor Intelligence
    // Simulation Intelligence
    // Spatiotemporal
    // Analytics
    // Step 1: User Post EXIF Geolocation
    a.command(nsid("ai.gftd.apps.maps.extractPostLocation"), (_, body) => cmdExtractPostLocation(sdk, body), asAgentTool("Extract geolocation from post images via EXIF"), withCapabilityTags("vision", "exif", "write"));
    // Step 2: Mapraly Ingest
    a.command(nsid("ai.gftd.apps.maps.mapralyIngest"), (_, body) => cmdMapralyIngest(sdk, body), asAgentTool("Create Mapraly collection job"), withCapabilityTags("mapraly", "ingest"));
    // Step 3: Murakumo Vision Analysis
    a.command(nsid("ai.gftd.apps.maps.analyzeImage"), (_, body) => cmdAnalyzeImage(sdk, body), asAgentTool("Analyze image for spatial entities via Murakumo Vision"), withCapabilityTags("vision", "analyze"));
    ...
    a.query(nsid("ai.gftd.apps.maps.crawlerLocations"), (_, body) => cmdCrawlerLocations(sdk, body));
    a.query(nsid("ai.gftd.apps.maps.actorLocations"), (_, body) => cmdActorLocations(sdk, body));
    a.query(nsid("ai.gftd.apps.maps.listLiveAircraft"), (_, body) => cmdListLiveAircraft(sdk, body));
    a.query(nsid("ai.gftd.apps.maps.listLiveSatellites"), (_, body) => cmdListLiveSatellites(sdk, body));
    a.query(nsid("ai.gftd.apps.maps.listCelestialObjects"), (_, body) => cmdListCelestialObjects(sdk, body));
    a.query(nsid("ai.gftd.apps.maps.aircraftTrack"), (_, body) => cmdAircraftTrack(sdk, body));
    a.command(nsid("ai.gftd.apps.maps.satellitePassQuery"), (_, body) => cmdSatellitePassQuery(sdk, body), asAgentTool("Upcoming satellite passes for an arbitrary observer (SGP4)"), withCapabilityTags("satellite", "pass", "sgp4", "visibility"));

    // AIS Marine — MarineTraffic-equivalent vessel tracking (ADR-2605011500)
    a.query(nsid("ai.gftd.apps.maps.aismarine.queryVesselsBbox" as any), (_, body) => cmdAismarineQueryVesselsBbox(sdk, body), asAgentTool("List AIS-tracked vessels (MarineTraffic-equivalent) inside a WGS84 bbox as GeoJSON"), withCapabilityTags("vessel", "ais", "marine", "live", "tracking"))
      .query(nsid("ai.gftd.apps.maps.aismarine.getVesselDetail" as any), (_, body) => cmdAismarineGetVesselDetail(sdk, body), asAgentTool("Vessel master + 24h track + active voyage by MMSI"), withCapabilityTags("vessel", "ais", "marine", "detail"))
      .query(nsid("ai.gftd.apps.maps.aismarine.searchVessels" as any), (_, body) => cmdAismarineSearchVessels(sdk, body), asAgentTool("Search vessels by name prefix, MMSI, or IMO"), withCapabilityTags("vessel", "ais", "marine", "search"))
      .query(nsid("ai.gftd.apps.maps.aismarine.getVesselDensityTile" as any), (_, body) => cmdAismarineGetVesselDensityTile(sdk, body), asAgentTool("Aggregated vessel density for low-zoom heatmap (Phase 1: 0.1° grid; Phase 2: H3 res-6)"), withCapabilityTags("vessel", "ais", "marine", "density", "grid"));
    ...
    a.query(nsid("ai.gftd.apps.maps.searchPlaces"), (_, body) => cmdSearchPlaces(sdk, body), asAgentTool("Substring search vertex_spatial → place rows with lat/lng"), withCapabilityTags("search", "place", "graph"))
      .query(nsid("ai.gftd.apps.maps.searchResources"), (_, body) => cmdSearchResources(sdk, body), asAgentTool("Multi-source keyword search: places + vessels + legal entities"), withCapabilityTags("search", "resource", "graph"))
      .query(nsid("ai.gftd.apps.maps.graphSearchNodes" as any), (_, body) => cmdGraphSearchNodes(sdk, body), asAgentTool("Cross-actor entity-graph keyword search; pins drawable from {lat,lng}"), withCapabilityTags("search", "graph", "entity"))
      .query(nsid("ai.gftd.apps.maps.searchSemanticNodes" as any), (_, body) => cmdSearchSemanticNodes(sdk, body), asAgentTool("IVF semantic search via Cloudflare Workers AI bge-base embeddings + 128 centroids"), withCapabilityTags("search", "ivf", "semantic", "embedding"));
