...
test('4. search — type "東京駅" and get results', async ({ page }) => {
  ...
  const xrpc = await page.request.get('/xrpc/ai.gftd.apps.maps.search_places?q=%E6%9D%B1%E4%BA%AC%E9%A7%85&limit=5');
...
test('8. XRPC — search_resources, graph_neighbors, weather_at', async ({ page }) => {
  ...
  // Backend uses camelCase NSIDs (from app.ts .command("ai.gftd.apps.maps.xxxx",...))
  // GET queries
  const queries: [string, string][] = [
    ['/xrpc/ai.gftd.apps.maps.listRoutes?limit=5', 'listRoutes'],
    ['/xrpc/ai.gftd.apps.maps.listDisplayLayers', 'listDisplayLayers'],
    ['/xrpc/ai.gftd.apps.maps.listGeoDomains', 'listGeoDomains'],
  ];
  ...
  const procs: [string, string, object][] = [
    ['/xrpc/ai.gftd.apps.maps.getDashboard', 'getDashboard', {}],
    ['/xrpc/ai.gftd.apps.maps.weatherAt', 'weatherAt', { lat: 35.6812, lng: 139.7671 }],
    ['/xrpc/ai.gftd.apps.maps.weatherGrid', 'weatherGrid', { lat: 35.6812, lng: 139.7671, gridStep: 0.5, gridRadius: 2 }],
    ['/xrpc/ai.gftd.apps.maps.ipGeolocate', 'ipGeolocate', {}],
  ];
...
test('10. crawler locations — poll API responds', async ({ page }) => {
  ...
  const res = await page.request.get('/xrpc/ai.gftd.apps.maps.list_crawler_locations?job_limit=3&results_per_job=5&limit=15');
  const status = res.status();
...
test('11. weather layer — weather_grid XRPC', async ({ page }) => {
  ...
  const res = await page.request.post('/xrpc/ai.gftd.apps.maps.weatherGrid', {
