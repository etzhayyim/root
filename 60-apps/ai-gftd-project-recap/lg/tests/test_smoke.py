...
def test_server_module_imports():
    from lg_recap.server import GRAPHS, NSID_MAP, app
...
def test_graphs_match_expected_set():
    from lg_recap.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_recap.server import NSID_MAP
    assert NSID_MAP == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_recap.server import GRAPHS, NSID_MAP
...
@pytest.fixture
def client():
    from lg_recap.server import app
...
def test_unknown_nsid_xrpc_404(client):
    r = client.post("/xrpc/ai.gftd.apps.recap.unknownMethod", json={})
    assert r.status_code == 404
