...
kubectl -n mitama-udf run smoke --rm -i --image=curlimages/curl --restart=Never -- \
  ...
  -d '{}' http://lg-mangaka.mitama-udf.svc.cluster.local:8000/xrpc/ai.gftd.mangaka.health
# expect: {"ok": true, ...}
```
...
psql "$ROOT_URL" -c \
  "DELETE FROM vertex_mangaka WHERE kind='document' AND collection='ai.gftd.mangaka.document' AND rkey LIKE 'doc-gh-%';"
```
