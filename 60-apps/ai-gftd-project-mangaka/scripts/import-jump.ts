...
const topProject = await xrpc("ai.gftd.mangaka.createProject", {
...
for (const slug of charDirs) {
  ...
  await xrpc("ai.gftd.mangaka.createCharacter", {
...
for (const slug of orgDirs) {
  ...
  await xrpc("ai.gftd.mangaka.createOrganization", {
...
for (const slug of envDirs) {
  ...
  await xrpc("ai.gftd.mangaka.createEnvironment", {
    id: `env-${slug}`,
    workId: topProjectId,
    name,
    slug,
...
for (const epSlug of episodeDirs) {
  ...
  await xrpc("ai.gftd.mangaka.createWork", {
    id: workId,
    title,
    arc: ep["gh:arc"] || "",
  ...
  const epProject = await xrpc("ai.gftd.mangaka.createProject", {
    name: title,
  ...
  if (acts.length === 0) {
    ...
    await xrpc("ai.gftd.mangaka.addChapter", {
      id: defaultChapterId,
      workId,
      chapterNum: 1,
      title: title,
  ...
  for (const pg of pages) {
    ...
    await xrpc("ai.gftd.mangaka.addPage", {
      id: pageId,
      chapterId,
      pageNum,
    ...
    for (let pi = 0; pi < panels.length; pi++) {
      ...
      await xrpc("ai.gftd.mangaka.createPanel", {
        id: panelId,
        pageId,
        order: pnl["panel"] ?? pi,
      ...
      for (let gi = 0; gi < genImages.length; gi++) {
        ...
        await xrpc("ai.gftd.mangaka.recordGeneratedImage", {
          id: `${panelId}-v${gi + 1}`,
          panelId,
          workId,
