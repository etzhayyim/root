/**
 * tsukuru rw-free — escrow_intent pattern helpers.
 *
 ...
 *     → openIntent() writes ai.gftd.apps.payment.escrowOpened with
 ...
 *       actual USDC.transfer. Writes ai.gftd.apps.payment.sent. The
 *       settlement step lives in qualityInspection module, not here.
 *
 *   cancel before delivery
 *     → refundIntent() writes ai.gftd.apps.payment.escrowRefunded.
...
export async function openIntent(
  ...
  const receipt = await e.write({
    collection: "ai.gftd.apps.payment.escrowOpened",
 ...
 * ai.gftd.apps.payment.escrowRefunded record.
 *
...
export async function refundIntent(
  ...
  const receipt = await e.write({
    collection: "ai.gftd.apps.payment.escrowRefunded",
    record: record as unknown as Record<string, unknown>,
  });
