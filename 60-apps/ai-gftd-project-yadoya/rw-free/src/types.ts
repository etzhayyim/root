/**
 * yadoya rw-free — record types.
 *
 * Tier-2 function-split per ADR-2606011400 (on-chain-only). Short-term lodging
 * (宿屋): listings + bookings on AT PDS records; bookings settle on-chain (USDC +
 ...
 *
 * Identity hierarchy:
 *   did:web:yadoya.etzhayyim.com                          — controller
 *   did:web:yadoya.etzhayyim.com:listing:{listingId}      — a listing
 *   did:web:yadoya.etzhayyim.com:booking:{bookingId}      — a booking
 */
...
export function listingDid(listingId: string): string {
  return `${YADOYA_DID_PREFIX}listing:${listingId.toLowerCase()}`;
}
...
export function bookingDid(bookingId: string): string {
  return `${YADOYA_DID_PREFIX}booking:${bookingId.toLowerCase()}`;
}
