...
write/read surface for calendar events (graph ``drive-v1``). Adapted from
...
dedicated ``drive-v1`` graph JWT-only (no CACAO) per ADR-2605302130 Option 3.
"""
...
_GRAPH_LABEL = os.environ.get("KOTOBA_GRAPH", "drive-v1")
