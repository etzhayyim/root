# ai-gftd-project-drive — Google Drive v3 + Microsoft Graph (OneDrive) compatible drive API

**drive.gftd.ai** — a drive/files API whose external surface is shaped like **both**
...
```
Google SDK  → drive.gftd.ai/drive/v3/files          ┐
MS Graph SDK→ drive.gftd.ai/v1.0/me/drive/root/children ┘ (two external skins)
  CF Worker drive-compat (50-infra/cloudflare/workers/drive-compat)
   ├─ /xrpc/ai.gftd.apps.drive.*  → pod (actor-worker passthrough)
   └─ provider REST → canonical XRPC → pod → reshape to provider JSON
  lg-drive pod (60-apps/ai-gftd-project-drive/lg, FastAPI server.py)
...
atproto catch-all actor:true → pipethroughActorWorker → drive.gftd.ai/xrpc →
`DRIVE_POD_URL` tunnel → pod; dedicated graph `drive-v1`).

## Canonical XRPC methods (ai.gftd.apps.drive.*)
...
Lexicons: `00-contracts/lexicons/ai/gftd/apps/drive/` (`defs.json` file/about/change superset).
...
## Test
```bash
cd 60-apps/ai-gftd-project-drive/lg && python3 -m pytest tests/ -q
