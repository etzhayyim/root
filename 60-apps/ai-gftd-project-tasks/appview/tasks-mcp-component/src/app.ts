// tasks.gftd.ai thin edge facade. OAuth/sync/cron run in AgentGateway MCP + pod-side LangServer.
...
const APP = "tasks";
const NSID_PREFIX = "ai.gftd.apps.tasks.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:tasks.gftd.ai", nanoid: env.APP_NANOID ?? "tasks-mcp", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "20-actors/magatama/py/src/pymagatama/ingest/gworkspace_lite.py", bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/tasks" });
