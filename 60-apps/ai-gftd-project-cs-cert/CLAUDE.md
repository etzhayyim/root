...
└── wasm/
    └── ai-gftd-wasm-cs-cert-badge-k7m3p9x2/
...
- `gftd:CsCertificate` — 発行済み認証 (ID, domain, level, status, score, expiry)
- `gftd:CsAssessment` — セキュリティ診断レコード (checks, findings, score)
- `gftd:CsFinding` — 個別検出事項 (severity, category, status)
- `gftd:CsBadge` — バッジメタデータ (format, color, embed URL)
