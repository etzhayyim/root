# worlds.gftd.ai 設計 (ai-gftd-project-worlds)
...
- `https://worlds.gftd.ai` (Editor)
  - 認証・保存・fork・LLM支援・アセット管理
  - セッションクッキーやトークンが存在しうる
- `https://cdn.worlds.gftd.ai` (Preview)
  - World を “公開HTML” としてホストするだけ (CDN配信)
  - 原則クッキーなし・最小ヘッダ・CSP強め
  - VR/WebXR の制約上、`iframe sandbox` だけで完結させず **別オリジンで安全に実行**
  - MVP: `https://cdn.worlds.gftd.ai/shell` を「受信した files を描画する preview shell」として提供し、Editor から `postMessage` でホットリロードする
...
- 中: コード (3ペイン or タブ)
  ...
  - 右: Preview (cdn.worlds.gftd.ai の iframe) + VR ボタン + Console
...
- CORS: `w.worlds.gftd.ai` から取得できること
...
- Quest等のHMDで `w.worlds.gftd.ai` のプレビューURLを直に開ける
...
静的配信:
- `worlds-ui` と `cdn.worlds.gftd.ai` は nginx/Cloud CDN で静的配信が基本
...
- ローカルでは preview shell を同一アプリ内の `/preview-shell` で代替し、`cdn.worlds.gftd.ai/shell` 相当の挙動を確認できる
