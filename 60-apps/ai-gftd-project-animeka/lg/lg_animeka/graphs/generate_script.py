...
NSID: ai.gftd.animeka.generateScript
...
collection=ai.gftd.animeka.script.
"""
...
async def _node_fetch_episode(state: _State) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            cur = conn.cursor()
            await cur.execute(
                "SELECT title, synopsis FROM vertex_animeka WHERE collection='ai.gftd.animeka.episode' AND rkey=%s LIMIT 1",
                [rkey],
            )
...
async def _node_insert(state: _State) -> dict[str, Any]:
    ...
    vertex_id = f"at://{_REPO}/ai.gftd.animeka.script/{rkey}"
        ...
        try:
            await conn.execute(
                """INSERT INTO vertex_animeka
                   ...
                   VALUES (%s, %s, %s, 'ai.gftd.animeka.script', 'script',
                           %s, %s, %s, %s, 'draft', %s)""",
