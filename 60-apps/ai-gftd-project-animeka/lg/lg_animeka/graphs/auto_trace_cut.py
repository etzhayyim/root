...
NSID: ai.gftd.animeka.autoTraceCut
...
_APP_DID = os.environ.get("ANIMEKA_APP_DID", "did:web:animeka.gftd.ai")
...
async def _node_insert(state: _State) -> dict[str, Any]:
    ...
    vertex_id = f"at://{_REPO}/ai.gftd.animeka.colorTrace/{rkey}"
        ...
        try:
            await conn.execute(
                """INSERT INTO vertex_animeka
                   ...
                   VALUES (%s, %s, %s, 'ai.gftd.animeka.colorTrace', 'colorTrace',
                           %s, %s, %s, 'draft', %s)""",
