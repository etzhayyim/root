...
async def _node_insert(state: _AddCutState) -> dict[str, Any]:
    ...
    collection = "ai.gftd.animeka.cut"
        ...
        try:
            ...
            await cur.execute(
                """SELECT vertex_id, episode_id, fps FROM vertex_animeka
                   WHERE collection='ai.gftd.animeka.scene' AND rkey=%s LIMIT 1""",
                [scene_rkey],
            )
            scene_row = await cur.fetchone()
            scene_vertex_id = scene_row[0] if scene_row else f"at://{owner_did}/ai.gftd.animeka.scene/{scene_rkey}"
