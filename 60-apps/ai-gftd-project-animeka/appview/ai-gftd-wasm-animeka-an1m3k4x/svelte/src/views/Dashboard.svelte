<script lang="ts">
    ...
    try {
      const [worksResp, cutsResp, episodesResp, retakesResp] = await Promise.all([
        atQuery<{ works?: Record<string, unknown>[]; items?: Record<string, unknown>[] }>(
          'ai.gftd.animeka.listWorks', { limit: 200 }
        ),
        atQuery<{ items?: CutRow[] }>('ai.gftd.animeka.listCuts', { limit: 500 }),
        atQuery<{ items?: Record<string, unknown>[] }>('ai.gftd.animeka.listEpisodes', { limit: 200 })
          .catch(() => ({ items: [] })),
        atQuery<{ items?: unknown[] }>('ai.gftd.animeka.listRetakes', { status: 'open', limit: 1 })
          .catch(() => ({ items: [] })),
      ]);
      ...
      // Fetch actual count
      atQuery<{ total?: number; items?: unknown[] }>('ai.gftd.animeka.listRetakes', { status: 'open', limit: 500 })
  ...
  async function createWork() {
    ...
    creating = true;
    try {
      await atProcedure('ai.gftd.animeka.createWork', { title: newTitle.trim() });
      newTitle = '';
      await load();
  ...
  async function publishEpisode() {
    ...
    try {
      const r = await atProcedure<{ episode_cid?: string; episodeCid?: string; cut_count?: number; cutCount?: number; error?: string }>(
        'ai.gftd.animeka.publishEpisode', { maxCuts: publishMaxCuts }
      );
  ...
  async function runScore() {
    ...
    try {
      const r = await atProcedure<{ scores?: unknown[]; summary?: Record<string,unknown>; error?: string }>(
        'ai.gftd.animeka.scoreCuts', { maxCuts: scoreCutCount }
      );
  ...
  async function runKaizen() {
    ...
    try {
      const r = await atProcedure<{ improved_count?: number; directives_used?: string[]; kaizen_results?: unknown[]; error?: string }>(
        'ai.gftd.animeka.kaizenCompositor', { maxCuts: scoreCutCount, threshold: kaizenThreshold }
      );
  ...
  async function runGenerateAudio() {
    ...
    try {
      const r = await atProcedure<{ summary?: { successful?: number; mood_distribution?: Record<string,number>; tts_cuts?: number }; error?: string }>(
        'ai.gftd.animeka.generateAudio', { maxCuts: audioCutCount }
      );
