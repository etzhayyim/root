 ...
 * public projection (`vertex_gftd_key_signing`, `human_self_custody`). The server
...
export async function verifySessionPoP(
  ...
  const row = await env.KEYS_DB.prepare(
    "SELECT public_key_multibase FROM vertex_gftd_key_signing WHERE vertex_id = ? AND key_custody_tier IN ('human_self_custody','agent_self_custody') LIMIT 1",
  )
    .bind(did)
