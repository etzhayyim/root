 ...
 * envelopes whose plaintext body matches the `ai.gftd.auth.credential`
 ...
 * `60-apps/ai-gftd-project-auth/yatachain-projection.toml`). This module
...
// ─── Plaintext credential shape (mirrors ai.gftd.auth.credential) ──
...
const INNER_TYPE = "ai.gftd.auth.credential";
 ...
 * `vertex_gftd_auth_credential` (passkey-only — other kinds project
 * into `edge_gftd_auth_linked` / `vertex_gftd_auth_otp` / etc.).
 *
