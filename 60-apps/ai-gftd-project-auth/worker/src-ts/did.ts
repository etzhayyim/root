...
export async function createetzhayyimDid(performerType: PerformerType): Promise<{ privateKeyB64url: string; did: string; publicKeyMultibase: string }> {
  ...
  const did = `did:gftd:${hashHex.slice(0, 24)}`;
...
/* ────────────────────────────────────────────────────────────────────────
 * ADR-0029 — did:gftd recursive hash/Merkle form
 *
 * Child DID = did:gftd:{h0}:{h1}:…:{hN}
...
export function didDepth(did: string): number {
  if (!did.startsWith("did:gftd:")) throw new Error("not a did:gftd");
  const tail = did.slice("did:gftd:".length);
...
/* ────────────────────────────────────────────────────────────────────────
 * ADR-0029 (revised 2026-04-19) — did:gftd recursive semantic path form
 *
 * Child DID = did:gftd:{s0}:{s1}:…:{sN}   where each sN is a semantic segment
 * (sub / id / lexicon / role / pubkey / grant). Collision prevention is via
 * vertex_gftd_identity UNIQUE (parent_did, segment_kind, segment_value).
 *
