...
async function ensureAuthTables(env: Env): Promise<void> {
  ...
  if (!authTablesReady) {
    authTablesReady = env.AUTH_DB.batch([
      ...
      `),
      // ── did:gftd auth control plane (GraphAr schema in D1) ──
      env.AUTH_DB.prepare(`
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_auth_credential (
          vertex_id TEXT PRIMARY KEY,
          sensitivity_ord INTEGER NOT NULL DEFAULT 3,
          owner_did TEXT,
          did TEXT NOT NULL,
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_auth_otp (
          vertex_id TEXT PRIMARY KEY,
          sensitivity_ord INTEGER NOT NULL DEFAULT 3,
          owner_did TEXT,
          account_did TEXT NOT NULL,
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS edge_gftd_auth_linked (
...
async function ensureKeysTables(env: Env): Promise<void> {
  ...
  if (!keysTablesReady) {
    keysTablesReady = env.KEYS_DB.batch([
      // ── did:gftd key custody (GraphAr schema, KEK envelope) ──
      env.KEYS_DB.prepare(`DROP TABLE IF EXISTS vertex_gftd_key_signing`),
      env.KEYS_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_key_signing (
          vertex_id TEXT PRIMARY KEY,
          sensitivity_ord INTEGER NOT NULL DEFAULT 3,
          owner_did TEXT,
          did TEXT NOT NULL,
      ...
      `),
      env.KEYS_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_key_otp (
          vertex_id TEXT PRIMARY KEY,
          sensitivity_ord INTEGER NOT NULL DEFAULT 3,
          owner_did TEXT,
          phone TEXT NOT NULL,
      ...
      `),
      env.KEYS_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_key_api_key (
          vertex_id TEXT PRIMARY KEY,
      ...
      try {
        await env.KEYS_DB.prepare(
          "ALTER TABLE vertex_gftd_key_revoked_session ADD COLUMN sid TEXT",
        ).run();
      } catch (_e) { /* column already exists — first run after fresh CREATE */ }
      try {
        await env.KEYS_DB.prepare(
          "CREATE INDEX IF NOT EXISTS idx_revoked_session_sid ON vertex_gftd_key_revoked_session (sid)",
        ).run();
...
function orgIdFromAccountDid(accountDid: string): string {
  const match = accountDid.match(/^did:web:authn\.gftd\.ai:user:([a-zA-Z0-9._:-]+)/);
 ...
 * a row to `vertex_gftd_key_revoked_session`. RFC 7009 §2.2 requires the
...
async function handleCheckRevoked(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const sql = `SELECT revoked_at FROM vertex_gftd_key_revoked_session WHERE ${conds.join(" OR ")} LIMIT 1`;
 ...
 * Requires a valid session (Authorization Bearer or gftd_session cookie).
 */
...
async function handleRotateAgentKey(request: Request): Promise<Response> {
  const body = await parseJson<{ did: string }>(request);
  const path = body.did.replace(/^did:web:authn\.gftd\.ai:/, "");
...
async function localCreateApiKey(
  ...
  await env.KEYS_DB.prepare(
    `INSERT INTO vertex_gftd_key_api_key
...
async function mintBootstrapApiKey(env: Env, accountDid: string): Promise<string> {
  ...
  const { key } = await localCreateApiKey(env, accountDid, "gftd-cli-login", "read,write");
  return key;
}
...
async function handleInternalVerifyApiKey(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const row = await env.KEYS_DB.prepare(
      `SELECT owner_did, scopes, product_scope FROM vertex_gftd_key_api_key
    ...
    env.KEYS_DB.prepare(
      `UPDATE vertex_gftd_key_api_key SET last_used_at = ? WHERE key_hash = ?`,
...
/**
 * Resolve a did:gftd DID Document.
 ...
 */
// ── ADR-0029 recursive did:gftd resolver helpers ───────────────────────
...
async function handleResolveGftdDid(request: Request, env: Env): Promise<Response> {
  ...
  if (env.AUTH_DB) {
    ...
    authAccount = await env.AUTH_DB.prepare(
      "SELECT handle, legacy_did, controller_did, actor_score FROM vertex_gftd_auth_account WHERE vertex_id = ? LIMIT 1"
    ).bind(did).first();
  }
...
async function handleMintChildDid(request: Request, env: Env): Promise<Response> {
  ...
  if (!callerAccount?.startsWith("did:gftd:")) {
    return jsonErr(403, "Forbidden", "caller session lacks did:gftd account");
  }
...
async function syncAuthMethodToGftdIdentity(env: Env, accountDid: string, provider: string, email: string, verified: boolean): Promise<void> {
  ...
  // RisingWave: write edge_gftd_authenticates + update vertex actor_score (fire-and-forget)
  if (env.PDS_SERVICE) {
    const now = nowIso();
    const graphPayload = {
      edges: [{
...
async function handleGetServiceAuth(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (!_SVC_AUTH_ISS_ALLOWLIST.has(body.iss) && !body.iss.startsWith("did:gftd:")) {
    ...
    // did:gftd issuer: envelope-decrypt signing key from vertex_gftd_key_signing
    if (body.iss.startsWith("did:gftd:") && env.KEYS_DB) {
      await ensureKeysTables(env);
      const keyRow = await env.KEYS_DB.prepare(
        "SELECT encrypted_private_key, wrapped_data_key, iv FROM vertex_gftd_key_signing WHERE vertex_id = ?"
...
async function handleEsimProvision(request: Request, env: Env): Promise<Response> {
  ...
  const resp = await postJson(
    ...
    { 'simCardGroupId': null, tags: [`gftd:${phone}`] },
    { Authorization: `Bearer ${telnyxKey}` },
  ).catch((_err) => null);
