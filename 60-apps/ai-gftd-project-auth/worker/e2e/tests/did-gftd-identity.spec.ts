/**
 * did:gftd identity integration test.
 *
...
test.describe("did:gftd identity lifecycle", () => {

  test("passkey sign-up mints did:gftd and resolves DID Document", async ({ page }) => {
    const cdp = await attachVirtualAuthenticator(page);
    await page.goto(`${AUTH_BASE}/sign-up`, { waitUntil: "domcontentloaded" });

    // ── Step 1: Passkey register → did:gftd mint ──
    ...
    if (isetzhayyimDid) {
      ...
      expect(accountDid).toMatch(/^did:web:auth\.gftd\.ai:user:/);
      test.info().annotations.push({ type: "note", description: "pre-deploy: did:web returned (did:gftd not yet live)" });
    }
    expect(accountDid).not.toBe(activeDid);
    expect(handle).toMatch(/^[a-z][a-z0-9]{7}\.gftd\.ai$/);
    ...
    // ── Step 4: Service Auth JWT with did:gftd iss ──
