/**
 * Headless E2E equivalent of `gftd authn signin`.
 *
 ...
 * writes `~/.gftd/auth.json` so the gftd CLI can authenticate without an
 * interactive browser session.
 *
 * Run:
 *   cd 60-apps/ai-gftd-project-auth/worker/e2e
 *   pnpm exec playwright test tests/mint-auth-json.spec.ts
 *
 * Then any subsequent `gftd ...` call uses the minted apiKey.
 */
...
test("mint auth.json via headless WebAuthn → OAuth PKCE → apiKey", async ({ page, request }) => {
  ...
  mkdirSync(join(homedir(), ".gftd"), { recursive: true, mode: 0o700 });
