...
async function registerPasskeySession(page: Page, request: any): Promise<{ did: string; cookie: string }> {
  ...
  expect(did).toMatch(/^did:(gftd:[a-f0-9]{24}|web:auth\.gftd\.ai:user:)/);

  // Extract accessJwt from response body — used as gftd_session cookie value.
  ...
  const cookie = `gftd_session=${accessJwt}`;
...
test.describe("T4 AuthZ Worker (authz.gftd.ai)", () => {
  ...
  test("2. getSession without auth returns {ok: false}", async ({ request }) => {
    const resp = await request.get(`${AUTHZ_BASE}/xrpc/ai.gftd.authz.getSession`, {
  ...
  test("4. canonical ai.gftd.authz.getSession works on authz", async ({ page, request }) => {
  ...
  test("5. org management: create → info → list → invite → accept → members", async ({
    ...
    const infoResp = await request.get(
      `${AUTHZ_BASE}/xrpc/ai.gftd.authz.orgInfo?orgDid=${encodeURIComponent(orgDid)}`,
    ...
    // 5c. orgList
    const listResp = await request.get(`${AUTHZ_BASE}/xrpc/ai.gftd.authz.orgList`, {
      headers: { Cookie: ownerCookie },
    });
    ...
    const acceptResp = await request.post(`${AUTHZ_BASE}/xrpc/ai.gftd.authz.orgInviteAccept`, {
    ...
    const membersResp = await request.get(
      `${AUTHZ_BASE}/xrpc/ai.gftd.authz.orgMembers?orgDid=${encodeURIComponent(orgDid)}`,
      { headers: { Cookie: ownerCookie } },
    );
    ...
    const leaveResp = await request.post(`${AUTHZ_BASE}/xrpc/ai.gftd.authz.orgLeave`, {
      headers: { Cookie: memberCookie, "Content-Type": "application/json" },
      data: { orgDid },
    });
