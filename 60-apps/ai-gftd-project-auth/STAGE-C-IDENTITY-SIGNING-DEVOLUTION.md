...
- **ADR-0074** (Ethereum Identity Bridge — CACAO + WebAuthn):
  `did:erc725:gftd:260425:{identityContract}` is the platform primary
...
- [ ] `vertex_gftd_key_signing` schema migration drops the `private_key_b64` column.
...
- ADR-0074 (Ethereum Identity Bridge — CACAO + WebAuthn)
- ADR-0029 (did:gftd method specification — legacy migration)
- `yatachain-projection.toml` — declares `vertex_gftd_*` as L0-rebuildable from MST
