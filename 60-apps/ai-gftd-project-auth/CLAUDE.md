...
- **設計詳細**: `60-apps/ai-gftd-project-moderator/CLAUDE.md` §Org-Account-Based Identity Design
...
→ `gftd dodaf tv1 query --id ai-gftd-project-auth-identity-model-didweb-+-server-assisted` / MCP `gftd.dodaf.tv1.query`
...
│       ├── did.ts              # DID Document CRUD + did:gftd + P-256 keypair
...
**PDS 変更**:
- `wrangler.jsonc` に `AUTH_SERVICE` service binding 追加 (`ai-gftd-auth`)
...
- `gftd deploy` が自動で agent credential を発行・登録
...
→ `gftd dodaf tv1 query --id ai-gftd-project-auth-secret-architecture-p3-final` / MCP `gftd.dodaf.tv1.query`

## CRITICAL: 3-System Integration (auth → PDS → yata)

→ `gftd dodaf tv1 query --id ai-gftd-project-auth-3-system-integration-auth-pds-yata` / MCP `gftd.dodaf.tv1.query`
...
| Table | DB | Content |
|---|---|---|
| `vertex_gftd_auth_account` | AUTH_DB | ERC725 root DID 認証制御 (handle, actor_score, status)。table name is legacy GraphAr namespace |
| `vertex_gftd_auth_credential` | AUTH_DB | WebAuthn passkey |
| `vertex_gftd_auth_invite` | AUTH_DB | org 招待 (HMAC token) |
| `vertex_gftd_auth_otp` | AUTH_DB | Email OTP |
| `edge_gftd_auth_linked` | AUTH_DB | OAuth/Email リンク |
| `vertex_gftd_key_signing` | KEYS_DB | **KEK envelope encrypted** signing key |
| `vertex_gftd_key_revoked_session` | KEYS_DB | session 無効化 |
| `vertex_gftd_key_otp` | KEYS_DB | SMS OTP |
...
- **Legacy テーブル (did_keys / revoked_sessions / otp_codes) の再導入禁止** — pruned 済み。GraphAr vertex_gftd_key_* テーブルに統合
