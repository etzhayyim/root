...
export interface EthRpcEnv {
  ...
  /** Service binding to `ai-gftd-geth-rpc-proxy`. When present, JSON-RPC
...
export async function rpc<T = unknown>(
  ...
  if (opts.privileged) {
    ...
    headers["x-gftd-rpc-auth"] = await hmacHex(key, body);
  }
  ...
  // back to public fetch when no binding (e.g. from outside the gftd account).
