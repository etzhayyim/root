...
interface Env {
  ...
  // `ai-gftd-claim-consumer` with an identical value (mirrored in
  // macOS Keychain `gftd.cloudflare/CLAIM_SETTLER_HMAC`).
...
async function ensureAuthTables(env: Env): Promise<void> {
  ...
  if (!authTablesReady) {
    authTablesReady = env.AUTH_DB.batch([
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_auth_account (
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_auth_org (
          vertex_id TEXT PRIMARY KEY,
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS edge_gftd_auth_member (
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_auth_invite (
          vertex_id TEXT PRIMARY KEY,
          sensitivity_ord INTEGER NOT NULL DEFAULT 3,
          owner_did TEXT,
          org_did TEXT NOT NULL,
      ...
      `),
      env.AUTH_DB.prepare(`
        CREATE TABLE IF NOT EXISTS vertex_gftd_claim_index (
      ...
      env.AUTH_DB.prepare(`
        CREATE INDEX IF NOT EXISTS idx_claim_index_account_did
          ON vertex_gftd_claim_index(account_did, posted_at DESC)
      `),
      env.AUTH_DB.prepare(`
        CREATE INDEX IF NOT EXISTS idx_claim_index_at_record_cid
          ON vertex_gftd_claim_index(at_record_cid)
      `),
...
async function syncAuthMethodToGraph(env: Env, accountDid: string, provider: string, email: string, verified: boolean): Promise<void> {
  ...
  await ensureAuthTables(env);
  const row = await env.AUTH_DB.prepare(
    "SELECT auth_methods_summary, actor_score FROM vertex_gftd_auth_account WHERE vertex_id = ? LIMIT 1"
  ...
  await env.AUTH_DB.prepare(
    "UPDATE vertex_gftd_auth_account SET auth_methods_summary = ?, actor_score = ?, updated_at = ? WHERE vertex_id = ?"
  ).bind(JSON.stringify(methods), score, nowIso(), rootDid).run();

  if (env.PDS_SERVICE) {
    // ADR-0036: auth-control graph edges (edge_gftd_authenticates) follow the
...
async function handleLinkEthereumBegin(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const statement = (body.statement && String(body.statement).trim())
      || `Link this Ethereum address to ${rootIdentity.rootDid} on gftd.`;
...
async function handleOrgCreate(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    // Upsert org vertex
    await env.AUTH_DB.prepare(`
      INSERT INTO vertex_gftd_auth_org (vertex_id, sensitivity_ord, owner_did, org_did, name, domain, org_type, settings_json, created_at, updated_at)
      VALUES (?, 2, ?, ?, ?, ?, ?, '{}', ?, ?)
    ...
    await env.AUTH_DB.prepare(
      "UPDATE vertex_gftd_auth_account SET performer_type='organization', updated_at=? WHERE vertex_id=?"
    ).bind(now, orgDid).run();

    // Auto-add creator as owner member
    await env.AUTH_DB.prepare(`
      INSERT INTO edge_gftd_auth_member (edge_id, src_vid, dst_vid, sensitivity_ord, owner_did, org_did, member_did, role, invited_by, joined_at, status)
...
async function handleOrgInfo(request: Request, env: Env): Promise<Response> {
  ...
  const org = await env.AUTH_DB.prepare(
    "SELECT org_did AS orgDid, name, domain, org_type AS orgType, settings_json AS settingsJson, created_at AS createdAt FROM vertex_gftd_auth_org WHERE vertex_id=? LIMIT 1"
  ...
  const countRow = await env.AUTH_DB.prepare(
    "SELECT COUNT(*) AS count FROM edge_gftd_auth_member WHERE org_did=? AND status='active'"
...
async function handleOrgMembers(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const membership = await env.AUTH_DB.prepare(
      "SELECT role FROM edge_gftd_auth_member WHERE org_did=? AND member_did=? AND status='active' LIMIT 1"
...
async function handleOrgInvite(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await env.AUTH_DB.prepare(`
      INSERT INTO vertex_gftd_auth_invite
...
async function handleOrgInviteAccept(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const invite = await env.AUTH_DB.prepare(
      "SELECT vertex_id, inviter_did AS inviterDid, status FROM vertex_gftd_auth_invite WHERE org_did=? AND email=? AND status='pending' LIMIT 1"
    ...
    await env.AUTH_DB.batch([
      ...
      env.AUTH_DB.prepare(
        "UPDATE vertex_gftd_auth_invite SET status='accepted', accepted_did=?, accepted_at=?, updated_at=? WHERE vertex_id=?"
...
async function handleOrgMemberRemove(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await env.AUTH_DB.prepare(
      "UPDATE edge_gftd_auth_member SET status='removed', joined_at=joined_at WHERE org_did=? AND member_did=?"
...
async function handleOrgMemberRoleUpdate(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await env.AUTH_DB.prepare(
      "UPDATE edge_gftd_auth_member SET role=? WHERE org_did=? AND member_did=?"
...
async function handleOrgTransferOwnership(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await env.AUTH_DB.batch([
      env.AUTH_DB.prepare(
        "UPDATE edge_gftd_auth_member SET role='admin' WHERE org_did=? AND member_did=?"
      ).bind(orgDid, callerDid),
      env.AUTH_DB.prepare(
        "UPDATE edge_gftd_auth_member SET role='owner' WHERE org_did=? AND member_did=?"
      ).bind(orgDid, newOwnerDid),
      env.AUTH_DB.prepare(
        "UPDATE vertex_gftd_auth_org SET owner_did=?, updated_at=? WHERE vertex_id=?"
...
async function handleOrgLeave(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await env.AUTH_DB.prepare(
      "UPDATE edge_gftd_auth_member SET status='left' WHERE org_did=? AND member_did=?"
    ).bind(orgDid, memberDid).run();
...
async function handleOrgList(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    await ensureAuthTables(env);
    const totalRow = await env.AUTH_DB.prepare(
      "SELECT COUNT(*) AS count FROM edge_gftd_auth_member WHERE member_did=? AND status='active'"
    ).bind(memberDid).first<{ count: number }>();
    const total = Number(totalRow?.count || 0);
    const rows = await env.AUTH_DB.prepare(`
      SELECT m.org_did AS orgDid, m.role, o.name, o.domain, o.org_type AS orgType
      FROM edge_gftd_auth_member m
      LEFT JOIN vertex_gftd_auth_org o ON o.vertex_id = m.org_did
...
async function handleOrgUpdate(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    values.push(orgDid);
    await env.AUTH_DB.prepare(
      `UPDATE vertex_gftd_auth_org SET ${setClauses.join(", ")} WHERE vertex_id=?`
    ).bind(...values).run();
...
// ClaimStakeEscrow on the gftd private chain. The handler does not write to
...
async function handlePostStakedAttestation(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (env.AUTH_DB) {
      env.AUTH_DB.prepare(
        `INSERT OR IGNORE INTO vertex_gftd_claim_index
...
async function handleListStakedAttestations(request: Request, env: Env): Promise<Response> {
  ...
  await ensureAuthTables(env);
  const rows = await env.AUTH_DB.prepare(
    `SELECT claim_id, at_record_cid, bond_gcc, posted_at FROM vertex_gftd_claim_index
       WHERE account_did = ?
...
async function handleLookupStakedAttestations(request: Request, env: Env): Promise<Response> {
  ...
  const rows = await env.AUTH_DB.prepare(
    `SELECT claim_id, at_record_cid FROM vertex_gftd_claim_index WHERE at_record_cid IN (${placeholders}) LIMIT 50`,
