...
async function cmdRegisterEmployee(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.registerEmployee", body);
...
async function cmdUpdateEmployeeStatus(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.updateEmployeeStatus", body);
...
async function cmdListEmployees(_sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.listEmployees", body);
  const limit = Number(args.limit ?? 50);
  const offset = Number(args.offset ?? 0);
  // TODO(kyber-hr): vertex_hr_employee not in @gftd/graph-schema — reads return empty
...
async function cmdCreatePosition(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.createPosition", body);
...
async function cmdListPositions(_sdk: HostSDK, _body: Uint8Array): Promise<Record<string, unknown>> {
  // TODO(kyber-hr): vertex_hr_position not in @gftd/graph-schema — reads return empty
  const rows = ([] as Record<string, unknown>[]);
...
async function cmdRegisterSkill(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.registerSkill", body);
...
async function cmdAssignSkill(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.assignSkill", body);
  const employeeId = str(args.employeeId ?? "");
...
async function cmdSkillGapAnalysis(_sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.skillGapAnalysis", body);
  const positionId = str(args.positionId ?? "");
  const employeeId = str(args.employeeId ?? "");
...
async function cmdCreateEvaluation(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.createEvaluation", body);
  const employeeId = str(args.employeeId ?? "");
...
async function cmdListEvaluations(_sdk: HostSDK, _body: Uint8Array): Promise<Record<string, unknown>> {
  // TODO(kyber-hr): vertex_hr_evaluation not in @gftd/graph-schema — reads return empty
  const rows = ([] as Record<string, unknown>[]);
...
async function cmdEnrollTraining(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.enrollTraining", body);
  const employeeId = str(args.employeeId ?? "");
...
async function cmdCompleteTraining(sdk: HostSDK, body: Uint8Array): Promise<Record<string, unknown>> {
  const args = parseLexiconInput("ai.gftd.app.kyber.hr.completeTraining", body);
...
function configureApp(sdk: HostSDK): void {
  ...
  app
    .command(nsid("ai.gftd.app.kyber.hr.health"), async () => cmdHealth(), asAgentTool("Kyber HR health check"), withCapabilityTags("diagnostics", "hr"))
    .command(nsid("ai.gftd.app.kyber.hr.describe"), async () => cmdDescribe(), asAgentTool("Describe Kyber HR capabilities"), withCapabilityTags("meta", "hr"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.registerEmployee"), async (_ctx, body) => cmdRegisterEmployee(sdk, body), asAgentTool("Register new employee with probation and leave tracking"), withCapabilityTags("hr", "employee", "talent"))
    .command(nsid("ai.gftd.app.kyber.hr.updateEmployeeStatus"), async (_ctx, body) => cmdUpdateEmployeeStatus(sdk, body), asAgentTool("Update employee status (active/on_leave/terminated/retired)"), withCapabilityTags("hr", "employee", "lifecycle"))
    .command(nsid("ai.gftd.app.kyber.hr.listEmployees"), async (_ctx, body) => cmdListEmployees(sdk, body), asAgentTool("List employees by department and status"), withCapabilityTags("hr", "employee", "query"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.createPosition"), async (_ctx, body) => cmdCreatePosition(sdk, body), asAgentTool("Create job position with required skills and salary range"), withCapabilityTags("hr", "position", "talent"))
    .command(nsid("ai.gftd.app.kyber.hr.listPositions"), async (_ctx, body) => cmdListPositions(sdk, body), asAgentTool("List open positions by department"), withCapabilityTags("hr", "position", "query"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.registerSkill"), async (_ctx, body) => cmdRegisterSkill(sdk, body), asAgentTool("Register skill/competency definition"), withCapabilityTags("hr", "skill", "competency"))
    .command(nsid("ai.gftd.app.kyber.hr.assignSkill"), async (_ctx, body) => cmdAssignSkill(sdk, body), asAgentTool("Assign skill proficiency to employee"), withCapabilityTags("hr", "skill", "talent"))
    .command(nsid("ai.gftd.app.kyber.hr.skillGapAnalysis"), async (_ctx, body) => cmdSkillGapAnalysis(sdk, body), asAgentTool("Analyze skill gaps between employee and position requirements"), withCapabilityTags("hr", "skill", "analytics"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.createEvaluation"), async (_ctx, body) => cmdCreateEvaluation(sdk, body), asAgentTool("Create performance evaluation with multi-dimensional scoring"), withCapabilityTags("hr", "evaluation", "performance"))
    .command(nsid("ai.gftd.app.kyber.hr.listEvaluations"), async (_ctx, body) => cmdListEvaluations(sdk, body), asAgentTool("List evaluations by employee or period"), withCapabilityTags("hr", "evaluation", "query"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.enrollTraining"), async (_ctx, body) => cmdEnrollTraining(sdk, body), asAgentTool("Enroll employee in training program"), withCapabilityTags("hr", "training", "development"))
    .command(nsid("ai.gftd.app.kyber.hr.completeTraining"), async (_ctx, body) => cmdCompleteTraining(sdk, body), asAgentTool("Record training completion with score and certificate"), withCapabilityTags("hr", "training", "development"));

  app
    .command(nsid("ai.gftd.app.kyber.hr.coverageStats"), async () => cmdCoverageStats(), asAgentTool("Kyber HR coverage statistics"), withCapabilityTags("coverage", "stats", "hr"));
