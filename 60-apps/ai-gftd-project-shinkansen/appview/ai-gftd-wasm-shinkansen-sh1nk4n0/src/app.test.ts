...
vi.mock("@gftd/magatama-host-sdk", () => ({
...
describe("Shinkansen Reservation Intelligence — Integration Tests", () => {
  ...
  describe("Queries", () => {
    it("listLines returns all 8 shinkansen lines", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.listLines");
    ...
    it("searchRoute returns routes array", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.searchRoute", {
    ...
    it("checkAvailability returns availability array", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.checkAvailability", {
    ...
    it("compareFare returns fares with cheapest", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.compareFare", {
        from: "東京", to: "新大阪", date: "2026-04-20",
      });
    ...
    it("getReservation returns not found for unknown ID", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.getReservation", {
    ...
    it("listReservations returns with offset/limit", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.listReservations", {});
    ...
    it("getOperation returns operations array", async () => {
      const result = await invokeQuery("ai.gftd.apps.shinkansen.getOperation", {});
  ...
  describe("Commands", () => {
    it("createReservation writes reservation record", async () => {
      const result = await invokeCommand("ai.gftd.apps.shinkansen.createReservation", {
        trainNumber: "のぞみ1号",
        departureStation: "東京",
        arrivalStation: "新大阪",
      ...
      const written = writtenRecords.find((r) => r.collection === "ai.gftd.apps.shinkansen.reservation");
    ...
    it("cancelReservation writes cancelled record", async () => {
      const result = await invokeCommand("ai.gftd.apps.shinkansen.cancelReservation", {
    ...
    it("selectSeat writes seat assignment", async () => {
      const result = await invokeCommand("ai.gftd.apps.shinkansen.selectSeat", {
        reservationId: "rsv-test123",
        carNumber: 7,
        seatNumber: "3A",
      });
    ...
    it("reportOperation writes operation status", async () => {
      const result = await invokeCommand("ai.gftd.apps.shinkansen.reportOperation", {
        lineId: "tokaido",
        status: "delay",
        detail: "強風のため15分遅延",
        cause: "strong wind",
      ...
      const written = writtenRecords.find((r) => r.collection === "ai.gftd.apps.shinkansen.operation");
      expect(written).toBeDefined();
      expect(written!.record.lineName).toBe("東海道新幹線");
    });

    it("seedTimetable writes timetable record", async () => {
      const result = await invokeCommand("ai.gftd.apps.shinkansen.seedTimetable", {
        lineId: "tohoku",
      });
      expect(result.lineId).toBe("tohoku");
      expect(result.status).toBe("seeded");
      const written = writtenRecords.find((r) => r.collection === "ai.gftd.apps.shinkansen.timetable");
      expect(written).toBeDefined();
  ...
  describe("Reactive Commit Handler", () => {
    it("calendar event with 出張 triggers social post", async () => {
      await commitFn(
        {
          action: "create",
          collection: "ai.gftd.apps.calendar.event",
    ...
    it("operation delay triggers social post", async () => {
      await commitFn(
        {
          action: "create",
          collection: "ai.gftd.apps.shinkansen.operation",
