<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      if (latest && firmDid) {
        lastConflictCheckRef = `at://${firmDid}/ai.gftd.apps.lawfirm.conflictCheck/${latest.rkey}`;
      }
    } finally {
      loading = false;
    }
  ...
  function onConflictClosed(res?: { rkey: string; result: ConflictResult; wallId?: string }) {
    showConflict = false;
    if (res && firmDid) {
      lastConflictCheckRef = `at://${firmDid}/ai.gftd.apps.lawfirm.conflictCheck/${res.rkey}`;
      load();
    }
  }
