...
export const didDepth = (did: string): number =>
  did.startsWith("did:gftd:") ? did.slice("did:gftd:".length).split(":").length : 0;

export const shortDid = (did: string, segs = 1): string => {
  if (!did.startsWith("did:gftd:")) return did;
  const parts = did.slice("did:gftd:".length).split(":");
  const take = parts.slice(-segs).map((p) => `${p.slice(0, 6)}…`).join(":");
  return `did:gftd:…:${take}`;
};
