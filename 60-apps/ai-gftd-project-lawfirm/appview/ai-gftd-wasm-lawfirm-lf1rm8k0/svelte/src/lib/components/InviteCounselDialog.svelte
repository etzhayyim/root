<script lang="ts">
  ...
  let conflictDetected = $derived(
    granteeDid.startsWith("did:gftd:") &&
  ...
  async function submit() {
    error = ""; success = null;
    if (!grantDidValid) { error = "granteeDid must be a depth-1 did:gftd root"; return; }
    ...
    >
      ...
      {:else}
        <div class="p-5 space-y-3">
          <div>
            <label class="text-xs font-medium" for="invite-counsel-grantee-did">Grantee DID (did:gftd root)</label>
            <input id="invite-counsel-grantee-did" class="w-full mt-1 rounded border border-neutral-300 dark:border-neutral-700 bg-transparent px-2 py-1 text-sm font-mono"
              placeholder="did:gftd:a1b2c3d4e5f6a1b2c3d4e5f6"
