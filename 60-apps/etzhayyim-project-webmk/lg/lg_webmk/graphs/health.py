...
async def _check_rw(state: _State) -> dict[str, Any]:
    if not _RW_URL:
    ...
    try:
        ...
        try:
            ...
            await conn.close()
        return {"rw_ok": True, "rw_latency_ms": int((time.monotonic() - t) * 1000)}
    except Exception as exc:  # noqa: BLE001
        return {"rw_ok": False, "error": f"rw: {exc!s}"[:200]}


async def _summarize(state: _State) -> dict[str, Any]:
    return {"ok": bool(state.get("rw_ok")), "server_now": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}


async def _audit(state: _State) -> dict[str, Any]:
    emit_audit_bg(actor=_APP_DID, activity="webmk.health.check", object_id=f"health:{int(time.time())}", object_type="webmk.health", attributes={"ok": state.get("ok", False)})
    return {}


def _build() -> StateGraph:
    g: StateGraph = StateGraph(_State)
    g.add_node("check_rw", _check_rw)
