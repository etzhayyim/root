...
// Per-actor identity (handle + entity list), generated by gen_rust_actor.py so
// each actor's module bytes — and thus its raw CID — are distinct. A default is
// committed so the crate builds standalone.
include!("actor_meta.rs");

#[no_mangle]
pub extern "C" fn actor_id() -> *const u8 { HANDLE.as_ptr() }
#[no_mangle]
pub extern "C" fn actor_id_len() -> usize { HANDLE.len() }
#[no_mangle]
pub extern "C" fn entity_count_meta() -> u64 { ENTITIES.len() as u64 }
