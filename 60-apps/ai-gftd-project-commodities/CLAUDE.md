...
- ISIN cross-link — `(:Commodity)-[:PRODUCED_BY]->(:Security {isin})` SQL edge (旧 `gftd:public-company` WIT は除去済み → `gftd:isin@1.0.0` + SQL graph に移行)
- `gftd:resource-flow@0.1.0` — Shared Sankey visualization patterns
