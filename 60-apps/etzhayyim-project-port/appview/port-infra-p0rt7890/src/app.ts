// Port Infrastructure thin edge. Business logic runs in AgentGateway MCP + pod-side LangServer.
...
const NSID_PREFIX = "com.etzhayyim.apps.port.";
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: "did:web:p0rt7890.etzhayyim.com",
        nanoid: env.APP_NANOID ?? "p0rt7890",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        businessLogic: "40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/ingest/port.py",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/port",
      });
    }
    ...
    return json({ error: "NotFound", message: "port not found" }, 404);
  },
