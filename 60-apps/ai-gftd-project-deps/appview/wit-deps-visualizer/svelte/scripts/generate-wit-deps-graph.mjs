...
const DOMAIN_META = {
	isco: { label: 'ISCO-08', witPkg: 'gftd:isco-*', description: 'Occupation performer agents', color: '#3b82f6' },
	isic: { label: 'ISIC Rev.4', witPkg: 'gftd:isic-*', description: 'Industry classification interfaces', color: '#10b981' },
	apqc: { label: 'APQC PCF', witPkg: 'gftd:apqc-*', description: 'Process classification framework', color: '#f59e0b' },
	tsukuru: { label: 'Tsukuru', witPkg: 'gftd:tsukuru*', description: 'B2B factory-direct marketplace', color: '#ef4444' },
	'tukuru-process': { label: 'Tukuru Process', witPkg: 'gftd:tukuru-process*', description: 'Manufacturing process lifecycle', color: '#ec4899' },
	bpmn: { label: 'BPMN', witPkg: 'gftd:bpmn-*', description: 'Cross-domain process definitions', color: '#8b5cf6' },
	resources: { label: 'Resources', witPkg: 'gftd:resources*', description: 'Entity/resource graph', color: '#06b6d4' },
	states: { label: 'States', witPkg: 'gftd:states*', description: 'Government/state domain', color: '#0ea5e9' },
	cpc: { label: 'CPC', witPkg: 'gftd:cpc*', description: 'Central Product Classification domain', color: '#22c55e' },
	unispsc: { label: 'UniSPSC', witPkg: 'gftd:unispsc*', description: 'UniSPSC taxonomy/classification domain', color: '#fb7185' },
...
const PKG_DOMAIN_MAP = {
	...
	'gftd:isic': 'isic',
	'gftd:apqc': 'apqc',
	'gftd:tsukuru': 'tsukuru',
	'gftd:tukuru-process': 'tukuru-process',
	'gftd:supply-chain': 'isic',
	'gftd:bpmn': 'bpmn',
	'gftd:cpc': 'cpc',
	'gftd:unispc': 'unispsc',
	'gftd:unispsc': 'unispsc',
};
...
function main() {
	...
	// 7. Parse legacy gftd:platform worlds
...
function buildDomainGraph(graph) {
	...
	const grpcDeps = [
		{ source: 'tsukuru', target: 'resources', kind: 'grpc-call', interfaceRef: 'gftd.supply_company.v1.SupplyCompanyService/CreateResource', description: 'Supplier sync: RegisterSupplier → CreateResource' },
		{ source: 'tsukuru', target: 'isic', kind: 'conversation-discover', interfaceRef: 'gftd:isic-c/*', description: 'ISIC Section C manufacturing coordinators' },
...
function resolveDomain(ref) {
	...
	const pkgMatch = String(ref).match(/^gftd:([a-z0-9-]+)/i);
	if (pkgMatch) {
...
function readDeployMetadata(componentDir, componentId) {
	const gftdPath = path.join(componentDir, 'gftd.json');
	if (!fs.existsSync(gftdPath)) {
		return {
			appId: null,
			appName: null,
			runtime: null,
	...
	try {
		const raw = JSON.parse(fs.readFileSync(gftdPath, 'utf8'));
...
function loadStatesRegistry() {
	const registryPath = path.join(PROJECTS_ROOT, 'ai-gftd-project-states', 'tools', 'component-registry.json');
	try {
...
function makeStatesStageRef(stage, countryCode) {
	return `gftd:states-${stage}@0.1.0/organization-directory`;
}
...
function augmentStatesHierarchyComponents(components, statesRegistry) {
	...
	for (const component of components) {
		if (component.project !== 'ai-gftd-project-states') continue;
...
function isHostProvidedImport(ref, runtimeImportSet) {
	...
	// host-provided. Project-local gftd:* refs must resolve through explicit
...
function isHostProvidedRequire(req, ref) {
	...
	if (req?.package === 'gftd:governance@0.1.0') return true;
...
function computeAppWitDefinitionMetrics(components, linksByComponent) {
	...
	for (const component of components ?? []) {
		...
		const hostBridgedDomain = resolved.filter(link => link.providerProject === '__runtime__' && (link.ref ?? '').startsWith('gftd:'));
...
function normalizeRef(raw) {
	...
	const legacyAliasMap = new Map([
		['gftd:workflow/workflow@0.1.0', 'magatama:workflow/workflow@1.0.0'],
		['gftd:activity/activity@0.1.0', 'magatama:workflow/activity@1.0.0'],
		['gftd:crawler@0.1.0/crawl-query', 'gftd:crawler@0.1.0/crawler'],
		['gftd:india/district-registry@0.1.0', 'gftd:states-country@0.1.0/organization-directory'],
		['gftd:india/passport-seva@0.1.0', 'gftd:states-country@0.1.0/organization-directory'],
		['gftd:india/court-services@0.1.0', 'gftd:states-admin@0.1.0/organization-directory'],
		['gftd:india/revenue-services@0.1.0', 'gftd:states-admin@0.1.0/organization-directory'],
		['gftd:india/driving-license@0.1.0', 'gftd:states-admin@0.1.0/organization-directory'],
		['gftd:india/land-registration@0.1.0', 'gftd:states-admin@0.1.0/organization-directory'],
		['gftd:india/electricity-services@0.1.0', 'gftd:states-admin@0.1.0/organization-directory'],
		['gftd:india/municipal-services@0.1.0', 'gftd:states-municipality@0.1.0/organization-directory'],
	]);
