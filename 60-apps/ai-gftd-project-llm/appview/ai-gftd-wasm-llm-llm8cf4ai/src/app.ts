...
const ANSWER_WITH_KNOWLEDGE_NSID = "ai.gftd.apps.llm.answerWithKnowledge";
...
// ── Workers AI Model Registry (SSoT: @gftd/magatama-host-sdk llm-model-registry) ──
...
export function handleComAtprotoSyncSubscribeReposCommit(sdk: HostSDK, commit: ComAtprotoSyncSubscribeReposCommit): { ok: boolean; detail: string } {
  ...
  if (commit.collection === "ai.gftd.apps.llm.inferenceRequest") {
...
async function handleOpenAIPath(request: Request): Promise<globalThis.Response | null> {
  ...
  if (modelMatch && request.method === "GET") {
    ...
    if (modelId === "gemma4-runpod" || modelId === "tier0-runpod") {
      return json({ id: modelId, object: "model", created: 1711929600, owned_by: "gftd", permission: [], root: "gemma-4-e4b-it", parent: null });
    }
    ...
    return json({ id: modelId, object: "model", created: 1711929600, owned_by: "gftd", permission: [], root: def.cfModel, parent: null });
  }
...
const _inner = createWorkerExportFromEnvFactory((env) => {
  ...
  _env = env;
  sdk.app
    .command(nsid("ai.gftd.apps.llm.converse"), (ctx, body) => cmdConverse(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.llm.chatCompletions"), (ctx, body) => cmdChatCompletions(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.llm.listModels"), (ctx, body) => cmdListModels(sdk, body),
      asAgentTool("List available Workers AI models and their capabilities"),
      withCapabilityTags("llm", "models", "catalog"),
    )
    .command(nsid("ai.gftd.apps.llm.recommendModel"), (ctx, body) => cmdRecommendModel(sdk, body),
      asAgentTool("Recommend optimal Workers AI model for a use case"),
      withCapabilityTags("llm", "models", "recommendation"),
    )
    .command(nsid("ai.gftd.apps.llm.healthCheck"), (ctx, body) => cmdHealthCheck(sdk, body),
      asAgentTool("Health check all Workers AI model endpoints"),
      withCapabilityTags("llm", "health", "monitoring"),
    )
    .command(nsid("ai.gftd.apps.llm.verifyCellerAi"), (ctx, body) => cmdVerifyCellerAi(sdk, body),
...
export default {
  async fetch(request: Request, env: Record<string, unknown>, ctx?: { waitUntil(p: Promise<unknown>): void }) {
    ...
    const isInferenceNsid = nsid === "ai.gftd.apps.llm.converse" || nsid === "ai.gftd.apps.llm.chatCompletions";
