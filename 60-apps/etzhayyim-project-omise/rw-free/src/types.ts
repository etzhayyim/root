/**
 * omise rw-free — record types.
 *
 * Tier-2 function-split per ADR-2606011400 (on-chain-only). Storefront:
 * products + orders on AT PDS records; orders settle on-chain (USDC + ERC-4337 +
 * TitheRouter 10% Public-Fund split). No Stripe, no RW. ADR-2605172000 RW-free.
 *
 * Amounts are USDC base units (micros) as decimal STRINGS.
 *
 * Identity hierarchy:
 *   did:web:omise.etzhayyim.com                       — controller
 *   did:web:omise.etzhayyim.com:product:{sku}         — a product
 *   did:web:omise.etzhayyim.com:order:{orderId}       — an order
 */
...
export function productDid(sku: string): string {
  return `${OMISE_DID_PREFIX}product:${sku.toLowerCase()}`;
}
...
export function orderDid(orderId: string): string {
  return `${OMISE_DID_PREFIX}order:${orderId.toLowerCase()}`;
}
