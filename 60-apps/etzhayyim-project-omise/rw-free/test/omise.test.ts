...
const fakeSettle: SettlementExecutor = async () => ({ txHash: "0xomise" });

describe("omise rw-free", () => {
  let e: any;
  beforeEach(() => {
    e = new MockEtzhayyim({ did: "did:web:omise.etzhayyim.com" });
  });
  ...
  describe("catalog", () => {
    ...
    it("publishes + gets + lists", async () => {
      ...
      expect(got.product?.did).toContain("omise.etzhayyim.com:product:sku-1");
