> **DEPRECATED**: Actor migrated to `20-actors/jp-ashiba/actor-manifest.jsonld` (T1 MCP-Compose). This project wasm/*/src/app.ts is retained as T3 fallback only.
...
- **Write**: `sdk.pds.dispatch({ type: "com.atproto.repo.createRecord", collection: kind, record: payload })` → PDS → kagami
