...
Caller (kaisya/lawfirm/agent) 
  ↓ XRPC /xrpc/ai.gftd.apps.yuubin.composeAndPost
yuubin actor
...
| NSID | Tier | 用途 |
|---|---|---|
| `ai.gftd.apps.yuubin.composeAndPost` | 1 | 通常便/レターパック自動投函 |
| `ai.gftd.apps.yuubin.submitNaiyoShomei` | 1 | 電子内容証明 |
| `ai.gftd.apps.yuubin.confirmManualPost` | 1 | manual handoff 完了記録 |
| `ai.gftd.apps.yuubin.uploadDocument` | 2 | PDF base64 → B2 content-addressed |
...
| .docx A4 | 「ファイルのページサイズがA4(210x297,297x210)ではなかった」 | `scripts/normalize-a4-docx.sh` (Mac-local) or `ai.gftd.apps.yuubin.normalizeDocx` XRPC (CF Worker runtime) |
