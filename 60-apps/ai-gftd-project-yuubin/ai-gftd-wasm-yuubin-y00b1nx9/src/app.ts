...
function write(sdk: HostSDK, kind: string, rec: Record<string, unknown>): void {
  const collection = `ai.gftd.apps.yuubin.${kind}`;
  const enriched = {
    ...rec,
    createdAt: nowISO(),
    org_id: "gftd.co.jp",
    ...
    if (doNs) {
      ...
      try {
        ...
        return {
          ...
          hint: "Poll ai.gftd.apps.yuubin.getTxStatus or postalItemUpdate for final state. DO alarm runs the puppeteer flow.",
        };
      } catch (e) {
      ...
      const r = await fetch("https://mailer.etzhayyim.com/xrpc/ai.gftd.apps.mailer.send", {
        ...
        body: JSON.stringify({
          ...
          bodyHtml:
            ...
            `<p>投函完了後、ai.gftd.apps.yuubin.confirmManualPost({ txId: "${txId}", apptNumber, paidAt }) で記録更新してください。</p>`,
          importance: "high",
        }),
      });
      if (r.ok) {
  ...
  return {
    ...
    humanInstructions: {
      ...
      steps: [
        ...
        `Capture 申込番号`,
        `After completion, call ai.gftd.apps.yuubin.confirmManualPost({ txId: "${txId}", apptNumber, paidAt })`,
      ],
    },
  };
}
...
async function cmdGetTxStatus(sdk: HostSDK, body: Uint8Array) {
  ...
  return {
    ok: true,
    txId,
    hint: "Read ai.gftd.apps.yuubin.postalItem + postalItemUpdate AT Records filtered by txId. Final status lands in postalItemUpdate.",
  };
}
...
export default createWorkerExport((sdk) => {
  ...
  sdk.app
    ...
    .command(nsid("ai.gftd.apps.yuubin.composeAndPost"), async (_c, b) => cmdComposeAndPost(sdk, b),
    ...
    .command(nsid("ai.gftd.apps.yuubin.submitNaiyoShomei"), async (_c, b) => cmdSubmitNaiyoShomei(sdk, b),
    ...
    .command(nsid("ai.gftd.apps.yuubin.confirmManualPost"), async (_c, b) => cmdConfirmManualPost(sdk, b),
    ...
    .command(nsid("ai.gftd.apps.yuubin.getTxStatus"), async (_c, b) => cmdGetTxStatus(sdk, b),
    ...
    // Tier 2 (helpers)
    .command(nsid("ai.gftd.apps.yuubin.uploadDocument"), async (_c, b) => cmdUploadDocument(sdk, b),
    ...
    .command(nsid("ai.gftd.apps.yuubin.normalizeDocx"), async (_c, b) => cmdNormalizeDocx(sdk, b),
    ...
    .command(nsid("ai.gftd.apps.yuubin.validatePdf"), async (_c, b) => cmdValidatePdf(sdk, b),
