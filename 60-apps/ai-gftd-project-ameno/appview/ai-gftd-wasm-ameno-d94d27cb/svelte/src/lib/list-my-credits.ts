/**
 * list-my-credits.ts — client helper for app.etzhayyim.apps.ameno.listMyCredits.
 *
...
export async function listMyCredits(actorDid: string): Promise<MyCreditsResponse> {
  ...
  const res = await fetch(`/xrpc/app.etzhayyim.apps.ameno.listMyCredits?${qs.toString()}`, {
    method: "GET",
