// outlook.gftd.ai thin edge facade. OAuth and Microsoft Graph sync run in AgentGateway MCP + pod-side LangServer.
...
const APP = "outlook";
const NSID_PREFIX = "ai.gftd.apps.outlook.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:outlook.gftd.ai", nanoid: env.APP_NANOID ?? "outlook", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "20-actors/magatama/py/src/pymagatama/ingest/outlook.py", bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/outlook" });
