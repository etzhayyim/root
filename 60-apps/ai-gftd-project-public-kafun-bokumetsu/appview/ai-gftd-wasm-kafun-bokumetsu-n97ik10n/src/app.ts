 ...
 * (ADR-0019).
 *
 *   ai.gftd.apps.kafun.agent.research → kafun.research.v1 → vertex_kafun_research[]
 *   ai.gftd.apps.kafun.agent.think    → kafun.think.v1    → vertex_kafun_insight + vertex_kafun_proposal[]
 *   ai.gftd.apps.kafun.agent.tick     → kafun.tick.v1     → vertex_kafun_action  (+ optional follow-on)
 *
...
async function cmdResearch(sdk: HostSDK, body: Uint8Array): Promise<string> {
  ...
  const out = await dispatch("ai.gftd.apps.kafun.agent.research", { category, query });
...
async function cmdThink(sdk: HostSDK, body: Uint8Array): Promise<string> {
  ...
  const out = await dispatch("ai.gftd.apps.kafun.agent.think", { seed_findings });
...
async function cmdTick(sdk: HostSDK): Promise<string> {
  ...
  if (topo) {
    ...
    const action: AnyRow = {
      vertex_id:        `at://${EXECUTOR_DID}/ai.gftd.apps.kafun.action/${rkey}`,
  ...
  const out = await dispatch("ai.gftd.apps.kafun.agent.tick", { pending_proposals: pending, fund_balance_jpy });
...
export default createWorkerExport((sdk: HostSDK) => {
  ...
  sdk.app
    .command(
      nsid("ai.gftd.apps.kafun.agent.research"),
    ...
    .command(
      nsid("ai.gftd.apps.kafun.agent.think"),
    ...
    .command(
      nsid("ai.gftd.apps.kafun.agent.tick"),
