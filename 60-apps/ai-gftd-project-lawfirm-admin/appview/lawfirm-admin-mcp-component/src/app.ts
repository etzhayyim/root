...
// - Package import `@gftd/magatama-host-sdk` not used here (this file is a vanilla Worker handler).
//
...
// 8 methods: createCase / updateCase / listCases / getCase /
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "ujv6oh9s",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "60-apps/ai-gftd-project-lawfirm-admin/bpmn",
        methods: [
          "createCase", "updateCase", "listCases", "getCase",
          "createClient", "updateClient", "listClients", "getClient",
        ],
      });
    }
