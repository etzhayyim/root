"""RisingWave-compatible LangGraph checkpoint saver (mirrors lg-jukyu pattern).

RW PG :4566 does not support INSERT ... ON CONFLICT DO UPDATE.
We use delete-then-insert for checkpoint_writes.
"""
...
async def build_checkpointer() -> AsyncIterator[Any]:
    ...
    if not CHECKPOINTER_URL:
        raise RuntimeError("LG_CHECKPOINTER_URL or RW_URL must be set")

    class _RwSaver(AsyncPostgresSaver):
        ...
        async def aput(self, config: dict, checkpoint: Any, metadata: Any, new_versions: dict) -> dict:
            ...
            async with self._cursor() as cur:  # type: ignore[attr-defined]
                for channel, version in new_versions.items():
                    ...
                    val_type, val_blob = ("empty", b"") if val is None else self.serde.dumps_typed(val)
                    await cur.execute(_RW_INSERT_BLOB, (thread_id, checkpoint_ns, channel, str(version), val_type, val_blob))
                await cur.execute(_RW_INSERT_CHECKPOINT, (thread_id, checkpoint_ns, checkpoint_id, parent_id, "msgpack", ckpt_blob, meta_blob))
        ...
        async def aput_writes(self, config: dict, writes: list, task_id: str, task_path: str = "") -> None:
            cfg = config["configurable"]
            thread_id, checkpoint_ns, checkpoint_id = cfg["thread_id"], cfg.get("checkpoint_ns", ""), cfg["checkpoint_id"]
            async with self._cursor() as cur:  # type: ignore[attr-defined]
                for idx, (channel, value) in enumerate(writes):
                    val_type, val_blob = self.serde.dumps_typed(value)
                    await cur.execute(_RW_DELETE_WRITES, (thread_id, checkpoint_ns, checkpoint_id, task_id, idx))
                    await cur.execute(_RW_INSERT_WRITES, (thread_id, checkpoint_ns, checkpoint_id, task_id, idx, channel, val_type, val_blob, task_path))

    async with _RwSaver.from_conn_string(CHECKPOINTER_URL) as cp:
        yield cp
