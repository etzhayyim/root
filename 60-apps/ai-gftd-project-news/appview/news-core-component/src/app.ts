...
const INTEL_SOURCES: IntelSource[] = [
  {
    id: "social-arbitrage",
    name: "GFTD Social Arbitrage Intel",
    region: "global",
    country: "multi",
    topics: [
      "social-arbitrage",
...
async function dispatchLiveAudioIngest(
  ...
  const res = await fetch(`${baseUrl}/xrpc/ai.gftd.apps.news.liveAudioIngest`, {
    method: "POST",
    headers,
...
async function exportNewsEntitiesToMaps(
  ...
  for (const entity of candidates) {
    ...
    try {
      const res = await fetch(`${baseUrl}/xrpc/ai.gftd.apps.maps.spatialEventRecord`, {
        method: "POST",
        headers,
        body: JSON.stringify(payload),
      });
...
async function cmdIngest(_sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.news.ingest", body);
...
async function cmdLiveAudioIngest(_sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.news.liveAudioIngest", body);
  ...
  return {
    ...
    taskTypes: [
      ...
      "generic.llm.json",
      "xrpc.ai.gftd.apps.news.analyzeIntel",
      "xrpc.ai.gftd.apps.news.publishIntel",
      "generic.audit.emit",
    ],
    sourceId,
    sourceName,
...
async function cmdRegisterLiveAudioSource(
  ...
  try {
    record = liveAudioRecordFromArgs(
      parseLexiconInput("ai.gftd.apps.news.registerLiveAudioSource", body)
    );
  } catch (error) {
  ...
  try {
    await sdk.pds.comAtprotoRepoPutRecord(
      "ai.gftd.apps.news.liveAudioSource",
      rkey,
      {
        $type: "ai.gftd.apps.news.liveAudioSource",
        ...record,
      }
    );
  } catch (error) {
...
async function cmdCommitArticle(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.commitArticle", body);
  const sourceId = str(args.sourceId ?? "").trim();
  const sourceName = str(args.sourceName ?? sourceId).trim();
  ...
  try {
    await sdk.pds.comAtprotoRepoPutRecord(
      "ai.gftd.apps.news.article",
      rkey,
      {
        $type: "ai.gftd.apps.news.article",
...
async function cmdListArticles(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.listArticles", body);
...
async function cmdGetArticle(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.getArticle", body);
...
async function cmdListLiveAudioSources(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.listLiveAudioSources", body);
  const limit = clampInt(args.limit, 50, 1, 100);
  const status = str(args.status ?? "").toLowerCase();
  const region = str(args.region ?? "").toLowerCase();
  const topic = str(args.topic ?? "").toLowerCase();
  try {
    const result = await sdk.pds.listRecords("ai.gftd.apps.news.liveAudioSource", {
      limit,
...
async function cmdAuditLiveAudioPolicies(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.auditLiveAudioPolicies", body);
...
async function cmdScheduleLiveAudioIngest(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.scheduleLiveAudioIngest", body);
  const sourceId = registryKey(str(args.sourceId ?? ""));
  const region = str(args.region ?? "").toLowerCase();
  const topic = str(args.topic ?? "").toLowerCase();
  const limit = clampInt(args.limit, 50, 1, 100);
  ...
  try {
    const [sourceResult, stateResult] = await Promise.all([
      sdk.pds.listRecords("ai.gftd.apps.news.liveAudioSource", {
        limit,
        reverse: true,
      }),
      sdk.pds.listRecords("ai.gftd.apps.news.liveAudioScheduleState", {
        limit: 100,
        reverse: true,
      }),
    ]);
    ...
    for (const source of sources) {
      ...
      if (!dryRun) {
        await sdk.pds.comAtprotoRepoPutRecord(
          "ai.gftd.apps.news.liveAudioScheduleState",
          id,
          {
            $type: "ai.gftd.apps.news.liveAudioScheduleState",
            ...nextState,
          }
        );
      }
...
async function cmdAnalyzeIntel(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.analyzeIntel", body);
  const title = str(args.title ?? "").trim();
  ...
  try {
    await (sdk.env as any).PDS_RPC.comAtprotoRepoPutRecord(
      sdk.pds.selfRepo,
      "ai.gftd.apps.intel.report",
      id.replace(/[^a-zA-Z0-9-]/g, "").slice(0, 64),
      {
        $type: "ai.gftd.apps.intel.report",
        title: report.title,
...
async function cmdPublishIntel(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.publishIntel", body);
  const title = str(args.title ?? "").trim();
  const sourceUrl = str(args.url ?? "").trim();
  if (!title || !sourceUrl)
...
async function cmdArticleDiag(
  ...
  const args = parseLexiconInput("ai.gftd.apps.news.articleDiag", body);
...
async function cmdPdsDiag(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.news.pdsDiag", body);
  ...
  for (const [key, repo] of [
    ...
    try {
      const result = await sdk.pds.listRecords("ai.gftd.apps.news.article", {
        repo,
        limit: 3,
      });
...
function handleComAtprotoSyncSubscribeReposCommit(
  ...
  if (collection === "ai.gftd.apps.news.article") {
...
const _workerInner = createWorkerExport((sdk) => {
  sdk.app
    ...
    .command(
      nsid("ai.gftd.apps.news.liveAudioIngest"),
    ...
    .command(
      nsid("ai.gftd.apps.news.registerLiveAudioSource"),
    ...
    .command(
      nsid("ai.gftd.apps.news.scheduleLiveAudioIngest"),
    ...
    .command(
      nsid("ai.gftd.apps.news.listArticles"),
    ...
    .query(nsid("ai.gftd.apps.news.listIntelSources"), (_ctx, body) =>
      cmdListIntelSources(sdk, body)
    )
    .query(nsid("ai.gftd.apps.news.listLiveAudioSources"), (_ctx, body) =>
      cmdListLiveAudioSources(sdk, body)
    )
    .query(nsid("ai.gftd.apps.news.auditLiveAudioPolicies"), (_ctx, body) =>
      cmdAuditLiveAudioPolicies(sdk, body)
    )
    .command(
      nsid("ai.gftd.apps.news.analyzeIntel"),
    ...
    .command(
      nsid("ai.gftd.apps.news.articleDiag"),
    ...
    .command(
      nsid("ai.gftd.apps.news.pdsDiag"),
