// Analyze coverage by function, component, and route
...
function analyzeCoverage() {
  ...
  const functions: Array<{ file: string; data: any }> = [];
  ...
  // Function-level coverage (for key files)
  console.log('=== Function-Level Coverage (Key Files) ===\n');
  ...
  for (const { file } of keyFiles) {
    const fullPath = join(process.cwd(), file);
    if (existsSync(fullPath)) {
      const funcs = extractFunctions(fullPath);
      const data = coverage[file];
      if (data && funcs.length > 0) {
        console.log(`${file}:`);
        console.log(`  Coverage: ${data.lines.pct.toFixed(2)}% lines, ${data.functions.pct.toFixed(2)}% functions`);
        console.log(`  Functions: ${funcs.slice(0, 10).join(', ')}${funcs.length > 10 ? '...' : ''}`);
        console.log('');
      }
    }
  }
