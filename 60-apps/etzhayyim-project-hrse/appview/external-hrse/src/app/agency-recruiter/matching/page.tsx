...
/**
 * @etzhayyim/etzhayyim-hrse#AgencyRecruiterMatchingConnect
 * エージェンシー所属リクルーター向けマッチング結果ページ（Connect-Web版）
 */
...
import { ListMatchingResultsForRecruiterRequestSchema } from "@/gen/proto/hrse/v1/matching_pb";

export default function AgencyRecruiterMatchingPage() {
	...
	const fetchMatchingResults = useCallback(async () => {
		...
		setLoading(true);
		try {
			const res = await matchingClient.listMatchingResultsForRecruiter(
				create(ListMatchingResultsForRecruiterRequestSchema, {
			...
			if (filterNotified !== undefined) {
				results = results.filter((r) => filterNotified ? r.notifiedAt : !r.notifiedAt);
			}
	...
	}, [recruiter?.id, matchingClient, filterNotified, page]);
	...
	}, [recruiter?.id, fetchMatchingResults]);
				...
				<div className="mx-auto max-w-6xl">
					<div className="rounded-lg bg-white p-8 shadow dark:bg-neutral-900">
						<p className="text-neutral-600 dark:text-neutral-400">
							リクルータープロファイルが作成されていません。プロファイルを作成してください。
						</p>
						<TouchOptimizedButton
							onClick={() => {
								const profileUrl = organization?.id
									? `/${organization.id}/recruiter/profile`
									: "/recruiter/profile";
			...
			<div className="mx-auto max-w-6xl">
									...
									<div className="flex-1">
										<div className="mb-2 flex items-center gap-4">
											...
											<div className="flex items-center gap-2">
												...
												<span className="text-sm font-medium text-green-700 dark:text-green-500">マッチ度</span>
											</div>
										</div>
										<p className="mb-2 text-neutral-600 dark:text-neutral-400">{result.job?.description ?? ""}</p>
										...
										{result.semanticExplanation && (
											<div className="mt-4 rounded-lg bg-green-50 p-4 dark:bg-green-900/20">
												<p className="text-sm text-green-800 dark:text-green-400">{result.semanticExplanation}</p>
											</div>
										)}
									</div>
				...
				{matchingResults.length > 0 && (
					<div className="mt-8 flex justify-center gap-4">
						...
						<span className="flex items-center text-neutral-600 dark:text-neutral-400">ページ {page}</span>
						<TouchOptimizedButton
							variant="outline"
