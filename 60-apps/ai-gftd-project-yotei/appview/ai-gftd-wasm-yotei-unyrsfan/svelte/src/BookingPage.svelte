<script lang="ts">
  ...
  async function propose(): Promise<void> {
    ...
    submitting = true;
    result = '';
    const res = await xrpc<{ id: string; status: string }>('ai.gftd.apps.yotei.proposeBooking', {
      calendarId,
      requesterDid,
      durationMin,
      message,
  ...
  async function loadBookings(): Promise<void> {
    ...
    const res = await xrpc<{ bookings: BookingItem[] }>('ai.gftd.apps.yotei.listBookings', { calendarId });
