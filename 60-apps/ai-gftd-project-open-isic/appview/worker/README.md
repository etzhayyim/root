...
`/xrpc/ai.gftd.apps.isic.*` to the in-cluster `lg-open-isic` langserver.
...
```
POST /xrpc/ai.gftd.apps.isic.classify                # description -> top-K
POST /xrpc/ai.gftd.apps.isic.hierarchicalClassify    # section -> ... -> class
POST /xrpc/ai.gftd.apps.isic.invokeAgent             # classCode -> agent
GET  /xrpc/ai.gftd.apps.isic.listAgents              # paged registry
GET  /xrpc/ai.gftd.apps.isic.health                  # lexicon health
