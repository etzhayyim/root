...
- `open_isic_hierarchical_classify`: Dynamic LLM drill-down from Section -> Division -> Group -> Class using the `ai.gftd.apps.openIsic.getTaxonomy` tool.
...
```text
ai.gftd.apps.openIsic.verificationForConfidence
ai.gftd.apps.openIsic.classificationVertexId
```
