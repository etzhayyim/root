/**
 * kyber-qzzg06nh rw-free — entity + event + report knowledge-graph registries
 * + coverage. AT PDS records (no RW). Events FK-reference an existing entity;
 * reports may optionally FK-reference one. Public graph data only.
 */
...
export async function coverage(e: Etzhayyim, input: CoverageInput = {}): Promise<CoverageOutput> {
  ...
  const entityCount = await scanAll<EntityRecord>(e, ENTITY_COLLECTION, maxScan, (v) => {
    entitiesByKind[v.kind] = (entitiesByKind[v.kind] ?? 0) + 1;
  });
  const eventCount = await scanAll<EventRecord>(e, EVENT_COLLECTION, maxScan, () => {});
  const reportsByStatus: Record<string, number> = {};
  const reportCount = await scanAll<ReportRecord>(e, REPORT_COLLECTION, maxScan, (v) => {
