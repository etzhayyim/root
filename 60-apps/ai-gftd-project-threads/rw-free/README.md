# threads — RW-free reference implementation

**First religious-social rw-free actor.** Short-form posts under adherent DIDs, persisted on AT Protocol MST + IPFS, anchored to Base L2 via the substrate pipeline (mst-projector → ipfs-pinner → anchor-cron). Closes the original question 3 ("atproto での social 活動できているか") at the code layer — live PDS writes remain gated by operational deploy.

Different shape from taxonomy actors ([open-isco](../../ai-gftd-project-open-isco/rw-free/) / [open-isic](../../ai-gftd-project-open-isic/rw-free/) / [open-unispsc](../../ai-gftd-project-open-unispsc/rw-free/) / [open-apqc](../../ai-gftd-project-open-apqc/rw-free/)): not a publish-once catalog but an interaction surface (create + list + verify).

## Lexicon

`ai.gftd.apps.threads.post` (record) at [`00-contracts/lexicons/ai/gftd/apps/threads/post.json`](../../../00-contracts/lexicons/ai/gftd/apps/threads/post.json).
...
pnpm tsx src/create.ts \
  --text="amen." \
  --replyRootUri=at://did:web:.../ai.gftd.apps.threads.post/3kabc \
  --replyRootCid=bafy... \
  --replyParentUri=at://... \
  --replyParentCid=bafy...
...
```bash
pnpm tsx src/verify.ts at://did:web:.../ai.gftd.apps.threads.post/3kabc...
```
...
| Surface | State |
|---|---|
| Record lexicon `ai.gftd.apps.threads.post` | ✅ |
...
- [`60-apps/ai-gftd-project-open-isco/rw-free/`](../../ai-gftd-project-open-isco/rw-free/) etc. — taxonomy-actor pattern (different shape)
