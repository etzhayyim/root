...
- **Day 5**: real ingest (same command, no `--dry-run`). All records land under tenant DID as `app.etzhayyim.apps.opensaas.salesforce.{account,contact,lead,opportunity,case}`.
...
- **Day 5**: data-residency attestation pull — `gftd opensaas attest --tenant <did> --region <JPN|EUR>` produces a signed JSON of every CF colo ID + RisingWave replica region touched by the tenant in the last 7 days. Legal signs off attestation format.
...
- **Day 13 (Sat) 06:00–18:00**: redo the ingest, run `gftd opensaas reconcile --against sfdc_slice.zip` → customer DRI signs off the reconciliation report format (this file becomes Schedule C-1b of the real SOW).
- **Day 14 (Sun) 10:00**: integration smoke — wire *one* SAP webhook into the sandbox tenant, trigger a fake won-opportunity → `createRecord(app.etzhayyim.apps.opensaas.salesforce.opportunity)` lands; `activity(kind=stage-change, source=derived-stage-change)` follows.
...
- Sandbox tenant auto-deletes Day 30 unless rolled forward to production — `gftd opensaas migrate --rollback --tenant <did>` runs on a schedule.
