...
- **Day 14 (Sun) 10:00**: integration smoke — wire *one* SAP webhook into the sandbox tenant, trigger a fake won-opportunity → `createRecord(ai.gftd.apps.opensaas.salesforce.opportunity)` lands; `activity(kind=stage-change, source=derived-stage-change)` follows.
