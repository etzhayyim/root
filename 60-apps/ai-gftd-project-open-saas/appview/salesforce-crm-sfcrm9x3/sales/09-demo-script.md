...
4. Click the new Activity row → "View source commit" opens `/at/democo.opensaas.etzhayyim.com/ai.gftd.apps.opensaas.salesforce.activity/<rkey>`. Show the AT URI in the browser; copy-paste it to a curl in the terminal to show anyone-who-has-the-URI can verify the record (federation-grade).
