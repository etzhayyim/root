...
## Seeded tenant
Before the call:
- Spin up `did:web:democo.opensaas.gftd.ai` (SE does this once, re-used across demos; auto-rollback nightly via `migrate --rollback --older-than 24h`).
...
- Pre-bind one seat DID `did:web:democo.opensaas.gftd.ai:seat:ae-01` to a passkey on the SE's laptop.
...
Steps:
1. Open `https://salesforce.opensaas.gftd.ai/` in a fresh incognito. Login → WebAuthn passkey prompt → seat DID activates.
2. In a second browser, open `https://democo.opensaas.gftd.ai/` (same code, different tenant DID routing). Show the `did.json` at `/.well-known/did/web/democo.opensaas.gftd.ai` — explain this is the tenant's identity doc, signed by a key the tenant could in principle custody themselves.
3. Terminal side-by-side: `curl https://salesforce.opensaas.gftd.ai/_app/meta | jq .tenant` vs the democo one. Different tenant DID, identical Worker build.

Gate: "If we spun up `<prospect>.opensaas.gftd.ai` right now, you'd see your own subdomain resolve within 3 minutes and zero cross-tenant data leakage is the default, not a setting." → wait for nod.
...
Steps:
1. Open the pipeline view → it renders from `listPipeline({tenantDid: "did:web:democo.opensaas.gftd.ai"})`. Hover a stage bucket → shows `stageRollup[stage].weightedAmountJpy`. Note: this is a streaming MV; latency <100ms.
...
4. Click the new Activity row → "View source commit" opens `/at/democo.opensaas.gftd.ai/ai.gftd.apps.opensaas.salesforce.activity/<rkey>`. Show the AT URI in the browser; copy-paste it to a curl in the terminal to show anyone-who-has-the-URI can verify the record (federation-grade).
...
Steps:
1. Terminal: `gftd opensaas attest --tenant did:web:democo.opensaas.gftd.ai --region JPN` → signed JSON lists: CF colo IDs, RisingWave replica region, vault KMS region. All JPN.
...
5. Terminal: `curl https://atproto.gftd.ai/xrpc/com.atproto.repo.getRecord?uri=at://...` on the activity → shows the attestation is content-addressed, immutable.
...
2. DevTools: show the XRPC call goes to a handler that invokes `magatama.Invoke(murakumoFleetDid, "summariseOpportunity", { uri, windowDays: 90 })`. The caller identity in the invoke is the seat DID `did:web:democo.opensaas.gftd.ai:seat:ae-01`, not a shared API key.
3. LLM returns a 4-paragraph summary pulled from `opportunity` + its linked `activity` + the account's 3 most recent `case` records.
4. Toggle: switch `ConfigPut fleet=did:web:murakumo.gftd.ai:fleet:m4`. Rerun. Different model, same call. Show the audit log entry: `activity(kind=note, source=derived-convo)` with `actorDid=<seat>`, `convoUri=<murakumo-invocation>`.
...
3. Show the audit trail in the operator console: the revoke + re-issue are two `ai.gftd.audit.*` events, cross-referenceable to the seat DID's `activity` history.
...
- A public demo-tenant URL (`https://democo.opensaas.gftd.ai/`) with safe-for-sharing seed data lets prospects self-serve act 1 + act 2 before even booking a call.
