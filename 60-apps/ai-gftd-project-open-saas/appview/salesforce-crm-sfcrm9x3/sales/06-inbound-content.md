...
- **Primary surface**: `https://salesforce.opensaas.gftd.ai/` root docs + `/at/` deep-links to example records so every post can link to a *live* lexicon-shaped artifact.
- **Secondary surface**: `atproto.gftd.ai` blog (AT Record-native; every post is itself `app.bsky.feed.post` + a `ai.gftd.apps.opensaas.site.article` record, dog-fooding the protocol).
...
3. How the `derive` rule in `magatama.jsonld` emits `ai.gftd.apps.opensaas.salesforce.activity` with `source=derived-stage-change`, and the commit CID is embedded. Link to a live record: `at://demo.opensaas.gftd.ai/ai.gftd.apps.opensaas.salesforce.activity/act-example`.
...
3. Prompt privacy: Murakumo is a tenant-configurable fleet (`did:web:murakumo.gftd.ai:fleet:<tier>`). Swappable to OpenAI / Anthropic / in-house via one `ConfigPut`.
...
- Every form POST lands as `ai.gftd.apps.opensaas.salesforce.createLead` with `source=web-form` and `campaignId=<pillar-id>`.
