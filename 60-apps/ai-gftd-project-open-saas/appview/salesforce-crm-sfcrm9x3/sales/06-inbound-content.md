...
- **Secondary surface**: `atproto.etzhayyim.com` blog (AT Record-native; every post is itself `app.bsky.feed.post` + a `ai.gftd.apps.opensaas.site.article` record, dog-fooding the protocol).
...
3. How the `derive` rule in `magatama.jsonld` emits `ai.gftd.apps.opensaas.salesforce.activity` with `source=derived-stage-change`, and the commit CID is embedded. Link to a live record: `at://demo.opensaas.etzhayyim.com/ai.gftd.apps.opensaas.salesforce.activity/act-example`.
