...
export function getBlueprint() {
  return {
    ...
    lexicons: [
      ...
      "ai.gftd.apps.opensaas.salesforce.contact",
      "ai.gftd.apps.opensaas.salesforce.lead",
      "ai.gftd.apps.opensaas.salesforce.opportunity",
      "ai.gftd.apps.opensaas.salesforce.case",
      "ai.gftd.apps.opensaas.salesforce.activity",
      "ai.gftd.apps.opensaas.salesforce.createLead",
      "ai.gftd.apps.opensaas.salesforce.convertLead",
      "ai.gftd.apps.opensaas.salesforce.listPipeline",
    ],
    piiTier: {
...
export function createLead(input: CreateLeadInput): Lead {
  ...
  const lead: Lead = {
    uri: `at://${input.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.lead/${rkey}`,
...
export function convertLead(input: ConvertLeadInput): {
  ...
  const account: Account =
    (input.existingAccountDid && state.accounts.find((a) => a.uri === input.existingAccountDid)) || {
      uri: `at://${lead.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.account/${`acct-${Math.random().toString(36).slice(2, 10)}`}`,
  ...
  const contact: Contact = {
    uri: `at://${lead.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.contact/${`ctc-${Math.random().toString(36).slice(2, 10)}`}`,
    tenantDid: lead.tenantDid,
  ...
  if (input.createOpportunity !== false) {
    ...
    opportunity = {
      uri: `at://${lead.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.opportunity/${`opp-${Math.random().toString(36).slice(2, 10)}`}`,
      tenantDid: lead.tenantDid,
      accountDid: account.uri,
  ...
  const activity: Activity = {
    uri: `at://${lead.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.activity/${`act-${Math.random().toString(36).slice(2, 10)}`}`,
    tenantDid: lead.tenantDid,
    accountDid: account.uri,
...
export function advanceStage(input: AdvanceStageInput): { opportunity: Opportunity; activity: Activity } {
  ...
  const activity: Activity = {
    uri: `at://${opp.tenantDid.replace("did:web:", "")}/ai.gftd.apps.opensaas.salesforce.activity/${`act-${Math.random().toString(36).slice(2, 10)}`}`,
