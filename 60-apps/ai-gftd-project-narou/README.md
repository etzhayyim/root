# ai-gftd-project-narou (ghosthacker-based refactor)
...
```bash
python3 60-apps/ai-gftd-project-narou/70-tools/70-tools/70-tools/scripts/generate_content_jsonld.py
```

### 2) スケジューラー実行（例: 10分ごとに3回）

```bash
python3 60-apps/ai-gftd-project-narou/70-tools/70-tools/70-tools/scripts/schedule_jsonld_generation.py \
