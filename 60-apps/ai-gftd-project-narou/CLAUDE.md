# ai-gftd-project-narou

小説・漫画自動生成プラットフォーム (`narou.gftd.ai`)。
...
| Component | nanoid | 役割 |
|---|---|---|
| `ai-gftd-wasm-narou-nr0uxk2p` | `nr0uxk2p` | Main control plane + UI |

## DID Structure

| DID | 用途 |
|---|---|
| `did:web:narou.gftd.ai` (primary) | Platform agent |
| `did:web:narou.gftd.ai:work:{work_id}` (path) | 作品ごとの AI 著者 persona |
| `did:web:narou.gftd.ai:generator` (path) | テキスト/画像生成 agent |
...
- `gftd:narou/generation@1.0.0` — create-work, generate-chapter, evolve-persona, publish-chapter
- `gftd:narou/catalog@1.0.0` — get/list/search works+chapters

## Cross-App Invoke

- `Invoke("did:web:manga.gftd.ai", "submit-from-narou", params)` — 画像付きエピソード配信
- `Invoke("did:web:syosetsu.gftd.ai", "publish-chapter", params)` — テキストリーダー配信
...
- **画像生成**: Murakumo animage `https://murakumo.gftd.ai/api/openai/v1`
...
| Event type | 説明 |
|---|---|
| `org.gftd.command.narou.create_work` | 作品・世界観・初期 persona 作成 |
| `org.gftd.command.narou.generate_chapter` | 章生成 BPMN 起動 |
| `org.gftd.command.narou.evolve_agent` | persona 強制進化 (human feedback) |
| `org.gftd.command.narou.publish_chapter` | 章を published 状態へ遷移 |
...
- App: `https://nr0uxk2p.gftd.ai`
- XRPC: `https://nr0uxk2p.gftd.ai/xrpc`

## Smoke Test

```bash
curl https://nr0uxk2p.gftd.ai/health
curl -X POST https://nr0uxk2p.gftd.ai/xrpc/gftd.narou.v1.NarouQueryService/ListWorks \
