# ai-gftd-narou WebManga Agent Design

## 1. Goal
- `ai-gftd-narou` 上で小説/プロットから WebManga を自動生成する。
- 生成した作品を `manga.gftd.ai` に Matrix command で投稿し、Query API で状態確認する。
...
- Responsibility:
  ...
  - 品質ゲートを通過したものだけ `manga.gftd.ai` に投稿。
...
```json
{
  ...
  "published_event": "org.gftd.command.manga.submit-from-narou",
  "errors": []
}
```
...
- `publish.agent`
  - `narou.manga.submit` を実行して `manga.gftd.ai` に投稿。
- `audit.agent`
...
Write Contract
- room: `#manga-commands:gftd.ai`
- event: `org.gftd.command.manga.submit-from-narou`
- content:
  - `service: gftd.manga.v1.MangaCommandService`
  - `method: SubmitFromNarou`
  - `payload: {...}`
  - `source_app: narou.gftd.ai`
- compatibility ingress:
  - `POST https://manga.gftd.ai/xrpc/gftd.manga.v1.MangaCommandService/SubmitFromNarou`

Read Endpoint (polling)
- `POST https://manga.gftd.ai/xrpc/gftd.manga.v1.MangaQueryService/GetSubmissionStatus`
