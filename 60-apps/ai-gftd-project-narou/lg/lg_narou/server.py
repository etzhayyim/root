"""OSS FastAPI server for lg-narou (mirrors lg-shinshi pattern).
...
Narou NSID namespace covers ~13 task types; the initial scaffold
...
from lg_narou.checkpointer import build_checkpointer
from lg_narou.cron import start_cron, stop_cron
from lg_narou.graphs.agent_chat import GRAPH as AGENT_CHAT
from lg_narou.graphs.health import GRAPH as HEALTH
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    _log.info("lg-narou server up: graphs=%s crons=%s",
...
app = FastAPI(title="lg-narou OSS server", version="0.1.0", lifespan=_lifespan)
...
async def xrpc_compat(nsid: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": f"lg-narou {type(exc).__name__}",
