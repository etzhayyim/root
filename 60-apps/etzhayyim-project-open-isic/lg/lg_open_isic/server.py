"""lg-open-isic OSS FastAPI server (ISIC Rev.4 classification runtime).
...
NSID namespace: com.etzhayyim.apps.openIsic.*
...
from lg_open_isic.checkpointer import build_checkpointer
from lg_open_isic.cron import start_cron, stop_cron
from lg_open_isic.graphs.health import GRAPH as HEALTH
from lg_open_isic.graphs.classify_entity import GRAPH as CLASSIFY_ENTITY
from lg_open_isic.graphs.hierarchical_classify import GRAPH as HIERARCHICAL_CLASSIFY
...
GRAPHS: dict[str, Any] = {
    ...
    "hierarchical_classify": HIERARCHICAL_CLASSIFY,
}
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    _log.info("lg-open-isic up: graphs=%s", list(bound))
    try:
        yield
    finally:
...
app = FastAPI(title="lg-open-isic OSS server", version="0.1.0", lifespan=_lifespan)
...
async def get_thread_state(thread_id: str, assistant_id: str = "classify_entity") -> dict[str, Any]:
