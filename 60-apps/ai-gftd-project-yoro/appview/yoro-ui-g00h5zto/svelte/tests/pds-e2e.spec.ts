...
// ─── Endpoint reachability (accepts 200 or 401) ─────────────────────────────
	...
	test('ai.gftd.convo.search is reachable', async () => {
		const { status } = await xrpc('ai.gftd.convo.search', { q: 'hello', limit: 5 });
		expect([200, 401]).toContain(status);
	});
	...
	test('ai.gftd.convo.getUnread is reachable', async () => {
		const { status } = await xrpc('ai.gftd.convo.getUnread', {});
		expect([200, 401]).toContain(status);
	});
	...
	test('ai.gftd.convo.createConvo is reachable', async () => {
		const { status } = await xrpc('ai.gftd.convo.createConvo', { name: `probe-${Date.now()}` });
...
test.describe('atproto.etzhayyim.com — Authenticated Round-Trip', () => {
	...
	test('createConvo', async () => {
		const { status, data } = await xrpc('ai.gftd.convo.createConvo', { name: `e2e-auth-${Date.now()}` });
	...
	test('Send message', async () => {
		...
		const { status, data } = await xrpc('ai.gftd.convo.send', { convoId, body: 'E2E authenticated test' });
		expect(status).toBe(200);
	...
	test('Send reply', async () => {
		...
		const { status } = await xrpc('ai.gftd.convo.send', { convoId, body: 'E2E reply', replyTo: rootRkey, threadId: rootRkey });
		expect(status).toBe(200);
	});

	test('listEnvelopes', async () => {
		if (!convoId) test.skip();
		const { status } = await xrpc('ai.gftd.convo.listEnvelopes', { convoId, limit: 10 });
		expect(status).toBe(200);
	});

	test('getThread', async () => {
		if (!convoId || !rootRkey) test.skip();
		const { status } = await xrpc('ai.gftd.convo.getThread', { convoId, rootRkey });
		expect(status).toBe(200);
	});

	test('react', async () => {
		if (!convoId || !rootRkey) test.skip();
		const { status } = await xrpc('ai.gftd.convo.react', { convoId, rkey: rootRkey, emoji: 'thumbsup' });
		expect(status).toBe(200);
	});

	test('markRead', async () => {
		if (!convoId || !rootRkey) test.skip();
		const { status } = await xrpc('ai.gftd.convo.markRead', { convoId, lastRkey: rootRkey });
		expect(status).toBe(200);
	});
	...
	test('listMembers', async () => {
		if (!convoId) test.skip();
		const { status } = await xrpc('ai.gftd.convo.listMembers', { convoId });
		expect(status).toBe(200);
	});

	test('listPublicConvos (authenticated)', async () => {
		const { status, data } = await xrpc('ai.gftd.convo.listPublicConvos', { limit: 10 });
		expect(status).toBe(200);
	});
	...
	test('Search messages', async () => {
		const { status } = await xrpc('ai.gftd.convo.search', { q: 'E2E', limit: 5 });
		expect(status).toBe(200);
	});
