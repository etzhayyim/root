<!--
  /profile/{handle}/feed/{rkey}/liked-by — Who liked this feed generator.
-->
<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			const result = await getLikes(uri);
			likes = Array.isArray(result) ? result : (result as any).likes ?? [];
		} catch (e) { console.warn('feed liked-by: loadLikes failed', e); } finally {
			loading = false;
		}
	}
	...
	function goBack() {
		...
		else void goto(`/profile/${encodeURIComponent(handle)}/feed/${encodeURIComponent(rkey)}`);
	}
...
<svelte:head>
	<title>Feed liked by — YORO</title>
</svelte:head>
