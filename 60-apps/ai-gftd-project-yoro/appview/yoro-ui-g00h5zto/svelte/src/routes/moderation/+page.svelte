<!--
  /moderation — Bluesky-compatible moderation settings.
-->
<script lang="ts">
	...
	const sections = [
		{ path: '/moderation/muted-accounts', label: 'Muted accounts', icon: '🔇', desc: 'Accounts you have muted' },
		{ path: '/moderation/blocked-accounts', label: 'Blocked accounts', icon: '🚫', desc: 'Accounts you have blocked' },
		{ path: '/moderation/interaction-settings', label: 'Interaction settings', icon: '💬', desc: 'Control who can interact with you' },
	];
</script>

<svelte:head>
	<title>Moderation — YORO</title>
</svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<span class="text-[17px] font-bold text-gv2-text-primary">Moderation</span>
	</div>
	...
	<div class="flex-1 overflow-y-auto scrollbar-none">
		<div class="divide-y divide-gv2-border/20">
			{#each sections as s, i}
				<button
					type="button"
					class="flex w-full items-center gap-4 px-4 py-4 text-left touch-manipulation active:bg-gv2-bg-hover/40"
					onclick={() => goto(s.path)}
					in:fade={staggerFade(i, { duration: 150 })}
				>
					<span class="text-[24px]">{s.icon}</span>
