<!--
  ...
  Reads vertex_yoro_product_research via XRPC ai.gftd.apps.yoro.listProductResearch.
  Triggers ingest via XRPC ai.gftd.apps.yoro.ingestProductCategory.
-->
<script lang="ts">
	...
	async function load() {
		loading = true;
		try {
			const res = await atQuery('ai.gftd.apps.yoro.listProductResearch', { limit: 50 }) as { items: ResearchRow[] };
			items = res.items ?? [];
		} catch {
			items = [];
		} finally {
	...
	async function trigger() {
		...
		lastJobStatus = null;
		try {
			const res = await atProcedure('ai.gftd.apps.yoro.ingestProductCategory', {
