 ...
 * etc. Credit transactions are written as `ai.gftd.apps.credits.creditTransaction`.
 *
 * ## Lifecycle
 *
...
async function sendProjectLogMessage(text: string, signal?: AbortSignal): Promise<void> {
	...
	if (!convoId) return;
	await xrpcProcedure('ai.gftd.projector.sendProjectMessage', { convoId, text }, signal);
}
...
async function ensureEvolutionProjectConvo(forceNew = false, signal?: AbortSignal): Promise<string | null> {
	...
	try {
		const parsed = await xrpcProcedure<Record<string, unknown>>('ai.gftd.projector.newProjectConvo', {
		...
		await xrpcProcedure('ai.gftd.projector.sendProjectMessage', {
			convoId,
...
async function ensureActorMemberInProject(actorDid: string, actorName: string, signal?: AbortSignal): Promise<void> {
	...
	if (!convoId) return;
	try {
		await xrpcProcedure('ai.gftd.projector.addConvoMember', {
			convoId,
			memberDid: actorDid,
			role: 'member',
		}, signal);
...
async function restoreFromGraph(): Promise<void> {
	...
	// TODO(evolution): migrate to SQL-backed ai.gftd.kagami.sql with Kysely SELECT once
	// vertex_{koji_discovery,kyumei_validation,shinka_evolution,hinshitsu_assessment,shinka_knowledge}
	// tables land in @gftd/graph-schema.
...
async function fetchActorEvidence(actorDid: string, signal: AbortSignal): Promise<string> {
	...
	const relRes = await fetch(`${PDS}/xrpc/ai.gftd.kagami.sql`, {
...
const COLLECTIONS: Record<EvolutionTaskType, string> = {
	koji: 'ai.gftd.apps.yoro.kojiDiscovery',
	kyumei: 'ai.gftd.apps.yoro.kyumeiValidation',
	shinka: 'ai.gftd.apps.yoro.shinkaEvolution',
	hinshitsu: 'ai.gftd.apps.yoro.hinshitsuAssessment',
	shinkaKnowledge: 'ai.gftd.apps.yoro.shinkaKnowledge',
};
 ...
 * (e.g. `ai.gftd.apps.yoro.kojiDiscovery`), then writes a credit
...
async function persistResult(
	...
	try {
		...
		if (taskType === 'shinkaKnowledge' && result.type === 'shinkaKnowledge') {
			...
			if (sk.domainSummary) {
				await fetch(`${PDS}/xrpc/ai.gftd.actor.update`, {
					method: 'POST', headers, credentials: 'include', signal,
...
async function selfGenerateTasks(): Promise<void> {
	...
	try {
		...
		const graphRes = await fetch(`${PDS}/xrpc/ai.gftd.kagami.sql`, {
			method: 'POST',
			headers: gqHeaders,
			credentials: 'include',
			signal: ac.signal,
