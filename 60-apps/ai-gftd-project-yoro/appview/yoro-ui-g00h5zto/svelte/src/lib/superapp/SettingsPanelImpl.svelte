<script lang="ts">
	...
	onMount(() => {
		if (!get(clerkLoaded) && !clerkInitialized) {
			...
			initClerk().catch((error) => { console.warn("[silent-fail] projects/ai-gftd-project-yoro/wasm/yoro-ui-g00h5zto/svelte/src/lib/superapp/SettingsPanelImpl.svelte: suppressed async error", error); });
		}
