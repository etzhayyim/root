 ...
 * Read: repo record scan on ai.gftd.apps.yoro.browsingHistory.
 */
...
export function recordVisit(entry: Omit<HistoryEntry, 'visitedAt' | 'rkey'>) {
	...
	void (async () => {
		...
		await atProcedure('com.atproto.repo.createRecord', {
			collection: 'ai.gftd.apps.yoro.browsingHistory',
			record: {
				$type: 'ai.gftd.apps.yoro.browsingHistory',
				path: entry.path,
				title: entry.title,
 ...
 * Collection: `ai.gftd.apps.yoro.searchHistory`.
...
export function recordSearch(query: string, tab: string, resultCount: number = 0) {
	...
	void (async () => {
		...
		await atProcedure('com.atproto.repo.createRecord', {
			collection: 'ai.gftd.apps.yoro.searchHistory',
			record: {
				$type: 'ai.gftd.apps.yoro.searchHistory',
				query: query.trim(),
				tab,
				resultCount,
				createdAt,
...
export async function loadHistory(): Promise<HistoryEntry[]> {
	...
	try {
		...
		const result = await listRecords(did, 'ai.gftd.apps.yoro.browsingHistory', { limit: 200 });
