...
- Profile tab / Drawer のフォロワー数等のハードコード (`ai.gftd.yoro.actor.getProfile` の response を使用)
...
$lib/atproto-agent AtpAgent(service: "https://atproto.gftd.ai")
  ...
  ├─ ai.gftd.convo.* → PDS direct (Platform Service)
...
| **Layer 1 (W extension)** | `ai.gftd.convo.*` (Tier 3 Platform) | GFTD AT Protocol extension — presence/typing/pin/forward/encryption/consent | updatePresence, sendTyping, pinMessage, setEncryption |
| **Layer 2 (Projector)** | `ai.gftd.yoro.projector.*` (Tier 1 AppView) | AI PM project convo — recursive nesting, slash commands, MCP tool calling | newProjectConvo, sendProjectMessage, listProjectConvos, getProjectConvo |
...
|---|---|---|---|
| **Social write** | `ai.gftd.yoro.feed.like/unlike/repost/unrepost` | `app.bsky.feed.*` | `ai.gftd.yoro.` → `app.bsky.` |
| **Social graph** | `ai.gftd.yoro.graph.follow/unfollow/block/unblock` | `app.bsky.graph.*` | `ai.gftd.yoro.` → `app.bsky.` |
| **Threadgate** | `ai.gftd.yoro.feed.threadgate/removeThreadgate` | `app.bsky.feed.*` | `ai.gftd.yoro.` → `app.bsky.` |
| **Repo CRUD** | `ai.gftd.atproto.repo.createRecord/deleteRecord` | `com.atproto.repo.*` | `ai.gftd.atproto.` → `com.atproto.` |
| **Projector** | `ai.gftd.yoro.projector.sendProjectMessage/edit` | `ai.gftd.projector.*` | `ai.gftd.yoro.projector.` → `ai.gftd.projector.` |
| **Convo (Tier 3)** | `ai.gftd.convo.send/edit/redact` | `ai.gftd.convo.*` | なし (透過 proxy) |
| **RTC (Tier 3)** | `ai.gftd.rtc.sendCallOffer/sendCallAnswer/hangupCall` | `ai.gftd.rtc.*` | なし (透過 proxy) |
...
**Search のデフォルトタブは Actors (AT Protocol 用語)。** `ai.gftd.yoro.actor.searchActors` (yoro AppView) を使用。「Agent」ではなく「Actor」が AT Protocol 標準。

| Tab | 用途 | データソース |
|---|---|---|
| **Actors** (default) | Actor 検索 (登録日/更新日ソート) | `searchActors("gftd", {limit:25})` (`ai.gftd.yoro.actor.searchActors`) → yoro AppView → HYPERDRIVE → RisingWave direct。vertex_app + vertex_repo_record |
| Posts | 投稿セマンティック検索 (client embed → vector) + text fallback | `searchPosts({q, vector})` (`ai.gftd.yoro.feed.searchPosts`) |
| People | ユーザー検索 | `searchActors()` (`ai.gftd.yoro.actor.searchActors`) |
...
|---|---|---|---|
| **Actor** (`app.bsky.actor`) | `ai.gftd.yoro.actor.*` | "Actor" | 全 social context で使用 |
| **Record** (`com.atproto.repo`) | `ai.gftd.atproto.repo.*` | — | PDS Gateway 経由 |
| **Convo** (`chat.bsky.convo`) | `ai.gftd.convo.*` (Tier 3 Platform) | "Messages" | DM は AT Protocol `convo` 準拠 |
| **Post** (`app.bsky.feed.post`) | `ai.gftd.yoro.feed.*` | "Post" | AppView local handler |
| **Profile** (`app.bsky.actor.getProfile`) | `ai.gftd.yoro.actor.getProfile` | "Profile" | AppView local handler |
| **Projector** (yoro 独自) | `ai.gftd.yoro.projector.*` | "Projects" | AppView → PDS fallback |
| **Signal E2E** (AT Protocol) | `ai.gftd.signal.*` (Tier 3 Platform) | — | PDS proxy |
| **HandleStream** (wRPC) | `ai.gftd.stream.*` (Tier 3 Platform) | — | PDS proxy |
...
**Twitter/X 風の view 数 (impression count) を全 feed に表示。** `ai.gftd.yoro.feed.sendInteractions` で view event を記録し、PDS が `Interaction` label から per-post view count を集計。
...
| 層 | 実装 |
|---|---|
| **Write** | `atProcedure('com.atproto.repo.createRecord', { collection: 'ai.gftd.apps.yoro.browsingHistory', record })` → PDS → yata `BrowsingHistory` node |
