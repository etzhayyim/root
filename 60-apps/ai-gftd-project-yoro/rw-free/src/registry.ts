...
export async function projectEntity(
  ...
  try {
    const result = await e.write({
      collection: "ai.gftd.yoro.projectEntity",
      record,
      rkey,
    });
...
export async function productResearch(
  ...
  try {
    const result = await e.write({
      collection: "ai.gftd.yoro.productResearch",
      record,
      rkey,
    });
    return { uri: result.uri, rkey: result.rkey };
...
export async function activitySeen(
  ...
  try {
    const result = await e.write({
      collection: "ai.gftd.yoro.activitySeen",
      record,
      rkey,
    });
    return { uri: result.uri, rkey: result.rkey };
...
export async function shinkaEvolution(
  ...
  try {
    const result = await e.write({
      collection: "ai.gftd.yoro.shinkaEvolution",
      record,
      rkey,
    });
    return { uri: result.uri, rkey: result.rkey };
...
export async function shinkaKnowledge(
  ...
  try {
    const result = await e.write({
      collection: "ai.gftd.yoro.shinkaKnowledge",
      record,
      rkey,
    });
    return { uri: result.uri, rkey: result.rkey };
