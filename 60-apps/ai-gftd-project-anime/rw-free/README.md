...
Per [ADR-2605203000](../../../90-docs/adr/2605203000-rw-free-write-target-options.md), anime migrates from vendor's `createKyselyDb` pattern (RW direct write) to **Option B** — PDS XRPC writes via `@etzhayyim/sdk e.write()`.
...
All 10 canonical anime lexicons now have rw-free reference impl. Wire-up to a Worker / LangServer pod XRPC handler is the next operator task per ADR-2605203000.
...
| `db.insertInto("vertex_anime_title").values({...}).execute()` | `e.write({ collection: "app.etzhayyim.anime.title", record, rkey })` |
...
import { createTitle, getTitle, listTitles, submitReview } from "@etzhayyim/anime-rw-free";
...
- [kiyo rw-free](../../ai-gftd-project-kiyo/rw-free/) — sibling Option B reference (12/12 ✓)
- [sbom rw-free](../../ai-gftd-project-sbom/rw-free/) — Option B reference (17/N)
