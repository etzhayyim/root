...
async function cmdListJobPostings(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.listJobPostings", body);
...
async function cmdGetJobPosting(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.getJobPosting", body);
...
async function cmdSearchJobPostings(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.searchJobPostings", body);
  const q = str(args.q ?? "");
...
async function cmdListCompanies(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.listCompanies", body);
  const limit = Math.min(Number(args.limit) || 50, 200);
  const offset = Number(args.offset) || 0;
...
function cmdWave(sdk: HostSDK, body: Uint8Array): unknown {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.wave", body);
...
function handleComAtprotoSyncSubscribeReposCommit(sdk: HostSDK, commit: ComAtprotoSyncSubscribeReposCommit): { ok: true; detail: string } {
  ...
  if (collection === "ai.gftd.apps.shigotoba.collectionJob" && record) return { ok: true, detail: "collectionJob accepted" };

  if (collection === "ai.gftd.apps.shigotoba.jobPosting") {
    return { ok: true, detail: "jobPosting persisted" };
  }

  if (collection === "ai.gftd.apps.shigotoba.companyProfile") {
...
async function cmdExportShigotoba(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.export", body);
...
async function cmdAuditShigotoba(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.shigotoba.audit", body);
...
export default createWorkerExport((sdk) => {
  ...
  sdk.app
      ...
      .command(nsid("ai.gftd.apps.shigotoba.getJobPosting"), (ctx, body) => cmdGetJobPosting(sdk, body),
      asAgentTool("Get job posting by ID"),
      withCapabilityTags('query', 'shigotoba', 'jobs'),
      )
      .command(nsid("ai.gftd.apps.shigotoba.searchJobPostings"), (ctx, body) => cmdSearchJobPostings(sdk, body),
      asAgentTool("Search job postings by keyword"),
      withCapabilityTags('search', 'shigotoba', 'jobs'),
      )
      .command(nsid("ai.gftd.apps.shigotoba.listCompanies"), (ctx, body) => cmdListCompanies(sdk, body),
      asAgentTool("List companies with job postings"),
      withCapabilityTags('query', 'shigotoba', 'companies'),
      )
      .command(nsid("ai.gftd.apps.shigotoba.ingestJobs"), (_ctx, body) => proxyStage(sdk, "ai.gftd.apps.shigotoba.ingestJobs", body),
      asAgentTool("Trigger LangServer job ingestion from public APIs"),
      withCapabilityTags('pipeline', 'shigotoba', 'ingest', 'bpmn'),
      )
      .command(nsid("ai.gftd.apps.shigotoba.wave"), (ctx, body) => cmdWave(sdk, body),
      asAgentTool("Respond to wave greeting"),
      withCapabilityTags('social', 'greeting'),
      )
      .command(nsid("ai.gftd.apps.shigotoba.stats"), (ctx, body) => cmdStatsShigotoba(sdk, body),
      asAgentTool("Job posting statistics"),
      withCapabilityTags("analytics", "shigotoba"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.export"), (ctx, body) => cmdExportShigotoba(sdk, body),
      asAgentTool("Export job posting data"),
      withCapabilityTags("export", "shigotoba"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.health"), (ctx, body) => cmdHealthShigotoba(sdk, body),
      asAgentTool("Health check"),
      withCapabilityTags("diagnostics", "shigotoba"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.describe"), (ctx, body) => cmdDescribeShigotoba(sdk, body),
      asAgentTool("Describe capabilities"),
      withCapabilityTags("meta", "shigotoba"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.summarize"), (_ctx, body) => proxyStage(sdk, "ai.gftd.apps.shigotoba.summarize", body),
      asAgentTool("AI summary of job market data via BPMN"),
      withCapabilityTags("ai", "shigotoba", "bpmn"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.audit"), (ctx, body) => cmdAuditShigotoba(sdk, body),
      asAgentTool("Audit log for job postings"),
      withCapabilityTags("audit", "shigotoba"),
      )
      .command(nsid("ai.gftd.apps.shigotoba.dataSources"), (ctx, body) => cmdDataSources(sdk, body),
