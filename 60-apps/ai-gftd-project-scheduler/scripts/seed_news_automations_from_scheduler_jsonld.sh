...
NEWS_SCHEDULER_JSON="${REPO_ROOT}/projects/ai-gftd-project-news/scheduler.jsonld"

# Scheduler cron MCP endpoint (scheduler-cron-component).
# Override to point at another actor id or local dev.
SCHEDULER_API_BASE="${SCHEDULER_API_BASE:-https://1.gftd.ai}"
...
if [[ -z "${NEWS_JOBS_BASE}" ]]; then
  echo "NEWS_JOBS_BASE is required (example: https://gftd.ai/<news-nanoid>)" >&2
  exit 1
fi
...
create_automation() {
  ...
  tool_args="$(jq -n \
    ...
    '{
      name: $name,
      projects: ["ai-gftd-project-news"],
      work_dir: "/workspace/projects/ai-gftd-project-news",
      prompt: "Auto-generated from projects/ai-gftd-project-news/scheduler.jsonld",
      timezone: $timezone,
      schedule: {
        mode: "interval",
