# ai-gftd-project-news 自動記事生成を App 定期実行にする設計

## 1. 目的
`60-apps/ai-gftd-project-news` の記事生成ジョブ（`/jobs/news-generate`, `/jobs/anime-generate`, `/jobs/game-generate`, `/jobs/japanese-food-generate`）を、App scheduler 依存なしで `scheduler.gftd.ai` から定期実行できるようにする。

## 2. 現状
- `60-apps/ai-gftd-project-news/scheduler.jsonld` に cron 定義はある。
- `60-apps/ai-gftd-project-scheduler/wasm/scheduler-mcp-component` は KV 永続化と API を提供済み。
- `60-apps/ai-gftd-project-www/wasm/www-mcp-component` に `/scheduler/automations` と `/jobs/scheduler-tick` があり、定期実行メタデータを管理可能。
...
4. Target Service: `ai-gftd-project-news` wasm endpoint
- 既存の `/jobs/*` エンドポイントを利用
...
```json
{
  ...
  "target": {
    "method": "POST",
    "url": "https://gftd.ai/<news-nanoid>/jobs/news-generate",
...
3. `60-apps/ai-gftd-project-news/scheduler.jsonld` から job seed を投入
...
## 8. 受け入れ条件
1. `ai-gftd-project-news` の `news-generate` が 4時間間隔で自動実行される。
