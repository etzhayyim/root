# ai-gftd-project-resources

Public resource repository for GFTD. Content under `content/` is stored as JSON-LD entity files and published to `https://resources.gftd.ai/`.

## Public namespace

The `resources.gftd.ai` CDN app now serves:

- `https://resources.gftd.ai/ai-gftd-project-resources/`
- All JSON-LD documents under `60-apps/ai-gftd-project-resources` including `TECH_STACK.jsonld`.
- `https://resources.gftd.ai/ai-gftd-project-resources/PROJECT.jsonld`
...
```bash
python 60-apps/ai-gftd-project-resources/tools/fetch_jp_laws_jsonld.py
```

## Compliance Rule Bundles

Compiled jurisdiction bundles for `gftd:compliance` are published at:
...
```bash
python 60-apps/ai-gftd-project-resources/tools/compile_compliance_bundles.py
python 60-apps/ai-gftd-project-resources/tools/compile_legal_jurisdiction_index.py
```
...
- `60-apps/ai-gftd-project-contracts/data/social-contracts/*.jsonld`
...
`content/legal/jurisdictions/index.jsonld` joins the global legal source registry, currently published raw corpora, and compiled compliance bundles so `resources.gftd.ai` exposes which jurisdictions are still scaffold-only versus actually integrated.
...
`ai-gftd-project-collector` (`collector.export_jsonld`) ingest is provided as a
App component:

- `60-apps/ai-gftd-project-resources/wasm/components/resources-collector-ingest-r8n4c1j6`

```bash
cd 60-apps/ai-gftd-project-resources/wasm/components/resources-collector-ingest-r8n4c1j6
gftd build
...
## Crawler Data (crawler.gftd.ai)

Web crawl data collected by `crawler.gftd.ai` is normalized to JSON-LD and stored under `content/crawl/`.
...
|---|---|---|
| `gftd:CrawlJob` | `shacl/crawler/shapes.jsonld` | Crawl job metadata (seed URL, depth, status, timestamps) |
| `gftd:CrawlPage` | `shacl/crawler/shapes.jsonld` | Per-page crawl result (URL, HTTP status, title, size, links) |
...
- `gftd:` (resources.gftd.ai/ontology#) for crawler-specific terms (depth, httpStatus, linkCount)

### Example: CrawlJob

```json
{
  "@context": "https://resources.gftd.ai/ontology/crawler/context.jsonld",
  "@type": "gftd:CrawlJob",
  "@id": "https://resources.gftd.ai/content/crawl/job/crawl-1739760000",
...
```json
{
  "@context": "https://resources.gftd.ai/ontology/crawler/context.jsonld",
  "@type": "gftd:CrawlPage",
  "@id": "https://resources.gftd.ai/content/crawl/page/res-001",
  "identifier": "res-001",
  "url": "https://gftd.ai",
  "jobId": "crawl-1739760000",
  "name": "GFTD - Artificial Organism",
  "httpStatus": 200,
  "contentSize": 24576,
  "linkCount": 18,
...
```json
{
  "@context": "https://resources.gftd.ai/ontology/crawler/context.jsonld",
  "@type": "schema:WebSite",
  "@id": "https://resources.gftd.ai/content/crawl/site/gftd.ai",
  "url": "https://gftd.ai",
  "name": "GFTD",
  "pageCount": 42,
  "lastCrawledAt": "2026-02-17T06:00:00Z",
  "hasPart": [
    "https://resources.gftd.ai/content/crawl/page/res-001",
    "https://resources.gftd.ai/content/crawl/page/res-002"
  ]
}
```
...
resource-writer (scheduler / Tekton task)
    ...
    v
ai-gftd-project-resources (git repo)
    |
    | content/crawl/job/<job_id>.jsonld
    | content/crawl/page/<result_id>.jsonld
    | content/crawl/site/<domain>.jsonld
    v
resources.gftd.ai (published)
```

### SHACL Validation

Shapes are defined in `shacl/crawler/shapes.jsonld`:
- `gftd:CrawlJobShape` - validates CrawlJob entities
- `gftd:CrawlPageShape` - validates CrawlPage entities
- `gftd:CrawlSiteShape` - validates WebSite (site summary) entities
...
```bash
python 60-apps/ai-gftd-project-resources/tools/260303-public-company-ir-news-ingest.py \
  --company-id vale-sa \
  --dry-run
```
...
Storage backend has migrated to Cypher graph (magatama WIT).

Tool:

- `60-apps/ai-gftd-project-resources/tools/jsonld-arrow-ingest`
...
- `60-apps/ai-gftd-project-resources/tools/curated-arrow-ingest`
...
```bash
cd 60-apps/ai-gftd-project-resources/tools/curated-arrow-ingest
go run . --dry-run
```
