...
|---|---|---|---|
| **microsoft-send** (m1cr5nd5) | `appview/ai-gftd-wasm-microsoft-m1cr5nd5` | `microsoft.etzhayyim.com` | T3 TS Native write facade — `ai.gftd.apps.microsoft.{sendMail,sendDraft,listDrafts}` XRPC + MCP. Policy-routed: all-internal recipients direct Mail.Send, any external → auto-draft. Teams channel posting = channel email address as recipient |

### microsoft-send (write facade)

**Policy** (SSoT: `deps.toml [etzhayyim_agent.auth]`):

- `email_send_internal = "direct"` → `INTERNAL_EMAIL_DOMAINS` (`gftd.co.jp,etzhayyim.com,gftd.works,gftd.group`) + `INTERNAL_EMAIL_SUFFIXES` (`.onmicrosoft.com` — Teams channel email) の全宛先一致時は即時 `Mail.Send`
...
新規 Worker deploy 後、MCP `tools/list` (`mcp.etzhayyim.com/xrpc/ai.gftd.mcp.message`) が per-actor tool を返すには `vertex_capability` に `ai.gftd.tool.tool` record を事前投入する必要がある。**Worker 側の自動登録は行わない** — INSERT 1 行あたり ~11s (RisingWave back-pressure) で、3 行 awaited すると 25s XRPC handler timeout を超える。
...
**Verify**: `curl -sS https://mcp.etzhayyim.com/xrpc/ai.gftd.mcp.message -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'` の result.tools に `microsoft.sendMail/sendDraft/listDrafts` が含まれれば登録済み。Claude.ai org connector は tool list を短時間キャッシュするため、新規 tool 追加後は connector 側で再接続が必要な場合がある。
