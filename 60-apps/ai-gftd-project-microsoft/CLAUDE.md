...
|---|---|---|---|
| **microsoft-send** (m1cr5nd5) | `appview/ai-gftd-wasm-microsoft-m1cr5nd5` | `microsoft.etzhayyim.com` | T3 TS Native write facade — `com.etzhayyim.apps.microsoft.{sendMail,sendDraft,listDrafts}` XRPC + MCP. Policy-routed: all-internal recipients direct Mail.Send, any external → auto-draft. Teams channel posting = channel email address as recipient |
...
```bash
AT_TOKEN=$(gftd agent-token --lxm com.etzhayyim.kagami.sql --ttl 180)
for ENTRY in \
