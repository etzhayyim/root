...
|---|---|---|---|
| **microsoft-send** (m1cr5nd5) | `appview/ai-gftd-wasm-microsoft-m1cr5nd5` | `microsoft.gftd.ai` | T3 TS Native write facade — `ai.gftd.apps.microsoft.{sendMail,sendDraft,listDrafts}` XRPC + MCP. Policy-routed: all-internal recipients direct Mail.Send, any external → auto-draft. Teams channel posting = channel email address as recipient |

### microsoft-send (write facade)

**Policy** (SSoT: `deps.toml [gftdcojp_agent.auth]`):

- `email_send_internal = "direct"` → `INTERNAL_EMAIL_DOMAINS` (`gftd.co.jp,gftd.ai,gftd.works,gftd.group`) + `INTERNAL_EMAIL_SUFFIXES` (`.onmicrosoft.com` — Teams channel email) の全宛先一致時は即時 `Mail.Send`
...
1. Secrets Store (`1824561668fe47cc9127d493961885af`) に `m365_tenant_id` / `m365_client_id` / `m365_client_secret` を登録 (Keychain `gftd.m365` からコピー)
2. DNS: `microsoft.gftd.ai` CNAME (`gftd dns-sync` が自動)
3. `gftd deploy` を `appview/ai-gftd-wasm-microsoft-m1cr5nd5/` で実行
4. PDS MCP adapter が `/mcp` の `tools/list` で 3 tool (`ai.gftd.apps.microsoft.sendMail`, `sendDraft`, `listDrafts`) を自動公開 (ADR-0042)

**Read-only peer**: `m365-ingest` (T1, `m3650xin.gftd.ai`) — `Application.Mail.Read` のみ。書き込みは全て microsoft-send 経由。

### MCP tool registration (one-shot seed)

新規 Worker deploy 後、MCP `tools/list` (`mcp.gftd.ai/xrpc/ai.gftd.mcp.message`) が per-actor tool を返すには `vertex_capability` に `ai.gftd.tool.tool` record を事前投入する必要がある。**Worker 側の自動登録は行わない** — INSERT 1 行あたり ~11s (RisingWave back-pressure) で、3 行 awaited すると 25s XRPC handler timeout を超える。

正攻法は `ai.gftd.tool.registerBatch` XRPC だが、2026-04-23 時点で PDS handler が Kysely `onConflict` を使っており RisingWave parser が `ON CONFLICT` を拒否 (`expected end of statement, found: on`)。これが直るまでは `ai.gftd.kagami.sql` 直叩きで seed する。

```bash
AT_TOKEN=$(gftd agent-token --lxm ai.gftd.kagami.sql --ttl 180)
for ENTRY in \
  ...
  # description, collection='ai.gftd.tool.tool', status='active', input_schema_json,
  # tags, capability_worker='m1cr5nd5') VALUES (...)
  # ~11s per INSERT — run serially.
done
```

Current canonical rkeys (2026-04-23 seed): `3lmcrsdml01` (sendMail) / `3lmcrsddr01` (sendDraft) / `3lmcrsdls01` (listDrafts) — all `repo='did:web:m1cr5nd5.gftd.ai'`, `capability_worker='m1cr5nd5'`.

**Verify**: `curl -sS https://mcp.gftd.ai/xrpc/ai.gftd.mcp.message -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'` の result.tools に `microsoft.sendMail/sendDraft/listDrafts` が含まれれば登録済み。Claude.ai org connector は tool list を短時間キャッシュするため、新規 tool 追加後は connector 側で再接続が必要な場合がある。

**Follow-up**: PDS `ai.gftd.tool.register*` ハンドラーを Kysely `.onConflict` から RisingWave 互換の DELETE+INSERT パターンに書き換えれば自動登録が再び可能になる ([`50-infra/cloudflare/workers/atproto/src/actor/tools.ts`](../../50-infra/cloudflare/workers/atproto/src/actor/tools.ts))。

## CRITICAL: Microsoft Import Boundary

→ `gftd dodaf tv1 query --id ai-gftd-project-microsoft-microsoft-import-boundary` / MCP `gftd.dodaf.tv1.query`
