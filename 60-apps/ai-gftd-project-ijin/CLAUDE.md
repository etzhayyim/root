...
| Collection | 用途 |
|---|---|
| `ai.gftd.apps.ijin.historicalPerson` | 偉人 profile (生没年, 国, 分野, era, 業績) |
| `ai.gftd.apps.ijin.influenceEdge` | 影響関係 (from_did → to_did, influence_type, weight) |
| `ai.gftd.apps.ijin.legacyResource` | 偉人の業績が影響を与えた現代の resource/activity |
| `ai.gftd.apps.ijin.socialVoice` | 偉人 DID の social post 設定 (voice, topics, style) |
| `ai.gftd.apps.ijin.historicalOrg` | 過去の組織 (effective_from/until で時間範囲, 構造・役職) |
| `ai.gftd.apps.ijin.orgRole` | 偉人と組織の関係 (role, effective_from/until) |
| `ai.gftd.apps.ijin.ownershipHistory` | 土地・建物の所有者履歴 (person/org → parcel/building) |
| `ai.gftd.apps.ijin.spatialAnchor` | 偉人・組織の地理的接続 (lat/lng, maps_node_id) |
