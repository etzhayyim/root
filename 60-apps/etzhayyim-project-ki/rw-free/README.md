...
Per [ADR-2605203000](../../../90-docs/adr/2605203000-rw-free-write-target-options.md), ki migrates from vendor's `createKyselyDb` pattern (RW direct write via dispatcher proxy) to **Option B** — PDS XRPC writes via `@etzhayyim/sdk e.write()`.
...
- This `rw-free` package supplies the persistence layer used by **both** pod and Worker — `absorb` and `bloom` are simple records the Worker can write directly (no LLM); `synthesize` is pod-side because it needs LLM compute.
