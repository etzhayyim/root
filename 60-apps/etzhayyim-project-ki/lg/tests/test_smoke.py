"""Local smoke tests for lg-organism OSS server.
...
def test_server_module_imports():
    from lg_organism.server import GRAPHS, _NSID_TO_ASSISTANT, app
...
def test_graphs_match_expected_set():
    from lg_organism.server import GRAPHS
...
def test_nsid_to_assistant_completeness():
    from lg_organism.server import GRAPHS, _NSID_TO_ASSISTANT
...
def test_langgraph_json_has_no_crons():
    """organism has no scheduled crons — verify the config stays clean."""
    cfg = json.loads((_LG_DIR / "langgraph.json").read_text())
...
@pytest.fixture
def client():
    from lg_organism.server import app
...
def test_unknown_xrpc_404(client):
    r = client.post("/xrpc/com.etzhayyim.ki.unknownMethod", json={})
    assert r.status_code == 404
