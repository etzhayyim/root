...
const NSID_PREFIX = "com.etzhayyim.apps.airCargo.";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "a1rcarg0",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/air-cargo",
        methods: ["createCargoBooking", "issueAirWaybill", "acceptCargo", "assignUld", "trackShipment", "processClaim", "settleCargoAccount", "reportCargoSecurity"],
      });
    }
