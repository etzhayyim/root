...
Run `ai-gftd-project-yabai` analysis pipeline against `ai-gftd-project-resources` and persist normalized watchlist/source/entity/evidence/risk rows into Tonbo/LanceDB REST tables backed by Arrow-compatible schemas.

## Inputs

- Resources crawl data: `60-apps/ai-gftd-project-resources/content/crawl/page/*.jsonld`
- Resources TI data (optional): `60-apps/ai-gftd-project-resources/content/ti/indicator/**/*.jsonld`
...
```bash
cd 60-apps/ai-gftd-project-yabai/tools/yabai-pipeline
...
```bash
GOCACHE=/tmp/go-build-yabai go run . \
  --resources-root /path/to/60-apps/ai-gftd-project-resources/content \
  --output /path/to/60-apps/ai-gftd-project-yabai/content
```
