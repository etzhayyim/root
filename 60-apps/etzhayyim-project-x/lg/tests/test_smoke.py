"""Local smoke tests for lg-x OSS server.

Run from `60-apps/etzhayyim-project-x/lg/`:
...
touching `60-apps/etzhayyim-project-x/lg/**`.
"""
...
def test_server_module_imports():
    from lg_x.server import GRAPHS, _NSID_TO_ASSISTANT, app
...
def test_graphs_match_expected_set():
    from lg_x.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_x.server import _NSID_TO_ASSISTANT
    assert _NSID_TO_ASSISTANT == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_x.server import GRAPHS, _NSID_TO_ASSISTANT
...
def client(monkeypatch):
    ...
    from lg_x import server as srv
...
def test_unknown_nsid_xrpc_404(client):
    r = client.post("/xrpc/com.etzhayyim.apps.x.unknownMethod", json={})
