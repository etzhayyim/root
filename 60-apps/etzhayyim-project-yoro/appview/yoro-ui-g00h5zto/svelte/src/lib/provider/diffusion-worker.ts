...
export interface DiffusionWorkerMessage {
	...
	params?: {
		...
		cdnBase: string;
		/** Relative path to CLIP text encoder ONNX. */
		clipPath: string;
		/** Relative path to UNet ONNX. */
		unetPath: string;
		/** Relative path to UNet external weights (null if single-file). */
...
async function generate(params: NonNullable<DiffusionWorkerMessage['params']>) {
	const { prompt, negativePrompt, steps, cfgScale, width, height, seed,
		cdnBase, clipPath, unetPath, unetWeightsPath, vaePath, tokenizerModel } = params;
	...
	const clipBuffer = await fetchModelFile(
		`${cdnBase}/${clipPath}`,
		(pct) => postProgress('loading-clip', 0, steps, `Downloading CLIP... ${pct}%`),
	);
	...
	if (unetWeightsPath) {
		...
		const weightsBuffer = await fetchModelFile(
			`${cdnBase}/${unetWeightsPath}`,
			(pct) => postProgress('loading-unet', 0, steps, `Downloading UNet weights... ${pct}%`),
		);
