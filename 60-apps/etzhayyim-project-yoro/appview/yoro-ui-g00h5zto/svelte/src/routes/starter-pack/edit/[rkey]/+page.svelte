<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			const pack = (result as any)?.starterPack ?? result;
			name = (pack as any)?.record?.name ?? '';
			description = (pack as any)?.record?.description ?? '';
			selectedUsers = (result as any)?.items?.map((it: any) => it.subject) ?? [];
		} catch (e) { console.warn('starter pack load failed', e); } finally { loading = false; }
	}
	...
	async function doSearch() {
		...
		} catch (e) { console.warn('starter pack search failed', e); }
	}

	function addUser(u: typeof selectedUsers[0]) {
		if (!selectedUsers.find(s => s.did === u.did)) selectedUsers = [...selectedUsers, u];
		searchQuery = ''; searchResults = [];
	}

	function removeUser(did: string) { selectedUsers = selectedUsers.filter(u => u.did !== did); }

	onMount(() => { void load(); });
</script>

<svelte:head><title>Edit Starter Pack — YORO</title></svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<span class="flex-1 text-[17px] font-bold text-gv2-text-primary">Edit Starter Pack</span>
		<button type="button" class="min-h-[36px] rounded-full bg-[#1185FE] px-4 py-1.5 text-[14px] font-bold text-white touch-manipulation active:opacity-80 disabled:opacity-50" disabled={!name.trim()}>Save</button>
	</div>
	{#if loading}
		<div class="p-4"><Skeleton variant="text" class="w-1/2 h-5 mb-3" /><Skeleton variant="text" class="w-full h-4" /></div>
	{:else}
		<div class="flex-1 overflow-y-auto scrollbar-none px-4 py-4">
			...
			<textarea bind:value={description} placeholder="Description" rows="2" class="mb-4 min-h-[60px] w-full rounded-lg border border-gv2-border/40 bg-gv2-bg-primary px-3 py-2 text-[15px] text-gv2-text-primary placeholder:text-gv2-text-muted focus:outline-none focus:ring-2 focus:ring-[#1185FE]/50 resize-none"></textarea>
				...
				<div class="mb-4 divide-y divide-gv2-border/20 rounded-lg border border-gv2-border/40">
					{#each searchResults as u (u.did)}
						<button type="button" class="flex w-full items-center gap-3 px-3 py-2 text-left touch-manipulation active:bg-gv2-bg-hover/40" onclick={() => addUser(u)}>
							...
							<span class="truncate text-[14px] text-gv2-text-primary">{u.displayName || u.handle}</span>
						</button>
					{/each}
				</div>
			{/if}
			{#if selectedUsers.length > 0}
				<h4 class="mb-2 text-[12px] font-bold uppercase tracking-wider text-gv2-text-muted">Members ({selectedUsers.length})</h4>
				{#each selectedUsers as u (u.did)}
					<div class="flex items-center gap-3 rounded-lg px-3 py-2">
						...
						<button type="button" class="text-[13px] text-red-500 touch-manipulation" onclick={() => removeUser(u.did)}>Remove</button>
					</div>
				{/each}
			{/if}
		</div>
