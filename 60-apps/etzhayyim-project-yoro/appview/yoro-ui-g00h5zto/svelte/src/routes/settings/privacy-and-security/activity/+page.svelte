<script lang="ts">
	...
	function goBack() { if (history.length > 1) history.back(); else void goto('/settings/privacy-and-security'); }
	let showLikes = $state(true);
	let showFollows = $state(true);
</script>

<svelte:head><title>Activity Visibility — YORO</title></svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<span class="text-[17px] font-bold text-gv2-text-primary">Activity Visibility</span>
	</div>
	<div class="flex-1 overflow-y-auto scrollbar-none px-4 py-4">
		<p class="mb-4 text-[13px] text-gv2-text-muted">Choose what activity is visible on your profile.</p>
		{#each [{ label: 'Show likes', desc: 'Allow others to see posts you have liked', get: () => showLikes, set: (v: boolean) => showLikes = v }, { label: 'Show follows', desc: 'Allow others to see who you follow', get: () => showFollows, set: (v: boolean) => showFollows = v }] as item}
