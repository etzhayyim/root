<!--
  /profile/{handle}/followers — Bluesky-compatible followers list.
-->
<script lang="ts">
	...
	import { staggerFade } from '@etzhayyim/design-system/motion';
		import { getFollowers, getAuthorProfile, followUser, unfollowUser, getCurrentDID } from '$lib/atproto-agent';
	...
	async function loadFollowers(append = false) {
		...
		try {
			if (!selfDid) try { selfDid = await getCurrentDID(); } catch (e) { console.warn('followers: getCurrentDID failed', e); }
			if (!profile) profile = await getAuthorProfile(handle);
			const did = (profile as any)?.did || handle;
			const result = await getFollowers(did, { limit: 50, cursor: append ? cursor : undefined });
			const items = Array.isArray(result) ? result : (result as any).followers ?? [];
			followers = append ? [...followers, ...items] : items;
			cursor = (result as any)?.cursor;
			hasMore = !!cursor;
		} catch (e) { console.warn('followers: loadFollowers failed', e); } finally {
			loading = false;
		}
	}
	...
	onMount(() => { void loadFollowers(); });
</script>

<svelte:head>
	<title>Followers — {handle} — YORO</title>
</svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<div class="min-w-0 flex-1">
			<span class="block truncate text-[17px] font-bold text-gv2-text-primary">Followers</span>
	...
	{:else if followers.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 p-8">
			<p class="text-[17px] font-bold text-gv2-text-primary">No followers yet</p>
		</div>
	{:else}
		<div class="flex-1 overflow-y-auto scrollbar-none divide-y divide-gv2-border/20">
			{#each followers as f, i (f.did)}
				<button
					type="button"
			...
			{/each}
			{#if hasMore}
				<button type="button" class="w-full py-4 text-center text-[14px] text-[#1185FE] touch-manipulation active:opacity-70" onclick={() => void loadFollowers(true)}>Load more</button>
			{/if}
		</div>
	{/if}
</div>
