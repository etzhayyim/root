<!--
  /profile/{handle}/post/{rkey}/liked-by — Who liked this post.
-->
<script lang="ts">
	...
	import { getLikes, getAuthorProfile } from '$lib/atproto-agent';
	...
	let likes = $state<Array<{ did: string; handle: string; displayName?: string; avatar?: string }>>([]);
	...
	async function loadLikes(append = false) {
		...
		try {
			...
			const result = await getLikes(uri, { limit: 50, cursor: append ? cursor : undefined } as any);
			const items = Array.isArray(result) ? result : (result as any).likes ?? [];
			likes = append ? [...likes, ...items] : items;
			cursor = (result as any)?.cursor;
			hasMore = !!cursor;
		} catch (e) { console.warn('liked-by: loadLikes failed', e); } finally {
			loading = false;
		}
	}
	...
	onMount(() => { void loadLikes(); });
</script>

<svelte:head>
	<title>Liked by — YORO</title>
</svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<span class="text-[17px] font-bold text-gv2-text-primary">Liked by</span>
	</div>
	...
	{:else if likes.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 p-8">
			<p class="text-[17px] font-bold text-gv2-text-primary">No likes yet</p>
		</div>
	{:else}
		<div class="flex-1 overflow-y-auto scrollbar-none divide-y divide-gv2-border/20">
			{#each likes as u, i (u.did)}
				<button
					type="button"
			...
			{/each}
			{#if hasMore}
				<button type="button" class="w-full py-4 text-center text-[14px] text-[#1185FE] touch-manipulation active:opacity-70" onclick={() => void loadLikes(true)}>Load more</button>
			{/if}
		</div>
	{/if}
</div>
