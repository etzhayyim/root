<script lang="ts">
	...
	function goBack() { if (history.length > 1) history.back(); else void goto('/moderation'); }

	let modlists = $state<Array<{ uri: string; name: string; description?: string; avatar?: string }>>([]);
	...
	async function load() {
		...
		try {
			...
			const all = Array.isArray(result) ? result : (result as any).lists ?? [];
			modlists = all.filter((l: any) => l.purpose === 'app.bsky.graph.defs#modlist');
		} catch (e) { console.warn('modlists load failed', e); } finally { loading = false; }
	}

	onMount(() => { void load(); });
</script>

<svelte:head><title>Moderation Lists — YORO</title></svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex min-h-[48px] items-center gap-3 border-b border-gv2-border/40 bg-gv2-bg-primary/90 material-blur sticky top-0 z-10 px-4">
		...
		<span class="text-[17px] font-bold text-gv2-text-primary">Moderation Lists</span>
	</div>
	{#if loading}
		<div class="p-4" in:fade={staggerFade(0, { duration: 300 })}>
			{#each { length: 4 } as _}<div class="flex items-center gap-3 py-3"><Skeleton variant="rectangular" class="h-10 w-10" /><Skeleton variant="text" class="w-1/3 h-4" /></div>{/each}
		</div>
	{:else if modlists.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 p-8">
			<p class="text-[17px] font-bold text-gv2-text-primary">No moderation lists</p>
			<p class="text-[14px] text-gv2-text-muted text-center">Moderation lists allow you to mute or block groups of accounts.</p>
		</div>
	{:else}
		<div class="flex-1 overflow-y-auto scrollbar-none divide-y divide-gv2-border/20">
			{#each modlists as l, i (l.uri)}
				<div class="flex items-center gap-3 px-4 py-3" in:fade={staggerFade(i, { duration: 150 })}>
					<Avatar src={l.avatar || undefined} fallback={(l.name || '?').slice(0, 2).toUpperCase()} size="sm" class="!h-10 !w-10 rounded-lg" />
