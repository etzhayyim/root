# ai-gftd-project-gameka — Game Studio Actor (BPMN-as-actor)
...
(issues from prior   │     ▲                              │   │
 iterations)         │     └─── revise (score < 0.8) ─────┘   │
                     ...
                     │   evaluate __gftdProbe(5000) → metrics   │
...
## Lexicons (`00-contracts/lexicons/ai/gftd/apps/gameka/`)
...
**Probe contract** (`window.__gftdProbe(durationMs)` matches the
...
`pkg/*.wasm` + `pkg/*.js` to `b2://ai-gftd-gameka/builds/{wasmCid}.wasm`.
...
DATABASE_URL=$(security find-generic-password -s gftd.rw -a ROOT_URL -w) pnpm db:migrate latest
pnpm db:gen
pnpm db:drift
