// Apps Directory thin edge facade. Listing/search/install intent logic runs in AgentGateway MCP + pod-side LangServer.
...
const NSID_PREFIX = "com.etzhayyim.apps.apps.";
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/healthz" || url.pathname === "/readyz" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: "did:web:apps.etzhayyim.com",
        nanoid: env.APP_NANOID ?? "a9p5l1st",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        businessLogic: "40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/ingest/apps_directory.py",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/appsDirectory",
      });
    }
    ...
    return json({ error: "NotFound", message: "apps directory not found" }, 404);
  },
