...
const SHINKA_NS = "ai.gftd.apps.shinka";
...
async function processJobQueue(sdk: HostSDK, partition?: string): Promise<{ processed: number; failed: number }> {
  ...
  for (const job of claimed) {
    ...
    try {
      ...
      if (sponsorDid) {
        try {
          await sdk.pds.xrpc("ai.gftd.apps.credits.rewardFromCompute", {
...
async function fetchDomainResearch(domain: string): Promise<string> {
  try {
    ...
    const resp = await globalThis.fetch(url, { headers: { "user-agent": "gftd-shinka/1.0" } });
    if (!resp.ok) return "";
...
async function cmdForceShinka(sdk: HostSDK, body: unknown): Promise<unknown> {
  const { did } = parseLexiconInput("ai.gftd.apps.shinka.forceShinka", body);
...
async function cmdSeedPropagation(sdk: HostSDK, body: unknown): Promise<unknown> {
  const params = parseLexiconInput("ai.gftd.apps.shinka.seedPropagation", body);
...
export function setup(sdk: HostSDK): void {
  ...
  sdk.app.command(nsid("ai.gftd.apps.shinka.listTasks"), (_ctx, body) => cmdListTasks(sdk, body));
  console.log("[shinka-setup] listTasks registered");

  sdk.app.command(nsid("ai.gftd.apps.shinka.stats"), (_ctx, body) => cmdStats(sdk, body));
  console.log("[shinka-setup] stats registered");

  sdk.app.command(nsid("ai.gftd.apps.shinka.forceShinka"), (_ctx, body) => cmdForceShinka(sdk, body));
  console.log("[shinka-setup] forceShinka registered");

  sdk.app.command(nsid("ai.gftd.apps.shinka.seedPropagation"), (_ctx, body) => cmdSeedPropagation(sdk, body));
  console.log("[shinka-setup] seedPropagation registered");

  sdk.app.command(nsid("ai.gftd.apps.shinka.generatePropagationChain"), async (_ctx, body) => {
    /** Generate a propagation chain for a historical event via LLM. Returns chain JSON to pass to seedPropagation. */
    const { title, eventAt, location, description, involvedActors } = parseLexiconInput("ai.gftd.apps.shinka.generatePropagationChain", body);
  ...
  // Job Queue commands
  sdk.app.command(nsid("ai.gftd.apps.shinka.claimJobs"), async (_ctx, body) => {
    /** Claim and process pending jobs from the graph job queue. */
    const { partition, batchSize } = parseLexiconInput("ai.gftd.apps.shinka.claimJobs", body);
    return processJobQueue(sdk, partition);
  });

  sdk.app.command(nsid("ai.gftd.apps.shinka.queueStats"), async () => {
  ...
  sdk.app.command(nsid("ai.gftd.apps.shinka.sponsorEvent"), async (_ctx, body) => {
    /** Spend credits to sponsor a historical event's propagation (priority boost). */
    const { userId, eventId, credits } = parseLexiconInput("ai.gftd.apps.shinka.sponsorEvent", body);
    ...
    try {
      await sdk.pds.xrpc("ai.gftd.apps.credits.spendCredits", {
  ...
  sdk.app.command(nsid("ai.gftd.apps.shinka.listSponsorable"), async (_ctx, body) => {
    /** List historical events available for sponsorship. */
    const { limit } = parseLexiconInput("ai.gftd.apps.shinka.listSponsorable", body);
    try {
  ...
  sdk.app.command(nsid("ai.gftd.apps.shinka.listPartitions"), async () => {
  ...
  // Cron handler — delegates to LangServer batchTick.bpmn (ai.gftd.shinka.tick × 10 actors).
  // Heavy work: 10 actors × up to 3 LLM calls each exceeds CF Worker 25s budget (ADR-0056).
  sdk.app.onHeartbeat(async () => {
    await proxyToBpmn(sdk, "ai.gftd.apps.shinka.batchTick", {});
    return [];
  });
}
