...
def emit_country(iso3, out_dir):
    ...
    profile_rec = {
        "$type": "ai.gftd.apps.states.stateProfile",
        "iso3": iso3, "name": name,
    ...
    (out_dir / "profile" / f"{iso3}.json").write_text(
        json.dumps(put_body("ai.gftd.apps.states.stateProfile", iso3, profile_rec), ensure_ascii=False))
    ...
    for m in ministries[:20]:  # cap per-country to control volume
        ...
        has_bpmn = bpmn_name in bpmn_files
        proc_rec = {
            "$type": "ai.gftd.apps.states.stateProcedure",
            "iso3": iso3, "path": path,
            "title": m.get("nameEn") or m.get("name", ""),
            "titleLocal": m.get("name", ""),
            "authority": m.get("name", ""),
        ...
        (out_dir / "procedure" / f"{rkey}.json").write_text(
            json.dumps(put_body("ai.gftd.apps.states.stateProcedure", rkey, proc_rec), ensure_ascii=False))
        proc_count += 1
    ...
    for c in contracts[:15]:  # cap
        ...
        doc_rec = {
            "$type": "ai.gftd.apps.states.stateDocument",
        ...
        (out_dir / "document" / f"{rkey}.json").write_text(
            json.dumps(put_body("ai.gftd.apps.states.stateDocument", rkey, doc_rec), ensure_ascii=False))
        doc_count += 1
