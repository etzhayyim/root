...
- `infra/magatama-runtime/org-gov-rus-dst-50899621-kosh-agachsky-rayon/app.yaml` – App CR; `metadata.name=image stem`, namespace `magatama-runtime`, image `ghcr.io/gftdcojp/org-gov-rus-dst-50899621-kosh-agachsky-rayon:<tag>`, env for endpoint + ADM payload.  
- `infra/magatama-runtime/org-gov-rus-dst-50899621-kosh-agachsky-rayon/service.yaml` – ClusterIP Service (port 8080) for App.  
- `infra/magatama-runtime/org-gov-rus-dst-50899621-kosh-agachsky-rayon/edge-route.yaml` – `HTTPRoute` in `edge-router-performers`, host `https://{nanoid}.gftd.ai/api/grpc`, parent gateway refs, backend Service.  
...
- `HTTPRoute/org-gov-rus-dst-50899621-kosh-agachsky-rayon` (ns `edge-router-performers`, host `xxxxx.gftd.ai`).

**Quality Gates Checklist**  
- [ ] `metadata.name`, Service name, slug, and image stem identical.  
- [ ] Image hosted at `ghcr.io/gftdcojp/org-gov-rus-dst-50899621-kosh-agachsky-rayon:<tag>`; tag pinned.  
- [ ] App env exports `SHAPE_ID=50074027B11056050899621`, `COUNTRY_ISO3=RUS`, `CONNECT_ENDPOINT=https://{nanoid}.gftd.ai/api/grpc`.  
