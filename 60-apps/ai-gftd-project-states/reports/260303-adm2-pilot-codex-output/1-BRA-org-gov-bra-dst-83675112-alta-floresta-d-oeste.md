...
**Required Files List**  
- `deploy/.../org-gov-bra-dst-83675112-alta-floresta-d-oeste.yaml`: App CR with namespace `magatama-runtime`, image `ghcr.io/gftdcojp/org-gov-bra-dst-83675112-alta-floresta-d-oeste:<<tag>>`, endpoint `https://<nanoid>.gftd.ai/api/grpc`.  
...
- `GRPCRoute/org-gov-bra-dst-83675112-alta-floresta-d-oeste` (ns `edge-router-performers`, host `*.gftd.ai`).  
- If telemetry needed: `ServiceMonitor/org-gov-bra-dst-83675112-alta-floresta-d-oeste` (optional).

**Quality Gates Checklist**  
- App image/tag hosted on `ghcr.io/gftdcojp/*`, stem exactly slug.  
- Endpoint uses `https://{nanoid}.gftd.ai/api/grpc`; no legacy subdomains.  
