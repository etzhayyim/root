...
**Required Files List**
- `deploy/magatama/org-gov-rou-dst-36487680-nasturelu.app.yaml` – App spec (namespace `magatama-runtime`, image pulled from `ghcr.io/gftdcojp/org-gov-rou-dst-36487680-nasturelu`, endpoint `https://<nanoid>.gftd.ai/api/grpc`).
- `deploy/edge/org-gov-rou-dst-36487680-nasturelu.hhroute.yaml` – Edge HTTPRoute in `edge-router-performers`, referencing magatama service.
...
- `App/org-gov-rou-dst-36487680-nasturelu` (ns `magatama-runtime`).
- `Service/org-gov-rou-dst-36487680-nasturelu` (ns `magatama-runtime`) to expose magatama port.
- `HTTPRoute/org-gov-rou-dst-36487680-nasturelu` (ns `edge-router-performers`) with hostname `<nanoid>.gftd.ai`, backend reference to service.
- `ConfigMap/org-gov-rou-dst-36487680-nasturelu` (ns `magatama-runtime`) for pilot metadata.

**Quality Gates Checklist**
- Image built/pushed to `ghcr.io/gftdcojp/org-gov-rou-dst-36487680-nasturelu:<tag>`; metadata.name matches stem.
...
- Service ports expose magatama listener (default 3000) with `touch-action: manipulation` UI compliance noted.
