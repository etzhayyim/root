...
- `magatama/org-gov-jpn-dst-37401071-ine.magatama.toml` (Magatama app manifest referencing `ghcr.io/gftdcojp/org-gov-jpn-dst-37401071-ine:<tag>` and `https://{nanoid}.gftd.ai/api/grpc`)
...
- HTTPRoute: `org-gov-jpn-dst-37401071-ine-route` (namespace `edge-router-performers`, parent refs to Edge Gateway, host `*.gftd.ai`)
...
- magatama image hosted at `ghcr.io/gftdcojp/org-gov-jpn-dst-37401071-ine:<tag>`; no Harbor usage.
- App references Connect gRPC-Web endpoint `https://{nanoid}.gftd.ai/api/grpc` and uses `@connectrpc/connect-web`.
