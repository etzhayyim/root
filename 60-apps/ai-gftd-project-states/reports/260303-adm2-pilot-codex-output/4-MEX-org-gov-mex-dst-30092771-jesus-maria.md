...
- Nanoid `30092771` aligns with required endpoint convention (`https://30092771.gftd.ai/api/grpc`) and can be reused across manifests.
...
- App: `org-gov-mex-dst-30092771-jesus-maria`  
  ...
  - `spec.image`: `ghcr.io/gftdcojp/org-gov-mex-dst-30092771-jesus-maria:<<tag>>` (stem matches metadata).  
  - `spec.template.env`: include `CONNECT_ENDPOINT=https://30092771.gftd.ai/api/grpc`, `OBJSTORE_SITE_BUCKETS=jesus-maria`.  
- Edge HTTPRoute: `org-gov-mex-dst-30092771-jesus-maria-route`  
  - Hostname `jesus-maria.gftd.ai` (or shared subdomain per DNS strategy).  
...
- Container image from `ghcr.io/gftdcojp/*` with explicit tag; `imagePullSecrets: ghcr-pull-secret`.  
- Endpoint respects `https://{nanoid}.gftd.ai/api/grpc`.  
- UI plan references Apple HIG / iPad breakpoints; Svelte uses Tailwind + `@gftdcojp` components, no `<style>` blocks.  
