// govAtg state app thin facade. Domain logic runs in AgentGateway MCP + pod-side LangServer gov primitives.
...
const ACTOR_DID = "did:web:atg-state.etzhayyim.com";
const NSID_PREFIX = "app.etzhayyim.govAtg.";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/healthz" || url.pathname === "/readyz" || url.pathname === "/_app/meta") {
      return json({
        ...
        bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/govAtg",
      });
    }
