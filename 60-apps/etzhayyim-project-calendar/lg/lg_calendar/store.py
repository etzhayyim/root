...
- ``KotobaCalendarStore`` — production, backed by kotoba datomic (graph
  ``calendar-v1``) via :class:`lg_calendar.kotoba_datomic.KotobaDatomic`.
- ``FakeCalendarStore`` — in-memory dict, used by the unit/smoke tests so the
...
class CalendarStore(Protocol):
    async def get_event_attrs(self, slug: str) -> dict[str, Any] | None: ...
    async def all_event_attrs(self) -> list[dict[str, Any]]: ...
...
class KotobaCalendarStore:
    ...
    async def all_event_attrs(self) -> list[dict[str, Any]]:
        rows = await self._dm.q('[:find ?slug :where [?e :cal/type "Event"] [?e :cal/slug ?slug]]')
        out: list[dict[str, Any]] = []
        for row in rows:
            if not row:
                continue
            attrs = await self.get_event_attrs(str(row[0]))
            if attrs:
                out.append(attrs)
        return out
    ...
    async def lookup_slug(self, attr: str, value: Any) -> str | None:
        ...
        query = f"[:find ?slug :where [?e :{bare} {encode(value)}] [?e :cal/slug ?slug]]"
