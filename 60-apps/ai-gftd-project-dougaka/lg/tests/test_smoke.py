"""Local smoke tests for lg-dougaka OSS server.

Run from `60-apps/ai-gftd-project-dougaka/lg/`:
...
touching `60-apps/ai-gftd-project-dougaka/lg/**`.
"""
...
_EXPECTED_NSID_MAP = {
    "ai.gftd.apps.dougaka.render": "render",
}


def test_server_module_imports():
    from lg_dougaka.server import GRAPHS, NSID_MAP, app
...
def test_graphs_match_expected_set():
    from lg_dougaka.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_dougaka.server import NSID_MAP
    assert NSID_MAP == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_dougaka.server import GRAPHS, NSID_MAP
...
@pytest.fixture
def client():
    from lg_dougaka.server import app
...
def test_unknown_nsid_xrpc_404(client):
    r = client.post("/xrpc/ai.gftd.apps.dougaka.unknownMethod", json={})
    assert r.status_code == 404
