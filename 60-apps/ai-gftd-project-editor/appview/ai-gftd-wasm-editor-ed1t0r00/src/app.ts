...
async function cmdCreateProject(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.createProject", body);
...
async function cmdListProjects(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.listProjects", body);
...
async function cmdWriteFile(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.writeFile", body) as WriteFileArgs;
...
async function cmdReadFile(_sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.readFile", body);
  if (!args.projectId || !args.path) return { error: "projectId and path required" };
  const db = createKyselyDb();
...
async function cmdListFiles(_sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.listFiles", body);
...
async function cmdGenerate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.editor.generate", body);
...
document.getElementById("newProject").onclick = async () => {
  ...
  if (!name) return;
  const res = await xrpc("ai.gftd.apps.editor.createProject", { name, template: "react" });
...
document.getElementById("saveBtn").onclick = async () => {
  ...
  const res = await xrpc("ai.gftd.apps.editor.writeFile", {
...
document.getElementById("generateBtn").onclick = async () => {
  ...
  try {
    const res = await xrpc("ai.gftd.apps.editor.generate", {
...
const _inner = createWorkerExport((sdk) => {
  ...
  sdk.app
    .command(nsid("ai.gftd.apps.editor.createProject"), (_ctx, body) => cmdCreateProject(sdk, body),
      asAgentTool("Create a new code editor project"),
      withCapabilityTags("editor", "project", "create"),
      withOCELEvent("editor.project.created"),
    )
    .query(nsid("ai.gftd.apps.editor.listProjects"), (_ctx, body) => cmdListProjects(sdk, body))
    .command(nsid("ai.gftd.apps.editor.writeFile"), (_ctx, body) => cmdWriteFile(sdk, body),
    ...
    .query(nsid("ai.gftd.apps.editor.readFile"), (_ctx, body) => cmdReadFile(sdk, body))
    .query(nsid("ai.gftd.apps.editor.listFiles"), (_ctx, body) => cmdListFiles(sdk, body))
    .command(nsid("ai.gftd.apps.editor.generate"), (_ctx, body) => cmdGenerate(sdk, body),
