// Vessel Registry thin edge. Business logic runs in AgentGateway MCP + pod-side LangServer.
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: "did:web:v3ss3l01.etzhayyim.com",
        nanoid: env.APP_NANOID ?? "v3ss3l01",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        businessLogic: "40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/ingest/vessel.py",
    ...
    return json({ error: "NotFound", message: "vessel not found" }, 404);
  },
