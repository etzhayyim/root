# ai-gftd-project-comprehend: Scheduler Design (v0)

## Goal

Crawler/collector を使って、世界中の以下カテゴリを JSON-LD (RDF) として収集し、SHACL 形状で品質を定義し、`ai-gftd-project-resources` に git 永続化する。
...
2. `gftd-comprehend` が `config/targets.json` を読み、collector を実行
3. collector 出力を JSON-LD に正規化し `gitstate` 経由で `60-apps/ai-gftd-project-resources/content/` 配下に保存
...
- `60-apps/ai-gftd-project-resources/content/comprehend/domain/<domain>/latest.jsonld`
- `60-apps/ai-gftd-project-resources/content/comprehend/ip/<ip>/latest.jsonld`
- `60-apps/ai-gftd-project-resources/content/comprehend/runs/<run_id>.jsonld`

## SHACL

SHACL shapes は JSON-LD で `60-apps/ai-gftd-project-resources/shacl/comprehend/shapes.jsonld` に配置。
