...
Lexicon: `00-contracts/lexicons/ai/gftd/apps/publicDomainColorization/colorizeWork.json`
...
```sh
node 60-apps/ai-gftd-project-public-domain-colorization/scripts/pdcolor-demo-ingest.mjs --work gertie-the-dinosaur-1914
...
After the `ai-gftd-ipfs-proxy` Worker with `/gftd/v1/demo/ingest-public-domain` is deployed, the script can ask the Worker to fetch an allowlisted demo source and add it without exposing `IPFS_HMAC` to the caller:

```sh
node 60-apps/ai-gftd-project-public-domain-colorization/scripts/pdcolor-demo-ingest.mjs --work gertie-the-dinosaur-1914 --worker-ingest
```
