  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ...
        methods: [
          "probeEnvironment", "transferSignal", "formColony",
          "handoffToKi", "lyse",
        ],
        mcp: {
          envelope: MCP_NSID,
    ...
    const isAppRoute = nsid.startsWith(NSID_PREFIX);
    const isMcpRoute = nsid === MCP_NSID;
    if ((isAppRoute || isMcpRoute) && (req.method === "POST" || req.method === "GET")) {
