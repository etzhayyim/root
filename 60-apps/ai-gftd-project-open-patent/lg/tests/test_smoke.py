"""Local smoke tests for lg-open-patent OSS server.

Run from `60-apps/ai-gftd-project-open-patent/lg/`:
...
touching `60-apps/ai-gftd-project-open-patent/lg/**`.
"""
...
def test_server_module_imports():
    from lg_open_patent.server import GRAPHS, NSID_MAP, app
    assert app is not None
    assert isinstance(GRAPHS, dict)
    assert isinstance(NSID_MAP, dict)


def test_graphs_match_expected_set():
    from lg_open_patent.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_open_patent.server import NSID_MAP
    assert NSID_MAP == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_open_patent.server import GRAPHS, NSID_MAP
    for nsid, graph_name in NSID_MAP.items():
        assert graph_name in GRAPHS, f"NSID_MAP[{nsid!r}] → {graph_name!r} not in GRAPHS"
...
def test_langgraph_json_has_crons_for_scheduled_graphs():
    ...
    assert "ingest_multi" in cron_graphs, "ingest_multi must have a daily cron"
    assert "synthesize_invention" in cron_graphs, "synthesize_invention must have a weekly cron"
...
def client(monkeypatch):
    ...
    from lg_open_patent import server as srv
...
def test_unknown_nsid_xrpc_404(client):
    r = client.post("/xrpc/ai.gftd.apps.openPatent.unknownMethod", json={})
    assert r.status_code == 404
