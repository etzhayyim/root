...
async function cmdScanCreate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.sense.scan.create", body);
...
async function cmdScanUpdate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { sessionId, frame } = parseLexiconInput("ai.gftd.apps.sense.scan.update", body);
...
async function cmdScanComplete(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { sessionId } = parseLexiconInput("ai.gftd.apps.sense.scan.complete", body);
  return { sessionId, status: "processing" };
}

async function cmdScanGet(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { sessionId } = parseLexiconInput("ai.gftd.apps.sense.scan.get", body);
...
async function cmdScanList(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.sense.scan.list", body);
...
async function cmdBuildingCreate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { sessionId, name, meshCid, metadata } = parseLexiconInput("ai.gftd.apps.sense.building.create", body);
  const now = nowISO();
...
async function cmdBuildingGet(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId } = parseLexiconInput("ai.gftd.apps.sense.building.get", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
  return parsed[0] ?? null;
}
...
async function cmdBuildingList(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.sense.building.list", body);
  const limit = Math.min(Number(args.limit) || 20, 200);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
...
async function cmdBuildingExport(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, format } = parseLexiconInput("ai.gftd.apps.sense.building.export", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
...
async function cmdFloorCreate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, floorNumber, boundary, height } = parseLexiconInput("ai.gftd.apps.sense.floor.create", body);
...
async function cmdFloorGet(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, floorNumber } = parseLexiconInput("ai.gftd.apps.sense.floor.get", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
  return parsed[0] ?? null;
}
...
async function cmdRoomCreate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { floorId, name, boundary, acousticProfile, material } = parseLexiconInput("ai.gftd.apps.sense.room.create", body);
...
async function cmdRoomGet(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { roomId } = parseLexiconInput("ai.gftd.apps.sense.room.get", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
  return parsed[0] ?? null;
}
...
async function cmdStructureDetect(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, meshCid } = parseLexiconInput("ai.gftd.apps.sense.structure.detect", body);
...
async function cmdStructureGet(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { structureId } = parseLexiconInput("ai.gftd.apps.sense.structure.get", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
  return parsed[0] ?? null;
}
...
async function cmdStructureList(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const args = parseLexiconInput("ai.gftd.apps.sense.structure.list", body);
...
async function cmdStructureCrossSection(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, plane } = parseLexiconInput("ai.gftd.apps.sense.structure.crossSection", body);
...
async function cmdSensorRegister(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { deviceType, deviceId, capabilities } = parseLexiconInput("ai.gftd.apps.sense.sensor.register", body);
...
async function cmdSensorCalibrate(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { deviceId, calibrationData } = parseLexiconInput("ai.gftd.apps.sense.sensor.calibrate", body);
...
async function cmdSensorStatus(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { deviceId } = parseLexiconInput("ai.gftd.apps.sense.sensor.status", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
  const parsed = parseYataRows(rows);
  return parsed[0] ?? null;
}
...
async function cmdVizRender(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, cameraPose, options } = parseLexiconInput("ai.gftd.apps.sense.viz.render", body);
...
async function cmdVizHeatmap(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId, floorNumber, heatmapType } = parseLexiconInput("ai.gftd.apps.sense.viz.heatmap", body);
...
async function cmdVizTimeline(sdk: HostSDK, body: Uint8Array): Promise<unknown> {
  const { buildingId } = parseLexiconInput("ai.gftd.apps.sense.viz.timeline", body);
  const rows = [] as Record<string, unknown>[]; // SQL deprecated 2026-04-12
...
export default createWorkerExport((sdk) => {
  sdk.app
    // Scan Session
    .command(nsid("ai.gftd.apps.sense.scan.create"), (ctx, body) => cmdScanCreate(sdk, body),
      asAgentTool("Create a new sensor scan session"),
      withCapabilityTags("sensor-fusion", "3d-reconstruction"),
    )
    .command(nsid("ai.gftd.apps.sense.scan.update"), (ctx, body) => cmdScanUpdate(sdk, body),
      asAgentTool("Add sensor frame to scan session"),
      withCapabilityTags("sensor-fusion"),
    )
    .command(nsid("ai.gftd.apps.sense.scan.complete"), (ctx, body) => cmdScanComplete(sdk, body),
      asAgentTool("Complete scan and trigger fusion pipeline"),
      withCapabilityTags("sensor-fusion", "3d-reconstruction"),
    )
    .command(nsid("ai.gftd.apps.sense.scan.get"), (ctx, body) => cmdScanGet(sdk, body),
      asAgentTool("Get scan session details"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.scan.list"), (ctx, body) => cmdScanList(sdk, body),
      asAgentTool("List scan sessions"),
      withCapabilityTags("domain-query"),
    )
    // Building Model
    .command(nsid("ai.gftd.apps.sense.building.create"), (ctx, body) => cmdBuildingCreate(sdk, body),
      asAgentTool("Create 3D building model from scan"),
      withCapabilityTags("3d-reconstruction", "data-management"),
    )
    .command(nsid("ai.gftd.apps.sense.building.get"), (ctx, body) => cmdBuildingGet(sdk, body),
      asAgentTool("Get building 3D model"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.building.list"), (ctx, body) => cmdBuildingList(sdk, body),
      asAgentTool("List buildings"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.building.export"), (ctx, body) => cmdBuildingExport(sdk, body),
      asAgentTool("Export building model as glTF/PLY/OBJ"),
      withCapabilityTags("3d-reconstruction"),
    )
    // Floor & Room
    .command(nsid("ai.gftd.apps.sense.floor.create"), (ctx, body) => cmdFloorCreate(sdk, body),
      asAgentTool("Create floor plan"),
      withCapabilityTags("3d-reconstruction", "data-management"),
    )
    .command(nsid("ai.gftd.apps.sense.floor.get"), (ctx, body) => cmdFloorGet(sdk, body),
      asAgentTool("Get floor plan"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.room.create"), (ctx, body) => cmdRoomCreate(sdk, body),
      asAgentTool("Create room data with acoustic profile"),
      withCapabilityTags("sensor-fusion", "data-management"),
    )
    .command(nsid("ai.gftd.apps.sense.room.get"), (ctx, body) => cmdRoomGet(sdk, body),
      asAgentTool("Get room data including structures and acoustics"),
      withCapabilityTags("domain-query"),
    )
    // Structure Analysis
    .command(nsid("ai.gftd.apps.sense.structure.detect"), (ctx, body) => cmdStructureDetect(sdk, body),
      asAgentTool("Detect structures (wall/column/beam/pipe/wiring)"),
      withCapabilityTags("3d-reconstruction", "sensor-fusion"),
    )
    .command(nsid("ai.gftd.apps.sense.structure.get"), (ctx, body) => cmdStructureGet(sdk, body),
      asAgentTool("Get structure details"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.structure.list"), (ctx, body) => cmdStructureList(sdk, body),
      asAgentTool("List structures in building"),
      withCapabilityTags("domain-query"),
    )
    .command(nsid("ai.gftd.apps.sense.structure.crossSection"), (ctx, body) => cmdStructureCrossSection(sdk, body),
      asAgentTool("Generate cross-section view"),
      withCapabilityTags("3d-reconstruction"),
    )
    // Sensor Device
    .command(nsid("ai.gftd.apps.sense.sensor.register"), (ctx, body) => cmdSensorRegister(sdk, body),
      asAgentTool("Register sensor device"),
      withCapabilityTags("sensor-fusion", "data-management"),
    )
    .command(nsid("ai.gftd.apps.sense.sensor.calibrate"), (ctx, body) => cmdSensorCalibrate(sdk, body),
      asAgentTool("Calibrate sensor device"),
      withCapabilityTags("sensor-fusion"),
    )
    .command(nsid("ai.gftd.apps.sense.sensor.status"), (ctx, body) => cmdSensorStatus(sdk, body),
      asAgentTool("Get sensor device status"),
      withCapabilityTags("domain-query"),
    )
    // Visualization
    .command(nsid("ai.gftd.apps.sense.viz.render"), (ctx, body) => cmdVizRender(sdk, body),
      asAgentTool("Request 3D render of building"),
      withCapabilityTags("3d-reconstruction"),
    )
    .command(nsid("ai.gftd.apps.sense.viz.heatmap"), (ctx, body) => cmdVizHeatmap(sdk, body),
      asAgentTool("Generate WiFi/BT/acoustic heatmap"),
      withCapabilityTags("sensor-fusion"),
    )
    .command(nsid("ai.gftd.apps.sense.viz.timeline"), (ctx, body) => cmdVizTimeline(sdk, body),
