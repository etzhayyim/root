"""Local smoke tests for lg-pregel OSS server.

Run from `60-apps/etzhayyim-project-pregel/lg/`:
...
touching `60-apps/etzhayyim-project-pregel/lg/**`.
"""
...
os.environ.setdefault("LG_PREGEL_API_KEY", "")
...
def test_server_module_imports():
    from lg_pregel.server import GRAPHS, app
    assert app is not None
    assert isinstance(GRAPHS, dict)


def test_graphs_match_expected_set():
    from lg_pregel.server import GRAPHS
...
def test_langgraph_json_has_crons():
    cfg = json.loads((_LG_DIR / "langgraph.json").read_text())
    cron_graph_ids = {c.get("graph_id") or c.get("graph") for c in cfg.get("crons", [])}
    assert "outlook_triage" in cron_graph_ids, "outlook_triage must have a cron"
    assert "projector_driver" in cron_graph_ids, "projector_driver must have a cron"
...
@pytest.fixture
def client():
    from lg_pregel.server import app
