...
- **mDNS**: `_gftd-celler._tcp.local.` (同一 LAN)
...
既存 NSID 再利用: `ai.gftd.rtc.*` (signaling), `ai.gftd.signal.*` (E2E), `chat.bsky.convo.*` (messaging)
...
|---|---|
| ai-gftd-project-phone | PSTN bridge (AWS Connect, legacy path) |
| ai-gftd-project-network-mobile | mesh infra sibling |
| ai-gftd-project-murakumo | AI inference (STT/TTS/translation) |
| ai-gftd-project-maps | mesh topology visualization |
