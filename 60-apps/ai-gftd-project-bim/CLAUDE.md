...
`ai-gftd-project-bim` の権威ルール。この project は **App** として設計・実装する。
...
- 公開 contract は XRPC (`ai.gftd.apps.bim.*`)
...
|---|---|---|
| Project | `ai.gftd.apps.bim.project` | `vertex_bim_project` |
| Revision | `ai.gftd.apps.bim.revision` | `vertex_bim_revision` |
| Building | `ai.gftd.apps.bim.building` | `vertex_bim_building` |
| Storey | `ai.gftd.apps.bim.storey` | `vertex_bim_storey` |
| Space | `ai.gftd.apps.bim.space` | `vertex_bim_space` |
| Element | `ai.gftd.apps.bim.element` | `vertex_bim_element` |
| PropertySet | `ai.gftd.apps.bim.propertySet` | `vertex_bim_pset` |
| Annotation | `ai.gftd.apps.bim.annotation` | `vertex_bim_annotation` |
| ImportJob / ExportJob | `ai.gftd.apps.bim.importJob` / `exportJob` | `vertex_bim_job` |
...
| NSID | Type | 用途 |
|---|---|---|
| `ai.gftd.apps.bim.importIfc` | procedure | IFC → tessellation job enqueue |
| `ai.gftd.apps.bim.getStoreyScene` | query | storey 単位の scene projection |
| `ai.gftd.apps.bim.listSpaces` | query | room schedule / area take-off |
| `ai.gftd.apps.bim.annotateElement` | procedure | 要素付き comment / issue (BCF viewpoint) |
| `ai.gftd.apps.bim.requestExport` | procedure | IFC / glTF / BCF / xlsx / PDF export job |
...
- Blob: IFC 原本・glTF 派生・BCF zip は B2 (`ai-gftd-bim` bucket: `bim/blobs/{sha256}`, `bim/meshes/{sha256}`, `bim/exports/{sha256}`)。PDS `uploadBlob` の SHA-256 content-addressed convention を踏襲

## Collaboration (Phase 1 = review / annotate / presence)

- `subscribeRepos`: `app.bsky.feed.*` + `app.bsky.graph.follow` + `ai.gftd.apps.bim.annotation` + `ai.gftd.apps.bim.revision`
...
- `sdk.pds.createRecord` で `ai.gftd.apps.bim.*` を書くこと禁止 (ADR-0036, Hyperdrive 直接)
