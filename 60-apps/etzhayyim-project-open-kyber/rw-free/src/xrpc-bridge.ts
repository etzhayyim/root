 ...
 * ADR-2605262130 — no RisingWave) and route every command through the tested rw-free
 * functions instead, with the kotoba Datom log as the canonical store.
 *
 *   worker cmd handler ── createXrpcBridge(sdk.pds, { did }) ──▶ rw-free fn ──▶ Datom log
 *
...
/**
 * Adapt an AT-repo XrpcClient to the `Etzhayyim` read/write surface the rw-free functions
 * use. Returns an object usable as the `e` argument to every rw-free function.
 */
export function createXrpcBridge(pds: XrpcRepoClient, opts: BridgeOptions): Etzhayyim {
  ...
  // The rw-free functions only touch read / write / encryptedRead / encryptedWrite / did.
