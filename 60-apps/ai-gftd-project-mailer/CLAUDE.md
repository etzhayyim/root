...
§gftdcojp Agent / Email Policy (2026-05-28) で **mailer.gftd.ai = repo-wide
primary email platform** と再定義された。よって vendor 側 (`gftdcojp`) でも
本ソースを SSoT として保持する (旧 `DEPRECATED.md` は除去)。etzhayyim 側にも
canonical copy が存在しうるが、`*.gftd.ai` 本番 worker (`magatama-a8wwtz73`,
CF account `ai-gftd-cloud`) の lifecycle は gftdcojp が担う。

> Split-brain 注意: etzhayyim/root も同名アプリを出荷しうる。`mailer.gftd.ai`
> 本番 worker は vendor がデプロイ主体。etzhayyim CI が同 worker を上書きする
> 構成なら停止調整が必要。

### Live appview deploy

- **Worker**: `magatama-a8wwtz73` (routes: `mailer.gftd.ai/*`, `a8wwtz73.gftd.ai/*`,
  `ai-gftd-project-mailer.gftd.ai/*`, `8wwtz73p.gftd.ai/*`)
- **Source**: `appview/mailer-mcp-component/` (SvelteKit edge BFF)
- **Read path**: `/api/{emails,stats,bindings}` → `proxyToDispatcher` → POST
  `https://dispatcher.gftd.ai/xrpc/ai.gftd.apps.mailer.{listEmails,stats,listBindings}`
...
- **Secrets (wrangler, NOT in wrangler.jsonc vars)**: `DISPATCHER_URL`
  (= `https://dispatcher.gftd.ai`), `DISPATCHER_INTERNAL_SECRET`, `SS_RESEND_API_KEY`.
...
- **Fix (live)**: `wrangler secret put DISPATCHER_URL` = `https://dispatcher.gftd.ai`。
