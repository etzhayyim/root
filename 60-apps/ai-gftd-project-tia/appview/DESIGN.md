...
`ai-gftd-project-tia` の MCP エンドポイントを App コンポーネントで実運用可能な形に再設計し、
...
- TIA MCP: `https://tia.gftd.ai/api/mcp`
- TIA Seeker MCP: `https://seeker.tia.gftd.ai/api/mcp`

legacy path-based endpoint (`https://{nanoid}.gftd.ai/xrpc`) は新規設計で使用しない。
...
CORS は `tia.gftd.ai`, `seeker.tia.gftd.ai`, `gftd.ai`, `gftd.ai`, `localhost` 系を許可。

## 8. Deployment Notes

- WADM `Application` namespace は `magatama-runtime`。
- `HTTPRoute` は gateway namespace (`ai-gftd-performers-org-org_34dKrNTTK3cNixZzHIzzFwLw1s4`) を使用し、
