# ai-gftd-project-sys-tms — Translation Management System
...
- `gftd:tms/tms@0.1.0` — 翻訳コア (translate, translate-batch, get-job)
- `gftd:tms/catalogs@0.1.0` — Paraglide カタログ管理 (import, export, diff)
- `gftd:tms/memory@0.1.0` — 翻訳メモリ (lookup, upsert, count)
- `gftd:tms/glossary@0.1.0` — 用語集 (set-term, get-term, list-terms)
...
package gftd:my-component;

world component {
  include gftd:platform/gftd-tms-consumer@0.1.0;
}
```

TMS App is called via XRPC (HTTP/2) from other Apps:
```
http://tms-magatama.magatama-runtime:80/xrpc/gftd.tms.v1.TmsService/<Method>
```
