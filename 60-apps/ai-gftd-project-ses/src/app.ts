...
//   /xrpc/ai.gftd.apps.ses.ingestAnken          procedure (Bearer auth)
//   /xrpc/ai.gftd.apps.ses.updateJokyo          procedure (Bearer auth)
//   /xrpc/ai.gftd.apps.ses.getAnken             query     (Bearer auth)
//   /xrpc/ai.gftd.apps.ses.listAnken            query     (Bearer auth)
//   /xrpc/ai.gftd.apps.ses.listJokyo            query     (Bearer auth)
//   /xrpc/ai.gftd.apps.ses.coverage             query     (Bearer auth)
//
// Auth: Bearer sk_live_* / ES256 JWT → local decode → HMAC forward.
// Dispatch: forwards to bpmn-dispatcher with x-internal-trust HMAC.
//
// LangGraph backend: bpmn-dispatcher routes `ai.gftd.apps.ses.*` to
...
app.get("/_app/meta", (c) =>
  c.json({
    ...
    surfaces: [
      ...
      "/xrpc/ai.gftd.apps.ses.updateJokyo",
      "/xrpc/ai.gftd.apps.ses.getAnken",
      "/xrpc/ai.gftd.apps.ses.listAnken",
      "/xrpc/ai.gftd.apps.ses.listJokyo",
      "/xrpc/ai.gftd.apps.ses.coverage",
    ],
...
async function resolveAuthContext(
  ...
  try {
    ...
    if (!sub) return null;
    return { did: sub, orgDid: "did:erc725:gftd:260425:anon", activeDid: sub };
  } catch {
    return null;
  }
}
...
app.all("/xrpc/:nsidParam", async (c) => {
  ...
  if (!nsid.startsWith("ai.gftd.apps.ses.")) {
