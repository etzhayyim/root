...
Browser / AI Agent
  └─ MCP/XRPC → {nanoid}.etzhayyim.com/xrpc
       └─ App: yorishiro-{service} (TS Native)
                 ...
                 │
                 └─ [API adapters] imports gftd:provider-vault/credentials + wasi:http
...
|---|---|---|---|---|
| yorishiro-provider | — | native Go provider | `gftd-yorishiro-provider` | Playwright Chromium pool, session persistence, auth helpers |
| yorishiro-google | `g00gl3ws` | TS Native | `gftd-yorishiro-google-provider` | Google Search, Gmail, Drive, Calendar |
| yorishiro-microsoft | `m5ftws0t` | TS Native | `gftd-yorishiro-microsoft-provider` | Outlook, OneDrive, Teams |
| yorishiro-aws | `4w5c0n5l` | TS Native | `gftd-yorishiro-aws-provider` | AWS Console, EC2, S3, CloudWatch, IAM |
| yorishiro-x | `x7w1tt3r` | TS Native | `gftd-yorishiro-x-provider` | Timeline, Compose, Profile, DM, Notifications |
| yorishiro-linkedin | `l1nk3d1n` | TS Native | `gftd-yorishiro-linkedin-provider` | Feed, Profile, Messaging, Jobs |
| yorishiro-mufg-card | `mufgcrd1` | TS Native | `gftd-yorishiro-mufg-card-provider` | MUFG Card public site, notices, member portal |
| yorishiro-trafficstars | `tr4f1c5t` | TS Native | `gftd-yorishiro-trafficstars-provider` | Ad delivery: Sites, Spots, Statistics, Ad Tags |
| yorishiro-humeai-voice | `humev01c` | TS Native | `gftd-yorishiro-humeai-voice-provider` | HumeAI TTS voice generation for read-aloud |
...
| provider-vault-provider | — | native Go provider | `gftd-provider-vault-provider` | HashiCorp Vault KV v2, credential storage per user/org |
...
| Package | Interfaces |
|---|---|
| `gftd:yorishiro-google@0.1.0` | `search`, `gmail`, `drive`, `calendar` |
| `gftd:yorishiro-microsoft@0.1.0` | `outlook`, `onedrive`, `teams` |
| `gftd:yorishiro-aws@0.1.0` | `console`, `ec2`, `s3`, `cloudwatch`, `iam` |
| `gftd:yorishiro-x@0.1.0` | `timeline`, `compose`, `profile`, `dm`, `notifications` |
| `gftd:yorishiro-linkedin@0.1.0` | `feed`, `profile`, `messaging`, `jobs` |
| `gftd:yorishiro-mufg-card@0.1.0` | `public-site`, `notices`, `member-portal` |
| `gftd:yorishiro-trafficstars@0.1.0` | `sites`, `spots`, `statistics`, `account` |
| `gftd:yorishiro-humeai-voice@0.1.0` | `voice` |
| `gftd:yorishiro-mturk@0.1.0` | `hit-command`, `hit-query`, `assignment-command`, `assignment-query`, `account` |
| `gftd:yorishiro-marqeta@0.1.0` | `user-command`, `user-query`, `card-command`, `card-query`, `gpa`, `transaction-query` |
| `gftd:yorishiro-1password@0.1.0` | `vault-query`, `item-query`, `item-command`, `file-query` |
| `gftd:yorishiro-squarespace@0.1.0` | `domains`, `transfer`, `dns-export` |
| `gftd:yorishiro-japanpost-enaiyo@0.1.0` | `draft`, `submit`, `receipt`, `tracking` |
| `gftd:yorishiro-flyio@0.1.0` | `account`, `app`, `org`, `cancellation` |
| `gftd:yorishiro-nuro@0.1.0` | `offers`, `cashback` (claim + tracking) |
| `gftd:provider-vault@0.1.0` | `credentials` (put/get/delete/list, per-service, user/org scoped) |
...
1. Create WIT: `60-apps/ai-gftd-project-yorishiro/wit/yorishiro-{service}/package.wit`
2. Add world: Append `gftd-yorishiro-{service}-provider` to `gftd-component.wit`
3. Add imports to `gftd-component` world
4. Create component dir: `60-apps/ai-gftd-project-yorishiro/wasm/ai-gftd-wasm-yorishiro-{service}-{nanoid}/`
...
The TrafficStars adapter stores API tokens via `gftd:provider-vault/credentials`:
