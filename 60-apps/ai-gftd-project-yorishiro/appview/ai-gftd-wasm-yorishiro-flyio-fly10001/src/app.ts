...
// Replace each with `sdk.pds.xrpc("ai.gftd.apps.yorishiroFlyio.<method>", ...)`:
//   - getDb().selectFrom(tableForLabel(label))            → ai.gftd.apps.yorishiroFlyio.listJobs
//   - insertInto("vertex_yorishiroFlyio_cancellationJob") → ai.gftd.apps.yorishiroFlyio.putCancellationJob
//   - insertInto("vertex_yorishiroFlyio_appDeleteJob")    → ai.gftd.apps.yorishiroFlyio.putAppDeleteJob
//   - insertInto("vertex_yorishiroFlyio_orgDeleteJob")    → ai.gftd.apps.yorishiroFlyio.putOrgDeleteJob
//   - second cancellationJob INSERT (status update)       → ai.gftd.apps.yorishiroFlyio.updateCancellationStatus
...
const NSID = {
  cancellationJob: "ai.gftd.yorishiro.flyio.cancellationJob",
  appDeleteJob: "ai.gftd.yorishiro.flyio.appDeleteJob",
...
async function cmdGetAccountInfo(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.getAccountInfo", payload);
  ...
  await getDb()
    ...
    .values({
      vertex_id: `at://${ownerDid}/ai.gftd.yorishiro.flyio.cancellationJob/${jobId}`,
      job_id: jobId,
...
async function cmdListApps(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.listApps", payload);
...
async function cmdGetJobStatus(_sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.getJobStatus", payload);
...
async function cmdDeleteApp(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.deleteApp", payload);
  ...
  await getDb()
    ...
    .values({
      vertex_id: `at://${ownerDid}/ai.gftd.yorishiro.flyio.appDeleteJob/${jobId}`,
      job_id: jobId,
...
async function cmdDeleteOrg(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.deleteOrg", payload);
  ...
  await getDb()
    ...
    .values({
      vertex_id: `at://${ownerDid}/ai.gftd.yorishiro.flyio.orgDeleteJob/${jobId}`,
      job_id: jobId,
      org_slug: req.orgSlug,
...
async function cmdCloseAccount(sdk: HostSDK, payload: Uint8Array): Promise<unknown> {
  const req = parseLexiconInput("ai.gftd.yorishiro.flyio.closeAccount", payload);
...
function registerYorishiroFlyioApp(sdk: HostSDK): void {
  sdk.app
    // ── Query ──────────────────────────────────────────────────────────────
    .command(nsid("ai.gftd.yorishiro.flyio.getAccountInfo"),
    ...
    .command(nsid("ai.gftd.yorishiro.flyio.listApps"),
    ...
    .command(nsid("ai.gftd.yorishiro.flyio.getJobStatus"),
    ...
    // ── Destructive — App ──────────────────────────────────────────────────
    .command(nsid("ai.gftd.yorishiro.flyio.deleteApp"),
    ...
    // ── Destructive — Org ──────────────────────────────────────────────────
    .command(nsid("ai.gftd.yorishiro.flyio.deleteOrg"),
    ...
    .command(nsid("ai.gftd.yorishiro.flyio.closeAccount"),
