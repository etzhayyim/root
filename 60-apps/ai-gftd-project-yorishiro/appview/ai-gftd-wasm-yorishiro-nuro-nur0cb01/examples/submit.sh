...
LIST=$(curl -sS -X POST "$BASE/xrpc/ai.gftd.apps.yorishiroNuro.listOffers" \
...
for i in 1 2 3 4 5 6; do
  sleep 15
  OFFERS=$(curl -sS -X POST "$BASE/xrpc/ai.gftd.apps.yorishiroNuro.getOffers" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
...
read -r
curl -sS -X POST "$BASE/xrpc/ai.gftd.apps.yorishiroNuro.claimCashback" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
for i in 1 2 3 4 5 6; do
  sleep 30
  curl -sS -X POST "$BASE/xrpc/ai.gftd.apps.yorishiroNuro.getClaimStatus" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    --data '{"campaignCode":"B195"}'
done
