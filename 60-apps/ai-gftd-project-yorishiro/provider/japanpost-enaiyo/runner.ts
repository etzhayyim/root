...
async function recordReceipt(params: {
  ...
  const r = await fetch(`${ENAIYO_APP_BASE}/xrpc/ai.gftd.apps.yorishiroEnaiyo.recordReceipt`, {
    method: "POST",
    headers: {
...
async function resolveDraft(_draftId: string): Promise<{ sender: any; recipient: any }> {
  throw new Error("resolveDraft not implemented — query ai.gftd.apps.yorishiroEnaiyo.draftNaiyo by draftId");
}
