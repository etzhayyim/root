...
async function recordOffers(params: {
  ...
  const r = await fetch(`${NURO_APP_BASE}/xrpc/ai.gftd.apps.yorishiroNuro.recordOffers`, {
    method: "POST",
    headers: {
...
async function recordClaim(params: {
  ...
  const r = await fetch(`${NURO_APP_BASE}/xrpc/ai.gftd.apps.yorishiroNuro.recordClaim`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${params.sessionToken}`,
