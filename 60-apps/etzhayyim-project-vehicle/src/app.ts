// Vehicle Manufacturing Graph thin edge. Business logic runs in AgentGateway MCP + pod-side LangServer.
...
const NSID_PREFIX = "com.etzhayyim.apps.vehicle.";
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: "did:web:vehicle.etzhayyim.com",
        nanoid: env.APP_NANOID ?? "v3h1cl01",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        businessLogic: "40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/ingest/vehicle.py",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/vehicle",
      });
    }
    ...
    return json({ error: "NotFound", message: "vehicle not found" }, 404);
  },
