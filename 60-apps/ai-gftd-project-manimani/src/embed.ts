...
//
// What the UI shows:
//   1. Project list  → /xrpc/ai.gftd.apps.manimani.listProjects
//   2. Coverage      → /xrpc/ai.gftd.apps.manimani.coverage
...
//        reassign) → /xrpc/ai.gftd.apps.manimani.resumeRun
//
// Auth model:
...
(() => {
  ...
  window.addEventListener("message", (ev) => {
    ...
    if (d.type === "gftd:embed:auth" && typeof d.token === "string") {
      bearer = d.token;
  ...
  try { window.parent?.postMessage({ type: "gftd:embed:ready", nanoid: "manimani" }, "*"); } catch {}
  ...
  async function loadPendingQueue() {
    try {
      const data = await call("/xrpc/ai.gftd.apps.manimani.listPendingRuns?limit=25");
      const root = $("pending");
  ...
  async function loadCoverage() {
    try {
      const cov = await call("/xrpc/ai.gftd.apps.manimani.coverage?windowDays=7");
      const root = $("cov");
      root.innerHTML = "";
      const kpis = [
  ...
  async function loadProjects() {
    try {
      const data = await call("/xrpc/ai.gftd.apps.manimani.listProjects?limit=50");
  ...
  async function decide(runId, decision, targetProjectId) {
    try {
      ...
      const r = await call("/xrpc/ai.gftd.apps.manimani.resumeRun", {
        method: "POST",
