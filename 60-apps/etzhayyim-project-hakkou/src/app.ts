// hakkou.etzhayyim.com — fermentation pipeline layer (hakkou = 発酵)
// Thin-edge dispatcher: business logic in AgentGateway MCP + pod-side LangServer.
// 2 methods: startFerment / getFerment
...
const NSID_PREFIX = "com.etzhayyim.hakkou.";
const ACTOR_DID = "did:web:hakkou.etzhayyim.com";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "h4kk0001",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/hakkou",
        methods: ["startFerment", "getFerment"],
      });
    }
