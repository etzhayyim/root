# open-apqc — RW-free reference implementation (Phase 3 step 1)
...
| L2 (process group) | ~80 | future PR (`ai.gftd.apqc.processGroup`) |
...
`ai.gftd.apqc.processCategory` (record), at [`00-contracts/lexicons/ai/gftd/apqc/processCategory.json`](../../../00-contracts/lexicons/ai/gftd/apqc/processCategory.json). Naming convention follows the existing `ai.gftd.apqc.*` namespace (where sibling `getProcess` / `materializeSubprocesses` / `coverageSnapshot` / `emitEvent` are procedures) — record-vs-procedure split.
...
| Surface | State |
|---|---|
| Record lexicon `ai.gftd.apqc.processCategory` | ✅ |
