...
// vertex_collection_job, vertex_wat, vertex_screenshot. Tables NOT in @gftd/graph-schema:
...
function parseRobotsTxt(text: string): RobotsTxtRules {
  ...
  for (const line of text.split("\n")) {
    ...
    if (dir === "user-agent") {
      activeAgent = value === "*" || value.toLowerCase() === "gftd-bot";
    } else if (activeAgent) {
  ...
  for (const topic of topicCoordinators) {
    ...
    try {
      await getDb().insertInto("vertex_web_topic" as any).values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.topic/${topic.slug}`,
        topic: topic.slug,
        slug: topic.slug,
        did: topic.did,
        name: topic.name,
...
async function cmdFetchAozora(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.collectAozora", payload);
  const jobs: string[] = [];
...
async function cmdFetchNdl(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.collectNdl", payload);
  const jobs: string[] = [];
...
async function cmdFetchWikisource(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.collectWikisource", payload);
...
async function cmdFetchGutenberg(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.collectGutenberg", payload);
  const jobs: string[] = [];
...
async function cmdFetchImages(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.collectImages", payload);
...
async function cmdListPages(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.listPages", payload);
...
async function cmdSearchPages(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.searchPages", payload);
  const limit = Math.min(Math.max(req.limit ?? 50, 1), 100);
  const offset = req.offset ?? 0;
...
async function cmdListJobs(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.listJobs", payload);
  const limit = Math.min(Math.max(req.limit ?? 50, 1), 100);
  const offset = req.offset ?? 0;
...
async function cmdGetStats(sdk: HostSDK, _payload: Uint8Array): Promise<Uint8Array> {
  ...
  // TODO(site): vertex_web_topic not in @gftd/graph-schema
...
async function cmdEnqueueUrl(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.enqueueUrl", payload);
  ...
  try {
    ...
    await getDb().insertInto("vertex_frontier").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${euFeRkey}`,
      rkey: euFeRkey,
      url: req.url,
...
async function cmdRegisterDomain(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.registerDomain", payload);
...
async function cmdCrawlPage(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.crawlPage", payload);
  if (!req.url) return encodeJson({ error: "url is required" });
  ...
  await getDb().insertInto("vertex_frontier").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${feRkey}`,
    rkey: feRkey,
    url: req.url,
    domain: hostname,
    status: "pending",
...
async function cmdCrawlDomain(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.crawlDomain", payload);
  if (!req.domain) return encodeJson({ error: "domain is required" });
  ...
  await getDb().insertInto("vertex_frontier").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${seedFeRkey}`,
    rkey: seedFeRkey,
    url: seedUrl,
    domain: req.domain,
    status: "pending",
  ...
  try {
    await getDb().insertInto("vertex_crawl_session" as any).values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.crawl/${sessionId}`,
...
async function cmdRecordPage(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.recordPage", payload);
  ...
  for (const para of paragraphs) {
    try {
      ...
      await getDb().insertInto("vertex_page").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.page/${rpRkey}`,
        rkey: rpRkey,
        url: req.url,
        domain: hostname,
...
async function cmdGetPage(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.getPage", payload);
...
async function cmdGetDomainOverview(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.getDomainOverview", payload);
  if (!req.domain) return encodeJson({ error: "domain is required" });

  // Serialized per Hyperdrive origin pool guideline.
  const db = createKyselyDb();
  // TODO(site): vertex_web_domain / vertex_crawl_session not in @gftd/graph-schema
...
async function cmdGetLinkGraph(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.getLinkGraph", payload);
...
async function cmdEnqueueBulk(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.enqueueBulk", payload);
  ...
  for (const url of req.urls) {
    try {
      ...
      await getDb().insertInto("vertex_frontier").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${bulkFeRkey}`,
        rkey: bulkFeRkey,
        url,
        domain: hostname,
        status: "pending",
...
async function cmdDequeueUrls(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.dequeueUrls", payload);
...
async function getDomainLastCrawled(domain: string): Promise<number> {
  // TODO(site): vertex_web_domain not in @gftd/graph-schema — cooldown falls back to 0 until schema lands
  void domain;
  return 0;
}
...
async function cmdProcessFrontier(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.processFrontier", payload);
  ...
  for (const entry of rows) {
    ...
    await getDb().insertInto("vertex_frontier").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${pfRkey}`,
      rkey: pfRkey,
      url,
      domain,
      status: "processing",
...
async function cmdBulkIngestAozora(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  ...
  await getDb().insertInto("vertex_collection_job").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.collectionJob/${jobId}`,
    rkey: jobId,
...
async function processCollectionJobResult(sdk: HostSDK, recordJson: string): Promise<{ ok: boolean; detail: string }> {
    ...
    if (targetDomain && content) {
      ...
      try {
        await getDb().insertInto("vertex_robots_txt" as any).values({
          vertex_id: `at://${appId}/ai.gftd.apps.site.robotsTxt/${rbRkey}`,
          domain: targetDomain,
  ...
  if (crawlType === "frontier" || crawlType === "singlePage" || format === "html") {
    ...
    await getDb().insertInto("vertex_page").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.page/${pgRkey}`,
      rkey: pgRkey,
      url: sourceUrl,
      domain: hostname,
  ...
  for (const paragraph of paragraphs) {
    ...
    await getDb().insertInto("vertex_page").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.page/${pageId}`,
      rkey: pageId,
      url: sourceUrl,
...
async function processRawPage(sdk: HostSDK, recordJson: string): Promise<{ ok: boolean; detail: string }> {
  ...
  for (const para of paragraphs) {
    ...
    await getDb().insertInto("vertex_page").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.page/${rawPgRkey}`,
      rkey: rawPgRkey,
      url: sourceUrl,
      domain: sourceUrl ? (() => { try { return new URL(sourceUrl).hostname; } catch { return sourceId; } })() : sourceId,
...
async function processInboundMention(sdk: HostSDK, recordJson: string): Promise<{ ok: boolean; detail: string }> {
  ...
  for (const url of urls) {
    ...
    await getDb().insertInto("vertex_frontier").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${mentionFeRkey}`,
      rkey: mentionFeRkey,
      url,
...
async function generateWET(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    await getDb().insertInto("vertex_wet_chunk").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.wet/${wetId}`,
    ...
    await getDb().insertInto("edge_chunk_of").values({
      edge_id: `at://${appId}/ai.gftd.apps.site.chunkOf/${coId}`,
      src_vid: `at://${appId}/ai.gftd.apps.site.wet/${wetId}`,
...
async function generateWAT(
  ...
  await getDb().insertInto("vertex_wat").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.wat/${watRkey}`,
    rkey: watRkey,
    url,
    domain: new URL(url).hostname,
    language,
  ...
  if (childDepth < MAX_FRONTIER_DEPTH) {
    ...
    for (const link of outlinks.filter(l => l.internal).slice(0, 50)) {
      ...
      await getDb().insertInto("vertex_frontier").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${childFeRkey}`,
        rkey: childFeRkey,
        url: link.url,
        domain: new URL(url).hostname,
        status: "pending",
  ...
  if (meta.frontierId) {
    ...
    await getDb().insertInto("vertex_frontier").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${doneFeRkey}`,
      rkey: doneFeRkey,
      url,
      domain: new URL(url).hostname,
      status: "done",
...
async function processScreenshotResult(sdk: HostSDK, record: Record<string, unknown>): Promise<{ ok: boolean; detail: string }> {
  ...
  await getDb().insertInto("vertex_screenshot").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.screenshot/${ssRkey}`,
    rkey: ssRkey,
    url,
    domain: hostname,
    blob_ref: blobRef,
...
async function processPdfResult(
  ...
  if (content) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await getDb().insertInto("vertex_wet_chunk").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.wet/${pdfWetRkey}`,
        page_rkey: pageDid,
        url: sourceUrl,
        domain: hostname,
        chunk_index: i,
  ...
  await getDb().insertInto("vertex_wat").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.wat/${pdfWatRkey}`,
    rkey: pdfWatRkey,
    url: sourceUrl,
    domain: hostname,
    language,
...
async function processIiifManifestResult(
  ...
  if (ocrText) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await getDb().insertInto("vertex_wet_chunk").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.wet/${iiifWetRkey}`,
        page_rkey: pageDid,
        url: ndlViewUrl,
        domain: hostname,
        chunk_index: i,
  ...
  await getDb().insertInto("vertex_wat").values({
    vertex_id: `at://${appId}/ai.gftd.apps.site.wat/${iiifWatRkey}`,
    rkey: iiifWatRkey,
    url: ndlViewUrl,
    domain: hostname,
    language: language || "ja",
  ...
  if (isbn || label) {
    ...
    await getDb().insertInto("vertex_page").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.page/${iiifPgRkey}`,
      rkey: iiifPgRkey,
      url: ndlViewUrl,
      domain: hostname,
      title: label,
...
async function cmdFetchPdf(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.fetchPdf", payload);
  if (!req.url) return encodeJson({ error: "url is required" });
...
function cmdGenerateWET(sdk: HostSDK, payload: Uint8Array): Uint8Array {
  const req = parseLexiconInput("ai.gftd.apps.site.generateWet", payload);
...
function cmdGenerateWAT(sdk: HostSDK, payload: Uint8Array): Uint8Array {
  const req = parseLexiconInput("ai.gftd.apps.site.generateWat", payload);
  if (!req.url || !req.html) return encodeJson({ error: "url and html are required" });
...
async function cmdCaptureScreenshot(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.captureScreenshot", payload);
  if (!req.url) return encodeJson({ error: "url is required" });
...
async function cmdCheckRobotsTxt(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.checkRobotsTxt", payload);
  if (!req.domain) return encodeJson({ error: "domain is required" });

  // TODO(site): vertex_robots_txt not in @gftd/graph-schema — always cache-miss, falls through to fetch
...
async function cmdSeedFromCommonCrawl(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.seedFromCommonCrawl", payload);
...
async function emitGeoRecord(sdk: HostSDK, fields: {
  ...
  // Use full NSID so expandCollection passes it through unchanged → ai.gftd.apps.site.geoRecord
  await sdk.pds.comAtprotoRepoCreateRecord("ai.gftd.apps.site.geoRecord", {
    id: genID("gr"),
...
async function cmdIngestGeoData(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.ingestGeoData", payload);
...
async function cmdSeedForProject(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.seedForProject", payload);
  ...
  for (const domain of req.domains) {
    ...
    let existingPages = 0;
    try {
      // TODO(site): vertex_web_domain not in @gftd/graph-schema — derive pageCount from view
    ...
    await getDb().insertInto("vertex_frontier").values({
      vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${projFeRkey}`,
      rkey: projFeRkey,
      url: `https://${domain}/`,
      domain,
      status: "pending",
...
async function cmdSeedGovPdfs(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.seedGovPdfs", payload);
...
async function cmdTriggerTextEmbedding(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.triggerTextEmbedding", payload);
  ...
  // TODO(site): vertex_wpg_wet not in @gftd/graph-schema — legacy path returns empty
  if (rows.length === 0) {
    ...
    sdk.pds.dispatch({
      ...
      payload: {
        ...
        params: JSON.stringify({
          ...
          'writeBackCollection': "ai.gftd.apps.site.wetChunkEmbedding",
        }),
      },
    });
  }
...
async function cmdTriggerVisualEmbedding(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.triggerVisualEmbedding", payload);
...
async function cmdRegisterWebTopic(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.registerWebTopic", payload);
...
async function processCDXSeedResult(sdk: HostSDK, record: Record<string, unknown>, content: string): Promise<{ ok: boolean; detail: string }> {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      await getDb().insertInto("vertex_frontier").values({
        vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${cdxFeRkey}`,
        rkey: cdxFeRkey,
        url,
        domain: new URL(url).hostname,
        status: "pending",
...
async function cmdGetCrawlOutputStats(sdk: HostSDK, _payload: Uint8Array): Promise<Uint8Array> {
  ...
  const db = createKyselyDb();
  // TODO(site): vertex_wpg_wet not in @gftd/graph-schema — mv_site_wet_chunk_total covers vertex_wet_chunk
...
async function cmdAnswerConvo(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.answerConvo", payload);
  ...
  if (chunks.length === 0) {
    ...
    chunks = await db
      ...
      .execute()
      .catch(() => [] as Array<Record<string, unknown>>);
    // TODO(site): vertex_wpg_wet not in @gftd/graph-schema — legacy fallback removed
  ...
  // TODO(site): vertex_web_domain not in @gftd/graph-schema — return empty profile
...
async function handleDomainConvos(sdk: HostSDK): Promise<Array<Record<string, unknown>>> {
  ...
  // TODO(site): vertex_web_domain not in @gftd/graph-schema — derive registered domain DIDs from vertex_domain
  const db = createKyselyDb();
...
export async function runHeartbeat(sdk: HostSDK): Promise<{ ok: boolean; actions: Array<Record<string, unknown>> }> {
  ...
  if (!prioritySeedsEnqueued) {
    ...
    for (const seed of PRIORITY_SEED_DOMAINS) {
      ...
      try {
        // TODO(site): vertex_web_domain not in @gftd/graph-schema — derive from vertex_page count
        const existing = await createKyselyDb()
          .selectFrom("vertex_page")
      ...
      for (const url of seed.urls) {
        ...
        await getDb().insertInto("vertex_frontier").values({
          vertex_id: `at://${appId}/ai.gftd.apps.site.frontier/${psFeRkey}`,
          rkey: psFeRkey,
          url,
          domain: seed.domain,
          status: "pending",
  ...
  // TODO(site): vertex_wpg_wet not in @gftd/graph-schema — use vertex_wet_chunk (no topics column)
...
function registerWebpageApp(sdk: HostSDK): void {
  sdk.app
    // --- Collection commands ---
    .command(nsid("ai.gftd.apps.site.collectAozora"), (ctx, body) => cmdFetchAozora(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.collectNdl"), (ctx, body) => cmdFetchNdl(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.fetchNdlManifest"), (ctx, body) => cmdFetchNdlManifest(sdk, body),
    ...
    // --- Query commands ---
    .command(nsid("ai.gftd.apps.site.listPages"), (ctx, body) => cmdListPages(sdk, body),
      asAgentTool("List archived web pages with topic/language/source filters"),
      withCapabilityTags("query", "pages"),
    )
    .command(nsid("ai.gftd.apps.site.searchPages"), (ctx, body) => cmdSearchPages(sdk, body),
      asAgentTool("Full-text search across archived pages"),
      withCapabilityTags("query", "search", "pages"),
    )
    .query(nsid("ai.gftd.apps.site.searchSemantic"), (ctx, body) => cmdSearchSemantic(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.seedForProject"), (ctx, body) => cmdSeedForProject(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.ingestGeoData"), (ctx, body) => cmdIngestGeoData(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.triggerTextEmbedding"), (ctx, body) => cmdTriggerTextEmbedding(sdk, body),
      asAgentTool("Trigger text embedding batch for WET chunks via Murakumo"),
      withCapabilityTags("embedding", "text", "murakumo"),
    )
    .command(nsid("ai.gftd.apps.site.triggerVisualEmbedding"), (ctx, body) => cmdTriggerVisualEmbedding(sdk, body),
      asAgentTool("Trigger visual embedding batch for screenshots via ColPali"),
      withCapabilityTags("embedding", "visual", "colpali"),
    )
    .command(nsid("ai.gftd.apps.site.registerWebTopic"), (ctx, body) => cmdRegisterWebTopic(sdk, body),
      asAgentTool("Register a new web topic coordinator DID"),
      withCapabilityTags("topic", "register"),
    )
    .command(nsid("ai.gftd.apps.site.getCrawlOutputStats"), (ctx, body) => cmdGetCrawlOutputStats(sdk, body),
      asAgentTool("Get WET/WAT/screenshot output statistics"),
      withCapabilityTags("stats", "wet", "wat", "screenshot"),
    )
    // --- GraphRAG convo ---
    .command(nsid("ai.gftd.apps.site.answerConvo"), (ctx, body) => cmdAnswerConvo(sdk, body),
    ...
    .command(nsid("ai.gftd.apps.site.fetch"), (ctx, body) => cmdFetchSync(sdk, body),
...
async function cmdFetchSync(sdk: HostSDK, payload: Uint8Array): Promise<Uint8Array> {
  const req = parseLexiconInput("ai.gftd.apps.site.fetch", payload);
  if (!req.url) return encodeJson({ error: "url is required" });
