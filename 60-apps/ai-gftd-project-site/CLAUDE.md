...
→ `gftd dodaf tv1 query --id ai-gftd-project-site-internet-clone-gateway-role` / MCP `gftd.dodaf.tv1.query`
...
| **Tier 2 Domain** | `DIDWrite(pageDID, ...)` / `ComAtprotoRepoCreateRecord(...)` | `ai.gftd.apps.site.*` | page metadata, topic metadata, follower events |
...
- robots.txt: cached as `ai.gftd.apps.site.robots_txt` record (24h TTL), `User-agent: gftd-bot` + `*`
...
|---|---|---|---|
| **WET** | `ai.gftd.apps.site.wet` | Markdown text chunks (512 tokens, sentence boundary) | Text embedding (Murakumo) |
| **WAT** | `ai.gftd.apps.site.wat` | JSON metadata (URL, headers, outlinks, OG, language) | Link graph, analytics |
| **WebP** | `ai.gftd.apps.site.screenshot` | WebP screenshot blob (1280x720, quality 80) | ColPali visual embedding |
...
Phase 3: phase3_wat_to_cypher.py (60-apps/ai-gftd-project-common-crawl/70-tools/70-tools/70-tools/scripts/)
...
Phase 4: phase4_murakumo_classify_did.py
  ...
  → domain_mapping.json → PDS XRPC DID inject (ai.gftd.apps.site.classified)
...
|---|---|---|---|
| `site_topic` | `ai.gftd.apps.site.topic` | Primary DID | topic, slug, did |
| `site_domain` | `ai.gftd.apps.site.domain` | Primary DID | domain, slug, did, topics |
| `site_page` | `ai.gftd.apps.site.page` | Page DID | url, domain, did, title, content_hash, topics (legacy catalog) |
| `site_wet` | `ai.gftd.apps.site.wet` | Primary DID | url, domain, chunk_index, total_chunks, markdown, content_hash, language, section, token_count |
| `site_wat` | `ai.gftd.apps.site.wat` | Primary DID | url, domain, title, language, mime_type, status_code, outlinks, outlink_count, content_hash |
| `site_screenshot` | `ai.gftd.apps.site.screenshot` | Primary DID | url, domain, blob_ref, format, width, height, quality, file_size |
| `site_robots_txt` | `ai.gftd.apps.site.robots_txt` | Primary DID | domain, rules, crawl_delay, sitemap_urls, expires_at |
| `site_crawl` | `ai.gftd.apps.site.crawl` | Domain DID | session_id, domain, page_count |
| `site_frontier` | `ai.gftd.apps.site.frontier` | Primary DID | url, domain, status, priority, depth, topics, source |
| `site_follower_event` | `ai.gftd.apps.site.follower_event` | Primary DID | follower_did, action |
...
```bash
cd 60-apps/ai-gftd-project-site/wasm/ai-gftd-wasm-webpage-w3bpg001
