...
export async function reviewAnomaly(
  ...
  const resp = await fetch(`${service()}/xrpc/ai.gftd.apps.resourceFlow.reviewAnomaly`, {
    method: 'POST',
    headers,
...
//
// Single round-trip via ai.gftd.apps.resourceFlow.getActorLabels which
...
async function getActorLabels(dids: string[]): Promise<ActorLabel[]> {
  ...
  const url = `${service()}/xrpc/ai.gftd.apps.resourceFlow.getActorLabels?${sp}`;
  const resp = await fetch(url, { headers: { accept: 'application/json' } });
  if (!resp.ok) {
