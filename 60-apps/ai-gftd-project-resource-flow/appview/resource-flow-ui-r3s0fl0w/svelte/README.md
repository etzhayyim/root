...
Svelte 5 (runes mode) AppView for `resource-flow.gftd.ai`.

- Calls `ai.gftd.apps.resourceFlow.getSankey` via `atproto.gftd.ai` PDS pipethrough (or same-origin when served from `resource-flow.gftd.ai`).
...
- Resolves counterparty labels via `app.bsky.actor.getProfile` (works for both `did:web:hospitality.gftd.ai:actor:chain:*` and ERC725 root DIDs once `did:erc725` profile resolution is wired into the PDS).
