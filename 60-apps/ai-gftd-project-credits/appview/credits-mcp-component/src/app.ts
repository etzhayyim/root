// credits.gftd.ai thin edge facade. Ledger and reward logic run in AgentGateway MCP + pod-side LangServer.
...
const APP = "credits";
const NSID_PREFIX = "ai.gftd.apps.credits.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:credits.gftd.ai", nanoid: env.APP_NANOID ?? "a5ce95af", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "20-actors/magatama/py/src/pymagatama/ingest/credits.py", bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/credits" });
