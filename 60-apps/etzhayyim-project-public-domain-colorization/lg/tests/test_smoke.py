"""Local smoke tests for lg-pd-color OSS server.

Run from `60-apps/etzhayyim-project-public-domain-colorization/lg/`:
...
touching `60-apps/etzhayyim-project-public-domain-colorization/lg/**`.
"""
...
def test_server_module_imports():
    from lg_pd_color.server import GRAPHS, _NSID_TO_ASSISTANT, app
...
def test_graphs_match_expected_set():
    from lg_pd_color.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_pd_color.server import _NSID_TO_ASSISTANT
    assert _NSID_TO_ASSISTANT == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_pd_color.server import GRAPHS, _NSID_TO_ASSISTANT
...
@pytest.fixture
def client():
    from lg_pd_color.server import app
