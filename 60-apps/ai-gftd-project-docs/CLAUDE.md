# ai-gftd-project-docs — Google Docs v1 + Microsoft Graph (Word) compatible document API

**docs.gftd.ai** — a documents API whose external surface is shaped like **both** Google Docs v1
...
```
Google SDK  → docs.gftd.ai/v1/documents/{id}:batchUpdate (structural)  ┐
MS Graph SDK→ docs.gftd.ai/v1.0/me/drive/items/{id}/content (plaintext) ┘
  CF Worker docs-compat (50-infra/cloudflare/workers/docs-compat)
   ├─ /xrpc/ai.gftd.apps.docs.*  → pod (actor-worker passthrough)
   └─ provider REST → canonical XRPC → pod → reshape to provider JSON
  lg-docs pod (60-apps/ai-gftd-project-docs/lg, FastAPI server.py)
   → kotoba datomic transact/q/pull on graph docs-v1 (:doc/* namespace)
```

## Canonical XRPC methods (ai.gftd.apps.docs.*)
...
## Test
```bash
cd 60-apps/ai-gftd-project-docs/lg && python3 -m pytest tests/ -q
