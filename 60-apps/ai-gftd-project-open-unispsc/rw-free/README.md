# open-unispsc — RW-free reference implementation
...
`ai.gftd.apps.openUnispsc.segmentDef` (record), at [`00-contracts/lexicons/ai/gftd/apps/openUnispsc/segmentDef.json`](../../../00-contracts/lexicons/ai/gftd/apps/openUnispsc/segmentDef.json). Naming note: the sibling `segment.json` is a **procedure** that actively resolves a segment as a business-portfolio boundary; `segmentDef` is the **record** companion that persists the catalog on PDS. The two are complementary — the procedure does runtime work, the record gives the substrate pipeline a content-addressable target.
...
| Surface | State |
|---|---|
| Record lexicon `ai.gftd.apps.openUnispsc.segmentDef` | ✅ |
