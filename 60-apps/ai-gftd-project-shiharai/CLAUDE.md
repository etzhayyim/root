...
```
┌─────────────────────────────────────────────────────────────────┐
│ T5 User (Claude Code / gftd CLI)                                 │
│   gftd shiharai list            → listPendingBills               │
│   gftd shiharai prepare <id>    → preparePayment                 │
│   gftd shiharai confirm <id>    → confirmPayment (destructive)   │
...
- **On the Mac**: `security add-generic-password -s gftd.shiharai -a <biller>`
...
- ✅ **Phase 3** (2026-04-19): **architectural pivot — shiharai becomes a
  thin consumer of `ai.gftd.apps.bpmn` (orchestrator) +
  `ai.gftd.apps.playwright` (primitives) +
  `ai.gftd.apps.cloudflareBrowserRender` (substrate).** Biller-specific
  ...
  `60-apps/ai-gftd-project-playwright/daemon/` — Phase 3b).
