...
describe("OS Messaging Gateway — Integration Tests", () => {
  ...
  describe("Discord webhook", () => {
    it("dispatches a Discord message", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookDiscord", {
        t: "MESSAGE_CREATE",
        d: { content: "新幹線を予約して", channel_id: "ch-123", author: { id: "user-456" } },
      });
      expect(result.status).toBe("dispatched");
      const inbound = writtenRecords.find(r => r.collection === "ai.gftd.apps.osMessaging.inbound");
    ...
    it("responds to Discord PING", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookDiscord", { type: 1 });
      expect(result.type).toBe(1);
    });

    it("ignores empty Discord messages", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookDiscord", { d: {} });
  ...
  describe("Telegram webhook", () => {
    it("dispatches a Telegram message", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookTelegram", {
    ...
    it("ignores non-text Telegram updates", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookTelegram", { update_id: 1 });
      expect(result.status).toBe("ignored");
    });
  });
  ...
  describe("Slack webhook", () => {
    it("responds to Slack URL verification", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookSlack", {
  ...
  describe("LINE webhook", () => {
    it("dispatches a LINE message", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookLine", {
        events: [{
          type: "message",
  ...
  describe("WhatsApp webhook", () => {
    it("dispatches a WhatsApp message", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.webhookWhatsapp", {
        entry: [{
          changes: [{
            value: {
  ...
  describe("Platform connections", () => {
    ...
    it("disconnectPlatform writes disconnect record", async () => {
      const result = await invokeCommand("ai.gftd.apps.osMessaging.disconnectPlatform", {
        platform: "telegram",
        platformUid: "67890",
      });
    ...
    it("listConnections returns empty array", async () => {
      const result = await invokeQuery("ai.gftd.apps.osMessaging.listConnections", {});
