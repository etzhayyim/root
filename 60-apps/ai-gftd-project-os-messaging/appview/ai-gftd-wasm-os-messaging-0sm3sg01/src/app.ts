...
async function resolveUserDid(platform: Platform, platformUid: string): Promise<string | null> {
  try {
    ...
    const row = await db
      ...
      .select(["gftd_did"] as any[])
      .where("platform" as any, "=", platform)
      .where("platform_uid" as any, "=", platformUid)
      .limit(1)
      .executeTakeFirst() as { gftd_did: string } | undefined;
    return row?.gftd_did || null;
  } catch {
    return null;
  }
}
    ...
    asAgentTool(
      withCapabilityTags(["platform-user-mapping"])(
        withOCELEvent("osMessaging:listConnections")(async (input: { gftdDid?: string; platform?: string }) => {
          ...
          if (input.gftdDid) match.gftd_did = input.gftdDid;
          if (input.platform) match.platform = input.platform;
          const db = createKyselyDb();
          let q = db.selectFrom(MAPPING_TABLE as any)
            .select(["platform", "platform_uid", "gftd_did", "connected_at"] as any[]);
      ...
      { name: "listConnections", description: "List platform connections. Optional filters: gftdDid, platform." },
    ),
  );
