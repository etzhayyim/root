 ...
 *
 * Identity hierarchy:
 *   did:web:shopping.etzhayyim.com                       — controller
 *   did:web:shopping.etzhayyim.com:product:{sku}         — a product
 *   did:web:shopping.etzhayyim.com:order:{orderId}       — an order
 */
...
export interface PaymentRecord {
  orderId: string;
  buyerDid: string;
  purpose: ShoppingPaymentPurpose;
  grossMicros: string;
  titheMicros: string;
  netMicros: string;
  txHash?: string;
...
export function productDid(sku: string): string {
  return `${SHOPPING_DID_PREFIX}product:${sku.toLowerCase()}`;
}
...
export function orderDid(orderId: string): string {
  return `${SHOPPING_DID_PREFIX}order:${orderId.toLowerCase()}`;
}
