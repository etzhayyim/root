...
const fakeSettle: SettlementExecutor = async () => ({ txHash: "0xshop" });

describe("shopping rw-free", () => {
  let e: any;
  beforeEach(() => {
    e = new MockEtzhayyim({ did: "did:web:shopping.etzhayyim.com" });
  });
  ...
  describe("catalog", () => {
    ...
    it("publishes + gets + lists", async () => {
      ...
      expect(got.product?.did).toContain("shopping.etzhayyim.com:product:sku-1");
      expect((await listProducts(e, { category: "tools" })).total).toBe(1);
    });
