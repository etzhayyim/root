// air-sms.etzhayyim.com — airline safety management system layer
// Thin-edge dispatcher: business logic in AgentGateway MCP + pod-side LangServer.
// 8 methods: submitSafetyReport / assessRisk / recordIosaFinding / fileRegulatoryReport / reportOccurrence / distributeSafetyBulletin / screenDangerousGoods / handleSecurityAlert
...
const NSID_PREFIX = "com.etzhayyim.apps.airSms.";
const ACTOR_DID = "did:web:air-sms.etzhayyim.com";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/_app/meta") {
      return json({
        ok: true,
        actor: ACTOR_DID,
        nanoid: env.APP_NANOID ?? "a1rsms01",
        execution: "edge-proxy+agentgateway-mcp+langserver",
        bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/air-sms",
        methods: ["submitSafetyReport", "assessRisk", "recordIosaFinding", "fileRegulatoryReport", "reportOccurrence", "distributeSafetyBulletin", "screenDangerousGoods", "handleSecurityAlert"],
      });
    }
