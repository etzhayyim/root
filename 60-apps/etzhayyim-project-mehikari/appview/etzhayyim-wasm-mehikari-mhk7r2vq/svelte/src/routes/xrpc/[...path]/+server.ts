...
export const POST: RequestHandler = async (event) => {
  ...
  headers.set('x-etzhayyim-bff', 'mehikari-sveltekit');
  ...
  try {
    ...
    // Non-JSON upstream failures are surfaced as-is below.
  }

  if (!upstream.ok) {
    return noStore({ error: 'MCP router request failed', upstream: payload }, { status: upstream.status });
  }
