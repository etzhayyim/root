...
def test_nsid_to_assistant_mapping():
    ...
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.health"] == "health"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.listVideos"] == "list_videos"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.getVideo"] == "get_video"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.compose"] == "compose"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.generateScript"] == "generate_script"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.synthesizeVoice"] == "synthesize_voice"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.generateVisual"] == "generate_visual"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.generateBgm"] == "generate_bgm"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.renderVideo"] == "render_video"
    assert _NSID_TO_ASSISTANT["ai.gftd.apps.yukkuri.reviewVideo"] == "review_video"
...
def test_unknown_nsid_xrpc_404(client_no_lifespan):
    r = client_no_lifespan.post(
        "/xrpc/ai.gftd.apps.yukkuri.unknownMethod",
        json={},
    )
    assert r.status_code == 404
