...
Godot のゲームを `ops.etzhayyim.com` の基準（`gftd:platform/gftd-mcp@0.1.0`）で扱える
...
- ここでの WIT 方針は `gftd:platform/gftd-mcp@0.1.0` を取り込み、将来 `ops.etzhayyim.com` 系のワークフロー API を
  追加実装しやすい状態を保つことです。
- 既存の `gftd:platform/gftd-mcp@0.1.0` を起点に、将来 `ops.etzhayyim.com` 固有インターフェースへ拡張する前提を残す。
