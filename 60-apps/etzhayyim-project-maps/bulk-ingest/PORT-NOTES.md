...
| Var | Default | Purpose |
|---|---|---|
| `USE_PYMAGATAMA_SUBSTRATE` | `0` | Set to `1` to enable substrate write path |
...
| `ETZ_PDS_URL` | `https://pds.etzhayyim.com` | PDS endpoint (read by `pymagatama.substrate.Etzhayyim`) |
| `ETZ_SESSION_JWT` | — | User-side auth (preferred for human-driven seeds) |
| `MAGATAMA_INTERNAL_TOKEN` | — | Service-to-service auth (used by pod-side workers) |
