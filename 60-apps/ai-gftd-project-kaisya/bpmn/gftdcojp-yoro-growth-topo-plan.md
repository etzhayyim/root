...
**AT Record**: `ai.gftd.apps.kaisya.opportunitySignal` → yoro feed に可視化
...
→ `ai.gftd.apps.kaisya.brandingActivity` AT Record 生成
```
...
|---|---|---|---|
| Cybersecurity | n.takahashi | 脅威評価レポート | `ai.gftd.apps.kaisya.securityReport` AT Record |
| Branding | t.ichihara | ブランド戦略書 | `app.bsky.feed.post` (Bluesky 互換投稿) |
| Legal | k.bakshi | 契約書・法務意見書 | `ai.gftd.apps.kaisya.legalNote` AT Record (hash only, Tier 1) |
| Creative | k.takahashi | クリエイティブ制作物 | `app.bsky.feed.post` + blob 添付 |
| Engineering | f.tanaka/y.nishino | 実装 PR・リリースノート | `ai.gftd.apps.kaisya.devRelease` AT Record |
...
|---|---|---|
| 新規案件数 | `ai.gftd.apps.kaisya.processStarted` | CEO/COO ダッシュボード |
| 成約率 | BPMN Step L 完了数 / Step A 開始数 | 月次レポート投稿 |
| MRR | `ai.gftd.apps.kaisya.contractSigned` 金額合計 | 四半期振り返り |
...
- [ ] `ai.gftd.apps.kaisya.growthKpi` AT Record + yoro ダッシュボード
