...
// ── ai.gftd.apps.cad.importCadFile ──
...
// ── ai.gftd.apps.cad.getRevisionScene ──
...
// ── ai.gftd.apps.cad.addAnchoredComment ──
async function cmdAddAnchoredComment(sdk: HostSDK, body: Uint8Array): Promise<string> {
  ...
  return JSON.stringify({
    commentUri: `at://${PRIMARY_DID}/ai.gftd.apps.cad.comment/${rkey}`,
    rkey,
    createdAt,
  });
}

// ── ai.gftd.apps.cad.listComments ──
...
// ── ai.gftd.apps.cad.requestExport ──
...
export default createWorkerExport((sdk) => {
  ...
  sdk.app.command(
    nsid("ai.gftd.apps.cad.importCadFile"),
  ...
  sdk.app.query(
    nsid("ai.gftd.apps.cad.getRevisionScene"),
  ...
  sdk.app.command(
    nsid("ai.gftd.apps.cad.addAnchoredComment"),
  ...
  sdk.app.query(
    nsid("ai.gftd.apps.cad.listComments"),
  ...
  sdk.app.command(
    nsid("ai.gftd.apps.cad.requestExport"),
  ...
  sdk.app.command(nsid("ai.gftd.apps.cad.health"), async () => cmdHealth());
...
async function ingestCadCatalog(env: Record<string, unknown>, body: CadJobCompleteBody): Promise<Response> {
  ...
  for (const part of body.catalog.parts ?? []) {
    const featureId = `at://${owner}/ai.gftd.apps.cad.feature/${genID()}`;
    try {
      await sql`
