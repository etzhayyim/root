"""Docs persistence stores.

``DocStore`` is the narrow interface the handlers use. Two implementations:

- ``KotobaDocStore`` — production, backed by kotoba datomic (graph
  ``docs-v1``) via :class:`lg_calendar.kotoba_datomic.KotobaDatomic`.
- ``FakeDocStore`` — in-memory dict, used by the unit/smoke tests so the
...
class DocStore(Protocol):
    async def get_doc_attrs(self, slug: str) -> dict[str, Any] | None: ...
    async def all_doc_attrs(self) -> list[dict[str, Any]]: ...
...
class KotobaDocStore:
    ...
    async def all_doc_attrs(self) -> list[dict[str, Any]]:
        rows = await self._dm.q('[:find ?slug :where [?e :doc/type "Document"] [?e :doc/slug ?slug]]')
        out: list[dict[str, Any]] = []
        for row in rows:
            if not row:
                continue
            attrs = await self.get_doc_attrs(str(row[0]))
            if attrs:
                out.append(attrs)
        return out
    ...
    async def lookup_slug(self, attr: str, value: Any) -> str | None:
        ...
        query = f"[:find ?slug :where [?e :{bare} {encode(value)}] [?e :doc/slug ?slug]]"
