// meet.gftd.ai thin edge facade. OAuth/sync/cron run in AgentGateway MCP + pod-side LangServer.
...
const APP = "meet";
const NSID_PREFIX = "ai.gftd.apps.meet.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:meet.gftd.ai", nanoid: env.APP_NANOID ?? "meet-mcp", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "20-actors/magatama/py/src/pymagatama/ingest/gworkspace_lite.py", bpmn: "etzhayyim-root/00-contracts/bpmn/ai/gftd/meet" });
