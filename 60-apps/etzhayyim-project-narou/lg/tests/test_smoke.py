"""Local smoke tests for lg-narou OSS server.

Run from `60-apps/etzhayyim-project-narou/lg/`:
...
touching `60-apps/etzhayyim-project-narou/lg/**`.
"""
...
_EXPECTED_GRAPHS = {"health", "agent_chat"}
...
def test_server_module_imports():
    from lg_narou.server import GRAPHS, _NSID_TO_ASSISTANT, app
...
def test_graphs_match_expected_set():
    from lg_narou.server import GRAPHS
...
def test_nsid_map_completeness():
    from lg_narou.server import _NSID_TO_ASSISTANT
    assert _NSID_TO_ASSISTANT == _EXPECTED_NSID_MAP


def test_nsid_map_references_known_graphs():
    from lg_narou.server import GRAPHS, _NSID_TO_ASSISTANT
...
def client(monkeypatch):
    ...
    from lg_narou import server as srv
...
def test_unknown_nsid_xrpc_404(client):
    r = client.post("/xrpc/com.etzhayyim.narou.unknownMethod", json={})
