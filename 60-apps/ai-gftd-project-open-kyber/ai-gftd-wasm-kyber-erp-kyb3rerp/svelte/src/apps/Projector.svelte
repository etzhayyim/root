...
<script lang="ts">
  ...
  const nsid = {
    listProjectTree: 'ai.gftd.projector.listProjectTree',
    newProjectConvo: 'ai.gftd.projector.newProjectConvo',
    sendProjectMessage: 'ai.gftd.projector.sendProjectMessage',
    addConvoTask: 'ai.gftd.projector.addConvoTask',
    listConvoTasks: 'ai.gftd.projector.listConvoTasks',
    completeConvoTask: 'ai.gftd.projector.completeConvoTask',
    getConvoProjectStatus: 'ai.gftd.projector.getConvoProjectStatus',
    addReflection: 'ai.gftd.projector.addReflection',
    branchConvo: 'ai.gftd.projector.branchConvo'
  } as const;
...
<div class="grid grid-cols-[260px_minmax(0,1fr)] gap-0 -m-5 min-h-[calc(100vh-160px)]">
  <!-- Left: project tree -->
  <aside class="border-r border-gftd-border bg-gftd-sidebar p-3 flex flex-col gap-3 overflow-y-auto">
    <Button size="md" variant="solid-fill"
      onclick={() => run(nsid.newProjectConvo, { name: 'New Project ' + new Date().toLocaleDateString() })}>
      ＋ New project
    </Button>
    <div class="text-[11px] uppercase tracking-wider text-gftd-muted px-2">Projects</div>
    <ul class="text-sm">
      {#each flat as n}
        <li
        ...
        >
          {#if n.children?.length}
            <button type="button" class="w-4 text-xs text-gftd-muted" onclick={() => toggle(n.id)}>
              {expanded.has(n.id) ? '▾' : '▸'}
            </button>
          {:else}
            <span class="w-4"></span>
    ...
    <div class="mt-auto border-t border-gftd-border pt-3 text-[11px] text-gftd-muted">
      <p class="font-semibold text-gftd-secondary mb-1">Reasoning</p>
  ...
  <section class="flex flex-col min-w-0">
    <header class="px-5 py-3 border-b border-gftd-border flex items-center gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-[11px] text-gftd-muted">convoId: {activeId}</p>
      </div>
    ...
    <nav class="flex gap-1 px-5 py-2 border-b border-gftd-border text-sm">
    ...
    <div class="flex-1 overflow-auto">
      {#if view === 'overview'}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-5">
          <div class="rounded-xl border border-gftd-border bg-gftd-card p-4">
            <p class="text-[11px] uppercase tracking-wider text-gftd-muted">Tasks</p>
            <p class="text-3xl font-semibold mt-1">{tasks.length}</p>
            <p class="text-xs text-gftd-secondary">{tasks.filter(t => t.done).length} done · {tasks.filter(t => !t.done).length} open</p>
          </div>
          <div class="rounded-xl border border-gftd-border bg-gftd-card p-4">
            <p class="text-[11px] uppercase tracking-wider text-gftd-muted">Messages</p>
            <p class="text-3xl font-semibold mt-1">{msgs.length}</p>
            <p class="text-xs text-gftd-secondary">across {new Set(msgs.map(m => m.from)).size} actor(s)</p>
          </div>
          <div class="rounded-xl border border-gftd-border bg-gftd-card p-4">
            <p class="text-[11px] uppercase tracking-wider text-gftd-muted">Reflections</p>
            <p class="text-3xl font-semibold mt-1">3</p>
            <p class="text-xs text-gftd-secondary">last: 09:30 (Acc Bot)</p>
          </div>

          <div class="md:col-span-3 rounded-xl border border-gftd-border bg-gftd-card p-4">
            <p class="text-sm font-semibold mb-3">Members</p>
            <div class="flex flex-wrap gap-2">
              {#each ['AP Bot', 'Acc Bot', 'HR Bot', 'me'] as m}
                <div class="flex items-center gap-2 rounded-full border border-gftd-border px-3 py-1 text-xs">
      ...
      {:else if view === 'tasks'}
        <div class="px-5 py-4 flex flex-col gap-2">
          ...
          <ul class="flex flex-col gap-1">
            {#each tasks as t}
              <li class="flex items-center gap-3 rounded-lg border border-gftd-border bg-gftd-card px-3 py-2 hover:bg-gftd-hover">
                <input
                  type="checkbox"
                  checked={t.done}
                  onclick={() => run(nsid.completeConvoTask, { convoId: activeId, taskId: t.id })}
                />
                <span class="flex-1 text-sm" class:line-through={t.done} class:text-gftd-muted={t.done}>{t.title}</span>
                {#if t.assignee}<Badge value={t.assignee} variant="default" />{/if}
                {#if t.due}<span class="text-[11px] text-gftd-secondary">{t.due}</span>{/if}
              </li>
            {:else}
              <li class="text-center py-12 text-sm text-gftd-muted">No tasks. Use /task in chat.</li>
            {/each}
          </ul>
        </div>
      ...
      {:else if view === 'chat'}
        <div class="flex flex-col h-full">
          <div class="flex-1 overflow-auto px-5 py-4 space-y-3">
            {#each msgs as m}
              {#if m.type === 'system'}
                <div class="text-center text-[11px] text-gftd-muted">— {m.text} ({m.time}) —</div>
              {:else}
                <div class="flex gap-3" class:flex-row-reverse={m.type === 'human'}>
                  <div class="h-8 w-8 rounded-full bg-gftd-accent grid place-items-center text-[10px] font-bold text-white flex-shrink-0">
                    {m.initial}
                  </div>
                  <div
                    class="max-w-[70%] rounded-2xl px-3 py-2"
                    class:bg-gftd-accent={m.type === 'human'}
                    class:text-white={m.type === 'human'}
                    class:bg-gftd-card={m.type !== 'human'}
                    class:border={m.type !== 'human'}
                    class:border-gftd-border={m.type !== 'human'}
                  >
          ...
          <div class="border-t border-gftd-border p-3">
      ...
      {:else if view === 'reflections'}
        <div class="px-5 py-4 flex flex-col gap-3">
          <div class="rounded-xl border border-gftd-border bg-gftd-card p-4">
            <div class="flex items-center gap-2 text-xs text-gftd-muted mb-2">
