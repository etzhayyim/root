    ...
    <div class="relative">
      ...
      {#if showNewMenu}
        <div class="absolute z-20 mt-2 w-full rounded-xl border border-etzhayyim-border bg-gftd-card shadow-xl py-1 text-sm">
          ...
          >📁 New folder</button>
          <label class="block w-full text-left px-3 py-2 hover:bg-gftd-hover cursor-pointer flex items-center gap-2">
            ⬆️ File upload
  ...
  <section class="flex flex-col min-w-0">
    ...
    <div class="flex items-center justify-between gap-3 px-5 py-3 border-b border-etzhayyim-border">
      ...
      <div class="flex items-center gap-2">
        ...
        <button type="button"
          class="p-2 rounded hover:bg-gftd-hover"
          class:bg-gftd-hover={view === 'list'}
          onclick={() => (view = 'list')} title="List view">☰</button>
        <button type="button"
          class="p-2 rounded hover:bg-gftd-hover"
          class:bg-gftd-hover={view === 'grid'}
