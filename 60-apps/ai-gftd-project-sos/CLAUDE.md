# ai-gftd-project-sos — System of System Resource Intelligence
...
| **Nanoid** | `s0s5ys0s` |
| **App path** | `appview/ai-gftd-wasm-systemofsystem-s0s5ys0s/svelte/` |
...
This app uses **Threlte** (not the religious-corp `@etzhayyim/kami-engine-sdk`) for its 3D visualization layer. Same pattern as `60-apps/ai-gftd-project-cad/` (per its CLAUDE.md "3D viewer 標準は Threlte"). The three.js + `@threlte/*` deps in `appview/.../svelte/package.json` are documented design intent, NOT dead deps.
