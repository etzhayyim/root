...
const SPEC = {
  ...
  components: {
    securitySchemes: {
      bearerAuth: {
        ...
        description:
          "Tenant API key minted via POST /auth/v1/signup. Same shape as `sk_live_*` keys minted via PDS `com.etzhayyim.auth.createApiKey`. AT Protocol session JWTs (ES256) are also accepted.",
      },
    },
    schemas: {
      Error: {
  ...
  paths: {
    ...
    "/xrpc/{nsid}": {
      parameters: [{ name: "nsid", in: "path", required: true, schema: { type: "string" }, description: "AT Protocol NSID, e.g. com.etzhayyim.apps.yata.runCypher", examples: { yata: { value: "com.etzhayyim.apps.yata.runCypher" }, billing: { value: "com.etzhayyim.apps.billing.getInvoice" } } }],
