# `app.etzhayyim.cable.*` — kotoba-native cable lexicons + legacy migration inventory

**ADR-2606012600 · actor: watatsuna 綿津綱**

The `app.etzhayyim.cable.*` namespace is the **kotoba-native** home for the world
...
| Legacy lexicon (`app.etzhayyim.apps.*`) | Disposition | Maps to | Notes |
|---|---|---|---|
...
| `cableRepairFleet.registerRepairVessel` | **moved to watatsumi** | `watatsumi/data/cable-laying-fleet.kotoba.edn` + (future) `app.etzhayyim.watatsumi.cableLayVesselAttestation` | repair/lay fleet is operational → belongs to watatsumi 綿津見, not the KG actor |
...
- **R1+**: any live caller migrates to `app.etzhayyim.cable.*`; legacy lexicons then archived.
