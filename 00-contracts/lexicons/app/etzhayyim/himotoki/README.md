# `app.etzhayyim.himotoki.*` — Disclosure-request Lexicons
...
| `disclosureResponse` | `himotoki_response_intake` | Response metadata. PII payload referenced via an `app.etzhayyim.encrypted.*` DID-bound envelope ONLY (G6, ADR-2605181100); never inline. |
...
- **G6** disclosed PII lives **only** in `app.etzhayyim.encrypted.*` DID-bound envelopes.
