...
DELETE FROM dim_domain_coverage_target;
...
DROP MATERIALIZED VIEW IF EXISTS mv_domain_repo_authority_count;

DROP MATERIALIZED VIEW IF EXISTS mv_domain_repo_did_count;

CREATE MATERIALIZED VIEW mv_domain_repo_did_count AS
...
CREATE MATERIALIZED VIEW mv_domain_repo_authority_count AS
...
CREATE MATERIALIZED VIEW mv_domain_coverage_live AS
    SELECT
      t.kind,
      t.repo,
      t.app,
    ...
    LEFT JOIN mv_domain_repo_authority_count a ON a.authority_kind = t.authority_kind AND a.repo = t.repo;
