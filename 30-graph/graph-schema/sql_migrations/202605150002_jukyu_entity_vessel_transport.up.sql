...
ALTER TABLE edge_jukyu_supply_dependency ADD COLUMN IF NOT EXISTS transport_mode VARCHAR;
ALTER TABLE edge_jukyu_supply_dependency ADD COLUMN IF NOT EXISTS route_kind VARCHAR;
ALTER TABLE edge_jukyu_supply_dependency ADD COLUMN IF NOT EXISTS route_risk_score DOUBLE PRECISION;

CREATE INDEX IF NOT EXISTS idx_vertex_jukyu_transport_leg_domain_route
  ON vertex_jukyu_transport_leg (domain, origin_locode, destination_locode);

CREATE INDEX IF NOT EXISTS idx_vertex_jukyu_transport_leg_supply_edge
  ON vertex_jukyu_transport_leg (supply_edge_id);

CREATE INDEX IF NOT EXISTS idx_vertex_jukyu_transport_leg_vessel
  ON vertex_jukyu_transport_leg (vessel_imo, vessel_mmsi);

CREATE INDEX IF NOT EXISTS idx_edge_jukyu_entity_controls_node_src
  ON edge_jukyu_entity_controls_node (src_vid, role);

CREATE INDEX IF NOT EXISTS idx_edge_jukyu_entity_controls_node_dst
  ON edge_jukyu_entity_controls_node (dst_vid, domain);

CREATE INDEX IF NOT EXISTS idx_edge_jukyu_transport_moves_product_src
  ON edge_jukyu_transport_moves_product (src_vid);

CREATE INDEX IF NOT EXISTS idx_edge_jukyu_transport_moves_product_dst
  ON edge_jukyu_transport_moves_product (dst_vid);
