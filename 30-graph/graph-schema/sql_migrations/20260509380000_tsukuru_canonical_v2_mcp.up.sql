...
-- v2:        select_manufacturers via mcp://ai.gftd.tools.sql.query
--            (named-param bind on state.industry_codes)
--            emit_audit          via mcp://ai.gftd.tools.audit.emit
--
...
VALUES
  ('at://did:web:tsukuru.gftd.ai/ai.gftd.mcp.toolDef/ai-gftd-apps-tsukuru-selectManufacturers',
   ...
   'public', 1, TRUE,
   '00-contracts/lexicons/ai/gftd/apps/tsukuru/selectManufacturers.json',
...
VALUES
  ('tsukuru_isic_pulse.v2:select_manufacturers', 0, 0, 'tsukuru_isic_pulse.v2', 'select_manufacturers',
   ...
   '{"input_keys":["industry_codes"],"result_key":"queryOut","args":{"name":"ai.gftd.tools.sql.query","sql":"SELECT props FROM vertex_other WHERE label = ''TsukuruManufacturer'' AND coalesce(props::jsonb ->> ''industryCode'', '''') = ANY(%(industry_codes)s) LIMIT 25"}}',
