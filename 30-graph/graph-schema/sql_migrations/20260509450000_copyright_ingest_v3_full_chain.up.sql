...
VALUES
  ...
  ('copyright_ingest.v3:crossref_fetch', 0, 0, 'copyright_ingest.v3', 'crossref_fetch',
   'mcp_tool', 'mcp://ai.gftd.tools.http.fetch',
   '{"input_keys":[],"result_key":"fetchOut","args":{"name":"ai.gftd.tools.http.fetch","url":"https://api.crossref.org/works?rows=100&sort=indexed&order=desc&filter=from-pub-date%3A2020&mailto=jun%40gftd.group","headers":{"User-Agent":"gftd-copyright/2.0 (mailto:jun@gftd.group)","Accept":"application/json"},"timeout":60}}',
  ...
  ('copyright_ingest.v3:crossref_extract', 0, 0, 'copyright_ingest.v3', 'crossref_extract',
   'mcp_tool', 'mcp://ai.gftd.tools.json.extract',
   '{"input_keys":[],"input_paths":{"json":"fetchOut.result.body"},"result_key":"itemsOut","args":{"name":"ai.gftd.tools.json.extract","path":"message.items","default":[]}}',
   '2026-05-09T00:00:00Z'),
  ...
  ('copyright_ingest.v3:crossref_insert', 0, 0, 'copyright_ingest.v3', 'crossref_insert',
   'mcp_tool', 'mcp://ai.gftd.tools.sql.exec',
   '{"input_keys":[],"input_paths":{"rows":"rowsOut.result.rows"},"result_key":"insertOut","args":{"name":"ai.gftd.tools.sql.exec","sql":"INSERT INTO vertex_work (vertex_id, owner_did, repo, did, status, kind, title, doi, registry, berne_automatic, source_url, sensitivity_ord) VALUES (%(vertex_id)s, %(owner_did)s, %(repo)s, %(did)s, %(status)s, %(kind)s, %(title)s, %(doi)s, %(registry)s, %(berne_automatic)s, %(source_url)s, %(sensitivity_ord)s)","confirmWrite":true}}',
   '2026-05-09T00:00:00Z'),
