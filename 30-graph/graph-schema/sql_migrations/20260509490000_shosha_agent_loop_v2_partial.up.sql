...
--   call_llm       mcp_tool      RETIRED → mcp://ai.gftd.tools.llm.chat
...
INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('shosha_agent_loop.v2:call_llm', 0, 0, 'shosha_agent_loop.v2', 'call_llm', 'mcp_tool', 'mcp://ai.gftd.tools.llm.chat', '{"input_keys":[],"input_paths":{"user":"_userMessage","tier":"tier","maxTokens":"maxTokens"},"result_key":"llmOut","args":{"name":"ai.gftd.tools.llm.chat","system":"You are 商社 (shosha.etzhayyim.com), an autonomous AI sogo-shosha agent. You have read access to recent market intel, market views, and open exposure. Be concise, factual, and acknowledge uncertainty. Default to Japanese unless the user writes English. Keep replies under 600 tokens.","temperature":0.3}}', '2026-05-09T07:00:00Z');

INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('shosha_agent_loop.v2:emit_audit', 0, 0, 'shosha_agent_loop.v2', 'emit_audit', 'mcp_tool', 'mcp://ai.gftd.tools.audit.emit', '{"input_keys":[],"input_paths":{"recordJson":"llmOut.result"},"result_key":"auditOut","args":{"name":"ai.gftd.tools.audit.emit","repo":"did:web:shosha.etzhayyim.com","collection":"ai.gftd.apps.shosha.agentLoop","action":"create"}}', '2026-05-09T07:00:00Z');
