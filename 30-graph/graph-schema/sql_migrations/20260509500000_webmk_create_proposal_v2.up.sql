...
INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('webmk_create_proposal.v2:research_company', 0, 0, 'webmk_create_proposal.v2', 'research_company', 'mcp_tool', 'mcp://ai.gftd.tools.llm.chat', '{"input_paths":{"websiteUrl":"websiteUrl","industry":"industry"},"result_key":"companyContextOut","args":{"name":"ai.gftd.tools.llm.chat","tier":"fast","system":"You are a marketing researcher. Respond with concise JSON only.","user_template":"Analyze this company URL: {websiteUrl}\nIndustry: {industry}\n\nReturn JSON with keys: offering, targetSegment, marketingTone, angles[]","maxTokens":600,"temperature":0.3}}', '2026-05-09T08:00:00Z');

INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('webmk_create_proposal.v2:analyze_competitors', 0, 0, 'webmk_create_proposal.v2', 'analyze_competitors', 'mcp_tool', 'mcp://ai.gftd.tools.llm.chat', '{"input_paths":{"clientName":"clientName","industry":"industry","companyContext":"companyContextOut.result.content"},"result_key":"competitorSummaryOut","args":{"name":"ai.gftd.tools.llm.chat","tier":"fast","system":"You are a competitive intelligence analyst. Respond with concise JSON only.","user_template":"Client: {clientName}\nIndustry: {industry}\nContext: {companyContext}\n\nIdentify 3 likely competitors. Return JSON: competitors[{{name,positioning,keywords[],opportunity}}], mainGap","maxTokens":700,"temperature":0.3}}', '2026-05-09T08:00:00Z');

INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('webmk_create_proposal.v2:generate_strategy', 0, 0, 'webmk_create_proposal.v2', 'generate_strategy', 'mcp_tool', 'mcp://ai.gftd.tools.llm.chat', '{"input_paths":{"clientName":"clientName","websiteUrl":"websiteUrl","industry":"industry","budgetJpy":"budgetJpy","companyContext":"companyContextOut.result.content","competitorSummary":"competitorSummaryOut.result.content"},"result_key":"strategyOut","args":{"name":"ai.gftd.tools.llm.chat","tier":"balanced","system":"You are a senior digital marketing strategist. Respond with JSON only.","user_template":"Client: {clientName}\nWebsite: {websiteUrl}\nIndustry: {industry}\nBudget JPY: {budgetJpy}\n\nResearch:\n{companyContext}\n\nCompetitors:\n{competitorSummary}\n\nReturn JSON: {{executiveSummary, channels[{{name,priority,rationale,kpis[]}}], contentThemes[], sixMonthMilestones[{{month,goal,metric}}], estimatedRoi}}","maxTokens":1200,"temperature":0.4}}', '2026-05-09T08:00:00Z');

INSERT INTO vertex_langgraph_assistant_node (vertex_id, _seq, sensitivity_ord, assistant_id, node_id, kind, ref, config, created_at) VALUES ('webmk_create_proposal.v2:generate_copy', 0, 0, 'webmk_create_proposal.v2', 'generate_copy', 'mcp_tool', 'mcp://ai.gftd.tools.llm.chat', '{"input_paths":{"clientName":"clientName","websiteUrl":"websiteUrl","strategy":"strategyOut.result.content"},"result_key":"copyOut","args":{"name":"ai.gftd.tools.llm.chat","tier":"balanced","system":"You are a copywriter. Format output in Markdown.","user_template":"Client: {clientName} ({websiteUrl})\nStrategy: {strategy}\n\nWrite:\n## Hero Headlines (3)\n## Sub-headline\n## SNS Posts (Instagram/X/LinkedIn)\n## Email Subject Lines (A/B)\n## Google Ads Headlines (3, ≤30 chars)\n## CTA Options (3)","maxTokens":1000,"temperature":0.5}}', '2026-05-09T08:00:00Z');
