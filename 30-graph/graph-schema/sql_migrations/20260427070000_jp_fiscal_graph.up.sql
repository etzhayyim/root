...
CREATE INDEX IF NOT EXISTS idx_jp_fiscal_source_collection ON vertex_jp_fiscal_source (collection, status);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_document_source ON vertex_jp_fiscal_document (source_id, collection);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_budget_year ON vertex_jp_fiscal_budget_book (fiscal_year, account_type, doc_type);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_appropriation_ministry ON vertex_jp_fiscal_appropriation (ministry_did, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_procurement_issuer ON vertex_jp_fiscal_procurement_bid (issuer_did, opened_at);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_contract_issuer ON vertex_jp_fiscal_contract (issuer_did, signed_date);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_contract_contractor ON vertex_jp_fiscal_contract (contractor_did, signed_date);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_payment_payee ON vertex_jp_fiscal_payment_record (payee_did, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_tax_receiver ON vertex_jp_fiscal_tax_payment (receiver_did, period_iso);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_audit_target ON vertex_jp_fiscal_audit_finding (target_did, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_ubo_child ON vertex_jp_fiscal_beneficial_owner (child_did, observed_at);

CREATE INDEX IF NOT EXISTS idx_jp_fiscal_statement_speaker ON vertex_jp_fiscal_public_statement (speaker_did, stated_at);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_flow_year ON edge_jp_fiscal_flow (fiscal_year, flow_type);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_flow_source ON edge_jp_fiscal_flow (source_did, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_flow_dest ON edge_jp_fiscal_flow (dest_did, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_flow_recipient ON edge_jp_fiscal_flow (recipient_id, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_ownership_child ON edge_jp_fiscal_ownership (child_did, observed_at);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_actor_rel_subject ON edge_jp_fiscal_actor_relationship (subject_did, relationship_type);

CREATE INDEX IF NOT EXISTS idx_edge_jp_fiscal_actor_rel_object ON edge_jp_fiscal_actor_relationship (object_did, relationship_type);
