"""Captured from Kysely migration 20260424173100_seed_open_doi_bpmn_actors."""
...
revision = "r_20260424173100_seed_open_doi_bpmn_actors"
down_revision = 'r_20260424173000_vertex_open_doi'
branch_labels = None
depends_on = None
...
UP = [{'sql': '\n'
  ...
  'parameters': ['at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/open-doi-register-doi-v1',
                 ...
                 'targetRef="Task_Save"/>\n'
                 '\n'
                 '    <bpmn:serviceTask id="Task_Save" name="doi 保存">\n'
                 '      <bpmn:extensionElements>\n'
                 '        <zeebe:taskDefinition type="generic.db.insert"/>\n'
                 '        <zeebe:ioMapping>\n'
                 '          <zeebe:input source="=&quot;vertex_open_doi_doi&quot;" '
                 'target="table"/>\n'
                 '          <zeebe:input source="={\n'
                 '              vertex_id: vertexId, doi: doi,\n'
                 '              doi_prefix: substring before(doi, &quot;/&quot;), doi_suffix: '
                 'substring after(doi, &quot;/&quot;),\n'
                 '              registrant_org_id: registrantOrgId, title: title,\n'
                 '              publication_type: publicationType, publisher: publisher,\n'
                 '              published_at: publishedAt, authors_orcid: authorsOrcid,\n'
                 '              verification: verification, status: &quot;active&quot;,\n'
                 '              created_at: string(now()), owner_did: callerDid,\n'
                 '              sensitivity_ord: 1, org_id: registrantOrgId,\n'
                 '              user_id: callerDid, actor_id: &quot;sys.bpmn.open-doi&quot;\n'
                 '          }" target="values"/>\n'
                 ...
                 'target="action"/>\n'
                 '          <zeebe:input source="={vertexId: vertexId, doi: doi, publisher: '
                 'publisher, verification: verification}" target="payload"/>\n'
                 '        </zeebe:ioMapping>\n'
                 '      </bpmn:extensionElements>\n'
                 '      '
                 ...
                 'targetRef="End_R"/>\n'
                 '\n'
                 '    <bpmn:serviceTask id="Task_AuditAuto" name="audit doi.register.accept">\n'
                 '      <bpmn:extensionElements>\n'
                 '        <zeebe:taskDefinition type="generic.audit.emit"/>\n'
                 '        <zeebe:ioMapping>\n'
                 '          <zeebe:input source="=&quot;did:web:open-doi.etzhayyim.com&quot;" '
 ...
 {'sql': '\n'
  ...
  'parameters': ['at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/open-doi-record-citation-v1',
                 ...
                 '        <zeebe:taskDefinition type="generic.db.insert"/>\n'
                 '        <zeebe:ioMapping>\n'
                 '          <zeebe:input source="=&quot;vertex_open_doi_citation&quot;" '
                 'target="table"/>\n'
                 '          <zeebe:input source="={\n'
                 '              vertex_id: vertexId, citing_doi: citingDoi, cited_doi: citedDoi,\n'
                 ...
                 'target="action"/>\n'
                 '          <zeebe:input source="={vertexId: vertexId, citingDoi: citingDoi, '
                 'citedDoi: citedDoi}" target="payload"/>\n'
                 '        </zeebe:ioMapping>\n'
                 '      </bpmn:extensionElements>\n'
                 '      '
