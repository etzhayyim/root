"""Captured from Kysely migration 20260430703000_natural_person_all_latent_materialization."""
...
revision = "r_20260430703000_natural_person_all_latent_materialization"
down_revision = 'r_20260430702000_seed_natural_person_seed_latent_entities_bpmn'
branch_labels = None
depends_on = None
...
UP = [{'sql': '\n'
         ...
         '  ',
  'parameters': []},
 {'sql': 'CREATE INDEX IF NOT EXISTS idx_np_latent_cursor_status ON '
         'vertex_natural_person_latent_materialization_cursor(status, updated_at)',
  'parameters': []},
 {'sql': 'CREATE INDEX IF NOT EXISTS idx_np_latent_cursor_hash ON '
         'vertex_natural_person_latent_materialization_cursor(cohort_hash)',
  'parameters': []},
 {'sql': '\n'
  ...
  'parameters': ['at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/natural-person-materialize-all-latent-entities-v1',
                 ...
                 "vertex_latent_entity WHERE entity_kind = 'natural_person_individual_latent') AS "
                 'total_individual_latent_entities, (SELECT COALESCE(SUM(GREATEST(target_count - '
                 'next_ordinal + 1, 0)), 0)::bigint FROM '
                 "vertex_natural_person_latent_materialization_cursor WHERE status = 'active') AS "
                 'remaining_individuals&quot;" target="sql"/>\n'
                 '          <zeebe:input source="=[]" target="params"/>\n'
                 '          <zeebe:input source="=60000" target="resultTimeoutMs"/>\n'
                 ...
                 '              &quot;totalIndividualLatentEntities&quot;: '
                 'materializationVerify.total_individual_latent_entities,\n'
