...
/**
 * Domain coverage live read model.
 *
 * Goal:
 *   - replace N x per-domain COUNT queries with one MV read
 *   - normalize alias kinds (family/cultural, academic) at the schema layer
 *   - expose a stable Kysely read path for CLI / PDS / apps
 */
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`INSERT INTO dim_domain_coverage_target (
    ...
    ('blockchain', 'did:web:blockchain.etzhayyim.com', 'blockchain.etzhayyim.com', 100, 5000, 200, 5300, 30, 65, 20, 115, 'vertex_authority_blockchain', 'blockchain')`.execute(db);

  await sql`CREATE MATERIALIZED VIEW IF NOT EXISTS mv_domain_repo_did_count AS
    SELECT
      t.kind,
      t.repo,
  ...
  await sql`CREATE MATERIALIZED VIEW IF NOT EXISTS mv_domain_repo_authority_count AS
    ...
    GROUP BY repo`.execute(db);

  await sql`CREATE MATERIALIZED VIEW IF NOT EXISTS mv_domain_coverage_live AS
    SELECT
      t.kind,
      t.repo,
      t.app,
    ...
    LEFT JOIN mv_domain_repo_authority_count a
      ON a.authority_kind = t.authority_kind AND a.repo = t.repo`.execute(db);
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`DROP TABLE IF EXISTS dim_domain_coverage_target`.execute(db);
}
