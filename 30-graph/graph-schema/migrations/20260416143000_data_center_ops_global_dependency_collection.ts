...
/**
 * Data Center Ops global dependency collection.
 *
 * RisingWave does not support cross-join style nested-loop plans for streaming
 * MVs, so we persist global expansions into physical vertex/edge tables, then
 * expose actor-facing read surfaces via MVs.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_dc_dep_global_actor_country ON vertex_data_center_dependency_global (actor_did, country_code)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_dc_dep_global_domain_level ON vertex_data_center_dependency_global (dependency_domain, dependency_level)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_dc_dep_global_node ON vertex_data_center_dependency_global (dependency_key, reverse_topology_rank)`.execute(db);
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_dc_dep_global_actor_country ON edge_data_center_dependency_global (actor_did, country_code)`.execute(db);
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_mv_dc_dep_global_actor_country ON mv_data_center_dependency_global_actor (actor_did, country_code)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_mv_dc_dep_global_actor_domain ON mv_data_center_dependency_global_actor (dependency_domain, dependency_level)`.execute(db);
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_mv_dc_dep_global_edge_country ON mv_data_center_dependency_global_edge (actor_did, country_code)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_mv_dc_dep_global_edge_path ON mv_data_center_dependency_global_edge (from_node_key, to_node_key)`.execute(db);
}

export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_data_center_dependency_global_edge`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_data_center_dependency_global_actor`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_data_center_dependency_global`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_data_center_dependency_global`.execute(db);
}
