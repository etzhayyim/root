...
const ownerDid = "did:web:kaikei.gftd.ai";
...
const processVertexId = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.processDef/kaikei-${s.slug}-v1`;
const bindingVertexId = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.binding/kaikei-${s.op}-v1`;
...
export async function up(db: Kysely<unknown>): Promise<void> {
  for (const s of seeds) {
    ...
    await sql`
      ...
      SELECT
        ${bindingVertexId(s)}, ${ownerDid}, ${`ai.gftd.apps.kaikei.${s.op}`}, ${s.processId}, 1,
