...
const ownerDid = "did:web:isekai.gftd.ai";
...
const seeds: Seed[] = [
  ...
  nsid: `ai.gftd.apps.isekai.${proc}`,
  resultTimeoutMs: 30000,
  writeTableAllowlist: writeProcs.has(proc) ? writeAllowlist : "",
}));

const bpmnPath = (s: Seed) => `00-contracts/bpmn/ai/gftd/isekai/${s.proc}.bpmn`;
const processVid = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.processDef/isekai-${slug(s.proc)}-v1`;
const bindingVid = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.binding/isekai-${slug(s.proc)}-v1`;
