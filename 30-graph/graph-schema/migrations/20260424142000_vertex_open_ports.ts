...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_ports_open_incidents`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_open_ports_call_endpoint`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_ports_incident`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_ports_vessel_call`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_ports_vessel`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_ports_port`.execute(db);
}
