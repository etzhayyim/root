...
const ownerDid = "did:web:oshikatsu.etzhayyim.com";
const createdAt = "2026-05-07T02:15:00Z";
const actorId = "sys.bpmn.seed.oshikatsu";

const snake = (proc: string) => proc.replace(/([A-Z])/g, "_$1").toLowerCase();
const slug = (proc: string) => proc.replace(/([A-Z])/g, "-$1").toLowerCase();
const writeProcs = new Set(["createCreatorProfile", "updateTiers", "subscribe", "unsubscribe", "publishContent", "tip"]);
const writeAllowlist = [
  "vertex_oshikatsu_creator_profile",
  "vertex_oshikatsu_subscription_tier",
  "vertex_oshikatsu_subscription",
  "vertex_oshikatsu_subscription_cancel",
  "vertex_oshikatsu_exclusive_content",
  "vertex_oshikatsu_tip",
  "edge_oshikatsu_creator_tier",
  "edge_oshikatsu_subscription",
  "edge_oshikatsu_content_by_creator",
  "edge_oshikatsu_tip_to_creator",
  "edge_oshikatsu_tip_for_content",
].join(",");
const procs = ["createCreatorProfile", "getCreatorProfile", "listCreators", "updateTiers", "subscribe", "unsubscribe", "listSubscriptions", "checkAccess", "publishContent", "getContent", "listContent", "tip", "creatorStats", "search"];

const seeds: Seed[] = procs.map((proc) => ({
  proc,
  bpmnProcessId: `oshikatsu_${snake(proc)}`,
  nsid: `ai.gftd.apps.oshikatsu.${proc}`,
...
const processVid = (s: Seed) => `at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/oshikatsu-${slug(s.proc)}-v1`;
const bindingVid = (s: Seed) => `at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.binding/oshikatsu-${slug(s.proc)}-v1`;
