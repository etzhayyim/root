...
// JP fiscal record coverage projection.
// Connects parsed records to supporting documents and budget appropriation flows.

export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    CREATE MATERIALIZED VIEW IF NOT EXISTS mv_jp_fiscal_record_document_coverage AS
    SELECT
      records.collection,
      records.vertex_id,
      records.fiscal_year,
  ...
  await sql`
    CREATE MATERIALIZED VIEW IF NOT EXISTS mv_jp_fiscal_collection_record_coverage AS
    SELECT
      collection,
      fiscal_year,
      account_type,
  ...
  await sql`
    CREATE MATERIALIZED VIEW IF NOT EXISTS mv_jp_fiscal_budget_pipeline_coverage AS
    SELECT
