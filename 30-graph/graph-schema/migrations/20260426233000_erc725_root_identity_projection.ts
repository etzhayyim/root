...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN root_did_hash VARCHAR`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN root_identity_addr VARCHAR`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN facade_did VARCHAR`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN facade_did_hash VARCHAR`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN identity_method VARCHAR`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity ADD COLUMN migration_status VARCHAR`.execute(db);
  await sql`FLUSH`.execute(db);
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_gftd_identity_root_hash
    ON vertex_gftd_identity (root_did_hash)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`DROP INDEX IF EXISTS idx_vertex_gftd_identity_root_hash`.execute(db);
  ...
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN migration_status`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN identity_method`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN facade_did_hash`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN facade_did`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN root_identity_addr`.execute(db);
  await sql`ALTER TABLE vertex_gftd_identity DROP COLUMN root_did_hash`.execute(db);
