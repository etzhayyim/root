...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_ems_compliance`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_ems_order`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_ems_facility`.execute(db);
}
