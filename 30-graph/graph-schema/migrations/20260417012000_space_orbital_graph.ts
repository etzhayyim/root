...
/**
 * Typed orbital graph for cosmic maps.
 *
 * - orbital systems: Earth-Moon, Solar System, Milky Way frame
 * - orbital bodies: Earth, Moon, Sun, ISS, GEO belt representative satellites
 * - enough fields to drive ephemeris / TLE / spiral-vortex scene assembly
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_orbital_system_system_id ON vertex_orbital_system (system_id)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_orbital_body_system_id ON vertex_orbital_body (system_id)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_orbital_body_body_kind ON vertex_orbital_body (body_kind)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_orbital_body_norad_id ON vertex_orbital_body (norad_id)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP INDEX IF EXISTS idx_vertex_orbital_body_norad_id`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_orbital_body_body_kind`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_orbital_body_system_id`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_orbital_system_system_id`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_orbital_body`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_orbital_system`.execute(db);
}
