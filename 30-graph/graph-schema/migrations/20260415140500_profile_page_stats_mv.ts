 ...
 *
 * Purpose:
 * - fast actor counters for profile surfaces (followers/following/posts/pages/tools/governance)
 * - normalize did:web path DIDs to base host DID for stable aggregation
 * - expose both raw actor_did and canonical_actor_did for flexible lookups
 */
      ...
      SELECT
        CASE
          WHEN actor_did LIKE 'did:web:%'
            THEN regexp_replace(actor_did, '^(did:web:[^:]+).*$' , '$1')
          ELSE actor_did
        END AS canonical_actor_did,
