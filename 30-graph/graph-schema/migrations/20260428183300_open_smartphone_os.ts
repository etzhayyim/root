...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_smartphone_os_cve_exposure`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_os_hal_driver`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_os_ota`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_os_build`.execute(db);
}
