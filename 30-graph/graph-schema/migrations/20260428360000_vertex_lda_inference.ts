...
GROUP BY e.dst_vid`,
  ...
  for (let i = 0; i < viewpoints.length; i++) {
    ...
    const vid = `at://did:web:coverage.etzhayyim.com/com.etzhayyim.apps.coverage.ldaViewpoint/${vp.kind}`;
    await db
  ...
  const signals: SignalSeed[] = [
    // lexical
    { vp_kind: "lexical", token: "nsid:com.etzhayyim.apps", kind: "nsid_prefix", source: "vertex_repo_record", expr: "SPLIT_PART(collection,'.',3)" },
    ...
    // behavioral
    { vp_kind: "behavioral", token: "col:com.etzhayyim.apps.naturalPerson.naturalPerson", kind: "collection", source: "vertex_repo_record", expr: "collection" },
    { vp_kind: "behavioral", token: "col:app.bsky.feed.post", kind: "collection", source: "vertex_repo_record", expr: "collection" },
    { vp_kind: "behavioral", token: "col:com.etzhayyim.apps.coverage.ldaViewpoint", kind: "collection", source: "vertex_repo_record", expr: "collection" },
  ...
  for (let i = 0; i < signals.length; i++) {
    const s = signals[i];
    const vpVid = `at://did:web:coverage.etzhayyim.com/com.etzhayyim.apps.coverage.ldaViewpoint/${s.vp_kind}`;
    const hash6 = s.token.replace(/[^a-z0-9]/g, "").substring(0, 12).padEnd(6, "0").substring(0, 6);
    const sid = `at://did:web:coverage.etzhayyim.com/com.etzhayyim.apps.coverage.ldaSignal/${s.vp_kind}-${hash6}${i}`;
    await db
