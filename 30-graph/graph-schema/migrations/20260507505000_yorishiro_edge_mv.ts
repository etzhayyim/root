...
export async function up(db: Kysely<unknown>): Promise<void> {
  for (const table of [
    "edge_yorishiro_nuro_offer_claim",
    "edge_yorishiro_nuro_claim_receipt",
    "edge_yorishiro_enaiyo_draft_docx_blob",
    "edge_yorishiro_enaiyo_draft_submit_job",
    "edge_yorishiro_enaiyo_submit_receipt",
    "edge_yorishiro_enaiyo_batch_draft",
    "edge_yorishiro_flyio_job_provider",
  ]) {
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yorishiro_flyio_job_status`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yorishiro_enaiyo_submission_flow`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yorishiro_nuro_claim_flow`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_flyio_job_provider`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_enaiyo_batch_draft`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_enaiyo_submit_receipt`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_enaiyo_draft_submit_job`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_enaiyo_draft_docx_blob`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_nuro_claim_receipt`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yorishiro_nuro_offer_claim`.execute(db);
}
