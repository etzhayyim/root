...
// ADR-2604270030 — Japan My Number open reference graph schema.
// tier: B for runtime audit/state vertices, C for derived status projections.
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_jpn_mynumber_audit_timeline`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_jpn_mynumber_file_transfer_status`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_jpn_mynumber_oauth_token_status`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_open_jpn_mynumber_transfer_manifest`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_open_jpn_mynumber_event_subject`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_file_transfer`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_file_manifest`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_oauth_token`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_agency_alias`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_consent_receipt`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_jpn_mynumber_audit_event`.execute(db);
}
