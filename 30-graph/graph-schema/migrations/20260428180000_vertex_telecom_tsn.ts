...
// tier: B (TSN domain / bridge / sync / stream catalog);
// tier: C (shaper, FRER, sync deviation, SLA breach audit)

/**
 * telecom Phase 15 — TSN (IEEE 802.1 Time-Sensitive Networking).
 *
 * Builds on Phase 2 (networkAsset — bridge attached_asset_vid), Phase 7
 * (alarm — SLA breach correlation), Phase 8 (NWDAF QOS_SUSTAINABILITY),
 * Phase 11 (NPN — industrial 5G domain), Phase 12 (SDN southbound flow).
 *
 * gPTP sync is gauge data; deviations + breaches are append-only audit.
 * TAS gate schedules persist via vault://+sha256 only.
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_breach_summary`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_sync_health`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_shaper_state`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_stream_state`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_bridge_inventory`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_telecom_tsn_domain_state`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_telecom_tsn_breach_for_stream`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_telecom_tsn_stream_via_bridges`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_telecom_tsn_bridge_in_domain`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_sla_breach`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_sync_deviation`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_frer_profile`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_shaper`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_stream`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_sync_profile`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_bridge`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_telecom_tsn_domain`.execute(db);
}
