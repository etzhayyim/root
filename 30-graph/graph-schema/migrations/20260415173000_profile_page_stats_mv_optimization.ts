...
/**
 * Reduce memory pressure for profile page counters.
 *
 * Strategy:
 * 1) Keep a single MV to avoid multi-stage backfill orchestration cost.
 * 2) Replace regex normalization with split-based normalization.
 * 3) Add read indexes for profile hot-path lookups.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_profile_page_stats`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_profile_page_count_by_canonical_did`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_profile_page_count_by_owner_did`.execute(db);
