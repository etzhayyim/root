...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_soc_export_flag`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_soc_fab_order`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_soc_design`.execute(db);
}
