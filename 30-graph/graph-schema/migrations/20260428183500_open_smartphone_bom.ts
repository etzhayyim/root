...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_bom_sourcer`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_bom_line`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_bom`.execute(db);
}
