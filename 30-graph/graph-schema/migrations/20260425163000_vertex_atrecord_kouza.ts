...
// tier: B
/**
 * ADR-0093 — kouza financial account aggregator.
 *
 * Read-only source-of-truth records for external bank, securities, card,
 * custody, and e-money accounts. Credential material and full account numbers
 * stay in vault; RisingWave stores masked identifiers, hashes, CIDs, sync
 * audit logs, and accounting projection hints.
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_kouza_account_sync_health`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_atrecord_kouza_sync_run`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_atrecord_kouza_account_document`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_atrecord_kouza_external_transaction`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_atrecord_kouza_financial_account`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_atrecord_kouza_institution_connection`.execute(db);
}
