...
/**
 * Flight graph spine:
 * - aircraft registry with operator/owner + lifecycle
 * - per-flight operation snapshots (delay, occupancy, revenue/cost/profit)
 * - explicit edges for ownership and operation topology
 * - streaming MVs for real-time airline/aircraft monitoring
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_flight_operator_kpi_daily`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_flight_operation_latest_by_aircraft`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_flight_arrives_at`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_flight_departs_from`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_flight_operated_by`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_flight_uses_aircraft`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_aircraft_operated_by`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_aircraft_owned_by`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_flight_operation`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_aircraft`.execute(db);
}
