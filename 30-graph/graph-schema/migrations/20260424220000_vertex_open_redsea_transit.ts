...
/**
 * open-redsea-transit — chokepoint cluster (ADR-0017 Wave 9).
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_redsea_houthi_risk`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_redsea_transit_passage`.execute(db);
}
