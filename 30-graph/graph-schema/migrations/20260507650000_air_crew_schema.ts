...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_air_crew_fatigue_risk`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_crew_duty_time_crew_date`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_crew_pairing_carrier_id`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_crew_roster_crew_date`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_air_crew_roster_has_pairing`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_crew_qualification`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_crew_duty_time`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_crew_pairing`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_crew_roster`.execute(db);
}
