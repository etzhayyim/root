...
const ownerDid = "did:web:ads.gftd.ai";
...
const sourcePath = (s: Seed) => `00-contracts/bpmn/ai/gftd/ads/${s.op}.bpmn`;
const processVertexId = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.processDef/ads-${s.slug}-v1`;
const bindingVertexId = (s: Seed) => `at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.binding/ads-${s.op}-v1`;
...
export async function up(db: Kysely<unknown>): Promise<void> {
  for (const s of seeds) {
    ...
    await sql`
      ...
      SELECT
        ${bindingVertexId(s)}, ${ownerDid}, ${`ai.gftd.apps.ads.${s.op}`}, ${s.processId}, 1,
