...
async function createEdgeTable(db: Kysely<unknown>, table: string): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS ${sql.ref(`idx_${table}_chain`)} ON ${sql.table(table)} (chain_id)`.execute(db);
}

export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`CREATE INDEX IF NOT EXISTS idx_intel_cohort_subject_domain ON vertex_intel_inferred_cohort (subject_did, target_domain)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_intel_cohort_confidence ON vertex_intel_inferred_cohort (confidence)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_intel_evidence_payload ON vertex_intel_evidence (payload_json)`.execute(db);

  for (const table of [
    "edge_intel_report_entity",
    "edge_intel_chain_cohort",
    "edge_intel_chain_evidence",
    "edge_intel_cohort_evidence",
  ]) {
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_intel_coverage_projection`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_intel_inference_chain_flow`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_intel_cohort_evidence`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_intel_chain_evidence`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_intel_chain_cohort`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_intel_report_entity`.execute(db);
}
