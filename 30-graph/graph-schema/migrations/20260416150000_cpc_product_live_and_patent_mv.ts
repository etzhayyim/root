 ...
 * - `com.etzhayyim.apps.cpc.product` records written by the CPC app
 * - `vertex_patent` promoted patent rows
 *
 * This migration adds:
 * - `view_cpc_product_live`            typed projection for the new CPC app collection
 * - `mv_cpc_patent_prefix_match`       subclass/product ↔ patent class-prefix hits
 * - `mv_cpc_patent_coverage`           compact rollup for CPC-driven patent coverage queries
 *
 * No new dedicated vertex/edge table is introduced here because taxonomy records
 * already land in RisingWave through the existing AT record mirror.
 */
