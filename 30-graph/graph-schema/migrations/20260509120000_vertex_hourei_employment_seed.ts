...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  for (const h of HOUREI) {
    const vid = `at://did:web:hourei.etzhayyim.com/com.etzhayyim.apps.hourei.law/${h.hid}`;
  ...
  for (const j of JOBUN) {
    const vid = `at://did:web:hourei.etzhayyim.com/com.etzhayyim.apps.hourei.article/${j.hid}--${j.art}`;
