...
const seeds: Seed[] = [
  { proc: "searchPrecedent",  bpmnProcessId: "lawfirm_search_precedent",
    nsid: "app.etzhayyim.apps.lawfirm.searchPrecedent", resultTimeoutMs: 30000, version: 1 },
  ...
  { proc: "runConflictCheck", bpmnProcessId: "lawfirm_run_conflict_check",
    nsid: "app.etzhayyim.apps.lawfirm.runConflictCheck", resultTimeoutMs: 60000, version: 2 },
];
...
const processVertexId = (s: Seed) =>
  `at://did:web:bpmn.etzhayyim.com/app.etzhayyim.apps.bpmn.processDef/${project}-${slug(s.proc)}-v${s.version}`;
const bindingVertexId = (s: Seed) =>
  `at://did:web:bpmn.etzhayyim.com/app.etzhayyim.apps.bpmn.binding/${project}-${s.proc}-v${s.version}`;
...
async function insertBinding(db: Kysely<unknown>, s: Seed): Promise<void> {
  ...
  if (s.proc === "runConflictCheck") {
    const v1Vid = `at://did:web:bpmn.etzhayyim.com/app.etzhayyim.apps.bpmn.binding/${project}-${s.proc}-v1`;
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  const v1Vid = `at://did:web:bpmn.etzhayyim.com/app.etzhayyim.apps.bpmn.binding/${project}-runConflictCheck-v1`;
