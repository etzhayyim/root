...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_sensor_driver`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_sensor_calibration`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_sensor_module`.execute(db);
}
