...
export async function up(db: Kysely<unknown>): Promise<void> {
  // ── Assignments (vertex_gftdcojp_assignment) ────────────────────────────
  await sql`
  ...
  // ── RACI (vertex_gftdcojp_raci) — tanaka/nishino on lawfirm task NSIDs ──
  await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DELETE FROM vertex_gftdcojp_raci WHERE context LIKE '%lawfirm%' OR context LIKE '%pwc%' OR context LIKE '%pipeline stage%'`.execute(db);
  await sql`DELETE FROM vertex_gftdcojp_assignment WHERE project_id = 'lawfirm-india-program'`.execute(db);
}
