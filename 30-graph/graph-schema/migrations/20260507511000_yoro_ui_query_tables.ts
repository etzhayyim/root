...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_browsing_history_repo_created ON vertex_yoro_browsing_history (repo, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_browsing_history_path ON vertex_yoro_browsing_history (path)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_koji_actor_created ON vertex_yoro_koji_discovery (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_kyumei_actor_created ON vertex_yoro_kyumei_validation (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_shinka_actor_created ON vertex_yoro_shinka_evolution (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_hinshitsu_actor_created ON vertex_yoro_hinshitsu_assessment (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_knowledge_actor_created ON vertex_yoro_shinka_knowledge (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_koji_source ON vertex_yoro_koji_discovery (source)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_kyumei_source ON vertex_yoro_kyumei_validation (source)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_shinka_source ON vertex_yoro_shinka_evolution (source)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_hinshitsu_source ON vertex_yoro_hinshitsu_assessment (source)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_yoro_knowledge_source ON vertex_yoro_shinka_knowledge (source)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_dojo_step_actor_created ON vertex_dojo_step_completed_event (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_joucho_review_actor_created ON vertex_joucho_review (actor_did, created_at DESC)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_state_profile_repo_rkey ON vertex_state_profile (repo, rkey)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_state_profile_iso3 ON vertex_state_profile (iso3)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_agent_governance_rule_repo ON vertex_agent_governance_rule (repo)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_agent_role_binding_app_id ON vertex_agent_role_binding (app_id)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_agent_role_binding_principal ON vertex_agent_role_binding (principal_did)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_yoro_actor_evolution_src ON edge_yoro_actor_evolution (src_vid, relation)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_yoro_actor_browsing_src ON edge_yoro_actor_browsing_history (src_vid, relation)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_yoro_actor_score_src ON edge_yoro_actor_score_event (src_vid, relation)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_state_profile_repo_src ON edge_state_profile_repo (src_vid, relation)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_edge_agent_governance_src ON edge_agent_governance (src_vid, relation)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_agent_role_binding_status`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_state_profile_status`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yoro_actor_score_counts`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yoro_actor_evolution_counts`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yoro_evolution_recent`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yoro_evolution_stats`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_yoro_browsing_history_recent`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_agent_governance`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_state_profile_repo`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yoro_actor_score_event`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yoro_actor_browsing_history`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_yoro_actor_evolution`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_agent_role_binding`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_agent_governance_rule`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_state_profile`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_joucho_review`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_dojo_step_completed_event`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_shinka_knowledge`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_hinshitsu_assessment`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_shinka_evolution`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_kyumei_validation`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_koji_discovery`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_yoro_browsing_history`.execute(db);
}
