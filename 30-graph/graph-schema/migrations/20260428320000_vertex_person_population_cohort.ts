...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  for (let i = 0; i < eraLabels.length - 1; i++) {
    ...
    const dstVid = `${REPO}/ai.gftd.apps.naturalPerson.populationCohort/${dstRkey}`;
    const edgeId = `${REPO}/ai.gftd.apps.naturalPerson.cohortAncestorOf/anc-${srcLabel}-${dstLabel}`;
