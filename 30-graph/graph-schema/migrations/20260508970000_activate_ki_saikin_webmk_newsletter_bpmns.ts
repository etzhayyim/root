...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  const sourcePath = "00-contracts/bpmn/ai/gftd/newsletter/weeklySend.bpmn";
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`
    ...
    WHERE vertex_id = 'at://did:web:bpmn.gftd.ai/ai.gftd.apps.bpmn.processDef/newsletter-weeklySend-v1'
  `.execute(db);
}
