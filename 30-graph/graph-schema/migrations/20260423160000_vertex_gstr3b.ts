import type { Kysely } from "kysely";
import { sql } from "kysely";

// ADR-0040 vertex tier declarations for tables created in this migration.
// tier: B

/**
 * ADR-0056 — GSTR-3B monthly summary return actor schema.
 *
 * Tier split:
 *   vertex_gstr3b_return       — Tier 1, federable, hash + monthly totals + ARN
 *   vertex_gstr3b_return_pii   — Tier 3, internal-trust, signal:v1: payloads
 *
 * Plus:
 *   edge_gstr3b_amend_chain    — predecessor → DRC-03 amended return
 *   mv_gstr3b_filed_returns    — finance dashboard / kaikei feed
 *
 * GSTIN goes through `gstin_hash = sha256(GSTIN)` on Tier 1, raw GSTIN
 * + PAN live only in Tier 3 `applicant_payload` (signal:v1: encrypted).
 *
 * Currency: bigint paise.
 *
 * Retention: 6 years (CGST §35(1) + Rule 56), shorter than EPFO/ESIC/ITR-1
 * (7 years).
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    CREATE TABLE vertex_gstr3b_return (
      vertex_id                       varchar PRIMARY KEY,
      _seq                            bigint,
      created_date                    date,
      sensitivity_ord                 int,
      owner_did                       varchar,
      gstin_hash                      varchar NOT NULL,
      tax_period                      varchar NOT NULL,
      process_type                    varchar NOT NULL,
      status                          varchar NOT NULL,
      filing_hash                     varchar NOT NULL,
      total_outward_tax_inr_paise     bigint,
      total_inward_itc_inr_paise      bigint,
      total_net_tax_inr_paise         bigint,
      filing_frequency                varchar,
      arn                             varchar,
      filed_via                       varchar,
      filed_at                        varchar,
      predecessor_vertex_id           varchar,
      amendment_reason                varchar,
      amendment_count                 int,
      bpmn_instance_key               bigint,
      created_at                      varchar,
      org_id                          varchar,
      user_id                         varchar,
      actor_id                        varchar
    )
  `.execute(db);

  await sql`
    CREATE TABLE vertex_gstr3b_return_pii (
      vertex_id              varchar PRIMARY KEY,
      _seq                   bigint,
      created_date           date,
      sensitivity_ord        int,
      owner_did              varchar,
      gstin_hash             varchar NOT NULL,
      applicant_payload      varchar NOT NULL,
      supplies_payload       varchar NOT NULL,
      itc_payload            varchar NOT NULL,
      tax_payment_payload    varchar NOT NULL,
      amendment_log          varchar,
      retention_until        varchar NOT NULL,
      created_at             varchar,
      org_id                 varchar,
      user_id                varchar,
      actor_id               varchar
    )
  `.execute(db);

  await sql`
    CREATE TABLE edge_gstr3b_amend_chain (
      edge_id                varchar PRIMARY KEY,
      _seq                   bigint,
      created_date           date,
      sensitivity_ord        int,
      owner_did              varchar,
      src_vid                varchar NOT NULL,
      dst_vid                varchar NOT NULL,
      amendment_reason       varchar,
      delta_tax_inr_paise    bigint,
      created_at             varchar,
      org_id                 varchar,
      user_id                varchar,
      actor_id               varchar
    )
  `.execute(db);

  await sql`
    CREATE MATERIALIZED VIEW mv_gstr3b_filed_returns AS
    SELECT
      gstin_hash,
      tax_period,
      vertex_id,
      status,
      arn,
      filed_via,
      total_outward_tax_inr_paise,
      total_inward_itc_inr_paise,
      total_net_tax_inr_paise,
      filed_at
    FROM vertex_gstr3b_return
    WHERE status IN ('filed', 'amended')
  `.execute(db);
}

export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_gstr3b_filed_returns`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_gstr3b_amend_chain`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_gstr3b_return_pii`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_gstr3b_return`.execute(db);
}
