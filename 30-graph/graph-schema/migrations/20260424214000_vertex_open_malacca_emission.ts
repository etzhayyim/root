...
/**
 * open-malacca-emission — chokepoint cluster (ADR-0017 Wave 9).
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_open_malacca_imo_compliance`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_malacca_emission_report`.execute(db);
}
