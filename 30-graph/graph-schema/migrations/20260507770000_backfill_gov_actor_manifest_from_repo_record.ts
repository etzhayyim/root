...
// tier: C
// Legacy repo records are grandfathered, but their current graph state should
// be materialized into the typed gov actor manifest vertex table.
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  const repoRecordTable = await sql<{ exists: boolean }>`
    SELECT EXISTS (
      SELECT 1 FROM information_schema.tables
      WHERE table_schema = current_schema()
        AND table_name = 'vertex_repo_record'
    ) AS exists
  `.execute(db);

  if (!repoRecordTable.rows[0]?.exists) {
    return;
  }
...
export async function down(_db: Kysely<unknown>): Promise<void> {
  // No-op: repo-record legacy rows are grandfathered and this migration only
  // materializes equivalent typed graph state.
}
