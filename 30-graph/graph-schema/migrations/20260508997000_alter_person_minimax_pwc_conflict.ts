...
/**
 * Add pwc_conflict_violated axis to vertex_gftdcojp_person_minimax.
 *
...
export async function up(db: Kysely<unknown>): Promise<void> {
  const existing = await sql<{ column_name: string }>`
    ...
    WHERE table_name = 'vertex_gftdcojp_person_minimax'
  `.execute(db);
  const have = new Set(existing.rows.map((r: any) => r.column_name));
  if (!have.has('pwc_conflict_violated')) {
    await sql`ALTER TABLE vertex_gftdcojp_person_minimax ADD COLUMN pwc_conflict_violated boolean DEFAULT false`.execute(db);
  }
  if (!have.has('conflict_scope')) {
    await sql`ALTER TABLE vertex_gftdcojp_person_minimax ADD COLUMN conflict_scope varchar`.execute(db);
  }
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`
    ...
    WHERE spirit_floor_violated = false
  `.execute(db);
  await sql`ALTER TABLE vertex_gftdcojp_person_minimax DROP COLUMN conflict_scope`.execute(db);
  await sql`ALTER TABLE vertex_gftdcojp_person_minimax DROP COLUMN pwc_conflict_violated`.execute(db);
}
