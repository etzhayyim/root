...
/**
 * Migration 0072: legal-entity statement provenance + relation graph.
 *
 * Why:
 *   `edge_legal_entity_trades_with` is too coarse for public-company
 *   relationship evidence such as public-relations, investor-relations,
 *   strategic alliances, cloud-provider arrangements, or revenue-share
 *   disclosures. Those links need explicit provenance.
 *
 * Adds:
 ...
 *
 * Extends:
 *   - mv_legal_entity_disclosure_coverage
 *     adds statement_count and relationship_edge_count
 *     and upgrades disclosure_coverage_score denominator from 5 -> 6
 *     using provenance-backed relationship coverage as its own dimension.
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`DROP TABLE IF EXISTS edge_legal_entity_relates_to`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_legal_entity_mentions`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_public_statement`.execute(db);
}
