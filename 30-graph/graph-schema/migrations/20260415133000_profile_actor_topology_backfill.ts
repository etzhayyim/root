...
/**
 * Backfill actor vertices from profile vertices and expose explicit profile identity edges.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  // 1) Backfill missing actors from profiles (idempotent).
  await sql`
  ...
  // 2) Explicit topology edges as read model (profile -> did / actor).
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_profile_identity_edges`.execute(db);
  await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_profile_identity_edges`.execute(db);
  // Do not delete backfilled actor rows in down() to avoid destructive rollback.
}
