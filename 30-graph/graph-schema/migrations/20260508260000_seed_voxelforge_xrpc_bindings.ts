 ...
 * ``bpmn-dispatcher`` routes ``ai.gftd.apps.voxelforge.*`` to the
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  const rows: Array<{ nsid: string; processId: string; timeoutMs: number }> = [
    { nsid: "ai.gftd.apps.voxelforge.generate", processId: "voxelforge_generate", timeoutMs: 30_000 },
    { nsid: "ai.gftd.apps.voxelforge.getRun", processId: "voxelforge_get_run", timeoutMs: 5_000 },
    { nsid: "ai.gftd.apps.voxelforge.listArtifacts", processId: "voxelforge_list_artifacts", timeoutMs: 5_000 },
    { nsid: "ai.gftd.apps.voxelforge.coverage", processId: "voxelforge_coverage", timeoutMs: 5_000 },
  ];
  for (const r of rows) {
    ...
    await sql`
      ...
      SELECT
        ${vid}, 1, 'did:web:voxelforge.gftd.ai',
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`
    ...
    WHERE nsid IN (
      'ai.gftd.apps.voxelforge.generate',
      'ai.gftd.apps.voxelforge.getRun',
      'ai.gftd.apps.voxelforge.listArtifacts',
      'ai.gftd.apps.voxelforge.coverage'
    )
  `.execute(db);
}
