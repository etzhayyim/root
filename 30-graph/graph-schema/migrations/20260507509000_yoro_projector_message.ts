...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_projector_message_convo_ts ON vertex_projector_message (convo_id, ts_ms)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_projector_message_repo ON vertex_projector_message (repo)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_projector_message_sender ON vertex_projector_message (sender_did)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_projector_message_convo_activity`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_projector_message`.execute(db);
}
