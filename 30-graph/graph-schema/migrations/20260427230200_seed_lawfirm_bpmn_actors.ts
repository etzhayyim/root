...
const seeds: Seed[] = [
  { proc: "runConflictCheck", bpmnProcessId: "lawfirm_run_conflict_check",
    nsid: "ai.gftd.apps.lawfirm.runConflictCheck", resultTimeoutMs: 60000 },
  { proc: "submitFiling",     bpmnProcessId: "lawfirm_submit_filing",
    nsid: "ai.gftd.apps.lawfirm.submitFiling",     resultTimeoutMs: 60000 },
  { proc: "issueInvoice",     bpmnProcessId: "lawfirm_issue_invoice",
    nsid: "ai.gftd.apps.lawfirm.issueInvoice",     resultTimeoutMs: 60000 },
];

const sourcePath = (s: Seed) => `00-contracts/bpmn/ai/gftd/${project}/${s.proc}.bpmn`;
const readContract = (rel: string) => readFileSync(path.resolve(repoRoot, rel), "utf8");
const slug = (proc: string) => proc.replace(/([A-Z])/g, "-$1").toLowerCase();
const processVertexId = (s: Seed) => `at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.processDef/${project}-${slug(s.proc)}-v1`;
const bindingVertexId = (s: Seed) => `at://did:web:bpmn.etzhayyim.com/ai.gftd.apps.bpmn.binding/${project}-${s.proc}-v1`;
