...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_graph_sos_relation_inventory`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_graph_sos_finding_affects_relation`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_graph_sos_intel_finding`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_graph_sos_intel_snapshot`.execute(db);
}
