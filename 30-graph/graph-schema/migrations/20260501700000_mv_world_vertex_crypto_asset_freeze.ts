...
/**
 * Add vertex_crypto_asset_freeze_incident to mv_world_vertex_per_host
 * under a dedicated 'crypto-asset-freeze' app_host, and update the
 * dim_world_domain entry for crypto_asset_freeze to use that host.
 *
 * vertex_crypto_asset_freeze_incident was populated with 18,899 OFAC SDN
 * rows (2026-05-01) but was missing from the MV and had no kaisya_app grant.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  // Grant read access so the MV query can see this table under kaisya_app role.
  await sql`GRANT SELECT ON TABLE vertex_crypto_asset_freeze_incident TO root`.execute(db);
  await sql`GRANT SELECT ON TABLE vertex_crypto_asset_freeze_incident TO kaisya_app`.execute(db);

  // Reroute dim_world_domain so crypto_asset_freeze maps to its own app_host.
  await sql`
    UPDATE dim_world_domain
    SET app_host = 'crypto-asset-freeze'
    WHERE domain = 'crypto_asset_freeze'
  `.execute(db);

  // Rebuild MV to add the new crypto-asset-freeze branch.
  ...
  await sql`
    ...
    GROUP BY app_host
  `.execute(db);
  await sql`SET background_ddl = false`.execute(db);
