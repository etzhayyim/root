...
/**
 * open-hormuz-warrisk — Strait of Hormuz warrisk workflow (ADR-0017 Maritime Cluster).
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_hormuz_premium_by_flag`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_open_hormuz_premium_zone`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_hormuz_warrisk_premium`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_hormuz_warrisk_zone`.execute(db);
}
