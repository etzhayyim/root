...
// JP fiscal procurement pipeline coverage.
// Tracks bid document evidence, award metadata, and bid→contract linkage.
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_jp_fiscal_procurement_pipeline_coverage`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_jp_fiscal_procurement_bid_contract_coverage`.execute(db);
}
