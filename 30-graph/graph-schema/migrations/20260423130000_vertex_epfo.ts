...
/**
 * ADR-0052 — EPFO ECR actor schema.
 *
 * Two-tier split:
 *   vertex_epfo_ecr       — Tier 1 monthly totals and audit metadata
 *   vertex_epfo_ecr_pii   — Tier 3 encrypted roster payloads
 *
 * Plus:
 *   edge_epfo_employer_employee — establishment ↔ member relationship
 *   mv_epfo_active_ecr          — finance dashboard / kaikei feed
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_epfo_active_ecr`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_epfo_employer_employee`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_epfo_ecr_pii`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_epfo_ecr`.execute(db);
}
