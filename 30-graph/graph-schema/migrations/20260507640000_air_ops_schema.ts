...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_air_flight_ops_status`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_ops_notam_location_from`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_ops_notam_id`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_ops_flight_plan_carrier_flight_date`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_air_dispatch_brief_uses_flight_plan`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_ops_tech_log`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_ops_pirep`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_ops_notam`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_ops_dispatch_brief`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_ops_flight_plan`.execute(db);
}
