...
/**
 * open-gas Phase 1 schema (ADR-0056 BPMN-as-actor).
 * Topology-wise parallels open-water but with DOT-aligned leak class (1/2/3).
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_gas_open_leaks`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_open_gas_segment_endpoint`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_gas_leak`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_gas_segment`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_gas_node`.execute(db);
}
