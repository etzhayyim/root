...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_smartphone_patent_free_zone`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_patent_dep`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_patent_pool`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_smartphone_patent_sep`.execute(db);
}
