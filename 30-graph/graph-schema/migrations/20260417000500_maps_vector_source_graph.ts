...
/**
 * Maps vector source graph spine.
 *
 * - Add typed vector source and asset vertices for OSM planet lineage
 * - Seed planet.osm.pbf, minute replication, Geofabrik extract index, and assembled MVT service
 * - Let maps runtime config resolve vector lineage and tile service from graph data
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_vector_source_source_id ON vertex_vector_source (source_id)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_vector_source_slug ON vertex_vector_source (slug)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_vector_source_default_priority ON vertex_vector_source (is_default, priority)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_vector_asset_source_id ON vertex_vector_asset (source_id)`.execute(db);
  await sql`CREATE INDEX IF NOT EXISTS idx_vertex_vector_asset_role ON vertex_vector_asset (asset_role)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP INDEX IF EXISTS idx_vertex_vector_asset_role`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_vector_asset_source_id`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_vector_source_default_priority`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_vector_source_slug`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_vertex_vector_source_source_id`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_vector_asset`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_vector_source`.execute(db);
}
