...
// ADR-2604291500 JP Corporate Finance Ingest process mining.
// Projects generic.audit.emit rows into OCEL-style trace/KPI materialized views.
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_jp_corp_finance_process_case_summary`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_jp_corp_finance_process_kpi`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_jp_corp_finance_process_trace`.execute(db);
}
