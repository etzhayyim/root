...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_air_fleet_dispatch_reliability`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_mro_ad_compliance_ad_no`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_mro_component_part_serial`.execute(db);
  await sql`DROP INDEX IF EXISTS idx_air_mro_work_order_reg_status`.execute(db);
  await sql`DROP TABLE IF EXISTS edge_air_work_order_on_component`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_mro_reliability_report`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_mro_ad_compliance`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_mro_component`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_air_mro_work_order`.execute(db);
}
