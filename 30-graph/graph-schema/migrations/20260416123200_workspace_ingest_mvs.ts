...
/**
 * Workspace ingest rollup MVs.
 * Guardrail: narrow GROUP BY keys, no wide VARCHAR MAX fan-out.
 */
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_workspace_contact_strength`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_workspace_action_queue`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_workspace_activity_daily`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_workspace_sync_lag`.execute(db);
}
