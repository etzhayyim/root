...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS edge_bunken_same_as`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_bunken_record`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_bunken_collection_job`.execute(db);
}
