 ...
 *   did:web:app.gftd.ai
 *   did:web:app.gftd.ai:child
 *   did:web:app.gftd.ai:child:grandchild
 *     -> did:web:app.gftd.ai
 *
...
export async function up(db: Kysely<any>): Promise<void> {
  await sql`CREATE MATERIALIZED VIEW IF NOT EXISTS mv_actor_repo_stats AS
    WITH roots AS (
      SELECT DISTINCT
    ...
    LEFT JOIN repo_rec rr ON rr.actor_did = r.actor_did`.execute(db);
}
