...
/**
 * Migration 20260428183800: open-adnetwork actor schema.
 *
 * Google AdSense/AdWords equivalent open advertising network.
 * 8 vertex tables + 3 streaming MVs.
 * ADR-2604281900.
 */
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // MV 1: Campaign funnel (impressions / clicks / conversions / ctr / cvr / spend)
  await sql`
  ...
  // MV 2: Publisher daily KPI (revenue_snapshot から射影)
  await sql`
  ...
  // MV 3: Market CPM range by ad unit type
  await sql`
...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_adnetwork_market_cpm_range`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_adnetwork_publisher_daily_kpi`.execute(db);
  await sql`DROP MATERIALIZED VIEW IF EXISTS mv_open_adnetwork_campaign_funnel`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_revenue_snapshot`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_conversion`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_click`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_impression`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_ad_unit`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_campaign`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_advertiser`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_open_adnetwork_publisher`.execute(db);
}
