...
export async function up(db: Kysely<unknown>): Promise<void> {
  // vertex_handotai_source — RSS feed source registry for handotai writers
  await sql`
  ...
  // vertex_handotai_article — deduplicated article rows per RSS item
  await sql`
  ...
  // vertex_handotai_digest — daily LLM-generated digest
  await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS vertex_handotai_digest`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_handotai_article`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_handotai_source`.execute(db);
}
