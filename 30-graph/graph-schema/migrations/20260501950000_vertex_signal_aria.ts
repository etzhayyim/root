...
// ARIA protocol — 6 signal vertex tables + 2 streaming MVs.
// Tables capture per-axis entropy state for the A_info area integral:
//   attention (w=1.45) + request (w=1.35) + money (w=1.45)
//   + emotion (w=1.55) + market (w=1.35) + influence (w=1.80)
// mv_signal_entropy: rolling 1-min entropy per signal type
// mv_signal_area_integral: global A_info + eta_global + coverage_grade
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  // ── mv_signal_entropy: rolling 1-min entropy per signal type ────────────
  // Pre-flight: each source table starts at 0 rows — no high-cardinality backfill risk.
  // GROUP BY (signal_type, tick) cardinality bounded by #types(6) × #minutes-in-window.
  await sql`
  ...
  // ── mv_signal_area_integral: rolling A_info integral ────────────────────
  // Downstream of mv_signal_entropy (6 × #ticks rows — low cardinality).
  // coverage_grade thresholds: baseline=1.567, partial=3.0, optimal=4.475 (sum of all w×η at η=1.0 cap).
  await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  // Drop MVs before tables (dependency order)
  ...
  await sql`DROP TABLE IF EXISTS vertex_signal_influence`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_signal_market`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_signal_emotion`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_signal_money`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_signal_request`.execute(db);
  await sql`DROP TABLE IF EXISTS vertex_signal_attention`.execute(db);
}
