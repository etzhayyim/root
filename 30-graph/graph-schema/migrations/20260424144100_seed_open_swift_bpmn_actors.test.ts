...
const migrationSource = readFileSync(
  resolve(__dirname, "20260424144100_seed_open_swift_bpmn_actors.ts"),
  "utf-8",
);
...
describe("Seed open-swift BPMN actors migration", () => {
  ...
  it("BPMN processIds match the seed rows (dispatcher lookup key)", () => {
    expect(bpmn1).toContain('id="open_swift_register_institution"');
    expect(bpmn2).toContain('id="open_swift_send_customer_credit_transfer"');
  });
});
