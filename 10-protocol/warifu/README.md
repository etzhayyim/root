# 10-protocol/warifu — `app.etzhayyim.card.*` lexicons
...
```
Stripe REST  ─┐
ISO 8583     ─┼─►  app.etzhayyim.card.{authorize,capture,settle,refund,dispute}  ─►  kotoba EAVT + ERC-4337
HCE/NFC      ─┘                                                                        (USDC, Base L2)
```

| Lexicon | NSID | Maps to compat |
|---|---|---|
| issue | `app.etzhayyim.card.issue` | (provisioning; HCE) |
| authorize | `app.etzhayyim.card.authorize` | REST `payment_intents` / ISO 8583 `0100→0110` / NFC tap |
| capture | `app.etzhayyim.card.capture` | REST capture |
| settle | `app.etzhayyim.card.settle` | on-chain settlement (T+0) |
| refund | `app.etzhayyim.card.refund` | REST `refunds` / ISO 8583 `0400` |
| dispute | `app.etzhayyim.card.dispute` | chargeback → chigiri |
