...
> task. No section is implemented in this directory — yatachain *is* the
...
Each `Agent` has exactly one `yatachain-chain` = an atproto PDS repo MST:
...
A new record kind is yatachain-mountable when (L1, L2, L3) are all populated and
...
## §5 yatachain-witnesses

**Status**: scaffold shipped 2026-05-23 at `20-actors/etzhayyim-sdk/src/yatachain/`
...
Default quorum is 3-of-5; configurable per-NSID in `00-contracts/policies/yatachain-quorum.rego`.
...
A token is yatachain-valid iff:
...
zome registry for yatachain. Adding a yatachain-cell = adding a row to
...
**projection** (derived read-path cache, per [ADR-2605231500](../../90-docs/adr/2605231500-yatachain-projection.md)).
...
The current `60-apps/ai-gftd-project-open-isic/rw-free/` reference implementation
...
from `yatachain-chain + yatachain-dht`** and is **never the only place a write
lives**. Per [ADR-2605231500](../../90-docs/adr/2605231500-yatachain-projection.md):

| Level | Requirements |
|---|---|
| **L0-projection nominal** | Rebuildable in principle; marked with `// yatachain-projection` line comment or `yatachain-projection.toml` manifest; documented rebuild runbook |
...
etc., per [`MIGRATION-TODO.md`](../../60-apps/ai-gftd-project-maps/MIGRATION-TODO.md))
